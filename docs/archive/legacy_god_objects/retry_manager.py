...
# OpenTelemetry conditional imports with proper type annotations
...
try:
    ...
    OPENTELEMETRY_AVAILABLE = True  # type: ignore[misc] # Pylance false positive on conditional import pattern
except ImportError:
    pass
...
# Pydantic models for validated retry options
class RetryConfigOverrides(BaseModel):
    """Validated retry configuration overrides with constraints."""

    max_attempts: int = Field(
        default=3, ge=1, le=100, description="Maximum retry attempts"
    )
    ...
    max_delay: float = Field(
        default=60.0, ge=1.0, le=3600.0, description="Maximum delay in seconds"
    )
    jitter: bool = Field(default=True, description="Enable jitter")
    jitter_factor: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Jitter factor"
    )
    backoff_multiplier: float = Field(
        default=2.0, ge=1.0, le=10.0, description="Backoff multiplier"
    )
    operation_timeout: float = Field(
        default=30.0, ge=1.0, le=600.0, description="Operation timeout"
    )
    total_timeout: float = Field(
        default=300.0, ge=1.0, le=3600.0, description="Total timeout"
    )
    log_attempts: bool = Field(default=True, description="Log retry attempts")
    log_level: str = Field(default="INFO", description="Log level")
    track_metrics: bool = Field(default=True, description="Track metrics")
    enable_circuit_breaker: bool = Field(
        default=False, description="Enable circuit breaker"
    )
    failure_threshold: int = Field(
        default=5, ge=1, le=100, description="Failure threshold"
    )
    success_threshold: int = Field(
        default=5, ge=1, le=100, description="Success threshold"
    )
    recovery_timeout_seconds: float = Field(
        default=60.0, ge=1.0, le=3600.0, description="Recovery timeout"
    )
    half_open_max_calls: int = Field(
        default=3, ge=1, le=50, description="Half-open max calls"
    )
    operation_name: str = Field(default="unknown", description="Operation name")
    resource_aware: bool = Field(default=False, description="Resource aware retries")
    adaptive_delays: bool = Field(
        default=False, description="Adaptive delay adjustment"
    )

    @field_validator("max_delay")
    @classmethod
    def max_delay_greater_than_base(cls, v: float, info: ValidationInfo) -> float:
    ...
    @classmethod
    def total_timeout_reasonable(cls, v: float, info: ValidationInfo) -> float:
        """Ensure total_timeout allows for reasonable retry attempts."""
        if (
            hasattr(info, "data")
            and "operation_timeout" in info.data
            and v <= info.data["operation_timeout"]
        ):
            raise ValueError("total_timeout must be greater than operation_timeout")
        return v
...
class StrategyOptions(BaseModel):
    """Validated strategy-specific options."""
...
class RetryDecoratorOptions(BaseModel):
    """Validated retry decorator options."""

    enable_circuit_breaker: bool = Field(
        default=False, description="Enable circuit breaker"
    )
    operation_name: str = Field(default="unknown", description="Operation name")
    max_attempts: int = Field(default=3, ge=1, le=100, description="Maximum attempts")
    strategy: RetryStrategy = Field(
        default=RetryStrategy.EXPONENTIAL_BACKOFF, description="Retry strategy"
    )
    base_delay: float = Field(default=1.0, ge=0.1, le=60.0, description="Base delay")
    max_delay: float = Field(
        default=60.0, ge=1.0, le=3600.0, description="Maximum delay"
    )
...
class CircuitBreaker:
    ...
    async def call(
        self, operation: Callable[[], Coroutine[Any, Any, T]], context: RetryContext
    ) -> T:
        ...
        if self.state_data.state == CircuitBreakerState.OPEN:
            ...
            raise CircuitBreakerOpenError(
                f"Circuit breaker is OPEN for {self.operation_name}"
            )
        try:
    ...
    async def _check_state_transition(self):
        ...
        async with self._lock:
            if (
                ...
                and self.state_data.last_failure_time
                and (
                    datetime.now(UTC) - self.state_data.last_failure_time
                ...
                logger.info(
                    f"Circuit breaker transitioning to HALF_OPEN for {self.operation_name}"
                )
        ...
        async with self._lock:
            ...
            if self.state_data.state == CircuitBreakerState.HALF_OPEN:
                ...
                if self.state_data.success_count >= self.config.success_threshold:
                    ...
                    logger.info(
                        f"Circuit breaker transitioning to CLOSED for {self.operation_name}"
                    )
    ...
    async def _on_failure(self, context: RetryContext, error: Exception):
        ...
        async with self._lock:
            ...
            if (
                self.state_data.state == CircuitBreakerState.CLOSED
                and self.state_data.failure_count >= self.config.failure_threshold
            ):
                self.state_data.state = CircuitBreakerState.OPEN
                logger.warning(
                    f"Circuit breaker transitioning to OPEN for {self.operation_name}"
                )
                self._record_event("opened")
            elif self.state_data.state == CircuitBreakerState.HALF_OPEN:
                self.state_data.state = CircuitBreakerState.OPEN
                logger.warning(
                    f"Circuit breaker returning to OPEN from HALF_OPEN for {self.operation_name}"
                )
                self._record_event("opened")
            self._update_state_metric()
    ...
    def reset(self) -> None:
        ...
        if task_manager:
            # Use a simple approach - try to get the real priority or use fallback
            try:
                from prompt_improver.performance.monitoring.health.background_manager import (
                    TaskPriority,
                )

                priority = cast("Any", TaskPriority.HIGH)
            except ImportError:
                priority = cast("Any", 2)  # HIGH priority fallback value
...
class UnifiedSystemService:
    ...
    - CircuitBreakerManager (circuit breaker patterns)
    - RateLimitManager (traffic control)
    - SystemHealthManager (health monitoring)
    ...
    - Support for both sync and async operations
    - OpenTelemetry integration for distributed tracing
    - Resource-aware policies and intelligent classification
    - Database-specific and ML orchestration optimizations
    - Unified system resilience patterns following 2025 best practices
    """
    ...
    def _setup_tracing(self):
        ...
        if OPENTELEMETRY_AVAILABLE and trace is not None and otel_metrics is not None:
    ...
    async def execute_with_retry(
        ...
        **kwargs: Any,
        ...
        operation_name = retry_config.operation_name or getattr(
            operation, "__name__", "unknown"
        )
        context = RetryContext(operation_name=operation_name)
        if asyncio.iscoroutinefunction(operation):
            return await self._execute_async_with_retry(
                operation, retry_config, context, *args, **kwargs
            )
        return await self._execute_sync_with_retry(
            operation, retry_config, context, *args, **kwargs
        )
        ...
        **kwargs: Any,
        ...
        if asyncio.iscoroutinefunction(operation):
            return await self.retry_async(
                operation, db_config, operation_name, **kwargs
            )
    ...
    async def retry_ml_operation(
        ...
        **kwargs: Any,
        ...
        ml_config = RetryConfig(
            ...
            max_delay=60.0,
            operation_name=operation_name,
            retry_on_exceptions=[ConnectionError, TimeoutError, RuntimeError],
            failure_threshold=2,
        ...
        if asyncio.iscoroutinefunction(operation):
            return await self.retry_async(
                operation, ml_config, operation_name, **kwargs
            )
    ...
    def _generate_recommendations(self, stats: dict[str, Any]) -> list[str]:
        ...
        if success_rate < 0.5:
            recommendations.append(
                "Consider investigating root cause of high failure rate"
            )
        ...
        if failed_attempts > successful_attempts:
            recommendations.append(
                "Operation appears to be failing more than succeeding"
            )
                ...
                try:
                    ...
                    if config.enable_circuit_breaker:
                        result = await circuit_breaker.call(
                            lambda: operation(**kwargs), context
                        )
                    else:
                        result = await operation(**kwargs)
                    duration_ms = (time.perf_counter() - start_time) * 1000
                    await self._record_success(
                        context.operation_name, attempt + 1, duration_ms, config
                    )
                    for observer in self._observers:
                    ...
                    if not self._is_retryable_error(error_type, config):
                        await self._record_failure(
                            context.operation_name, attempt + 1, str(e), config
                        )
                        raise
                    await self._record_failure(
                        context.operation_name, attempt + 1, str(e), config
                    ...
                    if attempt >= config.max_attempts - 1:
                        for observer in self._observers:
                            observer.on_retry_failure(
                                context.operation_name, attempt + 1, e
                            )
                        raise
                    delay = config.calculate_delay(attempt)
                    context.total_delay_ms += delay * 1000
                    self.logger.debug(
                        f"Retry {attempt + 1}/{config.max_attempts} for {context.operation_name} in {delay:.2f}s due to {error_type.value}: {e}"
                    )
    ...
    async def _get_circuit_breaker(
        self, operation_name: str, config: RetryConfig
    ...
    def _classify_error(
        self, error: Exception, config: RetryConfig
    ) -> RetryableErrorType:
    ...
    async def _record_success(
        ...
        async with self._stats_lock:
            ...
            if stats["total_attempts"] > 0:
                stats["average_delay_ms"] = (
                    stats["total_duration_ms"] / stats["total_attempts"]
                )
        self.retry_success_total.labels(
            operation=operation_name, strategy=config.strategy.value
        ).inc()
    ...
    def _start_span(self, operation_name: str, config: RetryConfig):
        ...
        span.set_attribute(
            "retry.circuit_breaker_enabled", config.enable_circuit_breaker
        )
        return span

    def _end_span(self, span: Any, context: RetryContext) -> None:
        ...
        span.set_attribute(
            "retry.circuit_breaker_triggered", context.circuit_breaker_triggered
        )
        if context.last_error and Status is not None and StatusCode is not None:
...
class NoOpCircuitBreaker:
    ...
    async def call(
        self, operation: Callable[[], Coroutine[Any, Any, T]], _context: RetryContext
    ) -> T:
...
async def retry_ml_operation(
    operation: Callable[..., Any], operation_name: str = "ml_operation", **kwargs: Any
) -> Any:
    ...
    def decorator(
        ...
        async def wrapper(*args: Any, **func_kwargs: Any) -> T:
            ...
            return await retry_manager.retry_async(
                lambda: func(*args, **func_kwargs), config=config
            )
