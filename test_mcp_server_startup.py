...
async def test_mcp_server_startup() -> Dict[str, Any]:
    ...
    try:
        ...
        # Test 3: Basic tools (non-database)
        ...
        try:
            ...
            session_working = (
                set_result.get('success', False) and 
                get_result.get('exists', False) and
                touch_result.get('success', False) and
        ...
        # Ready health check (may timeout due to database)
        try:
            # Use shorter timeout for database-dependent operations
        ...
        # Test 5: Database operations (with timeout)
        ...
        # Test 6: Prompt improvement (with safe prompt)
        print("\n6. Testing prompt improvement...")
        try:
            prompt_task = asyncio.create_task(server._improve_prompt_impl(
...
def main():
    ...
    total_tests = (
        1 +  # server_instantiation
        1 +  # server_initialization
    ...
    passed_tests = (
        ...
        sum(1 for status in results['basic_tools'].values() if status in [True, 'security_blocked']) +
