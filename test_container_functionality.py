...
async def test_container_basic_functionality():
    ...
    assert service is service2  # Should be the same instance
...
async def test_ml_service_registration():
    ...
    # Test that we can resolve all ML interfaces
    ...
    # Test that all services are the same instance (unified service)
...
async def test_ml_service_functionality():
    ...
    # Test resource usage (health interface)
    try:
...
async def test_container_health_check():
    ...
    # Resolve a service to initialize it
