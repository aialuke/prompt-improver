...
# Note: workflow integration handled separately to avoid circular imports
    ...
    def post_task_hook(self, delegation_id: str, agent_response: str,
                      outcome: str = "success", execution_time: float | None = None) -> dict[str, Any]:
    ...
    def create_task_delegation_context(self, subagent_type: str, task_description: str,
        ...
        try:
            ...
            if relevant_insights:
                ...
                # Top 3 most relevant
                context_parts.extend(f"• {insight.get('insight', 'Unknown insight')} (confidence: {insight.get('confidence', 0) * 100:.0f}%)" for insight in relevant_insights[:3])
            ...
            if effective_collabs:
                ...
                # Top 2 collaborators
                context_parts.extend(f"• Consider delegating to {collab['agent_name']} (proven {collab['success_rate'] * 100:.0f}% success rate)" for collab in effective_collabs[:2])
    ...
    def _get_delegation_recommendations(self, agent_type: str, context: dict[str, Any]) -> list[str]:
        ...
        collabs = context.get("collaboration_patterns", [])
        recommendations = [f"Leverage {collab['agent_name']} collaboration (proven {collab['success_rate'] * 100:.0f}% success)" for collab in collabs if collab.get("success_rate", 0) > 0.85]
        ...
        recommendations.extend(f"Focus on {area} - historically high impact area" for area in list(high_impact_areas)[:2])
...
if __name__ == "__main__":
    ...
    if pre_result["success"]:
        ...
        if post_result["success"]:
            print("✅ Post-task hook successful")
