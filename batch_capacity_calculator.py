...
class BatchCapacityCalculator:
    ...
    def calculate_throughput_capacity(self) -> CapacityEstimate:
        ...
        # Bottleneck analysis
        bottleneck_factors = {
            "ML_INFERENCE": actual_batch_time_ms / (processing_time_ms * prompts_per_batch),
            "CONCURRENCY": concurrent_tests / 10,  # Assuming 10 is optimal
            "BATCH_SIZE": prompts_per_batch / 20,  # Assuming 20 is optimal
        }
        
        primary_bottleneck = min(bottleneck_factors.keys(), 
                               key=lambda k: bottleneck_factors[k])
    ...
    def calculate_sample_size_requirements(self, effect_size: float = None) -> SampleSizeEstimate:
        ...
        final_sample_size = max(sequential_sample_size, 
                              self.stats_config.minimum_sample_size * 2)
    ...
    def estimate_test_duration(self, capacity: CapacityEstimate, 
                             sample_size: SampleSizeEstimate) -> float:
        ...
        total_duration = max(duration_days + buffer_days, 3)  # Minimum 3 days
        
        return total_duration
    
    def _generate_capacity_recommendations(self, hourly_throughput: float, 
    ...
    def generate_capacity_report(self) -> Dict:
        ...
        for effect_size, estimate in sample_estimates.items():
            report["sample_size_analysis"][f"effect_size_{effect_size}"] = {
                ...
                "group_sequential_inflation": estimate.group_sequential_params["inflation_factor"]
            }
...
def main():
    ...
    parser.add_argument("--concurrent-tests", type=int, default=5,
                       help="Max concurrent tests (default: 5)")
    parser.add_argument("--processing-time", type=float, default=500,
                       help="Processing time per prompt in ms (default: 500)")
    parser.add_argument("--effect-size", type=float, default=0.1,
