...
def downgrade() -> None:
    ...
    op.execute(
        "DELETE FROM rule_metadata WHERE rule_id IN ('clarity_enhancement', 'specificity_enhancement', 'chain_of_thought', 'few_shot_examples', 'role_based_prompting', 'xml_structure_enhancement')"
    )

    # Note: We won't restore the old incorrect rules as they were buggy
    print(
        "✅ Removed corrected rules - database is now empty for manual restoration if needed"
    )
