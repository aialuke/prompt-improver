...
class GenerationHistoryTracker:
    ...
    async def start_tracking_session(
        ...
        session = await self.db_service.create_generation_session(
        ...
        logger.info(f"Started tracking generation session {session.session_id}")
        return session.session_id

    async def track_batch_completion(
        self,
        session_id: str,
        batch_number: int,
        batch_size: int,
        ...
        # Create batch record
        batch = await self.db_service.create_generation_batch(
            session_id=session_id,
            batch_number=batch_number,
            batch_size=batch_size,
            generation_method=generation_method,
            samples_requested=batch_size
        )

        # Update with performance metrics
        await self.db_service.update_batch_performance(
            batch_id=batch.batch_id,
            processing_time_seconds=processing_time,
            samples_generated=samples_generated,
            samples_filtered=samples_filtered,
            error_count=error_count,
            memory_usage_mb=performance_metrics.get("memory_usage_mb") if performance_metrics else None,
            memory_peak_mb=performance_metrics.get("memory_peak_mb") if performance_metrics else None,
            efficiency_score=performance_metrics.get("efficiency_score") if performance_metrics else None,
            average_quality_score=quality_metrics.get("average_quality") if quality_metrics else None,
            diversity_score=quality_metrics.get("diversity_score") if quality_metrics else None
        )

        logger.info(f"Tracked batch {batch_number} completion for session {session_id}")
    ...
    async def complete_session(
        ...
        await self.db_service.update_session_status(
            session_id=session_id,
            status=status,
        ...
        logger.info(f"Completed tracking for session {session_id} with {final_sample_count} samples")
    ...
    async def record_method_performance(
        ...
        await self.db_service.record_method_performance(
            ...
            generation_time_seconds=performance_metrics.get("generation_time", 0.0),
            quality_score=performance_metrics.get("quality_score", 0.0),
            diversity_score=performance_metrics.get("diversity_score", 0.0),
            memory_usage_mb=performance_metrics.get("memory_usage_mb", 0.0),
            success_rate=performance_metrics.get("success_rate", 0.0),
            samples_generated=performance_metrics.get("samples_generated", 0),
            performance_gaps_addressed=performance_metrics.get("performance_gaps_addressed"),
            batch_size=performance_metrics.get("batch_size"),
            configuration=performance_metrics.get("configuration")
        )
...
class GenerationAnalytics:
    ...
    async def get_performance_trends(
        ...
        if method_name:
            performance_history = await self.db_service.get_method_performance_history(
                method_name=method_name,
                days_back=days_back
            )
        else:
            # Get all methods performance
            performance_history = []
            for method in ["statistical", "neural", "hybrid", "diffusion"]:
                method_history = await self.db_service.get_method_performance_history(
                    method_name=method,
                    days_back=days_back
                )
        ...
        # Calculate trends (correlation with time)
        timestamps = [(p.recorded_at - performance_history[0].recorded_at).total_seconds()
        ...
        efficiency_trend = self._calculate_trend(timestamps, [-t for t in generation_times])  # Negative for efficiency
        ...
        return {
            ...
            "performance_ranges": {
                ...
                "success_range": [min(success_rates), max(success_rates)]
            }
        }
    ...
    async def get_method_comparison(
        ...
        for method in methods:
            ...
            if performance_history:
                ...
                method_stats[method] = {
                    ...
                    "avg_generation_time": np.mean(generation_times),
                    "total_samples": sum(samples_generated),
                    "efficiency_score": sum(samples_generated) / sum(generation_times) if sum(generation_times) > 0 else 0
                }
            else:
                method_stats[method] = {
                    "executions": 0,
        ...
        for method, stats in method_stats.items():
            if stats["executions"] > 0:
                ...
                performance_score = (
                    ...
                    0.2 * stats["avg_diversity"] +
                    0.2 * stats["avg_success_rate"] +
                    0.3 * min(1.0, stats["efficiency_score"] / 100.0)  # Normalize efficiency
                )
        ...
        return {
            ...
            "method_rankings": dict(ranked_methods),
            "best_method": ranked_methods[0][0] if ranked_methods else None,
            "worst_method": ranked_methods[-1][0] if ranked_methods else None,
    ...
    async def get_effectiveness_report(
        ...
        if stats["total_sessions"] > 0:
            effectiveness_score = (
                0.4 * stats["average_quality_score"] +
                0.3 * stats["average_efficiency"] +
                0.3 * (stats["total_samples_generated"] / (stats["total_sessions"] * 1000))  # Normalize by expected samples
            )
        ...
        return {
            ...
            "method_performance": method_comparison,
            "performance_trends": trends,
            "recommendations": self._generate_recommendations(stats, method_comparison, trends)
        }
