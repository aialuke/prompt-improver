...
class ImprovementCalculation(BaseModel):
...
class PlateauDetection(BaseModel):
    """Plateau detection result"""
    status: PlateauStatus = Field(description='Current plateau status')
    plateau_start_iteration: int | None = Field(default=None, ge=0, description='Iteration where plateau began')
...
class PerformanceImprovementCalculator:
    ...
    def _extract_performance_metrics(self, iteration: TrainingIteration) -> dict[str, float] | None:
    ...
    def _find_plateau_start(self, improvement_history: list[float]) -> int | None:
