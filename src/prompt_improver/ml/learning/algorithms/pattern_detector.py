...
class PatternDetector:
    ...
    async def identify_failure_patterns(self, failures: list[dict[str, Any]]) -> list[dict[str, Any]]:
                ...
                for cluster_id in set(clusters):
                    if cluster_id != -1:
                        ...
                        if len(cluster_indices) >= self.config.min_pattern_size:
                            ...
                            patterns.append({'pattern_id': f'prompt_cluster_{cluster_id}', 'type': 'prompt_content', 'description': f"Failures with similar content: {', '.join(common_terms[:3])}", 'frequency': len(cluster_indices), 'severity': 0.5, 'characteristics': {'common_terms': common_terms, 'cluster_size': len(cluster_indices)}, 'examples': [failures[i] for i in cluster_indices[:3]]})
            except Exception as e:
                self.logger.warning('Text clustering failed: %s', e)
        return patterns
        ...
        if time_deltas:
            ...
            for i, burst in enumerate(bursts):
                if len(burst) >= self.config.min_pattern_size:
                    ...
                    patterns.append({'pattern_id': f'temporal_burst_{i}', 'type': 'temporal', 'description': f'Burst of {len(burst)} failures over {duration:.1f} seconds', 'frequency': len(burst), 'severity': 0.6, 'characteristics': {'start_time': start_time.isoformat(), 'end_time': end_time.isoformat(), 'duration_seconds': duration, 'failures_per_second': len(burst) / max(duration, 1)}, 'examples': burst_failures[:3]})
        return patterns

    async def identify_root_causes_comparative(self, failures: list[dict[str, Any]], successes: list[dict[str, Any]]) -> list[dict[str, Any]]:
                    ...
                    if self._is_numeric_characteristic(failure_values):
                        ...
                        for category in failure_counts:
                            ...
                            if failure_rate > success_rate * 2 and failure_counts[category] >= 3:
                                root_causes.append({'cause_id': f'categorical_{characteristic}_{category}', 'type': 'overrepresented_category', 'description': f'{category} in {characteristic} overrepresented in failures', 'affected_failures': failure_counts[category], 'correlation_strength': failure_rate / (success_rate + 0.01), 'evidence': [f'Failure rate: {failure_rate:.3f}', f'Success rate: {success_rate:.3f}', f'Overrepresentation: {failure_rate / (success_rate + 0.01):.1f}x'], 'category': category})
                except Exception as e:
                    self.logger.warning('Statistical comparison failed for %s: %s', characteristic, e)
        root_causes.sort(key=lambda x: x['correlation_strength'], reverse=True)
        return root_causes[:self.config.max_root_causes]

    async def find_missing_rules(self, failures: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for context_key, gap_failures in context_gaps.items():
            if len(gap_failures) >= self.config.min_pattern_size:
                ...
                rule_gaps.append({'gap_id': f'rule_gap_{hash(context_key) % 10000}', 'context': context_key, 'affected_failures': len(gap_failures), 'description': f'Insufficient rule coverage for {context_key}', 'common_issues': common_issues, 'suggested_rule_types': self._suggest_rule_types(gap_failures), 'priority': 'high' if len(gap_failures) > 10 else 'medium'})
        return rule_gaps

    async def find_edge_cases(self, failures: list[dict[str, Any]]) -> list[dict[str, Any]]:
