...
class FailureConfig:
    ...
    # Phase 3 enhancements - OpenTelemetry Monitoring
    enable_prometheus_monitoring: bool = True  # Legacy name for compatibility
    prometheus_port: int = 8000  # Legacy name for compatibility
...
class FailureModeAnalyzer:
    ...
    def __init__(self, config: FailureConfig | None = None, training_loader=None):
        ...
        self.logger.info("Failure analyzer integrated with training data pipeline")
        ...
        # Phase 3 enhancements - OpenTelemetry monitoring state
        ...
        try:
            ...
            result = {
                "failure_patterns": [
                    {
                        ...
                        "examples": pattern.examples[:5]  # Limit examples for orchestrator
                    }
                ...
                "root_causes": [
                    {
                        ...
                        "impact": cause.impact,
                        "evidence": cause.evidence[:3]  # Limit evidence for orchestrator
                    }
                ...
                "summary": {
                    ...
                    "patterns_identified": len(analysis_result.failure_patterns),
                    "critical_patterns": len([p for p in analysis_result.failure_patterns if p.severity == "critical"]),
            ...
            self.logger.error(f"Orchestrated failure analysis failed: {e}")
            return {
                "orchestrator_compatible": True,
                "component_result": {"error": str(e), "failure_patterns": [], "root_causes": []},
                "local_metadata": {
                    "execution_time": (datetime.now() - start_time).total_seconds(),
                    "error": True,
                    "component_version": "1.0.0"
    ...
    async def analyze_historical_failures(self, db_session) -> dict[str, Any]:
        ...
        try:
            ...
            # Extract failure features for anomaly detection
            failure_features = await self._extract_failure_features(failure_data)
            
            # Train anomaly detection model
            self.anomaly_model = await self._train_isolation_forest(failure_features)
            
            # Identify failure patterns through clustering
            self.failure_patterns = await self._cluster_failure_modes(failure_features, failure_data)
            
            # Analyze failure trends and root causes
            failure_trends = await self._analyze_failure_trends(failure_data)
            root_causes = await self._identify_root_causes(failure_data)
            
            result = {
                "status": "success",
                "total_training_samples": training_data["metadata"]["total_samples"],
                "failure_cases": len(failure_data["features"]),
                "failure_rate": len(failure_data["features"]) / training_data["metadata"]["total_samples"],
                "detected_patterns": len(self.failure_patterns),
                "anomaly_model_trained": self.anomaly_model is not None,
                "failure_trends": failure_trends,
                "root_causes": root_causes,
                "feature_dimensions": failure_features.shape[1] if failure_features.size > 0 else 0
            }
            
            self.logger.info(f"Historical failure analysis completed. Found {len(self.failure_patterns)} failure patterns")
            return result
            
        except Exception as e:
            self.logger.error(f"Failed to analyze historical failures: {e}")
            return {"status": "error", "error": str(e)}

    async def predict_failure_probability(self, prompt_features: np.ndarray) -> dict[str, Any]:
        """Predict probability of optimization failure for given prompt features
        
        Uses trained anomaly detection model to predict likelihood of failure
        and provides similar historical failure cases for context.
        """
        try:
            if not hasattr(self, 'anomaly_model') or self.anomaly_model is None:
                self.logger.warning("Anomaly model not trained, cannot predict failure probability")
                return {"status": "model_not_trained", "probability": 0.5}
            
            # Ensure features are in correct format
            if prompt_features.ndim == 1:
                prompt_features = prompt_features.reshape(1, -1)
            
            # Calculate anomaly score
            anomaly_score = self.anomaly_model.decision_function(prompt_features)[0]
            
            # Convert to probability (0 = certain failure, 1 = certain success)
            failure_prob = self._score_to_probability(anomaly_score)
            
            # Find similar failures for context
            ...
            result = {
                ...
                "recommendations": self._generate_failure_recommendations(failure_prob, similar_failures)
            }
            
            self.logger.debug(f"Failure probability prediction: {failure_prob:.3f} (risk: {risk_level})")
            return result
            
        except Exception as e:
            self.logger.error(f"Failed to predict failure probability: {e}")
            return {"status": "error", "error": str(e), "probability": 0.5}
    ...
    async def _extract_failure_cases(self, training_data: dict) -> dict:
        ...
        # In a real implementation, this would filter based on actual failure criteria
        # For now, we'll simulate by taking a subset as "failures"
        failure_indices = []
        failure_features = []
        failure_labels = []
        
        # Simple heuristic: assume bottom 20% of performance are failures
        if labels and len(labels) > 0:
            sorted_indices = np.argsort(labels)
            failure_count = max(1, len(labels) // 5)  # Bottom 20%
            failure_indices = sorted_indices[:failure_count]
            
            failure_features = [features[i] for i in failure_indices]
            failure_labels = [labels[i] for i in failure_indices]
    ...
    async def _extract_failure_features(self, failure_data: dict) -> np.ndarray:
        """Extract feature vectors from failure data for anomaly detection"""
        features = failure_data.get("features", [])
        
        if not features:
            return np.array([])
        
        # Convert to numpy array and handle different input formats
        try:
            ...
            self.logger.info(f"Extracted {feature_matrix.shape[0]} failure feature vectors of dimension {feature_matrix.shape[1]}")
            return feature_matrix
            
        except Exception as e:
            self.logger.error(f"Failed to extract failure features: {e}")
            return np.array([])

    async def _train_isolation_forest(self, failure_features: np.ndarray) -> Any:
        """Train Isolation Forest for anomaly detection on failure patterns"""
        try:
            if not ANOMALY_DETECTION_AVAILABLE:
                self.logger.warning("Anomaly detection not available, skipping model training")
                return None
            
            if failure_features.size == 0:
                self.logger.warning("No failure features available for training")
                return None
            ...
            isolation_forest.fit(failure_features)
            
            self.logger.info("Isolation Forest trained successfully for failure detection")
    ...
    async def _cluster_failure_modes(self, failure_features: np.ndarray, failure_data: dict) -> dict:
        """Cluster failure modes to identify distinct failure patterns"""
        try:
            if failure_features.size == 0:
                return {}
            
            # Use DBSCAN for clustering failure modes
            dbscan = DBSCAN(eps=0.5, min_samples=2)
            cluster_labels = dbscan.fit_predict(failure_features)
            ...
            for label in unique_labels:
                ...
                pattern_id = f"failure_pattern_{label}"
                failure_patterns[pattern_id] = FailurePattern(
                    pattern_id=pattern_id,
                    frequency=len(cluster_indices),
                    severity=0.5,  # Default severity as float
                    description=f"Failure pattern {label} with {len(cluster_indices)} cases",
                    affected_contexts=[f"context_{i}" for i in cluster_indices[:3]],
                    common_characteristics={"cluster_label": int(label), "confidence": 0.8},
                    example_cases=[{"index": i, "label": int(label)} for i in cluster_indices[:5]]
                )
            
            self.logger.info(f"Identified {len(failure_patterns)} distinct failure patterns")
            return failure_patterns
            
        except Exception as e:
            self.logger.error(f"Failed to cluster failure modes: {e}")
            return {}

    async def _analyze_failure_trends(self, failure_data: dict) -> dict:
        """Analyze trends in failure patterns over time"""
    ...
    async def _identify_root_causes(self, failure_data: dict) -> list[dict]:
        """Identify root causes of failures through pattern analysis"""
        # Placeholder for root cause analysis
        return [
            {
                "cause": "high_complexity_prompts",
                "frequency": 0.4,
                "severity": "medium",
                "description": "Failures often occur with highly complex prompts"
            },
            {
                "cause": "insufficient_context",
                "frequency": 0.3,
                "severity": "high",
                "description": "Lack of sufficient context leads to optimization failures"
            }
        ]
    ...
    def _score_to_probability(self, anomaly_score: float) -> float:
        ...
        # Convert to probability where 0 = certain failure, 1 = certain success
        return max(0.0, min(1.0, (anomaly_score + 0.5) * 2))
    ...
    async def _get_similar_failures(self, prompt_features: np.ndarray) -> list[dict]:
        ...
        return [
            {
                "similarity": 0.85,
                "description": "Complex technical prompt with domain-specific terms",
                "failure_reason": "insufficient_domain_knowledge"
            },
            {
                "similarity": 0.72,
                "description": "Long prompt with multiple requirements",
    ...
    def _generate_failure_recommendations(self, failure_prob: float, similar_failures: list) -> list[str]:
        """Generate recommendations based on failure probability and similar cases"""
        recommendations = []
        
        if failure_prob > 0.7:
            recommendations.extend([
                "Consider simplifying the prompt structure",
            ...
            recommendations.extend([
                ...
                "Consider adding relevant context"
            ])
        else:
            recommendations.append("Prompt appears to have low failure risk")
    ...
    async def analyze_failures(
        ...
        failure_analysis = {
            ...
            "root_causes": await self._identify_root_causes_comparative(failures, successes),
        ...
        if self.config.enable_prometheus_monitoring:
            await self._update_otel_metrics(failure_analysis)
            failure_analysis[
                "otel_alerts"
    ...
    async def _identify_root_causes_comparative(
    ...
    def _initialize_otel_monitoring(self) -> None:
        ...
        try:
            ...
            # Store reference for legacy code compatibility
    ...
    def _initialize_alert_definitions(self) -> None:
        """Initialize alert definitions for OpenTelemetry monitoring"""
        # Alert definitions are now handled by MLAlertingMetrics
        # Store reference to alerting metrics for compatibility
        if hasattr(self, 'otel_alerting_metrics'):
            self.alert_definitions = self.otel_alerting_metrics.alert_definitions
            self.logger.info(f"Initialized {len(self.alert_definitions)} OpenTelemetry alert definitions")
        else:
            self.alert_definitions = []
            self.logger.warning("OpenTelemetry alerting metrics not available")
