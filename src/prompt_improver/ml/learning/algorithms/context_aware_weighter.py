...
class ContextAwareFeatureWeighter:
    ...
    def calculate_feature_weights(self, domain_features: DomainFeatures, feature_names: tuple[str]) -> np.ndarray:
        ...
        self.logger.debug('Calculated weights for %s: mean=%s, std=%s', domain_features.domain.value, format(np.mean(weights), '.3f'), format(np.std(weights), '.3f'))
        if self.config.enable_weight_caching:
            cache_key = self._create_cache_key(domain_features, feature_names)
    ...
    def _apply_static_weights(self, base_weights: dict[str, float], feature_names: tuple[str]) -> np.ndarray:
    ...
    def _apply_adaptive_weights(self, base_weights: dict[str, float], domain_features: DomainFeatures, feature_names: tuple[str]) -> np.ndarray:
    ...
    def _apply_dynamic_weights(self, base_weights: dict[str, float], domain_features: DomainFeatures, feature_names: tuple[str]) -> np.ndarray:
    ...
    def _apply_hybrid_weights(self, base_weights: dict[str, float], domain_features: DomainFeatures, feature_names: tuple[str]) -> np.ndarray:
    ...
    def _post_process_weights(self, weights: np.ndarray) -> np.ndarray:
        ...
        weights = np.clip(weights, self.config.min_weight_threshold, self.config.max_weight_threshold)
    ...
    def _create_cache_key(self, domain_features: DomainFeatures, feature_names: tuple[str]) -> str:
        ...
        key_parts = [domain_features.domain.value, f'conf_{domain_features.confidence:.3f}', f'comp_{domain_features.complexity_score:.3f}', f'spec_{domain_features.specificity_score:.3f}', f'hybrid_{domain_features.hybrid_domain}', f'strategy_{self.config.weighting_strategy.value}', f'boost_{self.config.confidence_boost_factor}', str(hash(feature_names))]
    ...
    def get_weighting_info(self, domain_features: DomainFeatures) -> dict[str, Any]:
        ...
        base_weights = self._get_base_weights(domain_features)
        return {'domain': domain_features.domain.value, 'confidence': domain_features.confidence, 'hybrid_domain': domain_features.hybrid_domain, 'secondary_domains': [(d.value, c) for d, c in domain_features.secondary_domains], 'base_weights': base_weights, 'weighting_strategy': self.config.weighting_strategy.value, 'config': {'confidence_boost_factor': self.config.confidence_boost_factor, 'min_weight_threshold': self.config.min_weight_threshold, 'max_weight_threshold': self.config.max_weight_threshold}}
