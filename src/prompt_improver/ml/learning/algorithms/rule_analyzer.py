...
try:
    ...
    warnings.warn('Time series analysis libraries not available. Install with: pip install scikit-learn pandas')
try:
    import arviz as az
    import pymc as pm
    ...
    try:
        ...
        warnings.warn('Bayesian modeling libraries not available. Install with: pip install pymc arviz')
...
class RuleEffectivenessAnalyzer:
            ...
            for rule_id, rule_data in test_data.items():
                if 'observations' in rule_data:
                    ...
                    for obs in observations:
                        applied_rules = [{'ruleId': rule_id, 'improvementScore': obs.get('score', 0)}]
                        adapted_data.append({'appliedRules': applied_rules, 'context': {'projectType': obs.get('context', 'unknown'), 'domain': 'test'}, 'timestamp': obs.get('timestamp'), 'overallImprovement': obs.get('score', 0)})
                else:
                    applied_rules = [{'ruleId': rule_id, 'improvementScore': rule_data.get('improvementScore', 0)}]
                    adapted_data.append({'appliedRules': applied_rules, 'context': rule_data.get('context', {}), 'overallImprovement': rule_data.get('improvementScore', 0)})
            return adapted_data
        return []

    async def analyze_rule_effectiveness(self, test_results: list[dict[str, Any]]) -> dict[str, Any]:
        """Analyze effectiveness of rules with Phase 2 enhancements"""
        adapted_test_results = self._adapt_test_data_format(test_results)
        self.logger.info('Analyzing rule effectiveness for %s results', len(adapted_test_results))
        rule_data = self._extract_rule_data(adapted_test_results)
        rule_metrics = await self._analyze_individual_rules(rule_data)
        combination_metrics = await self._analyze_rule_combinations(adapted_test_results)
        ts_validation_results = {}
        if self.config.enable_time_series_cv and TIME_SERIES_AVAILABLE:
            ts_validation_results = await self._perform_time_series_validation(rule_data)
        bayesian_results = {}
        ...
        if self.config.enable_time_series_cv and TIME_SERIES_AVAILABLE:
            advanced_time_series_results = await self._perform_advanced_time_series_analysis(rule_data)
        recommendations = await self._generate_rule_recommendations(rule_metrics, combination_metrics, ts_validation_results, bayesian_results, advanced_time_series_results)
        result = {'rule_metrics': {rule_id: metrics.__dict__ for rule_id, metrics in rule_metrics.items()}, 'individual_rules': {rule_id: metrics.__dict__ for rule_id, metrics in rule_metrics.items()}, 'rule_combinations': [combo.__dict__ for combo in combination_metrics], 'time_series_validation': {rule_id: result.__dict__ for rule_id, result in ts_validation_results.items()}, 'advanced_time_series_analysis': advanced_time_series_results, 'recommendations': recommendations, 'summary': self._generate_analysis_summary(rule_metrics, combination_metrics, ts_validation_results, bayesian_results, advanced_time_series_results), 'metadata': {'total_rules_analyzed': len(rule_metrics), 'total_combinations_analyzed': len(combination_metrics), 'time_series_validation_enabled': self.config.enable_time_series_cv and TIME_SERIES_AVAILABLE, 'bayesian_modeling_enabled': self.config.enable_bayesian_modeling and BAYESIAN_AVAILABLE, 'advanced_time_series_analysis_enabled': self.config.enable_time_series_cv and TIME_SERIES_AVAILABLE, 'analysis_date': datetime.now().isoformat()}}
        if self.config.enable_bayesian_modeling and BAYESIAN_AVAILABLE and bayesian_results:
            result['bayesian_analysis'] = {rule_id: bayesian_result.__dict__ for rule_id, bayesian_result in bayesian_results.items()}
        return result

    def _extract_rule_data(self, test_results: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
        ...
        for result in test_results:
            ...
            for rule in applied_rules:
                ...
                rule_data[rule_id].append({'score': rule_score, 'context': result.get('context', {}), 'timestamp': result.get('timestamp'), 'overall_score': result.get('overallImprovement', 0) or result.get('improvementScore', 0), 'other_rules': [r.get('ruleId', r.get('id', 'unknown')) for r in applied_rules if r != rule]})
        return dict(rule_data)

    async def _analyze_individual_rules(self, rule_data: dict[str, list[dict[str, Any]]]) -> dict[str, RuleMetrics]:
        ...
        for rule_id, data_points in rule_data.items():
            ...
            common_combinations = [combo for combo, count in combination_counts.most_common(5) if count >= 3]
            performance_trend = self._calculate_performance_trend(data_points)
            rule_metrics[rule_id] = RuleMetrics(rule_id=rule_id, total_applications=total_applications, success_rate=success_rate, avg_improvement=avg_improvement, std_improvement=std_improvement, median_improvement=median_improvement, consistency_score=consistency_score, contexts_used=contexts, common_combinations=common_combinations, performance_trend=performance_trend)
        return rule_metrics

    async def _analyze_rule_combinations(self, test_results: list[dict[str, Any]]) -> list[RuleCombinationMetrics]:
        """Analyze rule combination effectiveness"""
        combination_data = defaultdict(list)
        for result in test_results:
            applied_rules = result.get('appliedRules', [])
            if len(applied_rules) > 1:
                rule_ids = sorted([r.get('ruleId', r.get('id', 'unknown')) for r in applied_rules])
                combo_key = '|'.join(rule_ids)
                combination_data[combo_key].append({'rule_ids': rule_ids, 'score': result.get('overallImprovement', 0) or result.get('improvementScore', 0), 'context': result.get('context', {}), 'individual_scores': [r.get('improvementScore', 0) for r in applied_rules]})
        combination_metrics = []
    ...
    async def _generate_rule_recommendations(self, rule_metrics: dict[str, RuleMetrics], combination_metrics: list[RuleCombinationMetrics], ts_validation_results: dict[str, TimeSeriesValidationResult] | None=None, bayesian_results: dict[str, BayesianModelResult] | None=None, advanced_time_series_results: dict[str, Any] | None=None) -> list[dict[str, Any]]:
        """Generate recommendations for rule optimization"""
        recommendations = []
        for rule_id, metrics in rule_metrics.items():
            if metrics.avg_improvement > 0.7 and metrics.consistency_score > self.config.consistency_threshold and (metrics.total_applications >= 20):
                recommendations.append({'type': 'promote_rule', 'rule_id': rule_id, 'priority': 'high', 'description': f'Promote {rule_id} - consistently high performance', 'rationale': f'Avg improvement: {metrics.avg_improvement:.3f}, Consistency: {metrics.consistency_score:.3f}', 'action': 'Increase usage priority and expand to more contexts'})
            elif metrics.avg_improvement < 0.4 and metrics.total_applications >= 20:
                recommendations.append({'type': 'review_rule', 'rule_id': rule_id, 'priority': 'high', 'description': f'Review {rule_id} - consistently poor performance', 'rationale': f'Avg improvement: {metrics.avg_improvement:.3f}, Applications: {metrics.total_applications}', 'action': 'Consider modification, replacement, or deprecation'})
            elif metrics.consistency_score < 0.5 and metrics.total_applications >= 15:
                recommendations.append({'type': 'stabilize_rule', 'rule_id': rule_id, 'priority': 'medium', 'description': f'Stabilize {rule_id} - high performance variability', 'rationale': f'Consistency score: {metrics.consistency_score:.3f}, Std dev: {metrics.std_improvement:.3f}', 'action': 'Investigate and reduce performance variability'})
        high_synergy_combos = [c for c in combination_metrics if c.synergy_score > 0.1 and c.frequency >= 5]
        for combo in high_synergy_combos[:3]:
            recommendations.append({'type': 'promote_combination', 'rule_ids': combo.rule_ids, 'priority': 'medium', 'description': f"Promote combination {'+'.join(combo.rule_ids)} - positive synergy", 'rationale': f'Synergy score: {combo.synergy_score:.3f}, Frequency: {combo.frequency}', 'action': 'Create specialized combination rule or increase co-application'})
        if ts_validation_results:
            for rule_id, ts_result in ts_validation_results.items():
                if ts_result.temporal_stability < 0.6:
                    recommendations.append({'type': 'temporal_stability', 'rule_id': rule_id, 'priority': 'high', 'description': f'Address temporal instability in {rule_id}', 'rationale': f'Temporal stability: {ts_result.temporal_stability:.3f}, CV std: {ts_result.std_cv_score:.3f}', 'action': 'Investigate time-dependent factors affecting rule performance', 'phase2_insight': 'time_series'})
                if ts_result.trend_coefficient and ts_result.trend_coefficient < -0.01:
                    recommendations.append({'type': 'declining_trend', 'rule_id': rule_id, 'priority': 'medium', 'description': f'Rule {rule_id} shows declining performance trend', 'rationale': f'Trend coefficient: {ts_result.trend_coefficient:.4f}', 'action': 'Investigate causes of performance decline and consider rule updates', 'phase2_insight': 'time_series'})
                if len(ts_result.change_points) > 0:
                    recommendations.append({'type': 'performance_shift', 'rule_id': rule_id, 'priority': 'medium', 'description': f'Performance shifts detected in {rule_id}', 'rationale': f'Change points at indices: {ts_result.change_points}', 'action': 'Analyze external factors that may have caused performance changes', 'phase2_insight': 'time_series'})
        if bayesian_results:
            for rule_id, bayes_result in bayesian_results.items():
                if bayes_result.posterior_std > 0.2:
                    recommendations.append({'type': 'high_uncertainty', 'rule_id': rule_id, 'priority': 'medium', 'description': f'High performance uncertainty in {rule_id}', 'rationale': f'Posterior std: {bayes_result.posterior_std:.3f}, CI width: {bayes_result.credible_interval[1] - bayes_result.credible_interval[0]:.3f}', 'action': 'Gather more data or refine rule conditions to reduce uncertainty', 'phase2_insight': 'bayesian'})
                if bayes_result.rhat_statistic > 1.1:
                    recommendations.append({'type': 'convergence_issue', 'rule_id': rule_id, 'priority': 'low', 'description': f'Bayesian model convergence issues for {rule_id}', 'rationale': f'R-hat statistic: {bayes_result.rhat_statistic:.3f}', 'action': 'Review rule data quality and consider longer sampling', 'phase2_insight': 'bayesian'})
                if bayes_result.hierarchical_effects:
                    best_context = max(bayes_result.hierarchical_effects.items(), key=lambda x: x[1])
                    worst_context = min(bayes_result.hierarchical_effects.items(), key=lambda x: x[1])
                    if best_context[1] - worst_context[1] > 0.2:
                        recommendations.append({'type': 'context_specialization', 'rule_id': rule_id, 'priority': 'medium', 'description': f'Strong context effects detected for {rule_id}', 'rationale': f'Best context: {best_context[0]} (+{best_context[1]:.3f}), Worst: {worst_context[0]} ({worst_context[1]:.3f})', 'action': 'Consider context-specific rule variants or conditions', 'phase2_insight': 'bayesian'})
        priority_order = {'high': 3, 'medium': 2, 'low': 1}
        recommendations.sort(key=lambda x: priority_order.get(x['priority'], 0), reverse=True)
        return recommendations

    def _generate_analysis_summary(self, rule_metrics: dict[str, RuleMetrics], combination_metrics: list[RuleCombinationMetrics], ts_validation_results: dict[str, TimeSeriesValidationResult] | None=None, bayesian_results: dict[str, BayesianModelResult] | None=None, advanced_time_series_results: dict[str, Any] | None=None) -> dict[str, Any]:
    ...
    async def _perform_advanced_time_series_analysis(self, rule_data: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
        ...
        analysis_results = {'seasonal_decomposition': {}, 'trend_analysis': {}, 'change_point_detection': {}, 'forecasting': {}, 'stability_assessment': {}, 'cross_validation': {}}
        try:
            for rule_id, data_points in rule_data.items():
                ...
                if len(scores) >= self.config.time_series_min_train_size:
                    analysis_results['seasonal_decomposition'][rule_id] = await self._seasonal_decomposition(timestamps, scores)
                    analysis_results['trend_analysis'][rule_id] = await self._analyze_trends(timestamps, scores)
                    analysis_results['change_point_detection'][rule_id] = await self._detect_change_points_async(timestamps, scores)
                    analysis_results['forecasting'][rule_id] = await self._forecast_performance(timestamps, scores)
                    analysis_results['stability_assessment'][rule_id] = await self._assess_temporal_stability(timestamps, scores)
                    analysis_results['cross_validation'][rule_id] = await self._perform_cross_validation(data_points)
            analysis_results['summary'] = self._generate_time_series_summary(analysis_results)
            return analysis_results
        except Exception as e:
            self.logger.error('Advanced time series analysis failed: %s', e)
            return {'error': str(e)}

    def _extract_time_series(self, data_points: list[dict[str, Any]]) -> tuple[list[datetime], list[float]]:
    ...
    async def _seasonal_decomposition(self, timestamps: list[datetime], scores: list[float]) -> dict[str, Any]:
    ...
    async def _detect_change_points_async(self, timestamps: list[datetime], scores: list[float]) -> dict[str, Any]:
    ...
    async def _analyze_trends(self, timestamps: list[datetime], scores: list[float]) -> dict[str, Any]:
    ...
    async def _forecast_performance(self, timestamps: list[datetime], scores: list[float]) -> dict[str, Any]:
        """Forecast future rule performance"""
        try:
            if len(scores) < 10:
                return {'insufficient_data': 'Need at least 10 data points for forecasting'}
            alpha = 0.3
            forecasts = []
            ewma = scores[0]
            ...
            return {'forecasts': forecasts, 'forecast_method': 'exponential_smoothing', 'accuracy_metrics': accuracy_metrics, 'confidence_level': 0.95}
        except Exception as e:
            return {'error': str(e)}

    async def _assess_temporal_stability(self, timestamps: list[datetime], scores: list[float]) -> dict[str, Any]:
        """Assess temporal stability of rule performance"""
        try:
            if len(scores) < 10:
                return {'insufficient_data': 'Need at least 10 data points for stability assessment'}
    ...
    async def _perform_cross_validation(self, data_points: list[dict[str, Any]]) -> dict[str, Any]:
        """Advanced time series cross-validation"""
        self.logger.info('Performing cross-validation for time series data')
        if not TIME_SERIES_AVAILABLE or len(data_points) < self.config.time_series_min_train_size:
    ...
    def _generate_time_series_summary(self, analysis_results: dict[str, Any]) -> dict[str, Any]:
        ...
        summary = {'rules_analyzed': 0, 'stability_distribution': {'stable': 0, 'moderate': 0, 'unstable': 0}, 'trend_distribution': {'increasing': 0, 'decreasing': 0, 'stable': 0}, 'change_points_detected': 0, 'forecast_reliability': [], 'recommendations': []}
        stability_results = analysis_results.get('stability_assessment', {})
        for rule_id, stability_data in stability_results.items():
            if isinstance(stability_data, dict) and 'stability_classification' in stability_data:
        ...
        high_change_points = summary['change_points_detected'] > summary['rules_analyzed'] * 2
        if unstable_rules > summary['rules_analyzed'] * 0.3:
            summary['recommendations'].append('High number of unstable rules detected - review rule consistency')
        if decreasing_rules > summary['rules_analyzed'] * 0.4:
            summary['recommendations'].append('Multiple rules showing decreasing performance trends - investigate rule degradation')
    ...
    def _calculate_performance_trend(self, data_points: list[dict[str, Any]]) -> str | None:
        """Calculate performance trend over time"""
        timestamped_points = [dp for dp in data_points if dp.get('timestamp')]
        if len(timestamped_points) < 7:
            return None
        try:
    ...
    async def _perform_time_series_validation(self, rule_data: dict[str, list[dict[str, Any]]]) -> dict[str, TimeSeriesValidationResult]:
        ...
        try:
            for rule_id, data_points in rule_data.items():
                ...
                if ts_data is None or len(ts_data) < self.config.time_series_min_train_size:
                    continue
                cv_result = self._time_series_cross_validate(ts_data, rule_id)
                if cv_result:
                    results[rule_id] = cv_result
            self.logger.info('Completed time series validation for %s rules', len(results))
        except Exception as e:
            self.logger.error('Time series validation failed: %s', e)
        return results

    def _prepare_time_series_data(self, data_points: list[dict[str, Any]]) -> pd.DataFrame | None:
        ...
        try:
            ...
            df[f'score_rolling_{self.config.rolling_window_size}'] = df['score'].rolling(window=self.config.rolling_window_size, min_periods=1).mean()
            return df
        except Exception as e:
            self.logger.warning('Failed to prepare time series data: %s', e)
            return None
    ...
    def _detect_seasonal_patterns(self, ts_data: pd.DataFrame) -> dict[str, float] | None:
        """Detect seasonal patterns in performance data"""
        try:
            if len(ts_data) < 28:
                return None
            seasonal_stats = {}
            if 'timestamp' in ts_data.index.names or hasattr(ts_data.index, 'dayofweek'):
                ts_data_reset = ts_data.reset_index()
                ts_data_reset['dayofweek'] = pd.to_datetime(ts_data_reset['timestamp']).dt.dayofweek
    ...
    def _perform_bayesian_modeling(self, rule_data: dict[str, list[dict[str, Any]]]):
        """Phase 2: Perform Bayesian modeling for uncertainty quantification

        Args:
            rule_data: Dictionary mapping rule IDs to their data points

        Returns:
            Dict[str, BayesianModelResult]: Results for each rule
        """
        results = {}
        ...
        try:
            ...
            self.logger.info('Completed Bayesian modeling for %s rules', len(results))
        except Exception as e:
            self.logger.error('Bayesian modeling failed: %s', e)
        return results

    async def _perform_bayesian_modeling_async(self, rule_data: dict[str, list[dict[str, Any]]]) -> dict[str, BayesianModelResult]:
        ...
        try:
            ...
            with pm.Model() as model:
                ...
                if len(unique_contexts) > 1:
                    context_effects_logit = pm.Normal('context_effects_logit', mu=0, sigma=1, shape=len(unique_contexts))
                ...
                var_term_safe = pm.math.clip(var_term, 0.1, 1000)
                alpha = mu * var_term_safe
                beta = (1 - mu) * var_term_safe
                alpha_safe = pm.math.clip(alpha, 0.1, 1000)
                beta_safe = pm.math.clip(beta, 0.1, 1000)
                observed = pm.Beta('observed', alpha=alpha_safe, beta=beta_safe, observed=scores)
                trace = pm.sample(draws=self.config.bayesian_samples, tune=self.config.bayesian_tune, chains=self.config.bayesian_chains, return_inferencedata=True, progressbar=False, random_seed=42)
            try:
            ...
            try:
                ...
                try:
                    ...
                    effective_sample_size = float(self.config.bayesian_samples * 0.8)
                    rhat_statistic = 1.01
            ...
            if len(unique_contexts) > 1:
                try:
                    ...
                    for i, context in enumerate(unique_contexts):
                        hierarchical_effects[context] = float(context_effects_samples[..., i].mean())
                except Exception:
                    pass
            result = BayesianModelResult(rule_id=rule_id, posterior_mean=posterior_mean, posterior_std=posterior_std, credible_interval=credible_interval, effective_sample_size=effective_sample_size, rhat_statistic=rhat_statistic, model_comparison_score=None, hierarchical_effects=hierarchical_effects)
    ...
    async def run_orchestrated_bayesian_analysis(self, orchestrator_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            orchestrator_config: Configuration from orchestrator containing:
                - rule_id: Rule to analyze
                - performance_data: Historical performance data
                - analysis_type: Type of analysis ('effectiveness', 'time_series', 'hierarchical')
                - orchestrator_metadata: Additional orchestrator context

        Returns:
            Orchestrator-compatible analysis results
        """
        try:
            rule_id = orchestrator_config.get('rule_id')
            performance_data = orchestrator_config.get('performance_data', [])
            analysis_type = orchestrator_config.get('analysis_type', 'effectiveness')
            self.logger.info('Running orchestrated Bayesian analysis for rule %s', rule_id)
            if not BAYESIAN_AVAILABLE:
                return {'orchestrator_compatible': True, 'error': 'Bayesian analysis not available - PyMC not installed', 'fallback_analysis': await self._run_fallback_analysis(orchestrator_config)}
            if analysis_type == 'effectiveness':
                analysis_result = await self._bayesian_effectiveness_analysis(rule_id, performance_data)
            elif analysis_type == 'time_series':
                analysis_result = await self._bayesian_time_series_analysis(rule_id, performance_data)
            elif analysis_type == 'hierarchical':
                analysis_result = await self._bayesian_hierarchical_analysis(rule_id, performance_data)
            else:
                analysis_result = await self._bayesian_effectiveness_analysis(rule_id, performance_data)
            if analysis_result is None:
                return {'orchestrator_compatible': True, 'error': 'Bayesian analysis failed', 'fallback_analysis': await self._run_fallback_analysis(orchestrator_config)}
            orchestrator_result = {'orchestrator_compatible': True, 'rule_id': rule_id, 'analysis_type': analysis_type, 'bayesian_analysis': {'posterior_mean': getattr(analysis_result, 'posterior_mean', 0.0), 'posterior_std': getattr(analysis_result, 'posterior_std', 0.0), 'credible_interval': getattr(analysis_result, 'credible_interval', {}), 'uncertainty_estimate': getattr(analysis_result, 'uncertainty_estimate', 0.0), 'effective_sample_size': getattr(analysis_result, 'effective_sample_size', 0), 'rhat_statistic': getattr(analysis_result, 'rhat_statistic', 1.0)}, 'uncertainty_quantification': self._extract_uncertainty_metrics(analysis_result), 'recommendations': self._generate_orchestrator_recommendations(analysis_result), 'metadata': {'timestamp': datetime.now().isoformat(), 'sample_size': len(performance_data), 'bayesian_method': 'pymc_beta_binomial', 'orchestrator_version': '1.0'}}
    ...
    async def _bayesian_effectiveness_analysis(self, rule_id: str, performance_data: List[Dict[str, Any]]):
        ...
        for data_point in performance_data:
            if isinstance(data_point, dict):
                score = data_point.get('effectiveness_score', data_point.get('score', 0.5))
    ...
    def _extract_uncertainty_metrics(self, analysis_result) -> Dict[str, Any]:
        ...
        return {'posterior_uncertainty': getattr(analysis_result, 'posterior_std', 0.0), 'credible_interval_width': self._calculate_credible_interval_width(analysis_result), 'effective_sample_size': getattr(analysis_result, 'effective_sample_size', 0), 'convergence_diagnostic': getattr(analysis_result, 'rhat_statistic', 1.0), 'uncertainty_level': self._categorize_uncertainty_level(analysis_result)}

    def _calculate_credible_interval_width(self, analysis_result) -> float:
        """Calculate width of credible interval."""
        credible_interval = getattr(analysis_result, 'credible_interval', {})
        if isinstance(credible_interval, dict):
            lower = credible_interval.get('lower', 0.0)
            upper = credible_interval.get('upper', 1.0)
            return float(upper - lower)
        elif isinstance(credible_interval, (list, tuple)) and len(credible_interval) == 2:
            return float(credible_interval[1] - credible_interval[0])
        else:
            return 0.0

    def _categorize_uncertainty_level(self, analysis_result) -> str:
        """Categorize uncertainty level based on analysis results."""
        posterior_std = getattr(analysis_result, 'posterior_std', 0.0)
        if posterior_std < 0.05:
            return 'low'
        elif posterior_std < 0.15:
            return 'medium'
        else:
            return 'high'
    ...
    def _generate_orchestrator_recommendations(self, analysis_result) -> List[str]:
        ...
        recommendations = []
        posterior_mean = getattr(analysis_result, 'posterior_mean', 0.5)
        posterior_std = getattr(analysis_result, 'posterior_std', 0.0)
        rhat = getattr(analysis_result, 'rhat_statistic', 1.0)
        if posterior_mean > 0.8:
            recommendations.append('High effectiveness - consider expanding rule usage')
        elif posterior_mean > 0.6:
            recommendations.append('Moderate effectiveness - monitor performance')
        elif posterior_mean > 0.4:
            recommendations.append('Low effectiveness - consider rule optimization')
        else:
            recommendations.append('Poor effectiveness - consider rule replacement')
        if posterior_std > 0.2:
            recommendations.append('High uncertainty - collect more data for better estimates')
        elif posterior_std > 0.1:
            recommendations.append('Moderate uncertainty - additional data would improve confidence')
        if rhat > 1.1:
            recommendations.append('Poor convergence - results may be unreliable')
        elif rhat > 1.05:
            recommendations.append('Marginal convergence - consider longer sampling')
        return recommendations

    def _generate_basic_recommendation(self, mean_score: float, std_score: float) -> str:
        """Generate basic recommendation from statistical analysis."""
        if mean_score > 0.8:
            return 'High performance rule - maintain current usage'
        elif mean_score > 0.6:
            return 'Good performance rule - monitor for consistency'
        elif mean_score > 0.4:
            return 'Average performance rule - consider optimization'
        else:
            return 'Poor performance rule - review and potentially replace'
