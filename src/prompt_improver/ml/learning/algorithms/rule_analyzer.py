...
class RuleEffectivenessAnalyzer:
    ...
    async def run_orchestrated_bayesian_analysis(self, orchestrator_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Running orchestrated Bayesian analysis for rule {rule_id}")

            if not BAYESIAN_AVAILABLE:
                return {
                    "orchestrator_compatible": True,
                    "error": "Bayesian analysis not available - PyMC not installed",
                    "fallback_analysis": await self._run_fallback_analysis(orchestrator_config)
                }

            # Run appropriate Bayesian analysis based on type
            ...
            # Format results for orchestrator consumption
            orchestrator_result = {
                "orchestrator_compatible": True,
                "rule_id": rule_id,
                "analysis_type": analysis_type,
                "bayesian_analysis": {
                    "posterior_mean": getattr(analysis_result, 'posterior_mean', 0.0),
                    "posterior_std": getattr(analysis_result, 'posterior_std', 0.0),
                    "credible_interval": getattr(analysis_result, 'credible_interval', {}),
                    "uncertainty_estimate": getattr(analysis_result, 'uncertainty_estimate', 0.0),
                    "effective_sample_size": getattr(analysis_result, 'effective_sample_size', 0),
                    "rhat_statistic": getattr(analysis_result, 'rhat_statistic', 1.0)
                },
                "uncertainty_quantification": self._extract_uncertainty_metrics(analysis_result),
                "recommendations": self._generate_orchestrator_recommendations(analysis_result),
                "metadata": {
                    "timestamp": datetime.now().isoformat(),
                    "sample_size": len(performance_data),
                    "bayesian_method": "pymc_beta_binomial",
            ...
            self.logger.error(f"Orchestrated Bayesian analysis failed: {e}")
            return {
                "orchestrator_compatible": True,
                "error": str(e),
                "fallback_analysis": await self._run_fallback_analysis(orchestrator_config)
    ...
    async def _bayesian_effectiveness_analysis(self, rule_id: str, performance_data: List[Dict[str, Any]]):
        ...
        # Convert performance data to format expected by existing method
        if not performance_data:
            return None

        # Extract effectiveness scores from performance data
        scores = []
        ...
        # Use existing Bayesian modeling method
        # Convert scores to data points format expected by _fit_bayesian_model
    ...
    async def _bayesian_time_series_analysis(self, rule_id: str, performance_data: List[Dict[str, Any]]):
        ...
        # For now, fall back to effectiveness analysis
        # In future, implement time series Bayesian modeling
        self.logger.info(f"Time series Bayesian analysis not yet implemented for {rule_id}, using effectiveness analysis")
    ...
    async def _bayesian_hierarchical_analysis(self, rule_id: str, performance_data: List[Dict[str, Any]]):
        ...
        # For now, fall back to effectiveness analysis
        # In future, implement hierarchical Bayesian modeling
        self.logger.info(f"Hierarchical Bayesian analysis not yet implemented for {rule_id}, using effectiveness analysis")
        return await self._bayesian_effectiveness_analysis(rule_id, performance_data)
    ...
    async def _run_fallback_analysis(self, orchestrator_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Extract scores for basic statistical analysis
        scores = []
        for data_point in performance_data:
            if isinstance(data_point, dict):
                score = data_point.get("effectiveness_score", data_point.get("score", 0.5))
        ...
        return {
            ...
            "mean_effectiveness": float(mean_score),
            "std_effectiveness": float(std_score),
            "sample_size": len(scores),
            "confidence_interval": {
                "lower": float(mean_score - 1.96 * std_score / np.sqrt(len(scores))),
                "upper": float(mean_score + 1.96 * std_score / np.sqrt(len(scores)))
            },
            "recommendation": self._generate_basic_recommendation(mean_score, std_score)
        }
    ...
    def _extract_uncertainty_metrics(self, analysis_result) -> Dict[str, Any]:
        ...
        return {
            "posterior_uncertainty": getattr(analysis_result, 'posterior_std', 0.0),
            "credible_interval_width": self._calculate_credible_interval_width(analysis_result),
            "effective_sample_size": getattr(analysis_result, 'effective_sample_size', 0),
            "convergence_diagnostic": getattr(analysis_result, 'rhat_statistic', 1.0),
            "uncertainty_level": self._categorize_uncertainty_level(analysis_result)
        }
