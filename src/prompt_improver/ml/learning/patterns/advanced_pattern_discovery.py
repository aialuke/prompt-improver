...
class AdvancedPatternDiscovery:
            ...
            with self._apriori_analyzer_lock:
                if self._apriori_analyzer is None:
                    ...
                    if db_mgr is not None:
                        try:
                            self._apriori_analyzer = AprioriAnalyzer(db_manager=db_mgr, config=AprioriConfig(min_support=0.08, min_confidence=0.6, min_lift=1.2, max_itemset_length=4, verbose=True))
                            logger.info('AprioriAnalyzer created via lazy initialization')
                        except Exception as e:
                            logger.error('Failed to create AprioriAnalyzer: %s', e)
                            return None
                    else:
            ...
            with self._training_loader_lock:
                if self._training_loader is None:
                    try:
                        ...
                        logger.error('Failed to create TrainingDataLoader: %s', e)
                        return None
    ...
    def _ensure_apriori_analyzer(self) -> bool:
        ...
        try:
            ...
            logger.error('AprioriAnalyzer check failed: %s', e)
            return False

    async def discover_advanced_patterns(self, db_session: AsyncSession, min_effectiveness: float=0.7, min_support: int=5, pattern_types: list[str] | None=None, use_ensemble: bool=True, include_apriori: bool=True) -> dict[str, Any]:
        ...
        logger.info('Starting advanced pattern discovery with types: %s, including Apriori: %s', pattern_types, include_apriori)
        performance_data = await self._get_performance_data(db_session, min_effectiveness)
        if len(performance_data) < min_support:
            logger.warning('Insufficient data points (%s) for pattern discovery', len(performance_data))
    ...
    async def _get_performance_data(self, db_session: AsyncSession, min_effectiveness: float) -> list[dict[str, Any]]:
        ...
        try:
            ...
            performance_data = []
            for row in rows:
                data_point = {'rule_id': row.rule_id, 'rule_name': row.rule_id, 'rule_category': row.category or 'general', 'improvement_score': row.improvement_score or 0.0, 'execution_time_ms': row.execution_time_ms or 0, 'user_satisfaction': 0.8, 'confidence_level': row.confidence_level or 0.0, 'weight': 1.0, 'priority': row.priority or 5, 'parameters': row.parameters_used or {}, 'metadata_params': row.rule_metadata_params or {}, 'before_metrics': {}, 'after_metrics': {}, 'prompt_characteristics': {}, 'created_at': row.created_at, 'timestamp': row.created_at.timestamp() if row.created_at else 0}
                performance_data.append(data_point)
            return performance_data
        except Exception as e:
            logger.error('Failed to get performance data: %s', e)
            return []

    async def _discover_parameter_patterns(self, performance_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error('Parameter pattern discovery failed: %s', e)
            return {'error': str(e)}

    async def _discover_sequence_patterns(self, performance_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            for data_point in performance_data:
                ...
                if transaction:
                    ...
                    effectiveness_map[transaction_key].append(data_point['improvement_score'])
            if len(transactions) < 5:
                return {'frequent_patterns': [], 'association_rules': [], 'message': 'Insufficient sequence data'}
            ...
            try:
                if hasattr(te, 'columns_') and te.columns_ is not None:
                    ...
                    if column_names:
                        df = pd.DataFrame(te_data, columns=column_names)
                    else:
                        df = pd.DataFrame(te_data)
                else:
                    df = pd.DataFrame(te_data)
            except (AttributeError, TypeError, ValueError) as e:
                logger.warning('DataFrame creation fallback due to: %s', e)
                df = pd.DataFrame(te_data)
            try:
            ...
            if frequent_itemsets.empty:
                return {'frequent_patterns': [], 'association_rules': [], 'message': 'No frequent patterns found'}
            try:
                ...
                rules = pd.DataFrame()
            patterns = self._process_frequent_patterns(frequent_itemsets, effectiveness_map)
            rule_patterns = self._process_association_rules(rules, effectiveness_map)
            return {'frequent_patterns': patterns, 'association_rules': rule_patterns, 'algorithm': 'FP-Growth', 'total_transactions': len(transactions), 'total_patterns': len(patterns), 'total_rules': len(rule_patterns)}
        except Exception as e:
            logger.error('Sequence pattern discovery failed: %s', e)
            return {'error': str(e)}

    async def _discover_performance_patterns(self, performance_data: list[dict[str, Any]]) -> dict[str, Any]:
        """Discover performance patterns using density-based clustering"""
        try:
            perf_features = []
            for data_point in performance_data:
                features = [data_point['improvement_score'], data_point['execution_time_ms'] / 1000.0, data_point['user_satisfaction'], data_point['confidence_level'], data_point['weight'], data_point['priority'] / 10.0]
            ...
            if len(set(cluster_labels)) > 1:
                ...
                silhouette_avg = 0.0
                calinski_score = 0.0
            clusters = self._process_performance_clusters(cluster_labels, performance_data, perf_features, outlier_scores)
            return {'clusters': clusters, 'algorithm': 'HDBSCAN' if HDBSCAN_AVAILABLE else 'DBSCAN', 'cluster_quality': {'silhouette_score': silhouette_avg, 'calinski_harabasz_score': calinski_score}, 'total_clusters': len([c for c in clusters if c['cluster_id'] != -1]), 'noise_points': len([c for c in clusters if c['cluster_id'] == -1])}
        except Exception as e:
            logger.error('Performance pattern discovery failed: %s', e)
            return {'error': str(e)}

    async def _discover_semantic_patterns(self, performance_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            return {'category_patterns': semantic_patterns, 'cross_category_patterns': cross_patterns, 'total_categories': len(category_groups), 'total_patterns': len(semantic_patterns) + len(cross_patterns)}
        except Exception as e:
            logger.error('Semantic pattern discovery failed: %s', e)
            return {'error': str(e)}

    def _params_to_features(self, params: dict[str, Any], data_point: dict[str, Any]) -> list[float] | None:
        ...
        try:
            ...
            features.extend([data_point['improvement_score'], data_point['execution_time_ms'] / 1000.0, data_point['user_satisfaction'], data_point['confidence_level']])
    ...
    def _fallback_sequence_patterns(self, performance_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for i, (label, score) in enumerate(zip(cluster_labels, outlier_scores, strict=False)):
            if label == -1 or score > 0.7:
                ...
                if data_point['improvement_score'] >= self.min_effectiveness:
                    outlier_info = {'rule_id': data_point['rule_id'], 'rule_name': data_point['rule_name'], 'effectiveness': data_point['improvement_score'], 'outlier_score': float(score), 'parameters': data_point['parameters'], 'unique_characteristics': self._analyze_outlier_uniqueness(data_point)}
                    outliers.append(outlier_info)
        return sorted(outliers, key=lambda x: x['effectiveness'], reverse=True)

    def _process_frequent_patterns(self, frequent_itemsets: pd.DataFrame, effectiveness_map: dict[tuple, list[float]]) -> list[dict[str, Any]]:
    ...
    def _process_association_rules(self, rules: pd.DataFrame, effectiveness_map: dict[tuple, list[float]]) -> list[dict[str, Any]]:
    ...
    def _process_performance_clusters(self, cluster_labels: np.ndarray, performance_data: list[dict[str, Any]], features: np.ndarray, outlier_scores: np.ndarray) -> list[dict[str, Any]]:
        ...
        for label in unique_labels:
            cluster_mask = cluster_labels == label
            cluster_data = [performance_data[i] for i, mask in enumerate(cluster_mask) if mask]
    ...
    def _analyze_semantic_category(self, category: str, data_points: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    def _analyze_cross_category_patterns(self, category_groups: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
        ...
        for i, cat1 in enumerate(categories):
            for cat2 in categories[i + 1:]:
                if len(category_groups[cat1]) >= 3 and len(category_groups[cat2]) >= 3:
                    pattern = self._compare_categories(cat1, category_groups[cat1], cat2, category_groups[cat2])
                    if pattern:
                        cross_patterns.append(pattern)
        return cross_patterns

    def _extract_common_parameters(self, cluster_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for param, value_counts in param_counts.items():
            if value_counts:
                ...
                if count / len(cluster_data) >= 0.5:
                    common_params[param] = {'value': most_common_value, 'frequency': count / len(cluster_data)}
    ...
    def _extract_common_characteristics(self, data_points: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    def _compare_categories(self, cat1: str, data1: list[dict[str, Any]], cat2: str, data2: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    def _analyze_pattern_ensemble(self, pattern_discovery: dict[str, Any]) -> dict[str, Any]:
        """Analyze patterns across different discovery methods"""
        ensemble_results = {'cross_validation': {}, 'consensus_patterns': [], 'algorithm_comparison': {}}
        if 'parameter_patterns' in pattern_discovery and 'performance_patterns' in pattern_discovery:
            param_clusters = pattern_discovery['parameter_patterns'].get('clusters', [])
            perf_clusters = pattern_discovery['performance_patterns'].get('clusters', [])
            consensus = self._find_consensus_patterns(param_clusters, perf_clusters)
            ensemble_results['consensus_patterns'] = consensus
        return ensemble_results

    def _find_consensus_patterns(self, param_clusters: list[dict[str, Any]], perf_clusters: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for param_cluster in param_clusters:
            if param_cluster['avg_effectiveness'] >= 0.8:
                for perf_cluster in perf_clusters:
                    if abs(param_cluster['avg_effectiveness'] - perf_cluster['characteristics']['avg_effectiveness']) < 0.1:
                        consensus_pattern = {'consensus_type': 'high_performance', 'parameter_cluster_id': param_cluster['cluster_id'], 'performance_cluster_id': perf_cluster['cluster_id'], 'avg_effectiveness': (param_cluster['avg_effectiveness'] + perf_cluster['characteristics']['avg_effectiveness']) / 2, 'confidence': 'high'}
                        consensus_patterns.append(consensus_pattern)
        return consensus_patterns

    def _validate_patterns_statistically(self, pattern_discovery: dict[str, Any]) -> dict[str, Any]:
        """Statistical validation of discovered patterns"""
        validation_results = {'pattern_significance': {}, 'confidence_intervals': {}, 'effect_sizes': {}}
        if 'parameter_patterns' in pattern_discovery:
            param_validation = self._validate_parameter_patterns(pattern_discovery['parameter_patterns'])
            validation_results['parameter_patterns'] = param_validation
        if 'sequence_patterns' in pattern_discovery:
            seq_validation = self._validate_sequence_patterns(pattern_discovery['sequence_patterns'])
            validation_results['sequence_patterns'] = seq_validation
        return validation_results

    def _validate_parameter_patterns(self, param_patterns: dict[str, Any]) -> dict[str, Any]:
    ...
    def _validate_sequence_patterns(self, seq_patterns: dict[str, Any]) -> dict[str, Any]:
        ...
        for i, pattern in enumerate(patterns):
            if pattern.get('effectiveness_count', 0) >= 3:
                validation['pattern_reliability'][i] = {'support_level': pattern['support'], 'effectiveness_reliability': pattern.get('effectiveness_std', 0) < 0.2, 'sample_adequacy': pattern.get('effectiveness_count', 0) >= 5}
        return validation

    def _generate_pattern_recommendations(self, results: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if param_patterns.get('clusters'):
            best_cluster = max(param_patterns['clusters'], key=lambda x: x['avg_effectiveness'])
            if best_cluster['avg_effectiveness'] > 0.8:
                recommendations.append({'type': 'parameter_optimization', 'priority': 'high', 'action': 'Deploy parameter configuration from top-performing cluster', 'details': {'cluster_id': best_cluster['cluster_id'], 'expected_improvement': best_cluster['avg_effectiveness'], 'common_parameters': best_cluster['common_parameters']}})
        seq_patterns = results.get('sequence_patterns', {})
        if seq_patterns.get('frequent_patterns'):
            top_pattern = max(seq_patterns['frequent_patterns'], key=lambda x: x['avg_effectiveness'])
            recommendations.append({'type': 'sequence_optimization', 'priority': 'medium', 'action': 'Implement high-performing rule sequence pattern', 'details': {'pattern': top_pattern['itemset'], 'expected_effectiveness': top_pattern['avg_effectiveness'], 'support': top_pattern['support']}})
        if param_patterns.get('outliers'):
            top_outlier = max(param_patterns['outliers'], key=lambda x: x['effectiveness'])
            recommendations.append({'type': 'outlier_investigation', 'priority': 'medium', 'action': 'Investigate high-performing outlier for unique insights', 'details': {'rule_name': top_outlier['rule_name'], 'effectiveness': top_outlier['effectiveness'], 'unique_characteristics': top_outlier['unique_characteristics']}})
    ...
    def _configure_hdbscan_performance(self):
        ...
        try:
            import os
            self._optimal_n_jobs = min(os.cpu_count() or 1, 4)
            ...
            logger.info('HDBSCAN performance configured: %s parallel jobs', self._optimal_n_jobs)
        except Exception as e:
            logger.warning('Failed to configure HDBSCAN performance: %s', e)
    ...
    async def benchmark_clustering_performance(self, dataset_sizes: list[int] | None=None, max_time: int=45, sample_size: int=2) -> dict[str, Any]:
        ...
        for size in dataset_sizes:
            ...
            for sample in range(sample_size):
                try:
                    ...
                    start_time = time.time()
                    if HDBSCAN_AVAILABLE:
                        clusterer = hdbscan.HDBSCAN(min_cluster_size=max(5, size // 100), algorithm='boruvka_kdtree', core_dist_n_jobs=self._get_optimal_n_jobs())
                        clusterer.fit(data)
                    execution_time = time.time() - start_time
                    if execution_time > max_time:
                        logger.warning('Benchmark timeout at size %s', size)
                        break
    ...
    def _analyze_performance_results(self, results: dict) -> dict[str, Any]:
        ...
        return {'max_dataset_size': max(sizes), 'fastest_time': min(times), 'slowest_time': max(times), 'complexity_estimate': complexity_estimate, 'scalability_rating': 'Good' if max(times) < 10 else 'Moderate' if max(times) < 60 else 'Poor'}

    async def _discover_apriori_patterns(self, performance_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            apriori_results = self.apriori_analyzer.analyze_patterns(window_days=30, save_to_database=True)
            if 'error' in apriori_results:
                logger.warning('Apriori analysis error: %s', apriori_results['error'])
                return apriori_results
            apriori_patterns = []
            for rule in apriori_results.get('top_rules', []):
                pattern = AprioriPattern(antecedents=rule['antecedents'], consequents=rule['consequents'], support=rule['support'], confidence=rule['confidence'], lift=rule['lift'], conviction=rule.get('conviction', 0), rule_strength=rule['rule_strength'], business_insight=self._generate_business_insight(rule), pattern_category=self._categorize_apriori_pattern(rule))
                apriori_patterns.append(pattern)
            performance_context = self._analyze_apriori_performance_context(performance_data, apriori_results)
            return {'patterns': [self._apriori_pattern_to_dict(p) for p in apriori_patterns], 'transaction_count': apriori_results.get('transaction_count', 0), 'frequent_itemsets_count': apriori_results.get('frequent_itemsets_count', 0), 'association_rules_count': apriori_results.get('association_rules_count', 0), 'top_itemsets': apriori_results.get('top_itemsets', []), 'pattern_insights': apriori_results.get('pattern_insights', {}), 'performance_context': performance_context, 'config': apriori_results.get('config', {}), 'discovery_type': 'apriori_association_rules', 'algorithm': 'mlxtend_apriori', 'timestamp': apriori_results.get('timestamp')}
        except Exception as e:
            logger.error('Error in Apriori pattern discovery: %s', e)
    ...
    def _calculate_rule_coverage(self, performance_data: list[dict[str, Any]], apriori_results: dict[str, Any]) -> dict[str, float]:
        ...
        if rules_in_performance:
            ...
            coverage = 0.0
        return {'coverage': coverage, 'rules_in_performance': len(rules_in_performance), 'rules_in_apriori': len(rules_in_apriori), 'overlap': len(rules_in_apriori.intersection(rules_in_performance))}

    def _validate_apriori_against_performance(self, performance_data: list[dict[str, Any]], apriori_results: dict[str, Any]) -> dict[str, Any]:
        """Validate Apriori patterns against actual performance data."""
        validation_results = {'pattern_accuracy': 0.0, 'prediction_confidence': 0.0, 'business_value_score': 0.0}
        high_conf_patterns = []
        for rule in apriori_results.get('top_rules', []):
    ...
    def _apriori_pattern_to_dict(self, pattern: AprioriPattern) -> dict[str, Any]:
        ...
        return {'antecedents': pattern.antecedents, 'consequents': pattern.consequents, 'support': pattern.support, 'confidence': pattern.confidence, 'lift': pattern.lift, 'conviction': pattern.conviction, 'rule_strength': pattern.rule_strength, 'business_insight': pattern.business_insight, 'pattern_category': pattern.pattern_category, 'type': 'apriori_association_rule'}

    async def get_contextualized_patterns(self, context_items: list[str], db_session: AsyncSession, min_confidence: float=0.6) -> dict[str, Any]:
        ...
        if not self._ensure_apriori_analyzer():
            ...
            return {'error': 'AprioriAnalyzer not initialized'}
        try:
            relevant_rules = self.apriori_analyzer.get_rules_for_context(context_items, min_confidence)
            performance_data = await self._get_performance_data(db_session, 0.5)
            recommendations = self._generate_contextualized_recommendations(relevant_rules, context_items, performance_data)
            return {'context_items': context_items, 'relevant_rules': relevant_rules, 'recommendations': recommendations, 'rule_count': len(relevant_rules), 'context_coverage': self._calculate_context_coverage(relevant_rules, context_items), 'timestamp': time.time()}
        except Exception as e:
            logger.error('Error getting contextualized patterns: %s', e)
            return {'error': f'Contextualized pattern analysis failed: {e!s}'}

    def _generate_contextualized_recommendations(self, relevant_rules: list[dict[str, Any]], context_items: list[str], performance_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def _calculate_context_coverage(self, relevant_rules: list[dict[str, Any]], context_items: list[str]) -> float:
    ...
    async def discover_training_data_patterns(self, db_session: AsyncSession, pattern_types: list[str] | None=None, min_effectiveness: float=0.7, use_clustering: bool=True, include_feature_patterns: bool=True) -> dict[str, Any]:
        ...
        try:
            ...
            if 'clustering' in pattern_types and use_clustering:
                clustering_patterns = await self._discover_training_clustering_patterns(features, labels, min_effectiveness)
                results['patterns']['clustering'] = clustering_patterns
                if clustering_patterns['status'] == 'success':
                    results['insights'].append(f"Discovered {clustering_patterns['n_clusters']} distinct pattern clusters")
            if 'feature_importance' in pattern_types and include_feature_patterns:
                feature_patterns = await self._discover_feature_importance_patterns(features, labels)
                ...
                if feature_patterns['status'] == 'success':
                    ...
                    results['insights'].append(f"Top influential features: {', '.join([f'Feature {i}' for i in top_features])}")
            if 'effectiveness' in pattern_types:
                effectiveness_patterns = await self._discover_effectiveness_patterns(features, labels, min_effectiveness)
                ...
                if effectiveness_patterns['status'] == 'success':
                    ...
                    results['insights'].append(f'High effectiveness patterns found in {high_eff_ratio:.1%} of training data')
            results['recommendations'] = self._generate_training_pattern_recommendations(results['patterns'], training_data['metadata'])
            logger.info('Training data pattern discovery completed: %s pattern types found', len(results['patterns']))
            return results
        except Exception as e:
    ...
    def _generate_training_pattern_recommendations(self, patterns: dict[str, Any], training_metadata: dict[str, Any]) -> list[str]:
        ...
        try:
            if 'clustering' in patterns and patterns['clustering'].get('status') == 'success':
                ...
                if best_cluster and best_cluster['avg_effectiveness'] > 0.8:
                    recommendations.append(f"🎆 High-Performance Cluster: Cluster {best_cluster['cluster_id']} shows {best_cluster['avg_effectiveness']:.2f} avg effectiveness")
            if 'feature_importance' in patterns and patterns['feature_importance'].get('status') == 'success':
                ...
                if max_correlation > 0.3:
                    recommendations.append(f'📊 Strong features: {n_significant} features show significant correlation (max: {max_correlation:.3f})')
                    recommendations.append('⚙️ Optimization Focus: Prioritize top correlated features for rule tuning')
            if 'effectiveness' in patterns and patterns['effectiveness'].get('status') == 'success':
                ...
                if high_ratio > 0.7:
                    recommendations.append(f'✅ Strong Performance: {high_ratio:.1%} of training data shows high effectiveness')
                elif high_ratio < 0.3:
                    recommendations.append(f'⚠️ Performance Opportunity: Only {high_ratio:.1%} of data shows high effectiveness - room for improvement')
            total_samples = training_metadata.get('total_samples', 0)
            synthetic_ratio = training_metadata.get('synthetic_ratio', 0)
            if total_samples > 1000:
                recommendations.append(f'📈 Rich Dataset: {total_samples} samples provide strong pattern discovery foundation')
            if synthetic_ratio > 0.3:
                recommendations.append(f'🤖 Synthetic Data: {synthetic_ratio:.1%} synthetic data - consider increasing real data collection')
        except Exception as e:
