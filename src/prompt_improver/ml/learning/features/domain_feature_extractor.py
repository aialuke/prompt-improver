...
class DomainFeatureExtractor:
    ...
    def __init__(self, config: DomainFeatureConfig | None=None):
        ...
        if DOMAIN_ANALYSIS_AVAILABLE and self.config.use_domain_analyzer:
            try:
                ...
                logger.warning('Failed to initialize domain analyzer: %s', e)
        ...
        self._creative_domains = {PromptDomain.CREATIVE_WRITING, PromptDomain.MARKETING, PromptDomain.CONTENT_CREATION, PromptDomain.STORYTELLING}
        self._academic_domains = {PromptDomain.RESEARCH, PromptDomain.EDUCATION, PromptDomain.ACADEMIC_WRITING, PromptDomain.SCIENTIFIC}
        self._business_domains = {PromptDomain.BUSINESS_ANALYSIS, PromptDomain.PROJECT_MANAGEMENT, PromptDomain.MARKETING, PromptDomain.CUSTOMER_SERVICE, PromptDomain.SALES}
    ...
    async def extract_features_async(self, text: str, context: dict[str, Any] | None=None) -> list[float]:
        ...
        if self.config.cache_enabled:
            ...
            if cached_result is not None:
                ...
                logger.debug('Cache hit for domain feature extraction [correlation_id=%s]', correlation_id)
                return cached_result
            self.metrics.cache_misses += 1
        try:
            async with self._extraction_semaphore:
                result, used_analyzer = await asyncio.wait_for(self._perform_extraction_async(validated_text, correlation_id), timeout=self.config.timeout_seconds)
            ...
            logger.error('Async domain feature extraction failed: %s [correlation_id=%s]', e, correlation_id)
            self._handle_circuit_breaker_failure()
            self.metrics.update_failure()
            return self._get_default_features()
    ...
    async def _validate_input_async(self, text: str) -> str | None:
        ...
        try:
            ...
            if len(text.strip()) < self.config.min_text_length:
                logger.debug('Text too short: {len(text)} < %s', self.config.min_text_length)
                return None
            if len(text) > self.config.max_text_length:
                logger.warning('Text too long: %s > %s, truncating', len(text), self.config.max_text_length)
                text = text[:self.config.max_text_length]
            sanitized_text = await asyncio.get_event_loop().run_in_executor(None, self.input_sanitizer.sanitize_html_input, text)
            return sanitized_text.strip()
        except Exception as e:
            logger.error('Async text validation failed: %s', e)
            return None

    def _generate_cache_key(self, text: str) -> str:
        """Generate cache key for text using secure hashing."""
        key_elements = [text[:500], str(self.config.weight), str(self.config.deterministic), str(self.domain_analyzer is not None), str(len(text))]
    ...
    async def _cache_features_async(self, cache_key: str, features: list[float]) -> None:
        ...
        logger.debug('Cached domain features for key: %s', cache_key)
    ...
    async def _perform_extraction_async(self, text: str, correlation_id: str | None) -> tuple[list[float], bool]:
        ...
        if self.domain_analyzer and self.config.use_domain_analyzer:
            try:
                features = await asyncio.get_event_loop().run_in_executor(None, self._extract_with_analyzer, text)
                normalized_features = self._normalize_features(features)
                used_analyzer = True
                logger.debug('Domain analyzer extracted %s features [correlation_id=%s]', len(normalized_features), correlation_id)
                return (normalized_features, used_analyzer)
            except Exception as e:
                logger.warning('Domain analyzer failed: %s, falling back to basic features [correlation_id=%s]', e, correlation_id)
                if not self.config.fallback_on_analyzer_failure:
                    raise
        features = await asyncio.get_event_loop().run_in_executor(None, self._extract_fallback_features, text)
        normalized_features = self._normalize_features(features)
    ...
    def _extract_with_analyzer(self, text: str) -> list[float]:
        ...
        try:
            ...
            features = [domain_features.confidence, domain_features.complexity_score, domain_features.specificity_score, 1.0 if domain_features.domain in self._technical_domains else 0.0, 1.0 if domain_features.domain in self._creative_domains else 0.0, 1.0 if domain_features.domain in self._academic_domains else 0.0, 1.0 if domain_features.domain in self._business_domains else 0.0, domain_features.technical_features.get('density', 0.5), domain_features.creative_features.get('density', 0.5) if hasattr(domain_features, 'creative_features') else 0.5, domain_features.academic_features.get('density', 0.5) if hasattr(domain_features, 'academic_features') else 0.5, domain_features.conversational_features.get('politeness_score', 0.5) if hasattr(domain_features, 'conversational_features') else 0.5, 1.0 if hasattr(domain_features, 'conversational_features') and domain_features.conversational_features.get('has_urgency', False) else 0.0, domain_features.conversational_features.get('question_density', 0.5) if hasattr(domain_features, 'conversational_features') else 0.5, domain_features.conversational_features.get('instruction_clarity', 0.5) if hasattr(domain_features, 'conversational_features') else 0.5, 1.0 if domain_features.hybrid_domain else 0.0]
            return features
        except Exception as e:
    ...
    def _handle_circuit_breaker_failure(self):
        ...
        if self.circuit_breaker_failures >= self.config.failure_threshold:
            ...
            logger.warning('Domain extractor circuit breaker opened after %s failures', self.circuit_breaker_failures)
    ...
    async def run_orchestrated_analysis(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if correlation_id:
                logger.info('Starting orchestrated domain analysis [correlation_id=%s]', correlation_id)
            if operation == 'extract_features':
                features = await self.extract_features_async(text, context)
                result = {'features': features, 'feature_names': self.get_feature_names(), 'feature_count': len(features), 'extraction_metadata': {'extractor_type': 'domain', 'config_weight': self.config.weight, 'text_length': len(text) if text else 0, 'analyzer_available': self.domain_analyzer is not None, 'analyzer_usage_rate': self.metrics.get_analyzer_usage_rate(), 'circuit_breaker_state': self.circuit_breaker_state.value, 'cache_enabled': self.config.cache_enabled, 'deterministic': self.config.deterministic}}
                ...
                if output_path:
                    ...
                    logger.info('Domain features saved to %s', output_path)
                return {'status': 'success', 'operation': operation, 'result': result, 'component': 'domain_feature_extractor', 'timestamp': datetime.now(timezone.utc).isoformat(), 'correlation_id': correlation_id}
            elif operation == 'health_check':
                health_status = self.get_health_status()
                return {'status': 'success', 'operation': operation, 'result': health_status, 'component': 'domain_feature_extractor', 'timestamp': datetime.now(timezone.utc).isoformat(), 'correlation_id': correlation_id}
            else:
                return {'status': 'error', 'error': f'Unsupported operation: {operation}', 'component': 'domain_feature_extractor', 'correlation_id': correlation_id}
        except Exception as e:
            logger.error('Orchestrated domain analysis failed: {e} [correlation_id=%s]', correlation_id)
            return {'status': 'error', 'error': str(e), 'component': 'domain_feature_extractor', 'correlation_id': correlation_id}
    ...
    def update_config(self, new_config: DomainFeatureConfig) -> None:
        ...
        if old_config.use_domain_analyzer != new_config.use_domain_analyzer and DOMAIN_ANALYSIS_AVAILABLE and new_config.use_domain_analyzer:
            try:
                self.domain_analyzer = DomainAnalyzer()
                logger.info('Domain analyzer reinitialized due to configuration change')
            except Exception as e:
                logger.warning('Failed to reinitialize domain analyzer: %s', e)
                self.domain_analyzer = None
