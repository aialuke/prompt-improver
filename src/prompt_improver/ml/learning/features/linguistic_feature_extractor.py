...
class LinguisticFeatureExtractor:
    ...
    def __init__(self, config: FeatureExtractionConfig | None=None, input_sanitizer: InputSanitizer | None=None, redis_cache=None):
        ...
        if LINGUISTIC_ANALYSIS_AVAILABLE:
            try:
                self.linguistic_analyzer = LinguisticAnalyzer(config=LinguisticConfig(enable_caching=self.config.cache_enabled, cache_size=1000, auto_download_nltk=False, nltk_fallback_enabled=True))
                logger.info('Linguistic analyzer initialized successfully')
            except Exception as e:
                logger.warning('Failed to initialize linguistic analyzer: %s', e)
    ...
    async def extract_features(self, request: FeatureExtractionRequest | str, context: dict[str, Any] | None=None) -> FeatureExtractionResponse:
        ...
        if isinstance(request, str):
            ...
            try:
                request = FeatureExtractionRequest(text=text, context=context, correlation_id=str(uuid.uuid4()))
            except Exception as e:
                correlation_id = str(uuid.uuid4())
                return self._create_default_response(correlation_id, 0.0, False)
        ...
        try:
            ...
            features = await self._compute_linguistic_features_async(validated_text, request.correlation_id)
            normalized_features = self._normalize_features(features)
            extraction_time = (time.perf_counter() - start_time) * 1000
            response = FeatureExtractionResponse(features=normalized_features, feature_names=self._feature_names, extraction_time_ms=extraction_time, cache_hit=cache_hit, correlation_id=request.correlation_id, confidence_score=self._calculate_confidence_score(features))
            if self.config.cache_enabled and self.redis_cache:
                await self._cache_features_async(validated_text, response)
            self.metrics.update_timing(extraction_time, cache_hit=False)
            logger.info('features extracted successfully', extra={'component': 'linguistic_feature_extractor', 'extraction_time_ms': extraction_time, 'cache_hit': cache_hit, 'correlation_id': request.correlation_id, 'text_length': len(validated_text)})
            return response
        except Exception as e:
            extraction_time = (time.perf_counter() - start_time) * 1000
    ...
    async def batch_extract_features(self, requests: list[FeatureExtractionRequest]) -> list[FeatureExtractionResponse]:
        ...
        for i in range(0, len(requests), batch_size):
            ...
            batch_results = await asyncio.gather(*[self.extract_features(request) for request in batch], return_exceptions=True)
            for j, result in enumerate(batch_results):
                if isinstance(result, Exception):
                    logger.error('Batch extraction failed for request {i+j}: %s', result)
                    results.append(self._create_default_response(batch[j].correlation_id, 0.0, False))
                else:
    ...
    async def _validate_and_sanitize_input(self, text: str) -> str | None:
        ...
        try:
            ...
            if len(text) > self.config.max_text_length:
                logger.warning('Text truncated from {len(text)} to %s characters', self.config.max_text_length)
                text = text[:self.config.max_text_length]
            sanitized_text = await asyncio.to_thread(self.input_sanitizer.sanitize_html_input, text)
    ...
    async def _get_cached_features(self, text: str, correlation_id: str) -> FeatureExtractionResponse | None:
        ...
        try:
            ...
            logger.warning('Cache retrieval failed: %s', e, extra={'correlation_id': correlation_id})
        return None

    async def _cache_features_async(self, text: str, response: FeatureExtractionResponse) -> None:
        ...
        try:
            ...
            await self.redis_cache.set_async(cache_key, cache_data, ttl=self.config.cache_ttl_seconds)
        except Exception as e:
    ...
    async def _extract_with_analyzer_async(self, text: str, correlation_id: str) -> list[float]:
        ...
        try:
            ...
            features = [linguistic_features.readability_score, linguistic_features.lexical_diversity, linguistic_features.entity_density, linguistic_features.syntactic_complexity, linguistic_features.sentence_structure_quality, linguistic_features.technical_term_ratio, min(1.0, linguistic_features.avg_sentence_length / 50.0), linguistic_features.instruction_clarity, 1.0 if linguistic_features.has_examples else 0.0, linguistic_features.overall_quality]
            return features
        except Exception as e:
    ...
    def _normalize_features(self, features: list[float]) -> list[float]:
        ...
        for i, feature in enumerate(features):
            try:
                ...
                logger.warning('Invalid feature value at index {i}: %s, using 0.5', feature)
    ...
    def _generate_cache_key(self, text: str) -> str:
        ...
        import hashlib
        config_hash = hashlib.md5(self.config.model_dump_json().encode(), usedforsecurity=False).hexdigest()[:8]
        text_hash = hashlib.md5(text.encode(), usedforsecurity=False).hexdigest()
        return f'linguistic_features:{config_hash}:{text_hash}'

    def _create_default_response(self, correlation_id: str, extraction_time: float, cache_hit: bool) -> FeatureExtractionResponse:
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'status': 'healthy', 'component': 'linguistic_feature_extractor', 'version': '2.0.0', 'timestamp': datetime.now(timezone.utc).isoformat(), 'metrics': {'total_extractions': self.metrics.total_extractions, 'cache_hit_rate': self.metrics.cache_hit_rate, 'error_rate': self.metrics.error_rate, 'average_response_time_ms': self.metrics.average_response_time_ms, 'health_check_time_ms': health_check_time}, 'configuration': {'cache_enabled': self.config.cache_enabled, 'linguistic_analyzer_available': self.linguistic_analyzer is not None, 'async_support': ASYNC_SUPPORT}}
        except Exception as e:
    ...
    def extract_features_sync(self, text: str, context: dict[str, Any] | None=None) -> list[float]:
        ...
        try:
            ...
            result = asyncio.run_coroutine_threadsafe(self.extract_features(request, context), loop).result()
    ...
    async def clear_cache(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'status': 'success', 'cleared_entries': cleared_count, 'timestamp': datetime.now(timezone.utc).isoformat()}
        except Exception as e:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            logger.info('LinguisticFeatureExtractor shutdown complete', extra=final_metrics)
        except Exception as e:
    ...
    def __repr__(self) -> str:
        ...
        return f'LinguisticFeatureExtractor(weight={self.config.weight}, cache_enabled={self.config.cache_enabled}, deterministic={self.config.deterministic}, extractions={self.metrics.total_extractions})'