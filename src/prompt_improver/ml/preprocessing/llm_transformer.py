...
class ConfigurablePatternProvider:
    ...
    def _get_default_patterns(self) -> Dict[str, Any]:
        ...
        return {
            "clarity": {
                "thing": {
                    "replacements": {
                        ...
                        "analysis": ["analysis report", "detailed analysis", "comprehensive review"],
                        "default": ["specific item", "particular element", "concrete example"]
                    },
                ...
                "stuff": {
                    "replacements": {
                        ...
                        "default": ["specific items", "particular elements", "relevant details"]
                    },
            ...
            "specificity": {
                "format_templates": {
                    "list": "Format: Please provide your response as a numbered list with clear bullet points.",
                    "summary": "Format: Please structure your response with key points in bullet format, limiting to 3-5 main ideas.",
                    "explanation": "Format: Please organize your explanation with clear headings and provide concrete examples for each main point."
                },
                "constraint_templates": {
                    "length": "Length: Aim for {min_words}-{max_words} words",
                    "items": "Limit: Include {min_items}-{max_items} main items",
                    "depth": "Depth: Focus on practical, actionable information"
                }
            }
        }
...
class MLPatternProvider:
    ...
    async def get_transformation_patterns(self) -> Dict[str, Any]:
        ...
        try:
            # Try to get ML-generated patterns
            ml_patterns = await self._generate_ml_patterns()
            if ml_patterns:
                self._cached_patterns = ml_patterns
                return ml_patterns
        except Exception as e:
            logger.warning(f"ML pattern generation failed: {e}, falling back to configurable patterns")

        # Fallback to configurable patterns
    ...
    async def _generate_ml_patterns(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Generate patterns using ML insights
            patterns = await self._discover_transformation_patterns()
            return patterns

        except Exception as e:
            logger.error(f"Failed to generate ML patterns: {e}")
            return None

    async def _discover_transformation_patterns(self) -> Dict[str, Any]:
        """Discover transformation patterns from historical data."""
        # This would use the ML service to analyze historical transformations
        # and discover effective patterns

        # For now, return enhanced default patterns with ML-derived confidence scores
        base_patterns = {
            "clarity": {
                "thing": {
                    "replacements": {
                        "document": ["document", "file", "text", "article", "manuscript"],
                        "data": ["dataset", "information", "data points", "records", "metrics"],
                        "analysis": ["analysis report", "detailed analysis", "comprehensive review", "evaluation"],
                        "default": ["specific item", "particular element", "concrete example", "defined object"]
                    },
                    "reason": "ML-enhanced replacement for 'thing' with context awareness",
                    "confidence": 0.92,  # ML-derived confidence
                    "ml_generated": True
                },
                "stuff": {
                    "replacements": {
                        "content": ["content", "material", "information", "substance"],
                        "data": ["data", "information", "details", "facts"],
                        "analysis": ["findings", "results", "insights", "conclusions"],
                        "default": ["specific items", "particular elements", "relevant details", "concrete components"]
                    },
                    "reason": "ML-enhanced replacement for 'stuff' with semantic understanding",
                    "confidence": 0.88,
                    "ml_generated": True
                }
            },
            "specificity": {
                "format_templates": {
                    "list": "Format: Please provide your response as a numbered list with clear bullet points and specific examples.",
                    "summary": "Format: Please structure your response with key points in bullet format, limiting to 3-5 main ideas with supporting details.",
                    "explanation": "Format: Please organize your explanation with clear headings, provide concrete examples for each main point, and include actionable insights.",
                    "analysis": "Format: Present your analysis with methodology, findings, and recommendations in clearly separated sections."
                },
                "constraint_templates": {
                    "length": "Length: Aim for {min_words}-{max_words} words with substantive content",
                    "items": "Limit: Include {min_items}-{max_items} main items with detailed explanations",
                    "depth": "Depth: Focus on practical, actionable information with specific examples and measurable outcomes",
                    "audience": "Audience: Tailor content for {audience_level} with appropriate technical depth"
                },
                "ml_confidence": 0.85
            }
        }
...
class LLMTransformerService:
    ...
    async def enhance_clarity(
        self, prompt: str, vague_words: List[str], context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        """
        with tracer.start_as_current_span("llm_transformer_enhance_clarity", attributes={
            "prompt_length": len(prompt),
            "vague_words_count": len(vague_words),
            "transformation_strategy": self.config.strategy.value
        }) as span:
            
            # Get current transformation patterns
            ...
            # Apply configurable transformations for each vague word
            for vague_word in vague_words[:self.config.max_transformations_per_prompt]:
                if vague_word in clarity_patterns:
                    ...
                    replacement = await self._find_best_replacement_async(
                    ...
                    if replacement:
                        ...
                        transformations.append({
                            "type": TransformationType.CLARITY_ENHANCEMENT.value,
    ...
    async def _find_best_replacement_async(
        self, vague_word: str, prompt: str, pattern_info: Dict[str, Any], context: Optional[Dict[str, Any]] = None
    ) -> Optional[str]:
        """Find the best context-appropriate replacement for a vague word."""
        with tracer.start_as_current_span("find_best_replacement", attributes={
            "vague_word": vague_word,
            "prompt_length": len(prompt)
        }):
            replacements = pattern_info.get("replacements", {})

            # If no context provided, use default replacement
            if not context:
            ...
            # Try domain-specific replacement first
            ...
            # Try task-type specific replacement
    ...
    async def _generate_clarity_guidance_async(
        self, prompt: str, vague_words: List[str], patterns: Dict[str, Any]
    ) -> Optional[str]:
        """Generate contextual guidance for improving prompt clarity."""
        with tracer.start_as_current_span("generate_clarity_guidance", attributes={
            "vague_words_count": len(vague_words),
            "prompt_length": len(prompt)
        }):
            if not vague_words:
            ...
            # Add specific guidance for vague words
            ...
            clarity_patterns = patterns.get("clarity", {})
            for vague_word in vague_words[:3]:  # Limit to first 3 for brevity
                if vague_word in clarity_patterns:
                    pattern_info = clarity_patterns[vague_word]
    ...
    def _analyze_prompt_structure(self, prompt: str) -> Dict[str, bool]:
        ...
        with tracer.start_as_current_span("analyze_prompt_structure", attributes={
            "prompt_length": len(prompt)
        }):
            analysis = {
                "lacks_format_specification": True,
                "lacks_constraints": True,
                "needs_examples": True,
                "has_examples": False,
                "has_clear_task": True,
                "has_context": False
    ...
    def _suggest_format_specification(self, prompt: str, context: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        # Determine appropriate format based on prompt content
    ...
    def _suggest_constraints(self, prompt: str, context: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        # Length constraints based on prompt type
        prompt_lower = prompt.lower()
