...
class ResourceMonitor:
    ...
    async def get_current_resources(self) -> ResourceSnapshot:
        ...
        try:
            ...
            snapshot = ResourceSnapshot(
                ...
                cpu_count=cpu_count,
                load_avg_1m=load_avg['1m'] if load_avg else None,
                load_avg_5m=load_avg['5m'] if load_avg else None,
                load_avg_15m=load_avg['15m'] if load_avg else None,
                memory_total_gb=memory_total_gb,
                memory_available_gb=memory_available_gb,
                memory_used_gb=memory_used_gb,
                memory_percent=memory_percent,
                gpu_count=gpu_metrics.get('gpu_count', 0),
                gpu_total_memory_gb=gpu_metrics.get('gpu_total_memory_gb', 0.0),
                gpu_used_memory_gb=gpu_metrics.get('gpu_used_memory_gb', 0.0),
            ...
            # Return minimal snapshot with error indicator
    ...
    async def _get_gpu_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'gpu_used_memory_gb': used_memory_gb,
                'gpu_utilization_percent': total_utilization / max(gpu_count, 1)
            }
    ...
    async def get_resource_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            if not recent_snapshots:
                # Get current snapshot if no history
            ...
            gpu_memory_values = [
                (s.gpu_used_memory_gb / max(s.gpu_total_memory_gb, 1)) * 100
                for s in recent_snapshots if s.gpu_total_memory_gb > 0
            ]
            ...
            return {
                ...
                # CPU summary
                "cpu_summary": {
                    "avg_percent": float(sum(cpu_values) / len(cpu_values)) if cpu_values else 0.0,
                    "max_percent": float(max(cpu_values)) if cpu_values else 0.0,
                    "min_percent": float(min(cpu_values)) if cpu_values else 0.0,
                    "cpu_count": recent_snapshots[0].cpu_count if recent_snapshots else 0
                },
                
                # Memory summary
                "memory_summary": {
                    "avg_percent": float(sum(memory_values) / len(memory_values)) if memory_values else 0.0,
                    "max_percent": float(max(memory_values)) if memory_values else 0.0,
                    "total_gb": recent_snapshots[0].memory_total_gb if recent_snapshots else 0.0,
                    "current_available_gb": recent_snapshots[-1].memory_available_gb if recent_snapshots else 0.0
                },
                
                # GPU summary
                "gpu_summary": {
                    "available": self._gpu_available,
                    "gpu_count": recent_snapshots[0].gpu_count if recent_snapshots else 0,
                    "avg_utilization_percent": float(sum(gpu_util_values) / len(gpu_util_values)) if gpu_util_values else 0.0,
                    "max_utilization_percent": float(max(gpu_util_values)) if gpu_util_values else 0.0,
                    "avg_memory_percent": float(sum(gpu_memory_values) / len(gpu_memory_values)) if gpu_memory_values else 0.0,
                    "total_memory_gb": recent_snapshots[0].gpu_total_memory_gb if recent_snapshots else 0.0
                },
                
                # Disk summary
                "disk_summary": {
                    "avg_percent": float(sum(disk_values) / len(disk_values)) if disk_values else 0.0,
                    "max_percent": float(max(disk_values)) if disk_values else 0.0,
                    "total_gb": recent_snapshots[0].disk_total_gb if recent_snapshots else 0.0,
                    "current_free_gb": recent_snapshots[-1].disk_free_gb if recent_snapshots else 0.0
                },
                
                # Alerts and recommendations
                "alerts": alerts,
                "resource_health": self._assess_resource_health(recent_snapshots[-1] if recent_snapshots else None)
            }
    ...
    def _generate_resource_alerts(self, current_snapshot: Optional[ResourceSnapshot]) -> List[Dict[str, Any]]:
        ...
        if current_snapshot.cpu_percent > self.cpu_alert_threshold:
            alerts.append({
                "type": "cpu_high",
                "severity": "warning" if current_snapshot.cpu_percent < 95 else "critical",
                "message": f"High CPU utilization: {current_snapshot.cpu_percent:.1f}%",
                "current_value": current_snapshot.cpu_percent,
        ...
        if current_snapshot.memory_percent > self.memory_alert_threshold:
            alerts.append({
                "type": "memory_high", 
                "severity": "warning" if current_snapshot.memory_percent < 95 else "critical",
                "message": f"High memory utilization: {current_snapshot.memory_percent:.1f}%",
                "current_value": current_snapshot.memory_percent,
                "threshold": self.memory_alert_threshold
            })
        
        # Low memory alert
        if current_snapshot.memory_available_gb < 1.0:  # Less than 1GB available
            alerts.append({
                "type": "memory_low",
                "severity": "critical",
                "message": f"Low available memory: {current_snapshot.memory_available_gb:.1f}GB",
                "current_value": current_snapshot.memory_available_gb,
                "threshold": 1.0
            })
        ...
        if current_snapshot.gpu_count > 0:
            ...
            if gpu_memory_percent > self.gpu_memory_alert_threshold:
                alerts.append({
                    ...
                    "severity": "warning",
                    "message": f"High GPU memory utilization: {gpu_memory_percent:.1f}%",
                    "current_value": gpu_memory_percent,
                    "threshold": self.gpu_memory_alert_threshold
                })
        ...
        if current_snapshot.disk_percent > self.disk_alert_threshold:
            alerts.append({
                "type": "disk_high",
                "severity": "warning" if current_snapshot.disk_percent < 95 else "critical",
                "message": f"High disk utilization: {current_snapshot.disk_percent:.1f}%",
                "current_value": current_snapshot.disk_percent,
    ...
    def _assess_resource_health(self, current_snapshot: Optional[ResourceSnapshot]) -> Dict[str, Any]:
        ...
        # Calculate health scores (0-1, higher is better)
        ...
        gpu_health = 1.0  # Default to healthy if no GPU
        ...
        return {
            ...
            "component_scores": {
                "cpu": float(cpu_health),
                "memory": float(memory_health),
                "disk": float(disk_health),
                "gpu": float(gpu_health)
            },
    ...
    def _generate_health_recommendations(
        self, snapshot: ResourceSnapshot, health_score: float
    ) -> List[str]:
