...
async def example_ml_pipeline_with_health_monitoring():
    ...
    await health_manager.register_model_for_monitoring(model_id=model_id, model=model, model_type='RandomForestClassifier', version='1.0.0', metadata={'features': 20, 'classes': 2})
    print(f'✅ Registered model {model_id} for health monitoring')
    print('\n🔄 Simulating inference requests...')
    for i in range(50):
        sample_features = X[i:i + 1]
        request_id = f'req_{i:03d}'
        await health_manager.track_inference(model_id=model_id, request_id=request_id, features=sample_features[0].tolist())
        start_time = time.time()
        try:
            prediction = model.predict(sample_features)[0]
            prediction_proba = model.predict_proba(sample_features)[0]
            confidence = max(prediction_proba)
            await asyncio.sleep(0.001 + i % 10 * 0.001)
            latency_ms = (time.time() - start_time) * 1000
            await health_manager.complete_inference_tracking(model_id=model_id, request_id=request_id, success=True, latency_ms=latency_ms)
            await health_manager.track_inference(model_id=model_id, request_id=request_id + '_drift', prediction=float(prediction), confidence=confidence, features=sample_features[0].tolist(), start_performance_tracking=False)
        except Exception as e:
            await health_manager.complete_inference_tracking(model_id=model_id, request_id=request_id, success=False, error_type=type(e).__name__)
        if (i + 1) % 10 == 0:
