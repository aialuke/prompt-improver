...
class MLModelService:
    ...
    def __init__(self, db_manager=None, orchestrator_event_bus=None):
        ...
        # Initialize input validator for ML operations
        self.input_validator = InputValidator()

        # Optional orchestrator integration (backward compatible)
    ...
    async def predict_rule_effectiveness(
        ...
        try:
            ...
            try:
                # Validate model_id
                validated_model_id = self.input_validator.validate_input(
                    "model_id", model_id, "session_id"  # Reuse session_id schema for alphanumeric validation
                )

                # Validate rule_features
                if not isinstance(rule_features, list):
                    return {"error": "rule_features must be a list", "status": "validation_error"}

                if len(rule_features) == 0:
                    return {"error": "rule_features cannot be empty", "status": "validation_error"}

                if len(rule_features) > 1000:  # Reasonable limit
                    return {"error": "rule_features too large (max 1000 features)", "status": "validation_error"}

                # Validate each feature is numeric
                validated_features = []
                for i, feature in enumerate(rule_features):
                    if not isinstance(feature, (int, float)):
                        return {"error": f"Feature at index {i} must be numeric", "status": "validation_error"}
                    if not np.isfinite(feature):
                        return {"error": f"Feature at index {i} must be finite", "status": "validation_error"}
                    validated_features.append(float(feature))

            except ValidationError as e:
                return {"error": f"Input validation failed: {str(e)}", "status": "validation_error"}
            except Exception as e:
                return {"error": f"Input validation error: {str(e)}", "status": "validation_error"}

            # Try to get model from cache first
            model = self.model_registry.get_model(validated_model_id)

            if model is None:
                # Try to load from MLflow if not in cache
                model = await self._lazy_load_model(validated_model_id)

            if model is None:
                return {"error": f"Model {validated_model_id} not found in cache or MLflow"}

            # Create features array with validated data
            features = np.array([validated_features])
            ...
            return {
                ...
                "features_validated": len(validated_features),
            }
        ...
        try:
            ...
            if operation == "train":
                ...
                if "batch" in training_data:
                    result = await self.send_training_batch(training_data["batch"])
                    operation_metadata = {
                        "operation_type": "batch_training",
                        "batch_size": len(training_data["batch"])
                    }
                elif "rules" in training_data:
                    # Create a mock training result for rules
                    result = {
                        "status": "success",
                        "model_id": f"rule_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                        "training_samples": len(training_data["rules"]),
                        "model_type": model_config.get("model_type", "random_forest")
                    }
                    operation_metadata = {
                        "operation_type": "rule_training",
                        "model_type": model_config.get("model_type", "random_forest"),
                        "rules_count": len(training_data["rules"])
                    }
                else:
                ...
                if "features" in prediction_data:
                    ...
                    operation_metadata = {
                        ...
                        "model_id": model_id,
                        "predictions_count": len(prediction_data["features"])
                    }
                else:
                ...
                if "rules" in training_data:
                    # Use the actual optimize_rules method but need to provide db_session
                    from prompt_improver.database import get_session
                    async with get_session() as db_session:
                        result = await self.optimize_rules(
                            training_data,
                            db_session,
                            rule_ids=optimization_config.get("rule_ids", [])
                        )
                    operation_metadata = {
            ...
            orchestrator_result = {
                ...
                "local_metadata": {
                    ...
                    "component_name": "MLModelService",
                    "operation_timestamp": start_time.isoformat(),
                    "configuration": config,
                    "model_registry_stats": {
                        "cached_models": len(self.model_registry._cache),
                        "cache_size_mb": self.model_registry._total_cache_size_mb
                    }
                }
            }
