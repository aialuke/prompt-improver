...
class LinguisticConfig:
    ...
    use_lightweight_models: bool = (
        False  # Use for testing/resource-constrained environments
    )
    use_ultra_lightweight_models: bool = (
        False  # Use for extreme memory constraints (<30MB)
    )
    ...
    technical_keywords: set[str] = field(
...
class LinguisticAnalyzer:
        ...
        try:
            ...
            if self.config.auto_download_nltk:
                ...
                if not setup_success:
                    self.logger.warning(
                        "NLTK setup incomplete, some features may be limited"
                    )

            # Initialize model manager if using transformers
            if self.config.use_transformers_ner:
                if (
                    self.config.use_ultra_lightweight_models
                    or self.config.use_lightweight_models
                ):
                    ...
                    if self.config.enable_4bit_quantization:
                        ...
                        model_config = get_memory_optimized_config(
                            self.config.max_memory_threshold_mb
                        )
                        self.logger.info(
                            f"Using memory-optimized config for {self.config.max_memory_threshold_mb}MB target"
                        )
                    else:
        ...
        try:
            if self.config.use_ultra_lightweight_models:
                ...
                if self.transformers_pipeline:
                    self.logger.info(
                        "Initialized ultra-lightweight transformers NER pipeline (<30MB)"
                    )
                else:
                    self.logger.warning(
                        "Failed to initialize ultra-lightweight pipeline"
                    )
                ...
                if self.transformers_pipeline:
                    self.logger.info(
                        "Initialized lightweight transformers NER pipeline"
                    )
                else:
                ...
                if self.transformers_pipeline:
                    ...
                    self.logger.info(
                        f"Initialized optimized transformers NER pipeline (Memory: {memory_usage:.1f}MB)"
                    )
                else:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Failed to initialize basic transformers pipeline: {e}"
                    )
                    self.config.use_transformers_ner = False
    ...
    async def analyze_async(self, text: str) -> LinguisticFeatures:
        ...
        try:
            ...
            features.overall_linguistic_quality = self._calculate_overall_quality(
                features
            )
        ...
        try:
            ...
            tasks = [
                ...
                loop.run_in_executor(
                    self.executor, textstat.flesch_kincaid_grade, text
                ),
    ...
    async def _analyze_prompt_structure(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Check for context
            has_context = any(
                re.search(pattern, text, re.IGNORECASE) for pattern in context_patterns
            )
    ...
    def _calculate_readability_score(
        self, flesch_ease: float, flesch_kincaid: float, gunning_fog: float
    ) -> float:
        """Calculate composite readability score."""
        ...
        return (
            ...
            + kincaid_normalized * 0.3
            + gunning_normalized * 0.3
        )
    ...
    def _calculate_overall_quality(self, features: LinguisticFeatures) -> float:
        ...
        readability_component = (
            min(features.readability_score, 1.0) * readability_weight
        )
        ...
        entity_richness = min(
            features.entity_density * 3.0, 1.0
        )  # Scale entity density
        lexical_richness = features.lexical_diversity
        technical_richness = min(
            len(features.technical_terms) / 10.0, 1.0
        )  # Scale technical terms
