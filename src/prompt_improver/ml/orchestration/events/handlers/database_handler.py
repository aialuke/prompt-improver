...
class DatabaseEventHandler:
    ...
    async def handle_event(self, event: MLEvent) -> None:
        ...
        self.logger.debug('Handling database event: %s', event.event_type)
        try:
            ...
            if event.event_type == EventType.DATABASE_PERFORMANCE_SNAPSHOT_TAKEN:
                ...
                self.logger.warning('Unknown database event type: %s', event.event_type)
        except Exception as e:
            self.events_failed += 1
            self.logger.error('Error handling database event {event.event_type}: %s', e)
            if self.event_bus:
                await self.event_bus.emit(MLEvent(event_type=EventType.ALERT_TRIGGERED, source='database_handler', data={'error': str(e), 'original_event': event.event_type.value, 'severity': 'error'}))
    ...
    async def _handle_performance_snapshot(self, event: MLEvent) -> None:
        ...
        self.performance_snapshots.append({'timestamp': event.data.get('timestamp', datetime.now(timezone.utc).isoformat()), 'data': snapshot_data})
        ...
        self.logger.debug('Processed performance snapshot: cache_hit={cache_hit_ratio}%, query_time=%sms', avg_query_time)
    ...
    async def _handle_cache_hit_ratio_low(self, event: MLEvent) -> None:
        ...
        self.logger.warning('Low cache hit ratio detected: %s%', cache_ratio)
        if self.orchestrator and hasattr(self.orchestrator, 'get_component'):
            try:
                ...
                self.logger.error('Failed to trigger optimization: %s', e)
    ...
    async def _handle_slow_query_detected(self, event: MLEvent) -> None:
        ...
        self.logger.warning('Slow query detected: %sms', execution_time)
        slow_query_data = {'timestamp': datetime.now(timezone.utc).isoformat(), 'query_text': query_info.get('query_text', ''), 'execution_time_ms': execution_time, 'calls': query_info.get('calls', 1)}
        if self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.ALERT_TRIGGERED, source='database_handler', data={'alert_type': 'slow_query', 'query_data': slow_query_data, 'severity': 'warning'}))

    async def _handle_connection_optimized(self, event: MLEvent) -> None:
        """Handle database connection optimized event."""
        optimization_data = {'timestamp': datetime.now(timezone.utc).isoformat(), 'optimization_type': 'connection_settings', 'applied_settings': event.data.get('applied_settings', {}), 'system_resources': event.data.get('system_resources', {})}
    ...
    async def _handle_indexes_created(self, event: MLEvent) -> None:
        ...
        self.logger.info('Performance indexes created: %s', index_data)
        optimization_data = {'timestamp': datetime.now(timezone.utc).isoformat(), 'optimization_type': 'index_creation', 'indexes_created': index_data.get('indexes_created', []), 'performance_impact': index_data.get('performance_impact', {})}
        self.optimization_history.append(optimization_data)
    ...
    async def _handle_performance_degraded(self, event: MLEvent) -> None:
        ...
        self.logger.error('Database performance degraded: %s', degradation_data)
        if self.orchestrator and hasattr(self.orchestrator, 'get_component'):
            try:
                optimizer = await self.orchestrator.get_component('database_connection_optimizer')
                ...
                self.logger.error('Failed to trigger emergency optimization: %s', e)
    ...
    async def _handle_performance_improved(self, event: MLEvent) -> None:
        ...
        self.logger.info('Database performance improved: %s', improvement_data)
    ...
    async def _handle_resource_analysis_completed(self, event: MLEvent) -> None:
        ...
        self.logger.info('Database resource analysis completed: %s', analysis_data)

    async def _emit_cache_hit_ratio_alert(self, cache_ratio: float) -> None:
        """Emit cache hit ratio alert."""
        if self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.DATABASE_CACHE_HIT_RATIO_LOW, source='database_handler', data={'cache_hit_ratio': cache_ratio, 'threshold': self.alert_thresholds['cache_hit_ratio_min'], 'severity': 'warning'}))

    async def _emit_slow_query_alert(self, avg_query_time: float) -> None:
        """Emit slow query alert."""
        if self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.DATABASE_SLOW_QUERY_DETECTED, source='database_handler', data={'avg_query_time_ms': avg_query_time, 'threshold': self.alert_thresholds['query_time_max_ms'], 'severity': 'warning'}))
