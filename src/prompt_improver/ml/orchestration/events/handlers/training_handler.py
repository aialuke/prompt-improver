...
class TrainingEventHandler:
    ...
    async def handle_event(self, event: MLEvent) -> None:
        ...
        self.logger.debug('Handling training event: %s', event.event_type)
        try:
            ...
            if event.event_type == EventType.TRAINING_STARTED:
                ...
                self.logger.warning('Unknown training event type: %s', event.event_type)
        except Exception as e:
            self.events_failed += 1
            self.logger.error('Error handling training event {event.event_type}: %s', e)
            if self.event_bus:
                await self.event_bus.emit(MLEvent(event_type=EventType.EVENT_HANDLER_ERROR, source='training_handler', data={'original_event': event.event_type.value, 'error_message': str(e), 'handler': 'training_handler'}))

    async def _handle_training_started(self, event: MLEvent) -> None:
        """Handle training started event."""
        self.logger.info('Training started: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        if workflow_id:
            self.active_training_sessions[workflow_id] = {'started_at': datetime.now(timezone.utc), 'status': 'training', 'parameters': event.data.get('parameters', {}), 'source': event.source}
        if self.orchestrator and hasattr(self.orchestrator, 'resource_manager'):
            try:
                await self.orchestrator.resource_manager.allocate_training_resources(workflow_id, event.data.get('parameters', {}))
            except Exception as e:
                self.logger.warning('Failed to allocate training resources: %s', e)
        self.logger.info('Training session %s registered and resources allocated', workflow_id)

    async def _handle_training_data_loaded(self, event: MLEvent) -> None:
        """Handle training data loaded event."""
        self.logger.info('Training data loaded: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        ...
        if data_validation_enabled and self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.DATA_VALIDATION_REQUESTED, source='training_handler', data={'workflow_id': workflow_id, 'data_source': event.data.get('data_source'), 'validation_type': 'training_data'}))

    async def _handle_training_completed(self, event: MLEvent) -> None:
        """Handle training completed event."""
        self.logger.info('Training completed: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        if workflow_id in self.active_training_sessions:
        ...
        if self.orchestrator and hasattr(self.orchestrator, 'resource_manager'):
            try:
                ...
                self.logger.warning('Failed to release training resources: %s', e)
        auto_evaluate = event.data.get('auto_evaluate', True)
        if auto_evaluate and self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.EVALUATION_REQUESTED, source='training_handler', data={'workflow_id': workflow_id, 'model_id': event.data.get('model_id'), 'trigger': 'training_completion'}))
        self.logger.info('Training session %s completed successfully', workflow_id)

    async def _handle_training_failed(self, event: MLEvent) -> None:
        """Handle training failed event."""
        self.logger.error('Training failed: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        if workflow_id in self.active_training_sessions:
            session = self.active_training_sessions[workflow_id]
        ...
        if self.orchestrator and hasattr(self.orchestrator, 'resource_manager'):
            try:
                ...
                self.logger.warning('Failed to release training resources after failure: %s', e)
        analyze_failures = event.data.get('analyze_failures', True)
        if analyze_failures and self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.FAILURE_ANALYSIS_REQUESTED, source='training_handler', data={'workflow_id': workflow_id, 'failure_type': 'training_failure', 'error_message': event.data.get('error_message'), 'context': event.data}))

    async def _handle_model_trained(self, event: MLEvent) -> None:
        """Handle model trained event."""
        self.logger.info('Model trained: %s', event.data)
        ...
        if auto_register and self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.MODEL_REGISTRATION_REQUESTED, source='training_handler', data={'model_id': model_id, 'workflow_id': workflow_id, 'model_metadata': event.data.get('model_metadata', {}), 'trigger': 'training_completion'}))
    ...
    async def _handle_hyperparameter_update(self, event: MLEvent) -> None:
        ...
        if workflow_id in self.active_training_sessions:
            ...
            session['hyperparameter_updates'].append({'timestamp': datetime.now(timezone.utc), 'updates': event.data.get('hyperparameters', {}), 'source': event.source})
    ...
    async def cleanup_completed_sessions(self, max_history: int=100) -> None:
        ...
        completed_sessions = [(workflow_id, session) for workflow_id, session in self.active_training_sessions.items() if session.get('status') in ['completed', 'failed']]
        ...
        if len(completed_sessions) > max_history:
            ...
            self.logger.info('Cleaned up %s completed training sessions', len(sessions_to_remove))

    def get_supported_events(self) -> List[EventType]:
        """Get list of supported event types."""
        return [EventType.TRAINING_STARTED, EventType.TRAINING_DATA_LOADED, EventType.TRAINING_COMPLETED, EventType.TRAINING_FAILED, EventType.MODEL_TRAINED, EventType.HYPERPARAMETER_UPDATE]
