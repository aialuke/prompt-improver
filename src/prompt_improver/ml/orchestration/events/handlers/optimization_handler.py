...
class OptimizationEventHandler:
    ...
    async def handle_event(self, event: MLEvent) -> None:
        ...
        self.logger.debug(f"Handling optimization event: {event.event_type}")
        ...
        try:
            ...
            if event.event_type == EventType.OPTIMIZATION_STARTED:
                ...
                self.logger.warning(f"Unknown optimization event type: {event.event_type}")
                
        except Exception as e:
            self.events_failed += 1
            self.logger.error(f"Error handling optimization event {event.event_type}: {e}")
            
            if self.event_bus:
                await self.event_bus.emit(MLEvent(
                    event_type=EventType.EVENT_HANDLER_ERROR,
                    source="optimization_handler",
                    data={
    ...
    async def _handle_optimization_started(self, event: MLEvent) -> None:
        ...
        self.logger.info(f"Optimization started: {event.data}")
        ...
        if self.orchestrator and hasattr(self.orchestrator, 'resource_manager'):
            try:
                await self.orchestrator.resource_manager.allocate_optimization_resources(
                    workflow_id, event.data.get("parameters", {})
                )
            except Exception as e:
                self.logger.warning(f"Failed to allocate optimization resources: {e}")
        
        self.logger.info(f"Optimization session {workflow_id} registered")
    
    async def _handle_optimization_completed(self, event: MLEvent) -> None:
        """Handle optimization completed event."""
        self.logger.info(f"Optimization completed: {event.data}")
        ...
        if self.orchestrator and hasattr(self.orchestrator, 'resource_manager'):
            try:
                ...
                self.logger.warning(f"Failed to release optimization resources: {e}")
        
        # Trigger model update if optimization improved performance
        best_result = event.data.get("best_result", {})
        if best_result.get("improved", False) and self.event_bus:
            await self.event_bus.emit(MLEvent(
                event_type=EventType.MODEL_UPDATE_REQUESTED,
                source="optimization_handler",
                data={
                    "workflow_id": workflow_id,
                    "optimization_result": best_result,
                    "trigger": "optimization_improvement"
                }
            ))
        
        self.logger.info(f"Optimization session {workflow_id} completed successfully")
    
    async def _handle_optimization_failed(self, event: MLEvent) -> None:
        """Handle optimization failed event."""
        self.logger.error(f"Optimization failed: {event.data}")
        ...
        if self.orchestrator and hasattr(self.orchestrator, 'resource_manager'):
            try:
                ...
                self.logger.warning(f"Failed to release optimization resources after failure: {e}")
        ...
        if self.event_bus:
            await self.event_bus.emit(MLEvent(
                ...
                data={
                    "workflow_id": workflow_id,
                    "failure_type": "optimization_failure",
    ...
    async def _handle_optimization_iteration(self, event: MLEvent) -> None:
        ...
        self.logger.debug(f"Optimization iteration: {event.data}")
        ...
        if workflow_id in self.active_optimizations:
            ...
            if optimization["iterations"] % 10 == 0:
                self.logger.info(f"Optimization {workflow_id} progress: {optimization['iterations']} iterations, best score: {optimization.get('best_score', 'N/A')}")
    ...
    async def get_handler_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_optimizations": len([o for o in self.active_optimizations.values() if o.get("status") == "running"]),
