...
class EvaluationEventHandler:
    ...
    async def handle_event(self, event: MLEvent) -> None:
        ...
        self.logger.debug('Handling evaluation event: %s', event.event_type)
        try:
            ...
            if event.event_type == EventType.EVALUATION_STARTED:
                ...
                self.logger.warning('Unknown evaluation event type: %s', event.event_type)
        except Exception as e:
            self.events_failed += 1
            self.logger.error('Error handling evaluation event {event.event_type}: %s', e)
            if self.event_bus:
                await self.event_bus.emit(MLEvent(event_type=EventType.EVENT_HANDLER_ERROR, source='evaluation_handler', data={'original_event': event.event_type.value, 'error_message': str(e), 'handler': 'evaluation_handler'}))

    async def _handle_evaluation_started(self, event: MLEvent) -> None:
        """Handle evaluation started event."""
        self.logger.info('Evaluation started: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        if workflow_id:
            self.active_evaluations[workflow_id] = {'started_at': datetime.now(timezone.utc), 'status': 'running', 'type': event.data.get('evaluation_type', 'standard'), 'parameters': event.data.get('parameters', {}), 'source': event.source, 'experiments': [], 'validation_results': {}, 'metrics': {}}
        if self.orchestrator and hasattr(self.orchestrator, 'resource_manager'):
            try:
                await self.orchestrator.resource_manager.allocate_evaluation_resources(workflow_id, event.data.get('parameters', {}))
            except Exception as e:
                self.logger.warning('Failed to allocate evaluation resources: %s', e)
        self.logger.info('Evaluation session %s registered', workflow_id)

    async def _handle_evaluation_completed(self, event: MLEvent) -> None:
        """Handle evaluation completed event."""
        self.logger.info('Evaluation completed: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        ...
        if self.orchestrator and hasattr(self.orchestrator, 'resource_manager'):
            try:
                ...
                self.logger.warning('Failed to release evaluation resources: %s', e)
        ...
        if auto_deploy and results.get('passed_thresholds', False) and self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.DEPLOYMENT_REQUESTED, source='evaluation_handler', data={'workflow_id': workflow_id, 'evaluation_results': results, 'trigger': 'evaluation_success'}))
        self.logger.info('Evaluation session %s completed successfully', workflow_id)

    async def _handle_evaluation_failed(self, event: MLEvent) -> None:
        """Handle evaluation failed event."""
        self.logger.error('Evaluation failed: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        if workflow_id in self.active_evaluations:
            evaluation = self.active_evaluations[workflow_id]
        ...
        if self.orchestrator and hasattr(self.orchestrator, 'resource_manager'):
            try:
                ...
                self.logger.warning('Failed to release evaluation resources after failure: %s', e)
        if self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.FAILURE_ANALYSIS_REQUESTED, source='evaluation_handler', data={'workflow_id': workflow_id, 'failure_type': 'evaluation_failure', 'error_message': event.data.get('error_message'), 'context': event.data}))

    async def _handle_experiment_created(self, event: MLEvent) -> None:
        """Handle experiment created event."""
        self.logger.info('Experiment created: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        experiment_id = event.data.get('experiment_id')
        if workflow_id in self.active_evaluations and experiment_id:
            evaluation = self.active_evaluations[workflow_id]
            evaluation['experiments'].append({'experiment_id': experiment_id, 'created_at': datetime.now(timezone.utc), 'type': event.data.get('experiment_type'), 'status': 'created'})
        self.logger.info('Experiment {experiment_id} registered for evaluation %s', workflow_id)

    async def _handle_experiment_completed(self, event: MLEvent) -> None:
        """Handle experiment completed event."""
        self.logger.info('Experiment completed: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        experiment_id = event.data.get('experiment_id')
        if workflow_id in self.active_evaluations:
            evaluation = self.active_evaluations[workflow_id]
    ...
    async def _handle_data_validation_completed(self, event: MLEvent) -> None:
        ...
        self.logger.info('Data validation completed: %s', event.data)
        workflow_id = event.data.get('workflow_id')
        if workflow_id in self.active_evaluations:
            evaluation = self.active_evaluations[workflow_id]
        ...
        if validation_passed and self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.EVALUATION_CONTINUE, source='evaluation_handler', data={'workflow_id': workflow_id, 'validation_passed': True}))
