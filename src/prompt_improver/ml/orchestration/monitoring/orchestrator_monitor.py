...
class OrchestratorMonitor:
    ...
    def __init__(self, orchestrator=None, event_bus=None, config=None):
        ...
        self.orchestrator_check_interval = self.config.get('orchestrator_check_interval', 10)
    ...
    async def start_monitoring(self) -> None:
        ...
        self.monitor_task_id = await task_manager.submit_enhanced_task(task_id=f'ml_orchestrator_monitor_{str(uuid.uuid4())[:8]}', coroutine=self._monitoring_loop(), priority=TaskPriority.HIGH, tags={'service': 'ml', 'type': 'monitoring', 'component': 'orchestrator_monitor', 'module': 'orchestrator_monitor'})
        if self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.MONITORING_STARTED, source='orchestrator_monitor', data={'monitor_type': 'orchestrator', 'started_at': datetime.now(timezone.utc).isoformat()}))
    ...
    async def stop_monitoring(self) -> None:
        ...
        if self.monitor_task:
            ...
            try:
                ...
                pass
        if self.event_bus:
            await self.event_bus.emit(MLEvent(event_type=EventType.MONITORING_STOPPED, source='orchestrator_monitor', data={'monitor_type': 'orchestrator', 'stopped_at': datetime.now(timezone.utc).isoformat()}))
    ...
    async def _collect_system_metrics(self) -> None:
        ...
        try:
            ...
            metrics = SystemMetrics(cpu_usage=cpu_usage, memory_usage=memory_usage, memory_total=memory_total, disk_usage=disk_usage, disk_total=disk_total, network_io=network_io, process_count=process_count, thread_count=thread_count, timestamp=datetime.now(timezone.utc))
    ...
    async def _collect_orchestrator_metrics(self) -> None:
        ...
        try:
            ...
            metrics = OrchestratorMetrics(active_workflows=active_workflows, completed_workflows=completed_workflows, failed_workflows=failed_workflows, active_components=active_components, event_queue_size=event_queue_size, event_processing_rate=event_processing_rate, average_workflow_duration=average_workflow_duration, resource_utilization=resource_utilization, timestamp=datetime.now(timezone.utc))
            ...
            self.logger.error('Error collecting orchestrator metrics: %s', e)
    ...
    async def _assess_health(self) -> None:
        ...
        try:
            ...
            if new_health != self.current_health:
                self.logger.info('Orchestrator health changed from {self.current_health.value} to %s', new_health.value)
                if self.event_bus:
                    await self.event_bus.emit(MLEvent(event_type=EventType.HEALTH_STATUS_CHANGED, source='orchestrator_monitor', data={'previous_health': self.current_health.value, 'current_health': new_health.value, 'health_factors': health_factors, 'timestamp': datetime.now(timezone.utc).isoformat()}))
                self.current_health = new_health
            self.last_health_check = datetime.now(timezone.utc)
        except Exception as e:
            self.logger.error('Error assessing health: %s', e)
            self.current_health = HealthStatus.UNKNOWN
