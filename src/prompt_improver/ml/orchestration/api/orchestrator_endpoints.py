...
class OrchestratorEndpoints:
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if component_registry:
                ...
                active_components = 0
            return HealthStatusResponse(overall_health=overall_health, orchestrator_status='running' if self.orchestrator else 'stopped', active_workflows=active_workflows, active_components=active_components, last_check=datetime.utcnow().isoformat(), details={'monitor_status': monitor_status, 'workflow_stats': workflow_stats if workflow_engine else {}, 'component_stats': component_stats if component_registry else {}}).model_dump()
    ...
    async def create_workflow(self, request: WorkflowRequest, background_tasks: BackgroundTasks) -> Dict[str, Any]:
        ...
        try:
            ...
            background_tasks.add_task(self._start_workflow_async, coordinator, workflow_id, request.parameters, request.timeout)
            return WorkflowResponse(workflow_id=workflow_id, status='starting', created_at=datetime.utcnow().isoformat(), parameters=request.parameters, current_step='initialization').model_dump()
        except Exception as e:
            self.logger.error('Error creating workflow: %s', e)
            raise HTTPException(status_code=500, detail=str(e))
    ...
    async def _start_workflow_async(self, coordinator, workflow_id: str, parameters: Dict[str, Any], timeout: Optional[int]):
        ...
        try:
            if hasattr(coordinator, 'start_training_workflow'):
                ...
                self.logger.error('Unknown coordinator type for workflow %s', workflow_id)
        except Exception as e:
    ...
    async def get_workflow_status(self, workflow_id: str) -> Dict[str, Any]:
        ...
        try:
            if not self.orchestrator:
                raise HTTPException(status_code=503, detail='Orchestrator not available')
            coordinators = [getattr(self.orchestrator, 'training_coordinator', None), getattr(self.orchestrator, 'evaluation_coordinator', None), getattr(self.orchestrator, 'deployment_coordinator', None), getattr(self.orchestrator, 'optimization_coordinator', None), getattr(self.orchestrator, 'data_pipeline_coordinator', None)]
            for coordinator in coordinators:
                if not coordinator:
                    continue
                try:
                    status_methods = ['get_workflow_status', 'get_training_session_status', 'get_evaluation_status', 'get_deployment_status', 'get_optimization_status', 'get_pipeline_status']
                    ...
                    self.logger.debug('Workflow {workflow_id} not found in coordinator: %s', e)
                    continue
        ...
        try:
            ...
            for coordinator in coordinators:
                if not coordinator:
                    continue
                try:
                    stop_methods = ['stop_workflow', 'stop_evaluation', 'stop_deployment', 'stop_optimization', 'stop_pipeline']
                    for method_name in stop_methods:
                        if hasattr(coordinator, method_name):
                            method = getattr(coordinator, method_name)
                            await method(workflow_id)
                            return {'workflow_id': workflow_id, 'status': 'stopped', 'stopped_at': datetime.utcnow().isoformat()}
                except Exception as e:
                    self.logger.debug('Failed to stop workflow {workflow_id} in coordinator: %s', e)
                    continue
            ...
            for component_name in components:
                try:
                    ...
                    if connector:
                        ...
                        component_details.append({'name': metadata.name, 'tier': metadata.tier.value, 'version': metadata.version, 'capabilities': [cap.name for cap in metadata.capabilities], 'status': connector.status.value if hasattr(connector, 'status') else 'unknown'})
                except Exception as e:
                    self.logger.warning('Error getting details for component {component_name}: %s', e)
                    component_details.append({'name': component_name, 'tier': 'unknown', 'version': 'unknown', 'capabilities': [], 'status': 'unknown'})
            return {'components': component_details, 'total': len(component_details)}
        except Exception as e:
    ...
    async def register_component(self, request: ComponentRegistrationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'component_name': request.component_name, 'status': 'registered', 'registered_at': datetime.utcnow().isoformat(), 'tier': request.component_tier, 'capabilities': request.capabilities}
        except Exception as e:
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'metrics': metrics, 'collected_at': datetime.utcnow().isoformat()}
        except Exception as e:
