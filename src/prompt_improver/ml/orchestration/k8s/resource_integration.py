...
class KubernetesResourceManager:
    ...
    def _initialize_k8s_clients(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning('Metrics API not available: %s', e)
        except Exception as e:
            self.logger.error('Failed to initialize Kubernetes clients: %s', e)
            self.v1 = None
    ...
    async def get_node_resources(self) -> List[NodeResourceInfo]:
        ...
        try:
            ...
            for node in nodes.items:
                ...
                taints = []
                if node.spec.taints:
                    taints = [{'key': taint.key, 'value': taint.value or '', 'effect': taint.effect} for taint in node.spec.taints]
                node_info = NodeResourceInfo(node_name=node_name, allocatable_cpu=allocatable.get('cpu', '0'), allocatable_memory=allocatable.get('memory', '0'), allocatable_gpu=allocatable.get('nvidia.com/gpu', '0'), capacity_cpu=capacity.get('cpu', '0'), capacity_memory=capacity.get('memory', '0'), capacity_gpu=capacity.get('nvidia.com/gpu', '0'), labels=labels, taints=taints)
                node_resources.append(node_info)
            return node_resources
        except api_exception as e:
            self.logger.error('Error getting node resources: %s', e)
            return []
    ...
    async def get_namespace_resource_quotas(self, namespace: str=None) -> List[ResourceQuotaInfo]:
        ...
        try:
            ...
            for quota in quotas.items:
                ...
                quota_data = ResourceQuotaInfo(namespace=target_namespace, quota_name=quota_name, cpu_hard=hard.get('requests.cpu', '0'), memory_hard=hard.get('requests.memory', '0'), gpu_hard=hard.get('requests.nvidia.com/gpu', '0'), cpu_used=used.get('requests.cpu', '0'), memory_used=used.get('requests.memory', '0'), gpu_used=used.get('requests.nvidia.com/gpu', '0'))
                quota_info.append(quota_data)
            return quota_info
        except api_exception as e:
            self.logger.error('Error getting resource quotas: %s', e)
            return []
    ...
    async def get_pod_resources(self, namespace: str=None) -> List[PodResourceInfo]:
        ...
        try:
            ...
            for pod in pods.items:
                ...
                pod_info = PodResourceInfo(pod_name=pod.metadata.name, namespace=target_namespace, cpu_request=f'{total_cpu_request}m', memory_request=f'{total_memory_request}', cpu_limit=f'{total_cpu_limit}m', memory_limit=f'{total_memory_limit}', gpu_request=str(total_gpu_request))
                pod_resources.append(pod_info)
            return pod_resources
        except api_exception as e:
            self.logger.error('Error getting pod resources: %s', e)
            return []

    async def create_hpa(self, deployment_name: str, min_replicas: int, max_replicas: int, target_cpu_percent: int=70, namespace: str=None) -> bool:
        ...
        target_namespace = namespace or self.namespace
        try:
            hpa = client.V2HorizontalPodAutoscaler(metadata=client.V1ObjectMeta(name=f'{deployment_name}-hpa', namespace=target_namespace), spec=client.V2HorizontalPodAutoscalerSpec(scale_target_ref=client.V2CrossVersionObjectReference(api_version='apps/v1', kind='Deployment', name=deployment_name), min_replicas=min_replicas, max_replicas=max_replicas, metrics=[client.V2MetricSpec(type='Resource', resource=client.V2ResourceMetricSource(name='cpu', target=client.V2MetricTarget(type='Utilization', average_utilization=target_cpu_percent)))]))
            self.autoscaling_v2.create_namespaced_horizontal_pod_autoscaler(namespace=target_namespace, body=hpa)
            self.logger.info('Created HPA for deployment %s', deployment_name)
            return True
        except api_exception as e:
            self.logger.error('Error creating HPA: %s', e)
            return False
    ...
    async def get_cluster_resource_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'cluster_summary': {'nodes': len(nodes), 'pods': len(pods), 'namespaces_with_quotas': len(set((quota.namespace for quota in quotas)))}, 'resource_totals': {'allocatable': {'cpu_millicores': total_allocatable_cpu, 'memory_bytes': total_allocatable_memory, 'gpu_count': total_allocatable_gpu}, 'requested': {'cpu_millicores': total_requested_cpu, 'memory_bytes': total_requested_memory, 'gpu_count': total_requested_gpu}, 'utilization': {'cpu_percent': total_requested_cpu / total_allocatable_cpu * 100 if total_allocatable_cpu > 0 else 0, 'memory_percent': total_requested_memory / total_allocatable_memory * 100 if total_allocatable_memory > 0 else 0, 'gpu_percent': total_requested_gpu / total_allocatable_gpu * 100 if total_allocatable_gpu > 0 else 0}}, 'nodes': [{'name': node.node_name, 'allocatable_cpu': node.allocatable_cpu, 'allocatable_memory': node.allocatable_memory, 'allocatable_gpu': node.allocatable_gpu, 'gpu_enabled': int(node.allocatable_gpu) > 0} for node in nodes], 'resource_quotas': [{'namespace': quota.namespace, 'quota_name': quota.quota_name, 'cpu_utilization': f'{quota.cpu_used}/{quota.cpu_hard}', 'memory_utilization': f'{quota.memory_used}/{quota.memory_hard}', 'gpu_utilization': f'{quota.gpu_used}/{quota.gpu_hard}'} for quota in quotas]}
        except Exception as e:
            self.logger.error('Error getting cluster resource summary: %s', e)
            return {'error': str(e)}
