...
class ExternalAPIHealthMonitor:
    ...
    def _get_default_endpoints(self) -> list[APIEndpoint]:
        ...
        if config.ml.mlflow_tracking_uri:
            mlflow_url = (
                config.ml.mlflow_tracking_uri.rstrip("/")
    ...
    async def check_all_endpoints(self) -> dict[str, APIHealthSnapshot]:
        ...
        for endpoint, result in zip(self.endpoints, results, strict=False):
            if isinstance(result, Exception):
                ...
                health_snapshots[endpoint.name] = self._create_error_snapshot(
                    endpoint, str(result)
                )
            else:
    ...
    async def _check_single_endpoint(self, endpoint: APIEndpoint) -> APIHealthSnapshot:
        ...
        status = self._determine_api_status(
            endpoint, response_metrics, availability, percentiles
        )
        sla_compliance = self._determine_sla_compliance(
            endpoint, percentiles, availability
        )
        cb_state = "disabled"
        cb_metrics = {}
        ...
        snapshot = APIHealthSnapshot(
            ...
            current_response_time_ms=response_metrics.response_time_ms
            if response_metrics
            else None,
            current_status_code=response_metrics.status_code
            if response_metrics
            else None,
            current_error=response_metrics.error if response_metrics else None,
            rate_limit_status=self._extract_rate_limit_info(response_metrics)
            if response_metrics
            else None,
        ...
        logger.info(
            ...
            extra={
                "endpoint": endpoint.name,
                "status": status.value,
                "response_time_ms": response_metrics.response_time_ms
                if response_metrics
                else None,
                "availability": availability,
                "circuit_breaker_state": cb_state,
    ...
    async def _perform_http_check(self, endpoint: APIEndpoint) -> ResponseMetrics:
        ...
        try:
            async with (
                ...
                session.get(
                    endpoint.url, headers=headers, allow_redirects=True
                ) as response,
            ):
                ...
                return ResponseMetrics(
                    ...
                    rate_limit_remaining=self._parse_rate_limit(
    ...
    async def _check_ssl_certificate(self, url: str) -> SSLMetrics:
        ...
        if not hostname:
            return SSLMetrics(
                timestamp=datetime.now(UTC),
                certificate_expiry=datetime.now(UTC),
                days_until_expiry=0,
                issuer="",
                subject="",
                valid=False,
        ...
        try:
            ...
            valid = (
                now >= cert.not_valid_before.replace(tzinfo=UTC)
                and now <= expiry
    ...
    def _extract_rate_limit_info(self, metrics: ResponseMetrics) -> dict[str, Any]:
        ...
        if metrics.rate_limit_reset is not None:
            ...
            info["reset_in_seconds"] = (
                metrics.rate_limit_reset - datetime.now(UTC)
            ).total_seconds()
        if (
            metrics.rate_limit_remaining is not None
            and metrics.rate_limit_limit is not None
        ):
            used = metrics.rate_limit_limit - metrics.rate_limit_remaining
            info["utilization"] = (
                used / metrics.rate_limit_limit if metrics.rate_limit_limit > 0 else 0.0
            )
        return info
    ...
    def _determine_sla_compliance(
        ...
        p99 = percentiles.get("p99", 0.0)
        if (
            p50 > endpoint.p50_target_ms
            or p95 > endpoint.p95_target_ms
            or p99 > endpoint.p99_target_ms
        ):
            return SLACompliance.VIOLATED
        if (
            p50 < endpoint.p50_target_ms * 0.5
            and p95 < endpoint.p95_target_ms * 0.5
            and (availability > endpoint.availability_target + 0.001)
        ):
    ...
    async def _store_health_snapshots(self, snapshots: dict[str, APIHealthSnapshot]):
        ...
        try:
            ...
            summary = {
                ...
                "healthy_apis": sum(
                    1 for s in snapshots.values() if s.status == APIStatus.HEALTHY
                ),
                "degraded_apis": sum(
                    1 for s in snapshots.values() if s.status == APIStatus.DEGRADED
                ),
                "unhealthy_apis": sum(
                    1 for s in snapshots.values() if s.status == APIStatus.UNHEALTHY
                ),
                "sla_compliant": sum(
                    1
                    for s in snapshots.values()
                    if s.sla_compliance != SLACompliance.VIOLATED
                ),
            }
            await redis_client.setex(
                summary_key,
    ...
    async def discover_dependencies(self) -> set[str]:
        ...
        if self._last_dependency_scan and datetime.now(
            UTC
        ) - self._last_dependency_scan < timedelta(hours=6):
    ...
    async def get_historical_metrics(
        ...
        try:
            ...
            logger.warning(
                f"Failed to retrieve historical metrics for {endpoint_name}: {e}"
            )
            return []
