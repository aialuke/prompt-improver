...
from prompt_improver.core.types import CacheLevel, InvalidationType
...
class CachePerformanceAlert(BaseModel):
...
class CacheWarmingPattern(BaseModel):
...
class UnifiedCacheMonitor:
    ...
    def add_dependency(self, key: str, depends_on: str, cascade_level: int = 1):
        ...
        logger.debug(
            f"Added dependency: {key} depends on {depends_on} (cascade: {cascade_level})"
        )
    ...
    @asynccontextmanager
    async def trace_invalidation(
        self, invalidation_type: InvalidationType, keys: list[str]
    ):
        ...
        try:
            ...
            if OPENTELEMETRY_AVAILABLE and cache_invalidation_events:
                cache_invalidation_events.add(
                    1,
                    {"invalidation_type": invalidation_type.value, "status": "success"},
                )
            ...
            if OPENTELEMETRY_AVAILABLE and cache_invalidation_events:
                cache_invalidation_events.add(
                    1, {"invalidation_type": invalidation_type.value, "status": "error"}
                )
    ...
    async def invalidate_by_pattern(
        self, pattern: str, cache_levels: list[CacheLevel] | None = None
    ) -> int:
        ...
        async with self.trace_invalidation(InvalidationType.PATTERN, [pattern]):
            ...
            if self._unified_manager and hasattr(self._unified_manager, "scan_keys"):
                ...
                matching_keys = [
                    key for key in self._dependencies.keys() if pattern in key
                ]
            for key in matching_keys:
                invalidated_count = await self._invalidate_with_cascade(
                    key, cache_levels
                )
                total_invalidated += invalidated_count
            logger.info(
                f"Pattern invalidation '{pattern}' affected {total_invalidated} keys"
            )
            return total_invalidated

    async def invalidate_by_dependency(
        self, root_key: str, cache_levels: list[CacheLevel] | None = None
    ) -> int:
        ...
        async with self.trace_invalidation(InvalidationType.DEPENDENCY, all_keys):
            ...
            for key in all_keys:
                ...
                total_invalidated += invalidated_count
            logger.info(
                f"Dependency invalidation from '{root_key}' affected {total_invalidated} keys"
            )
            return total_invalidated

    async def _invalidate_with_cascade(
        self, key: str, cache_levels: list[CacheLevel], depth: int = 0
    ) -> int:
    ...
    async def _invalidate_single_key(
        self, key: str, cache_levels: list[CacheLevel]
    ) -> int:
    ...
    def _update_warming_pattern(self, key: str):
        ...
        if key not in self._warming_patterns:
            self._warming_patterns[key] = CacheWarmingPattern(
                pattern=key, frequency=1.0, success_rate=1.0, last_access=now
            )
        else:
    ...
    async def _check_performance_alerts(self, operation: str, cache_level: CacheLevel):
        ...
        if avg_duration > self._alert_thresholds["response_time_critical"]:
            alerts_to_generate.append(
                self._create_response_time_alert(
    ...
    async def analyze_warming_opportunities(self) -> list[str]:
        ...
        for key, pattern in self._warming_patterns.items():
            if (
                ...
                and pattern.predicted_next_access <= now + timedelta(minutes=30)
                and (pattern.frequency > 0.5)
            ):
                warming_candidates.append(key)
        warming_candidates.sort(
            key=lambda k: self._warming_patterns[k].frequency
            * self._warming_patterns[k].success_rate,
            reverse=True,
        )
    ...
    def _calculate_invalidation_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_cascade_depth": statistics.mean([
                e.cascade_depth for e in recent_events
            ])
            if recent_events
            else 0,
        }
    ...
    def register_alert_callback(
        self, callback: Callable[[CachePerformanceAlert], None]
    ):
        """Register callback for performance alerts."""
        self._alert_callbacks.append(callback)

    async def integrate_with_unified_manager(self, unified_manager):
        """Integrate monitoring with DatabaseServices."""
        self._unified_manager = unified_manager
        task_manager = get_background_task_manager()
        if task_manager:
            await task_manager.submit_enhanced_task(
        ...
        logger.info("UnifiedCacheMonitor integrated with DatabaseServices")
...
def integrate_cache_monitoring(unified_manager):
    """Integrate cache monitoring with DatabaseServices.
