...
from prompt_improver.monitoring.cache.unified_cache_monitoring import AlertSeverity, get_unified_cache_monitor
...
try:
    ...
    slo_cache_performance_impact = slo_cache_meter.create_gauge('slo_cache_performance_impact', description='Impact of cache performance on SLO compliance', unit='ratio')
    slo_cache_budget_consumption = slo_cache_meter.create_gauge('slo_cache_error_budget_consumption', description='Error budget consumption due to cache performance', unit='percent')
    cache_sli_violations = slo_cache_meter.create_counter('cache_sli_violations_total', description='Total cache SLI violations by type and severity', unit='1')
    predictive_cache_alerts = slo_cache_meter.create_counter('predictive_cache_alerts_total', description='Total predictive cache performance alerts', unit='1')
except ImportError:
...
class CacheSLOIntegration:
    ...
    def __init__(self):
        ...
        self._cache_slo_targets = {CacheSLIType.AVAILABILITY: 0.999, CacheSLIType.LATENCY: 50.0, CacheSLIType.HIT_RATE: 0.85, CacheSLIType.ERROR_RATE: 0.001, CacheSLIType.THROUGHPUT: 1000.0}
    ...
    async def calculate_cache_slis(self, measurement_window: timedelta=None) -> dict[CacheSLIType, CacheSLI]:
        ...
        for sli_type, sli in slis.items():
            self._sli_history[sli_type.value].append({'timestamp': sli.timestamp.timestamp(), 'value': sli.current_value, 'compliance': sli.compliance_ratio})
        if OPENTELEMETRY_AVAILABLE:
            for sli_type, sli in slis.items():
                if cache_sli_violations and sli.compliance_ratio < 1.0:
                    cache_sli_violations.add(1, {'sli_type': sli_type.value, 'severity': 'warning' if sli.compliance_ratio > 0.8 else 'critical'})
        return slis

    def _calculate_availability_sli(self, cache_stats: dict, window: timedelta) -> CacheSLI | None:
        ...
        try:
            ...
            return CacheSLI(sli_type=CacheSLIType.AVAILABILITY, target_value=target, current_value=availability, compliance_ratio=compliance, measurement_window=window)
        except Exception as e:
            logger.error('Error calculating availability SLI: %s', e)
            return None

    def _calculate_latency_sli(self, cache_stats: dict, window: timedelta) -> CacheSLI | None:
        ...
        try:
            ...
            return CacheSLI(sli_type=CacheSLIType.LATENCY, target_value=target, current_value=current_p95, compliance_ratio=compliance, measurement_window=window)
        except Exception as e:
            logger.error('Error calculating latency SLI: %s', e)
            return None

    def _calculate_hit_rate_sli(self, cache_stats: dict, window: timedelta) -> CacheSLI | None:
        ...
        try:
            ...
            return CacheSLI(sli_type=CacheSLIType.HIT_RATE, target_value=target, current_value=overall_hit_rate, compliance_ratio=compliance, measurement_window=window)
        except Exception as e:
            logger.error('Error calculating hit rate SLI: %s', e)
            return None

    def _calculate_error_rate_sli(self, cache_stats: dict, window: timedelta) -> CacheSLI | None:
        ...
        try:
            ...
            return CacheSLI(sli_type=CacheSLIType.ERROR_RATE, target_value=target, current_value=current_error_rate, compliance_ratio=compliance, measurement_window=window)
        except Exception as e:
            logger.error('Error calculating error rate SLI: %s', e)
            return None
    ...
    async def analyze_performance_trends(self) -> dict[str, CachePerformanceTrend]:
        ...
        for sli_type_str, history in self._sli_history.items():
            ...
            try:
                ...
                logger.error('Error analyzing trend for {sli_type_str}: %s', e)
        return trends

    def _calculate_trend(self, metric_name: str, data_points: list[dict]) -> CachePerformanceTrend | None:
        ...
        return CachePerformanceTrend(metric_name=metric_name, trend_direction=trend_direction, slope=slope, confidence=confidence, projected_value=projected_value, projection_time=projection_time, historical_data=values[-50:])
    ...
    async def generate_predictive_alerts(self) -> list[PredictiveAlert]:
        ...
        if OPENTELEMETRY_AVAILABLE and predictive_cache_alerts:
            for alert in alerts:
                predictive_cache_alerts.add(1, {'alert_type': alert.alert_type, 'confidence': alert.confidence.value})
        return alerts
    ...
    def _create_predictive_alert(self, metric_name: str, trend: CachePerformanceTrend) -> PredictiveAlert | None:
        ...
        try:
            ...
            alert = PredictiveAlert(alert_id=f'predictive_{metric_name}_{int(time.time())}', alert_type=f'predicted_{metric_name}_violation', predicted_violation_time=violation_time, confidence=trend.confidence, current_trend=trend, impact_assessment=impact, recommended_actions=recommendations)
            return alert
        except Exception as e:
            logger.error('Error creating predictive alert for {metric_name}: %s', e)
            return None
    ...
    def _generate_recommendations(self, sli_type: CacheSLIType, trend: CachePerformanceTrend) -> list[str]:
        ...
        if sli_type == CacheSLIType.HIT_RATE:
            recommendations.extend(['Increase cache warming frequency for popular keys', 'Review cache TTL settings to prevent premature expiration', 'Analyze access patterns for cache optimization opportunities', 'Consider increasing cache capacity if memory allows'])
        elif sli_type == CacheSLIType.LATENCY:
            recommendations.extend(['Review cache backend performance and connections', 'Optimize serialization/deserialization processes', 'Check for network latency issues to cache backends', 'Consider cache data size optimization'])
        elif sli_type == CacheSLIType.AVAILABILITY:
            recommendations.extend(['Check cache backend health and connectivity', 'Review error handling and retry logic', 'Consider implementing cache failover mechanisms', 'Monitor resource utilization on cache backends'])
        elif sli_type == CacheSLIType.ERROR_RATE:
            recommendations.extend(['Investigate root causes of cache operation failures', 'Review timeout configurations', 'Check for resource contention or capacity issues', 'Implement better error handling and logging'])
    ...
    def _assess_performance_impact(self, sli_type: CacheSLIType, trend: CachePerformanceTrend) -> dict[str, Any]:
        ...
        impact = {'severity': 'medium', 'affected_services': ['cache_layer'], 'business_impact': 'potential_latency_increase', 'user_experience_impact': 'minimal'}
    ...
    async def correlate_with_slo_monitoring(self):
        ...
        try:
            ...
            for sli_type, sli in cache_slis.items():
                async with self.slo_observability.observe_slo_operation(operation='cache_sli_measurement', service_name='unified_cache', target_name=sli_type.value, component='cache_monitoring'):
                    await self.slo_observability.record_slo_compliance(service_name='unified_cache', target_name=sli_type.value, compliance_ratio=sli.compliance_ratio, error_budget_remaining=(1.0 - sli.compliance_ratio) * 100, time_window=sli.measurement_window, additional_metrics={'current_value': sli.current_value, 'target_value': sli.target_value})
            ...
            logger.error('Error correlating with SLO monitoring: %s', e)
    ...
    def _calculate_overall_performance_impact(self, cache_slis: dict[CacheSLIType, CacheSLI]) -> float:
        ...
        if not cache_slis:
            return 0.0
        weights = {CacheSLIType.AVAILABILITY: 0.3, CacheSLIType.LATENCY: 0.25, CacheSLIType.HIT_RATE: 0.25, CacheSLIType.ERROR_RATE: 0.2}
        weighted_impact = 0.0
        total_weight = 0.0
        for sli_type, sli in cache_slis.items():
    ...
    async def get_slo_cache_report(self) -> dict[str, Any]:
        ...
        return {'timestamp': datetime.now(UTC).isoformat(), 'cache_slis': {sli_type.value: {'current_value': sli.current_value, 'target_value': sli.target_value, 'compliance_ratio': sli.compliance_ratio, 'status': 'compliant' if sli.compliance_ratio >= 0.95 else 'at_risk' if sli.compliance_ratio >= 0.8 else 'violation'} for sli_type, sli in cache_slis.items()}, 'performance_trends': {metric: {'direction': trend.trend_direction, 'confidence': trend.confidence.value, 'projected_value': trend.projected_value, 'projection_time': trend.projection_time.isoformat()} for metric, trend in trends.items()}, 'predictive_alerts': [{'alert_id': alert.alert_id, 'type': alert.alert_type, 'predicted_time': alert.predicted_violation_time.isoformat(), 'confidence': alert.confidence.value, 'recommendations': alert.recommended_actions[:3]} for alert in predictive_alerts], 'overall_health': {'score': health_score, 'status': 'healthy' if health_score > 0.8 else 'degraded' if health_score > 0.6 else 'critical'}, 'integration_status': {'slo_observability_connected': self.slo_observability is not None, 'cache_monitor_connected': self.cache_monitor is not None, 'opentelemetry_available': OPENTELEMETRY_AVAILABLE}}
    ...
    async def start_monitoring(self):
        ...
        if task_manager:
            await task_manager.submit_enhanced_task(task_id='cache_slo_correlation', coroutine=self._periodic_slo_correlation(), priority=TaskPriority.NORMAL, tags={'service': 'cache_monitoring', 'type': 'slo_correlation', 'component': 'cache_slo_integration'})
            await task_manager.submit_enhanced_task(task_id='cache_predictive_analysis', coroutine=self._periodic_predictive_analysis(), priority=TaskPriority.BACKGROUND, tags={'service': 'cache_monitoring', 'type': 'predictive_analysis', 'component': 'cache_slo_integration'})
    ...
    async def _periodic_slo_correlation(self):
        ...
        while True:
            try:
                ...
                logger.error('Periodic SLO correlation failed: %s', e)
    ...
    async def _periodic_predictive_analysis(self):
        ...
        while True:
            try:
                ...
                if alerts:
                    logger.info('Generated %s predictive cache alerts', len(alerts))
                    for alert in alerts:
                        logger.warning('Predictive Alert: %s - %s', alert.alert_type, alert.predicted_violation_time)
                await asyncio.sleep(300)
            except Exception as e:
                logger.error('Periodic predictive analysis failed: %s', e)
                await asyncio.sleep(60)
