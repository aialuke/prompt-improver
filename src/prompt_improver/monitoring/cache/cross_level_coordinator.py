...
from prompt_improver.core.types import AccessPattern, CacheLevel, CoordinationAction
...
class CrossLevelCoordinator:
    ...
    def __init__(self, unified_manager=None, strategy: CoordinationStrategy = None):
        ...
        logger.info(
            "CrossLevelCoordinator initialized with intelligent cache management"
        )
    ...
    def _calculate_promotion_score(self, entry: CacheEntry) -> float:
        ...
        recent_accesses = [
            a
            for a in self._access_history[entry.key]
    ...
    def _determine_coordination_action(
        self, entry: CacheEntry
    ) -> tuple[CoordinationAction, CacheLevel, CacheLevel] | None:
        """Determine what coordination action to take."""
        if (
            entry.promotion_score >= self._strategy.promotion_threshold
            and entry.access_count >= self._strategy.min_access_count
        ):
            if (
                CacheLevel.L2 in entry.levels_present
                and CacheLevel.L1 not in entry.levels_present
                and self._has_l1_capacity()
            ):
                return (CoordinationAction.PROMOTE, CacheLevel.L2, CacheLevel.L1)
            if (
                CacheLevel.L3 in entry.levels_present
                and CacheLevel.L2 not in entry.levels_present
                and self._has_l2_capacity()
            ):
    ...
    def _needs_l1_space(self) -> bool:
        ...
        return (
            self._level_usage[CacheLevel.L1]["count"]
            >= self._strategy.max_l1_size * 0.9
        )

    def _needs_l2_space(self) -> bool:
        """Check if L2 cache needs space (for eviction)."""
        return (
            self._level_usage[CacheLevel.L2]["count"]
            >= self._strategy.max_l2_size * 0.9
        )

    @asynccontextmanager
    async def _trace_coordination(
        self,
        action: CoordinationAction,
        key: str,
    ...
    async def _execute_coordination_action(
        ...
        async with self._trace_coordination(action, key, source_level, target_level):
            ...
            if action == CoordinationAction.PROMOTE:
                self._coordination_stats["promotions"][
                    f"{source_level.value}_to_{target_level.value}"
                ] += 1
            elif action == CoordinationAction.DEMOTE:
                self._coordination_stats["demotions"][
                    f"{source_level.value}_to_{target_level.value}"
                ] += 1
    ...
    async def _execute_promotion(
        ...
        try:
            ...
            if value is None:
                ...
                return
            success = await self._set_to_level(
                key, value, target_level, entry.ttl_seconds
            )
            if success:
                entry.levels_present.add(target_level)
                logger.debug(
                    f"Promoted {key} from {source_level.value} to {target_level.value}"
                )
            else:
    ...
    async def _execute_demotion(
        ...
        try:
            ...
            if success:
                ...
                entry.levels_present.add(target_level)
                logger.debug(
                    f"Demoted {key} from {source_level.value} to {target_level.value}"
                )
            else:
    ...
    async def _get_from_level(self, key: str, level: CacheLevel) -> Any:
        ...
        try:
            if level == CacheLevel.L1:
                if (
                    hasattr(self._unified_manager, "_l1_cache")
                    and self._unified_manager._l1_cache
                ):
                    return self._unified_manager._l1_cache.get(key)
            elif level == CacheLevel.L2:
                if (
                    hasattr(self._unified_manager, "cache.redis_client")
                    and self._unified_manager.cache.redis_client
                ):
                    value = await self._unified_manager.cache.redis_client.get(key)
    ...
    async def _set_to_level(
        self, key: str, value: Any, level: CacheLevel, ttl: int | None = None
    ) -> bool:
        ...
        try:
            if level == CacheLevel.L1:
                ...
                if (
                    ...
                    if ttl:
                        await self._unified_manager.cache.redis_client.setex(
                            key, ttl, str(value)
                        )
                    else:
                        await self._unified_manager.cache.redis_client.set(
                            key, str(value)
                        )
                    return True
            elif level == CacheLevel.L3:
        ...
        try:
            if level == CacheLevel.L1:
                ...
                if (
                    ...
                    deleted = await self._unified_manager.cache.redis_client.delete(key)
                    return deleted > 0
            elif level == CacheLevel.L3:
                pass
        except Exception as e:
    ...
    async def check_cache_coherence(
        ...
        if OPENTELEMETRY_AVAILABLE and cache_coherence_violations:
            for key, key_violations in violations.items():
                cache_coherence_violations.add(
                    len(key_violations), {"violation_type": "coherence_mismatch"}
                )
        return violations
    ...
    async def _check_key_coherence(self, key: str) -> list[str]:
        ...
        present_values = {
            level: value for level, value in values.items() if value is not None
        }
        if len(present_values) > 1:
            unique_values = {str(v) for v in present_values.values()}
        ...
        try:
            ...
            for level in entry.levels_present:
                if level != authoritative_level:
                    set_success = await self._set_to_level(
                        key, authoritative_value, level, entry.ttl_seconds
                    )
                    if not set_success:
                        success = False
                        logger.warning(
                            f"Failed to repair coherence for {key} at {level.value}"
                        )
            logger.info(
                f"Repaired cache coherence for {key} using value from {authoritative_level.value}"
            )
            return success
        except Exception as e:
    ...
    def get_coordination_stats(self) -> dict[str, Any]:
        ...
        total_actions = sum(self._coordination_stats["promotions"].values()) + sum(
            self._coordination_stats["demotions"].values()
        )
        successful_events = sum(
            1 for event in self._coordination_events if event.success
        )
        efficiency = (
            successful_events / len(self._coordination_events)
            if self._coordination_events
            else 1.0
        )
        ...
        return {
            ...
            "performance": {
                "avg_coordination_time_ms": statistics.mean([
                    e.duration_ms for e in self._coordination_events
                ])
                if self._coordination_events
                else 0
            },
    ...
    async def integrate_with_unified_manager(self, unified_manager):
        """Integrate coordinator with DatabaseServices."""
        self._unified_manager = unified_manager
        ...
        logger.info("CrossLevelCoordinator integrated with DatabaseServices")
    ...
    async def _optimize_cache_distribution(self):
        ...
        for key, entry in self._entry_metadata.items():
            if (
                CacheLevel.L1 in entry.levels_present
                and entry.promotion_score < self._strategy.demotion_threshold
            ):
...
def integrate_cross_level_coordination(unified_manager):
    """Integrate cross-level coordination with DatabaseServices.
