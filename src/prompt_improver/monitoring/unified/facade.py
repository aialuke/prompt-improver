...
from prompt_improver.monitoring.unified.monitoring_orchestrator_service import (
    MonitoringOrchestratorService,
)
from prompt_improver.monitoring.unified.repository import MonitoringRepository
...
class UnifiedMonitoringFacade:
    ...
    async def start_monitoring(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Failed to start unified monitoring: {e}")
            raise
    ...
    async def stop_monitoring(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Error stopping unified monitoring: {e}")
    ...
    def record_cache_operation(
        ...
        try:
            ...
            # Convert string to CacheLevel enum (L1/L2 architecture)
            level_map = {
                "l1": CacheLevel.L1,
                "l2": CacheLevel.L2,
                "all": CacheLevel.ALL,
            ...
            logger.exception(f"Failed to record cache operation: {e}")
    ...
    def register_health_checker(self, checker: HealthCheckComponentProtocol) -> None:
        ...
        try:
            ...
            logger.exception(f"Failed to register health checker: {e}")
    ...
    async def get_monitoring_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Failed to get monitoring summary: {e}")
            return {
                "error": str(e),
                "timestamp": time.time(),
            }
    ...
    async def check_health(
        ...
        try:
            ...
            logger.exception(f"Health check compatibility method failed: {e}")
            return {}
    ...
    async def get_overall_health(self) -> HealthCheckResult:
        ...
        try:
            ...
            logger.exception(f"Overall health compatibility method failed: {e}")
            return HealthCheckResult(
                status=HealthStatus.UNKNOWN,
                message=f"Health check failed: {e!s}",
                check_name="system_overall",
    ...
    def _convert_to_protocol_result(self, result: HealthCheckResult) -> HealthCheckResult:
