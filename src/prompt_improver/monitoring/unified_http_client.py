...
import aiohttp
from prompt_improver.monitoring.external_api_health import APIEndpoint, ExternalAPIHealthMonitor, ResponseMetrics
...
class UnifiedHTTPClientFactory:
    ...
    def register_client(self, config: HTTPClientConfig, endpoint_config: APIEndpoint | None=None) -> None:
        """Register a new HTTP client configuration"""
        self.clients[config.name] = config
        if config.circuit_breaker_enabled:
            cb_config = CircuitBreakerConfig(failure_threshold=config.failure_threshold, recovery_timeout=config.recovery_timeout_seconds, response_time_threshold_ms=config.response_time_threshold_ms)
            self.circuit_breakers[config.name] = circuit_breaker_registry.get_or_create(f'http_client_{config.name}', cb_config)
        ...
        logger.info('Registered HTTP client: %s (%s)', config.name, config.usage_type.value)
    ...
    async def get_session(self, client_name: str):
        ...
        session = None
        try:
            session = aiohttp.ClientSession(timeout=timeout, headers=headers, connector=aiohttp.TCPConnector(ssl=ssl_context))
    ...
    async def make_request(self, client_name: str, method: str, url: str, **kwargs: RequestOptions) -> aiohttp.ClientResponse:
        """Make an HTTP request with circuit breaker and monitoring"""
        config = self.clients[client_name]
        request_context = HTTPRequestContext(request_id=str(uuid.uuid4())[:8], client_name=client_name, method=method.upper(), url=url, start_time=datetime.now(UTC), usage_type=config.usage_type, tags=kwargs.pop('_tags', {}))
        if config.circuit_breaker_enabled and client_name in self.circuit_breakers:
            ...
            try:
                response = await circuit_breaker.call(self._execute_request, config, request_context, method, url, **kwargs)
                return response
    ...
    async def _execute_request(self, config: HTTPClientConfig, context: HTTPRequestContext, method: str, url: str, **kwargs: RequestOptions) -> aiohttp.ClientResponse:
        ...
        async with self.get_session(config.name) as session:
            ...
            try:
                ...
                async with session.request(method, full_url, **kwargs) as response:
                    ...
                    if config.collect_metrics:
                        await self._record_request_metrics(config, context, response, None, response_time_ms)
                    logger.debug('HTTP %s %s -> %s (%sms)', method, full_url, response.status, format(response_time_ms, '.1f'), extra={'client_name': config.name, 'request_id': context.request_id, 'method': method, 'url': full_url, 'status_code': response.status, 'response_time_ms': response_time_ms, 'usage_type': config.usage_type.value})
                    return response
            except Exception as e:
                response_time_ms = (time.time() - start_time) * 1000
                if config.collect_metrics:
                    await self._record_request_metrics(config, context, None, str(e), response_time_ms)
                logger.error('HTTP %s %s failed: %s (%sms)', method, url, e, format(response_time_ms, '.1f'), extra={'client_name': config.name, 'request_id': context.request_id, 'method': method, 'url': url, 'error': str(e), 'response_time_ms': response_time_ms, 'usage_type': config.usage_type.value})
                raise

    async def _record_request_metrics(self, config: HTTPClientConfig, context: HTTPRequestContext, response: aiohttp.ClientResponse | None, error: str | None, response_time_ms: float | None=None) -> None:
        """Record request metrics for monitoring and SLA tracking"""
        if not config.collect_metrics:
            return
        if response_time_ms is None:
            response_time_ms = (datetime.now(UTC) - context.start_time).total_seconds() * 1000
        ...
        metrics = ResponseMetrics(timestamp=context.start_time, response_time_ms=response_time_ms, status_code=response.status if response else 0, success=response is not None and response.status < 400 and (error is None), error=error, headers=response_headers, rate_limit_remaining=rate_limit_remaining, rate_limit_reset=rate_limit_reset, rate_limit_limit=rate_limit_limit)
    ...
    def _parse_rate_limit_header(self, headers: dict[str, str], limit_type: str) -> int | None:
    ...
    def _parse_rate_limit_reset_header(self, headers: dict[str, str]) -> datetime | None:
    ...
    async def get_client_metrics(self, client_name: str) -> dict[str, Any]:
        ...
        metrics_data = {'client_name': client_name, 'total_requests': total_requests, 'successful_requests': success_count, 'failed_requests': total_requests - success_count, 'success_rate': success_count / total_requests if total_requests > 0 else 0.0, 'error_rate': (total_requests - success_count) / total_requests if total_requests > 0 else 0.0}
        if response_times:
            response_times.sort()
            n = len(response_times)
            metrics_data['response_times'] = {'min': min(response_times), 'max': max(response_times), 'mean': sum(response_times) / n, 'p50': response_times[n // 2] if n > 0 else 0.0, 'p95': response_times[int(n * 0.95)] if n > 0 else 0.0, 'p99': response_times[int(n * 0.99)] if n > 0 else 0.0}
        if client_name in self.circuit_breakers:
            cb = self.circuit_breakers[client_name]
            metrics_data['circuit_breaker'] = {'state': cb.state.value, 'failure_count': cb.failure_count, 'last_failure_time': cb.last_failure_time.isoformat() if cb.last_failure_time else None, 'metrics': cb.get_metrics()}
        return metrics_data
    ...
    async def get_all_metrics(self) -> dict[str, Any]:
        ...
        summary = {'total_clients': len(self.clients), 'total_requests': total_requests, 'overall_success_rate': total_successful / total_requests if total_requests > 0 else 0.0, 'clients': all_metrics}
        return summary
    ...
    async def health_check_all_clients(self) -> dict[str, bool]:
        ...
        for client_name, config in self.clients.items():
            try:
                ...
                logger.error('Health check failed for client {client_name}: %s', e)
...
def _register_default_clients(factory: UnifiedHTTPClientFactory) -> None:
    ...
    factory.register_client(HTTPClientConfig(name='webhook_alerts', usage_type=HTTPClientUsage.WEBHOOK_ALERTS, timeout_seconds=30.0, max_retries=3, circuit_breaker_enabled=True, failure_threshold=3, recovery_timeout_seconds=300, headers={'Content-Type': 'application/json'}))
    factory.register_client(HTTPClientConfig(name='health_monitoring', usage_type=HTTPClientUsage.HEALTH_CHECKS, timeout_seconds=10.0, max_retries=2, circuit_breaker_enabled=True, failure_threshold=5, recovery_timeout_seconds=60, headers={'User-Agent': 'prompt-improver-health-monitor/1.0'}))
    factory.register_client(HTTPClientConfig(name='api_calls', usage_type=HTTPClientUsage.API_CALLS, timeout_seconds=30.0, max_retries=3, circuit_breaker_enabled=True, failure_threshold=5, recovery_timeout_seconds=120, rate_limit_aware=True))
    factory.register_client(HTTPClientConfig(name='downloads', usage_type=HTTPClientUsage.DOWNLOADS, timeout_seconds=300.0, max_retries=3, circuit_breaker_enabled=True, failure_threshold=3, recovery_timeout_seconds=600))
    factory.register_client(HTTPClientConfig(name='testing', usage_type=HTTPClientUsage.TESTING, timeout_seconds=30.0, max_retries=1, circuit_breaker_enabled=False, collect_metrics=False))

async def make_webhook_request(url: str, data: dict[str, Any], **kwargs: RequestOptions) -> aiohttp.ClientResponse:
    """Make a webhook request with standardized configuration"""
    factory = get_http_client_factory()
    return await factory.make_request('webhook_alerts', 'POST', url, json=data, **kwargs)

async def make_health_check_request(url: str, **kwargs: RequestOptions) -> aiohttp.ClientResponse:
...
async def make_api_request(method: str, url: str, **kwargs: RequestOptions) -> aiohttp.ClientResponse:
