...
from .protocols import RedisClientProviderProtocol, RedisRecoveryServiceProtocol
from .types import CircuitBreakerState, RecoveryAction, RecoveryEvent
...
class RedisRecoveryService:
    ...
    async def _determine_recovery_strategy(self, reason: str) -> RecoveryAction:
        ...
        elif any(keyword in reason_lower for keyword in ["memory", "fragmentation", "slow"]):
            return RecoveryAction.FAILOVER
        
        # Critical availability issues - immediate failover
        elif any(keyword in reason_lower for keyword in ["unavailable", "failed", "critical"]):
            return RecoveryAction.FAILOVER
    ...
    async def _get_current_client(self) -> Optional[coredis.Redis]:
