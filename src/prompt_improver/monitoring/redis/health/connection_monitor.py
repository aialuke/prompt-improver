...
from .protocols import RedisClientProviderProtocol, RedisConnectionMonitorProtocol
from .types import ConnectionPoolMetrics, ConnectionStatus
...
class RedisConnectionMonitor:
    ...
    def __init__(
        ...
        self._last_metrics: Optional[ConnectionPoolMetrics] = None
    ...
    async def _collect_connection_info(
        ...
        try:
            ...
            logger.error(f"Failed to collect connection info: {e}")
            metrics.status = ConnectionStatus.DEGRADED
    ...
    async def _analyze_pool_health(self, metrics: ConnectionPoolMetrics) -> None:
        ...
        if metrics.pool_utilization > 95.0:
            metrics.status = ConnectionStatus.DEGRADED
        elif metrics.pool_utilization > 85.0:
            metrics.status = ConnectionStatus.DEGRADED
        elif metrics.connection_failures > self._issue_threshold_failures:
            metrics.status = ConnectionStatus.DEGRADED
        else:
            metrics.status = ConnectionStatus.CONNECTED
