...
class StandardBuckets:
    ...
    REQUEST_DURATION = [1.0, 5.0, 10.0, 25.0, 50.0, 100.0, 250.0, 500.0, 1000.0, 2500.0, 5000.0]
    DATABASE_DURATION = [0.5, 1.0, 2.5, 5.0, 10.0, 25.0, 50.0, 100.0, 250.0, 500.0, 1000.0]
    CACHE_DURATION = [0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0, 25.0, 50.0, 100.0]
    ML_INFERENCE_DURATION = [0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0, 30.0, 60.0]
...
class HttpMetrics:
    ...
    def __init__(self, service_name: str='prompt-improver'):
        ...
        if OTEL_AVAILABLE and self.meter is not None:
            self.request_counter = self.meter.create_counter(name='http_requests_total', description='Total number of HTTP requests', unit='1')
            self.request_duration = self.meter.create_histogram(name='http_request_duration_ms', description='HTTP request duration in milliseconds', unit='ms')
            self.response_size = self.meter.create_histogram(name='http_response_size_bytes', description='HTTP response size in bytes', unit='bytes')
        else:
    ...
    def record_request(self, method: str, endpoint: str, status_code: int, duration_ms: float, response_size_bytes: int=0):
        """Record HTTP request metrics."""
        labels = {'method': method, 'endpoint': endpoint, 'status_code': str(status_code), 'service': self.service_name}
        ...
        self._test_last_request = {
            'method': method, 'endpoint': endpoint, 'status_code': int(status_code),
            'duration_ms': float(duration_ms), 'response_size_bytes': int(response_size_bytes)
        }
...
class DatabaseMetrics:
    ...
    def __init__(self, service_name: str='prompt-improver'):
        ...
        self.meter = get_meter(__name__)
        if OTEL_AVAILABLE and self.meter is not None:
            self.query_counter = self.meter.create_counter(name='database_queries_total', description='Total number of database queries', unit='1')
            self.query_duration = self.meter.create_histogram(name='database_query_duration_ms', description='Database query duration in milliseconds', unit='ms')
            self.connection_pool = self.meter.create_gauge(name='database_connection_pool_active', description='Active database connections', unit='1')
        else:
    ...
    def record_query(self, operation: str, table: str, duration_ms: float, success: bool=True):
        """Record database query metrics."""
        labels = {'operation': operation, 'table': table, 'success': str(success), 'service': self.service_name}
...
class MLMetrics:
    ...
    def __init__(self, service_name: str='prompt-improver'):
        ...
        self.meter = get_meter(__name__)
        if OTEL_AVAILABLE and self.meter is not None:
            self.inference_counter = self.meter.create_counter(name='ml_inferences_total', description='Total number of ML inferences', unit='1')
            self.inference_duration = self.meter.create_histogram(name='ml_inference_duration_s', description='ML inference duration in seconds', unit='s')
            self.model_accuracy = self.meter.create_histogram(name='ml_model_accuracy_score', description='ML model accuracy score', unit='1')
            self.prompt_improvements = self.meter.create_counter(name='prompt_improvements_total', description='Total number of prompt improvements', unit='1')
        else:
    ...
    def record_inference(self, model_name: str, duration_s: float, success: bool=True):
        """Record ML inference metrics."""
        labels = {'model': model_name, 'success': str(success), 'service': self.service_name}
...
class BusinessMetrics:
    ...
    def __init__(self, service_name: str='prompt-improver'):
        ...
        if OTEL_AVAILABLE and self.meter is not None:
            ...
            self.feature_usage = self.meter.create_counter(name='feature_usage_total', description='Total feature usage count', unit='1')
            self.user_sessions = self.meter.create_counter(name='user_sessions_total', description='Total user sessions', unit='1')
            self.operational_costs = self.meter.create_histogram(name='operational_costs_usd', description='Operational costs in USD', unit='USD')
            # New user journey instruments
            self.journey_events = self.meter.create_counter(name='user_journey_events_total', description='Total number of user journey events', unit='1')
            self.journey_stage_gauge = self.meter.create_gauge(name='user_journey_stage', description='Current user journey stage as numeric code', unit='1')
        else:
    ...
    def record_feature_usage(self, feature: str, user_tier: str='unknown'):
        ...
        labels = {'feature': feature, 'user_tier': user_tier, 'service': self.service_name}
    ...
    def record_journey_event(self, user_id: str, session_id: str, stage: str, event_type: str, success: bool = True):
        labels = {
        ...
        self.journey_stage_gauge.record(code, {'service': self.service_name, 'user_id': user_id, 'stage': stage})
