...
class MLMonitoringMixin:
    ...
    def enable_ml_monitoring(self, service_name: str='ml-service', alert_thresholds: dict[str, float] | None=None):
        """Enable ML monitoring for this instance."""
        try:
            self._ml_metrics = get_ml_metrics(service_name)
            self._ml_alerting = None
            self._monitoring_enabled = True
            logger.info('ML monitoring enabled for %s', service_name)
        except Exception as e:
            logger.error('Failed to enable ML monitoring: %s', e)
            self._monitoring_enabled = False

    def record_ml_operation(self, operation_name: str, duration: float | None=None, success: bool=True, **metrics):
        ...
        try:
            ...
            logger.error('Failed to record ML operation: %s', e)
    ...
    def check_ml_alerts(self, analysis_data: dict[str, Any]) -> list[Any]:
        ...
        try:
            ...
            logger.error('Failed to check ML alerts: %s', e)
            return []

def ml_monitor(operation_name: str | None=None, record_duration: bool=True, record_errors: bool=True, service_name: str='ml-operation'):
            ...
            try:
                ...
                if hasattr(metrics_collector, '_instruments'):
                    if (counter := metrics_collector._instruments.get('ml_operations_total')):
                        counter.add(1, {'operation': op_name, 'status': 'success', 'service': service_name})
                if record_duration and start_time:
                    duration = time.time() - start_time
                    if (histogram := metrics_collector._instruments.get('ml_response_time')):
                        histogram.record(duration, {'operation': op_name, 'service': service_name})
                return result
            except Exception as e:
                if record_errors and hasattr(metrics_collector, '_instruments'):
                    if (counter := metrics_collector._instruments.get('ml_operations_total')):
                        counter.add(1, {'operation': op_name, 'status': 'error', 'error_type': type(e).__name__, 'service': service_name})
                logger.error('ML operation {op_name} failed: %s', e)
                raise
...
def create_ml_monitoring_context(service_name: str, alert_thresholds: dict[str, float] | None=None, enable_http_server: bool=False, http_port: int=8000) -> dict[str, Any]:
    """Create a complete ML monitoring context."""
    context = {'metrics': get_ml_metrics(service_name), 'alerting': None, 'http_server': None, 'service_name': service_name}
    if alert_thresholds:
        context['alerting'] = None
    if enable_http_server:
        context['http_server'] = None
    logger.info('Created ML monitoring context for %s', service_name)
    return context
...
class MLPerformanceTracker:
    ...
    def record_prediction_batch(self, batch_size: int, accuracy: float | None=None, latency: float | None=None, confidence: float | None=None):
        ...
        self.metrics.record_model_performance(accuracy=accuracy, model_name=self.model_name)
        ...
        self._performance_history.append({'timestamp': datetime.now(), 'batch_size': batch_size, 'accuracy': accuracy, 'latency': latency, 'confidence': confidence})
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        summary = {'model_name': self.model_name, 'total_predictions': sum((e['batch_size'] for e in recent_entries)), 'avg_accuracy': sum(accuracies) / len(accuracies) if accuracies else None, 'avg_latency': sum(latencies) / len(latencies) if latencies else None, 'recent_entries': len(recent_entries)}
        return summary

def monitor_failure_analysis(func: Callable) -> Callable:
    """Decorator specifically for failure analysis functions."""
    return ml_monitor(operation_name='failure_analysis', record_duration=True, record_errors=True, service_name='failure-analyzer')(func)

def monitor_classification(func: Callable) -> Callable:
    """Decorator specifically for classification functions."""
    return ml_monitor(operation_name='classification', record_duration=True, record_errors=True, service_name='failure-classifier')(func)
