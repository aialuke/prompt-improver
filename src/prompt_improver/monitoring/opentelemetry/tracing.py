...
def trace_async(
    ...
    *,
    ...
    span_kind: trace.SpanKind | None = None,
):
    ...
    def decorator(func: Callable[P, Awaitable[T]]) -> Callable[P, Awaitable[T]]:
        ...
        async def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
            ...
            with tracer.start_as_current_span(
                span_name, kind=span_kind or trace.SpanKind.INTERNAL
            ) as span:
                if component:
...
def span_context(
    ...
    *,
    ...
    if not tracer:
        ...
        return
    with tracer.start_as_current_span(
        operation_name, kind=span_kind or trace.SpanKind.INTERNAL
    ) as span:
        if component:
            span.set_attribute(SpanAttributes.COMPONENT, component)
        if attributes:
...
def create_span_link(
    span_context: trace.SpanContext, attributes: dict[str, Any] | None = None
) -> trace.Link:
