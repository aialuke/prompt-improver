...
try:
    ...
    # Get tracer and meter for SLO operations
    slo_tracer = trace.get_tracer(__name__ + ".slo")
    slo_meter = metrics.get_meter(__name__ + ".slo")
    
    # Create SLO-specific metrics
    slo_operations_counter = slo_meter.create_counter(
        "slo_operations_total",
        description="Total SLO operations by type, component, and status",
        unit="1"
    )
    
    slo_cache_performance_histogram = slo_meter.create_histogram(
        "slo_cache_operation_duration_seconds", 
        description="SLO cache operation duration by type and level",
        unit="s"
    )
    
    slo_compliance_ratio_gauge = slo_meter.create_gauge(
        "slo_compliance_ratio",
        description="SLO compliance ratio by service and target",
        unit="ratio"
    )
    
    slo_error_budget_gauge = slo_meter.create_gauge(
        "slo_error_budget_remaining",
        description="SLO error budget remaining percentage by service and target", 
        unit="percent"
    )
...
from .framework import SLOTarget, SLOTimeWindow
...
class UnifiedSLOObservability:
    ...
    def __init__(self, unified_manager=None):
        ...
        self._operation_stats = {
            "sli_calculation": {"count": 0, "total_duration": 0.0, "cache_hits": 0, "cache_misses": 0},
    ...
    @asynccontextmanager
    async def observe_slo_operation(
        self,
        operation: str,
        service_name: str,
        target_name: Optional[str] = None,
        ...
        **custom_attributes
        ...
        context = SLOObservabilityContext(
            operation=operation,
        ...
        if OPENTELEMETRY_AVAILABLE and slo_tracer:
            span = slo_tracer.start_span(
                name=f"slo.{operation}",
                attributes={
                    "slo.operation": operation,
                    "slo.service_name": service_name,
                    "slo.target_name": target_name or "all",
                    "slo.component": component,
                    **custom_attributes
                }
            )
        ...
        try:
            ...
            if OPENTELEMETRY_AVAILABLE and slo_cache_performance_histogram:
                slo_cache_performance_histogram.record(duration, {
                    "operation": operation,
                    "component": component,
                    "service_name": service_name
                })
    ...
    async def record_cache_operation(
        self,
        context_id: str,
        operation_type: str,  # "get", "set", "delete", "exists"
        cache_key: str,
        hit: bool,
        duration_ms: float,
        cache_level: str = "unified"  # "l1", "l2", "unified"
    ):
        ...
        # Add to active context if exists
        ...
        if OPENTELEMETRY_AVAILABLE and slo_cache_performance_histogram:
            slo_cache_performance_histogram.record(duration_ms / 1000.0, {
                "operation_type": operation_type,
                "cache_level": cache_level,
                "hit": str(hit).lower()
            })
    
    async def record_slo_compliance(
        self,
        service_name: str,
        target_name: str,
        ...
        if additional_metrics and OPENTELEMETRY_AVAILABLE:
            for metric_name, value in additional_metrics.items():
                # Create dynamic gauge for custom metrics
                try:
                    custom_gauge = slo_meter.create_gauge(
                        f"slo_{metric_name}",
                        description=f"SLO {metric_name} metric",
                        unit="1"
                    )
                    custom_gauge.set(value, labels)
                except Exception as e:
                    logger.warning(f"Failed to record custom SLO metric {metric_name}: {e}")
        
        logger.debug(f"Recorded SLO compliance for {service_name}.{target_name}: {compliance_ratio:.3f} compliance, {error_budget_remaining:.1f}% budget remaining")
    
    async def correlate_alert_with_cache(
        self,
        alert_id: str,
        service_name: str,
        target_name: str,
        ...
        """
        # Get recent cache statistics from unified manager
        ...
        alert_context = {
            ...
            "unified_manager_initialized": self._unified_manager._is_initialized,
    ...
    def get_operation_statistics(self) -> Dict[str, Any]:
        ...
        # Add unified manager statistics if available
        if hasattr(self._unified_manager, 'get_cache_stats'):
            stats["unified_manager_cache"] = self._unified_manager.get_cache_stats()
        
        # Add alert correlation statistics
        stats["alert_correlation"] = {
            "total_alerts_correlated": len(self._alert_contexts),
            "correlation_callbacks_registered": len(self._correlation_callbacks)
        }
    ...
    async def generate_observability_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "unified_manager_status": {
                "initialized": self._unified_manager._is_initialized,
                "mode": self._unified_manager.mode.value if hasattr(self._unified_manager, 'mode') else "unknown"
            },
            ...
            if isinstance(data, dict) and "cache_hit_rate" in data:
                ...
                if hit_rate < 0.5:
                    report["recommendations"].append({
                        ...
                        "severity": "warning",
                        "operation": operation,
                        "message": f"Low cache hit rate ({hit_rate:.1%}) for {operation} operations. Consider cache warming or TTL optimization."
                    })
                elif hit_rate > 0.9:
                    report["recommendations"].append({
                        "type": "cache_performance",
                        "severity": "info",
                        "operation": operation,
                        "message": f"Excellent cache hit rate ({hit_rate:.1%}) for {operation} operations."
                    })
                ...
                if "average_duration_seconds" in data:
                    ...
                    if avg_duration > 1.0:  # Operations taking more than 1 second
                        report["recommendations"].append({
                            ...
                            "severity": "warning",
                            "operation": operation,
                            "message": f"Slow {operation} operations averaging {avg_duration:.2f}s. Consider optimization."
                        })
        
        return report

# Global observability instance for SLO monitoring
_slo_observability: Optional[UnifiedSLOObservability] = None
