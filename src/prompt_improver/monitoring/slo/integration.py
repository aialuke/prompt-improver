...
class MetricsCollector:
    ...
    def increment_request(self, method: str, endpoint: str, status_code: int, duration_ms: float, response_size_bytes: int = 0) -> None:
        if _HAS_OTEL:
            get_http_metrics(self.service_name).record_request(method, endpoint, status_code, duration_ms, response_size_bytes)

    def record_db_query(self, duration_ms: float, success: bool = True) -> None:
        if _HAS_OTEL:
            from prompt_improver.monitoring.opentelemetry.metrics import get_database_metrics
    ...
    def set_custom_gauge(self, name: str, value: float, labels: Optional[Dict[str, str]] = None) -> None:
