...
class SLICalculator:
    ...
    def __init__(self, slo_target: SLOTarget, max_measurements: int=10000, unified_manager=None):
    ...
    async def _ensure_security_context(self):
        ...
        if self._security_context is None:
            self._security_context = await create_security_context(agent_id=f'slo_calculator_{self.slo_target.service_name}_{self.slo_target.name}', tier='professional', authenticated=True)
        return self._security_context

    async def add_measurement(self, value: float, timestamp: float | None=None, success: bool=True, labels: dict[str, str] | None=None, metadata: dict[str, Any] | None=None) -> None:
        """Add a new SLI measurement"""
        measurement = SLIMeasurement(timestamp=timestamp or time.time(), value=value, success=success, labels=labels or {}, metadata=metadata or {})
        ...
        await task_manager.submit_enhanced_task(task_id=f'slo_unified_store_{self.slo_target.name}_{str(uuid.uuid4())[:8]}', coroutine=self._store_measurement_unified(measurement), priority=TaskPriority.NORMAL, tags={'service': 'slo', 'type': 'unified_cache_storage', 'component': 'slo_calculator', 'slo_name': self.slo_target.name})
    ...
    async def _store_measurement_unified(self, measurement: SLIMeasurement) -> None:
        ...
        try:
            ...
            data = {'timestamp': measurement.timestamp, 'value': measurement.value, 'success': int(measurement.success), 'labels': measurement.labels, 'metadata': measurement.metadata}
            ...
            if success:
                logger.debug('Stored SLI measurement for %s in unified cache', self.slo_target.name)
            else:
                logger.warning('Failed to store SLI measurement for %s in unified cache', self.slo_target.name)
        except Exception as e:
            logger.warning('Failed to store measurement in unified cache: %s', e)

    async def calculate_sli(self, time_window: SLOTimeWindow | None=None, end_time: datetime | None=None) -> SLIResult:
        ...
        try:
            ...
            if cached_result is not None:
                logger.debug('SLI result cache hit for %s', self.slo_target.name)
                return SLIResult(**cached_result)
        except Exception as e:
            logger.warning('Failed to check unified cache for SLI result: %s', e)
        if cache_key in self._result_cache:
            result, cache_time = self._result_cache[cache_key]
            if time.time() - cache_time < self._cache_ttl:
                return result
        window_measurements = [m for m in self._measurements if start_time.timestamp() <= m.timestamp <= end_time.timestamp()]
        if not window_measurements:
            result = SLIResult(slo_target=self.slo_target, time_window=time_window, current_value=0.0, target_value=self.slo_target.target_value, compliance_ratio=0.0, measurement_count=0, window_start=start_time, window_end=end_time)
        else:
            ...
            result = SLIResult(slo_target=self.slo_target, time_window=time_window, current_value=current_value, target_value=self.slo_target.target_value, compliance_ratio=compliance_ratio, measurement_count=len(window_measurements), window_start=start_time, window_end=end_time, min_value=min(values), max_value=max(values), median_value=statistics.median(values), std_deviation=statistics.stdev(values) if len(values) > 1 else 0.0)
        self._result_cache[cache_key] = (result, time.time())
        try:
            security_context = await self._ensure_security_context()
            result_data = {'slo_target': {'name': self.slo_target.name, 'service_name': self.slo_target.service_name, 'slo_type': self.slo_target.slo_type.value, 'target_value': self.slo_target.target_value, 'unit': self.slo_target.unit}, 'time_window': time_window.value, 'current_value': result.current_value, 'target_value': result.target_value, 'compliance_ratio': result.compliance_ratio, 'measurement_count': result.measurement_count, 'window_start': result.window_start.isoformat(), 'window_end': result.window_end.isoformat(), 'min_value': result.min_value, 'max_value': result.max_value, 'median_value': result.median_value, 'std_deviation': result.std_deviation}
            await self._unified_manager.set_cached(key=cache_key, value=result_data, ttl_seconds=self._cache_ttl, security_context=security_context)
        except Exception as e:
            logger.warning('Failed to cache SLI result in unified cache: %s', e)
        return result
...
class MultiWindowSLICalculator:
    ...
    def __init__(self, slo_target: SLOTarget, windows: list[SLOTimeWindow] | None=None, max_measurements: int=50000, redis_url: str | None=None, unified_manager=None):
        self.slo_target = slo_target
        self.windows = windows or [SLOTimeWindow.HOUR_1, SLOTimeWindow.DAY_1, SLOTimeWindow.WEEK_1, SLOTimeWindow.MONTH_1]
        self._unified_manager = unified_manager or get_unified_manager(ManagerMode.ASYNC_MODERN)
        self.calculators = {window: SLICalculator(slo_target=slo_target, max_measurements=max_measurements, redis_url=redis_url, unified_manager=self._unified_manager) for window in self.windows}
        self.trend_window = SLOTimeWindow.HOUR_1
        self.trend_points = 12

    def add_measurement(self, value: float, timestamp: float | None=None, success: bool=True, labels: dict[str, str] | None=None, metadata: dict[str, Any] | None=None) -> None:
    ...
    def calculate_all_windows(self, end_time: datetime | None=None) -> dict[SLOTimeWindow, SLIResult]:
        ...
        for window, calculator in self.calculators.items():
            try:
                ...
                logger.error('Failed to calculate SLI for window {window}: %s', e)
                results[window] = SLIResult(slo_target=self.slo_target, time_window=window, current_value=0.0, target_value=self.slo_target.target_value, compliance_ratio=0.0, measurement_count=0, window_start=end_time or datetime.now(UTC), window_end=end_time or datetime.now(UTC))
        return results
    ...
    def analyze_trends(self, end_time: datetime | None=None) -> dict[str, Any]:
        ...
        for i in range(self.trend_points):
            ...
            try:
                result = self.calculators[self.trend_window].calculate_sli(SLOTimeWindow.HOUR_1, point_end_time)
                trend_results.append({'timestamp': point_end_time.timestamp(), 'value': result.current_value, 'compliance_ratio': result.compliance_ratio})
            except Exception as e:
                logger.warning('Failed to calculate trend point {i}: %s', e)
        if len(trend_results) < 2:
            return {'trend_direction': 'unknown', 'trend_slope': 0.0, 'trend_confidence': 0.0, 'data_points': trend_results}
        ...
        if ss_tot > 0:
            ...
            r_squared = 1.0
        return {'trend_direction': direction, 'trend_slope': slope, 'trend_confidence': max(0.0, min(1.0, r_squared)), 'data_points': trend_results}
    ...
    def get_summary(self, end_time: datetime | None=None) -> dict[str, Any]:
        ...
        return {'slo_target': self.slo_target.name, 'service_name': self.slo_target.service_name, 'slo_type': self.slo_target.slo_type.value, 'overall_health_score': health_score, 'worst_performing_window': worst_window.value if worst_window else None, 'worst_compliance_ratio': worst_compliance, 'window_results': {window.value: {'current_value': result.current_value, 'target_value': result.target_value, 'compliance_ratio': result.compliance_ratio, 'is_compliant': result.is_compliant, 'measurement_count': result.measurement_count} for window, result in window_results.items()}, 'trend_analysis': trend_analysis, 'calculated_at': (end_time or datetime.now(UTC)).isoformat()}

class PercentileCalculator:
    """Specialized calculator for latency percentiles"""

    def __init__(self, percentiles: list[int]=[50, 90, 95, 99], max_samples: int=10000):
    ...
    def calculate_percentiles(self, window_seconds: int=3600) -> dict[int, float]:
        ...
        recent_samples = [s['value'] for s in self._samples if s['timestamp'] >= cutoff_time]
        ...
        for percentile in self.percentiles:
            ...
            if index.is_integer():
                ...
                upper_index = int(math.ceil(index))
                weight = index - lower_index
                results[percentile] = sorted_samples[lower_index] * (1 - weight) + sorted_samples[upper_index] * weight
        return results
    ...
    def get_statistics(self, window_seconds: int=3600) -> dict[str, float]:
        ...
        recent_samples = [s['value'] for s in self._samples if s['timestamp'] >= cutoff_time]
        if not recent_samples:
            return {'count': 0, 'min': 0.0, 'max': 0.0, 'mean': 0.0, 'median': 0.0, 'std_dev': 0.0}
        percentiles = self.calculate_percentiles(window_seconds)
        return {'count': len(recent_samples), 'min': min(recent_samples), 'max': max(recent_samples), 'mean': statistics.mean(recent_samples), 'median': statistics.median(recent_samples), 'std_dev': statistics.stdev(recent_samples) if len(recent_samples) > 1 else 0.0, **{f'p{p}': v for p, v in percentiles.items()}}
...
class AvailabilityCalculator:
    ...
    def record_event(self, success: bool, timestamp: float | None=None, labels: dict[str, str] | None=None) -> None:
        """Record an availability event"""
        self._events.append({'success': success, 'timestamp': timestamp or time.time(), 'labels': labels or {}})
    ...
    def calculate_availability(self, window_seconds: int=3600) -> dict[str, Any]:
        ...
        if not recent_events:
            return {'availability_percentage': 100.0, 'total_events': 0, 'successful_events': 0, 'failed_events': 0, 'mttr_seconds': 0.0, 'mtbf_seconds': 0.0}
        ...
        return {'availability_percentage': availability_percentage, 'total_events': total_events, 'successful_events': successful_events, 'failed_events': failed_events, 'mttr_seconds': mttr, 'mtbf_seconds': mtbf}
    ...
    def _calculate_mtbf(self, events: list[dict[str, Any]]) -> float:
        ...
        intervals = [failure_timestamps[i + 1] - failure_timestamps[i] for i in range(len(failure_timestamps) - 1)]
