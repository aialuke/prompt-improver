...
class SLOReporter:
    ...
    def __init__(self, template_dir: str | None=None, output_dir: str | None=None):
        ...
        if JINJA2_AVAILABLE:
            try:
                self.jinja_env = jinja2.Environment(loader=jinja2.FileSystemLoader(str(self.template_dir)), autoescape=jinja2.select_autoescape(['html', 'xml']))
            except Exception as e:
                logger.warning('Failed to initialize Jinja2 environment: %s', e)
                self.jinja_env = None
        else:
            self.jinja_env = None

    async def generate_compliance_report(self, slo_monitor: SLOMonitor, period: ReportPeriod, end_date: datetime | None=None) -> SLOComplianceReport:
        ...
        compliance_data = self._calculate_compliance_metrics(evaluation_results, start_date, end_date)
        trend_data = await self._analyze_compliance_trends(slo_monitor, start_date, end_date)
        incident_data = self._analyze_incidents_and_alerts(slo_monitor, start_date, end_date)
        recommendations = self._generate_recommendations(compliance_data, trend_data, incident_data)
        report = SLOComplianceReport(service_name=slo_monitor.slo_definition.service_name, slo_name=slo_monitor.slo_definition.name, report_period=period, start_date=start_date, end_date=end_date, overall_compliance_percentage=compliance_data['overall_compliance'], target_compliance_percentage=compliance_data['target_compliance'], compliance_status=compliance_data['status'], window_compliance=compliance_data['window_results'], error_budget_consumed_percentage=compliance_data['budget_consumed'], error_budget_remaining=compliance_data['budget_remaining'], error_budget_status=compliance_data['budget_status'], trend_direction=trend_data['direction'], trend_confidence=trend_data['confidence'], incident_count=incident_data['incident_count'], alert_count=incident_data['alert_count'], mean_time_to_recovery=incident_data['mttr'], recommendations=recommendations)
        return report
    ...
    def _calculate_compliance_metrics(self, evaluation_results: dict[str, Any], start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        return {'overall_compliance': overall_compliance, 'target_compliance': 99.0, 'status': status, 'window_results': window_results, 'budget_consumed': budget_consumed, 'budget_remaining': budget_remaining, 'budget_status': budget_status}

    async def _analyze_compliance_trends(self, slo_monitor: SLOMonitor, start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        if slo_monitor.calculators:
            ...
            return {'direction': trends.get('trend_direction', 'unknown'), 'confidence': trends.get('trend_confidence', 0.0), 'slope': trends.get('trend_slope', 0.0)}
        return {'direction': 'unknown', 'confidence': 0.0, 'slope': 0.0}

    def _analyze_incidents_and_alerts(self, slo_monitor: SLOMonitor, start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        return {'incident_count': total_incidents, 'alert_count': total_alerts, 'mttr': mttr}

    def _generate_recommendations(self, compliance_data: dict[str, Any], trend_data: dict[str, Any], incident_data: dict[str, Any]) -> list[str]:
        """Generate actionable recommendations based on report data"""
        recommendations = []
        if compliance_data['overall_compliance'] < 95.0:
            recommendations.append('SLO compliance is below target. Consider reviewing error handling, improving monitoring, or adjusting SLO targets.')
        if compliance_data['budget_consumed'] > 75:
            recommendations.append('Error budget consumption is high. Implement stricter change management and consider freezing non-essential deployments.')
        if trend_data['direction'] == 'degrading' and trend_data['confidence'] > 0.7:
            recommendations.append('Performance is showing a degrading trend. Investigate potential causes such as increased load, recent changes, or infrastructure issues.')
        if incident_data['incident_count'] > 3:
            recommendations.append('High incident count detected. Review alerting thresholds, implement better monitoring, and conduct postmortem reviews.')
        if incident_data['mttr'] > 60:
            recommendations.append('Mean time to recovery is high. Improve incident response procedures, automation, and on-call training.')
        if not recommendations:
            recommendations.append('SLO performance is within acceptable parameters. Continue monitoring and maintain current operational practices.')
        return recommendations

    def export_report(self, report: SLOComplianceReport, format: ReportFormat, filename: str | None=None) -> str:
    ...
    def _export_html(self, report: SLOComplianceReport, filepath: Path) -> str:
        ...
        if not self.jinja_env:
            ...
            try:
                ...
                logger.warning('Failed to use HTML template: %s', e)
                html_content = self._generate_simple_html(report)
        with open(filepath, 'w') as f:
...
class SLAReporter:
    ...
    def record_sla_breach(self, service_name: str, sla_target: str, target_value: float, actual_value: float, breach_start: datetime, customer_id: str | None=None, affected_requests: int=0, estimated_revenue_impact: float=0.0) -> SLABreachReport:
        """Record a new SLA breach"""
        impact_level = self._calculate_impact_level(affected_requests, estimated_revenue_impact)
        breach_report = SLABreachReport(service_name=service_name, customer_id=customer_id, breach_start=breach_start, breach_end=None, sla_target=sla_target, target_value=target_value, actual_value=actual_value, breach_duration_minutes=0.0, affected_requests=affected_requests, estimated_revenue_impact=estimated_revenue_impact, customer_impact_level=impact_level, time_to_detection_minutes=0.0, time_to_resolution_minutes=None, root_cause=None, remediation_actions=[], customer_notified=False, compensation_required=impact_level in ['high', 'critical'], postmortem_required=impact_level in ['high', 'critical'])
        self.breach_history.append(breach_report)
        logger.warning('SLA breach recorded: {service_name}/%s', sla_target)
        return breach_report

    def resolve_sla_breach(self, breach_report: SLABreachReport, resolution_time: datetime, root_cause: str | None=None, remediation_actions: list[str] | None=None) -> None:
        """Resolve an SLA breach"""
        breach_report.breach_end = resolution_time
        breach_report.breach_duration_minutes = (resolution_time - breach_report.breach_start).total_seconds() / 60
        if root_cause:
        ...
        logger.info('SLA breach resolved: %s/%s', breach_report.service_name, breach_report.sla_target)

    def _calculate_impact_level(self, affected_requests: int, revenue_impact: float) -> str:
    ...
    def generate_breach_summary(self, start_date: datetime, end_date: datetime) -> dict[str, Any]:
        """Generate SLA breach summary for period"""
        period_breaches = [breach for breach in self.breach_history if start_date <= breach.breach_start <= end_date]
        if not period_breaches:
            return {'period_start': start_date.isoformat(), 'period_end': end_date.isoformat(), 'total_breaches': 0, 'total_revenue_impact': 0.0, 'breach_summary': {}}
        ...
        for breach in period_breaches:
            if breach.service_name not in service_summary:
                service_summary[breach.service_name] = {'breach_count': 0, 'revenue_impact': 0.0, 'affected_requests': 0}
        ...
        return {'period_start': start_date.isoformat(), 'period_end': end_date.isoformat(), 'total_breaches': len(period_breaches), 'total_revenue_impact': total_revenue_impact, 'impact_level_summary': impact_summary, 'service_summary': service_summary, 'average_resolution_time_minutes': avg_resolution_time, 'resolution_rate': len(resolved_breaches) / len(period_breaches) if period_breaches else 0.0}
...
class DashboardGenerator:
    ...
    def generate_slo_dashboard(self, slo_reports: list[SLOComplianceReport], title: str='SLO Dashboard') -> str:
        """Generate comprehensive SLO dashboard"""
        dashboard_data = {'title': title, 'generated_at': datetime.now(UTC).isoformat(), 'services': []}
        for report in slo_reports:
            service_data = {'name': report.service_name, 'slo_name': report.slo_name, 'compliance_percentage': report.overall_compliance_percentage, 'compliance_status': report.compliance_status, 'error_budget_consumed': report.error_budget_consumed_percentage, 'trend_direction': report.trend_direction, 'incident_count': report.incident_count, 'alert_count': report.alert_count}
    ...
    def _generate_dashboard_html(self, data: dict[str, Any]) -> str:
        ...
        for service in data['services']:
            ...
            trend_icon = {'improving': '↗️', 'stable': '→', 'degrading': '↘️'}.get(service['trend_direction'], '?')
...
class ExecutiveReporter:
    ...
    def generate_executive_summary(self, slo_reports: list[SLOComplianceReport], sla_breaches: list[SLABreachReport], period: ReportPeriod) -> dict[str, Any]:
        ...
        return {'reporting_period': period.value, 'executive_summary': {'overall_service_health': availability_score, 'total_incidents': total_incidents, 'average_resolution_time_hours': avg_mttr / 60, 'revenue_impact_total': total_revenue_impact, 'affected_customer_requests': total_affected_requests, 'customer_satisfaction_impact': satisfaction_impact, 'risk_level': risk_level}, 'key_metrics': {'service_availability': f'{avg_compliance:.2f}%', 'sla_breach_count': len(sla_breaches), 'revenue_at_risk': f'${total_revenue_impact:,.2f}', 'operational_excellence_score': self._calculate_operational_score(slo_reports)}, 'recommendations': self._generate_executive_recommendations(avg_compliance, total_incidents, total_revenue_impact, risk_level), 'next_period_projections': self._generate_projections(slo_reports, sla_breaches)}
    ...
    def _assess_risk_level(self, compliance: float, incidents: int, revenue_impact: float) -> str:
    ...
    def _calculate_operational_score(self, reports: list[SLOComplianceReport]) -> int:
        ...
        total_score = compliance_score * compliance_weight + budget_score * budget_weight + incident_score * incident_weight + trend_score * trend_weight
        return int(total_score)

    def _generate_executive_recommendations(self, compliance: float, incidents: int, revenue_impact: float, risk_level: str) -> list[str]:
        """Generate executive-level recommendations"""
        recommendations = []
        if risk_level in ['critical', 'high']:
            recommendations.append('IMMEDIATE ACTION REQUIRED: Service reliability is below acceptable levels. Recommend emergency review of operational procedures and resource allocation.')
        if compliance < 95:
            recommendations.append('Service availability is impacting customer experience. Recommend increased investment in infrastructure reliability and monitoring capabilities.')
        if revenue_impact > 10000:
            recommendations.append(f'Significant revenue impact (${revenue_impact:,.2f}) from service issues. Consider implementing stricter change management and improved testing procedures.')
        if incidents > 5:
            recommendations.append('High incident count indicates systemic issues. Recommend comprehensive review of architecture and implementation of chaos engineering practices.')
        if not recommendations:
            recommendations.append('Service performance is meeting business objectives. Continue current operational practices and consider optimization opportunities.')
        return recommendations

    def _generate_projections(self, slo_reports: list[SLOComplianceReport], sla_breaches: list[SLABreachReport]) -> dict[str, Any]:
        ...
        return {'trend_direction': projected_change, 'projected_compliance': f'{projected_compliance:.1f}%', 'projected_revenue_impact': f'${projected_revenue_impact:,.2f}', 'confidence': 'medium', 'recommended_actions': ['Continue monitoring trends', 'Review quarterly SLO targets', 'Assess infrastructure capacity planning']}
