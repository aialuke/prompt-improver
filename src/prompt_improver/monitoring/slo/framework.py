...
class SLOTimeWindow(Enum):
    ...
    def seconds(self) -> int:
        ...
        mapping = {'1h': 3600, '6h': 21600, '24h': 86400, '3d': 259200, '7d': 604800, '14d': 1209600, '30d': 2592000, '90d': 7776000}
        return mapping[self.value]

    @property
    def human_readable(self) -> str:
        """Human readable time window"""
        mapping = {'1h': '1 hour', '6h': '6 hours', '24h': '24 hours', '3d': '3 days', '7d': '7 days', '14d': '14 days', '30d': '30 days', '90d': '90 days'}
        return mapping[self.value]

class SLOTarget(SQLModel):
...
class SLOTemplates:
    ...
    def web_service_availability(service_name: str) -> SLODefinition:
        ...
        slo_def = SLODefinition(name=f'{service_name}_availability', service_name=service_name, description='Web service availability SLO', owner_team='platform')
        for window, target in [(SLOTimeWindow.DAY_1, 99.9), (SLOTimeWindow.WEEK_1, 99.5), (SLOTimeWindow.MONTH_1, 99.0)]:
            slo_def.add_target(SLOTarget(name=f'availability_{window.value}', slo_type=SLOType.AVAILABILITY, target_value=target, time_window=window, description=f'Service availability over {window.human_readable}', measurement_query=f'rate(http_requests_total{{service="{service_name}",status!~"5.."}}' + f'[{window.value}]) / rate(http_requests_total{{service="{service_name}"}}[{window.value}])'))
        return slo_def

    @staticmethod
    def api_latency(service_name: str) -> SLODefinition:
        """Standard latency SLO for API services"""
        slo_def = SLODefinition(name=f'{service_name}_latency', service_name=service_name, description='API latency SLO', owner_team='platform')
        for percentile, target_ms in [('p50', 100), ('p95', 500), ('p99', 1000)]:
            slo_def.add_target(SLOTarget(name=f'latency_{percentile}', slo_type=SLOType.LATENCY, target_value=target_ms, time_window=SLOTimeWindow.DAY_1, unit='ms', description=f'API {percentile} latency under {target_ms}ms', measurement_query=f'histogram_quantile(0.{percentile[1:]}, ' + f'rate(http_request_duration_seconds_bucket{{service="{service_name}"}}[24h]))'))
        return slo_def

    @staticmethod
    def error_rate(service_name: str) -> SLODefinition:
        """Standard error rate SLO"""
        slo_def = SLODefinition(name=f'{service_name}_error_rate', service_name=service_name, description='Service error rate SLO', owner_team='platform')
        slo_def.add_target(SLOTarget(name='error_rate_24h', slo_type=SLOType.ERROR_RATE, target_value=1.0, time_window=SLOTimeWindow.DAY_1, description='Error rate under 1% over 24 hours', measurement_query=f'rate(http_requests_total{{service="{service_name}",status=~"5.."}}[24h]) ' + f'/ rate(http_requests_total{{service="{service_name}"}}[24h]) * 100'))
        return slo_def
