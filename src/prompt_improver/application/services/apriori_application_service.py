...
class AprioriApplicationService:
    ...
    async def execute_apriori_analysis(
        ...
        try:
            ...
            if cached_result is not None:
                self.logger.info("Returning cached Apriori analysis result")
            ...
            async with self.session_manager.get_session() as db_session:
                try:
                    ...
                    self.logger.exception(f"Error in Apriori analysis transaction: {e}")
                    return AprioriAnalysisResponseData(
                        status="error",
                        error=f"Analysis transaction failed: {e!s}",
                        analysis_run_id=analysis_id,
                    )

        except Exception as e:
            self.logger.exception(f"Error in Apriori analysis workflow: {e}")
            return AprioriAnalysisResponseData(
                status="error",
    ...
    async def get_association_rules(
        ...
        try:
            ...
            self.logger.exception(f"Error retrieving association rules: {e}")
            raise
    ...
    async def get_contextualized_patterns(
        ...
        try:
            ...
            self.logger.exception(f"Error getting contextualized patterns: {e}")
            raise
