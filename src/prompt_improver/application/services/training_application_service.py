...
class TrainingApplicationService:
    ...
    async def start_training_workflow(
        ...
        try:
            ...
            self.logger.exception(f"Failed to start training workflow {workflow_id}: {e}")
            return {
                "status": "error",
                "workflow_id": workflow_id,
                "error": str(e),
    ...
    async def monitor_training_progress(
        ...
        try:
            ...
            self.logger.exception(f"Failed to monitor workflow {workflow_id}: {e}")
            return {
                "status": "error",
                "workflow_id": workflow_id,
                "error": str(e),
    ...
    async def pause_training_workflow(
        ...
        try:
            ...
            self.logger.exception(f"Failed to pause workflow {workflow_id}: {e}")
            return {
                "status": "error",
                "workflow_id": workflow_id,
                "error": str(e),
    ...
    async def resume_training_workflow(
        ...
        try:
            ...
            self.logger.exception(f"Failed to resume workflow {workflow_id}: {e}")
            return {
                "status": "error",
                "workflow_id": workflow_id,
                "error": str(e),
    ...
    async def _execute_workflow_async(
        ...
        try:
            ...
            self.logger.exception(f"Workflow {workflow_id} failed: {e}")
            if workflow_id in self.active_workflows:
    ...
    async def _get_detailed_progress(self, workflow_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.exception(f"Failed to get progress for {workflow_id}: {e}")
            return {}
    ...
    async def _create_workflow_checkpoint(
        ...
        try:
            ...
            self.logger.exception(f"Failed to create checkpoint for {workflow_id}: {e}")
            return None
    ...
    async def _restore_from_checkpoint(
        ...
        try:
            ...
            self.logger.exception(f"Failed to restore checkpoint for {workflow_id}: {e}")
            return None
    ...
    async def _stop_workflow_with_enhanced_manager(
        ...
        try:
            ...
            return await self.enhanced_workflow_manager.wait_for_workflow_completion(
                workflow_id=workflow_id,
                timeout=timeout,
                config=config,
            ...
            self.logger.exception(f"Enhanced manager stop failed for {workflow_id}: {e}")
    ...
    async def _create_final_checkpoint(
        ...
        try:
            final_checkpoint = {
                ...
                "total_runtime_seconds": (datetime.now(UTC) - workflow_info["started_at"]).total_seconds(),
            ...
            self.logger.exception(f"Failed to create final checkpoint for {workflow_id}: {e}")
            return None
