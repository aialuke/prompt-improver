...
class PatternSignificanceReport:
    ...
    pattern_interactions: Optional[Dict[str, Any]] = None
...
class PatternSignificanceAnalyzer:
    ...
    def analyze_pattern_significance(self,
                                   ...
                                   pattern_types: Optional[Dict[str, PatternType]] = None) -> PatternSignificanceReport:
        """
        ...
        try:
            ...
            if self.apply_multiple_testing_correction and len(test_results) > 1:
                ...
                significant_patterns = [r for r in test_results 
                                     if multiple_testing_correction['corrected_p_values'][test_results.index(r)] < self.alpha]
                non_significant_patterns = [r for r in test_results 
                                          if multiple_testing_correction['corrected_p_values'][test_results.index(r)] >= self.alpha]
            ...
            logger.info(f"Found {len(significant_patterns)} significant patterns out of {len(test_results)} tested")
    ...
    def _validate_pattern_data(self, patterns_data: Dict[str, Any], 
                             control_data: Dict[str, Any], 
        ...
        for pattern_id, pattern_info in patterns_data.items():
            try:
                ...
                if control_pattern is None or treatment_pattern is None:
                    logger.warning(f"Pattern {pattern_id} missing in control or treatment data")
                    continue
                ...
                if control_size < self.min_sample_size or treatment_size < self.min_sample_size:
                    logger.warning(f"Pattern {pattern_id} has insufficient sample size: control={control_size}, treatment={treatment_size}, required={self.min_sample_size}")
                    continue
    ...
    def _test_pattern_significance(self, pattern_id: str, pattern_type: PatternType,
                                 pattern_data: Any, control_data: Dict[str, Any],
                                 treatment_data: Dict[str, Any]) -> Optional[PatternTestResult]:
        ...
        try:
            ...
            return PatternTestResult(
                ...
                metadata={
                    ...
                    'sequence_counts': {'control': control_counts, 'treatment': treatment_counts}
                }
            )
    ...
    def _bootstrap_effect_size_ci(self, control_data: np.ndarray, treatment_data: np.ndarray, 
    ...
    def _analyze_pattern_interactions(self, significant_patterns: List[PatternTestResult],
                                   control_data: Dict[str, Any], 
                                   treatment_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_business_insights(self, significant_patterns: List[PatternTestResult],
                                  pattern_interactions: Dict[str, Any]) -> List[str]:
    ...
    def _calculate_quality_score(self, test_results: List[PatternTestResult],
                               multiple_testing_correction: Optional[Dict[str, Any]]) -> float:
    ...
    def _interpret_categorical_result(self, p_value: float, effect_size: float, 
                                    test_method: SignificanceMethod) -> str:
    ...
    def _generate_categorical_recommendations(self, p_value: float, effect_size: float, 
...
def quick_pattern_analysis(patterns_data: Dict[str, Any],
                         control_data: Dict[str, Any],
                         treatment_data: Dict[str, Any],
    ...
    try:
        ...
        return {
            ...
            'top_patterns': [
                ...
                for p in sorted(report.significant_patterns, key=lambda x: x.p_value)[:5]
            ],
        ...
        return {
            ...
            'false_discovery_rate': 0.0,
            'top_patterns': [],
            'business_insights': [f"Analysis error: {str(e)}"],
            'quality_score': 0.0
        }