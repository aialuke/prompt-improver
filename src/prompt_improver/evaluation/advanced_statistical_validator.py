...
class AdvancedValidationResult:
    ...
    multiple_testing_correction: Optional[MultipleTestingCorrection] = None
    
    # Sensitivity analysis
    sensitivity_analysis: Optional[Dict[str, Any]] = None
    
    # Power analysis
    post_hoc_power: Optional[float] = None
    prospective_power: Optional[float] = None
    
    # Assumptions testing
    normality_tests: Optional[Dict[str, StatisticalTestResult]] = None
    homogeneity_tests: Optional[Dict[str, StatisticalTestResult]] = None
    
    # Robustness checks
    non_parametric_tests: Optional[Dict[str, StatisticalTestResult]] = None
...
class AdvancedStatisticalValidator:
    ...
    def validate_ab_test(self,
                        ...
                        include_sensitivity: bool = True) -> AdvancedValidationResult:
        """
        ...
        try:
            ...
            # Effect size analysis
            effect_size_magnitude, practical_sig, clinical_sig = self._analyze_effect_size(
            ...
            if test_names and len(test_names) > 1:
                result.multiple_testing_correction = self._apply_multiple_testing_correction(
    ...
    def _calculate_cohens_d(self, control: np.ndarray, treatment: np.ndarray) -> float:
        ...
        try:
            ...
            if pooled_std == 0:
                ...
                if abs(mean_diff) > 1e-10:  # Use a reasonable threshold for numerical precision
    ...
    def _calculate_difference_ci(self, control: np.ndarray, treatment: np.ndarray, 
    ...
    def _calculate_mde(self, control: np.ndarray, treatment: np.ndarray) -> float:
        ...
        try:
            n1, n2 = len(control), len(treatment)
            pooled_var = ((n1 - 1) * np.var(control, ddof=1) + 
    ...
    def _analyze_effect_size(self, control: np.ndarray, treatment: np.ndarray, 
                           effect_size: float) -> Tuple[EffectSizeMagnitude, bool, bool]:
        ...
        try:
            ...
            clinical_significance = bool(abs(mean_improvement) >= 0.1)  # 10% relative improvement
    ...
    def _apply_multiple_testing_correction(self, p_values: List[float], 
                                         method: CorrectionMethod) -> MultipleTestingCorrection:
        ...
        try:
            ...
            return MultipleTestingCorrection(
                ...
                alpha_corrected=alpha_bonf if method == CorrectionMethod.BONFERRONI else self.alpha,
    ...
    def _test_normality_assumptions(self, control: np.ndarray, 
                                  treatment: np.ndarray) -> Dict[str, StatisticalTestResult]:
    ...
    def _test_homogeneity_assumptions(self, control: np.ndarray, 
                                    treatment: np.ndarray) -> Dict[str, StatisticalTestResult]:
    ...
    def _perform_non_parametric_tests(self, control: np.ndarray, 
                                    treatment: np.ndarray) -> Dict[str, StatisticalTestResult]:
        ...
        try:
            ...
            results['mann_whitney'] = StatisticalTestResult(
                ...
                statistic=float(stat),
                p_value=float(p_value),
                notes=["Non-parametric", "Tests median differences", "Robust to outliers"]
            )
    ...
    def _calculate_post_hoc_power(self, control: np.ndarray, treatment: np.ndarray, 
    ...
    def _perform_bootstrap_analysis(self, control: np.ndarray, 
        ...
        try:
            ...
            return {
                ...
                'mean_difference': {
                    ...
                    'confidence_interval_95': [float(ci_lower_diff), float(ci_upper_diff)]
                },
                'effect_size': {
    ...
    def _perform_sensitivity_analysis(self, control: np.ndarray, 
                                    treatment: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'outliers_removed': {
                    ...
                    'total': (len(control) + len(treatment)) - (len(control_clean) + len(treatment_clean))
                },
    ...
    def _generate_recommendations(self, result: AdvancedValidationResult) -> List[str]:
        ...
        try:
            ...
            if result.normality_tests:
                normal_violations = [test for test in result.normality_tests.values() 
            ...
            if result.bootstrap_results and result.primary_test.confidence_interval:
                ...
                if bootstrap_ci and parametric_ci:
                    ...
                    if (abs(bootstrap_ci[0] - parametric_ci[0]) > 0.1 or 
                        abs(bootstrap_ci[1] - parametric_ci[1]) > 0.1):
...
def quick_validation(control_data: List[float], 
                    treatment_data: List[float],
