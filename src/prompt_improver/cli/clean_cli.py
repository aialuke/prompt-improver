...
app = typer.Typer(name='apes', help='APES - Ultra-Minimal ML Training System', rich_markup_mode='rich')
console = Console()
...
@app.command()
def train(continuous: bool=typer.Option(True, '--continuous/--single', help='Continuous learning mode (default) or single run'), max_iterations: int | None=typer.Option(None, '--max-iterations', '-i', help='Maximum training iterations (unlimited by default)'), improvement_threshold: float=typer.Option(0.02, '--threshold', '-t', help='Minimum improvement threshold (2% default)'), timeout: int=typer.Option(3600, '--timeout', help='Training timeout in seconds (1 hour default)'), auto_init: bool=typer.Option(True, '--auto-init/--no-init', help='Auto-initialize missing components'), verbose: bool=typer.Option(False, '--verbose', '-v', help='Verbose output')) -> None:
    ...
    async def run_training():
        ...
        try:
            ...
            if not await training_manager.validate_ready_for_training():
                ...
                raise typer.Exit(1)
            session_config = {'continuous_mode': continuous, 'max_iterations': max_iterations, 'improvement_threshold': improvement_threshold, 'timeout': timeout, 'verbose': verbose}
            ...
            if continuous:
                ...
                with Progress(SpinnerColumn(), TextColumn('[progress.description]{task.description}'), BarColumn(), TaskProgressColumn(), console=console, expand=True) as progress:
                    ...
                    training_task_id = await task_manager.submit_enhanced_task(task_id=f'cli_training_{session.session_id}_{str(uuid.uuid4())[:8]}', coroutine=cli_orchestrator.start_continuous_training(session_id=session.session_id, config=session_config), priority=TaskPriority.CRITICAL, tags={'service': 'cli', 'type': 'training', 'component': 'clean_cli', 'session_id': session.session_id})
                    shutdown_task_id = await task_manager.submit_enhanced_task(task_id=f'cli_shutdown_{str(uuid.uuid4())[:8]}', coroutine=enhanced_graceful_shutdown(), priority=TaskPriority.HIGH, tags={'service': 'cli', 'type': 'shutdown', 'component': 'clean_cli'})
                    training_task = task_manager.get_task(training_task_id)
                    shutdown_task = task_manager.get_task(shutdown_task_id)
                    done, pending = await asyncio.wait([training_task, shutdown_task], return_when=asyncio.FIRST_COMPLETED)
                    for task in pending:
                        task.cancel()
                        try:
                            await task
                ...
                results = await cli_orchestrator.start_single_training(session_id=session.session_id, config=session_config)
...
@app.command()
def status(detailed: bool=typer.Option(False, '--detailed', '-d', help='Show detailed status'), json_output: bool=typer.Option(False, '--json', help='Output as JSON'), refresh: int=typer.Option(0, '--refresh', '-r', help='Auto-refresh interval in seconds')) -> None:
...
@app.command()
def why5(issue: str | None=typer.Argument(None, help='The problem or issue to analyze'), depth: int=typer.Option(5, '--depth', '-d', help="Number of 'why' iterations (default: 5)"), export: bool=typer.Option(False, '--export', '-e', help='Export analysis to file'), format: str=typer.Option('text', '--format', '-f', help='Output format: text, json, or markdown')) -> None:
    ...
    async def run_five_whys():
        ...
        try:
            ...
            analysis_results = {'problem_statement': issue_description, 'timestamp': datetime.now(UTC).isoformat(), 'analysis_chain': [], 'root_cause': None, 'proposed_solutions': []}
            current_question = issue_description
            for i in range(depth):
                why_number = i + 1
                console.print(f'\n❓ Why #{why_number}: {current_question}', style='yellow')
                answer = typer.prompt('   Answer')
                analysis_step = {'why_number': why_number, 'question': current_question, 'answer': answer}
...
@app.command()
def stop(graceful: bool=typer.Option(True, '--graceful/--force', help='Graceful shutdown'), timeout: int=typer.Option(30, '--timeout', '-t', help='Shutdown timeout in seconds'), save_progress: bool=typer.Option(True, '--save-progress/--no-save', help='Save training progress'), session_id: str | None=typer.Option(None, '--session', '-s', help='Specific session ID to stop'), export_results: bool=typer.Option(True, '--export-results/--no-export', help='Export training results during shutdown'), export_format: str=typer.Option('json', '--export-format', help='Export format (json or csv)')) -> None:
    ...
    async def enhanced_stop_training():
        try:
            ...
            for session in sessions_to_stop:
                ...
                if graceful:
                    result = await cli_orchestrator.stop_training_gracefully(session_id=session.session_id, _timeout=timeout, save_progress=save_progress)
                    if result.get('success'):
                        ...
                        if export_results:
                            ...
                            export_path = await progress_manager.export_session_results(session_id=session.session_id, export_format=export_format, include_iterations=True)
                            if export_path:
                    ...
                    if save_progress:
                        ...
                        try:
                            await progress_manager.save_training_progress(session_id=session.session_id, iteration=session.total_iterations or 0, performance_metrics=session.best_performance or {}, rule_optimizations={}, workflow_state={'force_shutdown': True}, improvement_score=0.0)
