...
class AsyncSignalHandler:
    ...
    def __init__(self, console: Console | None=None):
        ...
        self.signal_operations: dict[int, SignalOperation] = {signal.SIGUSR1: SignalOperation.CHECKPOINT, signal.SIGUSR2: SignalOperation.STATUS_REPORT, signal.SIGHUP: SignalOperation.CONFIG_RELOAD, signal.SIGINT: SignalOperation.SHUTDOWN, signal.SIGTERM: SignalOperation.SHUTDOWN}
        ...
        if sys.platform != 'win32':
            for sig in [signal.SIGINT, signal.SIGTERM, signal.SIGUSR1, signal.SIGUSR2, signal.SIGHUP]:
                try:
                    ...
                    pass
            self.loop.add_signal_handler(signal.SIGINT, self._handle_signal, signal.SIGINT, 'SIGINT')
            self.loop.add_signal_handler(signal.SIGTERM, self._handle_signal, signal.SIGTERM, 'SIGTERM')
            self.loop.add_signal_handler(signal.SIGUSR1, self._handle_signal, signal.SIGUSR1, 'SIGUSR1')
            self.loop.add_signal_handler(signal.SIGUSR2, self._handle_signal, signal.SIGUSR2, 'SIGUSR2')
            self.loop.add_signal_handler(signal.SIGHUP, self._handle_signal, signal.SIGHUP, 'SIGHUP')
    ...
    def _handle_signal(self, signum: int, signal_name: str) -> None:
        ...
        if not operation:
            self.logger.warning('Unknown signal {signum} (%s) received', signal_name)
            return
    ...
    def _handle_shutdown_signal(self, signum: int, signal_name: str, current_time: datetime) -> None:
        ...
        if chain_results:
            self.logger.info('Signal chain executed for %s: %s handlers', signal_name, len(chain_results))
        ...
        if len(received_shutdown_signals) > 1 and self.first_signal_time is not None:
            ...
            if time_since_first < 5:
                self.console.print('\n⚡ Multiple shutdown signals received - Force shutdown initiated!', style='bold red')
        ...
        self.shutdown_context = ShutdownContext(reason=reason, signal_name=signal_name, timeout=30, save_progress=True)
        ...
        self.console.print('   Training progress will be preserved. Please wait...', style='dim')
        if not self.shutdown_event.is_set():
            self.shutdown_event.set()
            self.logger.info('Graceful shutdown initiated by %s', signal_name)
    ...
    def _handle_operation_signal(self, signum: int, signal_name: str, operation: SignalOperation, current_time: datetime) -> None:
        ...
        signal_context = SignalContext(operation=operation, signal_name=signal_name, signal_number=signum, triggered_at=current_time)
        operation_messages = {SignalOperation.CHECKPOINT: '📋 Creating checkpoint...', SignalOperation.STATUS_REPORT: '📊 Generating status report...', SignalOperation.CONFIG_RELOAD: '🔄 Reloading configuration...'}
        ...
        if operation in self.operation_handlers:
            try:
                if self.loop and self.loop.is_running():
                    loop = self.loop
                    self.loop.call_soon_threadsafe(lambda: loop.create_task(self._execute_operation_handler(operation, signal_context)))
                else:
                    self.logger.info('Event loop not available, executing %s synchronously', operation.value)
                    self.execute_operation_sync(operation, signal_context)
            except Exception as e:
                self.logger.error('Error scheduling {operation.value} operation: %s', e)
                self.console.print(f'❌ Failed to execute {operation.value}: {e}', style='red')
        else:
            self.logger.warning('No handler registered for %s operation', operation.value)
    ...
    def register_shutdown_handler(self, name: str, handler: Callable) -> None:
        ...
        self.logger.debug('Registered shutdown handler: %s', name)
    ...
    def register_cleanup_handler(self, name: str, handler: Callable) -> None:
        ...
        self.logger.debug('Registered cleanup handler: %s', name)
    ...
    def register_operation_handler(self, operation: SignalOperation, handler: Callable) -> None:
        ...
        self.logger.debug('Registered operation handler for %s', operation.value)
    ...
    def add_signal_chain_handler(self, signum: int, handler: Callable, priority: int=0) -> None:
        ...
        self.logger.debug('Added signal chain handler for signal %s with priority %s', signum, priority)
    ...
    def remove_signal_chain_handler(self, signum: int, handler: Callable) -> bool:
        ...
        for i, (_priority, existing_handler) in enumerate(self.signal_chain[signum]):
            if existing_handler == handler:
                ...
                self.logger.debug('Removed signal chain handler for signal %s', signum)
                return True
        return False
    ...
    def execute_signal_chain(self, signum: int, signal_name: str) -> dict[str, Any]:
        ...
        self.logger.info('Executing signal chain for %s with %s handlers', signal_name, len(chain_handlers))
        for i, (priority, handler) in enumerate(chain_handlers):
            try:
                self.logger.debug('Executing chain handler %s/%s (priority %s)', i + 1, len(chain_handlers), priority)
                result = handler(signum, signal_name)
                results[f'handler_{i}'] = {'status': 'success', 'result': result, 'priority': priority}
            except Exception as e:
                self.logger.error('Error in signal chain handler {i + 1}: %s', e)
                results[f'handler_{i}'] = {'status': 'error', 'error': str(e), 'priority': priority}
                continue
        self.logger.info('Signal chain execution completed for %s', signal_name)
        return results
    ...
    def _register_default_emergency_handlers(self) -> None:
        ...
        if not self.emergency_ops:
            self.register_operation_handler(SignalOperation.CHECKPOINT, self._default_checkpoint_handler)
            self.register_operation_handler(SignalOperation.STATUS_REPORT, self._default_status_handler)
            self.register_operation_handler(SignalOperation.CONFIG_RELOAD, self._default_config_reload_handler)
            return
        try:
            self.register_operation_handler(SignalOperation.CHECKPOINT, self.emergency_ops.create_emergency_checkpoint)
            self.register_operation_handler(SignalOperation.STATUS_REPORT, self.emergency_ops.generate_status_report)
            self.register_operation_handler(SignalOperation.CONFIG_RELOAD, self.emergency_ops.reload_configuration)
            self.logger.info('Default emergency operation handlers registered')
        except Exception as e:
            self.logger.error('Failed to register emergency handlers: %s', e)
    ...
    async def _default_checkpoint_handler(self, _context) -> dict[str, Any]:
        ...
        return {'status': 'checkpoint_created', 'timestamp': datetime.now(UTC).isoformat(), 'message': 'Basic checkpoint created without emergency operations'}
    ...
    async def _default_status_handler(self, _context) -> dict[str, Any]:
        ...
        return {'status': 'running', 'timestamp': datetime.now(UTC).isoformat(), 'message': 'Basic status report (emergency operations not loaded)'}
    ...
    async def _default_config_reload_handler(self, _context) -> dict[str, Any]:
        ...
        return {'status': 'config_reloaded', 'timestamp': datetime.now(UTC).isoformat(), 'message': 'Basic config reload (emergency operations not loaded)'}
    ...
    async def _execute_operation_handler(self, operation: SignalOperation, context: SignalContext) -> None:
        ...
        if not handler:
            return None
        try:
            self.logger.info('Executing %s operation', operation.value)
            result = await handler(context)
            success_messages = {SignalOperation.CHECKPOINT: '✅ Checkpoint created successfully', SignalOperation.STATUS_REPORT: '✅ Status report generated', SignalOperation.CONFIG_RELOAD: '✅ Configuration reloaded'}
            message = success_messages.get(operation, f'✅ {operation.value} completed')
            self.console.print(message, style='green')
            return result
        except Exception as e:
            self.logger.error('Error executing {operation.value} operation: %s', e)
    ...
    def execute_operation_sync(self, operation: SignalOperation, context: SignalContext) -> None:
        ...
        handler = self.operation_handlers.get(operation)
        if not handler:
            self.logger.warning('No handler registered for %s', operation.value)
            return
        try:
            self.logger.info('Executing %s operation synchronously', operation.value)
            try:
                loop = asyncio.get_running_loop()
                _task = loop.create_task(handler(context))
                self.logger.info('Scheduled %s operation as task', operation.value)
            except RuntimeError:
    ...
    def cleanup_signal_handlers(self) -> None:
        ...
        cleanup_order = ['coredis_connections', 'database_connections', 'file_handles', 'asyncio_handlers', 'original_handlers']
        for cleanup_step in cleanup_order:
            try:
                ...
                self.logger.debug('Completed cleanup step: %s', cleanup_step)
            except Exception as e:
                self.logger.error('Error in cleanup step {cleanup_step}: %s', e)
    ...
    def _cleanup_coredis_connections(self) -> None:
        ...
        try:
            ...
            self.logger.warning('CoreDis cleanup failed: %s', e)
    ...
    def _cleanup_database_connections(self) -> None:
        ...
        try:
            ...
            self.logger.warning('Database cleanup scheduling failed: %s', e)
        ...
        if sys.platform != 'win32' and self.loop:
            for sig in [signal.SIGINT, signal.SIGTERM, signal.SIGUSR1, signal.SIGUSR2, signal.SIGHUP]:
                try:
                    ...
                    self.logger.debug('Removed asyncio handler for signal %s', sig)
    ...
    def _restore_original_handlers(self) -> None:
        ...
        for sig, original_handler in self.original_handlers.items():
            try:
                ...
                self.logger.debug('Restored original handler for signal %s', sig)
            except (OSError, ValueError):
                pass
    ...
    def _initiate_force_shutdown(self) -> None:
        ...
        self.shutdown_context = ShutdownContext(reason=ShutdownReason.FORCE, signal_name='FORCE', timeout=5, save_progress=False, force_after_timeout=False)
        if not self.shutdown_event.is_set():
            self.shutdown_event.set()
    ...
    async def execute_graceful_shutdown(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'status': 'success', 'reason': self.shutdown_context.reason.value if self.shutdown_context else 'unknown', 'duration_seconds': shutdown_duration, 'shutdown_results': shutdown_results, 'cleanup_results': cleanup_results, 'progress_saved': self.shutdown_context.save_progress if self.shutdown_context else False}
        except TimeoutError:
            ...
            if self.shutdown_context and self.shutdown_context.force_after_timeout:
                return await self._execute_force_shutdown()
            return {'status': 'error', 'error': str(e), 'duration_seconds': (datetime.now(UTC) - shutdown_start).total_seconds()}
        finally:
            self.shutdown_in_progress = False
    ...
    async def _execute_shutdown_handlers(self) -> dict[str, Any]:
        ...
        for name, handler in self.shutdown_handlers.items():
            try:
                self.logger.debug('Executing shutdown handler: %s', name)
                ...
                self.logger.warning('Shutdown handler %s timed out after %ss', name, timeout)
                results[name] = {'status': 'timeout', 'timeout': timeout}
                self.console.print(f'   ⏰ {name} (timed out)', style='dim yellow')
            except Exception as e:
                self.logger.error('Error in shutdown handler {name}: %s', e)
    ...
    async def _execute_cleanup_handlers(self) -> dict[str, Any]:
        ...
        for name, handler in self.cleanup_handlers.items():
            try:
                self.logger.debug('Executing cleanup handler: %s', name)
                result = await asyncio.wait_for(handler(), timeout=10)
                results[name] = {'status': 'success', 'result': result}
                self.console.print(f'   ✓ {name}', style='dim green')
            except TimeoutError:
                self.logger.warning('Cleanup handler %s timed out', name)
                results[name] = {'status': 'timeout'}
                self.console.print(f'   ⏰ {name} (timed out)', style='dim yellow')
            except Exception as e:
                self.logger.error('Error in cleanup handler {name}: %s', e)
                results[name] = {'status': 'error', 'error': str(e)}
                self.console.print(f'   ❌ {name} (error)', style='dim red')
        return results
    ...
    async def _execute_force_shutdown(self) -> dict[str, Any]:
        ...
        return {'status': 'force_shutdown', 'reason': 'timeout_or_force', 'duration_seconds': force_duration, 'critical_cleanup': critical_cleanup, 'progress_saved': False}
