...
class UnifiedProcessManager:
    ...
    def __init__(self, pid_dir: Path | None=None, backup_dir: Path | None=None, lock_timeout: float=5.0):
        ...
        for directory in [self.pid_dir, self.backup_dir]:
            try:
                ...
                try:
                    ...
                    pass
                self.logger.debug('Secured directory: %s', directory)
            except Exception as e:
                self.logger.warning('Failed to secure directory {directory}: %s', e)
    ...
    def _register_signal_handlers(self):
        ...
        if self.signal_handler is None:
            ...
            return
        import signal
        self.signal_handler.register_shutdown_handler('UnifiedProcessManager_shutdown', self.graceful_process_shutdown)
        self.signal_handler.register_operation_handler(SignalOperation.CHECKPOINT, self.perform_emergency_crash_detection)
        self.signal_handler.register_operation_handler(SignalOperation.STATUS_REPORT, self.generate_process_status_report)
        self.signal_handler.add_signal_chain_handler(signal.SIGTERM, self.prepare_process_cleanup, priority=self._shutdown_priority)
        self.signal_handler.add_signal_chain_handler(signal.SIGINT, self.prepare_process_interruption, priority=self._shutdown_priority)
        ...
        try:
            ...
            for session in active_sessions:
                try:
                    ...
                    cleanup_results.append({'session_id': session.session_id, 'success': success, 'message': message})
                except Exception as e:
                    cleanup_results.append({'session_id': session.session_id, 'success': False, 'error': str(e)})
            return {'status': 'success', 'component': 'UnifiedProcessManager', 'system_check': check_result, 'cleaned_stale_pids': len(cleaned_pids), 'active_sessions_cleaned': len(cleanup_results), 'cleanup_results': cleanup_results}
        except Exception as e:
            self.logger.error('UnifiedProcessManager shutdown error: %s', e)
            return {'status': 'error', 'component': 'UnifiedProcessManager', 'error': str(e)}
    ...
    async def perform_emergency_crash_detection(self, signal_context):
        ...
        try:
            ...
            cleaned_pids = await self.cleanup_stale_pids()
            return {'status': 'emergency_operation_completed', 'detected_crashes': len(detected_crashes), 'recovery_results': len(recovery_results), 'successful_recoveries': len([r for r in recovery_results if r.recovery_status == 'success']), 'cleaned_stale_pids': len(cleaned_pids), 'timestamp': datetime.now(UTC).isoformat()}
        except Exception as e:
            self.logger.error('Emergency crash detection failed: %s', e)
    ...
    async def generate_process_status_report(self, signal_context):
        ...
        try:
            ...
            active_sessions = await self.list_active_sessions()
            crash_summary = {'detected_crashes': len(self.detected_crashes), 'recovery_history': len(self.recovery_history), 'successful_recoveries': len([r for r in self.recovery_history if r.recovery_status == 'success'])}
            return {'status': 'report_generated', 'system_status': system_status, 'active_sessions': len(active_sessions), 'session_details': [{'session_id': session.session_id, 'pid': session.pid, 'state': session.state.value, 'started_at': session.started_at.isoformat(), 'cpu_percent': session.cpu_percent, 'memory_mb': session.memory_mb} for session in active_sessions], 'crash_summary': crash_summary, 'timestamp': datetime.now(UTC).isoformat()}
        except Exception as e:
            self.logger.error('Process status report generation failed: %s', e)
            return {'status': 'error', 'error': str(e)}

    def prepare_process_cleanup(self, signum, signal_name):
        """Prepare process management for coordinated cleanup."""
        self.logger.info('Preparing process cleanup (%s)', signal_name)
        try:
            preparation_status = {'prepared': True, 'component': 'UnifiedProcessManager', 'active_sessions': len(self.active_sessions), 'pid_files': len(self.pid_files), 'detected_crashes': len(self.detected_crashes), 'cleanup_locks_ready': True}
            return preparation_status
        except Exception as e:
            self.logger.error('Process cleanup preparation failed: %s', e)
            return {'prepared': False, 'component': 'UnifiedProcessManager', 'error': str(e)}

    def prepare_process_interruption(self, signum, signal_name):
        """Prepare process management for user interruption (Ctrl+C)."""
        self.logger.info('Preparing process interruption handling (%s)', signal_name)
        try:
            interruption_preparation = {'prepared': True, 'component': 'UnifiedProcessManager', 'interruption_type': 'user_requested', 'active_sessions': list(self.active_sessions.keys()), 'stale_pid_cleanup_ready': True, 'crash_recovery_ready': True}
            return interruption_preparation
        except Exception as e:
            self.logger.error('Process interruption preparation failed: %s', e)
            return {'prepared': False, 'component': 'UnifiedProcessManager', 'error': str(e)}

    async def create_pid_file(self, session_id: str, process_info: dict[str, Any] | None=None) -> tuple[bool, str]:
        ...
        async with self.coordination_lock:
            try:
                ...
                if pid_file_path.exists():
                    ...
                    self.logger.info('Removing stale PID file: %s', reason)
                    await self.remove_pid_file(session_id)
                temp_file = pid_file_path.with_suffix('.tmp')
                pid_data = {'pid': pid, 'session_id': session_id, 'created_at': datetime.now(UTC).isoformat(), 'command': ' '.join(sys.argv), 'process_info': process_info or {}}
                ...
                process_info_obj = ProcessInfo(pid=pid, session_id=session_id, started_at=datetime.now(UTC), command=pid_data['command'], metadata=process_info or {})
                self.active_sessions[session_id] = process_info_obj
                stat = pid_file_path.stat()
                pid_file_info = PIDFileInfo(session_id=session_id, pid=pid, file_path=pid_file_path, created_at=datetime.now(UTC), last_modified=datetime.fromtimestamp(stat.st_mtime, UTC), file_size=stat.st_size, owner_uid=stat.st_uid, owner_gid=stat.st_gid, is_locked=False)
                self.pid_files[session_id] = pid_file_info
                self.logger.info('Created PID file for session {session_id}: %s', pid)
    ...
    def _get_process_state(self, process: psutil.Process) -> ProcessState:
        ...
        try:
            ...
            status_map = {psutil.STATUS_RUNNING: ProcessState.RUNNING, psutil.STATUS_SLEEPING: ProcessState.SLEEPING, psutil.STATUS_DISK_SLEEP: ProcessState.DISK_SLEEP, psutil.STATUS_STOPPED: ProcessState.STOPPED, psutil.STATUS_ZOMBIE: ProcessState.ZOMBIE}
    ...
    async def _is_pid_file_stale(self, session_id: str) -> tuple[bool, str]:
        ...
        try:
            ...
            file_age = datetime.now(UTC) - datetime.fromtimestamp(pid_file_path.stat().st_mtime, UTC)
                ...
                for pid_file in self.pid_dir.glob('*.pid'):
                    ...
                    if is_stale:
                        ...
                        if success:
                            ...
                            self.logger.info('Cleaned stale PID: %s - %s (%s)', session_id, reason, message)
                        else:
                            self.logger.warning('Failed to clean stale PID %s: %s', session_id, message)
                return cleaned_sessions
            except Exception as e:
                self.logger.error('Failed to cleanup stale PIDs: %s', e)
                return cleaned_sessions
    ...
    async def detect_system_crashes(self) -> list[CrashContext]:
        ...
        async with self.detection_lock:
            ...
            try:
                ...
                self.logger.info('Detected %s crashes', len(detected_crashes))
                return detected_crashes
            except Exception as e:
                self.logger.error('Failed to detect crashes: %s', e)
                return []
        ...
        try:
            for pid_file in self.pid_dir.glob('*.pid'):
                ...
                if is_stale:
                    ...
                    crash_context = CrashContext(crash_id=crash_id, crash_type=CrashType.PROCESS_KILLED, detected_at=datetime.now(UTC), severity=CrashSeverity.MEDIUM, affected_sessions=[session_id], crash_indicators={'stale_reason': reason, 'pid_file': str(pid_file)}, system_state_at_crash={}, recovery_strategy='cleanup_and_restart', estimated_data_loss={}, recovery_confidence=0.8)
                    orphaned_crashes.append(crash_context)
            return orphaned_crashes
        except Exception as e:
            self.logger.error('Failed to detect orphaned processes: %s', e)
            return []
        ...
        try:
            for backup_file in self.backup_dir.glob('*.backup'):
                ...
                if session_id not in self.active_sessions:
                    ...
                    crash_context = CrashContext(crash_id=crash_id, crash_type=CrashType.UNKNOWN, detected_at=datetime.now(UTC), severity=CrashSeverity.LOW, affected_sessions=[session_id], crash_indicators={'backup_file': str(backup_file)}, system_state_at_crash={}, recovery_strategy='restore_from_backup', estimated_data_loss={}, recovery_confidence=0.9)
                    session_crashes.append(crash_context)
            return session_crashes
        except Exception as e:
            self.logger.error('Failed to detect session crashes: %s', e)
            return []
    ...
    async def _analyze_crash_context(self, crash: CrashContext) -> CrashContext:
        ...
        try:
            crash.system_state_at_crash = {'memory_usage': psutil.virtual_memory()._asdict(), 'disk_usage': psutil.disk_usage('/')._asdict(), 'cpu_count': psutil.cpu_count(), 'boot_time': psutil.boot_time()}
            ...
            self.logger.error('Failed to analyze crash context: %s', e)
        return crash
        ...
        async with self.recovery_lock:
            ...
            try:
                if crash_id not in self.detected_crashes:
                    return RecoveryResult(crash_id=crash_id, recovery_status='failed', recovered_sessions=[], failed_sessions=[], data_repaired=[], recovery_actions=[], recovery_duration_seconds=0, final_system_state={}, recommendations=['Crash not found in detected crashes'])
                ...
                if crash_context.recovery_strategy == 'cleanup_and_restart':
                    for session_id in crash_context.affected_sessions:
                        try:
                            ...
                            failed_sessions.append(session_id)
                            self.logger.error('Failed to recover session %s: %s', session_id, e)
                ...
                result = RecoveryResult(crash_id=crash_id, recovery_status=recovery_status, recovered_sessions=recovered_sessions, failed_sessions=failed_sessions, data_repaired=[], recovery_actions=recovery_actions, recovery_duration_seconds=duration, final_system_state=crash_context.system_state_at_crash, recommendations=[])
                self.recovery_history.append(result)
                self.logger.info('Recovery completed for crash %s: %s', crash_id, recovery_status)
                return result
            except Exception as e:
                error_msg = f'Failed to recover from crash {crash_id}: {e}'
                self.logger.error(error_msg)
                return RecoveryResult(crash_id=crash_id, recovery_status='failed', recovered_sessions=[], failed_sessions=crash_context.affected_sessions if crash_id in self.detected_crashes else [], data_repaired=[], recovery_actions=[], recovery_duration_seconds=time.time() - start_time, final_system_state={}, recommendations=[error_msg])
    ...
    async def get_system_status(self) -> dict[str, Any]:
        ...
        try:
            active_sessions = await self.list_active_sessions()
            return {'unified_manager': True, 'active_sessions_count': len(active_sessions), 'active_sessions': [{'session_id': session.session_id, 'pid': session.pid, 'state': session.state.value, 'cpu_percent': session.cpu_percent, 'memory_mb': session.memory_mb, 'started_at': session.started_at.isoformat()} for session in active_sessions], 'detected_crashes_count': len(self.detected_crashes), 'recovery_history_count': len(self.recovery_history), 'pid_files_count': len(self.pid_files), 'directories': {'pid_dir': str(self.pid_dir), 'backup_dir': str(self.backup_dir)}, 'status_timestamp': datetime.now(UTC).isoformat()}
        except Exception as e:
    ...
    async def full_system_check_and_recovery(self) -> dict[str, Any]:
        ...
        try:
            ...
            for crash in detected_crashes:
                ...
                recovery_results.append(recovery_result)
            system_status = await self.get_system_status()
            return {'check_completed': True, 'cleaned_stale_pids': cleaned_pids, 'detected_crashes': len(detected_crashes), 'recovery_results': len(recovery_results), 'successful_recoveries': len([r for r in recovery_results if r.recovery_status == 'success']), 'partial_recoveries': len([r for r in recovery_results if r.recovery_status == 'partial']), 'failed_recoveries': len([r for r in recovery_results if r.recovery_status == 'failed']), 'final_system_status': system_status, 'timestamp': datetime.now(UTC).isoformat()}
        except Exception as e:
            self.logger.error('Failed system check and recovery: %s', e)
