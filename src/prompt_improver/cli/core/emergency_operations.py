...
class EmergencyOperationsManager:
    ...
    def __init__(self, backup_dir: Path | None=None):
        ...
        self.operation_history: dict[str, list] = {'checkpoints': [], 'status_reports': [], 'config_reloads': []}
    ...
    async def create_emergency_checkpoint(self, context: 'SignalContext') -> dict[str, Any]:
        ...
        try:
            ...
            checkpoint_data = {'checkpoint_id': checkpoint_id, 'created_at': context.triggered_at.isoformat(), 'trigger': {'signal': context.signal_name, 'operation': context.operation.value, 'signal_number': context.signal_number}, 'system_state': system_state, 'training_state': training_state, 'emergency': True}
            ...
            try:
                ...
                self.logger.warning('Failed to save checkpoint to database: %s', e)
            self.operation_history['checkpoints'].append({'checkpoint_id': checkpoint_id, 'created_at': context.triggered_at.isoformat(), 'path': str(checkpoint_path)})
            result = {'status': 'success', 'checkpoint_id': checkpoint_id, 'path': str(checkpoint_path), 'size_bytes': checkpoint_path.stat().st_size, 'system_state': system_state, 'training_sessions': len(training_state.get('sessions', []))}
            self.logger.info('Emergency checkpoint created: %s', checkpoint_id)
            return result
        except Exception as e:
            self.logger.error('Failed to create emergency checkpoint: %s', e)
    ...
    async def generate_status_report(self, context: 'SignalContext') -> dict[str, Any]:
        ...
        try:
            ...
            status_report = {'report_id': report_id, 'generated_at': context.triggered_at.isoformat(), 'trigger': {'signal': context.signal_name, 'operation': context.operation.value, 'signal_number': context.signal_number}, 'system': system_status, 'training': training_status, 'resources': resource_status, 'emergency_operations': {'checkpoints_created': len(self.operation_history['checkpoints']), 'status_reports_generated': len(self.operation_history['status_reports']), 'config_reloads_performed': len(self.operation_history['config_reloads'])}}
            report_path = self.backup_dir / f'{report_id}.json'
            report_path.write_text(json.dumps(status_report, indent=2))
            self.operation_history['status_reports'].append({'report_id': report_id, 'generated_at': context.triggered_at.isoformat(), 'path': str(report_path)})
            self.logger.info('Status report generated: %s', report_id)
            return status_report
        except Exception as e:
            self.logger.error('Failed to generate status report: %s', e)
    ...
    async def reload_configuration(self, context: 'SignalContext') -> dict[str, Any]:
        ...
        try:
            ...
            reload_report = {'reload_id': reload_id, 'reloaded_at': context.triggered_at.isoformat(), 'trigger': {'signal': context.signal_name, 'operation': context.operation.value, 'signal_number': context.signal_number}, 'old_config': old_config, 'new_config': new_config, 'changes': reload_results.get('changes', []), 'status': reload_results.get('status', 'success')}
            report_path = self.backup_dir / f'{reload_id}.json'
            report_path.write_text(json.dumps(reload_report, indent=2))
            self.operation_history['config_reloads'].append({'reload_id': reload_id, 'reloaded_at': context.triggered_at.isoformat(), 'path': str(report_path)})
            self.logger.info('Configuration reloaded: %s', reload_id)
    ...
    async def _gather_system_state(self) -> dict[str, Any]:
        ...
        try:
            return {'pid': os.getpid(), 'timestamp': datetime.now(UTC).isoformat(), 'memory_usage_mb': psutil.Process().memory_info().rss / 1024 / 1024, 'cpu_percent': psutil.cpu_percent(), 'disk_usage': {'total_gb': psutil.disk_usage('/').total / 1024 ** 3, 'used_gb': psutil.disk_usage('/').used / 1024 ** 3, 'free_gb': psutil.disk_usage('/').free / 1024 ** 3}}
        except Exception as e:
            self.logger.warning('Failed to gather system state: %s', e)
            return {'error': str(e)}
    ...
    async def _gather_training_state(self) -> dict[str, Any]:
        ...
        try:
            async with get_session_context() as db_session:
                ...
                result = await db_session.execute(select(TrainingSession).where(text("status = 'running'")))
                active_sessions = result.scalars().all()
                sessions_data = []
                for session in active_sessions:
                    sessions_data.append({'session_id': session.session_id, 'started_at': session.started_at.isoformat() if session.started_at else None, 'max_iterations': session.max_iterations, 'current_iteration': session.current_iteration, 'status': session.status})
                return {'active_sessions': len(sessions_data), 'sessions': sessions_data}
        except Exception as e:
            self.logger.warning('Failed to gather training state: %s', e)
            return {'error': str(e)}
    ...
    async def _gather_resource_status(self) -> dict[str, Any]:
        ...
        try:
            status = {'memory': {'total_mb': psutil.virtual_memory().total / 1024 / 1024, 'available_mb': psutil.virtual_memory().available / 1024 / 1024, 'percent': psutil.virtual_memory().percent}, 'cpu': {'percent': psutil.cpu_percent(interval=1), 'count': psutil.cpu_count()}, 'connections': {'database': 'unknown', 'coredis': 'unknown'}}
            try:
            ...
            self.logger.warning('Failed to gather resource status: %s', e)
            return {'error': str(e)}

    async def _get_current_config(self) -> dict[str, Any]:
        """Get current configuration state."""
        return {'timestamp': datetime.now(UTC).isoformat(), 'config_files': [], 'environment': dict(os.environ)}
