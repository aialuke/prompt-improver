...
class EnhancedWorkflowManager:
    ...
    def __init__(self, cli_orchestrator: CLIOrchestrator, console: Console | None=None):
    ...
    async def wait_for_workflow_completion(self, workflow_id: str, timeout: int=3600, config: WorkflowMonitorConfig | None=None, show_progress: bool=True) -> WorkflowCompletionResult:
        ...
        self.logger.info('Starting workflow completion monitoring for %s (timeout: %ss)', workflow_id, timeout)
        try:
            if show_progress:
            ...
            self.logger.info('Workflow monitoring cancelled for %s', workflow_id)
            return WorkflowCompletionResult(workflow_id=workflow_id, status='cancelled', completed=False, duration_seconds=(datetime.now(UTC) - start_time).total_seconds(), timeout_reached=False, error='Monitoring cancelled')
        finally:
    ...
    async def _wait_with_progress_display(self, workflow_id: str, timeout: int, config: WorkflowMonitorConfig) -> WorkflowCompletionResult:
        ...
        with Progress(SpinnerColumn(), TextColumn('[progress.description]{task.description}'), BarColumn(), TaskProgressColumn(), console=self.console) as progress:
            progress_task = progress.add_task(f'Monitoring workflow {workflow_id}', total=timeout)
            start_time = datetime.now(UTC)
            ...
            while True:
                ...
                if elapsed >= timeout:
                    ...
                    return WorkflowCompletionResult(workflow_id=workflow_id, status='timeout', completed=False, duration_seconds=elapsed, timeout_reached=True, error=f'Workflow did not complete within {timeout}s')
                ...
                try:
                    ...
                    if status_result.get('completed', False):
                        progress.update(progress_task, completed=timeout)
                        return WorkflowCompletionResult(workflow_id=workflow_id, status='completed', completed=True, duration_seconds=elapsed, timeout_reached=False, final_state=status_result.get('state'), progress_data=status_result)
                    current_status = status_result.get('state', 'unknown')
                    progress.update(progress_task, description=f'Monitoring workflow {workflow_id} - {current_status}')
                except Exception as e:
                    self.logger.warning('Error checking workflow status: %s', e)
                if config.exponential_backoff:
                    current_poll_interval = min(current_poll_interval * config.backoff_multiplier, config.max_poll_interval)
                await asyncio.sleep(current_poll_interval)

    async def _wait_with_polling(self, workflow_id: str, timeout: int, config: WorkflowMonitorConfig) -> WorkflowCompletionResult:
        ...
        while True:
            ...
            if elapsed - last_status_log >= config.progress_update_interval:
                self.logger.info('Workflow %s still running (%ss elapsed)', workflow_id, format(elapsed, '.1f'))
