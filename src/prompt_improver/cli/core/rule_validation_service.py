...
class RuleValidationService:
    ...
    def __init__(self):
        ...
        self.expected_rules = {'clarity_enhancement': {'category': 'fundamental', 'min_priority': 8, 'required_parameters': ['min_clarity_score', 'use_structured_xml']}, 'chain_of_thought': {'category': 'reasoning', 'min_priority': 7, 'required_parameters': ['enable_step_by_step', 'use_thinking_tags']}, 'few_shot_examples': {'category': 'examples', 'min_priority': 6, 'required_parameters': ['optimal_example_count', 'require_diverse_examples']}, 'context_optimization': {'category': 'context', 'min_priority': 5, 'required_parameters': ['context_placement', 'relevance_threshold']}, 'output_formatting': {'category': 'formatting', 'min_priority': 4, 'required_parameters': ['structured_output', 'format_consistency']}, 'specificity_enhancement': {'category': 'fundamental', 'min_priority': 3, 'required_parameters': ['vague_language_threshold', 'require_specific_outcomes']}}
    ...
    async def validate_all_rules(self) -> dict[str, Any]:
        ...
        validation_report = {'validation_timestamp': validation_start.isoformat(), 'overall_status': 'unknown', 'rule_count': {'expected': len(self.expected_rules), 'found': 0, 'valid': 0}, 'seeded_rules': {}, 'metadata_validation': {}, 'performance_analysis': {}, 'recommendations': [], 'validation_time_ms': 0}
        try:
            ...
            async with session_manager.get_async_session() as db_session:
                ...
                recommendations = await self._generate_rule_recommendations(rule_existence, metadata_validation, parameter_validation, performance_analysis)
            ...
            self.logger.error('Rule validation failed: %s', e)
    ...
    async def _validate_rule_existence(self, db_session: AsyncSession) -> dict[str, Any]:
        ...
        for rule in result.scalars().all():
            db_rules[rule.rule_id] = {'rule_name': rule.rule_name, 'enabled': rule.enabled, 'priority': rule.priority, 'category': rule.category}
        rule_validation = {}
        for rule_id, expected in self.expected_rules.items():
            if rule_id in db_rules:
                ...
                rule_validation[rule_id] = {'status': 'valid' if not issues else 'issues', 'found': True, 'issues': issues, 'details': db_rule}
            else:
                rule_validation[rule_id] = {'status': 'missing', 'found': False, 'issues': ['Rule not found in database'], 'details': {}}
        return {'total_rules': len(db_rules), 'expected_rules': len(self.expected_rules), 'rules': rule_validation}
    ...
    async def _validate_rule_metadata(self, db_session: AsyncSession) -> dict[str, Any]:
        ...
        for rule in result.scalars().all():
            ...
            rule_metadata[rule_id] = {'issues': rule_issues, 'status': 'valid' if not rule_issues else 'issues', 'version': version, 'created_at': created_at.isoformat() if created_at else None, 'updated_at': updated_at.isoformat() if updated_at else None}
            metadata_issues.extend(rule_issues)
        return {'overall_status': 'healthy' if not metadata_issues else 'issues_found', 'total_issues': len(metadata_issues), 'rule_metadata': rule_metadata, 'issues': metadata_issues}
    ...
    async def _validate_rule_parameters(self, db_session: AsyncSession) -> dict[str, Any]:
        ...
        for rule in result.scalars().all():
            ...
            default_params = rule.default_parameters
            constraints = rule.parameter_constraints
            validation_result = {'status': 'valid', 'issues': [], 'parameter_count': 0, 'constraint_violations': []}
            try:
                if default_params:
                    if isinstance(default_params, str):
                        params = json.loads(default_params)
                            ...
                            if param_name in constraint_dict:
                                ...
                                if 'type' in constraint:
                                    ...
                                    if expected_type == 'boolean' and (not isinstance(param_value, bool)):
                                        validation_result['constraint_violations'].append(f'{param_name}: expected boolean, got {type(param_value).__name__}')
                                    elif expected_type == 'number' and (not isinstance(param_value, (int, float))):
                                        validation_result['constraint_violations'].append(f'{param_name}: expected number, got {type(param_value).__name__}')
                                if 'min' in constraint and isinstance(param_value, (int, float)):
                                    if param_value < constraint['min']:
                                        validation_result['constraint_violations'].append(f"{param_name}: {param_value} < minimum {constraint['min']}")
                                if 'max' in constraint and isinstance(param_value, (int, float)):
                                    if param_value > constraint['max']:
                                        validation_result['constraint_violations'].append(f"{param_name}: {param_value} > maximum {constraint['max']}")
    ...
    async def _analyze_rule_performance(self, db_session: AsyncSession) -> dict[str, Any]:
        ...
        performance_analysis = {'rules_with_performance': 0, 'total_performance_records': 0, 'average_improvement_score': 0.0, 'rule_performance': {}}
        try:
            ...
            for row in result.fetchall():
                ...
                performance_analysis['rule_performance'][rule_id] = {'performance_records': count, 'average_improvement': float(avg_improvement) if avg_improvement else 0.0, 'max_improvement': float(max_improvement) if max_improvement else 0.0, 'min_improvement': float(min_improvement) if min_improvement else 0.0, 'status': 'good' if avg_improvement and avg_improvement > 0.5 else 'needs_improvement'}
                total_records += count
            ...
            performance_analysis['average_improvement_score'] = total_improvement / len(performance_analysis['rule_performance']) if performance_analysis['rule_performance'] else 0.0
        except Exception as e:
            self.logger.error('Performance analysis failed: %s', e)
            performance_analysis['error'] = str(e)
        return performance_analysis

    async def _generate_rule_recommendations(self, existence: dict, metadata: dict, parameters: dict, performance: dict) -> list[str]:
        ...
        disabled_rules = [rule_id for rule_id, data in existence['rules'].items() if data['found'] and 'Rule is disabled' in data.get('issues', [])]
        if disabled_rules:
        ...
        violation_rules = [rule_id for rule_id, data in parameters.items() if data['status'] == 'constraint_violations']
        if violation_rules:
        ...
        if performance.get('rules_with_performance', 0) == 0:
            ...
            poor_performers = [rule_id for rule_id, data in performance['rule_performance'].items() if data['status'] == 'needs_improvement']
            if poor_performers:
