...
class SystemStateReporter:
    ...
    def _display_overall_status(self, results: dict[str, Any]) -> None:
        ...
        status_content = [f'Status: {status_text}', f"Initialization Time: {results.get('initialization_time_seconds', 0):.2f}s", f"Components Initialized: {len(results.get('components_initialized', []))}", f"Timestamp: {results.get('timestamp', 'Unknown')}"]
        panel = Panel('\n'.join(status_content), title='🎯 Overall System Status', border_style=status_color, expand=False)
    ...
    def _display_database_status(self, results: dict[str, Any]) -> None:
        ...
        db_table.add_row('Database Connection', f'[{conn_color}]{conn_status.title()}[/{conn_color}]', 'Connection successful' if conn_status == 'healthy' else 'Connection failed')
        ...
        db_table.add_row('Database Schema', f'[{schema_color}]{schema_status.title()}[/{schema_color}]', schema_detail_text)
        ...
        db_table.add_row('Seeded Rules', f'[{rules_color}]{rules_status.title()}[/{rules_color}]', rules_text)
    ...
    def _display_data_status(self, results: dict[str, Any]) -> None:
        ...
        data_table.add_row('Total Training Prompts', str(total_prompts), '[green]✅[/green]' if total_prompts >= 100 else '[yellow]⚠️[/yellow]')
        data_table.add_row('Synthetic Data', str(synthetic_prompts), '[green]✅[/green]' if synthetic_prompts > 0 else '[red]❌[/red]')
        data_table.add_row('User Data', str(user_prompts), '[green]✅[/green]' if user_prompts > 0 else '[yellow]⚠️[/yellow]')
        quality_details = data_quality.get('details', {})
        overall_quality = quality_details.get('overall_quality_score', 0)
        data_table.add_row('Data Quality Score', f'{overall_quality:.3f}', '[green]✅[/green]' if overall_quality > 0.7 else '[yellow]⚠️[/yellow]' if overall_quality > 0.5 else '[red]❌[/red]')
        requirements_details = requirements.get('details', {})
        requirements_status = requirements.get('status', 'unknown')
        data_table.add_row('Training Data Requirement', '≥100 prompts', '[green]✅[/green]' if requirements_details.get('training_data_count', False) else '[red]❌[/red]')
        data_table.add_row('Synthetic Data Available', 'Required', '[green]✅[/green]' if requirements_details.get('synthetic_data_available', False) else '[red]❌[/red]')
        data_table.add_row('Overall Requirements', requirements_status.replace('_', ' ').title(), '[green]✅[/green]' if requirements_status == 'met' else '[red]❌[/red]')
    ...
    def _display_initialization_details(self, results: dict[str, Any]) -> None:
        ...
        if execution:
            ...
            init_panel_content.extend([f'Actions Completed: {completed}', f'Actions Failed: {failed}', f'Actual Time: {actual_time:.2f}s'])
        if init_panel_content:
            panel = Panel('\n'.join(init_panel_content), title='⚙️ Initialization Execution', border_style='blue', expand=False)
            self.console.print(panel)
    ...
    def _display_performance_metrics(self, results: dict[str, Any]) -> None:
        ...
        if final_validation:
            ...
            metrics_content.extend([f"Overall Health: {('✅ Healthy' if overall_health else '❌ Issues')}", f"Training Ready: {('✅ Ready' if training_readiness else '❌ Not Ready')}", f'Validation Time: {validation_time:.1f}ms'])
            panel = Panel('\n'.join(metrics_content), title='📈 Performance Metrics', border_style='cyan', expand=False)
            self.console.print(panel)

    def export_state_report(self, results: dict[str, Any], filepath: str | None=None) -> str:
        ...
        import json
        export_data = {'report_timestamp': datetime.now(UTC).isoformat(), 'system_state': results, 'summary': {'overall_status': 'healthy' if results.get('success') else 'issues', 'components_initialized': len(results.get('components_initialized', [])), 'initialization_time': results.get('initialization_time_seconds', 0), 'recommendations_count': len(results.get('recommendations', []))}}
