...
class SystemStateReporter:
    ...
    def _display_component_status(self, results: dict[str, Any]) -> None:
        ...
        for component, status_info in component_status.items():
            ...
            if isinstance(details, dict):
                detail_text = ", ".join(
                    (f"{k}: {v}" for k, v in details.items() if k != "error")
                )
                if "error" in details:
    ...
    def _display_data_status(self, results: dict[str, Any]) -> None:
        ...
        data_table = Table(
            title="📊 Training Data Status", show_header=True, header_style="bold green"
        )
        ...
        data_table.add_row(
            ...
            "≥100 prompts",
            "[green]✅[/green]"
            if requirements_details.get("training_data_count", False)
            else "[red]❌[/red]",
        )
        data_table.add_row(
            ...
            "Required",
            "[green]✅[/green]"
            if requirements_details.get("synthetic_data_available", False)
            else "[red]❌[/red]",
        )
        data_table.add_row(
    ...
    def _display_recommendations(self, results: dict[str, Any]) -> None:
        ...
        if recommendations:
            ...
            self.console.print(
                "\n✅ No recommendations - system is optimally configured",
                style="green",
            )
    ...
    def export_state_report(
        self, results: dict[str, Any], filepath: str | None = None
    ) -> str:
        ...
        export_data = {
            ...
            "summary": {
                ...
                "components_initialized": len(
                    results.get("components_initialized", [])
                ),
