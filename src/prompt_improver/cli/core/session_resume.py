...
class SessionResumeManager:
            ...
            try:
                async with get_session_context() as db_session:
                    ...
                    for session in all_sessions:
                        if session.status in ['running', 'interrupted'] or (session.status == 'running' and session.last_activity_at < cutoff_time):
                ...
                for session in sessions:
                    ...
                    if context.detected_state in [SessionState.INTERRUPTED, SessionState.RECOVERABLE]:
                        ...
                        self.logger.info('Detected interrupted session: %s (state: %s, confidence: %s)', session.session_id, context.detected_state.value, format(context.recovery_confidence, '.2f'))
                self.logger.info('Found %s interrupted sessions', len(interrupted_sessions))
                return interrupted_sessions
            except Exception as e:
                self.logger.error('Failed to detect interrupted sessions: %s', e)
                return []
    ...
    async def _analyze_session_state(self, session: TrainingSession) -> SessionResumeContext:
        ...
        try:
            ...
            recovery_confidence = self._calculate_recovery_confidence(session, detected_state, integrity_score, time_since_activity)
            ...
            return SessionResumeContext(session_id=session.session_id, detected_state=detected_state, last_activity=last_activity, interruption_reason=self._infer_interruption_reason(session, time_since_activity), recovery_strategy=recovery_strategy, data_integrity_score=integrity_score, resume_from_iteration=resume_from_iteration, estimated_loss_minutes=estimated_loss, recovery_confidence=recovery_confidence)
        except Exception as e:
            self.logger.error('Failed to analyze session {session.session_id}: %s', e)
            return SessionResumeContext(session_id=session.session_id, detected_state=SessionState.UNRECOVERABLE, last_activity=datetime.now(UTC), interruption_reason=f'Analysis failed: {e}', recovery_strategy='none', data_integrity_score=0.0, resume_from_iteration=0, estimated_loss_minutes=0.0, recovery_confidence=0.0)
    ...
    async def _assess_data_integrity(self, session: TrainingSession) -> float:
        ...
        try:
            ...
            self.logger.warning('Failed to assess data integrity for %s: %s', session.session_id, e)
            return 0.5
    ...
    def _calculate_recovery_confidence(self, session: TrainingSession, state: SessionState, integrity: float, time_since_activity: timedelta) -> float:
        """Calculate confidence score for successful recovery."""
        state_confidence = {SessionState.RECOVERABLE: 0.9, SessionState.INTERRUPTED: 0.7, SessionState.RUNNING: 0.95, SessionState.CORRUPTED: 0.3, SessionState.FAILED: 0.2, SessionState.UNRECOVERABLE: 0.0}.get(state, 0.1)
        ...
        confidence = state_confidence * 0.3 + integrity_confidence * 0.3 + time_confidence * 0.2 + checkpoint_confidence * 0.2
    ...
    async def _find_safe_resume_point(self, session: TrainingSession) -> int:
        ...
        try:
            if not session.iterations:
                return 0
            valid_iterations = [iteration for iteration in session.iterations if iteration.completed_at is not None and iteration.error_message is None]
            if valid_iterations:
                valid_iterations.sort(key=lambda x: x.iteration)
                return valid_iterations[-1].iteration
            return 0
        except Exception as e:
            self.logger.warning('Failed to find safe resume point for %s: %s', session.session_id, e)
            return 0
    ...
    def _estimate_data_loss(self, session: TrainingSession, resume_point: int, time_since_activity: timedelta) -> float:
        ...
        try:
            ...
            self.logger.warning('Failed to estimate data loss for %s: %s', session.session_id, e)
            return 0.0

    def _infer_interruption_reason(self, session: TrainingSession, time_since_activity: timedelta) -> str | None:
    ...
    async def reconstruct_workflow_state(self, session_id: str) -> WorkflowState | None:
        ...
        """
        try:
            self.logger.info('Reconstructing workflow state for session: %s', session_id)
            async with get_session_context() as db_session:
                result = await db_session.execute(select(TrainingSession).where(text(f"session_id = '{session_id}'")))
                session = result.scalar_one_or_none()
                if session:
                    from prompt_improver.database.models import TrainingIteration
                    iterations_result = await db_session.execute(select(TrainingIteration).where(text(f"session_id = '{session_id}'")))
                ...
                workflow_parameters = {'session_id': session_id, 'continuous_mode': session.continuous_mode, 'max_iterations': session.max_iterations, 'improvement_threshold': session.improvement_threshold, 'timeout_seconds': session.timeout_seconds, 'current_iteration': session.current_iteration, 'resume_mode': True}
                execution_context = {'started_at': session.started_at.isoformat(), 'last_activity_at': session.last_activity_at.isoformat() if session.last_activity_at else None, 'total_iterations_completed': len(session.iterations), 'checkpoint_data': checkpoint_data, 'session_status': session.status}
                performance_metrics = {}
                if session.iterations:
                    performance_values = [iter.performance_metrics.get('improvement', 0.0) for iter in session.iterations if iter.performance_metrics]
                    if performance_values:
                        performance_metrics = {'average_improvement': sum(performance_values) / len(performance_values), 'best_improvement': max(performance_values), 'total_iterations': len(performance_values), 'performance_trend': performance_values[-5:] if len(performance_values) >= 5 else performance_values}
                workflow_state = WorkflowState(workflow_id=workflow_id, workflow_type=workflow_type, current_step=current_step, completed_steps=completed_steps, pending_steps=pending_steps, workflow_parameters=workflow_parameters, execution_context=execution_context, performance_metrics=performance_metrics)
                self.logger.info('Workflow state reconstructed for %s: %s', session_id, current_step)
                return workflow_state
        except Exception as e:
            self.logger.error('Failed to reconstruct workflow state for %s: %s', session_id, e)
            return None

    async def resume_training_session(self, session_id: str, training_manager: TrainingSystemManager, force_resume: bool=False) -> dict[str, Any]:
        ...
        async with self.resume_lock:
            self.logger.info('Attempting to resume training session: %s', session_id)
            try:
                if session_id not in self.detected_sessions:
                    await self.detect_interrupted_sessions()
                if session_id not in self.detected_sessions:
                    return {'status': 'error', 'error': f'Session {session_id} not found or not resumable', 'session_id': session_id}
                context = self.detected_sessions[session_id]
                if not force_resume and context.recovery_confidence < 0.6:
                    return {'status': 'error', 'error': f'Resume confidence too low: {context.recovery_confidence:.2f}', 'session_id': session_id, 'context': asdict(context)}
                workflow_state = await self.reconstruct_workflow_state(session_id)
                if not workflow_state:
                    return {'status': 'error', 'error': 'Failed to reconstruct workflow state', 'session_id': session_id}
                integrity_check = await self._verify_resume_integrity(session_id, workflow_state)
                if not integrity_check['valid']:
                    return {'status': 'error', 'error': f"Data integrity check failed: {integrity_check['reason']}", 'session_id': session_id, 'integrity_check': integrity_check}
                async with get_session_context() as db_session:
                    await db_session.execute(update(TrainingSession).where(text(f"session_id = '{session_id}'")).values(current_iteration=context.resume_from_iteration, last_activity_at=datetime.now(UTC), status='resuming'))
                    await db_session.commit()
                resume_result = await self._perform_resume_operation(session_id, context, workflow_state, training_manager)
                self.resume_history.append({'session_id': session_id, 'resumed_at': datetime.now(UTC).isoformat(), 'context': asdict(context), 'workflow_state': asdict(workflow_state), 'result': resume_result})
                self.logger.info('Resume operation completed for %s: %s', session_id, resume_result['status'])
                return resume_result
            except Exception as e:
                error_result = {'status': 'error', 'error': str(e), 'session_id': session_id, 'timestamp': datetime.now(UTC).isoformat()}
                self.resume_history.append(error_result)
                self.logger.error('Resume operation failed for {session_id}: %s', e)
                return error_result
    ...
    async def _verify_resume_integrity(self, session_id: str, workflow_state: WorkflowState) -> dict[str, Any]:
        ...
        try:
            ...
            return {'valid': len(integrity_issues) == 0, 'issues': integrity_issues, 'reason': '; '.join(integrity_issues) if integrity_issues else None}
        except Exception as e:
            return {'valid': False, 'issues': [f'Integrity check failed: {e}'], 'reason': str(e)}

    async def _perform_resume_operation(self, session_id: str, context: SessionResumeContext, workflow_state: WorkflowState, training_manager: TrainingSystemManager) -> dict[str, Any]:
        """Perform the actual resume operation."""
        try:
            async with get_session_context() as db_session:
                await db_session.execute(update(TrainingSession).where(text(f"session_id = '{session_id}'")).values(status='running', last_activity_at=datetime.now(UTC), checkpoint_data={**workflow_state.execution_context.get('checkpoint_data', {}), 'resumed_at': datetime.now(UTC).isoformat(), 'resume_context': asdict(context)}))
                await db_session.commit()
            training_manager._training_session_id = session_id
            resume_config = {**workflow_state.workflow_parameters, 'resume_from_iteration': context.resume_from_iteration, 'workflow_state': asdict(workflow_state)}
            return {'status': 'success', 'session_id': session_id, 'resumed_from_iteration': context.resume_from_iteration, 'recovery_confidence': context.recovery_confidence, 'estimated_loss_minutes': context.estimated_loss_minutes, 'workflow_state': asdict(workflow_state), 'resume_config': resume_config, 'timestamp': datetime.now(UTC).isoformat()}
        except Exception as e:
            return {'status': 'error', 'error': str(e), 'session_id': session_id, 'timestamp': datetime.now(UTC).isoformat()}
