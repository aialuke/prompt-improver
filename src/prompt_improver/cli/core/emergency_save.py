...
class EmergencySaveManager:
    ...
    async def perform_emergency_save(self, context: EmergencySaveContext, components: list[str] | None=None) -> EmergencySaveResult:
        ...
        self.logger.info('Starting emergency save: %s', context.save_id)
        if components is None:
            components = ['training_sessions', 'database_state', 'system_state', 'configuration', 'progress_snapshots']
        async with self.save_lock:
            try:
                ...
                for component in components:
                    try:
                        component_result = await self._save_component_atomic(component, temp_save_dir, context)
                        saved_components.append(component)
                        total_size += component_result.get('size_bytes', 0)
                    except Exception as e:
                        self.logger.error('Failed to save component {component}: %s', e)
                ...
                validation_results = {}
                if context.validate:
                    validation_results = await self._validate_emergency_save(temp_save_dir, saved_components, context)
                ...
                result = EmergencySaveResult(save_id=context.save_id, status='success' if not failed_components else 'partial', saved_components=saved_components, failed_components=failed_components, total_size_bytes=total_size, duration_seconds=duration, validation_results=validation_results)
                self.save_history.append(result)
                self.logger.info('Emergency save completed: %s (%ss)', context.save_id, format(duration, '.2f'))
                return result
            except Exception as e:
                duration = time.time() - start_time
                error_result = EmergencySaveResult(save_id=context.save_id, status='error', saved_components=[], failed_components=components, total_size_bytes=0, duration_seconds=duration, validation_results={}, error_message=str(e))
                self.save_history.append(error_result)
                self.logger.error('Emergency save failed: {context.save_id}: %s', e)
                return error_result
            finally:
                self.active_saves.pop(context.save_id, None)

    async def _save_component_atomic(self, component: str, temp_dir: Path, context: EmergencySaveContext) -> dict[str, Any]:
        ...
        component_data = {'component': component, 'saved_at': context.triggered_at.isoformat(), 'save_id': context.save_id, 'trigger': context.trigger_type, 'data': data}
        ...
        self.logger.debug('Saved component {component}: %s bytes', size_bytes)
        return {'size_bytes': size_bytes, 'path': str(component_file)}

    async def _validate_emergency_save(self, save_dir: Path, components: list[str], context: EmergencySaveContext) -> dict[str, Any]:
        """Validate emergency save data."""
        async with self.validation_lock:
            validation_results = {'validation_time': datetime.now(UTC).isoformat(), 'save_id': context.save_id, 'trigger_type': context.trigger_type, 'components_validated': [], 'validation_errors': [], 'integrity_checks': {}}
            for component in components:
                component_file = save_dir / f'{component}.json'
                try:
                    if not component_file.exists():
                        validation_results['validation_errors'].append(f'Component file missing: {component}')
                        continue
                    data = json.loads(component_file.read_text())
                    required_fields = ['component', 'saved_at', 'save_id', 'data']
                    for field in required_fields:
                        if field not in data:
                            validation_results['validation_errors'].append(f"Missing required field '{field}' in {component}")
                    if component == 'training_sessions':
                    ...
                    validation_results['validation_errors'].append(f'Validation error for {component}: {e!s}')
            validation_results['integrity_checks']['total_components'] = len(components)
            validation_results['integrity_checks']['validated_components'] = len(validation_results['components_validated'])
            validation_results['integrity_checks']['error_count'] = len(validation_results['validation_errors'])
            validation_results['integrity_checks']['success_rate'] = len(validation_results['components_validated']) / len(components) if components else 0
    ...
    async def _rollback_save(self, temp_dir: Path, save_id: str) -> None:
        ...
        try:
            ...
            self.logger.info('Rolled back failed save: %s', save_id)
        except Exception as e:
            self.logger.error('Failed to rollback save {save_id}: %s', e)
        ...
        try:
            async with get_session_context() as db_session:
                ...
                for session in sessions:
                    session_dict = {'session_id': session.session_id, 'status': session.status, 'started_at': session.started_at.isoformat() if session.started_at else None, 'completed_at': session.completed_at.isoformat() if session.completed_at else None, 'max_iterations': session.max_iterations, 'current_iteration': session.current_iteration, 'continuous_mode': session.continuous_mode, 'improvement_threshold': session.improvement_threshold}
                    sessions_data.append(session_dict)
                return {'total_sessions': len(sessions_data), 'sessions': sessions_data, 'gathered_at': datetime.now(UTC).isoformat()}
        except Exception as e:
            self.logger.warning('Failed to gather training sessions: %s', e)
            return {'error': str(e), 'sessions': []}

    async def _gather_database_state(self) -> dict[str, Any]:
        """Gather database state for emergency save."""
        return {'connection_status': 'unknown', 'gathered_at': datetime.now(UTC).isoformat()}

    async def _gather_system_state(self) -> dict[str, Any]:
        """Gather system state for emergency save."""
        try:
            import psutil
            return {'pid': os.getpid(), 'memory_mb': psutil.Process().memory_info().rss / 1024 / 1024, 'cpu_percent': psutil.cpu_percent(), 'gathered_at': datetime.now(UTC).isoformat()}
        except Exception as e:
            return {'error': str(e)}

    async def _gather_configuration(self) -> dict[str, Any]:
        """Gather configuration for emergency save."""
        return {'environment': dict(os.environ), 'gathered_at': datetime.now(UTC).isoformat()}
    ...
    async def _gather_progress_snapshots(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'active_snapshots': len(snapshots), 'snapshots': snapshots, 'gathered_at': datetime.now(UTC).isoformat()}
        except Exception as e:
