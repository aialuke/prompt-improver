...
from sklearn.decomposition import PCA, KernelPCA, FastICA, TruncatedSVD, SparsePCA
...
class DimensionalityConfig:
    ...
    # Evaluation criteria
    preservation_metrics: List[str] = None  # ['variance', 'structure', 'clustering', 'classification']
    ...
    feature_selection_method: str = 'mutual_info'  # 'f_score', 'mutual_info', 'rfe', 'variance'
...
class AdvancedDimensionalityReducer:
    ...
    def __init__(self, config: Optional[DimensionalityConfig] = None):
        ...
        self.logger.info(f"Advanced dimensionality reducer initialized with {len(self.available_methods)} methods")
        ...
        methods = {
            ...
            'kernel_pca': {
                ...
                'params': {
                    ...
                    'random_state': self.config.random_state,
                    'n_jobs': self.config.n_jobs if not self.config.memory_efficient else 1
                }
            },
            'ica': {
                'class': FastICA,
            ...
            'lda': {
                ...
                'preserves_variance': False,
                'params': {
                    'n_components': min(self.config.target_dimensions, 2)  # LDA limited by n_classes-1
                }
            }
        }
        ...
        if self.config.enable_manifold_learning and not self.config.fast_mode:
            methods.update({
                'tsne': {
                    ...
                    'preserves_variance': False,
                    'params': {
                        'n_components': min(self.config.target_dimensions, 3),  # t-SNE typically 2-3D
    ...
    async def reduce_dimensions(
        ...
        self.logger.info(f"Starting dimensionality reduction: {X.shape[0]} samples x {X.shape[1]} features")
        ...
        try:
            ...
            if X.shape[1] <= self.config.target_dimensions:
                self.logger.warning(f"Input dimensionality ({X.shape[1]}) already <= target ({self.config.target_dimensions})")
            ...
            if method is None:
                if self.config.auto_method_selection:
                    ...
                    method = self.config.preferred_methods[0] if self.config.preferred_methods else 'pca'
            
            # Validate method availability
            if method not in self.available_methods:
                self.logger.warning(f"Method '{method}' not available, falling back to PCA")
                method = 'pca'
            ...
            evaluation_metrics = await self._evaluate_reduction_quality(X_processed, result.transformed_data, y)
            result.evaluation_metrics = evaluation_metrics
            result.quality_score = self._compute_overall_quality_score(result, evaluation_metrics)
            ...
            self.logger.info(f"Dimensionality reduction completed with method '{method}': "
    ...
    async def _preprocess_data(self, X: np.ndarray) -> np.ndarray:
        ...
        if X_processed.shape[1] < X.shape[1]:
            self.logger.info(f"Removed {X.shape[1] - X_processed.shape[1]} constant features")
    ...
    async def _select_optimal_method(self, X: np.ndarray, y: Optional[np.ndarray] = None) -> str:
        ...
        if n_samples > 5000 or self.config.fast_mode:
            fast_methods = [m for m in suitable_methods if m in ['pca', 'truncated_svd', 'gaussian_rp', 'sparse_rp']]
            if fast_methods:
                return fast_methods[0]
        
        # Prefer supervised methods when labels are available
        if y is not None:
            supervised_methods = [m for m in suitable_methods if self.available_methods[m]['supervised']]
        ...
        if self.config.variance_preservation_weight > 0.5:
            variance_methods = [m for m in suitable_methods if self.available_methods[m]['preserves_variance']]
            if variance_methods:
    ...
    async def _apply_reduction_method(
        ...
        try:
            ...
            reconstruction_error = self._calculate_reconstruction_error(X, X_reduced, reducer)
    ...
    async def _evaluate_reduction_quality(
        ...
        try:
            ...
            metrics['variance_preservation'] = min(1.0, reduced_total_var / (original_total_var + 1e-8))
            ...
            if X_original.shape[0] >= 20:  # Need sufficient samples
                try:
                    ...
                    if n_clusters >= 2:
                        ...
                        metrics['clustering_preservation'] = adjusted_rand_score(labels_orig, labels_reduced)
                except Exception as e:
            ...
            if y is not None and len(np.unique(y)) > 1:
                try:
                    ...
                    scores_orig = cross_val_score(clf, X_original, y, cv=3, scoring='accuracy')
                    scores_reduced = cross_val_score(clf, X_reduced, y, cv=3, scoring='accuracy')
                    
                    metrics['classification_preservation'] = np.mean(scores_reduced) / (np.mean(scores_orig) + 1e-8)
                except Exception as e:
            ...
            try:
                ...
                if k >= 2:
                    ...
                    neighbors_orig = nn_orig.kneighbors(X_original[sample_indices], return_distance=False)
                    neighbors_reduced = nn_reduced.kneighbors(X_reduced[sample_indices], return_distance=False)
    ...
    def _compute_overall_quality_score(
        ...
        try:
            ...
            if 'classification_preservation' in evaluation_metrics and evaluation_metrics['classification_preservation'] is not None:
                score_components.append(evaluation_metrics['classification_preservation'])
            ...
            time_penalty = max(0.0, 1.0 - result.processing_time / 60.0)  # Penalty after 1 minute
            score_components.append(time_penalty)
            weights.append(0.1)
            
            # Dimensionality efficiency (favor achieving target dimensions)
            target_efficiency = min(1.0, result.reduced_dimensions / self.config.target_dimensions)
    ...
    def get_recommended_method(self, X_shape: Tuple[int, int], y: Optional[np.ndarray] = None) -> str:
        ...
        if n_samples > 10000 or self.config.fast_mode:
            ...
            return 'lda'
        elif UMAP_AVAILABLE and n_features > 20:
            return 'umap'
        else:
            return 'pca'
...
def get_dimensionality_reducer(config: Optional[DimensionalityConfig] = None) -> AdvancedDimensionalityReducer:
