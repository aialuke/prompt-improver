...
class OptimizationValidator:
    ...
    async def validate_optimization(self, 
        ...
        try:
            ...
            effect_size = (np.mean(optimized_scores) - np.mean(baseline_scores)) / pooled_std
            ...
            valid = statistically_significant and practically_significant and improvement_detected
            ...
            return {
                ...
                "improvement": float(np.mean(optimized_scores) - np.mean(baseline_scores)),
                "validation_date": datetime.now().isoformat()
            }
    ...
    def _validate_metrics_realism(self, baseline_data: Dict[str, Any], optimized_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        realistic_ranges = {
            ...
            'throughput_rps': {'min': 1, 'max': 10000, 'suspicious_below': 5, 'suspicious_above': 5000},
            'error_rate_percent': {'min': 0, 'max': 100, 'suspicious_below': -0.1, 'suspicious_above': 10},
            'cache_hit_ratio': {'min': 0, 'max': 100, 'suspicious_below': 30, 'suspicious_above': 100.1}
        }
        
        ranges = realistic_ranges.get(metric_type, {
            'min': 0, 'max': float('inf'), 'suspicious_below': -1, 'suspicious_above': float('inf')
        })
        ...
        suspicious_high = any(score > ranges['suspicious_above'] for score in all_scores)
        ...
        lower_is_better = metric_type in ['response_time_ms', 'memory_usage_mb', 'cpu_usage_percent', 'error_rate_percent']
        ...
        return {
            ...
            'validation_status': 'PASS' if realistic and not (suspicious_low or suspicious_high) and improvement_direction_correct else 'SUSPICIOUS'
        }
    
    def _perform_cross_validation(self, baseline_scores: List[float], optimized_scores: List[float]) -> Dict[str, Any]:
        ...
        for train_idx, test_idx in kf.split(baseline_array):
            ...
            optimized_fold = optimized_array[test_idx] if len(test_idx) <= len(optimized_array) else optimized_array[:len(test_idx)]
            
            if len(optimized_fold) < len(test_idx):
                # Pad if necessary
                optimized_fold = np.concatenate([
                    optimized_fold, 
                    np.random.choice(optimized_fold, len(test_idx) - len(optimized_fold))
                ])
        ...
        # Aggregate results
        significant_folds = sum(1 for result in fold_results if result.get('significant', False))
