...
class ClusteringConfig:
    ...
    hdbscan_min_samples: Optional[int] = None  # Auto-computed
    hdbscan_alpha: float = 1.0
    hdbscan_cluster_selection_epsilon: float = 0.0
    hdbscan_algorithm: str = 'best'  # 'best', 'generic', 'prims_kdtree', 'prims_balltree'
...
class ClusteringOptimizer:
    ...
    def __init__(self, config: Optional[ClusteringConfig] = None):
        ...
        self.dimensionality_reducer: Optional[Union[PCA, umap.UMAP]] = None
        ...
        self.logger.info(f"Clustering optimizer initialized with {self.config.target_dimensions}D target")
    ...
    async def optimize_clustering(
        ...
        self.logger.info(f"Starting clustering optimization for {features.shape[0]} samples x {features.shape[1]} features")
        ...
        try:
            ...
            quality_metrics = self._assess_clustering_quality(
                ...
                clustering_result["probabilities"] if "probabilities" in clustering_result else None
            )
            ...
            status, status_message = self._evaluate_clustering_success(features, quality_metrics)
            ...
            self.logger.info(f"Clustering optimization completed in {time.time() - start_time:.2f}s with status '{status}' and quality {quality_metrics.quality_score:.3f}")
            return result
    ...
    async def _preprocess_features(
        ...
        try:
            ...
            if self.config.auto_dim_reduction and features.shape[1] > self.config.target_dimensions:
                if labels is not None:
                    ...
                    if self.dimensionality_reducer is None or not isinstance(self.dimensionality_reducer, PCA):
                        self.dimensionality_reducer = PCA(
                            n_components=min(self.config.target_dimensions, features.shape[1]),
                            random_state=42
                        )
                    
                    selected_features = self.dimensionality_reducer.fit_transform(scaled_features)
            ...
            if self.config.auto_dim_reduction and selected_features.shape[1] > self.config.target_dimensions:
                ...
                umap_reducer = umap.UMAP(
                    ...
                    random_state=42,
                    n_jobs=1 if self.config.memory_efficient_mode else self.config.n_jobs
                )
            ...
            if self.config.enable_caching and len(self.cache) < self.config.cache_max_size:
    ...
    async def _optimize_parameters(
        ...
        optimal_params["memory"] = {
            "core_dist_n_jobs": self.config.n_jobs if self.config.parallel_processing else 1
    ...
    async def _perform_optimized_clustering(
        ...
        try:
            ...
            if self.config.memory_efficient_mode and features.shape[0] > self.config.batch_size:
            ...
            self.logger.info(f"Clustering completed: {n_clusters} clusters, {n_noise_points} noise points")
    ...
    def _assess_clustering_quality(
        ...
        if n_clusters > 1:
            try:
                        ...
                        if probabilities is not None:
                            ...
                            try:
                                if probabilities.ndim >= 2 and probabilities.shape[1] > 1:
                                    stability_score = float(np.mean(np.max(probabilities, axis=1)))
                                elif probabilities.ndim == 1:
                                    stability_score = float(np.mean(probabilities))  # 1D probabilities
                                else:
                                    stability_score = self._compute_stability_score(filtered_features, filtered_labels)
                            except (IndexError, ValueError) as e:
                                self.logger.warning(f"Error using probabilities for stability (shape: {probabilities.shape}): {e}")
                                stability_score = self._compute_stability_score(filtered_features, filtered_labels)
                        else:
                ...
                calinski_normalized = min(1.0, calinski_harabasz / 100.0)  # Normalize Calinski score
                davies_bouldin_normalized = max(0.0, 1.0 - davies_bouldin / 5.0)  # Invert and normalize DB score
                noise_penalty = max(0.0, 1.0 - 2.0 * noise_ratio)  # Penalty for high noise ratio
    ...
    def _compute_stability_score(self, features: np.ndarray, labels: np.ndarray) -> float:
        ...
        try:
            ...
            for cluster_id in set(labels):
                ...
                if len(cluster_points) > 1:
                    ...
                    stability = max(0.0, 1.0 - avg_distance / 10.0)  # Normalize by reasonable distance threshold
    ...
    def _analyze_performance(
        ...
        analysis = {
            "performance_summary": {
                ...
                "memory_efficiency": clustering_result["memory_delta"] < self.config.max_memory_mb
            },
            "quality_assessment": {
                "overall_quality": quality_metrics.quality_score,
                "meets_thresholds": {
                    "silhouette": quality_metrics.silhouette_score >= self.config.min_silhouette_score,
                    "calinski_harabasz": quality_metrics.calinski_harabasz_score >= self.config.min_calinski_harabasz,
                    "davies_bouldin": quality_metrics.davies_bouldin_score <= self.config.max_davies_bouldin,
                    "noise_ratio": quality_metrics.noise_ratio <= self.config.max_noise_ratio
                }
            },
            "recommendations": []
        }
    ...
    def _get_optimized_umap_params(self, shape: Tuple[int, int]) -> Dict[str, Any]:
        ...
        if n_features > 20:  # High-dimensional adjustment
            n_neighbors = max(5, min(int(n_samples * 0.1), 50))  # 10% of samples, capped at 50
    ...
    def _get_optimized_hdbscan_params(self, shape: Tuple[int, int]) -> Dict[str, Any]:
        ...
        if n_features > 20:  # High-dimensional data adjustments
            ...
            min_cluster_size = max(10, min(int(n_samples * 0.03), n_samples // 8))  # 3% of samples, max 1/8th
            min_samples = max(3, min_cluster_size // 5)  # More sensitive for real clusters
    ...
    def _batch_clustering(self, features: np.ndarray) -> np.ndarray:
        ...
        self.logger.warning("Batch clustering not fully implemented, using standard clustering")
    ...
    def _evaluate_clustering_success(self, features: np.ndarray, quality_metrics: 'ClusteringMetrics') -> Tuple[str, str]:
        """
        ...
        if n_features > 20:  # High-dimensional space
            ...
            elif quality_metrics.quality_score >= 0.3:  # Moderate quality (research minimum)
                max_noise_threshold = 0.90  # Allow up to 90% noise for moderate but acceptable clusters
            else:  # Poor quality
        ...
        max_reasonable_clusters = min(20, max(10, int(n_samples * 0.1)))  # Max 10% of samples as clusters
        
        # Research Finding 3: Quality-First Evaluation
        success_criteria = {
            "has_meaningful_clusters": quality_metrics.n_clusters >= min_expected_clusters,
            "reasonable_cluster_count": quality_metrics.n_clusters <= max_reasonable_clusters,
            "acceptable_noise": quality_metrics.noise_ratio <= max_noise_threshold,
            "sufficient_quality": quality_metrics.quality_score >= 0.3,  # Research minimum
        ...
        if not success_criteria["has_meaningful_clusters"]:
            critical_failures.append(f"No meaningful clusters found (need ≥{min_expected_clusters})")
        
        if not success_criteria["reasonable_cluster_count"]:
            critical_failures.append(f"Too many clusters ({quality_metrics.n_clusters} > {max_reasonable_clusters})")
        
        if not success_criteria["sufficient_quality"]:
            critical_failures.append(f"Poor cluster quality ({quality_metrics.quality_score:.3f} < 0.3)")
        ...
        if not success_criteria["acceptable_noise"]:
            if n_features > 20 and quality_metrics.quality_score >= 0.4:
                ...
                warnings.append(f"High noise ratio ({quality_metrics.noise_ratio:.1%}) but acceptable for {n_features}D data with quality {quality_metrics.quality_score:.3f}")
            else:
                critical_failures.append(f"Excessive noise ({quality_metrics.noise_ratio:.1%} > {max_noise_threshold:.1%})")
        ...
        self.logger.info(f"Clustering evaluation - Features: {n_features}D, Samples: {n_samples}")
        self.logger.info(f"Adaptive noise threshold: {max_noise_threshold:.1%} (base: {base_noise_threshold:.1%})")
...
def get_clustering_optimizer(config: Optional[ClusteringConfig] = None) -> ClusteringOptimizer:
