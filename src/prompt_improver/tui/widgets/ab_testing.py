...
class ABTestingWidget(Static):
    ...
    def update_display(self) -> None:
        ...
        if experiments:
            ...
            for exp in experiments:
                ...
                exp_table.add_row(name, status_display, str(participants), conversion_display, significance_display)
        ...
        if 'error' in self.ab_data:
            error_panel = Panel(f"[red]{self.ab_data['error']}[/red]", title='Error', border_style='red')
    ...
    def _get_status_color(self, status: str) -> str:
        ...
        status_colors = {'running': 'green', 'completed': 'blue', 'paused': 'yellow', 'stopped': 'red', 'draft': 'gray', 'analyzing': 'cyan'}
    ...
    def _create_experiment_detail(self, exp: dict[str, Any]) -> Panel:
        ...
        significance_color = 'green' if is_significant else 'red'
        detail_table.add_row('Significant', f'[{significance_color}]{significance_text}[/{significance_color}]')
        ...
        detail_table.add_row('Effect Size', f'[{effect_color}]{effect_size:.3f}[/{effect_color}]')
