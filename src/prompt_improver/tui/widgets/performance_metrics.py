...
try:
    from prompt_improver.performance.monitoring.health.unified_health_system import UnifiedHealthMonitor, get_unified_health_monitor
...
class PerformanceMetricsWidget(Static):
    ...
    def __init__(self, enable_slo_monitoring: bool=True, enable_adaptive_thresholds: bool=True, **kwargs):
        super().__init__(**kwargs)
        self.console = Console()
        self.logger = logging.getLogger(__name__)
        self.enable_slo_monitoring = enable_slo_monitoring and HEALTH_MONITORING_AVAILABLE
        self.enable_adaptive_thresholds = enable_adaptive_thresholds
        self.adaptive_thresholds = {'response_time': {'warning': 150.0, 'critical': 300.0}, 'error_rate': {'warning': 0.01, 'critical': 0.05}, 'cpu_usage': {'warning': 70.0, 'critical': 90.0}, 'memory_usage': {'warning': 80.0, 'critical': 95.0}, 'queue_length': {'warning': 20, 'critical': 50}, 'cache_hit_rate': {'warning': 0.8, 'critical': 0.6}}
        ...
        if self.enable_slo_monitoring and HEALTH_MONITORING_AVAILABLE:
            try:
                ...
                self.logger.warning('Failed to initialize health monitoring: %s', e)
    ...
    async def update_data(self, data_provider) -> None:
        ...
        try:
            ...
            self.logger.error('Failed to update performance data: %s', e)
    ...
    def update_display(self) -> None:
        ...
        metrics_table.add_row('Response Time', f'[{response_color}]{response_time:.1f}ms[/{response_color}]')
        ...
        metrics_table.add_row('Error Rate', f'[{error_color}]{error_rate:.2%}[/{error_color}]')
        ...
        metrics_table.add_row('Queue Length', f'[{queue_color}]{queue_length}[/{queue_color}]')
        ...
        metrics_table.add_row('Cache Hit Rate', f'[{cache_color}]{cache_hit_rate:.1%}[/{cache_color}]')
        ...
        if 'error' in self.performance_data:
            error_panel = Panel(f"[red]{self.performance_data['error']}[/red]", title='Error', border_style='red')
    ...
    async def _update_slo_data(self) -> None:
        ...
        try:
            response_time = self.performance_data.get('response_time', 0)
            error_rate = self.performance_data.get('error_rate', 0.0)
            await self.performance_monitor.record_performance_measurement(operation_name='widget_metrics', response_time_ms=response_time, is_error=error_rate > 0.01, business_value=1.0)
            self.slo_data = self.performance_monitor.get_slo_dashboard()
        except Exception as e:
            self.logger.warning('Failed to update SLO data: %s', e)
    ...
    def _update_adaptive_thresholds(self) -> None:
        ...
        try:
            ...
            self.logger.warning('Failed to update adaptive thresholds: %s', e)
    ...
    def _update_health_status(self) -> None:
        ...
        try:
            ...
            self.logger.warning('Failed to update health status: %s', e)
        ...
        try:
            ...
            if error_budgets:
                ...
                for slo_name, budget_info in error_budgets.items():
                    ...
                    budget_table.add_row(slo_name, f'[{remaining_color}]{remaining_percent:.1f}%[/{remaining_color}]', f'[{burn_rate_color}]{burn_rate:.2f}x[/{burn_rate_color}]')
            ...
            self.logger.warning('Failed to update SLO display: %s', e)

    def _get_health_color(self, health_status: str) -> str:
        """Get color for health status."""
        health_colors = {'healthy': 'green', 'warning': 'yellow', 'critical': 'red', 'unknown': 'gray'}
