    ...
    def update_display(self) -> None:
        ...
        try:
            ...
            progress_table = Table(
                title="AutoML Optimization", show_header=False, box=None
            )
            progress_table.add_column("Metric", style="cyan")
            progress_table.add_column("Value", style="white")
            progress_table.add_row(
                "Status", f"[{status_color}]{status.upper()}[/{status_color}]"
            )
            ...
            if total_trials > 0:
                ...
                progress_bar = Bar(
                    size=20, begin=0, end=total_trials, width=current_trial
                )
                progress_table.add_row(
                    "Progress",
                    f"{progress_percent:.1f}% ({current_trial}/{total_trials})",
                )
            else:
            ...
            success_rate = (
                trials_completed / max(trials_completed + trials_failed, 1) * 100
            )
            ...
            if best_params:
                params_table = Table(
                    title="Best Parameters", show_header=True, box=None
                )
