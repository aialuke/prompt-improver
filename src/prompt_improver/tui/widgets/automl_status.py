...
class AutoMLStatusWidget(Static):
    ...
    def on_mount(self) -> None:
        ...
        try:
            ...
            self.automl_data = {'error': f'Mount error: {e!s}'}
            self.update_display()
    ...
    def update_display(self) -> None:
        ...
        try:
            ...
            if total_trials > 0:
                ...
                progress_bar = Bar(size=20, begin=0, end=total_trials, width=current_trial)
            ...
            if 'error' in self.automl_data:
                error_panel = Panel(f"[red]{self.automl_data['error']}[/red]", title='Error', border_style='red')
            ...
            self._show_error_state(f'Display update error: {e!s}')

    def _show_default_state(self) -> None:
        """Show default state when no data is available."""
        try:
            default_content = Panel('AutoML Status: No data available', title='AutoML Optimization', border_style='dim')
            content_widget = self.query_one('#automl-status-content', Static)
    ...
    def _show_error_state(self, error_message: str) -> None:
        ...
        try:
            error_content = Panel(f'[red]{error_message}[/red]', title='AutoML Status Error', border_style='red')
            content_widget = self.query_one('#automl-status-content', Static)
    ...
    def _get_status_color(self, status: str) -> str:
        ...
        status_colors = {'running': 'green', 'optimizing': 'green', 'completed': 'blue', 'failed': 'red', 'idle': 'yellow', 'paused': 'yellow', 'error': 'red'}
