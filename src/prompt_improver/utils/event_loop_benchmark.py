...
class EventLoopBenchmark:
    ...
    def __init__(self):
        ...
        self.uvloop_results: Optional[Dict[str, Any]] = None
    
    async def run_latency_benchmark(
        self, 
        samples: int = 100,
        operation_type: str = "sleep_yield"
    ) -> Dict[str, Any]:
    ...
    def _calculate_statistics(self, latencies: List[float]) -> Dict[str, Any]:
        ...
        if not latencies:
            return {
                "avg_ms": 0, "min_ms": 0, "max_ms": 0, "median_ms": 0,
                "std_ms": 0, "p95_ms": 0, "p99_ms": 0, "samples": 0
            }
    ...
    async def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        operation_types = ["sleep_yield", "task_creation", "gather_operations", "context_switch"]
    ...
    def _calculate_comparison(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "comparison": {
                ...
                "both_meet_target": (
                    baseline_avg < self.target_latency_ms and 
                    uvloop_avg < self.target_latency_ms
                ),
            },
            "timestamp": time.time(),
        }
...
async def run_startup_benchmark() -> Dict[str, Any]:
    ...
    results = await benchmark.run_latency_benchmark(samples=50, operation_type="sleep_yield")
    
    if results["meets_target"]:
        logger.info(f"✅ Event loop meets target latency: {results['avg_ms']:.2f}ms < {benchmark.target_latency_ms}ms")
    else:
        logger.warning(f"⚠️ Event loop exceeds target latency: {results['avg_ms']:.2f}ms > {benchmark.target_latency_ms}ms")
