...
class ModelManager:
    ...
    def get_pipeline(self, force_reload: bool = False) -> Any | None:
    ...
    def _select_optimal_model(self) -> str:
        ...
        if available_memory_mb >= self.config.memory_thresholds["distilbert"]:
            return self.config.fallback_model
        if available_memory_mb >= self.config.memory_thresholds["mobilebert"]:
            return self.config.tiny_model_alternatives[0]  # MobileBERT
        if available_memory_mb >= self.config.memory_thresholds["tinybert"]:
            return self.config.tiny_model_alternatives[1]  # TinyBERT
        return self.config.tiny_model_alternatives[2]  # bert-tiny

    def _initialize_pipeline(self) -> Any | None:
    ...
    def _create_optimized_pipeline(self, model_kwargs: dict[str, Any]) -> Any | None:
        """Create pipeline with memory optimizations."""
        try:
            # Check if we need accelerate for device_map
            device_map_enabled = (
                self.config.device_map != "cpu" and self.config.device_map is not None
            )
            ...
            if device_map_enabled:
                try:
                    ...
                    self.logger.warning(
                        "Accelerate not available, falling back to CPU-only mode"
                    )
            ...
            if self.config.use_quantization and self.config.quantization_bits in [4, 8]:
                if (
                    ...
                    or self.config.quantization_bits == 4
                ):
                    ...
                    self.logger.info(
                        "Using 4-bit quantization with double quantization"
                    )
                    ...
                    quantization_config = BitsAndBytesConfig(
                        load_in_8bit=True,
                        bnb_4bit_compute_dtype=getattr(
                            torch, self.config.torch_dtype, torch.bfloat16
                        ),
                    )
                    model_kwargs["quantization_config"] = quantization_config
            ...
            if device_map_enabled:
                ...
                self.logger.info(
                    "Successfully created optimized pipeline with device mapping"
                )
            else:
    ...
    def _create_basic_pipeline(self) -> Any | None:
    ...
    def _monitor_memory_usage(self):
        ...
        try:
            if torch.cuda.is_available():
                ...
                if (
                    memory_mb > self.config.max_memory_threshold_mb * 5
                ):  # 5x threshold for warning
                    self.logger.warning(
                        f"High GPU memory usage detected: {memory_mb:.1f} MB"
                    )
            else:
...
def get_ner_pipeline(config: ModelConfig | None = None) -> Any | None:
...
def get_lightweight_ner_pipeline() -> Any | None:
...
def get_ultra_lightweight_ner_pipeline() -> Any | None:
