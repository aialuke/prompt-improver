...
from typing import Any, Callable, Optional, Union, Literal, Dict, Pattern
...
class PIIRedactionFilter(logging.Filter):
    ...
    def __init__(self):
        ...
        self.patterns = [
            ...
            (re.compile(r'(password|pwd|pass|secret|key|token)\s*[=:]\s*\S+', re.IGNORECASE), 
             lambda m: f'{m.group(1)}=[REDACTED]'),
        ]
        
        # PII field names to redact from structured logs
        self.pii_fields = {
            'password', 'pwd', 'pass', 'secret', 'key', 'token', 'api_key',
            'email', 'phone', 'ssn', 'credit_card', 'address', 'full_name',
            'first_name', 'last_name', 'birth_date', 'dob'
        }
...
class AsyncErrorBoundary:
    ...
    def __init__(
        self,
        operation_name: str,
        logger: Optional[AsyncContextLogger] = None,
        reraise: bool = True,
