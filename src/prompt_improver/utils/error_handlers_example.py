...
async def example_with_class_based_boundary():
    ...
    async with AsyncErrorBoundary(
        operation_name="successful_background_task",
        logger=logger,
        reraise=True
    ) as boundary:
    ...
    async with AsyncErrorBoundary(
        ...
        try:
            ...
            print(f"❌ This shouldn't be reached because reraise=False")
...
async def example_correlation_id_tracking():
    ...
    logger = configure_structured_logging(
        logger_name="correlation_example",
        enable_pii_redaction=True,
        json_format=False
    )
