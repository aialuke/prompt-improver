...
from prompt_improver.utils.error_handlers import AsyncContextLogger, AsyncErrorBoundary, PIIRedactionFilter, async_error_boundary, configure_structured_logging

async def example_background_task(task_name: str, should_fail: bool=False) -> dict[str, Any]:
    """Example background task that might succeed or fail."""
    await asyncio.sleep(0.1)
    if should_fail:
        raise ValueError(f'Task {task_name} failed as requested')
    return {'task': task_name, 'status': 'completed', 'result': f'Success for {task_name}'}
...
async def example_with_class_based_boundary():
    ...
    logger = configure_structured_logging(logger_name='example_app', enable_pii_redaction=True, json_format=False)
    async with AsyncErrorBoundary(operation_name='successful_background_task', logger=logger, reraise=True) as boundary:
        result = await example_background_task('success_task', should_fail=False)
        logger.info('Task completed', task_result=result)
        print(f'✅ Success: {result}')
    async with AsyncErrorBoundary(operation_name='failing_background_task', logger=logger, reraise=False, fallback_result={'status': 'fallback', 'error': 'handled'}) as boundary:
        try:
...
async def example_with_context_manager_factory():
    ...
    logger = configure_structured_logging(logger_name='factory_example', enable_pii_redaction=True, json_format=False)
    async with async_error_boundary(operation_name='api_call_simulation', logger=logger, reraise=False, fallback_result={'error': 'api_unavailable', 'fallback': True}) as boundary:
...
async def example_timeout_handling():
    ...
    logger = configure_structured_logging(logger_name='timeout_example', enable_pii_redaction=True, json_format=False)

    async def slow_task():
        await asyncio.sleep(2)
        return {'status': 'completed'}
    async with AsyncErrorBoundary(operation_name='slow_operation', logger=logger, reraise=False, timeout=1.0) as boundary:
        try:
...
def example_pii_redaction():
    ...
    logger = configure_structured_logging(logger_name='pii_example', enable_pii_redaction=True, json_format=False)
    user_data = {'user_id': '12345', 'email': 'user@example.com', 'phone': '555-123-4567', 'credit_card': '4111-1111-1111-1111', 'name': 'John Doe', 'password': 'secret123'}
...
async def example_correlation_id_tracking():
    ...
    logger = configure_structured_logging(logger_name='correlation_example', enable_pii_redaction=True, json_format=False)
    logger.set_context(user_id='user123', session_id='sess456')
    async with AsyncErrorBoundary(operation_name='user_workflow', logger=logger, reraise=False) as boundary:
