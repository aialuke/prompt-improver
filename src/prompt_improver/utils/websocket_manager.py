...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, experiment_id: str, user_id: Optional[str] = None):
        ...
        logger.info(f"WebSocket connected for experiment {experiment_id}, user {user_id}")
    ...
    async def _ensure_redis_subscription(self, experiment_id: str):
        ...
        existing_tasks = [task for task in self._background_tasks 
                         if not task.done() and task.get_name() == subscription_key]
        
        if not existing_tasks:
            task = asyncio.create_task(
                self._redis_subscription_handler(experiment_id),
    ...
    async def _redis_subscription_handler(self, experiment_id: str):
        ...
        try:
            ...
            logger.error(f"Redis subscription error for experiment {experiment_id}: {e}")
        finally:
            try:
    ...
    def get_connection_count(self, experiment_id: str = None) -> int:
        ...
        return sum(len(connections) for connections in self.experiment_connections.values())
...
async def setup_redis_connection(redis_url: str = "redis://localhost:6379") -> redis.Redis:
...
async def publish_experiment_update(experiment_id: str, update_data: Dict[str, Any], 
