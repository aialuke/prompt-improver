...
class RedisCache:
    ...
    async def _get_client(self) -> Optional['coredis.Redis']:
        ...
        try:
            ...
            try:
                ...
                self._client = coredis.Redis(host=redis_config.host, port=redis_config.port, db=redis_config.database, password=redis_config.password, username=redis_config.username or None, socket_connect_timeout=redis_config.connection_timeout, socket_timeout=redis_config.socket_timeout, max_connections=redis_config.max_connections, decode_responses=False)
                ...
                self._client = coredis.Redis(host=redis_host, port=redis_port, db=redis_db, password=redis_password, socket_connect_timeout=5, socket_timeout=5, max_connections=20, decode_responses=False)
                await self._client.ping()
                logger.info('Redis cache connection established using environment variables')
                self._connection_error_logged = False
        except ImportError:
            ...
            if not self._connection_error_logged:
                logger.warning('Failed to connect to Redis - L2 cache disabled: %s', e)
                self._connection_error_logged = True
            self._client = None
            return None
        return self._client
    ...
    async def get(self, key: str) -> bytes | None:
        ...
        try:
            ...
            logger.warning('Redis get error for key {key}: %s', e)
            return None
    ...
    async def set(self, key: str, value: bytes, expire: int | None=None) -> bool:
        ...
        try:
            ...
            logger.warning('Redis set error for key {key}: %s', e)
            return False
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            ...
            logger.warning('Redis delete error for key {key}: %s', e)
            return False
    ...
    async def close(self) -> None:
        ...
        if self._client is not None:
            try:
                ...
                self._client = None
try:
    from prompt_improver.performance.optimization.performance_optimizer import measure_cache_operation
...
try:
    ...
    cache_operations_counter = meter.create_counter('cache_operations_total', description='Total cache operations by type, level, and status', unit='1')
    cache_hit_ratio_gauge = meter.create_gauge('cache_hit_ratio', description='Cache hit ratio by level and type', unit='ratio')
    cache_latency_histogram = meter.create_histogram('cache_operation_duration_seconds', description='Cache operation duration by type and level', unit='s')
    cache_size_gauge = meter.create_gauge('cache_size_current', description='Current cache size by level', unit='1')
    cache_warming_counter = meter.create_counter('cache_warming_operations_total', description='Cache warming operations by status', unit='1')
    cache_memory_usage_gauge = meter.create_gauge('cache_memory_usage_bytes', description='Estimated cache memory usage', unit='bytes')
    cache_error_counter = meter.create_counter('cache_errors_total', description='Cache errors by type and operation', unit='1')
except ImportError:
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            with tracer.start_as_current_span(op_name) as span:
                ...
                if hasattr(self, '_l1_cache'):
                    ...
                    span.set_attribute('cache.l1_max_size', getattr(self._l1_cache, '_max_size', 0))
                    span.set_attribute('cache.l1_current_size', len(getattr(self._l1_cache, '_cache', {})))
                ...
                try:
                    ...
                    if hasattr(self, '_total_requests'):
                        ...
                        span.set_attribute('cache.l1_hits', getattr(self, '_l1_hits', 0))
                        span.set_attribute('cache.l2_hits', getattr(self, '_l2_hits', 0))
                        span.set_attribute('cache.l3_hits', getattr(self, '_l3_hits', 0))
                    ...
                    if cache_operations_counter:
                        cache_operations_counter.add(1, {'operation': func.__name__, 'cache_class': self.__class__.__name__, 'status': 'success', 'cache_level': cache_level_hit or 'unknown'})
                    if cache_latency_histogram:
                        cache_latency_histogram.record(duration, {'operation': func.__name__, 'cache_class': self.__class__.__name__, 'cache_level': cache_level_hit or 'unknown'})
                    if cache_size_gauge and hasattr(self, '_l1_cache'):
                        cache_size_gauge.set(len(getattr(self._l1_cache, '_cache', {})), {'cache_class': self.__class__.__name__, 'level': 'l1'})
                    return result
                except Exception as e:
                    duration = time.perf_counter() - start_time
                    span.set_attribute('cache.duration_seconds', duration)
                    ...
                    if cache_operations_counter:
                        cache_operations_counter.add(1, {'operation': func.__name__, 'cache_class': self.__class__.__name__, 'status': 'error'})
                    if cache_error_counter:
                        cache_error_counter.add(1, {'operation': func.__name__, 'cache_class': self.__class__.__name__, 'error_type': type(e).__name__})
                    raise
        return wrapper
    return decorator
...
class MultiLevelCache:
    ...
    def __init__(self, l1_max_size: int=1000, l2_default_ttl: int=3600, enable_l2: bool=True, enable_warming: bool=True, warming_threshold: float=2.0, warming_interval: int=300, max_warming_keys: int=50):
        ...
        self._operation_stats = defaultdict(lambda: {'count': 0, 'total_time': 0.0, 'min_time': float('inf'), 'max_time': 0.0, 'error_count': 0})
        self._response_times = []
        self._max_response_time_samples = 1000
        self._health_status = {'overall_health': 'healthy', 'l1_health': 'healthy', 'l2_health': 'healthy', 'warming_health': 'healthy', 'last_health_check': datetime.now(UTC)}
    ...
    def _ensure_warming_started(self) -> None:
        ...
        try:
            ...
            asyncio.create_task(task_manager.submit_enhanced_task(task_id=f'cache_warming_{id(self)}', coroutine=self._background_warming_loop(), priority=TaskPriority.BACKGROUND, tags={'service': 'cache', 'type': 'warming', 'component': 'multi_level_cache'}))
            self._background_warming_task = True
        except ImportError:
            logger.warning('Background task manager not available, using direct task creation for cache warming')
            self._background_warming_task = asyncio.create_task(self._background_warming_loop())
        except Exception as e:
            logger.error('Failed to start background warming: %s', e)
            self._background_warming_task = True
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.error('Error in cache warming cycle: %s', e)
                    self._warming_stats['warming_errors'] += 1
        except asyncio.CancelledError:
            logger.info('Cache warming background task cancelled')
            raise
        except Exception as e:
            logger.error('Fatal error in cache warming loop: %s', e)
            raise
    ...
    async def _perform_warming_cycle(self) -> None:
        ...
        if not hot_keys:
            ...
            return
        logger.debug('Warming %s hot cache keys', len(hot_keys))
        warmed_count = 0
        warming_attempts = 0
        for key, pattern in hot_keys[:self._max_warming_keys]:
            warming_attempts += 1
            try:
                ...
                if await self._warm_key_from_l2(key):
                    ...
                    if OPENTELEMETRY_AVAILABLE and cache_warming_counter:
                        cache_warming_counter.add(1, {'status': 'success', 'cache_type': 'multi_level'})
                else:
                    warm_duration = time.perf_counter() - warm_start
                    self._record_operation_stats('warming_miss', warm_duration)
            except Exception as e:
                logger.warning('Failed to warm key {key}: %s', e)
                self._warming_stats['warming_errors'] += 1
                self._handle_cache_error('warming', e)
                if OPENTELEMETRY_AVAILABLE and cache_warming_counter:
                    cache_warming_counter.add(1, {'status': 'error', 'cache_type': 'multi_level'})
        ...
        if warmed_count > 0:
            logger.debug('Cache warming cycle completed: warmed %s/%s keys in %ss', warmed_count, warming_attempts, format(warming_duration, '.3f'))
    ...
    async def _warm_key_from_l2(self, key: str) -> bool:
        ...
        try:
            ...
            logger.warning('Failed to warm key {key} from L2: %s', e)
        return False
    ...
    def _cleanup_old_patterns(self) -> None:
        ...
        old_keys = [key for key, pattern in self._access_patterns.items() if pattern.last_access < cutoff]
        for key in old_keys:
            del self._access_patterns[key]
        logger.debug('Cleaned up %s old access patterns', len(old_keys))

    @trace_cache_operation('cache.get')
    async def get(self, key: str, fallback_func: callable | None=None, l2_ttl: int | None=None, l1_ttl: int | None=None) -> Any | None:
        ...
        try:
            async with measure_cache_operation('multi_level_get') as perf_metrics:
                ...
                if l1_value is not None:
                    self._l1_hits += 1
                    self._record_operation_stats('get_l1', time.perf_counter() - operation_start)
                ...
                if self._enable_l2 and self._l2_cache:
                    try:
                        ...
                        if l2_value is not None:
                            ...
                            self._l2_hits += 1
                            self._record_operation_stats('get_l2', time.perf_counter() - operation_start)
                ...
                if fallback_func:
                    try:
                        ...
                        if value is not None:
                            ...
                            self._l3_hits += 1
                            self._record_operation_stats('get_l3', time.perf_counter() - operation_start)
                        ...
                        logger.error('Fallback function failed for key {key}: %s', e)
                        raise
                return None
        finally:
    ...
    async def set(self, key: str, value: Any, l2_ttl: int | None=None, l1_ttl: int | None=None) -> None:
    ...
    async def delete(self, key: str) -> None:
        ...
        try:
            async with measure_cache_operation('multi_level_delete'):
                ...
                self._record_operation_stats('delete_l1', time.perf_counter() - l1_start, l1_success)
                self._update_health_indicators('l1', True)
                if self._enable_l2 and self._l2_cache:
                    try:
                        l2_start = time.perf_counter()
                        ...
                        l2_duration = time.perf_counter() - l2_start
                        self._record_operation_stats('delete_l2', l2_duration, l2_success)
                        self._update_health_indicators('l2', True, l2_duration)
                    except Exception as e:
                        l2_duration = time.perf_counter() - l2_start
                        self._record_operation_stats('delete_l2', l2_duration, False)
                        self._handle_cache_error('l2_delete', e)
                        logger.warning('Failed to delete from L2 cache for key %s: %s', key, e)
        finally:
            total_duration = time.perf_counter() - operation_start
            self._record_response_time(total_duration)
    ...
    async def warm_cache(self, keys: list[str]) -> dict[str, bool]:
        ...
        for key in keys:
            try:
                ...
                logger.warning('Failed to manually warm key {key}: %s', e)
                results[key] = False
        return results
    ...
    async def get_warming_candidates(self, limit: int=20) -> list[dict[str, Any]]:
        ...
        for key, pattern in hot_keys[:limit]:
            candidates.append({'key': key, 'access_count': pattern.access_count, 'access_frequency': pattern.access_frequency, 'last_access': pattern.last_access.isoformat(), 'warming_priority': pattern.warming_priority, 'in_l1_cache': self._l1_cache.get(key) is not None})
        return candidates

    async def stop_warming(self) -> None:
        """Stop background cache warming."""
        self._enable_warming = False
        if self._background_warming_task and isinstance(self._background_warming_task, asyncio.Task) and (not self._background_warming_task.done()):
    ...
    def _record_operation_stats(self, operation: str, duration: float, success: bool=True) -> None:
    ...
    def _handle_cache_error(self, operation: str, error: Exception) -> None:
        ...
        logger.warning('Cache error in %s: %s (consecutive errors: %s)', operation, error, self._consecutive_errors)

    def _update_health_indicators(self, level: str, success: bool, duration: float=None) -> None:
    ...
    def _calculate_response_time_percentiles(self) -> dict[str, float]:
        ...
        n = len(sorted_times)
        return {'p50': sorted_times[int(n * 0.5)] if n > 0 else 0.0, 'p95': sorted_times[int(n * 0.95)] if n > 0 else 0.0, 'p99': sorted_times[int(n * 0.99)] if n > 0 else 0.0, 'mean': statistics.mean(sorted_times), 'max': max(sorted_times), 'count': n}
    ...
    def _get_operation_statistics(self) -> dict[str, dict[str, float]]:
        ...
        for operation, data in self._operation_stats.items():
            if data['count'] > 0:
                stats[operation] = {'count': data['count'], 'avg_time': data['total_time'] / data['count'], 'min_time': data['min_time'] if data['min_time'] != float('inf') else 0.0, 'max_time': data['max_time'], 'total_time': data['total_time'], 'error_count': data['error_count'], 'success_rate': 1 - data['error_count'] / data['count']}
        return stats
    ...
    def _calculate_cache_efficiency(self) -> dict[str, float]:
        ...
        return {'overall': overall_efficiency, 'l1_efficiency': l1_efficiency, 'l2_efficiency': l2_efficiency, 'cache_penetration_rate': self._l3_hits / self._total_requests}
    ...
    def _calculate_slo_compliance(self) -> dict[str, Any]:
        ...
        return {'compliant': compliance_rate >= 0.95, 'slo_target_ms': slo_target * 1000, 'compliance_rate': compliance_rate, 'compliant_requests': compliant_requests, 'total_requests': len(self._response_times), 'violations': len(self._response_times) - compliant_requests}
    ...
    def _calculate_error_rate(self) -> dict[str, float]:
        ...
        return {'overall_error_rate': error_rate, 'consecutive_errors': self._consecutive_errors, 'total_errors': total_errors, 'error_threshold_exceeded': error_rate > 0.05}
    ...
    def _get_health_status(self) -> dict[str, str]:
        ...
        component_healths = [self._health_status['l1_health'], self._health_status['l2_health'], self._health_status['warming_health']]
    ...
    def _update_opentelemetry_metrics(self, overall_hit_rate: float) -> None:
        ...
        if cache_hit_ratio_gauge:
            cache_hit_ratio_gauge.set(overall_hit_rate, {'cache_type': 'multi_level', 'level': 'overall'})
            l1_hit_rate = self._l1_hits / self._total_requests if self._total_requests > 0 else 0
            cache_hit_ratio_gauge.set(l1_hit_rate, {'cache_type': 'multi_level', 'level': 'l1'})
            l2_hit_rate = self._l2_hits / self._total_requests if self._total_requests > 0 else 0
            cache_hit_ratio_gauge.set(l2_hit_rate, {'cache_type': 'multi_level', 'level': 'l2'})
            l3_hit_rate = self._l3_hits / self._total_requests if self._total_requests > 0 else 0
            cache_hit_ratio_gauge.set(l3_hit_rate, {'cache_type': 'multi_level', 'level': 'l3'})
        if cache_size_gauge:
            cache_size_gauge.set(len(self._l1_cache._cache), {'cache_type': 'multi_level', 'level': 'l1'})
            cache_size_gauge.set(len(self._access_patterns), {'cache_type': 'multi_level', 'level': 'access_patterns'})
        if cache_memory_usage_gauge:
            cache_memory_usage_gauge.set(self._estimate_l1_memory_usage(), {'cache_type': 'multi_level', 'level': 'l1'})
        if cache_warming_counter:
            cache_warming_counter.add(self._warming_stats.get('warming_hits', 0) - getattr(self, '_last_warming_hits_reported', 0), {'status': 'success', 'cache_type': 'multi_level'})
            cache_warming_counter.add(self._warming_stats.get('warming_errors', 0) - getattr(self, '_last_warming_errors_reported', 0), {'status': 'error', 'cache_type': 'multi_level'})
            self._last_warming_hits_reported = self._warming_stats.get('warming_hits', 0)
            self._last_warming_errors_reported = self._warming_stats.get('warming_errors', 0)
        ...
        try:
            ...
            if self._enable_l2 and self._l2_cache:
                try:
                    ...
                    l2_healthy = False
                    logger.warning('L2 health check failed: %s', e)
            ...
            return {'healthy': overall_healthy, 'checks': {'l1_cache': {'healthy': l1_healthy, 'status': 'healthy' if l1_healthy else 'unhealthy'}, 'l2_cache': {'healthy': l2_healthy, 'enabled': self._enable_l2, 'latency_seconds': l2_latency, 'status': 'healthy' if l2_healthy else 'unhealthy'}, 'warming_service': {'healthy': self._health_status['warming_health'] == 'healthy', 'enabled': self._enable_warming, 'status': self._health_status['warming_health']}}, 'performance': {'health_check_duration_seconds': health_check_duration, 'l2_latency_seconds': l2_latency, 'overall_hit_rate': (self._l1_hits + self._l2_hits) / max(self._total_requests, 1), 'error_rate': sum(self._error_stats.values()) / max(self._total_requests, 1)}, 'timestamp': datetime.now(UTC).isoformat()}
        except Exception as e:
    ...
    def get_monitoring_metrics(self) -> dict[str, Any]:
        ...
        return {'cache.hit_rate.overall': stats['overall_hit_rate'], 'cache.hit_rate.l1': stats['l1_cache']['hit_rate'], 'cache.hit_rate.l2': stats['l2_cache']['hit_rate'], 'cache.hit_rate.l3': stats['l3_fallback']['hit_rate'], 'cache.size.l1_current': stats['l1_cache']['size'], 'cache.size.l1_max': stats['l1_cache']['max_size'], 'cache.memory.l1_bytes': stats['l1_cache']['estimated_memory_usage_bytes'], 'cache.requests.total': stats['total_requests'], 'cache.requests.l1_hits': stats['l1_cache']['hits'], 'cache.requests.l2_hits': stats['l2_cache']['hits'], 'cache.requests.l3_hits': stats['l3_fallback']['hits'], 'cache.performance.response_time_p50': stats['performance_metrics']['response_times']['p50'], 'cache.performance.response_time_p95': stats['performance_metrics']['response_times']['p95'], 'cache.performance.response_time_p99': stats['performance_metrics']['response_times']['p99'], 'cache.performance.error_rate': stats['performance_metrics']['error_rate']['overall_error_rate'], 'cache.performance.slo_compliance': stats['performance_metrics']['slo_compliance']['compliance_rate'], 'cache.warming.enabled': stats['intelligent_warming']['enabled'], 'cache.warming.keys_warmed': stats['intelligent_warming']['warming_stats']['keys_warmed'], 'cache.warming.warming_errors': stats['intelligent_warming']['warming_stats']['warming_errors'], 'cache.warming.hit_rate': stats['intelligent_warming']['warming_hit_rate'], 'cache.health.overall': stats['health_monitoring']['overall_health'], 'cache.health.l1': stats['health_monitoring']['l1_health'], 'cache.health.l2': stats['health_monitoring']['l2_health'], 'cache.health.warming': stats['health_monitoring']['warming_health']}

    def get_alert_metrics(self) -> dict[str, Any]:
        """Get metrics specifically for alerting systems with thresholds."""
        stats = self.get_performance_stats()
        THRESHOLDS = {'hit_rate_critical': 0.5, 'hit_rate_warning': 0.7, 'error_rate_critical': 0.1, 'error_rate_warning': 0.05, 'response_time_critical': 1.0, 'response_time_warning': 0.5, 'slo_compliance_critical': 0.9, 'memory_usage_warning': 0.8, 'consecutive_errors_critical': 10, 'connection_failures_critical': 5}
        ...
        slo_compliance = stats['performance_metrics']['slo_compliance']['compliance_rate']
        memory_utilization = stats['l1_cache']['utilization']
        consecutive_errors = stats['performance_metrics']['error_rate']['consecutive_errors']
        connection_failures = stats['l2_cache'].get('connection_failures', 0)
        return {'alerts': {'hit_rate_critical': overall_hit_rate < THRESHOLDS['hit_rate_critical'], 'hit_rate_warning': overall_hit_rate < THRESHOLDS['hit_rate_warning'], 'error_rate_critical': error_rate > THRESHOLDS['error_rate_critical'], 'error_rate_warning': error_rate > THRESHOLDS['error_rate_warning'], 'response_time_critical': response_time_p95 > THRESHOLDS['response_time_critical'], 'response_time_warning': response_time_p95 > THRESHOLDS['response_time_warning'], 'slo_compliance_critical': slo_compliance < THRESHOLDS['slo_compliance_critical'], 'memory_usage_warning': memory_utilization > THRESHOLDS['memory_usage_warning'], 'consecutive_errors_critical': consecutive_errors > THRESHOLDS['consecutive_errors_critical'], 'connection_failures_critical': connection_failures > THRESHOLDS['connection_failures_critical'], 'cache_unhealthy': stats['health_monitoring']['overall_health'] == 'unhealthy', 'cache_degraded': stats['health_monitoring']['overall_health'] == 'degraded'}, 'values': {'hit_rate': overall_hit_rate, 'error_rate': error_rate, 'response_time_p95': response_time_p95, 'slo_compliance': slo_compliance, 'memory_utilization': memory_utilization, 'consecutive_errors': consecutive_errors, 'connection_failures': connection_failures, 'health_status': stats['health_monitoring']['overall_health']}, 'thresholds': THRESHOLDS, 'timestamp': datetime.now(UTC).isoformat()}
    ...
    def get_performance_stats(self) -> dict[str, Any]:
        ...
        warming_hit_rate = self._warming_stats['warming_hits'] / max(self._warming_stats['keys_warmed'], 1)
        ...
        return {'total_requests': self._total_requests, 'overall_hit_rate': overall_hit_rate, 'cache_efficiency': self._calculate_cache_efficiency(), 'l1_cache': {'hits': self._l1_hits, 'hit_rate': self._l1_hits / self._total_requests if self._total_requests > 0 else 0, 'estimated_memory_usage_bytes': self._estimate_l1_memory_usage(), **l1_stats}, 'l2_cache': {'hits': self._l2_hits, 'hit_rate': self._l2_hits / self._total_requests if self._total_requests > 0 else 0, 'enabled': self._enable_l2, 'connection_failures': self._connection_failures, 'health_status': health_status['l2_health']}, 'l3_fallback': {'hits': self._l3_hits, 'hit_rate': self._l3_hits / self._total_requests if self._total_requests > 0 else 0, 'health_status': health_status.get('l3_health', 'unknown')}, 'intelligent_warming': {'enabled': self._enable_warming, 'warming_threshold': self._warming_threshold, 'warming_interval_seconds': self._warming_interval, 'max_warming_keys': self._max_warming_keys, 'tracked_patterns': len(self._access_patterns), 'last_warming_time': self._last_warming_time.isoformat() if self._last_warming_time else None, 'warming_stats': self._warming_stats.copy(), 'warming_hit_rate': warming_hit_rate, 'health_status': health_status['warming_health']}, 'performance_metrics': {'response_times': response_time_stats, 'operation_stats': operation_stats, 'slo_compliance': self._calculate_slo_compliance(), 'error_rate': self._calculate_error_rate()}, 'health_monitoring': health_status, 'error_statistics': dict(self._error_stats), 'observability': {'opentelemetry_enabled': OPENTELEMETRY_AVAILABLE, 'performance_optimizer_enabled': PERFORMANCE_OPTIMIZER_AVAILABLE, 'last_health_check': self._health_status['last_health_check'].isoformat()}}

class SpecializedCaches:
    """Specialized cache instances for different data types."""

    def __init__(self):
        self.rule_cache = MultiLevelCache(l1_max_size=500, l2_default_ttl=7200, enable_l2=True)
        self.session_cache = MultiLevelCache(l1_max_size=1000, l2_default_ttl=1800, enable_l2=True)
        self.analytics_cache = MultiLevelCache(l1_max_size=200, l2_default_ttl=3600, enable_l2=True)
        self.prompt_cache = MultiLevelCache(l1_max_size=2000, l2_default_ttl=900, enable_l2=True)

    def get_cache_for_type(self, cache_type: str) -> MultiLevelCache:
        """Get specialized cache by type."""
        cache_map = {'rule': self.rule_cache, 'session': self.session_cache, 'analytics': self.analytics_cache, 'prompt': self.prompt_cache}
        return cache_map.get(cache_type, self.prompt_cache)

    def get_all_stats(self) -> dict[str, Any]:
        """Get statistics for all specialized caches."""
        return {'rule_cache': self.rule_cache.get_performance_stats(), 'session_cache': self.session_cache.get_performance_stats(), 'analytics_cache': self.analytics_cache.get_performance_stats(), 'prompt_cache': self.prompt_cache.get_performance_stats()}
...
async def cached_get(key: str, fallback_func: callable, cache_type: str='prompt', ttl: int=300) -> Any:
...
async def cached_set(key: str, value: Any, cache_type: str='prompt', ttl: int=300) -> None:
