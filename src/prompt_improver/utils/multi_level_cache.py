...
class RedisCache:
    ...
    async def _get_client(self) -> Optional['coredis.Redis']:
        ...
        try:
            # Import coredis dynamically to avoid import issues
            import coredis
            
            # Get Redis config from core.config (avoiding circular imports)
            try:
                from prompt_improver.core.config import get_config
                config = get_config()
                redis_config = config.redis
                
                # Create direct coredis connection using configuration
                self._client = coredis.Redis(
                    ...
                    decode_responses=False  # Work with bytes for cache data
                )
...
try:
    ...
    # Create comprehensive cache-specific metrics
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            with tracer.start_as_current_span(op_name) as span:
                ...
                try:
                    ...
                    # Determine cache level hit from performance metrics context
...
class MultiLevelCache:
    ...
    def __init__(
        ...
        warming_threshold: float = 2.0,  # accesses per hour to trigger warming
        warming_interval: int = 300,  # seconds between warming cycles
        max_warming_keys: int = 50  # maximum keys to warm per cycle
    ):
        ...
        # Access pattern tracking for intelligent warming
        ...
        self._background_warming_task: Optional[asyncio.Task] = None
        self._warming_stats = {
            "cycles_completed": 0,
            "keys_warmed": 0,
            "warming_hits": 0,
        ...
        # Initialize OpenTelemetry metric tracking variables
        ...
        # Response time tracking for SLO monitoring
        self._response_times = []
        self._max_response_time_samples = 1000  # Keep last 1000 samples
    ...
    def _start_background_warming(self) -> None:
        ...
        # Defer warming initialization to avoid circular imports
        # This will be called lazily when the cache is first used
        pass
    ...
    def _ensure_warming_started(self) -> None:
        ...
        try:
            # Import the background task manager for unified async infrastructure
            ...
            asyncio.create_task(task_manager.submit_enhanced_task(
                ...
                tags={"service": "cache", "type": "warming", "component": "multi_level_cache"}
            ))
            
            # Mark as started
            self._background_warming_task = True  # Use as a flag
            
        except ImportError:
            # Fallback to direct task creation if background manager not available
            ...
            self._background_warming_task = True  # Prevent repeated attempts
    ...
    async def _perform_warming_cycle(self) -> None:
        ...
        # Get hot keys that should be warmed
            ...
            try:
                ...
                if await self._warm_key_from_l2(key):
                    ...
                    if OPENTELEMETRY_AVAILABLE and cache_warming_counter:
                        cache_warming_counter.add(1, {
                            "status": "success",
                            "cache_type": "multi_level"
                        })
                else:
                    # Record failed warming attempt
                    warm_duration = time.perf_counter() - warm_start
        ...
        # Record overall warming cycle performance
        self._record_operation_stats('warming_cycle', warming_duration)
        
        # Update health status based on warming performance
        warming_success_rate = warmed_count / max(warming_attempts, 1)
        if warming_success_rate < 0.1 and warming_attempts > 5:  # Less than 10% success rate
            self._health_status['warming_health'] = 'degraded'
        elif warming_success_rate >= 0.5:  # Good success rate
            self._health_status['warming_health'] = 'healthy'
        
        if warmed_count > 0:
            logger.debug(f"Cache warming cycle completed: warmed {warmed_count}/{warming_attempts} keys in {warming_duration:.3f}s")
    ...
    def _get_hot_keys_for_warming(self) -> List[Tuple[str, AccessPattern]]:
        ...
        for key, pattern in self._access_patterns.items():
            # Skip if not accessed recently
            if (now - pattern.last_access).total_seconds() > 3600:  # 1 hour
                continue
                
            # Skip if access frequency is below threshold
    ...
    def _record_access_pattern(self, key: str) -> None:
        ...
        if len(self._access_patterns) > 10000:  # Configurable limit
    ...
    def _cleanup_old_patterns(self) -> None:
        ...
        # Remove patterns not accessed in the last day
        old_keys = [
                ...
                if fallback_func:
                    try:
                        ...
                        if value is not None:
                            ...
                            # Track L3 performance for health monitoring
                            if l3_duration > 1.0:  # Slow L3 operation
    ...
    async def clear(self) -> None:
        ...
        try:
            async with measure_cache_operation("multi_level_clear"):
                ...
                # Reset performance counters after clear
                self._l1_hits = 0
                self._l2_hits = 0
                self._l3_hits = 0
                self._total_requests = 0
    ...
    async def stop_warming(self) -> None:
        ...
        if (self._background_warming_task and 
            isinstance(self._background_warming_task, asyncio.Task) and 
            not self._background_warming_task.done()):
    ...
    def _handle_cache_error(self, operation: str, error: Exception) -> None:
        ...
        logger.warning(f"Cache error in {operation}: {error} (consecutive errors: {self._consecutive_errors})")
    ...
    def _update_health_indicators(self, level: str, success: bool, duration: float = None) -> None:
        ...
        if success:
            self._consecutive_errors = 0
            if level == 'l2' and duration and duration < 0.1:  # L2 operations should be fast
    ...
    def _calculate_slo_compliance(self) -> Dict[str, Any]:
        ...
        slo_target = 0.2  # 200ms SLO target
    ...
    def _calculate_error_rate(self) -> Dict[str, float]:
        ...
        return {
            ...
            "error_threshold_exceeded": error_rate > 0.05  # 5% error threshold
        }
    ...
    def _get_health_status(self) -> Dict[str, str]:
        ...
        if self._enable_warming:
            ...
            if warming_error_rate > 0.1:  # 10% error rate threshold
                self._health_status['warming_health'] = 'degraded'
            if warming_error_rate > 0.3:  # 30% error rate threshold
    ...
    def _update_opentelemetry_metrics(self, overall_hit_rate: float) -> None:
        ...
        if cache_warming_counter:
            ...
            # Track what we've reported to avoid double-counting
        ...
        try:
            ...
            return {
                ...
                "performance": {
                    ...
                    "overall_hit_rate": (self._l1_hits + self._l2_hits) / max(self._total_requests, 1),
                    "error_rate": sum(self._error_stats.values()) / max(self._total_requests, 1)
                },
    ...
    def get_monitoring_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache.performance.response_time_p50": stats["performance_metrics"]["response_times"]["p50"],
            "cache.performance.response_time_p95": stats["performance_metrics"]["response_times"]["p95"],
            "cache.performance.response_time_p99": stats["performance_metrics"]["response_times"]["p99"],
            "cache.performance.error_rate": stats["performance_metrics"]["error_rate"]["overall_error_rate"],
            "cache.performance.slo_compliance": stats["performance_metrics"]["slo_compliance"]["compliance_rate"],
            "cache.warming.enabled": stats["intelligent_warming"]["enabled"],
            "cache.warming.keys_warmed": stats["intelligent_warming"]["warming_stats"]["keys_warmed"],
            "cache.warming.warming_errors": stats["intelligent_warming"]["warming_stats"]["warming_errors"],
    ...
    def get_alert_metrics(self) -> Dict[str, Any]:
        ...
        THRESHOLDS = {
            "hit_rate_critical": 0.5,      # 50% hit rate minimum
            ...
            "response_time_critical": 1.0, # 1 second critical
            "response_time_warning": 0.5,  # 500ms warning
            "slo_compliance_critical": 0.9, # 90% SLO compliance minimum
        ...
        return {
            "alerts": {
                ...
                "response_time_critical": response_time_p95 > THRESHOLDS["response_time_critical"],
                "response_time_warning": response_time_p95 > THRESHOLDS["response_time_warning"],
                "slo_compliance_critical": slo_compliance < THRESHOLDS["slo_compliance_critical"],
                "memory_usage_warning": memory_utilization > THRESHOLDS["memory_usage_warning"],
                "consecutive_errors_critical": consecutive_errors > THRESHOLDS["consecutive_errors_critical"],
                "connection_failures_critical": connection_failures > THRESHOLDS["connection_failures_critical"],
                "cache_unhealthy": stats["health_monitoring"]["overall_health"] == "unhealthy",
                "cache_degraded": stats["health_monitoring"]["overall_health"] == "degraded"
            },
            "values": {
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        warming_hit_rate = (
            self._warming_stats["warming_hits"] / 
            max(self._warming_stats["keys_warmed"], 1)
        )
        ...
        return {
            ...
            "observability": {
                ...
                "last_health_check": self._health_status['last_health_check'].isoformat()
            }
        }
