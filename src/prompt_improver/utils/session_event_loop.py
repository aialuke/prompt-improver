...
class SessionEventLoopWrapper:
    ...
    async def run_with_timeout(
        self, coro, timeout: float | None = None, operation_name: str = "operation"
    ) -> T:
    ...
    def create_task(self, coro, name: str | None = None) -> asyncio.Task:
        ...
        logger.debug(
            f"Session {self.session_id} created task {name or task.get_name()}"
        )
        return task

    async def gather_with_timeout(
        self, *coros, timeout: float | None = None, return_exceptions: bool = False
    ) -> list:
    ...
    async def performance_context(
        self, operation_name: str = "context_operation"
    ...
    def _update_metrics(self, duration_ms: float, success: bool) -> None:
        ...
        self._session_metrics["min_time_ms"] = min(
            self._session_metrics["min_time_ms"], duration_ms
        )
        self._session_metrics["max_time_ms"] = max(
            self._session_metrics["max_time_ms"], duration_ms
        )

        # Update average
        self._session_metrics["avg_time_ms"] = (
            self._session_metrics["total_time_ms"]
            / self._session_metrics["operations_count"]
        )
    ...
    async def cancel_all_tasks(self) -> None:
        ...
        logger.info(
            f"Cancelling {len(self._active_tasks)} active tasks for session {self.session_id}"
        )
