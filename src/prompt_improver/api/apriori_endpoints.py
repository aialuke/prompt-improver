...
from ..services.ml_integration import get_ml_service, MLModelService
...
async def run_apriori_analysis(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Internal error during Apriori analysis: {str(e)}"
        )
...
async def comprehensive_pattern_discovery(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Internal error during pattern discovery: {str(e)}"
        )
...
async def get_association_rules(
    ...
    try:
        ...
        if pattern_category:
            conditions.append(AprioriAssociationRule.pattern_category == pattern_category)  # type: ignore[arg-type]
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error retrieving association rules: {str(e)}"
        )
...
async def get_contextualized_patterns(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error analyzing contextualized patterns: {str(e)}"
        )
...
async def get_discovery_runs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error retrieving discovery runs: {str(e)}"
        )
...
async def get_discovery_insights(
    ...
    try:
        ...
        rules_query = select(AprioriAssociationRule).where(
            AprioriAssociationRule.discovery_run_id == discovery_run_id  # type: ignore[arg-type]
        ).order_by(AprioriAssociationRule.rule_strength.desc())  # type: ignore[attr-defined]
...
async def _store_pattern_discovery_results(
    ...
    try:
        ...
        results_record = AdvancedPatternResults(  # type: ignore[call-arg]
            ...
            sequence_patterns=results.get("advanced_patterns", {}).get("sequence_patterns"),
            performance_patterns=results.get("advanced_patterns", {}).get("performance_patterns"),
            semantic_patterns=results.get("advanced_patterns", {}).get("semantic_patterns"),
            apriori_patterns=results.get("apriori_patterns"),
            cross_validation=results.get("cross_validation"),
            ensemble_analysis=results.get("advanced_patterns", {}).get("ensemble_analysis"),
            unified_recommendations=results.get("unified_recommendations", []),
            business_insights=results.get("business_insights", {}),
