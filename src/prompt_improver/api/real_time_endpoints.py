    ...
    try:
        ...
        if message_type == "ping":
            ...
            try:
                ...
                logger.exception(f"Error getting metrics in WebSocket: {e}")
                metrics = None
            if metrics:
                await connection_manager.send_to_connection(
                    websocket,
...
async def get_experiment_metrics(
    ...
    try:
        ...
        logger.exception(f"Error getting experiment metrics: {e}")
        raise HTTPException(
...
async def get_active_monitoring(
    ...
    try:
        ...
        logger.exception(f"Error getting active monitoring: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_orchestrator_components(
    ...
    try:
        ...
        logger.exception(f"Error getting orchestrator components: {e}")
        return JSONResponse(
            status_code=500,
            content={
                "success": False,
...
async def get_orchestrator_history(
    ...
    try:
        ...
        logger.exception(f"Error getting orchestrator history: {e}")
        return JSONResponse(
            status_code=500,
            content={
                "success": False,
...
async def setup_real_time_system(redis_url: str | None = None):
    ...
    try:
        if redis_available:
            if redis_url is None:
                ...
                from prompt_improver.services.cache.l2_redis_service import (
        ...
        logger.exception(f"Failed to setup real-time system: {e}")
...
async def cleanup_real_time_system():
    ...
    try:
        ...
        logger.exception(f"Error during real-time system cleanup: {e}")
