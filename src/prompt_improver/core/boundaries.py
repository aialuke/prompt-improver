...
class ArchitecturalRules:
    ...
    LAYER_DEPENDENCIES = {
        ...
        ArchitecturalLayer.DOMAIN: {ArchitecturalLayer.SHARED, ArchitecturalLayer.CORE},
        ...
        ArchitecturalLayer.SHARED: {ArchitecturalLayer.CORE},
        ArchitecturalLayer.CORE: set(),
    }
    MODULE_BOUNDARIES = {
...
class DependencyAnalyzer:
    ...
    def validate_layer_dependencies(self) -> list[BoundaryViolation]:
        ...
        for source_module, dependencies in self.module_graph.items():
            ...
            for dep_module in dependencies:
                ...
                allowed_layers = ArchitecturalRules.LAYER_DEPENDENCIES.get(
                    source_layer, set()
                )
                if target_layer not in allowed_layers and source_layer != target_layer:
                    violations.append(
                        BoundaryViolation(
                            ...
                            suggestion="Use dependency inversion or move to allowed layer",
                        )
                    )
        return violations
    ...
    def validate_module_boundaries(self) -> list[BoundaryViolation]:
        ...
        for source_module, dependencies in self.module_graph.items():
            ...
            for dep_module in dependencies:
                if any(
                    dep_module.startswith(forbidden)
                    for forbidden in source_boundary.forbidden_dependencies
                ):
                    violations.append(
                        BoundaryViolation(
                ...
                if source_boundary.allowed_dependencies:
                    if not any(
                        dep_module.startswith(allowed)
                        for allowed in source_boundary.allowed_dependencies
                    ):
                        violations.append(
                            BoundaryViolation(
...
class BoundaryEnforcer:
    ...
    def _calculate_metrics(
        ...
        highest_coupling = max(
            dependency_graph.items(), key=lambda x: len(x[1]), default=("", set())
        )
        return {
