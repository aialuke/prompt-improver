...
from prompt_improver.core.protocols.ml_protocols import CacheServiceProtocol, DatabaseServiceProtocol, EventBusProtocol, HealthMonitorProtocol, MLflowServiceProtocol, ResourceManagerProtocol, ServiceContainerProtocol, WorkflowEngineProtocol
if TYPE_CHECKING:
...
class MLPipelineOrchestratorFactory:
    ...
    @staticmethod
    def create_with_dependencies(mlflow_service: MLflowServiceProtocol, cache_service: CacheServiceProtocol, database_service: DatabaseServiceProtocol, event_bus: EventBusProtocol, workflow_engine: WorkflowEngineProtocol, resource_manager: ResourceManagerProtocol, health_monitor: HealthMonitorProtocol, config: dict[str, Any] | None=None) -> 'MLPipelineOrchestrator':
        ...
        return MLPipelineOrchestrator(config=orch_config, external_services_config=external_config, mlflow_service=mlflow_service, cache_service=cache_service, database_service=database_service, event_bus=event_bus, workflow_engine=workflow_engine, resource_manager=resource_manager, health_monitor=health_monitor)

    @staticmethod
    async def create_from_container(service_container: ServiceContainerProtocol, config: dict[str, Any] | None=None) -> 'MLPipelineOrchestrator':
        ...
        try:
            ...
            component_registry = await service_container.get_service('component_registry')
            ...
            orchestrator_config = await service_container.get_service('orchestrator_config')
            external_config = await service_container.get_service('external_services_config')
            health_monitor = None
            retry_manager = None
            ...
            from prompt_improver.ml.orchestration.core.ml_pipeline_orchestrator import MLPipelineOrchestrator
            orchestrator = MLPipelineOrchestrator(event_bus=event_bus, workflow_engine=workflow_engine, resource_manager=resource_manager, component_registry=component_registry, component_factory=component_factory, mlflow_service=mlflow_service, cache_service=cache_service, database_service=database_service, config=orchestrator_config, external_services_config=external_config, health_monitor=health_monitor, retry_manager=retry_manager, input_sanitizer=input_sanitizer, memory_guard=memory_guard)
            logger.info('ML Pipeline Orchestrator created successfully from service container')
            return orchestrator
...
class ComponentLoaderFactory:
    ...
    @staticmethod
    def create_component_loader(cache_service: CacheServiceProtocol, config: dict[str, Any] | None=None) -> Any:
...
def create_ml_pipeline_orchestrator(services: dict[str, Any], config: dict[str, Any] | None=None) -> 'MLPipelineOrchestrator':
    ...
    """
    return MLPipelineOrchestratorFactory.create_with_dependencies(mlflow_service=services['mlflow_service'], cache_service=services['cache_service'], database_service=services['database_service'], event_bus=services['event_bus'], workflow_engine=services['workflow_engine'], resource_manager=services['resource_manager'], health_monitor=services['health_monitor'], config=config)

async def create_production_orchestrator(config_file: str | None=None) -> 'MLPipelineOrchestrator':
...
async def ml_pipeline_context(config: dict[str, Any] | None=None):
    ...
    orchestrator = None
    try:
        orchestrator = await MLPipelineOrchestratorFactory.create_with_external_services(config)
        yield orchestrator
    finally:
...
class ExternalServiceFactory:
    ...
    def create_mlflow_adapter(config: dict[str, Any]) -> MLflowServiceProtocol:
        ...
        from prompt_improver.core.services.ml_service_adapters import create_mlflow_service
    ...
    def create_redis_adapter(config: dict[str, Any]) -> CacheServiceProtocol:
        ...
        from prompt_improver.core.services.ml_service_adapters import create_cache_service
    ...
    def create_postgresql_adapter(config: dict[str, Any]) -> DatabaseServiceProtocol:
        ...
        from prompt_improver.core.services.ml_service_adapters import create_database_service
