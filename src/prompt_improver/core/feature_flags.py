...
class FeatureFlagDefinition(SQLModel):
...
class FeatureFlagMetrics(SQLModel):
...
class FileWatcher(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        try:
            ...
            logger.error('Error in file watcher callback: %s', e)
...
class FeatureFlagManager:
            ...
            with self._lock:
                ...
                for flag_key, flag_config in flags_data.items():
                    try:
                        ...
                        logger.error("Error loading flag '{flag_key}': %s", e)
                        continue
                self._flags = new_flags
                self._config_loaded_at = datetime.now(UTC)
                self._config_version = config_data.get('version', 'unknown')
                logger.info('Loaded %s feature flags from %s', len(self._flags), self.config_path)
        except Exception as e:
    ...
    def _start_file_watcher(self) -> None:
        ...
        try:
            ...
            logger.info('Started file watcher for %s', watch_path)
        except Exception as e:
            logger.error('Error starting file watcher: %s', e)
    ...
    def _evaluate_targeting_rules(self, flag: FeatureFlagDefinition, context: EvaluationContext) -> str | None:
        ...
        for rule in sorted_rules:
            try:
                ...
                logger.error("Error evaluating rule '%s' for flag '%s': %s", rule.name, flag.key, e)
                continue
        return None

    def _evaluate_condition(self, condition: str, context: EvaluationContext) -> bool:
        """Evaluate a simple condition against context."""
        context_dict = {'user_id': context.user_id, 'user_type': context.user_type, 'environment': context.environment, **context.custom_attributes}
        try:
    ...
    def evaluate_flag(self, flag_key: str, context: EvaluationContext, default_value: Any=False) -> EvaluationResult:
        ...
        try:
            with self._lock:
                ...
                if not flag:
                    result = EvaluationResult(flag_key=flag_key, variant='default', value=default_value, reason='FLAG_NOT_FOUND', evaluation_context=context)
                    self._update_metrics(result)
                    return result
                if flag.state == FlagState.DISABLED:
                    variant = flag.default_variant
                    value = flag.variants.get(variant, default_value)
                    result = EvaluationResult(flag_key=flag_key, variant=variant, value=value, reason='FLAG_DISABLED', evaluation_context=context)
                    self._update_metrics(result)
                    return result
                targeted_variant = self._evaluate_targeting_rules(flag, context)
                if targeted_variant:
                    value = flag.variants.get(targeted_variant, default_value)
                    result = EvaluationResult(flag_key=flag_key, variant=targeted_variant, value=value, reason='TARGETING_MATCH', evaluation_context=context)
                    self._update_metrics(result)
                    return result
                if flag.state == FlagState.ROLLOUT and self._evaluate_rollout(flag, context):
                    rollout_variant = 'on' if 'on' in flag.variants else next((v for v in flag.variants.keys() if v != flag.default_variant), flag.default_variant)
                    value = flag.variants.get(rollout_variant, default_value)
                    result = EvaluationResult(flag_key=flag_key, variant=rollout_variant, value=value, reason='ROLLOUT_MATCH', evaluation_context=context)
                    self._update_metrics(result)
                    return result
                variant = flag.default_variant
                value = flag.variants.get(variant, default_value)
                result = EvaluationResult(flag_key=flag_key, variant=variant, value=value, reason='DEFAULT', evaluation_context=context)
                self._update_metrics(result)
                return result
        except Exception as e:
            logger.error("Error evaluating flag '{flag_key}': %s", e)
            with self._lock:
            ...
            result = EvaluationResult(flag_key=flag_key, variant='error', value=default_value, reason='EVALUATION_ERROR', evaluation_context=context)
            return result
    ...
    def get_metrics(self, flag_key: str | None=None) -> FeatureFlagMetrics | dict[str, FeatureFlagMetrics]:
    ...
    def get_configuration_info(self) -> dict[str, Any]:
        ...
        with self._lock:
            return {'config_path': str(self.config_path), 'loaded_at': self._config_loaded_at.isoformat() if self._config_loaded_at else None, 'version': self._config_version, 'flags_count': len(self._flags), 'watching_files': bool(self._observer and self._observer.is_alive())}
...
def initialize_feature_flags(config_path: str | Path, watch_files: bool=True) -> FeatureFlagManager:
...
def is_feature_enabled(flag_key: str, user_id: str | None=None, **context_attrs) -> bool:
...
def get_feature_variant(flag_key: str, user_id: str | None=None, default: str='off', **context_attrs) -> str:
