...
if TYPE_CHECKING:
    from prompt_improver.core.events.ml_event_bus import MLEvent, MLEventBus, MLEventType
...
class MLServiceInterface(MLAnalysisInterface, MLTrainingInterface, MLHealthInterface, MLModelInterface):
...
async def request_ml_analysis_via_events(analysis_type: str, input_data: dict[str, Any], correlation_id: str | None=None) -> str:
    ...
    event = MLEvent(event_type=MLEventType.ANALYSIS_REQUEST, source='mcp_interface', data={'request_id': request_id, 'analysis_type': analysis_type, 'input_data': input_data}, correlation_id=correlation_id)
    await event_bus.publish(event)
    return request_id

async def request_ml_training_via_events(model_type: str, training_data: dict[str, Any], hyperparameters: dict[str, Any] | None=None) -> str:
    ...
    event = MLEvent(event_type=MLEventType.TRAINING_REQUEST, source='mcp_interface', data={'job_id': job_id, 'model_type': model_type, 'training_data': training_data, 'hyperparameters': hyperparameters or {}})
    await event_bus.publish(event)
    return job_id

async def request_ml_health_check_via_events(components: list[str] | None=None) -> str:
    ...
    event = MLEvent(event_type=MLEventType.HEALTH_CHECK_REQUEST, source='mcp_interface', data={'check_id': check_id, 'components': components or []})
    await event_bus.publish(event)
    return check_id
...
def create_ml_analysis_request(analysis_type: str, input_data: dict[str, Any], **kwargs: dict[str, Any]) -> MLAnalysisRequest:
    """Create a validated ML analysis request."""
    return MLAnalysisRequest(analysis_type=analysis_type, input_data=input_data, **kwargs)

def create_ml_training_request(model_type: MLModelType, training_data: dict[str, Any], **kwargs: dict[str, Any]) -> MLTrainingRequest:
    """Create a validated ML training request."""
    return MLTrainingRequest(model_type=model_type, training_data=training_data, **kwargs)
