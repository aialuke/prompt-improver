...
class EventBasedMLTrainingService(MLTrainingInterface):
    ...
    async def train_model(
        ...
        self._job_counter += 1
        job_id = (
            f"training_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{self._job_counter}"
        )
        try:
            ...
            self.logger.info(
                f"Training job {job_id} submitted for model type: {model_type}"
            )
            return job_id
    ...
    async def retrain_model(
        ...
        retrain_config: dict[str, Any] | None = None,
    ) -> str:
        ...
        self._job_counter += 1
        job_id = (
            f"retrain_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{self._job_counter}"
        )
        try:
            event_bus = await get_ml_event_bus()
            retrain_event = MLEvent(
                ...
                data={
                    ...
                    or {"incremental": True, "validation_split": 0.2},
                },
            )
            await event_bus.publish(retrain_event)
            self.logger.info(
                f"Model retraining job {job_id} submitted for model: {model_id}"
            )
            return job_id
        except Exception as e:
    ...
    async def cancel_training(self, job_id: str) -> bool:
        ...
        try:
            ...
            cancel_event = MLEvent(
                event_type=MLEventType.TRAINING_REQUEST,
                source="event_based_ml_training_service",
                data={"job_id": job_id, "operation": "cancel_training"},
            )
