...
class PromptDataProtection:
    ...
    def __init__(
        ...
        self.sensitive_patterns = [
            ("sk-[a-zA-Z0-9]{40,60}", "openai_api_key", 0.95, "HIGH"),
            ("ghp_[a-zA-Z0-9]{36,50}", "github_token", 0.95, "HIGH"),
            (
            ...
            ("AKIA[0-9A-Z]{16}", "aws_access_key", 0.95, "HIGH"),
            ("ya29\\.[0-9A-Za-z\\-_]+", "google_oauth_token", 0.9, "HIGH"),
            (
            ...
            ("\\b\\d{3}-\\d{2}-\\d{4}\\b", "ssn_pattern", 0.9, "HIGH"),
            (
            ...
            ("password[\\s]*[:=][\\s]*[^\\s]+", "password_field", 0.85, "HIGH"),
            ("api_key[\\s]*[:=][\\s]*[^\\s]+", "api_key_field", 0.9, "HIGH"),
            ("secret[\\s]*[:=][\\s]*[^\\s]+", "secret_field", 0.85, "HIGH"),
            (
                "\\b(?:Bearer|Token)\\s+[A-Za-z0-9\\-._~+/]+=*",
                "bearer_token",
                0.9,
                "HIGH",
            ),
            ("\\b[A-Z0-9]{20,}\\b", "long_token", 0.7, "MEDIUM"),
            ("[a-f0-9]{32,64}", "hash_or_token", 0.65, "MEDIUM"),
            ("(?i)ignore\\s+previous\\s+instructions", "prompt_injection", 0.8, "HIGH"),
            ("(?i)system\\s*:\\s*you\\s+are\\s+now", "system_override", 0.85, "HIGH"),
            ("(?i)jailbreak|bypass\\s+safety", "jailbreak_attempt", 0.8, "HIGH"),
        ]
    ...
    async def sanitize_prompt_before_storage(
        ...
        if self.gdpr_enabled and (not user_consent):
            self.logger.warning(
                f"Data processing without consent for session {session_id}"
            )
        ...
        for pattern, pattern_type, confidence, risk_level in self.sensitive_patterns:
            ...
            if matches:
                ...
                redaction_details[pattern_type] = {
                    ...
                    "risk_level": risk_level,
                    "technique": privacy_techniques_used[-1]
                    if privacy_techniques_used
                    else "none",
                    "matches": [
                        {"start": m.start_position, "end": m.end_position}
        ...
        if self.enable_differential_privacy and detected_matches:
            sanitized = await self._apply_differential_privacy(
                sanitized, detected_matches
            )
        ...
        if processing_time > self._max_processing_time_ms:
            self.logger.warning(
                f"Slow data protection processing: {processing_time:.2f}ms for session {session_id}"
            )
        ...
        compliance_score = self._calculate_compliance_score(
            detected_matches, privacy_techniques_used
        )
    ...
    async def _apply_differential_privacy(
        self, text: str, matches: list[SensitiveDataMatch]
    ) -> str:
    ...
    def _calculate_compliance_score(
        self, matches: list[SensitiveDataMatch], techniques: list[str]
    ) -> float:
        ...
        return {
            ...
            "performance": {
                ...
                "performance_threshold_breaches": sum(
                    1
                    for t in self._processing_times
                    if t > self._max_processing_time_ms
                ),
            },
            "compliance": {
    ...
    async def get_performance_metrics(self) -> dict[str, Any]:
        ...
        performance_score = max(
            0.0, min(100.0, 100.0 - avg_time / self._max_processing_time_ms * 50.0)
        )
        return {
    ...
    async def process_batch(
        self, prompts: list[str], session_ids: list[str]
    ) -> list[dict[str, Any]]:
