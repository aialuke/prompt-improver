...
from prompt_improver.core.protocols.retry_protocols import RetryStrategy
...
class BackoffStrategyService:
    ...
    def _calculate_custom_delay(
        ...
        try:
            ...
            self.logger.error(f"Custom strategy '{strategy_name}' failed: {e}, using base_delay")
            return base_delay
    ...
    def register_custom_strategy(
        ...
        if not callable(delay_func):
            raise ValueError(f"delay_func must be callable")
        ...
        try:
            ...
            if not isinstance(test_result, (int, float)) or test_result < 0:
                raise ValueError(f"delay_func must return non-negative number")
        except Exception as e:
