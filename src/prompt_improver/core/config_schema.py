...
class ConfigMigration:
    ...
    def apply(self, config_data: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(
            f"Applying migration {self.from_version.value} -> {self.to_version.value}: {self.description}"
        )
...
class ConfigurationSchema(BaseModel):
    ...
    config_hash: str | None = SQLModelField(
        None, description="Hash of configuration content"
    )
    migration_history: list[str] = SQLModelField(
...
class UnifiedConfigManager:
    """Unified Configuration Manager - Complete Configuration and Feature Flag Management.

    Consolidates all configuration functionality:
    - Configuration schema management and migrations
    - Feature flag management with hot-reloading
    ...
    def __init__(self, feature_flags_config_path: str | Path | None = None):
        ...
        Args:
            feature_flags_config_path: Optional path to feature flags configuration
        """
        ...
        self._feature_flags_manager = None
    ...
    def _init_feature_flags_manager(self, config_path: str | Path):
        ...
        try:
            from prompt_improver.core.feature_flags import FeatureFlagManager

            self._feature_flags_manager = FeatureFlagManager(config_path)
        except ImportError:
            logger.warning(
                "FeatureFlagManager not available - feature flag functionality disabled"
            )
    ...
    def is_enabled(
        ...
        if self._feature_flags_manager:
            return self._feature_flags_manager.is_enabled(
                flag_key, user_id, context or {}
            )
        return False
    ...
    def get_flag_value(
        ...
        if self._feature_flags_manager:
            return self._feature_flags_manager.get_flag_value(
                flag_key, default, user_id, context or {}
            )
        return default
    ...
    def reload_configuration(self) -> bool:
        ...
        if self._feature_flags_manager:
            return self._feature_flags_manager.reload_configuration()
        return False
    ...
    def _detect_version_from_content(self, config_path: Path) -> ConfigSchemaVersion:
        ...
        try:
            ...
            if any(
                var in content
                for var in ["MONITORING_ENABLED", "FEATURE_FLAG_", "COMPLIANCE_"]
            ):
                return ConfigSchemaVersion.V2_0
            if any(
                var in content for var in ["METRICS_EXPORT_INTERVAL", "TRACING_ENABLED"]
            ):
    ...
    def migrate_to_latest(self, config_path: Path) -> bool:
        ...
        if current_version == latest_version:
            logger.info(
                f"Configuration {config_path} is already at latest version {latest_version.value}"
            )
            return True
        return self.migrate_to_version(config_path, latest_version)

    def migrate_to_version(
        self, config_path: Path, target_version: ConfigSchemaVersion
    ) -> bool:
        ...
        if current_version == target_version:
            logger.info(
                f"Configuration {config_path} is already at version {target_version.value}"
            )
            return True
        migration_path = self._find_migration_path(current_version, target_version)
        if not migration_path:
            logger.error(
                f"No migration path found from {current_version.value} to {target_version.value}"
            )
            return False
        try:
        ...
        try:
            ...
            logger.info(
                f"Successfully migrated {config_path} to version {target_version.value}"
            )
            return True
        except Exception as e:
    ...
    def _find_migration_path(
        self, from_version: ConfigSchemaVersion, to_version: ConfigSchemaVersion
...
ConfigSchemaManager = UnifiedConfigManager
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--migrate", action="store_true", help="Migrate to latest version"
    )
    ...
    parser.add_argument(
        "--show-version", action="store_true", help="Show current schema version"
    )
