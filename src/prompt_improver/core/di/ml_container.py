...
from prompt_improver.core.protocols.ml_protocols import CacheServiceProtocol, DatabaseServiceProtocol, EventBusProtocol, HealthMonitorProtocol, MLflowServiceProtocol, ResourceManagerProtocol, ServiceContainerProtocol, ServiceStatus, WorkflowEngineProtocol
T = TypeVar('T')
...
class MLServiceContainer:
    ...
    def register_service(self, service_name: str, service_instance: Any) -> None:
        ...
        self._logger.debug('Registered service: %s', service_name)

    def register_factory(self, service_name: str, factory: Callable[[], Any], singleton: bool=True) -> None:
        """Register a service factory function."""
        self._factories[service_name] = factory
        if singleton:
            self._singletons[service_name] = None
        self._logger.debug('Registered factory for service: %s (singleton=%s)', service_name, singleton)

    def register_mlflow_service_factory(self, factory: Callable[[], MLflowServiceProtocol]) -> None:
        """Register MLflow service factory."""
        self.register_factory('mlflow_service', factory, singleton=True)

    def register_cache_service_factory(self, factory: Callable[[], CacheServiceProtocol]) -> None:
        """Register cache service (Redis) factory."""
        self.register_factory('cache_service', factory, singleton=True)

    def register_database_service_factory(self, factory: Callable[[], DatabaseServiceProtocol]) -> None:
        """Register database service factory."""
        self.register_factory('database_service', factory, singleton=True)

    def register_event_bus_factory(self, factory: Callable[[], EventBusProtocol]) -> None:
        """Register event bus factory."""
        self.register_factory('event_bus', factory, singleton=True)

    def register_workflow_engine_factory(self, factory: Callable[[], WorkflowEngineProtocol]) -> None:
        """Register workflow engine factory."""
        self.register_factory('workflow_engine', factory, singleton=True)

    def register_resource_manager_factory(self, factory: Callable[[], ResourceManagerProtocol]) -> None:
        """Register resource manager factory."""
        self.register_factory('resource_manager', factory, singleton=True)

    def register_health_monitor_factory(self, factory: Callable[[], HealthMonitorProtocol]) -> None:
        """Register health monitor factory."""
        self.register_factory('health_monitor', factory, singleton=True)

    def register_ml_pipeline_factory(self, factory: Callable[[dict[str, Any]], Any]) -> None:
        """Register ML pipeline orchestrator factory."""
        self.register_factory('ml_pipeline_orchestrator', lambda: factory(self.get_all_services()), singleton=False)
    ...
    async def get_service(self, service_name: str) -> Any:
        ...
        if service_name in self._factories:
            ...
            try:
                ...
                self._logger.info('Created service via factory: %s', service_name)
                return service_instance
            except Exception as e:
                self._logger.error("Failed to create service '{service_name}': %s", e)
    ...
    def get_all_services(self) -> dict[str, Any]:
        ...
        all_services.update({k: v for k, v in self._singletons.items() if v is not None})
        return all_services
    ...
    async def initialize_all_services(self) -> None:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                service_name = list(self._factories.keys())[i] if i < len(self._factories) else list(self._services.keys())[i - len(self._factories)]
                failed_services.append((service_name, result))
                self._logger.error("Failed to initialize service '%s': %s", service_name, result)
        if failed_services:
            raise RuntimeError(f'Failed to initialize services: {[name for name, _ in failed_services]}')
    ...
    async def _initialize_service(self, service_name: str) -> None:
        ...
        try:
            ...
            self._logger.debug('Initialized service: %s', service_name)
        except Exception as e:
            self._logger.error("Service initialization failed for '%s': %s", service_name, e)
            raise
    ...
    async def shutdown_all_services(self) -> None:
        ...
        shutdown_tasks = []
        all_services = list(self._services.items()) + [(k, v) for k, v in self._singletons.items() if v is not None]
    ...
    async def _shutdown_service(self, service_name: str, service: Any) -> None:
        ...
        try:
            ...
            self._logger.debug('Shutdown service: %s', service_name)
        except Exception as e:
            self._logger.error("Service shutdown failed for '{service_name}': %s", e)
    ...
    async def health_check_all_services(self) -> dict[str, ServiceStatus]:
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                ...
                self._logger.error("Health check failed for '%s': %s", service_name, result)
            else:
                health_results[service_name] = result
        return health_results

    async def _check_service_health(self, service_name: str, health_check_func: Callable) -> ServiceStatus:
        ...
        try:
            ...
            self._logger.error("Health check error for '{service_name}': %s", e)
...
def register_all_ml_services(container: MLServiceContainer, config: dict[str, Any] | None=None) -> None:
    ...
    """
    config = config or {}
    from prompt_improver.core.factories.ml_pipeline_factory import create_ml_pipeline_orchestrator
    from prompt_improver.core.services.ml_service_adapters import create_cache_service, create_database_service, create_event_bus, create_health_monitor, create_mlflow_service, create_resource_manager, create_workflow_engine
    container.register_mlflow_service_factory(lambda: create_mlflow_service(config.get('mlflow', {})))
    container.register_cache_service_factory(lambda: create_cache_service(config.get('cache', {})))
    container.register_database_service_factory(lambda: create_database_service(config.get('database', {})))
    container.register_event_bus_factory(lambda: create_event_bus(config.get('event_bus', {})))
    container.register_workflow_engine_factory(lambda: create_workflow_engine(config.get('workflow', {})))
    container.register_resource_manager_factory(lambda: create_resource_manager(config.get('resources', {})))
    container.register_health_monitor_factory(lambda: create_health_monitor(config.get('health', {})))
    container.register_ml_pipeline_factory(lambda services: create_ml_pipeline_orchestrator(services, config.get('orchestrator', {})))
