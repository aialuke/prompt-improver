...
class BaseConfigModel(SQLModel):
...
class BaseService(ConfigMixin, MetricsMixin, ABC):
    ...
    async def restart(self) -> bool:
        """Restart the service."""
        self.logger.info('Restarting service: %s', self.name)
        if not await self.stop():
            self.logger.error('Failed to stop service: %s', self.name)
            return False
        if not await self.start():
            self.logger.error('Failed to start service: %s', self.name)
            return False
        self.logger.info('Service restarted successfully: %s', self.name)
        return True
    ...
    def record_error(self, error: str) -> None:
        ...
        self.error_count += 1
        self.logger.error('Service {self.name} error: %s', error)
        self.increment_counter('service_errors_total', {'service': self.name, 'error': error})

    def get_status(self) -> dict[str, Any]:
        """Get service status information."""
        uptime = self.get_uptime()
        return {'name': self.name, 'state': self.state.value, 'uptime_seconds': uptime.total_seconds() if uptime is not None else None, 'error_count': self.error_count, 'last_error': self.last_error, 'config_status': self.config_status.success if hasattr(self, 'config_status') else None, 'metrics_available': self.metrics_available}
...
class BaseHealthChecker(MetricsMixin, ABC):
    ...
    async def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            self.consecutive_failures += 1
            result = HealthCheckResult(status=HealthStatus.UNHEALTHY, message=f'Health check timed out after {self.timeout}s', details={'timeout': True, 'consecutive_failures': self.consecutive_failures}, timestamp=start_time, duration_ms=duration_ms, critical=self.critical)
            self.last_result = result
            self._record_health_metrics(result)
            return result
            ...
            duration_ms = (datetime.now(UTC) - start_time).total_seconds() * 1000
            self.consecutive_failures += 1
            result = HealthCheckResult(status=HealthStatus.UNHEALTHY, message=f'Health check failed: {e!s}', details={'error': str(e), 'consecutive_failures': self.consecutive_failures}, timestamp=start_time, duration_ms=duration_ms, critical=self.critical)
            self.last_result = result
            self.logger.error('Health check {self.name} failed: %s', e)
            self._record_health_metrics(result)
            return result

    def _record_health_metrics(self, result: HealthCheckResult) -> None:
        """Record health check metrics."""
        self.observe_histogram('health_check_duration_seconds', result.duration_ms / 1000, {'check': self.name, 'status': result.status.value})
        self.increment_counter('health_checks_total', {'check': self.name, 'status': result.status.value, 'critical': str(result.critical)})
        if self.consecutive_failures > 0:
            self.set_gauge('health_check_consecutive_failures', self.consecutive_failures, {'check': self.name})
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {'name': self.name, 'timeout': self.timeout, 'critical': self.critical, 'consecutive_failures': self.consecutive_failures, 'max_failures': self.max_failures, 'is_degraded': self.is_degraded(), 'last_result': {'status': self.last_result.status.value, 'message': self.last_result.message, 'timestamp': self.last_result.timestamp.isoformat(), 'duration_ms': self.last_result.duration_ms} if self.last_result else None}
...
class BaseMonitor(BaseService):
    ...
    def add_health_checker(self, checker: BaseHealthChecker) -> None:
        ...
        self.logger.info('Added health checker: %s', checker.name)
    ...
    def remove_health_checker(self, name: str) -> None:
        ...
        if name in self.health_checkers:
            ...
            self.logger.info('Removed health checker: %s', name)

    async def start(self) -> bool:
        """Start monitoring."""
        if self.monitoring_task and (not self.monitoring_task.done()):
            self.logger.warning('Monitor %s already running', self.name)
            return True
        self.state = ServiceState.HEALTHY
        self.start_time = datetime.now(UTC)
        self.monitoring_enabled = True
        task_manager = get_background_task_manager()
        self.monitoring_task_id = await task_manager.submit_enhanced_task(task_id=f'monitor_{self.name}_{str(uuid.uuid4())[:8]}', coroutine=self._monitoring_loop(), priority=TaskPriority.HIGH, tags={'service': 'monitoring', 'type': 'health_monitoring', 'component': self.name, 'operation': 'continuous_monitoring'})
        self.logger.info('Started monitor: %s', self.name)
        return True
    ...
    async def stop(self) -> bool:
        ...
        self.logger.info('Stopped monitor: %s', self.name)
        return True

    async def health_check(self) -> HealthCheckResult:
        """Perform health check for this monitor."""
        if not self.monitoring_enabled:
            return HealthCheckResult(status=HealthStatus.UNHEALTHY, message='Monitoring is disabled', details={'enabled': False}, timestamp=datetime.now(UTC), duration_ms=0, critical=True)
        if not self.monitoring_task_id:
            return HealthCheckResult(status=HealthStatus.UNHEALTHY, message='Monitoring task is not running', details={'task_running': False}, timestamp=datetime.now(UTC), duration_ms=0, critical=True)
        task_manager = get_background_task_manager()
        ...
        if not task_status or task_status.get('status') not in ['running', 'pending']:
            return HealthCheckResult(status=HealthStatus.UNHEALTHY, message='Monitoring task is not running', details={'task_running': False, 'task_status': task_status}, timestamp=datetime.now(UTC), duration_ms=0, critical=True)
        uptime = self.get_uptime()
        return HealthCheckResult(status=HealthStatus.HEALTHY, message='Monitor is running normally', details={'checkers_count': len(self.health_checkers), 'uptime_seconds': uptime.total_seconds() if uptime is not None else 0}, timestamp=datetime.now(UTC), duration_ms=0, critical=False)
    ...
    async def get_all_health_results(self) -> dict[str, HealthCheckResult]:
        ...
        for name, checker in self.health_checkers.items():
            try:
                ...
                self.logger.error('Failed to check {name}: %s', e)
                results[name] = HealthCheckResult(status=HealthStatus.UNHEALTHY, message=f'Check failed: {e}', details={'error': str(e)}, timestamp=datetime.now(UTC), duration_ms=0, critical=checker.critical)
        return results
