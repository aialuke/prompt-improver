...
def _create_fallback_config():
    ...
    directory_paths = SimpleNamespace(
...
class ConfigMixin:
    ...
    def validate_config_requirements(self, required_paths: list) -> bool:
        """
...
def load_config_section(section_name: str, config_class: Type[T]) -> Optional[T]:
    ...
    try:
        ...
        if hasattr(section_data, '__dict__'):
            section_dict = vars(section_data)
...
def validate_timeout(value: Union[int, float], name: str = "timeout") -> Union[int, float]:
