...
from prompt_improver.core.protocols.retry_protocols import CircuitBreakerProtocol, MetricsRegistryProtocol, RetryableErrorType, RetryConfigProtocol, RetryManagerProtocol, RetryObserverProtocol, RetryStrategy
...
# TypedDicts for retry options (C2)
class RetryConfigOverrides(TypedDict, total=False):
    max_attempts: int
...
class RetryDecoratorOptions(TypedDict, total=False):
    enable_circuit_breaker: bool
    operation_name: str
    max_attempts: int
    strategy: RetryStrategy
...
class RetryConfig:
    ...
    retryable_errors: list[RetryableErrorType] = field(default_factory=lambda: [RetryableErrorType.TRANSIENT, RetryableErrorType.NETWORK, RetryableErrorType.TIMEOUT, RetryableErrorType.RATE_LIMIT])
    operation_name: str | None = None
    enable_metrics: bool = True
    enable_tracing: bool = True
    custom_delay_func: Callable[[int], float] | None = None
    error_classifier: Callable[[Exception], RetryableErrorType] | None = None
...
class CircuitBreaker:
    ...
    def __init__(self, config: RetryConfig, operation_name: str):
        ...
        self.state_gauge = self.metrics_registry.get_or_create_gauge(StandardMetrics.CIRCUIT_BREAKER_STATE, 'Circuit breaker state (0=closed, 1=half_open, 2=open)', ['operation'])
        self.events_counter = self.metrics_registry.get_or_create_counter(StandardMetrics.CIRCUIT_BREAKER_EVENTS_TOTAL, 'Circuit breaker events', ['operation', 'event'])
    ...
    async def _check_state_transition(self):
        ...
        async with self._lock:
            if self.state_data.state == CircuitBreakerState.OPEN and self.state_data.last_failure_time and (datetime.now(UTC) - self.state_data.last_failure_time > timedelta(seconds=self.config.recovery_timeout_seconds)):
                self.state_data.state = CircuitBreakerState.HALF_OPEN
                self.state_data.success_count = 0
                self.state_data.half_open_attempts = 0
                logger.info('Circuit breaker transitioning to HALF_OPEN for %s', self.operation_name)
    ...
    async def _on_success(self, context: RetryContext):
        ...
        async with self._lock:
            self.state_data.last_success_time = datetime.now(UTC)
            if self.state_data.state == CircuitBreakerState.HALF_OPEN:
                ...
                if self.state_data.success_count >= self.config.success_threshold:
                    ...
                    logger.info('Circuit breaker transitioning to CLOSED for %s', self.operation_name)
    ...
    async def _on_failure(self, context: RetryContext, error: Exception):
        ...
        async with self._lock:
            ...
            self.state_data.last_failure_time = datetime.now(UTC)
            if self.state_data.state == CircuitBreakerState.CLOSED and self.state_data.failure_count >= self.config.failure_threshold:
                self.state_data.state = CircuitBreakerState.OPEN
                logger.warning('Circuit breaker transitioning to OPEN for %s', self.operation_name)
                self._record_event('opened')
            elif self.state_data.state == CircuitBreakerState.HALF_OPEN:
                self.state_data.state = CircuitBreakerState.OPEN
                logger.warning('Circuit breaker returning to OPEN from HALF_OPEN for %s', self.operation_name)
                self._record_event('opened')
            self._update_state_metric()

    def _update_state_metric(self):
        """Update state metric."""
        state_value = {CircuitBreakerState.CLOSED: 0, CircuitBreakerState.HALF_OPEN: 1, CircuitBreakerState.OPEN: 2}[self.state_data.state]
    ...
    def reset(self) -> None:
        ...
        if task_manager:
            ...
            asyncio.create_task(task_manager.submit_enhanced_task(task_id=f'circuit_breaker_reset_{self.operation_name}_{str(uuid.uuid4())[:8]}', coroutine=self._reset_async(), priority=TaskPriority.HIGH, tags={'service': 'retry_manager', 'type': 'circuit_breaker', 'component': 'reset', 'operation': self.operation_name}))
    ...
    async def _reset_async(self):
        ...
        async with self._lock:
            ...
            logger.info('Circuit breaker reset for %s', self.operation_name)
...
class RetryManager:
    ...
    def _setup_metrics(self):
        ...
        self.metrics_registry = get_metrics_registry()
        self.retry_attempts_total = self.metrics_registry.get_or_create_counter(StandardMetrics.RETRY_ATTEMPTS_TOTAL, 'Total retry attempts', ['operation', 'strategy', 'attempt'])
        self.retry_success_total = self.metrics_registry.get_or_create_counter(StandardMetrics.RETRY_SUCCESS_TOTAL, 'Total successful retries', ['operation', 'strategy'])
        self.retry_failure_total = self.metrics_registry.get_or_create_counter(StandardMetrics.RETRY_FAILURE_TOTAL, 'Total failed retries', ['operation', 'strategy', 'error_type'])
        self.retry_duration_histogram = self.metrics_registry.get_or_create_histogram(StandardMetrics.RETRY_DURATION, 'Retry operation duration', ['operation', 'strategy'], buckets=[0.1, 0.5, 1.0, 2.5, 5.0, 10.0, 30.0, 60.0])
    ...
    async def execute_with_retry(self, operation: Callable[..., Any], config: RetryConfigProtocol, *args: Any, **kwargs: Any) -> Any:
    ...
    async def execute_with_circuit_breaker(self, operation: Callable[..., Any], config: RetryConfigProtocol, *args: Any, **kwargs: Any) -> Any:
    ...
    def get_retry_stats(self, operation_name: str) -> dict[str, Any]:
        ...
        result = {'operation_name': operation_name, 'total_attempts': stats.get('total_attempts', 0), 'successful_attempts': stats.get('successful_attempts', 0), 'failed_attempts': stats.get('failed_attempts', 0), 'success_rate': stats.get('success_rate', 0.0), 'average_delay_ms': stats.get('average_delay_ms', 0.0), 'last_attempt_at': stats.get('last_attempt_at')}
        if circuit_breaker:
            result['circuit_breaker'] = {'state': circuit_breaker.get_state(), 'failure_count': circuit_breaker.state_data.failure_count, 'success_count': circuit_breaker.state_data.success_count, 'is_open': circuit_breaker.is_open()}
        return result

    async def retry_async(self, operation: Callable[..., Coroutine[Any, Any, T]], config: RetryConfig | None=None, operation_name: str | None=None, **kwargs: Any) -> T:
    ...
    async def retry_sync(self, operation: Callable[..., T], config: RetryConfig | None=None, operation_name: str | None=None, **kwargs: Any) -> T:
    ...
    async def retry_database_operation(self, operation: Callable[..., Any], operation_name: str='database_operation', **kwargs: Any) -> Any:
        ...
        Returns:
            Result of successful operation execution
        """
        db_config = RetryConfig(max_attempts=5, strategy=RetryStrategy.EXPONENTIAL_BACKOFF, base_delay=0.5, max_delay=30.0, operation_name=operation_name, retry_on_exceptions=[ConnectionError, TimeoutError, OSError], failure_threshold=3, recovery_timeout_seconds=30.0)
        if asyncio.iscoroutinefunction(operation):
    ...
    async def retry_ml_operation(self, operation: Callable[..., Any], operation_name: str='ml_operation', **kwargs: Any) -> Any:
        ...
        Returns:
            Result of successful operation execution
        """
        ml_config = RetryConfig(max_attempts=3, strategy=RetryStrategy.EXPONENTIAL_BACKOFF, base_delay=1.0, max_delay=60.0, operation_name=operation_name, retry_on_exceptions=[ConnectionError, TimeoutError, RuntimeError], failure_threshold=2, recovery_timeout_seconds=120.0, resource_aware=True, adaptive_delays=True)
        if asyncio.iscoroutinefunction(operation):
    ...
    @asynccontextmanager
    async def with_retry(self, operation_name: str, config: RetryConfig | None=None):
    ...
    def configure_strategy(self, strategy: RetryStrategy, max_attempts: int=3, base_delay: float=1.0, max_delay: float=60.0, **options) -> None:
        ...
        self.logger.info('Configured retry strategy: %s', strategy.value)
    ...
    async def get_circuit_breaker_status(self, operation_name: str) -> dict[str, Any]:
        ...
        return {'state': circuit_breaker.get_state(), 'failure_count': circuit_breaker.state_data.failure_count, 'success_count': circuit_breaker.state_data.success_count, 'last_failure_time': circuit_breaker.state_data.last_failure_time, 'last_success_time': circuit_breaker.state_data.last_success_time, 'is_open': circuit_breaker.is_open()}
    ...
    async def reset_circuit_breaker(self, operation_name: str):
        ...
        if circuit_breaker:
            ...
            self.logger.info('Circuit breaker reset for %s', operation_name)
    ...
    def reset_stats(self, operation_name: str | None=None) -> None:
        ...
        if operation_name:
            if operation_name in self._stats:
                ...
                self.logger.info('Reset stats for operation: %s', operation_name)
        else:
            self._stats.clear()
    ...
    async def get_comprehensive_metrics(self, operation_name: str) -> dict[str, Any]:
        ...
        return {'operation_name': operation_name, 'retry_stats': stats, 'circuit_breaker': circuit_status, 'health_status': 'healthy' if stats.get('success_rate', 0.0) > 0.8 else 'degraded', 'recommendations': self._generate_recommendations(stats)}
    ...
    async def _execute_async_with_retry(self, operation: Callable[..., Coroutine[Any, Any, T]], config: RetryConfig, context: RetryContext, **kwargs: Any) -> T:
            ...
            for attempt in range(config.max_attempts):
                ...
                try:
                    ...
                    for observer in self._observers:
                        observer.on_retry_attempt(context.operation_name, attempt + 1, 0.0, last_exception or Exception('Starting attempt'))
                    ...
                    self.logger.debug('Retry %s/%s for %s in %ss due to %s: %s', attempt + 1, config.max_attempts, context.operation_name, format(delay, '.2f'), error_type.value, e)
    ...
    async def _execute_sync_with_retry(self, operation: Callable[..., T], config: RetryConfig, context: RetryContext, **kwargs) -> T:
    ...
    def _convert_config(self, config: RetryConfigProtocol) -> RetryConfig:
        ...
        return RetryConfig(max_attempts=config.max_attempts, strategy=config.strategy, base_delay=config.base_delay, max_delay=config.max_delay, jitter=config.jitter, jitter_factor=config.jitter_factor, backoff_multiplier=config.backoff_multiplier, retry_on_exceptions=config.retry_on_exceptions, retry_condition=config.retry_condition, operation_timeout=config.operation_timeout, total_timeout=config.total_timeout, log_attempts=config.log_attempts, log_level=config.log_level, track_metrics=config.track_metrics)
    ...
    def _classify_error(self, error: Exception, config: RetryConfig) -> RetryableErrorType:
        ...
        if any((term in error_str for term in ['connection', 'network', 'dns', 'socket'])) or isinstance(error, (ConnectionError, OSError)):
            return RetryableErrorType.NETWORK
        if any((term in error_str for term in ['timeout', 'timed out'])) or isinstance(error, (TimeoutError, asyncio.TimeoutError)):
    ...
    async def _record_success(self, operation_name: str, attempt: int, duration_ms: float, config: RetryConfig):
        ...
        async with self._stats_lock:
            if operation_name not in self._stats:
                self._stats[operation_name] = {'total_attempts': 0, 'successful_attempts': 0, 'failed_attempts': 0, 'total_duration_ms': 0.0, 'success_rate': 0.0, 'average_delay_ms': 0.0, 'last_attempt_at': None}
            stats = self._stats[operation_name]
            stats['total_attempts'] += attempt
            stats['successful_attempts'] += 1
            stats['total_duration_ms'] += duration_ms
            stats['last_attempt_at'] = datetime.now(UTC).isoformat()
        ...
        self.retry_success_total.labels(operation=operation_name, strategy=config.strategy.value).inc()
        self.retry_duration_histogram.labels(operation=operation_name, strategy=config.strategy.value).observe(duration_ms / 1000.0)
    ...
    async def _record_failure(self, operation_name: str, attempt: int, error_message: str, config: RetryConfig):
        ...
        async with self._stats_lock:
            ...
            if total_ops > 0:
                stats['success_rate'] = stats['successful_attempts'] / total_ops
        self.retry_failure_total.labels(operation=operation_name, strategy=config.strategy.value, error_type='unknown').inc()
...
class RetryExecutor:
    ...
    def __init__(self, retry_manager: RetryManager, config: RetryConfig, context: RetryContext, circuit_breaker: CircuitBreaker):
    ...
    async def execute(self, operation: Callable[[], Coroutine[Any, Any, T]]) -> T:
        ...
        return await self.retry_manager._execute_async_with_retry(operation, self.config, self.context)
...
async def retry_database_operation(operation: Callable[..., Any], operation_name: str='database_operation', **kwargs: Any) -> Any:
    ...
    Returns:
        Result of successful operation execution
    """
    return await get_retry_manager().retry_database_operation(operation, operation_name, **kwargs)

async def retry_ml_operation(operation: Callable[..., Any], operation_name: str='ml_operation', **kwargs: Any) -> Any:
    ...
    Returns:
        Result of successful operation execution
    """
    return await get_retry_manager().retry_ml_operation(operation, operation_name, **kwargs)

def retry(max_attempts: int=3, strategy: RetryStrategy=RetryStrategy.EXPONENTIAL_BACKOFF, base_delay: float=1.0, max_delay: float=60.0, enable_circuit_breaker: bool=True, operation_name: str | None=None, **config_kwargs: RetryDecoratorOptions):
    ...
    def decorator(func: Callable[..., Coroutine[Any, Any, T]]) -> Callable[..., Coroutine[Any, Any, T]]:
        ...
        async def wrapper(*args: Any, **func_kwargs: Any) -> T:
            config = RetryConfig(max_attempts=max_attempts, strategy=strategy, base_delay=base_delay, max_delay=max_delay, enable_circuit_breaker=enable_circuit_breaker, operation_name=operation_name or func.__name__, **config_kwargs)
            retry_manager = get_retry_manager()
            return await retry_manager.retry_async(lambda: func(*args, **func_kwargs), config=config)
        return wrapper
    return decorator
__all__ = ['RetryManager', 'RetryConfig', 'RetryContext', 'CircuitBreaker', 'CircuitBreakerState', 'CircuitBreakerOpenError', 'get_retry_manager', 'set_retry_manager', 'retry_database_operation', 'retry_ml_operation', 'retry', 'RetryStrategy', 'RetryableErrorType']
