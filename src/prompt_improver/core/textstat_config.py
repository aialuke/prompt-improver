...
class TextStatConfig(BaseModel):
    ...
    language: str = Field(default="en_US", description="Language for CMUdict optimization")
    enable_caching: bool = Field(default=True, description="Enable LRU caching for repeated texts")
    cache_size: int = Field(default=1024, ge=1, le=10000, description="Cache size for text analysis")
    suppress_warnings: bool = Field(default=True, description="Suppress pkg_resources warnings")
...
class TextStatWrapper:
    ...
    def __init__(self, config: Optional[TextStatConfig] = None):  # type: ignore[override]
    ...
    def _safe_textstat_call(
        self, 
        operation_name: str, 
        func: Callable[[str], Union[float, int]], 
        text: str, 
        default_value: Union[float, int] = 0
    ) -> Union[float, int]:
        ...
        try:
            ...
            logger.warning(f"TextStat {operation_name} failed for text length {len(text)}: {e}")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_stats': {
                'flesch_reading_ease': getattr(self.flesch_reading_ease, 'cache_info', lambda: {'hits': 0, 'misses': 0})(),
                'syllable_count': getattr(self.syllable_count, 'cache_info', lambda: {'hits': 0, 'misses': 0})(),
            }
        }
...
_global_wrapper: Optional[TextStatWrapper] = None


def get_textstat_wrapper(config: Optional[TextStatConfig] = None) -> TextStatWrapper:
...
def text_analysis(text: str, config: Optional[TextStatConfig] = None) -> Dict[str, Any]:
