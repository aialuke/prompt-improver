...
from prompt_improver.core.config.validation import validate_configuration, ValidationReport
...
class ConfigurationError(Exception):
    ...
    def __init__(self, message: str, config_section: Optional[str] = None, details: Optional[Dict[str, Any]] = None):
...
class ConfigurationFactory:
    ...
    def create_for_testing(self, overrides: Optional[Dict[str, Any]] = None) -> AppConfig:
    ...
    def reload_config(self, environment: Optional[str] = None) -> AppConfig:
        ...
        keys_to_remove = [key for key in self._cached_configs.keys() if key.startswith(env)]
...
def create_test_config(overrides: Optional[Dict[str, Any]] = None) -> AppConfig:
...
async def validate_config_file(config_path: str) -> ValidationReport:
    ...
    try:
        ...
        report.critical_failures = [f"config_file: Failed to load configuration file: {str(e)}"]
