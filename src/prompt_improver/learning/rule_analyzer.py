...
class TimeSeriesValidationResult:
    ...
    trend_coefficient: Optional[float]
...
class BayesianModelResult:
    ...
    model_comparison_score: Optional[float]
    hierarchical_effects: Optional[Dict[str, float]]


class RuleEffectivenessAnalyzer:
    """Analyzer for rule effectiveness and optimization"""
    
    def __init__(self, config: Optional[RuleAnalysisConfig] = None):
                ...
                if 'observations' in rule_data:
                    ...
                    for obs in observations:
                        ...
                        adapted_data.append({
                            ...
                            'context': {'projectType': obs.get('context', 'unknown'), 'domain': 'test'},
    ...
    async def analyze_rule_effectiveness(self, test_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Analyzing rule effectiveness for {len(adapted_test_results)} results")
        ...
        recommendations = await self._generate_rule_recommendations(
            rule_metrics, combination_metrics, ts_validation_results, bayesian_results, advanced_time_series_results
        )
        
        result = {
            "rule_metrics": {rule_id: metrics.__dict__ for rule_id, metrics in rule_metrics.items()},
            "individual_rules": {rule_id: metrics.__dict__ for rule_id, metrics in rule_metrics.items()},
            "rule_combinations": [combo.__dict__ for combo in combination_metrics],
            "time_series_validation": {rule_id: result.__dict__ for rule_id, result in ts_validation_results.items()},
            "advanced_time_series_analysis": advanced_time_series_results,
            "recommendations": recommendations,
            "summary": self._generate_analysis_summary(rule_metrics, combination_metrics, ts_validation_results, bayesian_results, advanced_time_series_results),
            "metadata": {
                "total_rules_analyzed": len(rule_metrics),
                "total_combinations_analyzed": len(combination_metrics),
                "time_series_validation_enabled": self.config.enable_time_series_cv and TIME_SERIES_AVAILABLE,
                "bayesian_modeling_enabled": self.config.enable_bayesian_modeling and BAYESIAN_AVAILABLE,
                "advanced_time_series_analysis_enabled": self.config.enable_time_series_cv and TIME_SERIES_AVAILABLE,
        ...
        for result in test_results:
            ...
            for rule in applied_rules:
                ...
                rule_data[rule_id].append({
                    ...
                    "overall_score": result.get("overallImprovement", 0) or result.get("improvementScore", 0),
                    "other_rules": [r.get("ruleId", r.get("id", "unknown")) for r in applied_rules if r != rule]
                })
    ...
    async def _analyze_rule_combinations(self, test_results: List[Dict[str, Any]]) -> List[RuleCombinationMetrics]:
        ...
        for result in test_results:
            ...
            if len(applied_rules) > 1:
                ...
                combination_data[combo_key].append({
                    ...
                    "context": result.get("context", {}),
                    "individual_scores": [r.get("improvementScore", 0) for r in applied_rules]
                })
        ...
        for combo_key, data_points in combination_data.items():
            ...
            combination_metrics.append(RuleCombinationMetrics(
    ...
    async def _generate_rule_recommendations(
        ...
        ts_validation_results: Dict[str, TimeSeriesValidationResult] = None,
        bayesian_results: Dict[str, BayesianModelResult] = None,
        advanced_time_series_results: Dict[str, Any] = None
    ) -> List[Dict[str, Any]]:
        ...
        for rule_id, metrics in rule_metrics.items():
            ...
            if (metrics.avg_improvement > 0.7 and 
                metrics.consistency_score > self.config.consistency_threshold and
                metrics.total_applications >= 20):
            ...
            elif (metrics.avg_improvement < 0.4 and 
                  metrics.total_applications >= 20):
            ...
            # Inconsistent rules
            elif (metrics.consistency_score < 0.5 and 
    ...
    def _generate_analysis_summary(
        ...
        summary = {
            ...
            "top_performing_rules": [
                {"rule_id": rule_id, "avg_improvement": metrics.avg_improvement, "consistency": metrics.consistency_score}
                for rule_id, metrics in top_rules
            ],
            "underperforming_rules": [
                {"rule_id": rule_id, "avg_improvement": metrics.avg_improvement, "applications": metrics.total_applications}
                for rule_id, metrics in bottom_rules
            ],
            "best_combinations": [
                {"rules": combo.rule_ids, "avg_improvement": combo.avg_improvement, "synergy": combo.synergy_score}
                for combo in best_combinations
            ],
            "rules_needing_attention": len([m for m in rule_metrics.values() if m.avg_improvement < 0.5 or m.consistency_score < 0.6])
        }
        ...
        if ts_validation_results:
            ...
            declining_trends = [result for result in ts_validation_results.values() 
                              if result.trend_coefficient and result.trend_coefficient < -0.01]
            rules_with_changes = [result for result in ts_validation_results.values() 
            ...
            summary["time_series_analysis"] = {
                ...
                "most_stable_rule": max(ts_validation_results.items(), key=lambda x: x[1].temporal_stability)[0] if ts_validation_results else None,
                "least_stable_rule": min(ts_validation_results.items(), key=lambda x: x[1].temporal_stability)[0] if ts_validation_results else None
            }
        ...
        if bayesian_results:
            ...
            summary["bayesian_analysis"] = {
                ...
                "most_certain_rule": min(bayesian_results.items(), key=lambda x: x[1].posterior_std)[0] if bayesian_results else None,
                "most_uncertain_rule": max(bayesian_results.items(), key=lambda x: x[1].posterior_std)[0] if bayesian_results else None
            }
    ...
    def _extract_time_series(self, data_points: List[Dict[str, Any]]) -> Tuple[List[datetime], List[float]]:
        ...
        return list(zip(*sorted_data)) if sorted_data else ([], [])
    ...
    async def _seasonal_decomposition(self, timestamps: List[datetime], scores: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(df) >= 10:
                ...
                return {
                    "trend_strength": float(np.var(decomposition.trend.dropna()) / np.var(df['score'])),
                    "seasonal_strength": float(np.var(decomposition.seasonal) / np.var(df['score'])),
    ...
    async def _detect_change_points_async(self, timestamps: List[datetime], scores: List[float]) -> Dict[str, Any]:
        ...
        try:
                ...
                if len(before_window) > 2 and len(after_window) > 2:
                    ...
                    if p_value < 0.05:  # Significant change detected
                        change_points.append({
                            ...
                            "magnitude": float(abs(np.mean(after_window) - np.mean(before_window))),
            ...
            return {
                ...
                "stability_score": 1.0 - min(1.0, len(change_points) / max(1, len(scores) // 10))
            }
    ...
    async def _analyze_trends(self, timestamps: List[datetime], scores: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "performance_change_pct": float(performance_change / historical_avg * 100) if historical_avg > 0 else 0.0
            }
    ...
    async def _forecast_performance(self, timestamps: List[datetime], scores: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(scores) >= 20:
                ...
                mape = np.mean([abs(error/actual) for error, actual in zip(forecast_errors, validation_data) if actual != 0]) * 100
    ...
    async def _assess_temporal_stability(self, timestamps: List[datetime], scores: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "stability_classification": "stable" if overall_stability > 0.7 else "moderate" if overall_stability > 0.4 else "unstable",
                "rolling_statistics": {
    ...
    def _calculate_performance_trend(self, data_points: List[Dict[str, Any]]) -> Optional[str]:
    ...
    async def _perform_time_series_validation(self, rule_data: Dict[str, List[Dict[str, Any]]]) -> Dict[str, TimeSeriesValidationResult]:
        ...
        try:
            ...
            self.logger.info(f"Completed time series validation for {len(results)} rules")
    ...
    def _prepare_time_series_data(self, data_points: List[Dict[str, Any]]) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df[f'score_rolling_{self.config.rolling_window_size}'] = df['score'].rolling(
                window=self.config.rolling_window_size, min_periods=1
            ).mean()
    ...
    def _time_series_cross_validate(self, ts_data: pd.DataFrame, rule_id: str) -> Optional[TimeSeriesValidationResult]:
        """Perform time series cross-validation"""
        try:
            ...
            tscv = TimeSeriesSplit(
                ...
                test_size=max(5, len(ts_data) // (self.config.time_series_cv_splits + 1))
            )
            ...
            self.logger.warning(f"Time series cross-validation failed for {rule_id}: {e}")
            return None
    
    def _calculate_trend_coefficient(self, ts_data: pd.DataFrame) -> Optional[float]:
    ...
    def _detect_seasonal_patterns(self, ts_data: pd.DataFrame) -> Optional[Dict[str, float]]:
    ...
    def _perform_bayesian_modeling(self, rule_data_or_rule_id=None, observations=None):
        ...
        try:
            ...
            elif isinstance(rule_data_or_rule_id, dict):
    ...
    def _fit_bayesian_model(self, data_points: List[Dict[str, Any]], rule_id: str) -> Optional[BayesianModelResult]:
        ...
        try:
            ...
            with pm.Model() as model:
                ...
                var_term_safe = pm.math.clip(var_term, 0.1, 1000)  # Ensure positive and reasonable
            ...
            try:
                ...
                try:
                    ...
                    effective_sample_size = float(self.config.bayesian_samples * 0.8)  # Conservative estimate
