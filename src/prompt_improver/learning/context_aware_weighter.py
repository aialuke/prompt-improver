...
try:
    from ..analysis.domain_detector import PromptDomain, DomainClassificationResult
...
class ContextAwareFeatureWeighter:
    ...
    def calculate_feature_weights(self, 
        ...
        self.logger.debug(f"Calculated weights for {domain_features.domain.value}: "
    ...
    def _get_base_weights(self, domain_features: DomainFeatures) -> Dict[str, float]:
        ...
        primary_profile = self._domain_weight_profiles.get(
            ...
            self._domain_weight_profiles[PromptDomain.GENERAL]
        )
        
        # Handle hybrid domains by blending profiles
        if (domain_features.hybrid_domain and 
            domain_features.secondary_domains and
            len(domain_features.secondary_domains) > 0):
    ...
    def _blend_domain_profiles(self, 
        ...
        for secondary_domain, secondary_confidence in secondary_domains:
            ...
            blend_factor = (secondary_confidence * 
                          self.config.secondary_domain_weight_factor)
    ...
    def _apply_static_weights(self, 
    ...
    def _apply_adaptive_weights(self, 
        ...
        confidence_boost = (domain_features.confidence * 
    ...
    def _apply_dynamic_weights(self, 
                             base_weights: Dict[str, float],
                             domain_features: DomainFeatures,
                             feature_names: Tuple[str]) -> np.ndarray:
    ...
    def _apply_hybrid_weights(self, 
                            base_weights: Dict[str, float],
                            domain_features: DomainFeatures,
                            feature_names: Tuple[str]) -> np.ndarray:
        ...
        weights = (static_weights * (1 - blend_factor) + 
                  adaptive_weights * blend_factor)
    ...
    def _categorize_feature(self, feature_name: str) -> str:
        ...
        if any(prefix in feature_name_lower for prefix in ['tech_', 'technical_', 'code_', 'api_', 'algorithm']):
            return 'technical'
        elif any(prefix in feature_name_lower for prefix in ['creative_', 'narrative_', 'emotional_', 'sensory_']):
            return 'creative'
        elif any(prefix in feature_name_lower for prefix in ['academic_', 'research_', 'citation_', 'formal_']):
            return 'academic'
        elif any(prefix in feature_name_lower for prefix in ['conv_', 'conversational_', 'question_', 'polite_']):
    ...
    def _post_process_weights(self, weights: np.ndarray) -> np.ndarray:
        ...
        weights = np.clip(weights, 
                         self.config.min_weight_threshold,
                         self.config.max_weight_threshold)
