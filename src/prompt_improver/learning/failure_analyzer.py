...
try:
    ...
    warnings.warn("Anomaly detection libraries not available. Install with: pip install scikit-learn")
...
try:
    ...
    ART_AVAILABLE = False
    warnings.warn("Adversarial Robustness Toolbox not available. Install with: pip install adversarial-robustness-toolbox")
...
class FailureModeAnalyzer:
    ...
    async def analyze_failures(self, test_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        self.logger.info(f"Starting failure analysis with {len(test_results)} test results")
        
        # Filter failures and successes
        failures = [
            result for result in test_results
            if (result.get("overallImprovement", 0) or result.get("improvementScore", 0)) < self.config.failure_threshold
        ]
        ...
        self.logger.info(f"Identified {len(failures)} failures and {len(successes)} successes")
        ...
        failure_analysis = {
            ...
            "systematic_issues": await self._find_systematic_issues(failures, test_results),
            
            # ML FMEA Analysis
            "ml_fmea": await self._perform_ml_fmea_analysis(failures, test_results),
            "anomaly_detection": await self._perform_ensemble_anomaly_detection(failures),
            "risk_assessment": await self._calculate_risk_priority_numbers(failures),
            
            # Comparative analysis
            "failure_vs_success": self._compare_failures_with_successes(failures, successes),
        ...
        if self.config.enable_robustness_validation:
            failure_analysis["robustness_validation"] = await self._perform_robustness_validation(failures, test_results)
        
        # Phase 3 enhancements: Update monitoring metrics
        if self.config.enable_prometheus_monitoring:
            await self._update_prometheus_metrics(failure_analysis)
            failure_analysis["prometheus_alerts"] = await self._check_and_trigger_alerts(failure_analysis)
        
        return failure_analysis
    
    def _generate_no_failure_report(self, test_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _generate_failure_summary(self, failures: List[Dict[str, Any]], all_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "avg_failure_score": float(np.mean(failure_scores)) if failure_scores else 0.0,
            "worst_failure_score": float(np.min(failure_scores)) if failure_scores else 0.0,
        ...
        summary["worst_performing_rules"] = [
            ...
            for rule_id, stats in worst_rules
        ]
        
        return summary
    
    async def _identify_failure_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _find_context_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _find_prompt_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Find prompt characteristic patterns"""
        patterns = []
        
        # Analyze prompt characteristics
        prompt_texts = [f.get("originalPrompt", "") for f in failures if f.get("originalPrompt")]
    ...
    async def _find_rule_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for failure in failures:
            ...
            rule_ids = [rule.get("ruleId") or rule.get("id", "unknown") for rule in applied_rules]
    ...
    async def _find_temporal_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _identify_root_causes(self, failures: List[Dict[str, Any]], successes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            if len(failure_values) >= 3 and len(success_values) >= 3:
                ...
                try:
                    if self._is_numeric_characteristic(failure_values):
                        ...
                        effect_size = (np.mean(failure_values) - np.mean(success_values)) / np.sqrt(
                        ...
                        for category in failure_counts:
                            ...
                            success_rate = success_counts.get(category, 0) / total_successes
                            
                            if failure_rate > success_rate * 2 and failure_counts[category] >= 3:
                                root_causes.append({
                                    ...
                                    "correlation_strength": failure_rate / (success_rate + 0.01),
                                    "evidence": [
                    ...
                    self.logger.warning(f"Statistical comparison failed for {characteristic}: {e}")
    ...
    async def _find_missing_rules(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _find_edge_cases(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _find_systematic_issues(self, failures: List[Dict[str, Any]], all_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for result in all_results:
            ...
            for rule in applied_rules:
                rule_id = rule.get("ruleId") or rule.get("id", "unknown")
                score = result.get("overallImprovement", 0) or result.get("improvementScore", 0)
    ...
    def _compare_failures_with_successes(self, failures: List[Dict[str, Any]], successes: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Compare failures with successes to identify key differences"""
        comparison = {}
        
        # Basic statistics
        failure_scores = [f.get("overallImprovement", 0) or f.get("improvementScore", 0) for f in failures]
        success_scores = [s.get("overallImprovement", 0) or s.get("improvementScore", 0) for s in successes]
        ...
        comparison["score_comparison"] = {
            ...
            "score_gap": float(np.mean(success_scores) - np.mean(failure_scores)) if failure_scores and success_scores else 0
        }
        ...
        comparison["rule_analysis"] = {
            "problematic_rules": sorted(problematic_rules, key=lambda x: x["bias_ratio"], reverse=True)[:5],
            "beneficial_rules": sorted(beneficial_rules, key=lambda x: x["benefit_ratio"], reverse=True)[:5]
        }
        
        return comparison
    
    async def _generate_failure_recommendations(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for pattern in patterns[:5]:  # Top 5 patterns
            if pattern["type"] == "context":
                ...
                recommendations.append({
                    ...
                    "expected_impact": pattern["frequency"] / len(failures),
                    "implementation_effort": "low" if not pattern["characteristics"]["is_combination"] else "medium",
    ...
    async def _prioritize_fixes_by_impact(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        fixes.append({
            ...
            "estimated_failures_prevented": len([
                f for f in failures 
                if len(f.get("originalPrompt", "").split()) < 5 or len(f.get("originalPrompt", "").split()) > 100
            ])
        })
    ...
    def _extract_characteristics(self, results: List[Dict[str, Any]]) -> Dict[str, List[Any]]:
        ...
        for result in results:
            ...
            context = result.get("context", {})
            characteristics["project_type"].append(context.get("projectType", "unknown"))
    ...
    def _initialize_ml_fmea_database(self) -> List:
        ...
        return [
            ...
            MLFailureMode(
                ...
                root_causes=["Environment changes", "User behavior shifts", "System updates"],
                detection_methods=["KS test", "PSI monitoring", "JS divergence"],
                mitigation_strategies=["Continuous monitoring", "Model retraining", "Adaptive thresholds"],
            ...
            MLFailureMode(
                ...
                root_causes=["Incomplete validation", "Pipeline errors", "Source system changes"],
                detection_methods=["Data profiling", "Schema validation", "Completeness checks"],
                mitigation_strategies=["Robust validation", "Pipeline monitoring", "Data contracts"],
            ...
            MLFailureMode(
                ...
                root_causes=["Insufficient regularization", "Limited training data", "Poor validation"],
                detection_methods=["Cross-validation", "Holdout testing", "Performance monitoring"],
                mitigation_strategies=["Regularization techniques", "More diverse data", "Ensemble methods"],
            ...
            MLFailureMode(
                ...
                root_causes=["Lack of adversarial training", "No robustness testing", "Model architecture"],
                detection_methods=["Adversarial testing", "Robustness metrics", "Security scanning"],
                mitigation_strategies=["Adversarial training", "Input sanitization", "Model hardening"],
            ...
            MLFailureMode(
                ...
                root_causes=["Insufficient provisioning", "Memory leaks", "Inefficient algorithms"],
                detection_methods=["Resource monitoring", "Performance profiling", "Load testing"],
                mitigation_strategies=["Auto-scaling", "Resource optimization", "Efficient algorithms"],
            ...
            MLFailureMode(
                ...
                root_causes=["Poor version control", "Inadequate testing", "Manual deployment"],
                detection_methods=["Version tracking", "Integration tests", "Deployment validation"],
                mitigation_strategies=["Automated deployment", "Version compatibility checks", "Rollback procedures"],
                affected_components=["deployment_pipeline", "model_registry"],
                impact_areas=["system_stability", "deployment_reliability"]
            )
        ]
    
    async def _perform_ml_fmea_analysis(self, failures: List[Dict[str, Any]], test_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for failure_mode in self.ml_failure_modes:
                ...
                if len(affected_failures) > 0:
                    ...
                    updated_rpn = failure_mode.severity * actual_occurrence * failure_mode.detection
                    ...
                    fmea_analysis["identified_failure_modes"].append({
                        ...
                        "priority": "critical" if updated_rpn > 150 else "high" if updated_rpn > 100 else "medium"
                    })
            
            # Build risk matrix
            fmea_analysis["risk_matrix"] = self._build_risk_matrix(fmea_analysis["identified_failure_modes"])
            
            # Identify critical failure paths
            fmea_analysis["critical_paths"] = self._identify_critical_failure_paths(fmea_analysis["identified_failure_modes"])
            
            # Generate mitigation plan
            fmea_analysis["mitigation_plan"] = self._generate_mitigation_plan(fmea_analysis["identified_failure_modes"])
    ...
    async def _perform_ensemble_anomaly_detection(self, failures: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Perform ensemble anomaly detection using multiple methods"""
        try:
            if len(failures) < 10:
                return {"insufficient_data": "Need at least 10 failures for anomaly detection"}
            
            # Extract features for anomaly detection
            features = self._extract_anomaly_features(failures)
            
            if len(features) == 0:
                return {"no_features": "Could not extract features for anomaly detection"}
            ...
            for detector_name, detector in self.anomaly_detectors.items():
                try:
                    ...
                    anomaly_results[detector_name] = {
                        ...
                        "anomaly_percentage": len(anomaly_indices) / len(failures) * 100,
                        "anomaly_indices": anomaly_indices.tolist(),
                        "anomalous_failures": [failures[i] for i in anomaly_indices[:5]]  # Top 5 examples
                    }
                    
                except Exception as e:
                    self.logger.warning(f"Anomaly detection with {detector_name} failed: {e}")
                    anomaly_results[detector_name] = {"error": str(e)}
            
            # Consensus-based anomaly identification
            consensus_anomalies = self._identify_consensus_anomalies(anomaly_results, failures)
            ...
            return {
                ...
                "anomaly_summary": {
                    ...
                    "consensus_anomaly_rate": len(consensus_anomalies) / len(failures) * 100
                }
            }
    ...
    async def _calculate_risk_priority_numbers(self, failures: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for failure_mode in self.ml_failure_modes:
            ...
            if affected_count > 0:
                ...
                rpn = failure_mode.severity * adjusted_occurrence * failure_mode.detection
        ...
        risk_analysis["overall_risk_score"] = total_rpn / mode_count if mode_count > 0 else 0
        
        return risk_analysis
    
    def _match_failures_to_mode(self, failures: List[Dict[str, Any]], failure_mode) -> List[Dict[str, Any]]:
        ...
        for failure in failures:
            ...
            if failure_mode.failure_type == "data":
                data_keywords = ["data", "input", "validation", "schema", "format", "missing", "null"]
    ...
    def _extract_anomaly_features(self, failures: List[Dict[str, Any]]) -> List[List[float]]:
        ...
        for failure in failures:
            ...
            exec_time = failure.get("executionTime", 0) or failure.get("processingTime", 0)
    ...
    def _identify_consensus_anomalies(self, anomaly_results: Dict[str, Any], failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for idx, count in anomaly_counts.items():
            if count >= consensus_threshold and idx < len(failures):
                ...
                total_detectors = len([r for r in anomaly_results.values() if "error" not in r])
                consensus_anomalies.append({
                    "failure_index": idx,
                    "detector_agreement": count,
                    "failure_data": failures[idx],
                    "anomaly_score": count / max(total_detectors, 1)  # Normalized score
                })
    ...
    def _count_affected_failures(self, failures: List[Dict[str, Any]], failure_mode) -> int:
    ...
    def _identify_critical_failure_paths(self, failure_modes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for mode in sorted_modes[:5]:  # Top 5 critical paths
            if mode["rpn"] >= 100:  # Only include high-risk modes
                critical_paths.append({
                    ...
                    "recommended_actions": mode["mitigation_strategies"][:3]  # Top 3 strategies
                })
        
        return critical_paths
    
    def _generate_mitigation_plan(self, failure_modes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, mode in enumerate(sorted_modes[:10]):  # Top 10 modes
            plan_item = {
                ...
                "timeline": "immediate" if mode["rpn"] >= 150 else "1-2 weeks" if mode["rpn"] >= 100 else "1 month"
            }
    ...
    def _initialize_robustness_components(self) -> None:
        ...
        try:
            ...
            self.logger.info("Robustness validation components initialized successfully")
        ...
        try:
            if PROMETHEUS_AVAILABLE:
                ...
                self.prometheus_metrics = {
                    "failure_rate": Gauge(
                        "ml_failure_rate",
                        "Current ML system failure rate"
                    ),
                    ...
                    "rpn_score": Gauge(
                        "ml_risk_priority_number",
                        "ML FMEA Risk Priority Number"
                    )
                }
    ...
    async def _perform_robustness_validation(self, failures: List[Dict[str, Any]], test_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(test_results) >= self.config.robustness_test_samples:
                validation_results["noise_robustness"] = await self._test_noise_robustness(test_results)
            
            # Test 2: Adversarial Robustness
            if self.config.adversarial_testing and ART_AVAILABLE:
                validation_results["adversarial_robustness"] = await self._test_adversarial_robustness(test_results)
            
            # Test 3: Edge Case Robustness
            validation_results["edge_case_robustness"] = await self._test_edge_case_robustness(failures)
            
            # Test 4: Data Drift Robustness
            validation_results["data_drift_robustness"] = await self._test_data_drift_robustness(test_results)
            ...
            validation_results["overall_robustness_score"] = np.mean(scores) if scores else 0.0
            
            # Generate robustness recommendations
            validation_results["robustness_recommendations"] = self._generate_robustness_recommendations(validation_results)
            ...
            self.logger.info(f"Robustness validation completed with overall score: {validation_results['overall_robustness_score']:.3f}")
    ...
    async def _test_noise_robustness(self, test_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for noise_level in self.config.noise_levels:
            ...
            success_rate = sum(1 for score in noisy_scores if score >= baseline_avg * 0.9) / len(noisy_scores)
            degradation_rate = max(0, (baseline_avg - noisy_avg) / baseline_avg) if baseline_avg > 0 else 0
            ...
            if len(failed_samples) > 0:
                noise_test_results["failed_samples"].extend(failed_samples[:3])  # Top 3 failures per level
        
        # Calculate overall metrics
        avg_degradation = total_degradation / noise_level_count if noise_level_count > 0 else 0
    ...
    async def _test_adversarial_robustness(self, test_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, sample in enumerate(test_samples):
                ...
                original_score = sample.get("overallImprovement", 0) or sample.get("improvementScore", 0)
                
                # Simple adversarial simulation: targeted degradation
                adversarial_score = max(0, original_score - self.config.adversarial_epsilon)
            ...
            adversarial_test_results["failed_samples"] = failed_samples[:10]  # Top 10 failures
            ...
            self.logger.info(f"Adversarial robustness test: {attack_success_rate:.1%} attack success rate")
    ...
    async def _test_edge_case_robustness(self, failures: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(edge_cases) > 0:
            ...
            edge_case_rate = edge_case_failures / total_failures if total_failures > 0 else 0
            ...
            for edge_case in edge_cases[:5]:  # Top 5 edge cases
                if "examples" in edge_case:
                    edge_case_test_results["failed_samples"].extend(edge_case["examples"][:2])
    ...
    async def _test_data_drift_robustness(self, test_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(test_results) < 20:
            drift_test_results["insufficient_data"] = "Need at least 20 samples for drift detection"
        ...
        try:
            ...
            if len(baseline_scores) > 0 and len(current_scores) > 0:
                ...
                ks_statistic, ks_p_value = stats.ks_2samp(baseline_scores, current_scores)
                ...
                pooled_std = np.sqrt((
                    np.var(baseline_scores) + np.var(current_scores)
                ) / 2)
                cohens_d = (np.mean(current_scores) - np.mean(baseline_scores)) / pooled_std if pooled_std > 0 else 0
                ...
                if drift_detected:
                    ...
                    drift_test_results["robustness_score"] = max(0, 1 - degradation_rate)
                    ...
                    for i, (idx, diff, baseline, current) in enumerate(score_differences[:5]):
    ...
    def _generate_robustness_recommendations(self, validation_results: Dict[str, Any]) -> List[str]:
        ...
        if overall_score < 0.5:
            recommendations.append("CRITICAL: Overall system robustness is poor - immediate action required")
    ...
    async def _update_prometheus_metrics(self, failure_analysis: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if "anomaly_summary" in anomaly_detection:
                anomaly_rate = anomaly_detection["anomaly_summary"].get("consensus_anomaly_rate", 0)
    ...
    async def _check_and_trigger_alerts(self, failure_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
                ...
                if metric_value is not None and self._check_threshold(metric_value, alert_def):
                    ...
                    if self._is_alert_ready_to_trigger(alert_def, current_time):
                        ...
                        triggered_alert = {
                            ...
                            "recommended_actions": self._get_alert_recommendations(alert_def.alert_name)
                        }
    ...
    def _extract_metric_value(self, failure_analysis: Dict[str, Any], metric_name: str) -> Optional[float]:
    ...
    def _is_alert_ready_to_trigger(self, alert_def: PrometheusAlert, current_time: datetime) -> bool:
        """Check if alert is not in cooldown period"""
    ...
    def _get_alert_recommendations(self, alert_name: str) -> List[str]:
        ...
        return recommendations.get(alert_name, ["Investigate the issue", "Check system logs"])
    
    def _update_active_alerts(self, failure_analysis: Dict[str, Any], current_time: datetime) -> None:
        ...
        for alert in self.active_alerts:
            ...
            if metric_value is not None and not self._check_threshold(metric_value, alert):
