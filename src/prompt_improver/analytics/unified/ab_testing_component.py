...
class ABTestingComponent(ABTestingProtocol, AnalyticsComponentProtocol):
    ...
    async def record_experiment_event(
        ...
        try:
            ...
            self.logger.error(f"Error recording event for experiment {experiment_id}: {e}")
            return False
    ...
    async def _analyze_frequentist(
        ...
        try:
            ...
            first_treatment_key = list(treatment_data.keys())[0]
            ...
            se_diff = np.sqrt((control_rate * (1 - control_rate) / control_samples) + 
            ...
            self.logger.error(f"Error in frequentist analysis: {e}")
            raise
    ...
    async def _analyze_bayesian(
        ...
        try:
            ...
            n_samples = 10000
            control_samples_dist = np.random.beta(control_alpha, control_beta, n_samples)
            treatment_samples_dist = np.random.beta(treatment_alpha, treatment_beta, n_samples)
            
            # Calculate probability that treatment > control
            prob_treatment_better = np.mean(treatment_samples_dist > control_samples_dist)
    ...
    async def _analyze_sequential(
        ...
        try:
            ...
            if information_fraction > 0:
                ...
                adjusted_alpha = experiment.target_significance * np.sqrt(information_fraction)
    ...
    async def _check_early_stopping(self, experiment_id: str) -> None:
        ...
        try:
            ...
            self.logger.error(f"Error checking early stopping for experiment {experiment_id}: {e}")
