...
class EnhancedResponseCompressor:
    ...
    def compress(
        ...
        if len(data) < min_size and (
            not self._should_force_compression(content_type, len(data))
        ):
    ...
    def _get_adaptive_min_size(self, content_type: str | None, data_size: int) -> int:
        ...
        base_threshold = adaptive_thresholds.get(
            content_type, adaptive_thresholds["default"]
        )
        if data_size > 10000:
...
class EnhancedPayloadOptimizer:
    ...
    def optimize_response(
        ...
        content_optimizer = self.content_optimizers.get(
            content_type, self.content_optimizers["default"]
        )
        ...
        compression_result = self.compressor.compress(
            serialized_data, algorithm=compression_algorithm, content_type=content_type
        )
    ...
    def _optimize_json_content(self, data: Any) -> Any:
        ...
        if isinstance(data, list):
            return [
                self._optimize_json_content(item) for item in data if item is not None
            ]
        return data
...
class ResponseOptimizer:
        ...
        async with measure_mcp_operation(f"optimize_{operation_name}") as perf_metrics:
            ...
            try:
                ...
                perf_metrics.metadata.update({
                    "original_size": optimized.get("optimization_metadata", {}).get(
                        "original_size_bytes", 0
                    ),
                    "compressed_size": optimized.get("optimization_metadata", {}).get(
                        "compressed_size_bytes", 0
                    ),
                    "compression_algorithm": optimized.get(
    ...
    def _update_stats(
        ...
        if metadata:
            bytes_saved = metadata.get("original_size_bytes", 0) - metadata.get(
                "compressed_size_bytes", 0
            )
            ...
            self._optimization_stats["compression_algorithm_usage"][algorithm] = (
                self._optimization_stats["compression_algorithm_usage"].get(
                    algorithm, 0
                )
                + 1
            )
    ...
    def get_optimization_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_bytes_saved_per_response": self._optimization_stats[
                "total_bytes_saved"
            ]
            / total_responses,
...
async def optimize_mcp_response(
    ...
    return await optimizer.optimize_mcp_response(
        response_data, operation_name, enable_compression
    )
