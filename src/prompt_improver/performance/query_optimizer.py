...
class PreparedStatementCache:
    ...
    def __init__(self, max_size: int=1000, warming_threshold: int=10):
        ...
        self._stats = {'cache_hits': 0, 'cache_misses': 0, 'evictions': 0, 'warming_operations': 0, 'total_queries': 0}
        self._warming_enabled = True
        self._warming_batch_size = 50

    def get_statement(self, query_hash: str) -> PreparedStatement | None:
    ...
    def store_statement(self, statement: PreparedStatement) -> None:
        ...
        if statement.query_hash not in self._metrics:
            self._metrics[statement.query_hash] = QueryMetrics(query_hash=statement.query_hash, query_type=statement.query_type)
    ...
    def _evict_lru_statements(self, count: int=None) -> None:
        ...
        while evicted < count and self._cache:
            ...
            logger.debug('Evicted prepared statement: %s', statement.statement_id)

    def update_metrics(self, query_hash: str, execution_time_ms: float, success: bool=True) -> None:
    ...
    def get_cache_statistics(self) -> dict[str, Any]:
        ...
        return {'cache_size': len(self._cache), 'max_size': self.max_size, 'utilization': len(self._cache) / self.max_size, 'hit_rate': hit_rate, 'total_requests': total_requests, 'cache_hits': self._stats['cache_hits'], 'cache_misses': self._stats['cache_misses'], 'evictions': self._stats['evictions'], 'warming_operations': self._stats['warming_operations'], 'performance_status': 'good' if hit_rate >= 0.85 else 'needs_optimization'}
        ...
        for sql, query_type in queries[:self._warming_batch_size]:
            try:
                ...
                if query_hash in self._cache and self._cache[query_hash].is_warmed:
                    continue
                statement = PreparedStatement(statement_id=f'warm_{query_hash[:8]}', query_hash=query_hash, query_type=query_type, sql_text=sql, parameter_names=self._extract_parameters(sql), created_at=time.time(), last_used=time.time(), is_warmed=True)
                self.store_statement(statement)
                warmed_count += 1
            except Exception as e:
                logger.warning('Failed to warm query: %s', e)
        self._stats['warming_operations'] += warmed_count
        logger.info('Cache warming completed: %s queries warmed', warmed_count)
        return warmed_count
...
class QueryOptimizer:
    ...
    def __init__(self, engine: AsyncEngine, sla_timeout_ms: float=200.0, query_timeout_ms: float=150.0):
        ...
        self._performance_stats = {'total_queries': 0, 'sla_violations': 0, 'timeouts': 0, 'cache_hits': 0, 'avg_response_time_ms': 0.0, 'p95_response_time_ms': 0.0, 'p99_response_time_ms': 0.0}
        self._response_times: list[float] = []
        self._max_response_history = 1000
        self._query_patterns = {QueryType.RULE_RETRIEVAL: ['SELECT * FROM rule_metadata', 'SELECT * FROM rule_performance', 'SELECT rule_id, rule_name FROM rule_metadata WHERE enabled = true'], QueryType.RULE_COMBINATIONS: ['SELECT * FROM rule_combinations WHERE combined_effectiveness >=', 'SELECT rule_set, combined_effectiveness FROM rule_combinations'], QueryType.FEEDBACK_STORAGE: ['INSERT INTO feedback_collection', 'INSERT INTO ml_training_feedback']}

    async def execute_optimized_query(self, session: AsyncSession, sql: str, parameters: dict[str, Any]=None, query_type: QueryType=QueryType.RULE_RETRIEVAL) -> Any:
        ...
        try:
            ...
            if prepared_stmt:
                ...
                prepared_stmt = PreparedStatement(statement_id=f'stmt_{query_hash[:8]}', query_hash=query_hash, query_type=query_type, sql_text=sql, parameter_names=list(parameters.keys()) if parameters else [], created_at=time.time(), last_used=time.time())
                self.statement_cache.store_statement(prepared_stmt)
            result = await asyncio.wait_for(session.execute(text(sql), parameters or {}), timeout=self.query_timeout_ms / 1000.0)
            ...
            if execution_time_ms > self.sla_timeout_ms:
                ...
                logger.warning('SLA violation: Query took %sms (limit: %sms)', format(execution_time_ms, '.1f'), self.sla_timeout_ms)
            return result
        except TimeoutError:
            execution_time_ms = (time.time() - start_time) * 1000
            self._performance_stats['timeouts'] += 1
            self._update_performance_metrics(execution_time_ms, query_hash, False)
            logger.error('Query timeout after %sms: %s...', format(execution_time_ms, '.1f'), sql[:100])
            raise
        except Exception as e:
            execution_time_ms = (time.time() - start_time) * 1000
            self._update_performance_metrics(execution_time_ms, query_hash, False)
    ...
    def _update_performance_metrics(self, execution_time_ms: float, query_hash: str, success: bool) -> None:
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        return {'query_performance': {'total_queries': total_queries, 'avg_response_time_ms': self._performance_stats['avg_response_time_ms'], 'p95_response_time_ms': self._performance_stats['p95_response_time_ms'], 'p99_response_time_ms': self._performance_stats['p99_response_time_ms'], 'sla_compliance_rate': sla_compliance_rate, 'sla_violations': self._performance_stats['sla_violations'], 'timeouts': self._performance_stats['timeouts']}, 'cache_performance': cache_stats, 'sla_status': {'target_ms': self.sla_timeout_ms, 'compliant': sla_compliance_rate >= 0.95, 'status': 'good' if sla_compliance_rate >= 0.95 else 'needs_optimization'}}
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        return {'status': overall_status, 'issues': health_issues, 'metrics': metrics, 'timestamp': time.time()}
