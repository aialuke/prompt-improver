...
from prompt_improver.performance.baseline.automation import BaselineAutomation, get_automation_dashboard, get_baseline_automation, start_automated_baseline_system, stop_automated_baseline_system
from prompt_improver.performance.baseline.baseline_collector import BaselineCollector, get_baseline_collector, record_operation, track_operation
from prompt_improver.performance.baseline.enhanced_dashboard_integration import PerformanceDashboard, get_dashboard_data, get_performance_dashboard, start_performance_dashboard
from prompt_improver.performance.baseline.load_testing_integration import LoadPattern, LoadTestConfig, LoadTestingIntegration, find_system_capacity, get_load_testing_integration, run_integrated_load_test
from prompt_improver.performance.baseline.models import STANDARD_METRICS, AlertSeverity, BaselineComparison, BaselineMetrics, MetricDefinition, MetricType, MetricValue, PerformanceTrend, ProfileData, RegressionAlert, TrendDirection, get_metric_definition
from prompt_improver.performance.baseline.performance_validation_suite import PerformanceValidationResult, PerformanceValidationSuite, SystemEfficiencyReport, get_validation_suite, quick_performance_check, validate_baseline_system_performance
from prompt_improver.performance.baseline.production_optimization_guide import DeploymentEnvironment, OptimizationLevel, ProductionOptimizationGuide, generate_production_checklist, get_development_config, get_optimization_guide, get_production_config, validate_for_production
from prompt_improver.performance.baseline.profiler import ContinuousProfiler, get_performance_summary, get_profiler, profile, profile_async_block, profile_block, start_continuous_profiling, stop_continuous_profiling
from prompt_improver.performance.baseline.regression_detector import RegressionDetector, check_baseline_for_regressions, get_regression_detector, setup_webhook_alerts
from prompt_improver.performance.baseline.reporting import BaselineReporter, generate_capacity_planning_report, generate_daily_performance_report, generate_weekly_performance_report, get_baseline_reporter
from prompt_improver.performance.baseline.statistical_analyzer import StatisticalAnalyzer, analyze_metric_trend, calculate_baseline_score, detect_performance_anomalies
...
class PerformanceBaselineSystem:
    ...
    def __init__(self, enable_collection: bool=True, enable_analysis: bool=True, enable_regression_detection: bool=True, enable_profiling: bool=False, enable_automation: bool=True, enable_reporting: bool=True, integration_with_existing_monitoring: bool=True):
    ...
    async def _integrate_with_existing_monitoring(self) -> None:
        ...
        try:
            ...
            if self.collector:
                ...
                async def enhanced_record_operation(operation_name: str, response_time_ms: float, is_error: bool=False, metadata: dict[str, Any] | None=None):
            ...
            try:
                ...
                baseline_metrics = {'baseline_collection_total': metrics_registry.get_or_create_counter('baseline_collection_total', 'Total baseline collections performed', ['status']), 'baseline_analysis_duration': metrics_registry.get_or_create_histogram('baseline_analysis_duration_seconds', 'Time spent analyzing baselines', buckets=[0.1, 0.5, 1.0, 2.0, 5.0, 10.0]), 'baseline_regressions_detected': metrics_registry.get_or_create_counter('baseline_regressions_detected_total', 'Total performance regressions detected', ['severity', 'metric'])}
                logger.info('✓ Integrated with metrics registry')
            except ImportError:
                logger.debug('Metrics registry not available for integration')
            try:
                from prompt_improver.performance.monitoring.health.service import HealthCheckService
            ...
            logger.error('Failed to integrate with existing monitoring: %s', e)

    async def record_production_traffic(self, operation_name: str, response_time_ms: float, is_error: bool=False, user_id: str | None=None, request_size_bytes: int | None=None, response_size_bytes: int | None=None, metadata: dict[str, Any] | None=None) -> None:
        ...
        enhanced_metadata.update({'traffic_type': 'production', 'timestamp': datetime.now(UTC).isoformat()})
        if user_id:
        ...
        await self.collector.record_request(response_time_ms, is_error, operation_name, enhanced_metadata)

    async def analyze_performance_trends(self, hours: int=24, metrics: list[str] | None=None) -> dict[str, Any]:
        ...
        for metric_name in metrics:
            try:
                ...
                trends[metric_name] = {'direction': trend.direction.value, 'magnitude': trend.magnitude, 'confidence_score': trend.confidence_score, 'is_significant': trend.is_significant(), 'sample_count': trend.sample_count}
            except Exception as e:
                trends[metric_name] = {'error': str(e)}
        return {'timeframe_hours': hours, 'baselines_analyzed': len(baselines), 'trends': trends, 'analysis_timestamp': datetime.now(UTC).isoformat()}
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        status = {'initialized': self._components_initialized, 'running': self._running, 'components': {}}
        if self.collector:
...
async def record_production_request(operation_name: str, response_time_ms: float, is_error: bool=False, **kwargs) -> None:
    """Record a production request for baseline analysis."""
    if _baseline_system_running:
        system = get_baseline_system()
        await system.record_production_traffic(operation_name, response_time_ms, is_error, **kwargs)
...
class track_production_operation:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time is not None:
            ...
            asyncio.create_task(task_manager.submit_enhanced_task(task_id=f'baseline_production_record_{self.operation_name}_{str(uuid.uuid4())[:8]}', coroutine=record_production_request(self.operation_name, duration_ms, is_error, **self.metadata), priority=TaskPriority.NORMAL, tags={'service': 'performance', 'type': 'tracking', 'component': 'baseline', 'operation': self.operation_name}))
__all__ = ['BaselineMetrics', 'MetricValue', 'MetricDefinition', 'PerformanceTrend', 'RegressionAlert', 'ProfileData', 'BaselineComparison', 'MetricType', 'TrendDirection', 'AlertSeverity', 'STANDARD_METRICS', 'get_metric_definition', 'BaselineCollector', 'StatisticalAnalyzer', 'RegressionDetector', 'ContinuousProfiler', 'BaselineAutomation', 'BaselineReporter', 'PerformanceDashboard', 'LoadTestingIntegration', 'PerformanceValidationSuite', 'ProductionOptimizationGuide', 'PerformanceValidationResult', 'SystemEfficiencyReport', 'LoadPattern', 'LoadTestConfig', 'DeploymentEnvironment', 'OptimizationLevel', 'PerformanceBaselineSystem', 'get_baseline_system', 'set_baseline_system', 'initialize_baseline_system', 'start_baseline_system', 'stop_baseline_system', 'is_baseline_system_running', 'record_production_request', 'track_production_operation', 'get_baseline_collector', 'get_regression_detector', 'get_profiler', 'get_baseline_automation', 'get_baseline_reporter', 'record_operation', 'track_operation', 'analyze_metric_trend', 'detect_performance_anomalies', 'calculate_baseline_score', 'check_baseline_for_regressions', 'setup_webhook_alerts', 'profile', 'profile_block', 'profile_async_block', 'start_continuous_profiling', 'stop_continuous_profiling', 'get_performance_summary', 'start_automated_baseline_system', 'stop_automated_baseline_system', 'get_automation_dashboard', 'generate_daily_performance_report', 'generate_weekly_performance_report', 'generate_capacity_planning_report', 'get_performance_dashboard', 'start_performance_dashboard', 'get_dashboard_data', 'get_load_testing_integration', 'run_integrated_load_test', 'find_system_capacity', 'get_validation_suite', 'validate_baseline_system_performance', 'quick_performance_check', 'get_optimization_guide', 'get_production_config', 'get_development_config', 'validate_for_production', 'generate_production_checklist']
