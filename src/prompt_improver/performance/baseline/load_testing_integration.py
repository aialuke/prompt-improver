...
class LoadTestingIntegration:
    ...
    async def run_load_test_with_baseline_collection(
        self, test_name: str, config: LoadTestConfig, target_endpoints: list[str]
    ) -> dict[str, Any]:
        ...
        collection_task_id = await task_manager.submit_enhanced_task(
            ...
            coroutine=self._collect_baselines_during_test(
                test_id, config.duration_minutes
            ),
            priority=TaskPriority.HIGH,
            tags={
                "service": "performance",
                "type": "load_testing",
                "component": "baseline_collection",
                "test_name": test_name,
            },
        )
    ...
    async def _collect_baselines_during_test(
        self, test_id: str, duration_minutes: int
    ...
    async def _run_constant_load_test(
        self, config: LoadTestConfig, endpoints: list[str]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for i in range(total_requests):
                ...
                await asyncio.sleep(
                    max(0, request_interval - (time.time() - request_start))
                )
        finally:
            await session.close()
        return results
    ...
    async def _send_test_request(
        self, session: "aiohttp.ClientSession", endpoint: str, results: dict
    ) -> None:
    ...
    def _simulate_load_test_results(self, config: LoadTestConfig) -> dict[str, Any]:
        ...
        response_times = [
            max(10, random.normalvariate(120, 30)) for _ in range(total_requests)
        ]
        return {
            "pattern": config.pattern.value,
    ...
    async def _analyze_load_test_results(
        ...
        analysis = {
            ...
            "capacity_assessment": self._assess_capacity(
                load_results, baseline_results, config
            ),
    ...
    def _analyze_load_performance(self, load_results: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": load_results.get("successful_requests", 0)
    ...
    def _analyze_load_system_correlation(
        self, load_results: dict[str, Any], baselines: list[BaselineMetrics]
    ) -> dict[str, Any]:
    ...
    def _assess_capacity(
        self, load_results: dict, baselines: list, config: LoadTestConfig
    ) -> dict:
    ...
    def _generate_load_test_recommendations(
        ...
        if baselines:
            ...
            if cpu_values and max(cpu_values) > 80:
                recommendations.append(
                    "High CPU utilization detected - consider scaling"
                )
            if memory_values and max(memory_values) > 85:
                recommendations.append(
                    "High memory utilization detected - check for leaks"
                )
        if config.pattern == LoadPattern.SPIKE:
            recommendations.append(
                "Consider implementing auto-scaling for spike handling"
            )
        if not recommendations:
            recommendations.append(
                "System performed well under load - monitor in production"
            )
    ...
    async def run_progressive_load_test(
        ...
        for rps in rps_levels:
            ...
            result = await self.run_load_test_with_baseline_collection(
                f"{test_name}_rps_{rps}", config, endpoints
            )
            test_results.append(result)
            analysis = result["analysis"]["load_performance"]
            if analysis["success_rate"] < 90:
                logger.warning(
                    f"Success rate dropped to {analysis['success_rate']:.1f}% at {rps:.1f} RPS"
                )
                break
    ...
    def _analyze_progressive_results(self, test_results: list[dict]) -> dict[str, Any]:
        ...
        return {
            ...
            "capacity_recommendation": {
                ...
                "scaling_threshold": int(max_sustainable_rps * 0.8),
            },
        }
...
async def find_system_capacity(
    endpoints: list[str], max_rps: int = 200, duration_minutes: int = 30
) -> dict[str, Any]:
