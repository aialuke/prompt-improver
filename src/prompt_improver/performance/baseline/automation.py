...
class AutomationConfig:
    ...
    def __init__(
        self,
        collection_schedule: str = "*/5 * * * *",
        analysis_schedule: str = "0 */1 * * *",
        reporting_schedule: str = "0 9 * * *",
        regression_check_schedule: str = "*/10 * * * *",
        baseline_retention_days: int = 30,
        analysis_retention_days: int = 90,
        alert_retention_days: int = 365,
        trend_analysis_hours: int = 24,
        regression_reference_hours: int = 168,
...
class BaselineAutomation:
    ...
    def __init__(
        ...
        logger.info(
            f"BaselineAutomation initialized with storage at {self.config.storage_path}"
        )
    ...
    async def _schedule_automation_tasks(self) -> None:
        ...
        try:
            if self.config.enable_auto_analysis:
                ...
                logger.info(
                    f"Scheduled trend analysis: {self.config.analysis_schedule}"
                )
            if self.config.enable_auto_regression_detection:
                ...
                logger.info(
                    f"Scheduled regression detection: {self.config.regression_check_schedule}"
                )
            if self.config.enable_auto_reporting:
                ...
                logger.info(
                    f"Scheduled automated reporting: {self.config.reporting_schedule}"
                )
            cleanup_task = aiocron.crontab(
                "0 1 * * *", func=self._run_cleanup, start=True
            )
            self._scheduled_tasks["cleanup"] = cleanup_task
            if self.config.backup_enabled:
                backup_task = aiocron.crontab(
                    self.config.backup_schedule, func=self._run_backup, start=True
                )
    ...
    async def _fallback_task_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if (
                    self.config.enable_auto_analysis
                    and (current_time - last_analysis).total_seconds() >= 3600
                ):
                    await self._run_trend_analysis()
                    last_analysis = current_time
                if (
                    self.config.enable_auto_regression_detection
                    and (current_time - last_regression_check).total_seconds() >= 600
                ):
    ...
    async def _run_trend_analysis(self) -> None:
        ...
        try:
            ...
            if len(baselines) < self.config.min_baselines_for_analysis:
                logger.warning(
                    f"Insufficient baselines for analysis ({len(baselines)} < {self.config.min_baselines_for_analysis})"
                )
                return
            metrics_to_analyze = [
                "response_time",
    ...
    async def _run_regression_detection(self) -> None:
        ...
        try:
            ...
            if len(reference_baselines) < self.config.min_baselines_for_analysis:
                logger.debug(
                    f"Insufficient reference baselines ({len(reference_baselines)})"
                )
                return
            ...
            if alerts:
                logger.info(
                    f"Regression detection (task: {task_id}): {len(alerts)} alerts generated"
                )
            else:
                logger.debug(
        ...
        try:
            ...
            result = AutomationTaskResult(
                ...
                result_data={
                    ...
                    "report_date": format_date_only(start_time),
                },
            )
            await self._record_task_result(result)
    ...
    async def _run_cleanup(self) -> None:
        ...
        try:
            ...
            baseline_cutoff = datetime.now(UTC) - timedelta(
                days=self.config.baseline_retention_days
            )
            cleanup_stats["baselines_removed"] = await self._cleanup_old_files(
                self.baselines_path, baseline_cutoff
            )
            analysis_cutoff = datetime.now(UTC) - timedelta(
                days=self.config.analysis_retention_days
            )
            cleanup_stats["analysis_removed"] = await self._cleanup_old_files(
                self.analysis_path, analysis_cutoff
            )
            alert_cutoff = datetime.now(UTC) - timedelta(
            ...
            self._task_history = [
                task for task in self._task_history if task.end_time > history_cutoff
            ]
    ...
    async def _run_backup(self) -> None:
        ...
        try:
            backup_dir = (
                ...
                / "backups"
                / format_compact_timestamp(start_time)
            )
    ...
    async def _save_regression_alerts(
        self, task_id: str, alerts: list[RegressionAlert]
    ) -> None:
    ...
    async def trigger_manual_analysis(self) -> AutomationTaskResult:
        ...
        await self._run_trend_analysis()
        analysis_tasks = [
            t for t in self._task_history if t.task_type == "trend_analysis"
        ]
    ...
    async def trigger_manual_regression_check(self) -> AutomationTaskResult:
        ...
        await self._run_regression_detection()
        regression_tasks = [
            t for t in self._task_history if t.task_type == "regression_detection"
        ]
    ...
    def get_automation_status(self) -> dict[str, Any]:
        ...
        recent_tasks = [
            ...
            if (datetime.now(UTC) - task.end_time).total_seconds() < 86400
        ]
        task_stats = {}
    ...
    def get_recent_task_results(
        self, hours: int = 24, task_type: str | None = None
    ) -> list[dict[str, Any]]:
        ...
        filtered_tasks = [
            task
            for task in self._task_history
            if task.end_time > cutoff_time
            and (task_type is None or task.task_type == task_type)
        ]
        return [task.model_dump() for task in filtered_tasks]
