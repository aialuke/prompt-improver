...
import psutil
from prompt_improver.performance.baseline.automation import BaselineAutomation
from prompt_improver.performance.baseline.baseline_collector import BaselineCollector
from prompt_improver.performance.baseline.enhanced_dashboard_integration import PerformanceDashboard
from prompt_improver.performance.baseline.load_testing_integration import LoadTestingIntegration
...
class PerformanceValidationSuite:
    ...
    def __init__(self):
        ...
        self.performance_targets = {'baseline_collection': 100.0, 'statistical_analysis': 150.0, 'regression_detection': 50.0, 'dashboard_update': 200.0, 'automation_cycle': 1000.0}
    ...
    async def _validate_baseline_collection(self) -> PerformanceValidationResult:
        ...
        for i in range(10):
            ...
            try:
                ...
                tracemalloc.stop()
                logger.error('Collection test {i} failed: %s', e)
        ...
        return PerformanceValidationResult(test_name='baseline_collection', duration_ms=p95_duration, memory_usage_mb=avg_memory, cpu_utilization_percent=avg_cpu, meets_target=meets_target, target_ms=target, optimization_recommendations=recommendations, detailed_metrics={'avg_duration_ms': avg_duration, 'max_duration_ms': max_duration, 'p95_duration_ms': p95_duration, 'duration_stddev': statistics.stdev(durations), 'success_rate': (10 - durations.count(1000.0)) / 10})
    ...
    async def _validate_statistical_analysis(self) -> PerformanceValidationResult:
        ...
        test_baselines = []
        for i in range(50):
            baseline = BaselineMetrics(collection_timestamp=datetime.now(UTC), response_times=[100 + i, 120 + i, 80 + i], cpu_utilization=[20 + i % 10, 25 + i % 10], memory_utilization=[40 + i % 20, 45 + i % 20], error_rates=[0.1 + i % 5 * 0.1], throughput_values=[1000 + i * 10, 1100 + i * 10])
        ...
        for i in range(5):
            ...
            try:
                ...
                tracemalloc.stop()
                logger.error('Analysis test {i} failed: %s', e)
                durations.append(1000.0)
        avg_duration = statistics.mean(durations)
        avg_memory = statistics.mean(memory_usage)
        ...
        return PerformanceValidationResult(test_name='statistical_analysis', duration_ms=avg_duration, memory_usage_mb=avg_memory, cpu_utilization_percent=0, meets_target=meets_target, target_ms=target, optimization_recommendations=recommendations, detailed_metrics={'analysis_operations': 3, 'data_points_analyzed': 50, 'throughput_ops_per_second': 1000 / avg_duration if avg_duration > 0 else 0})
    ...
    async def _validate_regression_detection(self) -> PerformanceValidationResult:
        ...
        current_baseline = BaselineMetrics(collection_timestamp=datetime.now(UTC), response_times=[300, 350, 280], cpu_utilization=[85, 90], memory_utilization=[60, 65], error_rates=[2.0], throughput_values=[800, 850])
        historical_baselines = []
        for i in range(20):
            baseline = BaselineMetrics(collection_timestamp=datetime.now(UTC) - timedelta(minutes=i), response_times=[100, 120, 90], cpu_utilization=[30, 35], memory_utilization=[40, 45], error_rates=[0.5], throughput_values=[1200, 1300])
            historical_baselines.append(baseline)
        durations = []
        for i in range(10):
            start_time = time.time()
            try:
                alerts = await self.detector.check_for_regressions(current_baseline, historical_baselines)
                end_time = time.time()
                duration_ms = (end_time - start_time) * 1000
                durations.append(duration_ms)
            except Exception as e:
                logger.error('Detection test {i} failed: %s', e)
                durations.append(1000.0)
        avg_duration = statistics.mean(durations)
        ...
        return PerformanceValidationResult(test_name='regression_detection', duration_ms=avg_duration, memory_usage_mb=0, cpu_utilization_percent=0, meets_target=meets_target, target_ms=target, optimization_recommendations=recommendations, detailed_metrics={'alerts_generated': len(alerts) if 'alerts' in locals() else 0, 'historical_data_points': 20, 'detection_accuracy': 'high'})
    ...
    async def _validate_dashboard_performance(self) -> PerformanceValidationResult:
        ...
        for i in range(5):
            ...
            try:
                ...
                logger.error('Dashboard test {i} failed: %s', e)
                durations.append(1000.0)
        avg_duration = statistics.mean(durations)
        ...
        return PerformanceValidationResult(test_name='dashboard_performance', duration_ms=avg_duration, memory_usage_mb=0, cpu_utilization_percent=0, meets_target=meets_target, target_ms=target, optimization_recommendations=recommendations, detailed_metrics={'operations_tested': 3, 'charts_generated': len(charts) if 'charts' in locals() else 0})
    ...
    async def _validate_system_integration(self) -> PerformanceValidationResult:
        ...
        return PerformanceValidationResult(test_name='system_integration', duration_ms=duration_ms, memory_usage_mb=0, cpu_utilization_percent=0, meets_target=meets_target, target_ms=target, optimization_recommendations=recommendations, detailed_metrics={'pipeline_stages': 3, 'end_to_end_latency': duration_ms})
    ...
    async def _validate_performance_under_load(self) -> PerformanceValidationResult:
        ...
        target = self.performance_targets['baseline_collection'] * 2
        ...
        return PerformanceValidationResult(test_name='performance_under_load', duration_ms=avg_duration, memory_usage_mb=0, cpu_utilization_percent=0, meets_target=meets_target, target_ms=target, optimization_recommendations=recommendations, detailed_metrics={'concurrent_operations': 10, 'max_duration_ms': max_duration, 'success_rate': success_rate, 'load_factor': 2.0})
    ...
    async def _validate_memory_efficiency(self) -> PerformanceValidationResult:
        ...
        while time.time() - start_time < 60:
            try:
                ...
                logger.error('Memory test collection failed: %s', e)
        tracemalloc.stop()
        memory_growth = max_memory - initial_memory
        collections_per_mb = collections / max(memory_growth, 1)
        meets_target = memory_growth < 100
        recommendations = []
        ...
        return PerformanceValidationResult(test_name='memory_efficiency', duration_ms=60000, memory_usage_mb=memory_growth, cpu_utilization_percent=0, meets_target=meets_target, target_ms=float('inf'), optimization_recommendations=recommendations, detailed_metrics={'initial_memory_mb': initial_memory, 'final_memory_mb': max_memory, 'memory_growth_mb': memory_growth, 'collections_performed': collections, 'collections_per_mb': collections_per_mb})
    ...
    async def _generate_efficiency_report(self) -> SystemEfficiencyReport:
        ...
        if success_rate >= 0.9:
            ...
            grade = 'F'
        production_readiness = collection_result.meets_target and analysis_result.meets_target and dashboard_result.meets_target and memory_result.meets_target and load_result.meets_target
        priorities = []
        for result in self.validation_results:
            if not result.meets_target:
                priorities.extend(result.optimization_recommendations)
        priorities = list(dict.fromkeys(priorities))[:5]
        return SystemEfficiencyReport(baseline_collection_overhead=collection_result.duration_ms, analysis_processing_time=analysis_result.duration_ms, dashboard_response_time=dashboard_result.duration_ms, memory_efficiency_score=memory_efficiency, cpu_efficiency_score=cpu_efficiency, overall_efficiency_grade=grade, production_readiness=production_readiness, optimization_priorities=priorities)
    ...
    def export_validation_report(self, filename: str=None) -> str:
        ...
        report_data = {'validation_timestamp': datetime.now(UTC).isoformat(), 'validation_results': [{'test_name': r.test_name, 'duration_ms': r.duration_ms, 'memory_usage_mb': r.memory_usage_mb, 'cpu_utilization_percent': r.cpu_utilization_percent, 'meets_target': r.meets_target, 'target_ms': r.target_ms, 'optimization_recommendations': r.optimization_recommendations, 'detailed_metrics': r.detailed_metrics} for r in self.validation_results], 'performance_targets': self.performance_targets, 'summary': {'total_tests': len(self.validation_results), 'passed_tests': sum((1 for r in self.validation_results if r.meets_target)), 'overall_success_rate': sum((1 for r in self.validation_results if r.meets_target)) / len(self.validation_results) if self.validation_results else 0}}
        try:
            with open(filename, 'w') as f:
                json.dump(report_data, f, indent=2, default=str)
            logger.info('Validation report exported to %s', filename)
            return filename
        except Exception as e:
            logger.error('Failed to export validation report: %s', e)
            return ''
_validation_suite: PerformanceValidationSuite | None = None
