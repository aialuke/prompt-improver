...
class PerformanceDashboard:
        ...
        while self.running:
            try:
                ...
                if baseline.response_times:
                    avg_response = sum(baseline.response_times) / len(
                ...
                if baseline.cpu_utilization:
                    avg_cpu = sum(baseline.cpu_utilization) / len(
                ...
                if baseline.memory_utilization:
                    avg_memory = sum(baseline.memory_utilization) / len(
                        baseline.memory_utilization
                    )
                ...
                if baseline.throughput_values:
                    avg_throughput = sum(baseline.throughput_values) / len(
                ...
                for key in self.real_time_data:
                    if len(self.real_time_data[key]) > self.config.max_data_points:
                        self.real_time_data[key] = self.real_time_data[key][
    ...
    async def _start_websocket_server(self, port: int) -> None:
        ...
        try:
            await websockets.serve(
                handle_websocket, os.getenv("DASHBOARD_HOST", "0.0.0.0"), port
            )
    ...
    async def _broadcast_real_time_update(self) -> None:
        ...
        update_data = {
            ...
            "data": {
                "latest_metrics": {
                    "response_time": self.real_time_data["response_times"][-1]
                    if self.real_time_data["response_times"]
                    else 0,
                    "cpu_utilization": self.real_time_data["cpu_utilization"][-1]
                    if self.real_time_data["cpu_utilization"]
                    else 0,
                    "memory_utilization": self.real_time_data["memory_utilization"][-1]
                    if self.real_time_data["memory_utilization"]
                    else 0,
                    "error_rate": self.real_time_data["error_rates"][-1]
                    if self.real_time_data["error_rates"]
                    else 0,
                    "throughput": self.real_time_data["throughput"][-1]
                    if self.real_time_data["throughput"]
                    else 0,
                },
    ...
    def get_current_status(self) -> dict[str, Any]:
        ...
        if len(self.active_alerts) > 0:
            health_status = (
                "critical"
                if any(
                    alert.severity.value == "critical" for alert in self.active_alerts
                )
                else "warning"
            )
        return {
