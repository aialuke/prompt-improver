...
from prompt_improver.performance.baseline.models import BaselineMetrics, PerformanceTrend, RegressionAlert, get_metric_definition
...
class ReportConfig:
    ...
    def __init__(self, output_directory: Path | None=None, include_charts: bool=True, include_capacity_planning: bool=True, include_recommendations: bool=True, chart_format: str='png', chart_dpi: int=150, report_formats: list[str]=None, capacity_forecast_days: int=30, performance_sla_targets: dict[str, float] | None=None):
        ...
        self.performance_sla_targets = performance_sla_targets or {'response_time_p95': 200.0, 'error_rate': 1.0, 'cpu_utilization': 70.0, 'memory_utilization': 80.0, 'availability': 99.9}

class PerformanceReport:
    """Comprehensive performance report."""

    def __init__(self, report_id: str, report_type: str, generation_time: datetime, time_period: tuple[datetime, datetime]):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'report_id': self.report_id, 'report_type': self.report_type, 'generation_time': self.generation_time.isoformat(), 'time_period': {'start': self.time_period[0].isoformat(), 'end': self.time_period[1].isoformat()}, 'executive_summary': self.executive_summary, 'performance_overview': self.performance_overview, 'trend_analysis': self.trend_analysis, 'capacity_planning': self.capacity_planning, 'alerts_summary': self.alerts_summary, 'recommendations': self.recommendations, 'appendices': self.appendices, 'charts': self.charts, 'data_files': self.data_files}
...
class BaselineReporter:
    ...
    def __init__(self, config: ReportConfig | None=None, collector: BaselineCollector | None=None, analyzer: StatisticalAnalyzer | None=None, detector: RegressionDetector | None=None):
        ...
        logger.info('BaselineReporter initialized (charts: %s)', self.config.include_charts)

    async def generate_daily_report(self, report_date: datetime | None=None) -> PerformanceReport:
        ...
        return await self._generate_report(report_type='daily', time_period=(start_time, end_time), comparison_period_days=7)

    async def generate_weekly_report(self, week_start: datetime | None=None) -> PerformanceReport:
        ...
        if week_start is None:
            ...
            week_start = datetime.combine(today - timedelta(days=days_since_monday + 7), datetime.min.time()).replace(tzinfo=UTC)
        end_time = week_start + timedelta(days=7)
        return await self._generate_report(report_type='weekly', time_period=(week_start, end_time), comparison_period_days=28)

    async def generate_monthly_report(self, month_start: datetime | None=None) -> PerformanceReport:
        ...
        return await self._generate_report(report_type='monthly', time_period=(month_start, end_time), comparison_period_days=90)

    async def generate_capacity_planning_report(self, forecast_days: int | None=None) -> PerformanceReport:
        ...
        report = await self._generate_report(report_type='capacity_planning', time_period=(start_time, end_time), comparison_period_days=60)
        await self._add_detailed_capacity_analysis(report, forecast_days)
        return report

    async def _generate_report(self, report_type: str, time_period: tuple[datetime, datetime], comparison_period_days: int) -> PerformanceReport:
        """Generate a performance report for the specified period."""
        report_id = str(uuid.uuid4())
        generation_time = datetime.now(UTC)
        logger.info('Generating %s report for period %s to %s', report_type, time_period[0], time_period[1])
        report = PerformanceReport(report_id=report_id, report_type=report_type, generation_time=generation_time, time_period=time_period)
        ...
        if not baselines:
            logger.warning('No baselines found for period %s', time_period)
            report.executive_summary['status'] = 'insufficient_data'
            return report
        comparison_start = time_period[0] - timedelta(days=comparison_period_days)
        comparison_baselines = await self._load_baselines_for_period(comparison_start, int((time_period[0] - comparison_start).total_seconds() / 3600))
        ...
        logger.info('Generated {report_type} report: %s', report_id)
        return report

    async def _load_baselines_for_period(self, start_time: datetime, hours: int) -> list[BaselineMetrics]:
        """Load baselines for a specific time period."""
        return await self.collector.load_recent_baselines(hours)

    async def _generate_executive_summary(self, report: PerformanceReport, baselines: list[BaselineMetrics], comparison_baselines: list[BaselineMetrics]) -> None:
        ...
        try:
            ...
            logger.error('Failed to calculate performance score: %s', e)
            overall_grade = 'N/A'
            overall_score = 0
        ...
        report.executive_summary = {'status': 'generated', 'period_summary': {'total_baselines': len(baselines), 'baseline_frequency_hours': self._calculate_baseline_frequency(baselines)}, 'performance_grade': overall_grade, 'performance_score': overall_score, 'sla_compliance': sla_compliance, 'key_trends': key_trends, 'active_issues': {'total_alerts': len(active_alerts), 'critical_alerts': len(critical_alerts), 'top_issues': [alert.metric_name for alert in critical_alerts[:3]]}, 'recommendations_count': 0}

    async def _generate_performance_overview(self, report: PerformanceReport, baselines: list[BaselineMetrics]) -> None:
        ...
        for metric_name, values in aggregated_metrics.items():
            if values:
                metrics_stats[metric_name] = {'count': len(values), 'mean': statistics.mean(values), 'median': statistics.median(values), 'min': min(values), 'max': max(values), 'p95': self._percentile(values, 95), 'p99': self._percentile(values, 99), 'std_dev': statistics.stdev(values) if len(values) > 1 else 0}
        ...
        report.performance_overview = {'metrics_statistics': metrics_stats, 'resource_analysis': resource_analysis, 'performance_distribution': performance_distribution, 'baseline_quality': {'completeness_score': self._calculate_completeness_score(baselines), 'consistency_score': self._calculate_consistency_score(baselines)}}

    async def _generate_trend_analysis(self, report: PerformanceReport, baselines: list[BaselineMetrics], comparison_baselines: list[BaselineMetrics]) -> None:
        """Generate trend analysis section."""
        trends = {}
        key_metrics = ['response_time', 'error_rate', 'throughput', 'cpu_utilization', 'memory_utilization']
        for metric_name in key_metrics:
            try:
                trend = await self.analyzer.analyze_trend(metric_name, baselines, timeframe_hours=len(baselines))
                trends[metric_name] = {'direction': trend.direction.value, 'magnitude': trend.magnitude, 'confidence_score': trend.confidence_score, 'is_significant': trend.is_significant(), 'sample_count': trend.sample_count, 'predicted_24h': trend.predicted_value_24h, 'predicted_7d': trend.predicted_value_7d}
            except Exception as e:
                logger.error('Failed to analyze trend for {metric_name}: %s', e)
                trends[metric_name] = {'error': str(e)}
        significant_trends = [(metric, data) for metric, data in trends.items() if isinstance(data, dict) and data.get('is_significant', False)]
        significant_trends.sort(key=lambda x: abs(x[1].get('magnitude', 0)), reverse=True)
        report.trend_analysis = {'all_trends': trends, 'significant_trends': dict(significant_trends[:5]), 'trend_summary': {'improving_metrics': len([t for t in trends.values() if isinstance(t, dict) and t.get('direction') == 'improving']), 'degrading_metrics': len([t for t in trends.values() if isinstance(t, dict) and t.get('direction') == 'degrading']), 'stable_metrics': len([t for t in trends.values() if isinstance(t, dict) and t.get('direction') == 'stable'])}}

    async def _generate_capacity_planning(self, report: PerformanceReport, baselines: list[BaselineMetrics]) -> None:
        ...
        report.capacity_planning = {'resource_forecasts': resource_forecasts, 'capacity_recommendations': capacity_recommendations, 'growth_analysis': growth_analysis, 'scaling_recommendations': self._generate_scaling_recommendations(resource_forecasts)}

    async def _generate_alerts_summary(self, report: PerformanceReport, time_period: tuple[datetime, datetime]) -> None:
        ...
        top_problem_metrics = sorted(alerts_by_metric.items(), key=lambda x: x[1], reverse=True)[:5]
        report.alerts_summary = {'total_active_alerts': len(active_alerts), 'alerts_by_severity': alerts_by_severity, 'alerts_by_metric': alerts_by_metric, 'top_problem_metrics': dict(top_problem_metrics), 'alert_statistics': alert_stats}

    async def _generate_recommendations(self, report: PerformanceReport, baselines: list[BaselineMetrics]) -> None:
        """Generate actionable recommendations."""
        recommendations = []
        sla_violations = self._identify_sla_violations(baselines)
        for metric, violation_data in sla_violations.items():
            recommendation = {'id': str(uuid.uuid4()), 'category': 'sla_compliance', 'priority': 'high' if violation_data['severity'] > 0.5 else 'medium', 'title': f"Address {metric.replace('_', ' ').title()} SLA Violations", 'description': f"The {metric} metric is exceeding SLA targets by {violation_data['excess_percentage']:.1f}%", 'action_items': self._generate_sla_action_items(metric, violation_data), 'expected_impact': violation_data['impact_assessment'], 'implementation_effort': 'medium'}
    ...
    async def _generate_charts(self, report: PerformanceReport, baselines: list[BaselineMetrics]) -> None:
        ...
        if any((b.response_times for b in baselines)):
            chart_path = await self._create_time_series_chart(timestamps, [statistics.mean(b.response_times) if b.response_times else 0 for b in baselines], 'Response Time Trend', 'Response Time (ms)', str(charts_dir / f'response_time.{self.config.chart_format}'))
        ...
        if any(cpu_data) or any(memory_data):
            chart_path = await self._create_dual_line_chart(timestamps, cpu_data, memory_data, 'Resource Utilization', 'CPU Usage (%)', 'Memory Usage (%)', str(charts_dir / f'resource_utilization.{self.config.chart_format}'))
        ...
        if any(error_data):
            chart_path = await self._create_time_series_chart(timestamps, error_data, 'Error Rate Trend', 'Error Rate (%)', str(charts_dir / f'error_rate.{self.config.chart_format}'))
            report.charts['error_rate'] = str(chart_path)

    async def _create_time_series_chart(self, timestamps: list[datetime], values: list[float], title: str, ylabel: str, output_path: str) -> Path:
    ...
    async def _create_dual_line_chart(self, timestamps: list[datetime], values1: list[float], values2: list[float], title: str, ylabel1: str, ylabel2: str, output_path: str) -> Path:
    ...
    def _generate_html_report(self, report: PerformanceReport) -> str:
        ...
        if self.jinja_env:
            try:
                ...
                logger.warning('Failed to use Jinja template: %s', e)
    ...
    def _generate_text_report(self, report: PerformanceReport) -> str:
        ...
        lines = [f'PERFORMANCE REPORT - {report.report_type.upper()}', '=' * 60, f'Report ID: {report.report_id}', f"Generated: {report.generation_time.strftime('%Y-%m-%d %H:%M:%S UTC')}", f"Period: {report.time_period[0].strftime('%Y-%m-%d %H:%M')} to {report.time_period[1].strftime('%Y-%m-%d %H:%M')}", '', 'EXECUTIVE SUMMARY', '-' * 20, f"Performance Grade: {report.executive_summary.get('performance_grade', 'N/A')}", f"Active Issues: {report.executive_summary.get('active_issues', {}).get('total_alerts', 0)} alerts", '', 'RECOMMENDATIONS', '-' * 15]
        for i, rec in enumerate(report.recommendations, 1):
            lines.extend([f"{i}. {rec['title']}", f"   Priority: {rec['priority'].upper()}", f"   Description: {rec['description']}", ''])
        return ''.join(lines)
    ...
    def _aggregate_baseline_metrics(self, baselines: list[BaselineMetrics]) -> dict[str, list[float]]:
        ...
        aggregated = {'response_time': [], 'error_rate': [], 'throughput': [], 'cpu_utilization': [], 'memory_utilization': []}
        ...
        for metric_name, target in self.config.performance_sla_targets.items():
            if metric_name.replace('_p95', '') in aggregated:
                ...
                if values:
                    ...
                    compliance[metric_name] = {'target': target, 'current_value': current_value, 'compliance_percentage': compliance_percentage, 'is_compliant': compliance_percentage >= 95.0}
        return compliance
...
async def generate_daily_performance_report(date: datetime | None=None) -> PerformanceReport:
    """Generate a daily performance report."""
    reporter = get_baseline_reporter()
    return await reporter.generate_daily_report(date)

async def generate_weekly_performance_report(week_start: datetime | None=None) -> PerformanceReport:
    """Generate a weekly performance report."""
    reporter = get_baseline_reporter()
    return await reporter.generate_weekly_report(week_start)

async def generate_capacity_planning_report(forecast_days: int | None=None) -> PerformanceReport:
