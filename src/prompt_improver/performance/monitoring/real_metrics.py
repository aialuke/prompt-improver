...
class InMemoryHistogram:
    ...
    def __init__(self, name: str, description: str, label_names: list[str] | None=None, buckets: list[float] | None=None) -> None:
        self.name = name
        self.description = description
        self.label_names = label_names or []
...
class RealMetricsRegistry:
    ...
    def get_or_create_histogram(self, name, description, labels=None, buckets=None, registry=None):
        """Get or create a histogram."""
        if name not in self.histograms:
            self.histograms[name] = InMemoryHistogram(name, description, labels, buckets)
        return self.histograms[name]

    def get_metrics_summary(self):
        """Get summary of all metrics."""
        return {'counters': {name: counter.get_all_values() for name, counter in self.counters.items()}, 'gauges': {name: gauge.get_all_values() for name, gauge in self.gauges.items()}, 'histograms': {name: len(hist.get_observations()) for name, hist in self.histograms.items()}}
