...
from prompt_improver.performance.monitoring.health.metrics import instrument_health_check
...
class MLModelHealthChecker(HealthChecker):
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            ...
            all_checks = [model_registry_health, deployed_models_health, model_cache_health, model_performance_health]
            ...
            return HealthResult(status=status, component=self.name, response_time_ms=response_time, message=message, details={'model_registry': model_registry_health, 'deployed_models': deployed_models_health, 'model_cache': model_cache_health, 'model_performance': model_performance_health, 'total_checks': len(all_checks), 'failed_checks': len(failed_checks), 'warning_checks': len(warning_checks)}, timestamp=start_time)
        except Exception as e:
            end_time = datetime.now(UTC)
            response_time = (end_time - start_time).total_seconds() * 1000
            self.logger.error('ML model health check failed: %s', e)
            return HealthResult(status=HealthStatus.FAILED, component=self.name, response_time_ms=response_time, message=f'ML model health check failed: {e!s}', error=str(e), timestamp=start_time)

    async def _check_model_registry(self) -> dict[str, Any]:
        """Check the health of the model registry."""
        try:
            from prompt_improver.ml.models.production_registry import ProductionModelRegistry
            registry = ProductionModelRegistry()
            models = await registry.list_models()
            return {'healthy': True, 'total_models': len(models), 'registry_accessible': True}
        except Exception as e:
    ...
    async def _check_deployed_models(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'healthy': True, 'cached_models': cache_stats.get('total_models', 0), 'cache_size_mb': cache_stats.get('total_size_mb', 0), 'cache_hit_rate': cache_stats.get('hit_rate', 0.0)}
        except Exception as e:
    ...
    async def _check_model_cache(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'healthy': not memory_pressure, 'warning': memory_pressure, 'cache_utilization': cache_utilization, 'memory_pressure': memory_pressure, 'evictions': cache_stats.get('evictions', 0)}
        except Exception as e:
    ...
    async def _check_model_performance(self) -> dict[str, Any]:
        ...
        try:
            return {'healthy': True, 'performance_monitoring': 'active', 'metrics_available': True}
        except Exception as e:
            return {'healthy': False, 'error': str(e)}
...
class MLDataQualityChecker(HealthChecker):
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            ...
            return HealthResult(status=status, component=self.name, response_time_ms=response_time, message=message, details={'training_data': training_data_health, 'synthetic_data': synthetic_data_health, 'preprocessing': preprocessing_health, 'total_checks': len(all_checks), 'failed_checks': len(failed_checks), 'warning_checks': len(warning_checks)}, timestamp=start_time)
        except Exception as e:
            end_time = datetime.now(UTC)
            response_time = (end_time - start_time).total_seconds() * 1000
            self.logger.error('ML data quality health check failed: %s', e)
            return HealthResult(status=HealthStatus.FAILED, component=self.name, response_time_ms=response_time, message=f'ML data quality health check failed: {e!s}', error=str(e), timestamp=start_time)

    async def _check_training_data(self) -> dict[str, Any]:
        """Check the health of training data loader."""
        try:
            from prompt_improver.ml.core.training_data_loader import get_training_data_stats
            ...
            return {'healthy': len(data_quality_issues) == 0, 'warning': len(data_quality_issues) > 0, 'data_quality_issues': data_quality_issues, 'total_samples': stats.get('total_samples', 0), 'missing_values_ratio': stats.get('missing_values_ratio', 0), 'duplicate_ratio': stats.get('duplicate_ratio', 0)}
        except Exception as e:
            return {'healthy': False, 'error': str(e)}
    ...
    async def _check_synthetic_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'healthy': True, 'generator_available': True, 'generation_method': generator.generation_method}
        except Exception as e:
...
class MLTrainingHealthChecker(HealthChecker):
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            ...
            return HealthResult(status=status, component=self.name, response_time_ms=response_time, message=message, details={'optimization': optimization_health, 'batch_processing': batch_processing_health, 'learning': learning_health, 'total_checks': len(all_checks), 'failed_checks': len(failed_checks), 'warning_checks': len(warning_checks)}, timestamp=start_time)
        except Exception as e:
            end_time = datetime.now(UTC)
            response_time = (end_time - start_time).total_seconds() * 1000
            self.logger.error('ML training health check failed: %s', e)
            return HealthResult(status=HealthStatus.FAILED, component=self.name, response_time_ms=response_time, message=f'ML training health check failed: {e!s}', error=str(e), timestamp=start_time)

    async def _check_optimization_algorithms(self) -> dict[str, Any]:
        """Check the health of optimization algorithms."""
        try:
            optimization_components = ['RuleOptimizer', 'MultiarmedBanditFramework', 'ClusteringOptimizer', 'AdvancedDimensionalityReducer']
            available_components = []
            for component in optimization_components:
                try:
                    if component == 'RuleOptimizer' or component == 'ClusteringOptimizer':
                        available_components.append(component)
                except ImportError:
                    pass
            return {'healthy': len(available_components) > 0, 'available_components': available_components, 'total_components': len(optimization_components)}
        except Exception as e:
            return {'healthy': False, 'error': str(e)}
    ...
    async def _check_batch_processing(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'healthy': False, 'error': str(e), 'batch_processor_available': False}

    async def _check_learning_algorithms(self) -> dict[str, Any]:
        """Check the health of learning algorithms."""
        try:
            learning_components = ['ContextLearner', 'InsightGenerationEngine', 'FailureModeAnalyzer']
            available_components = []
            for component in learning_components:
                try:
                    if component == 'ContextLearner' or component == 'InsightGenerationEngine' or component == 'FailureModeAnalyzer':
                        available_components.append(component)
                except ImportError:
                    pass
            return {'healthy': len(available_components) > 0, 'available_components': available_components, 'total_components': len(learning_components)}
        except Exception as e:
            return {'healthy': False, 'error': str(e)}
...
class MLPerformanceHealthChecker(HealthChecker):
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            ...
            return HealthResult(status=status, component=self.name, response_time_ms=response_time, message=message, details={'evaluation': evaluation_health, 'monitoring': monitoring_health, 'analytics': analytics_health, 'total_checks': len(all_checks), 'failed_checks': len(failed_checks), 'warning_checks': len(warning_checks)}, timestamp=start_time)
        except Exception as e:
            end_time = datetime.now(UTC)
            response_time = (end_time - start_time).total_seconds() * 1000
            self.logger.error('ML performance health check failed: %s', e)
            return HealthResult(status=HealthStatus.FAILED, component=self.name, response_time_ms=response_time, message=f'ML performance health check failed: {e!s}', error=str(e), timestamp=start_time)

    async def _check_evaluation_components(self) -> dict[str, Any]:
        """Check the health of evaluation components."""
        try:
            evaluation_components = ['CausalInferenceAnalyzer', 'AdvancedStatisticalValidator', 'PatternSignificanceAnalyzer']
            available_components = []
            for component in evaluation_components:
                try:
                    if component == 'CausalInferenceAnalyzer' or component == 'AdvancedStatisticalValidator' or component == 'PatternSignificanceAnalyzer':
                        available_components.append(component)
                except ImportError:
                    pass
            return {'healthy': len(available_components) > 0, 'available_components': available_components, 'total_components': len(evaluation_components)}
        except Exception as e:
            return {'healthy': False, 'error': str(e)}
    ...
    async def _check_performance_monitoring(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'healthy': False, 'error': str(e), 'performance_monitor_available': False}
