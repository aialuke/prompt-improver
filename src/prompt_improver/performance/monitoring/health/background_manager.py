...
class EnhancedBackgroundTaskManager:
    ...
    async def submit_enhanced_task(
        ...
        async with self._lock:
            if task_id in self._tasks:
                return task_id
            status = TaskStatus(
                task_id=task_id, status="queued", created_at=datetime.now(UTC)
            )
            self._tasks[task_id] = status
            heapq.heappush(
                self._task_queue, (-priority.value, time.time(), task_id, coroutine)
            )
    ...
    async def get_task_status(self, task_id: str) -> TaskStatus:
        ...
        return self._tasks.get(
            task_id,
            TaskStatus(
                task_id=task_id, status="not_found", created_at=datetime.now(UTC)
            ),
        )
