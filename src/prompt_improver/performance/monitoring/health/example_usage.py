...
from prompt_improver.performance.monitoring.health.health_config import EnvironmentType, create_category_config, get_health_config
from prompt_improver.performance.monitoring.health.plugin_adapters import create_all_plugins, register_all_plugins
from prompt_improver.performance.monitoring.health.unified_health_system import HealthCheckCategory, HealthCheckPlugin, HealthCheckPluginConfig, create_simple_health_plugin, get_unified_health_monitor
...
async def basic_usage_example():
    ...
    simple_plugin = create_simple_health_plugin(name='simple_service', category=HealthCheckCategory.CUSTOM, check_func=simple_check, config=HealthCheckPluginConfig(timeout_seconds=5.0))
...
async def custom_plugin_example():
    ...
    class CustomDatabasePlugin(HealthCheckPlugin):
        ...
        def __init__(self):
            config = create_category_config(HealthCheckCategory.DATABASE, critical=True, timeout_seconds=8.0)
            super().__init__(name='custom_database', category=HealthCheckCategory.DATABASE, config=config)
        ...
        async def execute_check(self) -> HealthCheckResult:
            ...
            if duration > 100:
                return HealthCheckResult(status=HealthStatus.DEGRADED, message=f'Database slow: {duration:.1f}ms', details={'duration_ms': duration, 'threshold_ms': 100}, check_name=self.name)
            return HealthCheckResult(status=HealthStatus.HEALTHY, message=f'Database healthy: {duration:.1f}ms', details={'duration_ms': duration}, check_name=self.name)
    monitor = get_unified_health_monitor()
...
async def performance_validation_example():
    ...
    for i in range(5):
        ...
        def quick_check():
            return True
        plugin = create_simple_health_plugin(name=f'test_plugin_{i}', category=HealthCheckCategory.SYSTEM, check_func=quick_check, config=HealthCheckPluginConfig(timeout_seconds=1.0))
