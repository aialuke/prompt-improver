    ...
    async def check(self) -> HealthResult:
        ...
        try:
            if not self._orchestrator:
                return HealthResult(status=HealthStatus.WARNING, component=self.name, message='ML orchestrator not initialized', details={}, timestamp=start_time, response_time_ms=0.0)
            if not self._orchestrator._is_initialized:
                return HealthResult(status=HealthStatus.FAILED, component=self.name, message='ML orchestrator not initialized', details={'state': 'uninitialized'}, timestamp=start_time, response_time_ms=0.0)
            ...
            return HealthResult(status=status, component=self.name, message=message, details={'total_components': total_components, 'healthy_components': healthy_components, 'health_percentage': health_percentage, 'active_workflows': active_workflows, 'resource_usage': resource_usage, 'orchestrator_state': self._orchestrator.state.value if hasattr(self._orchestrator, 'state') else 'unknown'}, timestamp=start_time, response_time_ms=response_time * 1000)
        except Exception as e:
            end_time = datetime.now(UTC)
            response_time = (end_time - start_time).total_seconds()
            self.logger.error('ML orchestrator health check failed: %s', e)
            return HealthResult(status=HealthStatus.FAILED, component=self.name, message=f'Health check failed: {e!s}', details={'error': str(e)}, timestamp=start_time, response_time_ms=response_time * 1000)
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            if not self._registry:
                return HealthResult(status=HealthStatus.WARNING, component=self.name, message='Component registry not initialized', details={}, timestamp=start_time, response_time_ms=0.0)
            ...
            return HealthResult(status=status, component=self.name, message=message, details={'health_summary': health_summary, 'total_registered_components': len(components), 'registry_response_time': response_time}, timestamp=start_time, response_time_ms=response_time * 1000)
        except Exception as e:
            end_time = datetime.now(UTC)
            response_time = (end_time - start_time).total_seconds()
            self.logger.error('Component registry health check failed: %s', e)
            return HealthResult(status=HealthStatus.FAILED, component=self.name, message=f'Health check failed: {e!s}', details={'error': str(e)}, timestamp=start_time, response_time_ms=response_time * 1000)
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            if not self._resource_manager:
                return HealthResult(status=HealthStatus.WARNING, component=self.name, message='Resource manager not initialized', details={}, timestamp=start_time, response_time_ms=0.0)
            ...
            return HealthResult(status=status, component=self.name, message=message, details={'usage_stats': usage_stats, 'total_allocations': len(allocations), 'critical_resources': critical_resources, 'warning_resources': warning_resources, 'monitoring_active': self._resource_manager.is_monitoring}, timestamp=start_time, response_time_ms=response_time * 1000)
        except Exception as e:
            end_time = datetime.now(UTC)
            response_time = (end_time - start_time).total_seconds()
            self.logger.error('Resource manager health check failed: %s', e)
            return HealthResult(status=HealthStatus.FAILED, component=self.name, message=f'Health check failed: {e!s}', details={'error': str(e)}, timestamp=start_time, response_time_ms=response_time * 1000)
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            if not self._workflow_engine:
                return HealthResult(status=HealthStatus.WARNING, component=self.name, message='Workflow engine not initialized', details={}, timestamp=start_time, response_time_ms=0.0)
            ...
            return HealthResult(status=status, component=self.name, message=message, details={'workflow_definitions': len(workflow_definitions), 'active_executors': active_executors, 'failed_workflows': len(failed_workflows), 'available_workflow_types': workflow_definitions}, timestamp=start_time, response_time_ms=response_time * 1000)
        except Exception as e:
            end_time = datetime.now(UTC)
            response_time = (end_time - start_time).total_seconds()
            self.logger.error('Workflow engine health check failed: %s', e)
            return HealthResult(status=HealthStatus.FAILED, component=self.name, message=f'Health check failed: {e!s}', details={'error': str(e)}, timestamp=start_time, response_time_ms=response_time * 1000)
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            if not self._event_bus:
                return HealthResult(status=HealthStatus.WARNING, component=self.name, message='Event bus not initialized', details={}, timestamp=start_time, response_time_ms=0.0)
            ...
            return HealthResult(status=status, component=self.name, message=message, details={'event_statistics': stats, 'test_emission_time': test_response_time, 'failure_rate': failure_rate}, timestamp=start_time, response_time_ms=response_time * 1000)
        except Exception as e:
            end_time = datetime.now(UTC)
            response_time = (end_time - start_time).total_seconds()
            self.logger.error('Event bus health check failed: %s', e)
            return HealthResult(status=HealthStatus.FAILED, component=self.name, message=f'Health check failed: {e!s}', details={'error': str(e)}, timestamp=start_time, response_time_ms=response_time * 1000)
