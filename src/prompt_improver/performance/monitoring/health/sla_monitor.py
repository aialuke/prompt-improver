...
class SLATarget:
    ...
    def from_global_config(cls, name: str, description: str, target_value: float, unit: str) -> 'SLATarget':
        ...
        return cls(name=name, description=description, target_value=target_value, unit=unit, measurement_window_seconds=sla_config.measurement_window_seconds, warning_threshold=sla_config.warning_threshold, critical_threshold=sla_config.critical_threshold, alert_on_breach=True, alert_cooldown_seconds=sla_config.alert_cooldown_seconds)
...
class SLAConfiguration:
    ...
    def from_global_config(cls, service_name: str) -> 'SLAConfiguration':
        ...
        config = get_config()
        sla_config = config.sla_monitoring
        return cls(service_name=service_name, response_time_p50_ms=sla_config.response_time_p50_ms, response_time_p95_ms=sla_config.response_time_p95_ms, response_time_p99_ms=sla_config.response_time_p99_ms, availability_target=sla_config.availability_target, error_rate_threshold=sla_config.error_rate_threshold, custom_targets=[])
...
class SLAMeasurement:
    ...
    def get_current_value(self) -> float | None:
        ...
        recent_values = [value for timestamp, value in self.measurements if window_start <= timestamp <= current_time]
    ...
    def get_compliance_status(self) -> SLAStatus:
        ...
        is_higher_better = 'availability' in self.target.name.lower() or 'success' in self.target.name.lower() or 'uptime' in self.target.name.lower() or ('throughput' in self.target.name.lower()) or ('hit_rate' in self.target.name.lower()) or ('accuracy' in self.target.name.lower())
        if is_higher_better:
...
class SLAMonitor:
    ...
    def __init__(self, service_name: str, config: SLAConfiguration, on_sla_breach: Callable[[str, SLATarget, float], None] | None=None):
    ...
    def _initialize_standard_slas(self):
        ...
        self.measurements['response_time_p50'] = SLAMeasurement(SLATarget(name='response_time_p50', description='50th percentile response time', target_value=self.config.response_time_p50_ms, unit='ms'))
        self.measurements['response_time_p95'] = SLAMeasurement(SLATarget(name='response_time_p95', description='95th percentile response time', target_value=self.config.response_time_p95_ms, unit='ms'))
        self.measurements['response_time_p99'] = SLAMeasurement(SLATarget(name='response_time_p99', description='99th percentile response time', target_value=self.config.response_time_p99_ms, unit='ms'))
        self.measurements['availability'] = SLAMeasurement(SLATarget(name='availability', description='Service availability', target_value=self.config.availability_target, unit='percent'))
        self.measurements['error_rate'] = SLAMeasurement(SLATarget(name='error_rate', description='Error rate', target_value=self.config.error_rate_threshold, unit='percent'))

    def record_health_check(self, success: bool, response_time_ms: float, custom_metrics: dict[str, float] | None=None):
    ...
    def _check_sla_compliance(self):
        ...
        for name, measurement in self.measurements.items():
            if measurement.should_alert():
                ...
                logger.warning('SLA breach detected for %s', self.service_name, extra={'service': self.service_name, 'sla_target': name, 'target_value': measurement.target.target_value, 'current_value': current_value, 'unit': measurement.target.unit, 'status': 'breaching'})
                measurement.last_alert_time = time.time()
                if self.on_sla_breach:
                    self.on_sla_breach(self.service_name, measurement.target, current_value)

    def get_sla_report(self) -> dict[str, Any]:
        """Generate comprehensive SLA compliance report"""
        report = {'service': self.service_name, 'timestamp': time.time(), 'uptime_seconds': time.time() - self._start_time, 'total_checks': self._check_count, 'successful_checks': self._success_count, 'overall_availability': self._success_count / max(1, self._check_count), 'sla_targets': {}}
        for name, measurement in self.measurements.items():
            current_value = measurement.get_current_value()
            status = measurement.get_compliance_status()
            report['sla_targets'][name] = {'description': measurement.target.description, 'target_value': measurement.target.target_value, 'current_value': current_value, 'unit': measurement.target.unit, 'status': status.value, 'compliance_ratio': current_value / measurement.target.target_value if current_value is not None else None}
    ...
    def get_sla_metrics_for_export(self) -> dict[str, float]:
        ...
        for name, measurement in self.measurements.items():
            ...
            if current_value is not None:
                ...
                status = measurement.get_compliance_status()
                status_map = {SLAStatus.MEETING: 0, SLAStatus.AT_RISK: 1, SLAStatus.BREACHING: 2}
                metrics[f'sla_{name}_status'] = status_map[status]
        return metrics
sla_monitors: dict[str, SLAMonitor] = {}

def get_or_create_sla_monitor(service_name: str, config: SLAConfiguration | None=None) -> SLAMonitor:
