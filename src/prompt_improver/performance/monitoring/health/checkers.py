...
try:
    from prompt_improver.performance.monitoring.health.redis_monitor import redis_health_monitor
...
class DatabaseHealthChecker(HealthChecker):
    ...
    async def check(self) -> HealthResult:
        ...
        if not DATABASE_AVAILABLE or get_session is None:
            return HealthResult(status=HealthStatus.WARNING, component=self.name, message='Database configuration not available', error='Database credentials not configured')
        try:
            ...
            async with get_session() as session:
                ...
                active_connections = await scalar(session, text("SELECT count(*) FROM pg_stat_activity WHERE state = 'active'"))
                ...
                return HealthResult(status=status, component=self.name, response_time_ms=response_time, message=f'Database responding in {response_time:.1f}ms', details={'long_running_queries': long_queries, 'active_connections': active_connections})
        except Exception as e:
            return HealthResult(status=HealthStatus.FAILED, component=self.name, error=str(e), message='Database connection failed')
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            try:
                ...
                return HealthResult(status=HealthStatus.WARNING, component=self.name, message='MCP server module not available', error='MCP server not configured')
            mcp_server = APESMCPServer()
            start_time = time.time()
            result = await mcp_server._improve_prompt_impl(prompt='Health check test prompt', context={'domain': 'health_check'}, session_id='health_check', rate_limit_remaining=None)
            response_time = (time.time() - start_time) * 1000
            if response_time > 500:
                ...
                status = HealthStatus.HEALTHY
            return HealthResult(status=status, component=self.name, response_time_ms=response_time, message=f'MCP server responding in {response_time:.1f}ms')
        except Exception as e:
            return HealthResult(status=HealthStatus.FAILED, component=self.name, error=str(e), message='MCP server health check failed')
...
class QueueHealthChecker(HealthChecker):
    ...
    async def check(self) -> HealthResult:
        ...
        if not QUEUE_SERVICES_AVAILABLE:
            return HealthResult(status=HealthStatus.WARNING, component=self.name, message='Queue services not configured', error='Queue or background services not available')
        try:
            start_time = time.time()
            metrics = await self._collect_queue_metrics()
            status = self._evaluate_queue_health(metrics)
            response_time = (time.time() - start_time) * 1000
            return HealthResult(status=status, component=self.name, response_time_ms=response_time, message=self._get_status_message(status, metrics), details=metrics, timestamp=datetime.now())
        except Exception as e:
            response_time = (time.time() - start_time) * 1000
            return HealthResult(status=HealthStatus.FAILED, component=self.name, error=str(e), message=f'Queue health check failed: {e!s}', response_time_ms=response_time, timestamp=datetime.now())
...
class SystemResourcesHealthChecker(HealthChecker):
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            ...
            return HealthResult(status=status, component=self.name, message=message, details={'memory_usage_percent': memory_usage_percent, 'disk_usage_percent': disk_usage_percent, 'cpu_usage_percent': cpu_percent, 'warnings': warnings, 'event_loop': loop_info})
        except ImportError:
            return HealthResult(status=HealthStatus.WARNING, component=self.name, message='psutil not available for system monitoring')
        except Exception as e:
            return HealthResult(status=HealthStatus.FAILED, component=self.name, error=str(e), message='System resource check failed')

    async def _check_event_loop_performance(self) -> dict[str, Any]:
        """Check event loop type and performance metrics using unified connection manager."""
        try:
            from prompt_improver.database.unified_connection_manager import ManagerMode, get_unified_manager
            manager = await get_unified_manager(ManagerMode.HIGH_AVAILABILITY)
            ...
            return {'loop_type': loop_type, 'uvloop_enabled': uvloop_enabled, 'avg_latency_ms': avg_latency, 'min_latency_ms': min_latency, 'max_latency_ms': max_latency, 'samples': len(latencies), 'enhanced_monitoring': True}
        except ImportError:
            return await self._fallback_event_loop_check()
        except Exception as e:
            return {'loop_type': 'unknown', 'uvloop_enabled': False, 'avg_latency_ms': 0, 'min_latency_ms': 0, 'max_latency_ms': 0, 'samples': 0, 'error': str(e), 'enhanced_monitoring': False}
    ...
    async def _fallback_event_loop_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            min_latency = min(latencies)
            max_latency = max(latencies)
            return {'loop_type': loop_type, 'uvloop_enabled': uvloop_enabled, 'avg_ms': avg_latency, 'min_ms': min_latency, 'max_ms': max_latency, 'samples': len(latencies), 'enhanced_monitoring': False}
        except Exception as e:
            return {'loop_type': 'unknown', 'uvloop_enabled': False, 'avg_ms': 0, 'min_ms': 0, 'max_ms': 0, 'samples': 0, 'error': str(e), 'enhanced_monitoring': False}
...
class RedisHealthChecker(HealthChecker):
    ...
    async def check(self) -> HealthResult:
        ...
        try:
            ...
            status_mapping = {'healthy': HealthStatus.HEALTHY, 'warning': HealthStatus.WARNING, 'critical': HealthStatus.WARNING, 'failed': HealthStatus.FAILED}
            redis_status = health_data.get('status', 'failed')
            health_status = status_mapping.get(redis_status, HealthStatus.FAILED)
            message_parts = [f'Redis {redis_status}', f'{health_data.get(response_time_ms, 0):.1f}ms latency', f'{health_data.get(memory_usage_mb, 0):.1f}MB memory', f'{health_data.get(hit_rate_percentage, 0):.1f}% hit rate']
            if health_data.get('issues'):
                message_parts.extend(health_data['issues'][:2])
            message = ', '.join(message_parts)
            details = {'redis_metrics': {'memory_usage_mb': health_data.get('memory_usage_mb', 0), 'hit_rate_percentage': health_data.get('hit_rate_percentage', 0), 'connected_clients': health_data.get('connected_clients', 0), 'total_commands': health_data.get('total_commands', 0), 'fragmentation_ratio': health_data.get('fragmentation_ratio', 1.0), 'response_time_ms': health_data.get('response_time_ms', 0)}, 'issues': health_data.get('issues', []), 'timestamp': health_data.get('timestamp')}
            return HealthResult(status=health_status, component=self.name, message=message, details=details)
        except ImportError as e:
            return await self._basic_redis_check()
        except Exception as e:
            return HealthResult(status=HealthStatus.FAILED, component=self.name, error=str(e), message=f'Redis health check failed: {e}')
    ...
    async def _basic_redis_check(self) -> HealthResult:
        ...
        try:
            ...
            if response_time_ms > 100:
                ...
                status = HealthStatus.HEALTHY
            details = {'response_time_ms': round(response_time_ms, 2), 'redis_version': info.get('redis_version'), 'connected_clients': info.get('connected_clients', 0), 'used_memory_human': info.get('used_memory_human'), 'fallback_check': True}
            return HealthResult(status=status, component=self.name, message=message, details=details)
        except Exception as e:
            return HealthResult(status=HealthStatus.FAILED, component=self.name, error=str(e), message=f'Redis connectivity failed: {e}')
