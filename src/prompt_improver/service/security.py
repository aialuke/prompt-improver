...
class PromptDataProtection:
    ...
    def __init__(self, console: Optional[Console] = None):
        ...
        self.sensitive_patterns = [
            (r'sk-[a-zA-Z0-9]{40,60}', 'openai_api_key'),  # Flexible length for real OpenAI keys
    ...
    async def sanitize_prompt_before_storage(self, prompt: str, session_id: str) -> Tuple[str, Dict[str, Any]]:
        ...
        if redactions_made and self.audit_enabled:
            await self.audit_redaction(session_id, len(redactions_made), redaction_details)
    ...
    async def audit_redaction(self, session_id: str, redaction_count: int, redaction_details: Dict[str, Any]):
        ...
        try:
            ...
            async with sessionmanager.session() as db_session:
                ...
                self.logger.info(f"Security audit logged for session {session_id}: {redaction_count} redactions")
                
        except Exception as e:
            self.logger.error(f"Failed to audit redaction for session {session_id}: {e}")
            ...
            async with sessionmanager.session() as db_session:
                ...
                security_report = {
                    ...
                    'compliance': {
                        ...
                        'redaction_rate_percent': round((audit_data[1] or 0) / total_sessions * 100, 2) if total_sessions > 0 else 0,
                        'status': 'EXCELLENT' if compliance_score >= 95 else 'GOOD' if compliance_score >= 90 else 'NEEDS_ATTENTION'
                    },
                    'redaction_patterns': {
                        'types_detected': [{'type': row[0], 'count': row[1]} for row in redaction_types],
                        'most_common': redaction_types[0][0] if redaction_types else None
                    },
...
class SecureMCPServer:
    ...
    async def validate_request(self, request_data: Dict[str, Any], client_ip: str = "127.0.0.1") -> Tuple[bool, str]:
    ...
    def get_rate_limit_status(self, client_ip: str = "127.0.0.1") -> Dict[str, Any]:
        ...
        return {
            ...
            'requests_remaining': max(0, self.config['rate_limit_calls'] - requests_used),
