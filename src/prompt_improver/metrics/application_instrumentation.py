...
class PromptImprovementInstrumentation:
    ...
    @staticmethod
    def instrument_prompt_service(service_class):
        """
...
class MLPipelineInstrumentation:
    ...
    @staticmethod
    def instrument_ml_service(service_class):
        """
        ...
        for method_name in ml_methods:
            if hasattr(service_class, method_name):
                ...
                if 'preprocess' in method_name.lower():
                    stage = ModelInferenceStage.INPUT_PREPROCESSING
                    category = FeatureCategory.ML_ANALYTICS
                elif 'postprocess' in method_name.lower():
                    stage = ModelInferenceStage.POSTPROCESSING
                    category = FeatureCategory.ML_ANALYTICS
                elif 'feature' in method_name.lower():
                    stage = ModelInferenceStage.INPUT_PREPROCESSING
                    category = FeatureCategory.ML_ANALYTICS
                else:
                    stage = ModelInferenceStage.MODEL_FORWARD
...
class APIServiceInstrumentation:
    ...
    @staticmethod
    def instrument_api_service(service_class, feature_category: FeatureCategory = FeatureCategory.API_INTEGRATION):
        """
...
class DatabaseInstrumentation:
    ...
    @staticmethod
    def instrument_database_class(db_class):
        """
        ...
        for method_name in db_methods:
            if hasattr(db_class, method_name):
                ...
                async def instrumented_method(self, query_or_sql, *args, **kwargs):
                ...
                if not inspect.iscoroutinefunction(original_method):
                    def sync_instrumented_method(self, query_or_sql, *args, **kwargs):
                        start_time = time.time()
                        table_name = "unknown"
...
class CacheInstrumentation:
    ...
    @staticmethod
    def instrument_cache_class(cache_class, cache_type: CacheType = CacheType.APPLICATION):
        """
        ...
        for method_name in cache_methods:
            if hasattr(cache_class, method_name):
                ...
                # Create instrumented method
                async def instrumented_method(self, key, *args, **kwargs):
                    start_time = time.time()
                    hit = False
                ...
                # For sync methods
                if not inspect.iscoroutinefunction(original_method):
                    def sync_instrumented_method(self, key, *args, **kwargs):
                        start_time = time.time()
                        hit = False
...
class BatchProcessingInstrumentation:
    ...
    @staticmethod
    def instrument_batch_processor(processor_class):
        """
...
def auto_instrument_module(module, instrumentation_config: Optional[Dict[str, Any]] = None):
    """
    ...
    """
    config = instrumentation_config or {}
