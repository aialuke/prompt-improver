...
def _emit_journey_event_otel(service_name: str, metric: "UserJourneyMetric") -> None:
    if not _HAS_OTEL:
        return
    try:
        from prompt_improver.monitoring.opentelemetry.metrics import get_business_metrics
...
class UserJourneyMetric(SQLModel):
...
class RateLimitMetric(SQLModel):
    endpoint: str
    rate_limited: bool
...
class AuthenticationMetric(SQLModel):
...
class APIMetricsCollector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.aggregation_window_minutes: int = int(cfg.get("aggregation_window_minutes", 1))
    ...
    async def record_api_usage(self, metric: APIUsageMetric) -> None:
        ...
        self.endpoint_popularity_cache[key] = self.endpoint_popularity_cache.get(key, 0) + 1
        # Update session activity
        if metric.session_id:
            sess = self.active_sessions.get(metric.session_id)
            if not sess:
                sess = {"user_id": metric.user_id, "endpoint_count": 0, "last_seen": datetime.now(UTC)}
    ...
    async def get_endpoint_analytics(self, hours: int = 1) -> Dict[str, Any]:
        ...
        recent: List[APIUsageMetric] = [m for m in self._api_usage if (m.timestamp or datetime.now(UTC)) >= cutoff]
    ...
    async def _cleanup_expired_sessions(self) -> None:
        ...
        expired = [sid for sid, s in self.active_sessions.items() if (now - s.get("last_seen", now)) > ttl]
        for sid in expired:
            self.active_sessions.pop(sid, None)


# Module-level singleton for convenience functions
_singleton: Optional[APIMetricsCollector] = None
