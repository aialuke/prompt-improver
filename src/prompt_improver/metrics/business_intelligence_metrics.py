...
import asyncio
import logging
from typing import Dict, Any, Optional, List, DefaultDict, Deque, Tuple
...
class BusinessIntelligenceMetricsCollector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.adoption_metrics: Deque[FeatureAdoptionMetric] = deque(maxlen=self.config.get("max_adoption_metrics", 25000))
        self.engagement_metrics: Deque[UserEngagementMetric] = deque(maxlen=self.config.get("max_engagement_metrics", 15000))
        self.cost_metrics: Deque[CostTrackingMetric] = deque(maxlen=self.config.get("max_cost_metrics", 10000))
        self.utilization_metrics: Deque[ResourceUtilizationMetric] = deque(maxlen=self.config.get("max_utilization_metrics", 20000))
        ...
        # Prometheus metrics (will be initialized in _initialize_prometheus_metrics)
        self.feature_adoption_rate: Any = None
        self.feature_usage_count: Any = None
        self.feature_time_spent: Any = None
        self.first_time_feature_usage: Any = None
        self.user_session_duration: Any = None
        self.user_actions_per_session: Any = None
        self.user_engagement_score: Any = None
        self.user_retention_rate: Any = None
        self.operational_cost_per_hour: Any = None
        self.cost_per_user: Any = None
        self.cost_efficiency_ratio: Any = None
        self.budget_utilization: Any = None
        self.resource_utilization_efficiency: Any = None
        self.resource_waste_percentage: Any = None
        self.auto_scaling_events: Any = None
        self.feature_roi: Any = None
        self.user_lifetime_value: Any = None
    ...
    async def _aggregate_adoption_metrics(self, window_start: datetime, window_end: datetime):
        ...
        category_tier_groups: DefaultDict[Tuple[FeatureCategory, UserTier], Dict[str, Any]] = defaultdict(lambda: {
            "total_users": set(),
            "total_usage": 0,
            "first_time_users": set()
        })
    ...
    async def _aggregate_engagement_metrics(self, window_start: datetime, window_end: datetime):
        ...
        # Group by user tier
        tier_groups: DefaultDict[UserTier, List[UserEngagementMetric]] = defaultdict(list)
        for metric in window_metrics:
    ...
    async def _aggregate_cost_metrics(self, window_start: datetime, window_end: datetime):
        ...
        cost_groups: DefaultDict[Tuple[CostType, str, str], float] = defaultdict(float)
        for metric in window_metrics:
    ...
    async def _aggregate_utilization_metrics(self, window_start: datetime, window_end: datetime):
        ...
        resource_groups: DefaultDict[Tuple[ResourceType, str], List[ResourceUtilizationMetric]] = defaultdict(list)
        for metric in window_metrics:
    ...
    async def get_feature_adoption_report(self, days: int = 7) -> Dict[str, Any]:
        ...
        def create_feature_stats() -> Dict[str, Any]:
            return {"users": set(), "usage": 0}

        def create_category_stats() -> Dict[str, Any]:
            return {
                "total_users": set(),
                "first_time_users": set(),
                "total_usage": 0,
                "success_rate": 0,
                "features": defaultdict(create_feature_stats)
            }

        category_stats: DefaultDict[FeatureCategory, Dict[str, Any]] = defaultdict(create_category_stats)
    ...
    async def get_cost_efficiency_report(self, days: int = 7) -> Dict[str, Any]:
        ...
        def create_cost_analysis() -> Dict[str, Any]:
            return {
                "total_cost": 0.0,
                "resource_units": 0.0,
        ...
        cost_analysis: DefaultDict[CostType, Dict[str, Any]] = defaultdict(create_cost_analysis)
    ...
    async def get_user_journey_analytics(self, hours: int = 24) -> Dict[str, Any]:
        """Get user journey analytics for the specified time period."""
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=hours)
        recent_metrics = [m for m in self.engagement_metrics if m.timestamp > cutoff_time]

        if not recent_metrics:
            return {"status": "no_data", "hours": hours}

        # Analyze user journey stages
        stage_analytics: Dict[str, Any] = {}
        total_sessions = len(set(m.session_id for m in recent_metrics))

        # Group by user tier for analysis
        tier_analysis: Dict[str, Dict[str, Any]] = {}
        ...
        for metric in recent_metrics:
            ...
            tier_analysis[tier]["unique_users"].add(metric.user_id)
            tier_analysis[tier]["total_sessions"] += 1
            tier_analysis[tier]["avg_session_duration"] += metric.session_duration_seconds
            tier_analysis[tier]["features_used"].update(metric.features_used)
            tier_analysis[tier]["actions_performed"] += metric.actions_performed

        # Convert to final format
        for tier, data in tier_analysis.items():
            unique_users_count = len(data["unique_users"])
            total_sessions_for_tier = data["total_sessions"]
            stage_analytics[tier] = {
                "unique_users": unique_users_count,
                "total_sessions": total_sessions_for_tier,
                "avg_session_duration": data["avg_session_duration"] / total_sessions_for_tier if total_sessions_for_tier > 0 else 0,
                "features_used_count": len(data["features_used"]),
                "avg_actions_per_session": data["actions_performed"] / total_sessions_for_tier if total_sessions_for_tier > 0 else 0
            }
    ...
    async def get_feature_flag_effectiveness(self, hours: int = 24) -> Dict[str, Any]:
        """Get feature flag effectiveness analysis for the specified time period."""
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=hours)
        recent_metrics = [m for m in self.adoption_metrics if m.timestamp > cutoff_time]
        ...
        # Analyze feature flags (treating experiment variants as flags)
        flag_breakdown: Dict[str, Dict[str, Any]] = {}
        total_exposures = 0

        for metric in recent_metrics:
            if metric.experiment_variant:
                flag_name = f"{metric.feature_name}_{metric.experiment_variant}"

                if flag_name not in flag_breakdown:
                ...
                flag_data = flag_breakdown[flag_name]
                flag_data["unique_users"].add(metric.user_id)
                flag_data["total_exposures"] += metric.usage_count
                flag_data["avg_performance_impact_ms"] += metric.time_spent_seconds * 1000  # Convert to ms

                if metric.success:
                    flag_data["success_rate"] += 1

                total_exposures += metric.usage_count

        # Calculate final metrics
        total_unique_users = len(set(m.user_id for m in recent_metrics))

        for flag_name, data in flag_breakdown.items():
            unique_users_count = len(data["unique_users"])
            exposures = data["total_exposures"]

            flag_breakdown[flag_name] = {
                "adoption_rate": unique_users_count / total_unique_users if total_unique_users > 0 else 0,
                "avg_performance_impact_ms": data["avg_performance_impact_ms"] / exposures if exposures > 0 else 0,
                "success_rate": data["success_rate"] / exposures if exposures > 0 else 0,
        ...
        return {
            ...
            "total_flag_exposures": total_exposures,
            "time_window_hours": hours,
            "generated_at": datetime.now(timezone.utc).isoformat()
        }
