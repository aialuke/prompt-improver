...
VAGUE_LANGUAGE_PATTERNS = {'quantifiers': ['some', 'many', 'few', 'several', 'various', 'different', 'numerous', 'multiple'], 'qualifiers': ['good', 'bad', 'nice', 'fine', 'okay', 'great', 'awesome', 'terrible', 'decent'], 'hedge_words': ['maybe', 'perhaps', 'might', 'could', 'should', 'probably', 'possibly', 'somewhat'], 'vague_nouns': ['thing', 'stuff', 'item', 'element', 'aspect', 'factor', 'feature', 'component'], 'vague_verbs': ['do', 'make', 'get', 'have', 'use', 'go', 'come', 'take', 'give'], 'vague_adjectives': ['appropriate', 'suitable', 'relevant', 'important', 'significant', 'interesting']}
SPECIFICITY_PATTERNS = {'who_what_when_where': {'who': ['target audience', 'specific user type', 'professional role', 'stakeholder group'], 'what': ['specific deliverable', 'exact outcome', 'concrete result', 'measurable output'], 'when': ['timeframe', 'deadline', 'schedule', 'timing constraints'], 'where': ['context', 'environment', 'platform', 'setting']}, 'concrete_examples': ['such as', 'for instance', 'including', 'like', 'e.g.'], 'quantifiable_metrics': ['number', 'percentage', 'count', 'length', 'duration', 'size', 'amount']}
MEASURABLE_INDICATORS = ['specific', 'measurable', 'quantifiable', 'concrete', 'precise', 'exact', 'numerical', 'detailed', 'explicit', 'clear-cut', 'well-defined']
SUCCESS_CRITERIA_KEYWORDS = ['criteria', 'requirements', 'must', 'should', 'needs to', 'has to', 'expected', 'desired', 'target', 'goal', 'objective', 'outcome']
...
class SpecificityRule(BasePromptRule):
    ...
    def __init__(self):
        ...
        self.config = {'vague_language_threshold': 0.3, 'require_specific_outcomes': True, 'include_success_criteria': True, 'enforce_measurable_goals': True, 'specificity_patterns': ['who_what_when_where', 'concrete_examples', 'quantifiable_metrics'], 'avoid_hedge_words': True, 'concrete_noun_preference': True, 'action_verb_specificity': True}
    ...
    def metadata(self):
        ...
        return {'name': 'Specificity and Detail Rule', 'type': 'Fundamental', 'description': 'Reduces vague language and increases prompt specificity using multi-source research patterns', 'category': 'fundamental', 'research_foundation': ['Multi-source specificity research', 'Vague language detection studies', 'Measurable outcome frameworks'], 'version': '2.0.0', 'priority': self.priority, 'source': 'Research Synthesis 2025'}
    ...
    def check(self, prompt: str, context=None) -> RuleCheckResult:
        ...
        if not applies:
            applies = not specificity_metrics['has_measurable_outcomes'] or not specificity_metrics['has_concrete_examples'] or specificity_metrics['specificity_score'] < 0.6
        return RuleCheckResult(applies=applies, confidence=0.9 if applies else 0.95, metadata={'vague_language_ratio': vague_language_ratio, 'vague_words': specificity_metrics['vague_words'], 'hedge_words': specificity_metrics['hedge_words'], 'specificity_score': specificity_metrics['specificity_score'], 'has_measurable_outcomes': specificity_metrics['has_measurable_outcomes'], 'has_concrete_examples': specificity_metrics['has_concrete_examples'], 'has_success_criteria': specificity_metrics['has_success_criteria'], 'missing_elements': specificity_metrics['missing_elements'], 'recommendations': specificity_metrics['recommendations']})
    ...
    def apply(self, prompt: str, context=None) -> TransformationResult:
        ...
        if self.config['avoid_hedge_words']:
            improved_prompt, hedge_transformations = self._remove_hedge_words(improved_prompt)
            transformations.extend(hedge_transformations)
        if 'who_what_when_where' in self.config['specificity_patterns']:
            improved_prompt, wwww_transformations = self._apply_who_what_when_where(improved_prompt)
            transformations.extend(wwww_transformations)
        if 'concrete_examples' in self.config['specificity_patterns']:
            improved_prompt, example_transformations = self._add_concrete_examples(improved_prompt)
            transformations.extend(example_transformations)
        if 'quantifiable_metrics' in self.config['specificity_patterns']:
            improved_prompt, metric_transformations = self._add_quantifiable_metrics(improved_prompt)
            transformations.extend(metric_transformations)
        if self.config['require_specific_outcomes']:
            improved_prompt, outcome_transformations = self._enforce_specific_outcomes(improved_prompt)
            transformations.extend(outcome_transformations)
        if self.config['include_success_criteria']:
            improved_prompt, criteria_transformations = self._add_success_criteria(improved_prompt)
            transformations.extend(criteria_transformations)
        improved_prompt, vague_transformations = self._replace_vague_language(improved_prompt)
        transformations.extend(vague_transformations)
        final_metrics = self._calculate_specificity_metrics(improved_prompt)
        improvement_score = final_metrics['specificity_score'] - specificity_metrics['specificity_score']
    ...
    def _calculate_specificity_metrics(self, prompt: str) -> dict[str, Any]:
        ...
        vague_language_ratio = (len(vague_words) + len(hedge_words)) / max(total_words, 1)
        has_measurable_outcomes = any((indicator in prompt.lower() for indicator in MEASURABLE_INDICATORS))
        has_concrete_examples = any((example_word in prompt.lower() for example_word in SPECIFICITY_PATTERNS['concrete_examples']))
        has_success_criteria = any((keyword in prompt.lower() for keyword in SUCCESS_CRITERIA_KEYWORDS))
        specificity_score = self._calculate_overall_specificity_score(prompt, vague_language_ratio)
        missing_elements = []
        ...
        return {'vague_language_ratio': vague_language_ratio, 'vague_words': vague_words, 'hedge_words': hedge_words, 'specificity_score': specificity_score, 'has_measurable_outcomes': has_measurable_outcomes, 'has_concrete_examples': has_concrete_examples, 'has_success_criteria': has_success_criteria, 'missing_elements': missing_elements, 'recommendations': recommendations}

    def _calculate_overall_specificity_score(self, prompt: str, vague_ratio: float) -> float:
        ...
        specific_indicators = ['specific', 'exactly', 'precisely', 'must', 'should', 'will']
        indicator_count = sum((1 for word in words if word.lower() in specific_indicators))
        ...
        overall_score = vague_score * 0.4 + length_score * 0.2 + indicator_score * 0.3 + number_score * 0.1
    ...
    def _remove_hedge_words(self, prompt: str) -> tuple[str, list[dict]]:
        ...
        improved_prompt = prompt
        transformations = []
        hedge_replacements = {'maybe': 'will', 'perhaps': 'will', 'might': 'will', 'could': 'should', 'probably': 'will', 'possibly': 'can', 'somewhat': '', 'rather': '', 'quite': '', 'fairly': '', 'pretty': ''}
        for hedge, replacement in hedge_replacements.items():
            if hedge in improved_prompt.lower():
                pattern = '\\b' + re.escape(hedge) + '\\b'
                if replacement:
                    improved_prompt = re.sub(pattern, replacement, improved_prompt, flags=re.IGNORECASE)
                    transformations.append({'type': 'hedge_word_replacement', 'description': f"Replaced '{hedge}' with '{replacement}' for directness", 'original': hedge, 'replacement': replacement})
                else:
                    improved_prompt = re.sub(pattern, '', improved_prompt, flags=re.IGNORECASE)
                    improved_prompt = re.sub('\\s+', ' ', improved_prompt).strip()
                    transformations.append({'type': 'hedge_word_removal', 'description': f"Removed unnecessary hedge word '{hedge}'", 'original': hedge})
    ...
    def _apply_who_what_when_where(self, prompt: str) -> tuple[str, list[dict]]:
        ...
        return (enhanced_prompt, [{'type': 'who_what_when_where_specificity', 'description': 'Added who/what/when/where specificity guidance', 'research_basis': 'Specificity pattern research'}])

    def _add_concrete_examples(self, prompt: str) -> tuple[str, list[dict]]:
        """Add concrete examples to improve specificity"""
        if any((example_word in prompt.lower() for example_word in SPECIFICITY_PATTERNS['concrete_examples'])):
            return (prompt, [])
        ...
        return (enhanced_prompt, [{'type': 'concrete_examples_addition', 'description': 'Added guidance for concrete examples', 'research_basis': 'Example-driven specificity improvement'}])

    def _add_quantifiable_metrics(self, prompt: str) -> tuple[str, list[dict]]:
        """Add quantifiable metrics and measurements"""
        if any((metric in prompt.lower() for metric in SPECIFICITY_PATTERNS['quantifiable_metrics'])):
            return (prompt, [])
        ...
        return (enhanced_prompt, [{'type': 'quantifiable_metrics_addition', 'description': 'Added guidance for quantifiable metrics', 'research_basis': 'Measurable outcome frameworks'}])
    ...
    def _enforce_specific_outcomes(self, prompt: str) -> tuple[str, list[dict]]:
        ...
        return (enhanced_prompt, [{'type': 'specific_outcomes_enforcement', 'description': 'Added specific outcome requirements', 'research_basis': 'Measurable outcome research'}])
    ...
    def _add_success_criteria(self, prompt: str) -> tuple[str, list[dict]]:
        ...
        return (enhanced_prompt, [{'type': 'success_criteria_addition', 'description': 'Added explicit success criteria framework', 'research_basis': 'Success criteria definition research'}])

    def _replace_vague_language(self, prompt: str) -> tuple[str, list[dict]]:
        """Replace vague language with specific alternatives"""
        improved_prompt = prompt
        transformations = []
        specific_replacements = {'some': 'approximately 3-5', 'many': 'more than 10', 'few': '2-3', 'several': '4-6', 'thing': 'specific item or element', 'stuff': 'relevant information or materials', 'good': 'high-quality and well-structured', 'bad': 'low-quality or inappropriate', 'nice': 'well-designed and effective', 'appropriate': 'suitable for the specific context', 'relevant': 'directly applicable to the requirements', 'important': 'critical for achieving the objectives'}
        for vague, specific in specific_replacements.items():
            if re.search('\\b' + vague + '\\b', improved_prompt, re.IGNORECASE):
                improved_prompt = re.sub('\\b' + vague + '\\b', specific, improved_prompt, flags=re.IGNORECASE)
                transformations.append({'type': 'vague_language_replacement', 'description': f"Replaced vague term '{vague}' with specific alternative '{specific}'", 'original': vague, 'replacement': specific})
        return (improved_prompt, transformations)
    ...
    def _fallback_specificity_improvement(self, prompt: str, metadata: dict) -> dict:
        ...
        if not metadata.get('has_success_criteria'):
            additions.append('Success criteria: Provide clear, measurable outcomes with specific quality standards.')
        if not metadata.get('has_concrete_examples') and metadata.get('word_count', 0) > 10:
            additions.append('Examples: Include specific, real-world examples to illustrate requirements.')
        if metadata.get('vague_language_ratio', 0) > 0.3:
            additions.append('Specificity: Replace vague terms with concrete, measurable language.')
        if metadata.get('word_count', 0) < 15:
            additions.append('Details: Provide comprehensive information including context, constraints, and expected outcomes.')
        if additions:
            improved_prompt = prompt + '\n\n' + '\n'.join(additions)
            transformations.append({'type': 'basic_specificity_improvement', 'additions': additions, 'metadata': metadata})
