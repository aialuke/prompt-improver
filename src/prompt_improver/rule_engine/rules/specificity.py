...
class SpecificityRule(BasePromptRule):
    ...
    def apply(self, prompt: str, context=None) -> TransformationResult:
        ...
        try:
            ...
            fallback_improvement = self._fallback_specificity_improvement(prompt, check_result.metadata)
            return TransformationResult(
                success=True,
...
"""
    ...
    def _fallback_specificity_improvement(self, prompt: str, metadata: dict) -> dict:
        ...
        if not metadata.get("has_constraints"):
            additions.append("Format: Provide your response in a clear, structured format.")
        
        # Add examples if missing
        if not metadata.get("has_examples") and metadata.get("word_count", 0) > 5:
            additions.append("Example: Include specific examples to illustrate your points.")
        
        # Add length constraint for short prompts
        if metadata.get("is_short"):
            additions.append("Detail: Please provide comprehensive information with specific details.")
