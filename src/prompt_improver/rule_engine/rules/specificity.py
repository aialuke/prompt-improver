...
VAGUE_LANGUAGE_PATTERNS = {
    'quantifiers': ['some', 'many', 'few', 'several', 'various', 'different', 'numerous', 'multiple'],
    'qualifiers': ['good', 'bad', 'nice', 'fine', 'okay', 'great', 'awesome', 'terrible', 'decent'],
    'hedge_words': ['maybe', 'perhaps', 'might', 'could', 'should', 'probably', 'possibly', 'somewhat'],
    'vague_nouns': ['thing', 'stuff', 'item', 'element', 'aspect', 'factor', 'feature', 'component'],
    'vague_verbs': ['do', 'make', 'get', 'have', 'use', 'go', 'come', 'take', 'give'],
    'vague_adjectives': ['appropriate', 'suitable', 'relevant', 'important', 'significant', 'interesting']
}

# Specificity improvement patterns
SPECIFICITY_PATTERNS = {
    'who_what_when_where': {
        'who': ['target audience', 'specific user type', 'professional role', 'stakeholder group'],
        'what': ['specific deliverable', 'exact outcome', 'concrete result', 'measurable output'],
    ...
    'quantifiable_metrics': ['number', 'percentage', 'count', 'length', 'duration', 'size', 'amount']
}

# Measurable outcome indicators
MEASURABLE_INDICATORS = [
    'specific', 'measurable', 'quantifiable', 'concrete', 'precise', 'exact',
    'numerical', 'detailed', 'explicit', 'clear-cut', 'well-defined'
]

# Success criteria patterns
SUCCESS_CRITERIA_KEYWORDS = [
    'criteria', 'requirements', 'must', 'should', 'needs to', 'has to',
    'expected', 'desired', 'target', 'goal', 'objective', 'outcome'
]
...
class SpecificityRule(BasePromptRule):
    ...
    def check(self, prompt: str, context=None) -> RuleCheckResult:
        ...
        if not applies:
            applies = (
                not specificity_metrics['has_measurable_outcomes'] or
                not specificity_metrics['has_concrete_examples'] or
                specificity_metrics['specificity_score'] < 0.6
            )
        ...
        return RuleCheckResult(
            ...
            metadata={
                ...
                "has_measurable_outcomes": specificity_metrics['has_measurable_outcomes'],
    ...
    def _replace_vague_language(self, prompt: str) -> Tuple[str, List[Dict]]:
        ...
        for vague, specific in specific_replacements.items():
            if re.search(r'\b' + vague + r'\b', improved_prompt, re.IGNORECASE):
                improved_prompt = re.sub(r'\b' + vague + r'\b', specific, improved_prompt, flags=re.IGNORECASE)
                transformations.append({
