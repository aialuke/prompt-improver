...
TASK_TYPE_PATTERNS = {'classification': ['\\b(classify|categorize|label|identify|determine)\\b', '\\b(what (type|kind|category)|which (class|group))\\b'], 'generation': ['\\b(write|create|generate|compose|draft)\\b', '\\b(make|build|develop|produce)\\b'], 'transformation': ['\\b(rewrite|rephrase|translate|convert|transform)\\b', '\\b(change|modify|adapt|adjust)\\b'], 'analysis': ['\\b(analyze|evaluate|assess|examine|review)\\b', '\\b(what does|what is|explain|describe)\\b'], 'extraction': ['\\b(extract|find|identify|locate|get)\\b', '\\b(list|enumerate|name|mention)\\b'], 'comparison': ['\\b(compare|contrast|difference|similarity)\\b', '\\b(versus|vs|better|worse)\\b']}
QUALITY_INDICATORS = ['clear_format', 'diverse_content', 'appropriate_length', 'relevant_domain', 'balanced_examples', 'progressive_difficulty']
...
class FewShotExampleRule(BasePromptRule):
    ...
    def __init__(self):
        self.config = {'optimal_example_count': 3, 'require_diverse_examples': True, 'include_negative_examples': True, 'use_xml_delimiters': True, 'example_placement': 'after_context', 'recency_bias_optimization': True, 'domain_specific_examples': True, 'format_consistency_check': True}
    ...
    def metadata(self):
        ...
        return {'name': 'Few-Shot Example Integration Rule', 'type': 'Examples', 'description': 'Incorporates 2-5 optimal examples based on research from PromptHub and OpenAI documentation', 'category': 'examples', 'research_foundation': ['Brown et al. (2020) few-shot learning', 'PromptHub example optimization', 'IBM research on example diversity', 'Recency bias studies'], 'version': '2.0.0', 'priority': self.priority, 'source': 'Research Synthesis 2025'}
    ...
    def check(self, prompt: str, context=None) -> RuleCheckResult:
        ...
        applies = example_metrics['task_benefits_from_examples'] and (not example_metrics['already_has_examples']) and (example_metrics['task_complexity'] > 0.3)
        confidence = 0.9 if applies else 0.85
        return RuleCheckResult(applies=applies, confidence=confidence, metadata={'task_type': example_metrics['task_type'], 'task_complexity': example_metrics['task_complexity'], 'task_benefits_from_examples': example_metrics['task_benefits_from_examples'], 'already_has_examples': example_metrics['already_has_examples'], 'recommended_example_count': example_metrics['recommended_example_count'], 'domain': example_metrics['domain'], 'format_requirements': example_metrics['format_requirements']})
    ...
    def apply(self, prompt: str, context=None) -> TransformationResult:
        ...
        example_count = min(self.config['optimal_example_count'], example_metrics.get('recommended_example_count', 3))
        examples = self._generate_examples(task_type, domain, example_count)
        if examples:
            improved_prompt, example_transformations = self._integrate_examples(improved_prompt, examples, example_metrics)
    ...
    def _analyze_example_requirements(self, prompt: str) -> dict[str, Any]:
        ...
        example_beneficial_tasks = ['classification', 'generation', 'transformation', 'extraction']
        ...
        already_has_examples = any((indicator in prompt.lower() for indicator in example_indicators))
        complexity_indicators = len([word for word in words if word in ['complex', 'detailed', 'comprehensive', 'specific', 'nuanced', 'sophisticated', 'advanced', 'technical', 'specialized']])
        ...
        return {'task_type': task_type, 'domain': domain, 'task_complexity': task_complexity, 'task_benefits_from_examples': task_benefits_from_examples, 'already_has_examples': already_has_examples, 'recommended_example_count': recommended_count, 'format_requirements': format_requirements}
    ...
    def _identify_domain(self, prompt: str) -> str:
        ...
        prompt_lower = prompt.lower()
        domain_keywords = {'business': ['business', 'marketing', 'sales', 'finance', 'corporate', 'company'], 'technical': ['code', 'programming', 'software', 'algorithm', 'technical', 'system'], 'academic': ['research', 'study', 'academic', 'scholarly', 'paper', 'thesis'], 'creative': ['creative', 'story', 'poem', 'art', 'design', 'writing'], 'legal': ['legal', 'law', 'contract', 'regulation', 'compliance', 'policy'], 'medical': ['medical', 'health', 'patient', 'diagnosis', 'treatment', 'clinical'], 'education': ['teach', 'learn', 'student', 'education', 'training', 'course']}
    ...
    def _identify_format_requirements(self, prompt: str) -> dict[str, Any]:
        ...
        format_requirements = {'structured': False, 'json': False, 'xml': False, 'bullet_points': False, 'numbered_list': False, 'table': False}
        prompt_lower = prompt.lower()
    ...
    def _generate_examples(self, task_type: str, domain: str, count: int) -> list[dict[str, str]]:
        ...
        if len(example_templates) >= count:
            ...
            while len(selected_examples) < count and len(example_templates) > 0:
                variation = self._create_example_variation(example_templates[0], len(selected_examples))
    ...
    def _get_example_templates(self, task_type: str, domain: str) -> list[dict[str, str]]:
        """Get example templates based on task type and domain"""
        base_templates = {'classification': [{'input': 'This product review is very positive and enthusiastic.', 'output': 'Sentiment: Positive'}, {'input': 'The service was disappointing and slow.', 'output': 'Sentiment: Negative'}, {'input': 'The experience was okay, nothing special.', 'output': 'Sentiment: Neutral'}], 'generation': [{'input': 'Write a professional email requesting a meeting.', 'output': 'Subject: Meeting Request\n\nDear [Name],\n\nI hope this email finds you well. I would like to schedule a meeting to discuss [topic]. Please let me know your availability for next week.\n\nBest regards,\n[Your name]'}, {'input': 'Create a brief product description for wireless headphones.', 'output': 'Premium wireless headphones featuring noise cancellation, 30-hour battery life, and crystal-clear audio quality. Perfect for music lovers and professionals alike.'}], 'transformation': [{'input': 'Make this formal: "Hey, can you send me the report?"', 'output': 'Could you please provide me with the report at your earliest convenience?'}, {'input': 'Simplify: "The implementation of the aforementioned protocol requires substantial consideration."', 'output': 'Setting up this protocol needs careful thought.'}], 'extraction': [{'input': 'Extract the main points from: "Our sales increased 25% this quarter due to new marketing strategies and improved customer service."', 'output': '• Sales increased 25% this quarter\n• Improvement due to new marketing strategies\n• Enhanced customer service contributed to growth'}]}
        templates = base_templates.get(task_type, [])
        if domain != 'general':
            templates = self._customize_for_domain(templates, domain)
        return templates

    def _customize_for_domain(self, templates: list[dict[str, str]], domain: str) -> list[dict[str, str]]:
        """Customize examples for specific domain"""
        domain_customizations = {'business': {'keywords': ['revenue', 'strategy', 'market', 'client', 'ROI'], 'context': 'business environment'}, 'technical': {'keywords': ['code', 'function', 'algorithm', 'system', 'API'], 'context': 'technical implementation'}, 'academic': {'keywords': ['research', 'hypothesis', 'methodology', 'analysis', 'findings'], 'context': 'academic research'}}
        customization = domain_customizations.get(domain, {})
        if customization:
            pass
        return templates

    def _select_diverse_examples(self, templates: list[dict[str, str]], count: int) -> list[dict[str, str]]:
    ...
    def _calculate_diversity_score(self, candidate: dict[str, str], selected: list[dict[str, str]]) -> float:
        ...
        for existing in selected:
            ...
            length_diversity = abs(candidate_length - existing_length) / max(candidate_length, existing_length, 1)
            word_overlap = len(candidate_words & existing_words) / len(candidate_words | existing_words) if candidate_words | existing_words else 0
            content_diversity = 1.0 - word_overlap
            diversity = (length_diversity + content_diversity) / 2
            diversity_scores.append(diversity)
        return sum(diversity_scores) / len(diversity_scores)

    def _optimize_example_order(self, examples: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    def _create_example_variation(self, template: dict[str, str], variation_index: int) -> dict[str, str]:
        """Create a variation of an existing example"""
        input_text = template.get('input', '')
        output_text = template.get('output', '')
        variations = [f'Alternative scenario: {input_text}', f'Similar case: {input_text}', f'Related example: {input_text}']
        return {'input': variations[variation_index % len(variations)], 'output': output_text}

    def _integrate_examples(self, prompt: str, examples: list[dict[str, str]], metrics: dict) -> tuple[str, list[dict]]:
        ...
        if placement == 'before_prompt':
            ...
            enhanced_prompt = f'{prompt}\n\nExamples:\n{examples_text}'
        transformations.append({'type': 'few_shot_examples', 'description': f'Added {len(examples)} diverse examples with XML formatting', 'example_count': len(examples), 'placement': placement, 'research_basis': 'Brown et al. 2-5 optimal examples + diversity optimization'})
