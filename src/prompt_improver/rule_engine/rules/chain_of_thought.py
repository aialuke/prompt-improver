...
REASONING_TASK_PATTERNS = [
    "\\b(calculate|compute|solve|determine|find|derive)\\b",
    "\\b(analyze|evaluate|assess|compare|contrast)\\b",
    "\\b(explain|justify|prove|demonstrate)\\b",
    "\\b(plan|design|strategy|approach)\\b",
    "\\b(why|how|what if|when|where)\\b",
    "\\b(because|therefore|thus|hence|consequently)\\b",
]
