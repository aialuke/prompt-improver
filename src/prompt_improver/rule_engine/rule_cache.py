...
class RuleEffectivenessCache:
    ...
    @classmethod
    async def create(
        cls, agent_id: str = "rule_engine_cache"
    ) -> "RuleEffectivenessCache":
        """Create and initialize RuleEffectivenessCache with async setup.

        Args:
            agent_id: Agent identifier for security context

        Returns:
            Initialized RuleEffectivenessCache instance
        """
        connection_manager = await get_database_services(ManagerMode.HIGH_AVAILABILITY)
        return cls(connection_manager, agent_id)

    async def get_rule_effectiveness(
        self, characteristics: PromptCharacteristics, cache_key: str
    ) -> list[RuleScore] | None:
        """Get rule effectiveness data from multi-level cache using DatabaseServices.
        ...
        # Connection manager already initialized during creation
        security_context = await create_security_context(
            agent_id=self.agent_id, tier="professional", authenticated=True
        )
        full_cache_key = f"{self.l2_key_prefix}{cache_key}"
        cached_data = await self.connection_manager.get_cached(
            full_cache_key, security_context
        )
        if cached_data:
            raw_level = cached_data.get("cache_level", CacheLevel.L2.value)
            try:
                level: CacheLevel = (
                    CacheLevel(raw_level) if isinstance(raw_level, str) else raw_level
                )
            except ValueError:
                level = CacheLevel.L2
            ...
            try:
                ...
                logger.warning(
                    f"Failed to deserialize cached rule scores for {cache_key}: {e}"
                )
    ...
    async def store_rule_effectiveness(
        ...
        """Store rule effectiveness data in multi-level cache using DatabaseServices.
        ...
        cache_data = {
            ...
            "cached_at": time.time(),
            "cache_level": CacheLevel.L2.value,
            "cache_backend": CacheBackend.REDIS.value,
    ...
    @staticmethod
    def _hash_characteristics(characteristics: PromptCharacteristics) -> str:
    ...
    def _track_access_pattern(
        self, cache_key: str, characteristics: PromptCharacteristics
    ) -> None:
    ...
    async def warm_cache(self, popular_patterns_limit: int = 50) -> int:
        ...
        for pattern_key, pattern_data in self.popular_patterns_cache.items():
            recency_factor = max(
                0.1, 1.0 - (current_time - pattern_data["last_accessed"]) / 3600
            )
            frequency_factor = min(10.0, pattern_data["access_count"] / 10.0)
            pattern_scores[pattern_key] = recency_factor * frequency_factor
        top_patterns = sorted(
            pattern_scores.keys(), key=lambda k: pattern_scores[k], reverse=True
        )[:popular_patterns_limit]
        warmed_count = 0
    ...
    async def invalidate_cache(self, cache_key: str | None = None) -> None:
        """Invalidate cache entries using DatabaseServices.
        ...
        if cache_key:
            full_cache_key = f"{self.l2_key_prefix}{cache_key}"
            success = await self.connection_manager.delete_cached(
                full_cache_key, security_context
            )
            if success:
            ...
            try:
                if (
                    hasattr(self.connection_manager, "cache.redis_client")
                    and self.connection_manager.cache.redis_client
                ):
                    redis = self.connection_manager.cache.redis_client
                    ...
                    logger.warning(
                        "Redis connection not available for full cache invalidation"
                    )
            except Exception as e:  # noqa: BLE001 - cleanup path, broad catch acceptable for stability
            ...
            if cache_stats.get("l1_cache_size", 0) > 0:
                logger.info("L1 cache cleared via DatabaseServices")
    ...
    def get_cache_status(self) -> dict[str, Any]:
        """Get comprehensive cache status information including DatabaseServices stats.
        ...
        return {
            ...
            "cache_warming_enabled": unified_cache_stats.get(
            ...
            "performance_status": {
                ...
                "retrieval_time_ok": self.metrics.avg_retrieval_time_ms
                <= self.target_retrieval_time_ms,
                "overall_status": "good"
                if self.metrics.hit_rate >= self.target_hit_rate
                and self.metrics.avg_retrieval_time_ms <= self.target_retrieval_time_ms
                else "needs_optimization",
                "unified_cache_performance": unified_cache_stats.get(
            ...
            "performance_improvement": "8.4x via DatabaseServices optimization",
        }
