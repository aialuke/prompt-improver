...
class RuleCombinationOptimizer:
    ...
    async def optimize_rule_combinations(self, selected_rules: list[RuleScore], characteristics: PromptCharacteristics, max_combinations: int=5) -> CombinationAnalysis:
        ...
        if len(selected_rules) < 2:
            return CombinationAnalysis(recommended_combinations=[], synergy_pairs=[], conflict_pairs=[], optimal_combination=None, analysis_confidence=0.0)
        ...
        candidate_combinations = await self._generate_candidate_combinations(selected_rules, characteristics)
        scored_combinations = await self._score_combinations(candidate_combinations, synergy_pairs, conflict_pairs)
        recommended_combinations = scored_combinations[:max_combinations]
        optimal_combination = None
        if recommended_combinations:
            optimal_combination = max(recommended_combinations, key=lambda c: c.combined_effectiveness * c.statistical_confidence)
        analysis_confidence = self._calculate_analysis_confidence(historical_combinations, scored_combinations)
        logger.info('Optimized %s combinations, found %s synergies, %s conflicts', len(candidate_combinations), len(synergy_pairs), len(conflict_pairs))
        return CombinationAnalysis(recommended_combinations=recommended_combinations, synergy_pairs=synergy_pairs, conflict_pairs=conflict_pairs, optimal_combination=optimal_combination, analysis_confidence=analysis_confidence)

    async def _get_historical_combinations(self, selected_rules: list[RuleScore]) -> list[dict[str, Any]]:
        ...
        result = await self.db_session.execute(query, {'effectiveness_threshold': self.effectiveness_threshold, 'min_sample_size': self.min_sample_size, 'min_confidence': self.min_confidence, 'rule_ids': rule_ids})
        return [dict(row._mapping) for row in result.fetchall()]

    async def _detect_synergies(self, selected_rules: list[RuleScore], historical_combinations: list[dict[str, Any]]) -> list[tuple[str, str, float]]:
        ...
        for combo in historical_combinations:
            ...
            if rule1 not in rule_effectiveness or rule2 not in rule_effectiveness:
                continue
            expected_effectiveness = (rule_effectiveness[rule1] + rule_effectiveness[rule2]) / 2.0
    ...
    async def _detect_conflicts(self, selected_rules: list[RuleScore], historical_combinations: list[dict[str, Any]]) -> list[tuple[str, str, float]]:
    ...
    async def _generate_candidate_combinations(self, selected_rules: list[RuleScore], characteristics: PromptCharacteristics) -> list[RuleCombination]:
        ...
        for size in range(2, min(len(selected_rules) + 1, self.max_combination_size + 1)):
            for rule_combo in combinations(selected_rules, size):
                ...
                candidate = RuleCombination(rule_ids=combo_rule_ids, rule_names=combo_rule_names, combined_effectiveness=baseline_effectiveness, individual_scores={rule.rule_id: rule.effectiveness_score for rule in rule_combo}, sample_size=min([rule.sample_size for rule in rule_combo]), statistical_confidence=mean([rule.confidence_level for rule in rule_combo]), synergy_score=0.0, conflict_score=0.0, metadata={'prompt_type': characteristics.prompt_type, 'domain': characteristics.domain, 'combination_size': size, 'generated_at': time.time()})
                candidates.append(candidate)
        return candidates

    async def _score_combinations(self, candidate_combinations: list[RuleCombination], synergy_pairs: list[tuple[str, str, float]], conflict_pairs: list[tuple[str, str, float]]) -> list[RuleCombination]:
        ...
        for combination in candidate_combinations:
            ...
            combination.combined_effectiveness = max(0.0, combination.combined_effectiveness + adjustment)
        candidate_combinations.sort(key=lambda c: c.combined_effectiveness * c.statistical_confidence, reverse=True)
        return candidate_combinations

    def _calculate_analysis_confidence(self, historical_combinations: list[dict[str, Any]], scored_combinations: list[RuleCombination]) -> float:
    ...
    async def get_combination_recommendations(self, rule_ids: list[str], prompt_type: str=None) -> list[dict[str, Any]]:
        ...
        result = await self.db_session.execute(query, {'effectiveness_threshold': self.effectiveness_threshold, 'min_sample_size': self.min_sample_size, 'rule_ids': rule_ids, 'prompt_type': prompt_type})
        return [dict(row._mapping) for row in result.fetchall()]

    async def record_combination_result(self, rule_ids: list[str], effectiveness_score: float, individual_scores: dict[str, float], prompt_type: str, confidence_level: float) -> None:
        ...
        result = await self.db_session.execute(query, {'rule_ids': rule_ids, 'prompt_type': prompt_type})
        existing = result.fetchone()
        if existing:
            new_sample_size = existing.sample_size + 1
            new_effectiveness = (existing.combined_effectiveness * existing.sample_size + effectiveness_score) / new_sample_size
            update_query = text('\n                UPDATE rule_combinations\n                SET\n                    combined_effectiveness = :new_effectiveness,\n                    sample_size = :new_sample_size,\n                    statistical_confidence = :confidence_level,\n                    updated_at = NOW()\n                WHERE id = :id\n            ')
            await self.db_session.execute(update_query, {'new_effectiveness': new_effectiveness, 'new_sample_size': new_sample_size, 'confidence_level': confidence_level, 'id': existing.id})
        else:
            ...
            await self.db_session.execute(insert_query, {'rule_ids': rule_ids, 'prompt_type': prompt_type, 'effectiveness_score': effectiveness_score, 'individual_scores': individual_scores, 'confidence_level': confidence_level})
        await self.db_session.commit()
        logger.info('Recorded combination result for %s rules: %s', len(rule_ids), format(effectiveness_score, '.3f'))
