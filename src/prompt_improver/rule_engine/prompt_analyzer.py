...
class PromptAnalyzer:
    ...
    def __init__(self, enable_ml_analysis: bool = True) -> None:
        ...
        self.task_type_patterns = {
            ...
            "text_generation": ["write", "create", "generate", "compose", "draft"],
            "analysis": [
                "analyze",
                "examine",
                "evaluate",
            ...
            "translation": ["translate", "convert", "transform", "change to"],
        ...
        if self.enable_ml_analysis:
            try:
                ...
                logger.info(
                    "ML models initialized successfully for enhanced prompt analysis"
                )
    ...
    def analyze_prompt(self, prompt: str) -> PromptCharacteristics:
        ...
        custom_attributes = {
            ...
            "question_count": len(re.findall("\\?", prompt)),
    ...
    def _classify_prompt_type(self, prompt: str) -> str:
        ...
        if any(
            pattern in prompt_lower for pattern in ["explain", "describe", "what is"]
        ):
            return "explanatory"
        ...
        if any(
            pattern in prompt_lower for pattern in ["analyze", "evaluate", "compare"]
        ):
            return "analytical"
    ...
    def _calculate_complexity_level(self, prompt: str) -> float:
        ...
        complexity_words = sum(
            1 for word in self.complexity_indicators if word in prompt.lower()
        )
    ...
    def _identify_domain(self, prompt: str) -> str:
        ...
        for domain, domain_data in self.domain_keywords.items():
            ...
            primary_score = sum(
                2 for keyword in primary_keywords if keyword in prompt_lower
            )
            secondary_score = sum(
                1 for keyword in secondary_keywords if keyword in prompt_lower
            )
    ...
    def _detect_reasoning_requirement(self, prompt: str) -> bool:
        ...
        prompt_lower = prompt.lower()
        reasoning_count = sum(
            1 for indicator in self.reasoning_indicators if indicator in prompt_lower
        )
        logical_patterns = ["if", "then", "because", "therefore", "since", "given"]
        logical_count = sum(
            1 for pattern in logical_patterns if pattern in prompt_lower
        )
        multi_step_patterns = ["first", "second", "then", "next", "finally", "step"]
        multi_step_count = sum(
            1 for pattern in multi_step_patterns if pattern in prompt_lower
        )
    ...
    def _calculate_specificity_level(self, prompt: str) -> float:
        ...
        if "?" in prompt:
            ...
            specific_count = sum(
                1 for word in specific_questions if word in prompt.lower()
            )
    ...
    def _calculate_context_richness(self, prompt: str) -> float:
        ...
        descriptive_words = len(
            re.findall("\\b\\w+ly\\b|\\b\\w+ful\\b|\\b\\w+ous\\b", prompt)
        )
        word_count = len(prompt.split())
    ...
    def _analyze_language_style(self, prompt: str) -> str:
        ...
        formal_count = sum(
            1 for indicator in formal_indicators if indicator in prompt.lower()
        )
        casual_indicators = ["hey", "hi", "thanks", "cool", "awesome", "gonna", "wanna"]
        casual_count = sum(
            1 for indicator in casual_indicators if indicator in prompt.lower()
        )
        ...
        technical_count = sum(
            1 for indicator in technical_indicators if indicator in prompt.lower()
        )
    ...
    def _detect_urgency(self, prompt: str) -> float:
        ...
        prompt_lower = prompt.lower()
        urgency_count = sum(
            1 for indicator in urgency_indicators if indicator in prompt_lower
        )
    ...
    def _analyze_formality(self, prompt: str) -> float:
        ...
        prompt_lower = prompt.lower()
        formal_count = sum(
            1 for indicator in formal_indicators if indicator in prompt_lower
        )
        informal_count = sum(
            1 for indicator in informal_indicators if indicator in prompt_lower
        )
    ...
    def _calculate_domain_confidence(self, prompt: str, domain: str) -> float | None:
        ...
        try:
            keyword_confidence = self._calculate_keyword_domain_confidence(
                prompt, domain
            )
            if self.semantic_model:
                semantic_confidence = self._calculate_semantic_domain_confidence(
                    prompt, domain
                )
    ...
    def _calculate_keyword_domain_confidence(self, prompt: str, domain: str) -> float:
        ...
        primary_matches = sum(
            1 for keyword in domain_data["primary"] if keyword in prompt_lower
        )
        secondary_matches = sum(
            1 for keyword in domain_data["secondary"] if keyword in prompt_lower
        )
    ...
    def _detect_reasoning_depth(self, prompt: str) -> int | None:
        ...
        reasoning_patterns = {
            ...
            2: ["how", "why", "explain"],
            3: ["analyze", "compare", "evaluate"],
            4: ["synthesize", "create", "design", "develop"],
            5: ["optimize", "strategize", "theorize", "philosophize"],
        }
        prompt_lower = prompt.lower()
        max_depth = 1
        ...
        if (
            len(re.findall("\\b(because|therefore|however|although)\\b", prompt_lower))
            > 2
        ):
    ...
    def _analyze_context_dependencies(self, prompt: str) -> list[str] | None:
        ...
        if any(
            ...
            for word in ["before", "after", "during", "while", "when"]
        ):
            dependencies.append("temporal")
        if any(
            word in prompt_lower
            for word in ["because", "due to", "caused by", "results in"]
        ):
            dependencies.append("causal")
        if any(
            word in prompt_lower for word in ["if", "unless", "provided", "assuming"]
        ):
            dependencies.append("conditional")
        if any(
            word in prompt_lower
            for word in ["this", "that", "these", "those", "above", "below"]
        ):
            dependencies.append("referential")
        if any(
            word in prompt_lower for word in ["according to", "based on", "following"]
        ):
    ...
    def _extract_linguistic_features(self, prompt: str) -> dict[str, float] | None:
        ...
        features["modal_verb_count"] = len(
            re.findall(
                "\\b(can|could|may|might|must|should|will|would)\\b", prompt.lower()
            )
        )
        features["negation_count"] = len(
            re.findall("\\b(not|no|never|none|nothing|neither)\\b", prompt.lower())
        )
        features["intensifier_count"] = len(
            re.findall("\\b(very|extremely|highly|quite|rather)\\b", prompt.lower())
        )
        return features
    ...
    def _generate_pattern_signatures(self, prompt: str) -> list[str] | None:
        ...
        if any(
            tech_word in prompt_lower
            for tech_word in ["code", "algorithm", "function", "api"]
        ):
            signatures.append("technical_domain")
        if any(
            biz_word in prompt_lower
            for biz_word in ["business", "strategy", "market", "customer"]
        ):
