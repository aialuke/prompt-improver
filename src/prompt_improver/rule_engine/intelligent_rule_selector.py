...
class IntelligentRuleSelector:
    ...
    def __init__(self, db_session: AsyncSession):
        ...
        self.scoring_weights = {'effectiveness': 0.35, 'characteristic_match': 0.25, 'historical_performance': 0.2, 'ml_prediction': 0.15, 'recency': 0.05}
    ...
    async def select_optimal_rules(self, prompt: str, prompt_characteristics: PromptCharacteristics, max_rules: int=3, strategy: RuleSelectionStrategy=RuleSelectionStrategy.BALANCED, min_score_threshold: float=0.5) -> list[RuleScore]:
        ...
        rule_intelligence = await self._get_precomputed_rule_intelligence(prompt_characteristics, max_rules * 2)
        scored_rules = await self._apply_selection_strategy(rule_intelligence, strategy, min_score_threshold)
        ...
        logger.info('Rule selection completed in %sms using pre-computed intelligence', format(selection_time, '.1f'))
        return final_rules

    async def _get_precomputed_rule_intelligence(self, characteristics: PromptCharacteristics, limit: int=10) -> list[dict[str, Any]]:
        ...
        result = await self.db_session.execute(query, {'characteristics_hash': characteristics_hash, 'min_confidence': self.min_confidence_level, 'min_sample_size': self.min_sample_size, 'limit': limit})
        rules = result.fetchall()
        if not rules:
            logger.warning('No pre-computed intelligence found for characteristics hash: %s', characteristics_hash)
            return await self._get_fallback_rules(characteristics, limit)
        result_dicts: list[dict[str, Any]] = []
        for rule in rules:
            rule_dict: dict[str, Any] = {'rule_id': rule.rule_id, 'rule_name': rule.rule_name, 'effectiveness_score': rule.effectiveness_score, 'characteristic_match_score': rule.characteristic_match_score, 'historical_performance_score': rule.historical_performance_score, 'ml_prediction_score': rule.ml_prediction_score, 'recency_score': rule.recency_score, 'total_score': rule.total_score, 'confidence_level': rule.confidence_level, 'sample_size': rule.sample_size, 'pattern_insights': rule.pattern_insights, 'optimization_recommendations': rule.optimization_recommendations, 'performance_trend': rule.performance_trend, 'rule_category': rule.rule_category, 'priority': rule.priority}
            result_dicts.append(rule_dict)
        return result_dicts

    async def _get_fallback_rules(self, characteristics: PromptCharacteristics, limit: int=10) -> list[dict[str, Any]]:
        ...
        result = await self.db_session.execute(query, {'min_sample_size': max(1, self.min_sample_size // 2), 'limit': limit})
        rules = result.fetchall()
        fallback_rules: list[dict[str, Any]] = []
        for rule in rules:
            rule_dict: dict[str, Any] = {'rule_id': rule.rule_id, 'rule_name': rule.rule_name, 'rule_category': rule.rule_category, 'priority': rule.priority, 'effectiveness_score': rule.effectiveness_score, 'confidence_level': rule.confidence_level, 'sample_size': rule.sample_size, 'characteristic_match_score': 0.5, 'historical_performance_score': rule.effectiveness_score, 'ml_prediction_score': None, 'recency_score': 0.5, 'total_score': rule.effectiveness_score * 0.7 + 0.3, 'pattern_insights': None, 'optimization_recommendations': None, 'performance_trend': 'stable'}
            fallback_rules.append(rule_dict)
        return fallback_rules

    async def _apply_selection_strategy(self, rule_intelligence: list[dict[str, Any]], strategy: RuleSelectionStrategy, min_score_threshold: float) -> list[RuleScore]:
        """Apply selection strategy to pre-computed rule intelligence."""
        scored_rules: list[RuleScore] = []
        for rule_data in rule_intelligence:
            rule_score = RuleScore(rule_id=rule_data['rule_id'], rule_name=rule_data['rule_name'], total_score=rule_data['total_score'], effectiveness_score=rule_data['effectiveness_score'], characteristic_match_score=rule_data['characteristic_match_score'], historical_performance_score=rule_data['historical_performance_score'], ml_prediction_score=rule_data.get('ml_prediction_score'), recency_score=rule_data['recency_score'], confidence_level=rule_data['confidence_level'], sample_size=rule_data['sample_size'], pattern_insights=rule_data.get('pattern_insights'), optimization_recommendations=rule_data.get('optimization_recommendations'), performance_trend=rule_data.get('performance_trend'), metadata={'rule_category': rule_data.get('rule_category'), 'priority': rule_data.get('priority'), 'source': 'precomputed_intelligence'})
    ...
    def _adjust_score_for_strategy(self, rule_score: RuleScore, strategy: RuleSelectionStrategy) -> RuleScore:
        """Adjust rule score based on selection strategy."""
        if strategy == RuleSelectionStrategy.PERFORMANCE_FOCUSED:
            rule_score.total_score = rule_score.effectiveness_score * 0.5 + (rule_score.ml_prediction_score or 0.5) * 0.3 + rule_score.historical_performance_score * 0.2
    ...
    def _generate_cache_key(self, characteristics: PromptCharacteristics, max_rules: int, strategy: RuleSelectionStrategy) -> str:
        """Generate cache key for rule selection request."""
        key_data: dict[str, Any] = {'prompt_type': characteristics.prompt_type, 'domain': characteristics.domain, 'complexity_level': round(characteristics.complexity_level, 2), 'length_category': characteristics.length_category, 'reasoning_required': characteristics.reasoning_required, 'max_rules': max_rules, 'strategy': strategy.value}
    ...
    def _hash_characteristics(self, characteristics: PromptCharacteristics) -> str:
        ...
        char_data: dict[str, Any] = {'prompt_type': characteristics.prompt_type, 'domain': characteristics.domain, 'complexity_level': round(characteristics.complexity_level, 1), 'length_category': characteristics.length_category, 'reasoning_required': characteristics.reasoning_required, 'specificity_level': round(characteristics.specificity_level, 1), 'task_type': characteristics.task_type}
    ...
    async def _cache_rules(self, cache_key: str, rules: list[RuleScore]) -> None:
        ...
        try:
            serializable_rules: list[dict[str, Any]] = [{'rule_id': rule.rule_id, 'rule_name': rule.rule_name, 'total_score': rule.total_score, 'effectiveness_score': rule.effectiveness_score, 'characteristic_match_score': rule.characteristic_match_score, 'historical_performance_score': rule.historical_performance_score, 'ml_prediction_score': rule.ml_prediction_score, 'recency_score': rule.recency_score, 'confidence_level': rule.confidence_level, 'sample_size': rule.sample_size, 'pattern_insights': rule.pattern_insights, 'optimization_recommendations': rule.optimization_recommendations, 'performance_trend': rule.performance_trend, 'metadata': rule.metadata} for rule in rules]
    ...
    async def _update_access_statistics(self, rules: list[RuleScore]) -> None:
        ...
        try:
            for rule in rules:
                await self.db_session.execute(text("SELECT update_cache_access_stats('rule_intelligence_cache', :cache_key)"), {'cache_key': f'rule_{rule.rule_id}'})
        except Exception as e:
            logger.warning('Failed to update access statistics: %s', e)

    def get_cache_statistics(self) -> dict[str, Any]:
        """Get cache performance statistics."""
        total_requests = self._cache_stats['hits'] + self._cache_stats['misses']
        hit_rate = self._cache_stats['hits'] / total_requests if total_requests > 0 else 0
        return {'cache_hits': self._cache_stats['hits'], 'cache_misses': self._cache_stats['misses'], 'hit_rate': hit_rate, 'target_hit_rate': self.cache_hit_rate_target, 'performance_status': 'good' if hit_rate >= self.cache_hit_rate_target else 'needs_improvement'}
