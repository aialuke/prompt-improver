...
class BackgroundTaskManager:
    ...
    async def stop(self, timeout: float = 30.0) -> None:
        ...
        running_tasks = [task.asyncio_task for task in self.tasks.values() 
    ...
    async def submit_task(self, task_id: str, coroutine: Callable, **kwargs) -> str:
        ...
        asyncio_task = asyncio.create_task(
            self._execute_task(bg_task, **kwargs)
        )
...
async def init_background_task_manager(max_concurrent_tasks: int = 10) -> BackgroundTaskManager:
