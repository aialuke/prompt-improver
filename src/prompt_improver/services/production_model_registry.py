...
class ProductionModelRegistry:
    ...
    async def register_model(
        ...
        try:
            ...
            # Set model version tags using model_version from above
    ...
    async def deploy_model(
        ...
        version: str,
        alias: ModelAlias,
        config: ModelDeploymentConfig | None = None,
    ) -> dict[str, Any]:
        ...
        try:
            ...
            self.client.set_registered_model_alias(
                name=model_name, alias=alias.value, version=version
            )
            ...
            deployment_record = {
                ...
                "deployment_time": aware_utc_now().isoformat(),
            ...
            logger.info(
                f"Successfully deployed {model_name}:{version} with alias @{alias.value}"
            )
        ...
        try:
            ...
            for deployment in reversed(self.deployment_history):
                if (
                    ...
                    and deployment["alias"] == alias.value
                    and deployment.get("previous_version")
                ):
            ...
            if not previous_deployment:
                raise ValueError(
                    f"No previous deployment found for {model_name}:@{alias.value}"
                )
            ...
            self.client.set_registered_model_alias(
                name=model_name, alias=alias.value, version=previous_version
            )
            ...
            rollback_record = {
                ...
                "reason": reason,
                "rollback_time": aware_utc_now().isoformat(),
            }

            self.deployment_history.append(rollback_record)

            logger.warning(
                f"Rolled back {model_name}:@{alias.value} to version {previous_version}: {reason}"
            )
    ...
    async def get_production_model(
        ...
        try:
            ...
            model_version = self.client.get_model_version_by_alias(
                name=model_name, alias=alias.value
            )
            ...
            logger.info(
                f"Loaded model {model_name}@{alias.value} (version {model_version.version})"
            )
            return model
    ...
    async def monitor_model_health(
        self, model_name: str, alias: ModelAlias, metrics: ModelMetrics
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if metrics.accuracy < config.min_accuracy:
                health_issues.append(
                    f"Accuracy {metrics.accuracy:.3f} below threshold {config.min_accuracy}"
                )

            if metrics.latency_p95 > config.max_latency_ms:
                health_issues.append(
                    f"P95 latency {metrics.latency_p95:.1f}ms above threshold {config.max_latency_ms}ms"
                )
            ...
            if len(self.model_metrics[model_key]) >= 10:
                recent_accuracy = np.mean([
                    m.accuracy for m in self.model_metrics[model_key][-5:]
                ])
                baseline_accuracy = np.mean([
                    m.accuracy for m in self.model_metrics[model_key][-10:-5]
                ])

                if baseline_accuracy - recent_accuracy > config.rollback_threshold:
                    health_issues.append(
                        f"Performance degraded by {(baseline_accuracy - recent_accuracy):.1%}"
                    )
            ...
            if not is_healthy:
                logger.warning(
                    f"Model {model_name}@{alias.value} health issues: {health_issues}"
                )
        ...
        try:
            ...
            for rm in self.client.search_registered_models():
                ...
                for version in latest_versions:
                    ...
                    if self.model_metrics.get(model_key):
    ...
    async def _get_current_alias(
        self, model_name: str, alias: ModelAlias
    ) -> str | None:
