...
class AprioriAnalyzer:
    ...
    def __init__(self, db_manager: DatabaseManager, config: AprioriConfig | None = None):
    ...
    @cached(ttl=3600, key_func=lambda *a, **kw: f"apriori_itemsets:{len(a[1]) if len(a) > 1 else 0}:{kw.get('min_support', 0.1)}")
    @handle_errors(return_format="dict")
