...
class SecureCacheError(Exception):
    """Base exception for secure cache operations."""


class EncryptionError(SecureCacheError):
    """Encryption/decryption operation failed."""


class CacheServiceError(SecureCacheError):
    """Underlying cache service operation failed."""
...
try:
    ...
    raise ImportError(
        "cryptography library is required for secure caching. Install with: pip install cryptography"
    ) from e
...
class SecureCacheWrapper:
    ...
    def __init__(self, cache_service: CacheServiceProtocol, encryption_key: str | None = None) -> None:
        ...
        if encryption_key:
            try:
                ...
                logger.exception(f"Invalid encryption key provided: {e}")
            ...
            logger.info("Generated encryption key for development use - not suitable for production")
    ...
    async def set_encrypted(
        ...
        try:
            ...
            logger.exception(f"Failed to encrypt and cache data for key {key[:50]}...: {e}")
            return False

    async def get_encrypted(self, key: str) -> Any | None:
    ...
    async def delete_encrypted(self, key: str) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to delete encrypted cache data for key {key[:50]}...: {e}")
            return False
    ...
    async def exists_encrypted(self, key: str) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to check encrypted cache existence for key {key[:50]}...: {e}")
            return False
    ...
    def validate_encryption_key(self, test_data: str = "security_test") -> bool:
        ...
        try:
            ...
            logger.exception(f"Encryption validation failed: {e}")
            return False
...
def create_secure_cache_wrapper(cache_service: CacheServiceProtocol, encryption_key: str | None = None) -> SecureCacheWrapper:
