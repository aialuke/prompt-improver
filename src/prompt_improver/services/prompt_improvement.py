...
class PromptImprovementService:
    ...
    def _classify_prompt_type(self, prompt: str) -> str:
        ...
        if any(word in prompt_lower for word in ["explain", "describe", "what is"]):
            ...
            return "analysis"
        elif any(word in prompt_lower for word in ["help", "how to", "guide"]):
            return "instruction"
        else:
            return "general"
    ...
    async def trigger_optimization(self, feedback_id: int, db_session: AsyncSession):
        ...
        if not performance_data:
            logger.warning(f"No performance data found for session {feedback.session_id}")
        ...
        if len(features) >= 10:
            ...
            if optimization_result.get("status") == "success":
                logger.info(f"ML optimization completed successfully: {optimization_result.get('best_score', 'N/A')}")
            ...
            logger.warning(f"Insufficient data for optimization: {len(features)} samples (minimum: 10)")
            return {
    ...
    async def run_ml_optimization(
        self, rule_ids: Optional[List[str]], db_session: AsyncSession
    ):
        ...
        if len(performance_data) < 20:
            logger.warning(f"Insufficient data for ML optimization: {len(performance_data)} samples (minimum: 20)")
            return {
                "status": "insufficient_data",
        ...
        if optimization_result.get("status") == "success":
            ...
            if len(features) >= 50:
                ...
                if ensemble_result.get("status") == "success":
                    logger.info(f"Ensemble optimization completed: {ensemble_result.get('ensemble_score', 'N/A')}")
    ...
    async def discover_patterns(
        ...
        if result.get("status") == "success" and result.get("patterns_discovered", 0) > 0:
            logger.info(f"Pattern discovery completed: {result['patterns_discovered']} patterns found")
            ...
            await self._store_pattern_discovery_results(
                db_session,
                result
            )
            ...
            logger.warning(f"Insufficient data for pattern discovery: {result.get('message')}")
            return result
        else:
