...
class AdvancedPatternDiscovery:
    ...
    async def discover_advanced_patterns(
        ...
        try:
            ...
            if 'parameter' in pattern_types:
                param_patterns = await self._discover_parameter_patterns(performance_data)
            ...
            if 'sequence' in pattern_types:
                sequence_patterns = await self._discover_sequence_patterns(performance_data)
            ...
            if 'performance' in pattern_types:
                perf_patterns = await self._discover_performance_patterns(performance_data)
            ...
            if 'semantic' in pattern_types:
                semantic_patterns = await self._discover_semantic_patterns(performance_data)
            ...
            if use_ensemble and len(pattern_types) > 1:
                ensemble_results = self._analyze_pattern_ensemble(results["pattern_discovery"])
                results["ensemble_results"] = ensemble_results
            
            # 6. Statistical Validation
            validation_results = self._validate_patterns_statistically(results["pattern_discovery"])
            ...
            logger.info(f"Advanced pattern discovery completed: {len(results['pattern_discovery'])} pattern types analyzed")
    ...
    async def _get_performance_data(
        ...
        try:
            ...
            stmt = select(
                ...
                RuleMetadata, RulePerformance.rule_id == RuleMetadata.rule_id, isouter=True
            ).where(
                RulePerformance.improvement_score >= min_effectiveness
            ).order_by(
                RulePerformance.created_at.desc()
    ...
    async def _discover_parameter_patterns(
        ...
        try:
            ...
            clusters = self._process_parameter_clusters(
                cluster_labels, performance_data, data_indices, outlier_scores, probabilities
            )
    ...
    def _create_transaction(self, data_point: Dict[str, Any]) -> Optional[List[str]]:
    ...
    def _process_parameter_clusters(
        ...
        for label in unique_labels:
            ...
            cluster_info = {
                ...
                "sample_rules": [d["rule_name"] for d in cluster_data[:3]]  # Sample rules
            }
    ...
    def _identify_parameter_outliers(
        ...
        for i, (label, score) in enumerate(zip(cluster_labels, outlier_scores)):
            if label == -1 or score > 0.7:  # High outlier score threshold
                ...
                if data_point["improvement_score"] >= self.min_effectiveness:
                    outlier_info = {
                        ...
                        "unique_characteristics": self._analyze_outlier_uniqueness(data_point)
                    }
        ...
        for label in unique_labels:
            ...
            cluster_info = {
                ...
                "characteristics": {
                    ...
                    "effectiveness_range": [float(min(effectiveness_scores)), float(max(effectiveness_scores))],
                    "execution_time_range": [float(min(execution_times)), float(max(execution_times))]
                },
    ...
    def _analyze_semantic_category(
        ...
        try:
            ...
            pattern = {
                ...
                "top_performers": sorted(data_points, key=lambda x: x["improvement_score"], reverse=True)[:3]
            }
    ...
    def _analyze_outlier_uniqueness(
        self,
        data_point: Dict[str, Any]
    ) -> List[str]:
    ...
    def _find_consensus_patterns(
        self,
        param_clusters: List[Dict[str, Any]],
        perf_clusters: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
                ...
                for perf_cluster in perf_clusters:
                    if (abs(param_cluster["avg_effectiveness"] - perf_cluster["characteristics"]["avg_effectiveness"]) < 0.1):
                        consensus_pattern = {
                            ...
                            "avg_effectiveness": (param_cluster["avg_effectiveness"] + perf_cluster["characteristics"]["avg_effectiveness"]) / 2,
                            "confidence": "high"
                        }
        ...
        for i, pattern in enumerate(patterns):
            if pattern.get("effectiveness_count", 0) >= 3:
                validation["pattern_reliability"][i] = {
                    ...
                    "effectiveness_reliability": pattern.get("effectiveness_std", 0) < 0.2,
    ...
    def _generate_pattern_recommendations(
        ...
        param_patterns = results.get("pattern_discovery", {}).get("parameter_patterns", {})
