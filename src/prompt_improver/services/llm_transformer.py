...
class LLMTransformerService:
    ...
    async def enhance_clarity(self, prompt: str, vague_words: List[str], context: Optional[Dict] = None) -> Dict[str, Any]:
        """
    ...
    async def enhance_specificity(self, prompt: str, context: Optional[Dict] = None) -> Dict[str, Any]:
        """
    ...
    def _load_transformation_patterns(self) -> Dict[str, Any]:
        ...
        return {
            "clarity": {
                ...
                "analyze": {
                    "replacements": {
                        "business": ["examine", "evaluate", "assess", "review systematically"],
                        "data": ["examine", "interpret", "evaluate patterns in"],
                        "text": ["examine", "review", "evaluate the content of"],
                        "default": ["examine thoroughly", "evaluate systematically", "assess in detail"]
                    },
                    "reason": "Replace generic 'analyze' with specific action"
                },
                "summarize": {
                    "replacements": {
                        "long": ["create a concise summary of", "provide key points from"],
                        "complex": ["distill the main ideas from", "extract essential points from"],
                        "default": ["provide a clear summary of", "outline the key points of"]
                    },
    ...
    def _generate_clarity_guidance(self, prompt: str, vague_words: List[str]) -> Optional[str]:
        ...
        for word in vague_words[:3]:  # Limit to top 3
            if word.lower() == "thing":
                suggestions.append("- What specific item or concept you're referring to")
            elif word.lower() == "stuff":
                suggestions.append("- What specific content or materials you mean")
            elif word.lower() == "analyze":
                suggestions.append("- What type of analysis you need (summary, comparison, evaluation)")
    ...
    def _analyze_prompt_structure(self, prompt: str) -> Dict[str, bool]:
        ...
        constraint_indicators = ["limit", "maximum", "minimum", "no more than", "at least", "exactly"]
        has_constraints = any(indicator in prompt_lower for indicator in constraint_indicators)
        
        # Check for examples
        example_indicators = ["example", "for instance", "such as", "like", "e.g."]
        has_examples = any(indicator in prompt_lower for indicator in example_indicators)
    ...
    def _suggest_constraints(self, prompt: str, context: Optional[Dict]) -> Optional[str]:
    ...
    def _generate_examples(self, prompt: str, context: Optional[Dict]) -> Optional[str]:
        ...
        elif "list" in prompt_lower and "benefit" in prompt_lower:
