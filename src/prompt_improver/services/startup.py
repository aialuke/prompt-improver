...
async def init_startup_tasks(
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ BackgroundTaskManager started (max_tasks: {max_concurrent_tasks})")
        ...
        try:
            ...
            logger.info(f"✅ SessionStore started (TTL: {session_ttl}s, cleanup: {cleanup_interval}s)")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"✅ Batch Processor initialized (size: {batch_config.get('batch_size', 10)})")
        except Exception as e:
        ...
        try:
            batch_task = await background_manager.submit_task(
                "periodic_batch_processor",
                periodic_batch_processor_coroutine,
                batch_processor=batch_processor
            )
            _startup_tasks.add(batch_task)
        ...
        try:
            ...
            health_task = await background_manager.submit_task(
                "health_monitor",
                health_monitor_coroutine,
                health_service=health_service
            )
            _startup_tasks.add(health_task)
        ...
        try:
            ...
            if health_result.overall_status.value in ["healthy", "warning"]:
                logger.info(f"✅ Initial health check passed ({health_result.overall_status.value})")
            else:
                logger.warning(f"⚠️ Initial health check returned: {health_result.overall_status.value}")
        except Exception as e:
    ...
    while not _shutdown_event.is_set():
        try:
            ...
            if health_result.overall_status.value == "healthy":
                ...
                logger.warning(f"⚠️ System health: Warnings in {len(warning_components)} components")
            else:
                failed_components = health_result.failed_checks or []
                logger.error(f"❌ System health: {len(failed_components)} components failed")
...
async def startup_context(
    ...
    if startup_result["status"] != "success":
        raise RuntimeError(f"Startup failed: {startup_result.get('error', 'Unknown error')}")
