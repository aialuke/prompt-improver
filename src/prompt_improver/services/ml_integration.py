...
from ..database.models import RuleMetadata, RulePerformance, MLModelPerformance
...
class MLModelService:
    ...
    async def optimize_rules(
        ...
        with mlflow.start_run(run_name=f"rule_optimization_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
            try:
                ...
                if len(X) < 10:
                    return {"error": f"Insufficient training data: {len(X)} samples (minimum: 10)"}
                ...
                precision = precision_score(y, final_model.predict(X), average='weighted')
    ...
    async def optimize_ensemble_rules(
        ...
        with mlflow.start_run(run_name=f"ensemble_optimization_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
            try:
                X = np.array(training_data["features"])
                y = np.array(training_data["effectiveness_scores"])
                
                if len(X) < 20:
                    return {"error": f"Insufficient data for ensemble: {len(X)} samples (minimum: 20)"}
