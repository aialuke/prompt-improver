...
class NetworkErrorService:
    ...
    async def _get_or_create_circuit_breaker(
        ...
        if cb_key not in self._circuit_breakers:
            try:
                ...
                logger.error(f"Failed to create circuit breaker for {cb_key}: {e}")
                return None
    ...
    async def _update_circuit_breaker(
        ...
        success: bool,
        error_context: Optional[NetworkErrorContext],
        ...
        try:
            ...
            logger.error(f"Failed to update network circuit breaker {circuit_breaker.name}: {e}")
            return False
    ...
    async def _record_network_error_metrics(self, error_context: NetworkErrorContext) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to record network error metrics: {e}")
