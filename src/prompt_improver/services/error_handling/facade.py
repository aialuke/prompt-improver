...
class UnifiedErrorContext:
    ...
    database_context: Optional[DatabaseErrorContext] = None
    network_context: Optional[NetworkErrorContext] = None
    validation_context: Optional[ValidationErrorContext] = None
...
class ErrorHandlingServiceProtocol(Protocol):
    ...
    async def handle_error(
        ...
        **context_kwargs: Any
        ...
        """Handle error with service-specific processing."""
        ...
...
class ErrorHandlingFacadeProtocol(Protocol):
    ...
    async def handle_unified_error(
        ...
        error: Exception,
        operation_name: str,
        service_type: Optional[ErrorServiceType] = None,
        processing_mode: ErrorProcessingMode = ErrorProcessingMode.SYNCHRONOUS,
        **context_kwargs: Any
    ) -> UnifiedErrorContext:
...
class ErrorHandlingFacade:
    ...
    def __init__(
        ...
        # Service registry
        self._error_services: Dict[ErrorServiceType, ErrorHandlingServiceProtocol] = {
        ...
        # Statistics tracking
        self._error_counts: Dict[ErrorServiceType, int] = {
            service_type: 0 for service_type in ErrorServiceType
        }
    ...
    async def batch_handle_errors(
        ...
        Args:
            ...
            processing_mode: Processing mode for batch
    ...
    def _classify_error_service_type(self, error: Exception, operation_name: str) -> ErrorServiceType:
        ...
        if any(pattern in operation_lower for pattern in ["db", "database", "sql", "query", "transaction"]):
            service_type = ErrorServiceType.DATABASE
        elif any(pattern in error_msg for pattern in ["connection", "database", "sql", "transaction", "rollback"]):
            service_type = ErrorServiceType.DATABASE
        
        # Network error patterns
        elif any(pattern in operation_lower for pattern in ["http", "api", "request", "network", "client"]):
            service_type = ErrorServiceType.NETWORK
        elif any(pattern in error_msg for pattern in ["connection", "timeout", "http", "ssl", "dns"]):
            service_type = ErrorServiceType.NETWORK
        
        # Validation error patterns
        elif any(pattern in operation_lower for pattern in ["validate", "validation", "input", "schema"]):
            service_type = ErrorServiceType.VALIDATION
        elif any(pattern in error_msg for pattern in ["validation", "invalid", "format", "required", "constraint"]):
            service_type = ErrorServiceType.VALIDATION
    ...
    async def _process_error_async(
        ...
        **context_kwargs: Any
        ...
        try:
            ...
            logger.error(f"Async error processing failed for {unified_context.error_id}: {async_error}")
    ...
    async def _record_unified_metrics(self, unified_context: UnifiedErrorContext) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to record unified error metrics: {e}")
    ...
    def get_unified_error_statistics(self) -> Dict[str, Any]:
        ...
        for service_type, service in self._error_services.items():
            try:
                ...
                logger.error(f"Failed to get statistics from {service_type.value} service: {e}")
