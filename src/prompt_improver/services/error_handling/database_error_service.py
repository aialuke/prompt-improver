...
class DatabaseErrorService:
        ...
        **kwargs
        ...
        if circuit_breaker.state == CBState.OPEN:
            ...
            self._metrics_registry.increment(
                ...
                tags={"operation": operation_name, "state": "open", "result": "rejected"}
            )
        ...
        try:
            ...
            self._metrics_registry.increment(
                StandardMetrics.CIRCUIT_BREAKER_CALLS_TOTAL,
                tags={"operation": operation_name, "state": circuit_breaker.state.value, "result": "success"}
            )
            ...
            self._metrics_registry.increment(
                StandardMetrics.CIRCUIT_BREAKER_CALLS_TOTAL,
                tags={"operation": operation_name, "state": circuit_breaker.state.value, "result": "failure"}
            )
    ...
    async def execute_with_retry(
        ...
        **kwargs
        ...
        return await self._retry_manager.retry_async(wrapped_operation, config=retry_config)
    ...
    async def database_transaction_context(
        ...
        try:
            ...
            self._metrics_registry.increment(
                ...
                tags={"operation": operation_name, "type": "started"}
            )
            ...
            self._metrics_registry.increment(
                "database_transactions_total", 
                tags={"operation": operation_name, "type": "committed"}
            )
            
            self._metrics_registry.record_value(
                "database_transaction_duration_seconds",
                execution_time,
                tags={"operation": operation_name, "result": "success"}
            )
            ...
            logger.error(f"Transaction {transaction_id} failed after {execution_time:.3f}s: {error_context.sanitized_message}")
            
            self._metrics_registry.increment(
                "database_transactions_total",
                tags={"operation": operation_name, "type": "rolled_back"}
            )
            
            self._metrics_registry.record_value(
                "database_transaction_duration_seconds", 
                execution_time,
                tags={"operation": operation_name, "result": "failure"}
            )
    ...
    def _sanitize_sql_statement(self, sql: Optional[str]) -> Optional[str]:
    ...
    async def _get_or_create_circuit_breaker(self, operation_name: str) -> Optional[CircuitBreaker]:
        ...
        if operation_name not in self._circuit_breakers:
            try:
                ...
                logger.error(f"Failed to create circuit breaker for {operation_name}: {e}")
                return None
    ...
    async def _update_circuit_breaker(
        ...
        success: bool,
        error_context: Optional[DatabaseErrorContext],
        ...
        try:
            ...
            if state_changed:
                logger.info(
                    f"Circuit breaker state changed for {circuit_breaker.name}: "
            ...
            logger.error(f"Failed to update circuit breaker {circuit_breaker.name}: {e}")
            return False
