...
from ..database.unified_connection_manager import ManagerMode, create_security_context, get_unified_manager
...
class PIIDetector:
    ...
    def __init__(self):
        ...
        self.phone_patterns = [re.compile('\\b\\d{3}-\\d{3}-\\d{4}\\b'), re.compile('\\b\\(\\d{3}\\)\\s*\\d{3}-\\d{4}\\b'), re.compile('\\b\\d{3}\\.\\d{3}\\.\\d{4}\\b'), re.compile('\\b\\+\\d{1,3}\\s*\\d{3,4}\\s*\\d{3,4}\\s*\\d{3,4}\\b')]
        self.credit_card_pattern = re.compile('\\b\\d{4}[\\s-]?\\d{4}[\\s-]?\\d{4}[\\s-]?\\d{4}\\b')
        ...
        self.common_names = {'first_names': {'john', 'jane', 'michael', 'sarah', 'david', 'lisa', 'robert', 'mary'}, 'last_names': {'smith', 'johnson', 'williams', 'brown', 'jones', 'garcia', 'miller'}}

    def detect_and_remove_pii(self, text: str, anonymization_level: AnonymizationLevel=AnonymizationLevel.ADVANCED) -> tuple[str, dict[str, Any]]:
        ...
        detection_metadata = {'pii_detected': [], 'replacements_made': 0, 'anonymization_level': anonymization_level.value}
        ...
        if anonymization_level == AnonymizationLevel.FULL:
            ...
            for i, word in enumerate(words):
                ...
                if clean_word in self.common_names['first_names'] or clean_word in self.common_names['last_names']:
                    detection_metadata['pii_detected'].append('name')
                    anonymized_text = re.sub('\\b' + re.escape(word) + '\\b', '[NAME]', anonymized_text, flags=re.IGNORECASE)
...
class EnhancedFeedbackCollector:
    ...
    def __init__(self, db_session: AsyncSession, redis_url: str='redis://localhost:6379/4', agent_id: str='enhanced_feedback_collector'):
        ...
        self._feedback_stats = {'total_collected': 0, 'successful_storage': 0, 'failed_storage': 0, 'pii_detections': 0, 'anonymization_errors': 0}
        self.max_retries = 3
    ...
    async def collect_feedback(self, background_tasks: BackgroundTasks, session_id: str, agent_id: str, agent_type: str, original_prompt: str, enhanced_prompt: str, applied_rules: list[str], effectiveness_score: float, user_rating: int | None=None, user_comments: str | None=None, performance_metrics: dict[str, Any] | None=None, anonymization_level: AnonymizationLevel=None) -> str:
        ...
        feedback_data = FeedbackData(feedback_id=feedback_id, feedback_type=FeedbackType.PROMPT_ENHANCEMENT, session_id=session_id, agent_id=agent_id, agent_type=agent_type, original_prompt=original_prompt, enhanced_prompt=enhanced_prompt, applied_rules=applied_rules, effectiveness_score=effectiveness_score, user_rating=user_rating, user_comments=user_comments, performance_metrics=performance_metrics or {}, timestamp=aware_utc_now(), anonymization_level=anonymization_level or self.default_anonymization_level, metadata={'collection_method': 'background_task', 'collector_version': '2.0.0'})
        background_tasks.add_task(self._process_feedback_async, feedback_data)
        self._feedback_stats['total_collected'] += 1
        logger.info('Feedback collection initiated: %s', feedback_id)
        return feedback_id
    ...
    async def _process_feedback_async(self, feedback_data: FeedbackData) -> None:
        ...
        try:
            ...
            success = await self._store_feedback_with_retry(feedback_data, anonymized_feedback)
            if success:
                self._feedback_stats['successful_storage'] += 1
                logger.info('Feedback processed successfully: %s', feedback_data.feedback_id)
            else:
                self._feedback_stats['failed_storage'] += 1
                await self._queue_failed_feedback(feedback_data)
                logger.error('Feedback processing failed: %s', feedback_data.feedback_id)
        except Exception as e:
            self._feedback_stats['failed_storage'] += 1
            logger.error('Feedback processing error for %s: %s', feedback_data.feedback_id, e)
            await self._queue_failed_feedback(feedback_data)
    ...
    async def _anonymize_feedback(self, feedback_data: FeedbackData) -> AnonymizedFeedback:
        ...
        """
        try:
            anonymized_original, original_metadata = self.pii_detector.detect_and_remove_pii(feedback_data.original_prompt, feedback_data.anonymization_level)
            anonymized_enhanced, enhanced_metadata = self.pii_detector.detect_and_remove_pii(feedback_data.enhanced_prompt, feedback_data.anonymization_level)
            anonymized_comments = None
            comments_metadata = {}
            if feedback_data.user_comments:
                anonymized_comments, comments_metadata = self.pii_detector.detect_and_remove_pii(feedback_data.user_comments, feedback_data.anonymization_level)
            session_hash = hashlib.sha256(f'{feedback_data.session_id}:{feedback_data.agent_id}'.encode()).hexdigest()[:16]
            prompt_structure = self._extract_prompt_structure(anonymized_original)
            enhancement_patterns = self._extract_enhancement_patterns(anonymized_original, anonymized_enhanced)
            rule_effectiveness = dict.fromkeys(feedback_data.applied_rules, feedback_data.effectiveness_score)
            total_pii_detected = len(original_metadata.get('pii_detected', [])) + len(enhanced_metadata.get('pii_detected', [])) + len(comments_metadata.get('pii_detected', []))
            if total_pii_detected > 0:
                self._feedback_stats['pii_detections'] += 1
            anonymized_feedback = AnonymizedFeedback(feedback_id=feedback_data.feedback_id, feedback_type=feedback_data.feedback_type, session_hash=session_hash, agent_type=feedback_data.agent_type, prompt_structure=prompt_structure, enhancement_patterns=enhancement_patterns, rule_effectiveness=rule_effectiveness, satisfaction_score=float(feedback_data.user_rating) if feedback_data.user_rating else None, performance_metrics=feedback_data.performance_metrics, timestamp=feedback_data.timestamp, anonymization_metadata={'original_pii': original_metadata, 'enhanced_pii': enhanced_metadata, 'comments_pii': comments_metadata, 'total_pii_detected': total_pii_detected, 'anonymization_level': feedback_data.anonymization_level.value})
            return anonymized_feedback
        except Exception as e:
            self._feedback_stats['anonymization_errors'] += 1
            logger.error('Anonymization error for {feedback_data.feedback_id}: %s', e)
            raise
    ...
    def _extract_prompt_structure(self, prompt: str) -> dict[str, Any]:
        ...
        return {'word_count': len(words), 'sentence_count': len([s for s in sentences if s.strip()]), 'avg_sentence_length': len(words) / max(1, len(sentences)), 'question_count': prompt.count('?'), 'exclamation_count': prompt.count('!'), 'has_imperatives': any((word.lower() in ['write', 'create', 'make', 'generate'] for word in words[:5])), 'has_context': any((word.lower() in ['given', 'considering', 'assuming'] for word in words)), 'complexity_indicators': sum((1 for word in words if word.lower() in ['complex', 'detailed', 'comprehensive'])), 'length_category': 'short' if len(words) < 20 else 'medium' if len(words) < 50 else 'long'}
    ...
    async def _store_feedback_with_retry(self, feedback_data: FeedbackData, anonymized_feedback: AnonymizedFeedback) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning('Storage attempt %s failed for %s: %s', attempt + 1, feedback_data.feedback_id, e)
                await self.db_session.rollback()
                if attempt < self.max_retries - 1:
                    await asyncio.sleep(self.retry_delay_seconds * 2 ** attempt)
                else:
                    logger.error('All storage attempts failed for %s', feedback_data.feedback_id)
                    return False
        return False
    ...
    async def _store_original_feedback(self, feedback_data: FeedbackData) -> None:
        ...
        await self.db_session.execute(query, {'feedback_id': feedback_data.feedback_id, 'feedback_type': feedback_data.feedback_type.value, 'session_id': feedback_data.session_id, 'agent_id': feedback_data.agent_id, 'agent_type': feedback_data.agent_type, 'original_prompt': feedback_data.original_prompt, 'enhanced_prompt': feedback_data.enhanced_prompt, 'applied_rules': feedback_data.applied_rules, 'effectiveness_score': feedback_data.effectiveness_score, 'user_rating': feedback_data.user_rating, 'user_comments': feedback_data.user_comments, 'performance_metrics': feedback_data.performance_metrics, 'anonymization_level': feedback_data.anonymization_level.value, 'metadata': feedback_data.metadata, 'created_at': feedback_data.timestamp})
    ...
    async def _store_anonymized_feedback(self, anonymized_feedback: AnonymizedFeedback) -> None:
        ...
        await self.db_session.execute(query, {'feedback_id': anonymized_feedback.feedback_id, 'feedback_type': anonymized_feedback.feedback_type.value, 'session_hash': anonymized_feedback.session_hash, 'agent_type': anonymized_feedback.agent_type, 'prompt_structure': anonymized_feedback.prompt_structure, 'enhancement_patterns': anonymized_feedback.enhancement_patterns, 'rule_effectiveness': anonymized_feedback.rule_effectiveness, 'satisfaction_score': anonymized_feedback.satisfaction_score, 'performance_metrics': anonymized_feedback.performance_metrics, 'anonymization_metadata': anonymized_feedback.anonymization_metadata, 'created_at': anonymized_feedback.timestamp})
    ...
    async def _queue_failed_feedback(self, feedback_data: FeedbackData) -> None:
        ...
        try:
            ...
            feedback_json = {'feedback_id': feedback_data.feedback_id, 'data': asdict(feedback_data), 'failed_at': time.time(), 'retry_count': 0}
            await redis.lpush(self.failed_feedback_key, str(feedback_json))
            logger.info('Queued failed feedback for retry: %s', feedback_data.feedback_id)
        except Exception as e:
            logger.error('Failed to queue feedback for retry: %s', e)
        ...
        try:
            ...
            for _ in range(max_items):
                ...
                try:
                    ...
                    logger.error('Failed to retry feedback item: %s', e)
                    retry_count = feedback_json.get('retry_count', 0)
                    if retry_count < self.max_retries:
                        feedback_json['retry_count'] = retry_count + 1
                        await redis.lpush(self.failed_feedback_key, str(feedback_json))
            if retried_count > 0:
                logger.info('Successfully retried %s failed feedback items', retried_count)
            return retried_count
        except Exception as e:
            logger.error('Error during failed feedback retry: %s', e)
            return 0
    ...
    def get_feedback_statistics(self) -> dict[str, Any]:
        ...
        base_stats = {'total_collected': self._feedback_stats['total_collected'], 'successful_storage': self._feedback_stats['successful_storage'], 'failed_storage': self._feedback_stats['failed_storage'], 'success_rate': success_rate, 'target_success_rate': self.success_rate_target, 'success_rate_status': 'good' if success_rate >= self.success_rate_target else 'needs_improvement', 'pii_detections': self._feedback_stats['pii_detections'], 'anonymization_errors': self._feedback_stats['anonymization_errors'], 'collection_method': 'background_tasks'}
        if self._use_unified_manager and self._unified_manager:
            try:
                ...
                base_stats['unified_connection_manager'] = {'enabled': True, 'healthy': self._unified_manager.is_healthy() if hasattr(self._unified_manager, 'is_healthy') else True, 'performance_improvement': '8.4x via connection pooling optimization', 'connection_pool_health': unified_stats.get('connection_pool_health', 'unknown'), 'mode': 'HIGH_AVAILABILITY', 'cache_optimization': 'L1/L2 cache available for frequently accessed feedback data'}
            except Exception as e:
                base_stats['unified_connection_manager'] = {'enabled': True, 'error': str(e)}
        else:
            base_stats['unified_connection_manager'] = {'enabled': False, 'reason': 'Using direct Redis connection'}
        return base_stats
    ...
    async def get_feedback_queue_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'failed_queue_length': failed_queue_length, 'queue_healthy': failed_queue_length < 100, 'redis_connected': True}
        except Exception as e:
            logger.error('Error getting queue status: %s', e)
            return {'failed_queue_length': -1, 'queue_healthy': False, 'redis_connected': False, 'error': str(e)}
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        return {'status': overall_status, 'issues': health_issues, 'statistics': stats, 'queue_status': queue_status, 'unified_connection_manager': unified_health, 'performance_enhancements': {'connection_pooling': unified_health.get('enabled', False), 'cache_optimization': unified_health.get('enabled', False), 'performance_improvement': unified_health.get('performance_improvement', 'N/A')}, 'timestamp': aware_utc_now().isoformat()}
