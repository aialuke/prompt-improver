...
class APESInitializer:
    ...
    async def initialize_system(self, data_dir: Optional[Path] = None, force: bool = False) -> Dict[str, Any]:
        ...
        with Progress(
            ...
            try:
                ...
                initialization_results["steps_failed"].append(f"Step failed: {str(e)}")
    ...
    async def setup_postgresql_cluster(self):
        ...
        try:
            ...
            self.console.print("⚠️  Could not verify PostgreSQL installation", style="yellow")
            return
    ...
    async def generate_initial_training_data(self):
        ...
        try:
            async with get_session() as session:
                ...
                result = await session.execute("SELECT COUNT(*) as count FROM training_prompts")
                count = result.scalar()
                
                if count > 0:
                    self.console.print(f"  📊 Found {count} existing training samples", style="dim")
                    return
                ...
                await session.commit()
                self.console.print(f"  🧠 Generated {samples_created} synthetic training samples", style="dim")
                
        except Exception as e:
            self.console.print(f"  ⚠️  Training data generation warning: {e}", style="yellow")
    ...
    async def verify_system_health(self) -> Dict[str, Any]:
        ...
        # Overall status
        if (health_results["database_connection"] and 
            health_results["configuration_valid"] and 
            health_results["directories_created"]):
