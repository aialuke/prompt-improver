...
class EnhancedQualityScorer:
    ...
    async def assess_comprehensive_quality(
        ...
        overall_score = self._calculate_weighted_score([
            fidelity, utility, privacy, statistical_validity, diversity, consistency
        ])
    ...
    async def _assess_fidelity(self, features: np.ndarray, effectiveness: np.ndarray) -> QualityDimension:
        ...
        for i in range(features.shape[1]):
            ...
            ks_stat, ks_p = stats.kstest(features[:, i], 'norm', 
        ...
        confidence_interval = self._bootstrap_confidence_interval(
            lambda x: np.mean([sub_metrics[k] for k in sub_metrics.keys()]), 
    ...
    async def _assess_utility(self, features: np.ndarray, effectiveness: np.ndarray, params: Dict) -> QualityDimension:
        ...
        sub_metrics['effectiveness_variance'] = min(1.0, eff_std / 0.3)  # Normalize to 0.3 target
        sub_metrics['effectiveness_range'] = float(min(1.0, eff_range / 0.8))   # Normalize to 0.8 target
        
        utility_score = np.mean(list(sub_metrics.values()))
        
        confidence_interval = self._bootstrap_confidence_interval(
            lambda x: np.mean(x), 
            list(sub_metrics.values())
        )
    ...
    async def _assess_statistical_validity(self, features: np.ndarray, effectiveness: np.ndarray) -> QualityDimension:
        ...
        features_in_range = np.all((features >= -5) & (features <= 5))  # Reasonable bounds
    ...
    def _calculate_confidence_score(self, dimensions: List[QualityDimension]) -> float:
        ...
        interval_widths = [dim.confidence_interval[1] - dim.confidence_interval[0] 
    ...
    def generate_quality_report(self, metrics: EnhancedQualityMetrics) -> Dict[str, Any]:
        ...
        for dim_name in dimension_names:
            ...
            report["dimensional_analysis"][dim_name] = {
                ...
                "confidence_interval": [round(x, 3) for x in dimension.confidence_interval],
                "interpretation": dimension.interpretation,
                "sub_metrics": {k: round(v, 3) for k, v in dimension.sub_metrics.items()}
            }
