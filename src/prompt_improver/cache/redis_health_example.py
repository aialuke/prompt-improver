...
from prompt_improver.cache.redis_health import RedisHealthMonitor, get_redis_health_summary
...
async def demonstrate_specific_metrics():
    ...
    await monitor.client.expire(test_key, 1)
...
async def demonstrate_keyspace_analysis():
    ...
    print('-' * 40)
    test_keys = [('user:123:profile', 'User profile data' * 100), ('session:abc123', 'Session data' * 50), ('cache:expensive_query', 'Query result' * 200), ('queue:email_notifications', 'Email queue item' * 10), ('lock:resource_123', 'Lock data'), ('analytics:2024:01:15', 'Analytics data' * 75)]
    for key, value in test_keys:
        await monitor.client.set(key, value, ex=300)
    ...
    if keyspace_data.get('key_patterns'):
        print('\n🔍 Key Patterns Found:')
    ...
    if keyspace_data.get('large_keys'):
        print('\n🔍 Large Keys Detected:')
    ...
    for key in test_key_names:
        await monitor.client.expire(key, 1)
...
async def demonstrate_slowlog_analysis():
    ...
    if slowlog_data.get('command_frequency'):
        print('\n  Top Slow Commands:')
        for cmd, count in list(slowlog_data['command_frequency'].items())[:3]:
            print(f'    {cmd}: {count} times')
    if slowlog_data.get('recent_entries'):
