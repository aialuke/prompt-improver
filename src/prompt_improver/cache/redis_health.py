...
CACHE_ERRORS = _metrics_registry.get_or_create_counter('cache_errors_total', 'Total cache operation errors', ['operation', 'error_type'])
...
async def get_redis_client_for_health_monitoring() -> coredis.Redis | None:
    ...
    try:
        ...
        logger.error('Failed to create Redis client for health monitoring: %s', e)
        CACHE_ERRORS.labels(operation='health_check', error_type='connection_error').inc()
        return None
    ...
    if _unified_manager is None:
        try:
            ...
            logger.warning('Failed to initialize UnifiedConnectionManager for health monitoring: %s', e)
            return None
...
class PersistenceMetrics:
    ...
    def is_healthy(self) -> bool:
        ...
        if self.last_save_age_minutes > 60.0 and self.rdb_changes_since_last_save > 1000:
            return False
        if self.rdb_last_bgsave_status != 'ok' or (self.aof_enabled and self.aof_last_bgrewrite_status != 'ok'):
            return False
        return True
    ...
    def get_status(self) -> RedisHealthStatus:
        ...
        if self.last_save_age_minutes > 30.0 and self.rdb_changes_since_last_save > 100:
            return RedisHealthStatus.WARNING
        if self.rdb_current_bgsave_time_sec > 300 or self.aof_current_rewrite_time_sec > 300:
            return RedisHealthStatus.WARNING
        return RedisHealthStatus.HEALTHY
...
class SlowLogMetrics:
    ...
    def analyze_entries(self):
        ...
        self.recent_slow_commands = sum((1 for entry in self.entries if entry.timestamp >= five_minutes_ago))
...
class RedisHealthMonitor:
    ...
    def __init__(self, client: coredis.Redis | None=None, agent_id: str='redis_health_monitor'):
    ...
    async def collect_all_metrics(self) -> dict[str, Any]:
        ...
        try:
            if not await self._ensure_client():
                return {'status': RedisHealthStatus.FAILED.value, 'timestamp': datetime.now(UTC).isoformat(), 'error': 'Redis client not available', 'check_duration_ms': round((time.time() - start_time) * 1000, 2), 'check_count': self.check_count}
            await self._test_connectivity()
            await asyncio.gather(self._collect_memory_metrics(), self._collect_performance_metrics(), self._collect_persistence_metrics(), self._collect_replication_metrics(), self._collect_connection_metrics(), self._collect_keyspace_metrics(), self._collect_slowlog_metrics(), return_exceptions=True)
            overall_status = self._calculate_overall_status()
            self.last_check_time = datetime.now(UTC)
            self.check_count += 1
            duration_ms = (time.time() - start_time) * 1000
            unified_cache_health = await self._get_unified_cache_health()
            return {'status': overall_status.value, 'timestamp': self.last_check_time.isoformat(), 'check_duration_ms': round(duration_ms, 2), 'check_count': self.check_count, 'memory': self._memory_metrics_to_dict(), 'performance': self._performance_metrics_to_dict(), 'persistence': self._persistence_metrics_to_dict(), 'replication': self._replication_metrics_to_dict(), 'connections': self._connection_metrics_to_dict(), 'keyspace': self._keyspace_metrics_to_dict(), 'slowlog': self._slowlog_metrics_to_dict(), 'unified_cache_health': unified_cache_health, 'recommendations': self._generate_recommendations()}
        except Exception as e:
            logger.error('Failed to collect Redis health metrics: %s', e)
            CACHE_ERRORS.labels(operation='health_check').inc()
            return {'status': RedisHealthStatus.FAILED.value, 'timestamp': datetime.now(UTC).isoformat(), 'error': str(e), 'check_duration_ms': round((time.time() - start_time) * 1000, 2), 'check_count': self.check_count}
    ...
    async def _collect_memory_metrics(self):
        ...
        try:
            ...
            if self.memory_metrics.maxmemory > 0:
                self.memory_metrics.memory_usage_percentage = self.memory_metrics.used_memory / self.memory_metrics.maxmemory * 100.0
            if self.memory_metrics.mem_fragmentation_ratio > 1.0:
                self.memory_metrics.fragmentation_bytes = self.memory_metrics.used_memory_rss - self.memory_metrics.used_memory
        except Exception as e:
    ...
    async def _collect_persistence_metrics(self):
        ...
        try:
            ...
            logger.error('Failed to collect persistence metrics: %s', e)
    ...
    async def _collect_replication_metrics(self):
        ...
        try:
            ...
            logger.error('Failed to collect replication metrics: %s', e)
    ...
    async def _collect_keyspace_metrics(self):
        ...
        try:
            ...
            logger.error('Failed to collect keyspace metrics: %s', e)
    ...
    async def _analyze_key_patterns(self):
        ...
        try:
                ...
                while cursor != 0 or sampled_keys == 0:
                    ...
                    for key in keys[:min(50, len(keys))]:
                        ...
                        try:
                            ...
                            logger.debug('Error analyzing key {key}: %s', key_error)
                            continue
                    if cursor == 0:
                        break
            ...
            logger.error('Failed to analyze key patterns: %s', e)
    ...
    def _extract_key_pattern(self, key: str | bytes) -> str:
        ...
        if not key_str:
            return 'unknown'
        patterns = [('^session:\\w+$', 'session:*'), ('^user:\\d+:', 'user:id:*'), ('^cache:\\w+:', 'cache:*'), ('^queue:\\w+$', 'queue:*'), ('^lock:\\w+$', 'lock:*'), ('^\\w+:\\d+$', 'prefix:id'), ('^\\w+:\\w+:\\w+$', 'prefix:key:value')]
    ...
    async def _collect_slowlog_metrics(self):
        ...
        try:
            ...
            for entry in slowlog_entries:
                slow_entry = SlowLogEntry(id=_safe_int(entry[0]), timestamp=_safe_int(entry[1]), duration_microseconds=_safe_int(entry[2]), command=[_safe_str(cmd) for cmd in entry[3]] if len(entry) > 3 and entry[3] else [], client_ip=_safe_str(entry[4]) if len(entry) > 4 else '', client_name=_safe_str(entry[5]) if len(entry) > 5 else '')
            ...
            logger.error('Failed to collect slow log metrics: %s', e)
    ...
    async def _get_unified_cache_health(self) -> dict[str, Any]:
        ...
        try:
            if _unified_manager and _unified_manager._is_initialized:
                ...
                return {'enabled': True, 'connection_manager_healthy': connection_health, 'l1_cache_size': cache_stats.get('l1_cache_size', 0), 'l1_hit_rate': cache_stats.get('l1_hit_rate', 0.0), 'l2_hit_rate': cache_stats.get('l2_hit_rate', 0.0), 'cache_warming_enabled': cache_stats.get('cache_warming_enabled', False), 'cache_warming_cycles': cache_stats.get('cache_warming_cycles', 0), 'performance_improvement': '8.4x via UnifiedConnectionManager', 'mode': 'HIGH_AVAILABILITY', 'connection_pool_health': cache_stats.get('connection_pool_health', 'unknown'), 'security_context_validation': cache_stats.get('security_enabled', False), 'health_status': cache_stats.get('cache_health_status', 'unknown')}
            return {'enabled': False, 'reason': 'UnifiedConnectionManager not initialized or not available'}
        except Exception as e:
            logger.warning('Failed to get UnifiedConnectionManager health metrics: %s', e)
            return {'enabled': False, 'error': str(e)}

    def _calculate_overall_status(self) -> RedisHealthStatus:
        """Calculate overall health status from all metrics."""
        statuses = [self.memory_metrics.get_status(), self.performance_metrics.get_status(), self.persistence_metrics.get_status(), self.replication_metrics.get_status(), self.connection_metrics.get_status(), self.keyspace_metrics.get_status(), self.slowlog_metrics.get_status()]
    ...
    def _memory_metrics_to_dict(self) -> dict[str, Any]:
        ...
        return {'status': self.memory_metrics.get_status().value, 'used_memory_mb': round(self.memory_metrics.used_memory / 1024 / 1024, 2), 'used_memory_human': self.memory_metrics.used_memory_human, 'used_memory_rss_mb': round(self.memory_metrics.used_memory_rss / 1024 / 1024, 2), 'peak_memory_mb': round(self.memory_metrics.used_memory_peak / 1024 / 1024, 2), 'peak_memory_human': self.memory_metrics.used_memory_peak_human, 'fragmentation_ratio': round(self.memory_metrics.mem_fragmentation_ratio, 2), 'fragmentation_bytes': self.memory_metrics.fragmentation_bytes, 'overhead_mb': round(self.memory_metrics.used_memory_overhead / 1024 / 1024, 2), 'dataset_mb': round(self.memory_metrics.used_memory_dataset / 1024 / 1024, 2), 'maxmemory_mb': round(self.memory_metrics.maxmemory / 1024 / 1024, 2) if self.memory_metrics.maxmemory > 0 else None, 'maxmemory_human': self.memory_metrics.maxmemory_human, 'maxmemory_policy': self.memory_metrics.maxmemory_policy, 'memory_usage_percentage': round(self.memory_metrics.memory_usage_percentage, 2), 'system_memory_mb': round(self.memory_metrics.total_system_memory / 1024 / 1024, 2) if self.memory_metrics.total_system_memory > 0 else None}

    def _performance_metrics_to_dict(self) -> dict[str, Any]:
        """Convert performance metrics to dictionary."""
        return {'status': self.performance_metrics.get_status().value, 'ops_per_sec': round(self.performance_metrics.ops_per_sec, 2), 'instantaneous_ops_per_sec': self.performance_metrics.instantaneous_ops_per_sec, 'total_commands': self.performance_metrics.total_commands_processed, 'keyspace_hits': self.performance_metrics.keyspace_hits, 'keyspace_misses': self.performance_metrics.keyspace_misses, 'hit_rate_percentage': round(self.performance_metrics.hit_rate, 2), 'expired_keys': self.performance_metrics.expired_keys, 'evicted_keys': self.performance_metrics.evicted_keys, 'rejected_connections': self.performance_metrics.rejected_connections, 'latency': {'avg_ms': round(self.performance_metrics.avg_latency_ms, 2), 'p50_ms': round(self.performance_metrics.p50_latency_ms, 2), 'p95_ms': round(self.performance_metrics.p95_latency_ms, 2), 'p99_ms': round(self.performance_metrics.p99_latency_ms, 2), 'max_ms': round(self.performance_metrics.max_latency_ms, 2), 'samples': len(self.performance_metrics.latency_samples)}}

    def _persistence_metrics_to_dict(self) -> dict[str, Any]:
        """Convert persistence metrics to dictionary."""
        return {'status': self.persistence_metrics.get_status().value, 'rdb': {'changes_since_last_save': self.persistence_metrics.rdb_changes_since_last_save, 'bgsave_in_progress': self.persistence_metrics.rdb_bgsave_in_progress, 'last_save_time': self.persistence_metrics.rdb_last_save_time, 'last_save_age_minutes': round(self.persistence_metrics.last_save_age_minutes, 2), 'last_bgsave_status': self.persistence_metrics.rdb_last_bgsave_status, 'last_bgsave_duration_sec': self.persistence_metrics.rdb_last_bgsave_time_sec, 'current_bgsave_duration_sec': self.persistence_metrics.rdb_current_bgsave_time_sec}, 'aof': {'enabled': self.persistence_metrics.aof_enabled, 'rewrite_in_progress': self.persistence_metrics.aof_rewrite_in_progress, 'rewrite_scheduled': self.persistence_metrics.aof_rewrite_scheduled, 'last_rewrite_duration_sec': self.persistence_metrics.aof_last_rewrite_time_sec, 'current_rewrite_duration_sec': self.persistence_metrics.aof_current_rewrite_time_sec, 'last_bgrewrite_status': self.persistence_metrics.aof_last_bgrewrite_status, 'current_size_mb': round(self.persistence_metrics.aof_current_size / 1024 / 1024, 2), 'base_size_mb': round(self.persistence_metrics.aof_base_size / 1024 / 1024, 2), 'pending_rewrite': self.persistence_metrics.aof_pending_rewrite, 'buffer_length': self.persistence_metrics.aof_buffer_length, 'rewrite_buffer_length': self.persistence_metrics.aof_rewrite_buffer_length, 'pending_bio_fsync': self.persistence_metrics.aof_pending_bio_fsync, 'delayed_fsync': self.persistence_metrics.aof_delayed_fsync} if self.persistence_metrics.aof_enabled else {'enabled': False}}

    def _replication_metrics_to_dict(self) -> dict[str, Any]:
        """Convert replication metrics to dictionary."""
        result = {'status': self.replication_metrics.get_status().value, 'role': self.replication_metrics.role.value, 'master_replid': self.replication_metrics.master_replid, 'master_repl_offset': self.replication_metrics.master_repl_offset, 'backlog_active': self.replication_metrics.repl_backlog_active, 'backlog_size': self.replication_metrics.repl_backlog_size, 'backlog_histlen': self.replication_metrics.repl_backlog_histlen}
        if self.replication_metrics.role == RedisRole.MASTER:
            result['master'] = {'connected_slaves': self.replication_metrics.connected_slaves, 'slaves': self.replication_metrics.slave_info}
        elif self.replication_metrics.role == RedisRole.SLAVE:
            result['slave'] = {'master_host': self.replication_metrics.master_host, 'master_port': self.replication_metrics.master_port, 'master_link_status': self.replication_metrics.master_link_status, 'master_last_io_seconds_ago': self.replication_metrics.master_last_io_seconds_ago, 'master_sync_in_progress': self.replication_metrics.master_sync_in_progress, 'slave_repl_offset': self.replication_metrics.slave_repl_offset, 'slave_priority': self.replication_metrics.slave_priority, 'slave_read_only': self.replication_metrics.slave_read_only, 'replication_lag_bytes': self.replication_metrics.replication_lag_bytes, 'replication_lag_seconds': round(self.replication_metrics.replication_lag_seconds, 2)}
        return result

    def _connection_metrics_to_dict(self) -> dict[str, Any]:
        """Convert connection metrics to dictionary."""
        return {'status': self.connection_metrics.get_status().value, 'connected_clients': self.connection_metrics.connected_clients, 'blocked_clients': self.connection_metrics.blocked_clients, 'tracking_clients': self.connection_metrics.tracking_clients, 'clients_in_timeout_table': self.connection_metrics.clients_in_timeout_table, 'total_connections_received': self.connection_metrics.total_connections_received, 'rejected_connections': self.connection_metrics.rejected_connections, 'maxclients': self.connection_metrics.maxclients, 'connection_utilization_percentage': round(self.connection_metrics.connection_utilization, 2), 'recent_max_input_buffer': self.connection_metrics.client_recent_max_input_buffer, 'recent_max_output_buffer': self.connection_metrics.client_recent_max_output_buffer}
    ...
    def _keyspace_metrics_to_dict(self) -> dict[str, Any]:
        ...
        for db_id, db_info in self.keyspace_metrics.databases.items():
            databases[f'db{db_id}'] = {'keys': db_info.keys, 'expires': db_info.expires, 'avg_ttl': db_info.avg_ttl, 'expiry_ratio_percentage': round(db_info.get_expiry_ratio(), 2), 'memory_usage_mb': round(db_info.memory_usage_mb, 2)}
        return {'status': self.keyspace_metrics.get_status().value, 'total_keys': self.keyspace_metrics.total_keys, 'databases': databases, 'key_patterns': {pattern: {'count': count, 'avg_memory_bytes': round(self.keyspace_metrics.sample_key_memory.get(pattern, 0))} for pattern, count in list(self.keyspace_metrics.key_distribution.items())[:20]}, 'large_keys': [{'key': key_name, 'size_mb': round(size / 1024 / 1024, 2), 'size_bytes': size} for key_name, size in self.keyspace_metrics.large_keys[:10]]}

    def _slowlog_metrics_to_dict(self) -> dict[str, Any]:
        """Convert slow log metrics to dictionary."""
        return {'status': self.slowlog_metrics.get_status().value, 'total_entries': self.slowlog_metrics.slowlog_len, 'max_entries': self.slowlog_metrics.slowlog_max_len, 'recent_slow_commands': self.slowlog_metrics.recent_slow_commands, 'avg_duration_ms': round(self.slowlog_metrics.avg_duration_ms, 2), 'max_duration_ms': round(self.slowlog_metrics.max_duration_ms, 2), 'command_frequency': dict(list(self.slowlog_metrics.command_frequency.items())[:10]), 'recent_entries': [{'id': entry.id, 'timestamp': entry.timestamp, 'duration_ms': round(entry.duration_ms, 2), 'command': entry.command_str, 'client_ip': entry.client_ip, 'client_name': entry.client_name} for entry in self.slowlog_metrics.entries[:5]]}

    def _generate_recommendations(self) -> list[str]:
        """Generate performance and configuration recommendations."""
        recommendations = []
        if self.memory_metrics.mem_fragmentation_ratio > 2.0:
            recommendations.append(f'High memory fragmentation ({self.memory_metrics.mem_fragmentation_ratio:.2f}). Consider using MEMORY PURGE or restarting Redis during low traffic.')
        if self.memory_metrics.memory_usage_percentage > 80.0:
            recommendations.append(f'High memory usage ({self.memory_metrics.memory_usage_percentage:.1f}%). Consider increasing maxmemory or implementing key expiration policies.')
        if self.performance_metrics.hit_rate < 80.0:
            recommendations.append(f'Low cache hit rate ({self.performance_metrics.hit_rate:.1f}%). Review caching strategy and key TTL settings.')
        if self.performance_metrics.p95_latency_ms > 100.0:
            recommendations.append(f'High P95 latency ({self.performance_metrics.p95_latency_ms:.1f}ms). Check for slow commands and consider optimizing queries.')
        if self.connection_metrics.connection_utilization > 80.0:
            recommendations.append(f'High connection utilization ({self.connection_metrics.connection_utilization:.1f}%). Consider increasing maxclients or implementing connection pooling.')
        if self.connection_metrics.blocked_clients > 0:
            recommendations.append(f'{self.connection_metrics.blocked_clients} blocked clients detected. Review blocking operations like BLPOP, BRPOP, or BZPOPMIN.')
        if self.persistence_metrics.last_save_age_minutes > 60.0:
            recommendations.append(f'Last RDB save was {self.persistence_metrics.last_save_age_minutes:.1f} minutes ago. Consider more frequent BGSAVE or enabling AOF for better durability.')
        if not self.persistence_metrics.aof_enabled and self.persistence_metrics.rdb_changes_since_last_save > 1000:
            recommendations.append('AOF is disabled with many unsaved changes. Consider enabling AOF for better durability.')
        if self.replication_metrics.role == RedisRole.SLAVE and self.replication_metrics.replication_lag_seconds > 10.0:
            recommendations.append(f'High replication lag ({self.replication_metrics.replication_lag_seconds:.1f}s). Check network connectivity and master load.')
        for key_name, size in self.keyspace_metrics.large_keys[:3]:
            if size > 10 * 1024 * 1024:
                recommendations.append(f"Large key detected: '{key_name}' ({size / 1024 / 1024:.1f}MB). Consider breaking into smaller keys or using different data structures.")
        if self.slowlog_metrics.recent_slow_commands > 5:
            recommendations.append(f'{self.slowlog_metrics.recent_slow_commands} slow commands in last 5 minutes. Review slow log for optimization opportunities.')
...
async def get_redis_health_summary() -> dict[str, Any]:
    ...
    try:
        if not await monitor._ensure_client():
            return {'status': 'failed', 'timestamp': datetime.now(UTC).isoformat(), 'error': 'Redis client not available via UnifiedConnectionManager', 'ping_ms': None, 'memory_mb': None, 'hit_rate': None, 'unified_cache_enabled': False}
        ...
        return {'healthy': status == 'healthy', 'status': status, 'response_time_ms': round(ping_time_ms, 2), 'memory_usage_mb': round(memory_usage_mb, 2), 'hit_rate_percentage': round(hit_rate, 2), 'connected_clients': _safe_int(info.get('connected_clients', 0)), 'total_commands': info.get('total_commands_processed', 0), 'fragmentation_ratio': round(fragmentation_ratio, 2), 'issues': issues, 'timestamp': datetime.now(UTC).isoformat(), 'unified_cache_enabled': unified_cache_health.get('enabled', False), 'unified_cache_healthy': unified_cache_health.get('connection_manager_healthy', False), 'performance_enhancement': unified_cache_health.get('performance_improvement', 'N/A')}
    except Exception as e:
        logger.error('Redis health summary failed: %s', e)
        return {'healthy': False, 'status': 'failed', 'error': str(e), 'timestamp': datetime.now(UTC).isoformat()}
