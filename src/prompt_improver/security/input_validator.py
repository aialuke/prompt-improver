...
class ValidationSchema(BaseModel):
    ...
    pattern: str | None = Field(
        default=None, max_length=500, description="Regex pattern for string validation"
    )
    allowed_chars: str | None = Field(
        default=None,
        max_length=1000,
        description="Allowed characters for string validation",
    )
    ...
    max_array_size: int | None = Field(
        default=None, ge=0, description="Maximum array size in bytes"
    )
    max_array_elements: int | None = Field(
        default=None, ge=0, description="Maximum number of array elements"
    )
    allowed_dtypes: list[Any] | None = Field(
        default=None, description="Allowed numpy data types"
    )
    required: bool = Field(default=True, description="Whether this field is required")
    allowed_types: list[Any] | None = Field(
        default=None, description="Allowed Python types"
    )
    custom_validator: Any | None = Field(
        default=None, description="Custom validation function"
    )
...
class InputValidator:
    ...
    def validate_input(
        ...
        if schema_key not in self.schemas:
            raise ValidationError(
                f"No validation schema defined for {schema_key}", field_name, value
            )
        schema = self.schemas[schema_key]
        if value is None:
            if schema.required:
                raise ValidationError(
                    f"Required field {field_name} is missing", field_name, value
                )
            return None
        if schema.allowed_types and (
            not isinstance(value, tuple(schema.allowed_types))
        ):
        ...
        if schema.custom_validator:
            try:
                ...
                if not is_valid:
                    raise ValidationError(
                        f"Custom validation failed for {field_name}", field_name, value
                    )
            except Exception as e:
                raise ValidationError(
                    f"Custom validation error for {field_name}: {e!s}",
                    field_name,
                    value,
                )
        return value
    ...
    def _validate_string(
        self, field_name: str, value: str, schema: ValidationSchema
    ) -> str:
        ...
        if schema.pattern and (not re.match(schema.pattern, value)):
            raise ValidationError(
                f"Field {field_name} does not match required pattern", field_name, value
            )
        ...
        if not self.sanitizer.validate_sql_input(value):
            raise ValidationError(
                f"Field {field_name} contains SQL injection patterns", field_name, value
            )
        if not self.sanitizer.validate_command_input(value):
            raise ValidationError(
                f"Field {field_name} contains command injection patterns",
                field_name,
                value,
            )
    ...
    def _validate_numpy_array(
        ...
        if not self.sanitizer.validate_ml_input_data(value):
            raise ValidationError(
                f"Array {field_name} contains invalid ML data", field_name, value
            )

    def _validate_dict(
        self, field_name: str, value: dict, schema: ValidationSchema
    ) -> dict:
    ...
    def _validate_list(
        self, field_name: str, value: list, schema: ValidationSchema
    ) -> list:
