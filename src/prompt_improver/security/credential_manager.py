...
class EnvironmentCredentialProvider(CredentialProvider):
    ...
    async def get_credential(self, name: str) -> str | None:
        ...
        if value:
            logger.debug("Retrieved credential '%s' from environment", name)
        else:
            logger.warning("Credential '%s' not found in environment", name)
        return value

    async def set_credential(self, name: str, value: str) -> bool:
        """Environment variables are read-only in this implementation."""
        logger.warning("Cannot set environment credential '%s' - read-only provider", name)
        return False

    async def rotate_credential(self, name: str) -> bool:
        """Environment credentials require external rotation."""
        logger.warning("Cannot rotate environment credential '%s' - external rotation required", name)
        return False
...
class FileCredentialProvider(CredentialProvider):
    ...
    async def get_credential(self, name: str) -> str | None:
        ...
        try:
            if secret_file.exists():
                ...
                logger.debug("Retrieved credential '%s' from file", name)
                return value
            logger.warning("Credential file '{name}' not found in %s", self.secrets_dir)
            return None
        except Exception as e:
            logger.error("Failed to read credential '{name}' from file: %s", e)
            return None
    ...
    async def set_credential(self, name: str, value: str) -> bool:
        ...
        try:
            ...
            logger.info("Stored credential '%s' to file", name)
            return True
        except Exception as e:
            logger.error("Failed to store credential '{name}' to file: %s", e)
            return False

    async def rotate_credential(self, name: str) -> bool:
        """File-based credentials require manual rotation."""
        logger.warning("Manual rotation required for file credential '%s'", name)
        return False
...
class CredentialManager:
    ...
    def add_provider(self, name: str, provider: CredentialProvider, priority: int=999):
    ...
    async def get_credential(self, name: str, required: bool=True) -> str | None:
        ...
        for provider_name in self.provider_order:
            ...
            try:
                ...
                if value:
                    self._log_access(name, provider_name, True, time.time() - start_time)
                    self.metadata[name] = CredentialMetadata(name=name, source=provider_name, last_accessed=time.time(), hash_signature=self._hash_credential(value))
                    return value
            except Exception as e:
                logger.error("Provider '%s' failed to get credential '%s': %s", provider_name, name, e)
                continue
        self._log_access(name, 'none', False, time.time() - start_time)
        if required:
            logger.warning("Required credential '%s' not found in any provider", name)
        return None
    ...
    async def validate_required_credentials(self, required_credentials: list[str]) -> dict[str, bool]:
    ...
    def _log_access(self, name: str, provider: str, success: bool, duration: float):
        ...
        log_entry: dict[str, Any] = {'timestamp': time.time(), 'credential_name': name, 'provider': provider, 'success': success, 'duration_ms': duration * 1000, 'masked_name': self._mask_credential_name(name)}
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        health_status: dict[str, Any] = {'status': 'healthy', 'providers': {}, 'total_credentials': len(self.metadata), 'recent_accesses': len([log for log in self.access_log if time.time() - log['timestamp'] < 3600]), 'timestamp': time.time()}
        providers_dict: dict[str, Any] = {}
        for provider_name, provider in self.providers.items():
            try:
                test_result = await provider.get_credential('__health_check_test__')
                providers_dict[provider_name] = {'status': 'healthy', 'type': type(provider).__name__}
            except Exception as e:
                providers_dict[provider_name] = {'status': 'unhealthy', 'error': str(e), 'type': type(provider).__name__}
...
async def migrate_hardcoded_credential(old_value: str, credential_name: str) -> str:
    ...
    try:
        ...
        if new_value:
            logger.info("Successfully migrated credential '%s' from hardcoded value", credential_name)
            return new_value
        logger.warning("Credential '%s' not found, using hardcoded fallback", credential_name)
        return old_value
    except Exception as e:
        logger.error("Failed to migrate credential '{credential_name}': %s", e)
        return old_value
if __name__ == '__main__':
    ...
    async def test_credential_manager():
        ...
        redis_host = await manager.get_credential_with_default('REDIS_HOST', 'localhost')
