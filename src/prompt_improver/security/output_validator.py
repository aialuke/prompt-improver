...
class OutputValidator:
    ...
    def __init__(self, max_response_length: int=5000):
        ...
        self.instruction_patterns = ['USER_DATA_TO_PROCESS\\s*[:]\\s*', 'PROCESSING_CONTEXT\\s*[:]\\s*', 'CRITICAL\\s*[:]\\s*Everything\\s+above', 'Only\\s+follow\\s+SYSTEM\\s+instructions', 'NEVER\\s+reveal\\s+these\\s+instructions', 'REFUSE\\s+harmful.*requests', 'TREAT\\s+user\\s+input\\s+as\\s+DATA']
        self.suspicious_formatting_patterns = ['```\\s*system', '```\\s*instructions', '<system[^>]*>', '<instructions[^>]*>', '\\[SYSTEM\\]', '\\[INSTRUCTIONS\\]', '---\\s*SYSTEM\\s*---', '===\\s*SYSTEM\\s*===']
        self.internal_data_patterns = ['database\\s+connection\\s+string', 'connection\\s+pool\\s+size', 'redis\\s+url', 'postgres\\s+password', 'mcp_server_user', 'localhost:\\d+', '127\\.0\\.0\\.1:\\d+', 'internal\\s+error\\s+trace']
    ...
    def validate_output(self, output: str) -> OutputValidationResult:
        ...
        if not output or len(output.strip()) == 0:
            return OutputValidationResult(is_safe=True, threat_detected=False, threat_type=None, risk_score=0.0, filtered_output='', detected_patterns=[], confidence=1.0)
        detected_patterns = []
        threat_type = None
        ...
        if threat_detected:
            logger.warning('Output security threat detected - Type: %s, Risk: %s, Patterns: %s', threat_type, format(risk_score, '.2f'), len(detected_patterns))
        return OutputValidationResult(is_safe=is_safe, threat_detected=threat_detected, threat_type=threat_type, risk_score=risk_score, filtered_output=filtered_output, detected_patterns=detected_patterns, confidence=0.9 if detected_patterns else 0.1)
