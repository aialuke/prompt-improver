...
class SecurityConfigValidator:
    ...
    INSECURE_PATTERNS = [
        ...
        ("postgresql://[^:]+:[^@]+@", "Database URL with embedded credentials"),
        ("apes_secure_password", "Default development password in use"),
        ("YOUR_SECURE_PASSWORD_HERE", "Template password not replaced"),
    ]
    REQUIRED_ENV_VARS = [
        "POSTGRES_PASSWORD",
        "POSTGRES_USERNAME",
    ...
    def _validate_password_strength(self, password: str) -> list[str]:
        ...
        if complexity_score < 3:
            issues.append(
                "Database password should contain uppercase, lowercase, numbers, and special characters"
            )
        return issues
    ...
    def validate_database_url(self, database_url: str) -> tuple[bool, list[str]]:
        ...
        if re.search(r"://[^:]+:[^@]+@", database_url):
            issues.append(
                "Database URL contains embedded credentials - use environment variables instead"
            )
        for pattern, message in self.INSECURE_PATTERNS:
