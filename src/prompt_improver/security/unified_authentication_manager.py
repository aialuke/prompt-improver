...
from prompt_improver.common.exceptions import SecurityError, InternalError
...
class AuthenticationResult(BaseModel):
    ...
    agent_id: str | None = Field(
        default=None, min_length=1, max_length=100, description="Authenticated agent ID"
    )
    authentication_method: AuthenticationMethod = Field(
        description="Method used for authentication"
    )
    security_context: SecurityContext | None = Field(
        default=None, description="Established security context"
    )
    ...
    rate_limit_tier: str = Field(
        min_length=1, max_length=50, description="Assigned rate limiting tier"
    )
    ...
    audit_metadata: dict[str, Any] = Field(
        default_factory=dict, description="Metadata for security audit logging"
    )
    performance_metrics: dict[str, float] = Field(
        default_factory=dict, description="Authentication performance metrics"
    )
...
class APIKeyData(BaseModel):
    ...
    key_id: str = Field(
        min_length=8, max_length=64, description="Unique API key identifier"
    )
    api_key_hash: str = Field(
        min_length=64, max_length=64, description="SHA-256 hash of API key"
    )
    agent_id: str = Field(
        min_length=1, max_length=100, description="Agent identifier owning this key"
    )
    permissions: list[str] = Field(
        description="List of permissions granted to this API key"
    )
    tier: str = Field(
        min_length=1, max_length=50, description="Rate limiting tier for this key"
    )
    created_at: datetime = Field(description="Timestamp when API key was created")
    expires_at: datetime | None = Field(
        default=None, description="Timestamp when API key expires"
    )
    last_used: datetime | None = Field(
        default=None, description="Timestamp of last API key usage"
    )
    usage_count: int = Field(
        ge=0, description="Number of times this API key has been used"
    )
    active: bool = Field(
        default=True, description="Whether this API key is currently active"
    )
...
class AuthenticationSession(BaseModel):
    ...
    session_id: str = Field(
        min_length=8, max_length=128, description="Unique session identifier"
    )
    agent_id: str = Field(
        min_length=1, max_length=100, description="Agent identifier for this session"
    )
...
class APIKeyManager:
    ...
    async def generate_api_key(
        ...
        try:
            ...
            api_key = (
                f"pi_key_{base64.urlsafe_b64encode(key_bytes).decode().rstrip('=')}"
            )
            key_data = APIKeyData(
            ...
            encrypted_data, encryption_key_id = self.key_manager.encrypt_string(
                json.dumps(key_data.model_dump()), None
            )
            ...
            raise SecurityError(
                f"API key generation failed: {e!s}",
                security_check="api_key_generation",
                threat_level="HIGH",
                cause=e
            )
    ...
    async def validate_api_key(self, api_key: str) -> dict[str, Any]:
        ...
        try:
            if api_key in self.api_keys:
                ...
                if key_data.active and (
                    not key_data.expires_at or key_data.expires_at > datetime.utcnow()
                ):
            ...
            if not cached_data or not cached_data.get("active"):
                return self._create_validation_failure(
                    "API key not found or inactive", start_time
                )
            decrypted_json = self.key_manager.decrypt_string(
                cached_data["encrypted_data"], cached_data["encryption_key_id"]
            )
            ...
            updated_encrypted_data, _ = self.key_manager.encrypt_string(
                json.dumps(key_data.model_dump()), cached_data["encryption_key_id"]
            )
            await self.connection_manager.set_cached(
                key=f"api_key:{self._hash_api_key(api_key)}",
                value={**cached_data, "encrypted_data": updated_encrypted_data},
                ttl_seconds=None,
                security_context=security_context,
            )
            self.api_keys[api_key] = key_data
            ...
            return self._create_validation_failure(
                f"Validation system error: {e!s}", start_time
            )
    ...
    async def revoke_api_key(self, api_key: str) -> bool:
        ...
        try:
            ...
            await self.connection_manager.delete_cached(
                key=f"api_key:{api_key_hash}", security_context=security_context
            )
    ...
    def _create_validation_success(
        self, key_data: APIKeyData, start_time: float
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "tier": key_data.tier,
            "age_hours": (datetime.utcnow() - key_data.created_at).total_seconds()
            / 3600,
            "usage_count": key_data.usage_count,
...
class UnifiedAuthenticationManager:
    ...
    def __init__(
        ...
        self.security_auditor = (
            SecurityAuditLogger("UnifiedAuthManager") if enable_audit_logging else None
        )
        ...
        logger.info(
            "UnifiedAuthenticationManager initialized with fail-secure and zero-trust policies"
        )
        ...
        try:
            ...
            if self._is_agent_locked_out(agent_id):
                return self._create_failed_result(
                    agent_id,
                    "Too many failed attempts - agent locked out",
                    auth_method,
                    start_time,
                    AuthenticationStatus.RATE_LIMITED,
                )
            if auth_method == AuthenticationMethod.API_KEY and api_key:
                result = await self._authenticate_api_key(
                    api_key, agent_id, request_context, start_time
                )
            elif auth_method == AuthenticationMethod.SESSION_TOKEN and session_token:
                result = await self._authenticate_session_token(
                    session_token, agent_id, request_context, start_time
                )
            else:
            ...
            self.performance_metrics["average_auth_time_ms"] = (
                current_avg * (total_auths - 1) + auth_time
            ) / total_auths
            if self.security_auditor:
                await self._log_authentication_attempt(
                    result, request_context, auth_time
                )
            return result
        except Exception as e:
            error_message = (
                f"Authentication system error: {e!s}" if self.fail_secure else str(e)
            )
            ...
            return self._create_failed_result(
                ...
                start_time,
                AuthenticationStatus.SYSTEM_ERROR,
            )
    ...
    async def create_session_token(
        ...
        try:
            ...
            if not success:
                raise InternalError(
                    "Failed to create session token in session store",
                    details={
                        "operation": "session_store_write",
            ...
            logger.info(
                f"Created session token for agent {agent_id}: {session_token[:8]}..."
            )
            return (session_token, metadata)
        except Exception as e:
            logger.error(f"Session token creation failed for agent {agent_id}: {e}")
            raise SecurityError(
                f"Session token creation failed: {e!s}",
                security_check="session_token_creation",
                threat_level="HIGH",
                cause=e
            )

    async def revoke_authentication(
        self, identifier: str, revocation_type: str = "api_key"
    ) -> bool:
    ...
    async def get_authentication_status(self, identifier: str) -> dict[str, Any]:
        ...
        if session_data:
            return {
                ...
                "active": datetime.fromisoformat(session_data["expires_at"])
                > datetime.utcnow(),
                "age_minutes": (
                    datetime.utcnow()
    ...
    async def get_performance_metrics(self) -> dict[str, Any]:
        ...
        return {
            **self.performance_metrics,
            ...
            "authentication_success_rate": self.performance_metrics[
    ...
    async def _authenticate_api_key(
        ...
        security_context = await create_security_context_from_auth_result(
            ...
            if validation_result["tier"] in ["professional", "enterprise"]
            else "basic",
            expires_minutes=240,
    ...
    def _determine_auth_method(
        self, api_key: str | None, session_token: str | None
    ...
    async def _apply_rate_limiting(
        self, auth_result: AuthenticationResult, request_context: dict[str, Any]
    ) -> None:
    ...
    async def _log_authentication_attempt(
        ...
        self.security_auditor.log_event(
            ...
            if result.success
            else AuditEvent.SECURITY_VIOLATION,
            {
...
async def get_unified_authentication_manager() -> UnifiedAuthenticationManager:
    ...
    if _unified_auth_manager is None:
        _unified_auth_manager = UnifiedAuthenticationManager(
            enable_audit_logging=True, fail_secure=True
        )
        ...
        async def wrapper(*args, **kwargs):
            ...
            if isinstance(result, dict):
                result["authentication"] = {
                    ...
                    "auth_time_ms": auth_result.performance_metrics.get(
                        "total_auth_time_ms", 0
                    ),
                }
            return result
...
class AuthenticationFlowOrchestrator:
    ...
    async def authenticate_local_tool(
            ...
            try:
                with open(session_file) as f:
                    ...
                    if session_data.get("session_token"):
                        request_context["headers"]["x-session-token"] = session_data[
                            "session_token"
                        ]
            except Exception as e:
    ...
    async def setup_local_tool_authentication(
        self,
        tool_name: str,
        permissions: list[str],
        tier: str = "basic",
    ...
    async def save_session_file(
        self, session_token: str, session_metadata: dict[str, Any], file_path: str
    ) -> bool:
