...
def get_secure_database_url() -> str:
    ...
    if missing_vars:
        raise SecureStartupError(f'Missing required environment variables for secure database connection: {missing_vars}')
    validator = SecurityConfigValidator()
...
def setup_secure_logging() -> None:
    ...
    class SecureFormatter(logging.Formatter):
        ...
        SENSITIVE_PATTERNS = [('password["\\s]*[:=]["\\s]*([^"\\s]+)', 'password=***REDACTED***'), ('POSTGRES_PASSWORD["\\s]*[:=]["\\s]*([^"\\s]+)', 'POSTGRES_PASSWORD=***REDACTED***'), ('postgresql://([^:]+):([^@]+)@', 'postgresql://\\1:***REDACTED***@'), ('postgresql\\+asyncpg://([^:]+):([^@]+)@', 'postgresql+asyncpg://\\1:***REDACTED***@'), ('Bearer\\s+([A-Za-z0-9\\-._~+/]+)', 'Bearer ***REDACTED***'), ('api[_-]?key["\\s]*[:=]["\\s]*([^"\\s]+)', 'api_key=***REDACTED***')]
