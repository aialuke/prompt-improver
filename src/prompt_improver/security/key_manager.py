...
class SecurityAuditLogger:
    ...
    def log_event(self, event: AuditEvent, details: dict[str, Any], security_level: SecurityLevel=SecurityLevel.basic):
        """Log security audit event with structured data."""
        audit_entry = {'timestamp': aware_utc_now().isoformat(), 'component': self.component_name, 'event': event.value, 'security_level': security_level.value, 'details': details, 'compliance': 'NIST-SP-800-57'}
        if event in [AuditEvent.SECURITY_VIOLATION]:
            self.logger.error('SECURITY_AUDIT: %s', audit_entry)
        elif event in [AuditEvent.KEY_GENERATED, AuditEvent.KEY_ROTATED]:
            self.logger.info('SECURITY_AUDIT: %s', audit_entry)
        else:
            self.logger.debug('SECURITY_AUDIT: %s', audit_entry)

class KeyRotationConfig:
    """Enhanced key rotation configuration following 2025 security best practices."""

    def __init__(self, rotation_interval_hours: int=24, max_key_age_hours: int=72, key_version_limit: int=5, auto_rotation_enabled: bool=True, security_level: SecurityLevel=SecurityLevel.enhanced, key_derivation_method: KeyDerivationMethod=KeyDerivationMethod.scrypt, enable_audit_logging: bool=True, zero_trust_mode: bool=True, hsm_ready: bool=False, compliance_mode: str='NIST-SP-800-57'):
...
class KeyInfo:
    ...
    def __init__(self, key: bytes, key_id: str, created_at: datetime, version: int=1, security_level: SecurityLevel=SecurityLevel.enhanced, derivation_method: KeyDerivationMethod=KeyDerivationMethod.scrypt):
        self.key = key
        self.key_id = key_id
        ...
        self.compliance_metadata = {'nist_compliant': True, 'zero_trust_verified': True, 'audit_trail': []}
    ...
    def mark_used(self, audit_logger: SecurityAuditLogger | None=None):
        ...
        if audit_logger:
            audit_logger.log_event(AuditEvent.KEY_ACCESSED, {'key_id': self.key_id, 'usage_count': self.usage_count, 'key_age_hours': (now - self.created_at).total_seconds() / 3600}, self.security_level)

    def get_security_metrics(self) -> dict[str, Any]:
        """Get comprehensive security metrics for the key."""
        now = aware_utc_now()
        age_hours = (now - self.created_at).total_seconds() / 3600
        return {'key_id': self.key_id, 'security_level': self.security_level.value, 'derivation_method': self.derivation_method.value, 'age_hours': age_hours, 'usage_count': self.usage_count, 'entropy_bits': self.entropy_bits, 'is_hsm_backed': self.is_hsm_backed, 'compliance_status': self.compliance_metadata, 'last_used_hours_ago': (now - self.last_used).total_seconds() / 3600, 'access_frequency': len(self.access_history) / max(age_hours, 1)}

    def to_dict(self) -> dict[str, Any]:
        """Convert key info to dictionary representation."""
        return {'key_id': self.key_id, 'created_at': self.created_at.isoformat(), 'last_used': self.last_used.isoformat(), 'usage_count': self.usage_count, 'version': self.version, 'security_level': self.security_level.value, 'derivation_method': self.derivation_method.value, 'entropy_bits': self.entropy_bits, 'is_hsm_backed': self.is_hsm_backed}
...
class SecureKeyManager:
    ...
    def __init__(self, config: KeyRotationConfig | None=None):
        ...
        self.audit_logger = SecurityAuditLogger('UnifiedKeyManager') if self.config.enable_audit_logging else None
        ...
        try:
            if operation == 'get_key':
                ...
                if key_id:
                    ...
                    return {'orchestrator_compatible': True, 'operation': operation, 'result': key_bytes, 'timestamp': aware_utc_now().isoformat()}
                key_bytes, current_key_id = self.get_current_key()
                return {'orchestrator_compatible': True, 'operation': operation, 'result': key_bytes, 'key_id': current_key_id, 'timestamp': aware_utc_now().isoformat()}
            ...
            if operation == 'get_status':
                ...
                return {'orchestrator_compatible': True, 'operation': operation, 'result': {'key_count': len(self.keys), 'current_key_id': self.current_key_id, 'security_status': security_status, 'last_operation': 'get_status'}, 'timestamp': aware_utc_now().isoformat()}
            if operation == 'cleanup':
                key_id = parameters.get('key_id')
                removed_count = self.cleanup_expired_keys()
                return {'orchestrator_compatible': True, 'operation': operation, 'result': removed_count, 'timestamp': aware_utc_now().isoformat()}
            return {'orchestrator_compatible': True, 'operation': operation, 'result': None, 'error': f'Unknown operation: {operation}', 'timestamp': aware_utc_now().isoformat()}
        except Exception as e:
            return {'orchestrator_compatible': True, 'operation': operation, 'result': None, 'error': str(e), 'timestamp': aware_utc_now().isoformat()}
    ...
    async def run_orchestrated_analysis_async(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if operation == 'get_key':
                ...
                result = {'key_available': True, 'key_id': current_key_id, 'key_version': self.keys[current_key_id].version, 'key_age_hours': (aware_utc_now() - self.keys[current_key_id].created_at).total_seconds() / 3600, 'usage_count': self.keys[current_key_id].usage_count}
            elif operation == 'rotate_key':
                new_key_id = self.rotate_key()
                result = {'rotation_completed': True, 'new_key_id': new_key_id, 'total_keys': len(self.keys), 'rotation_timestamp': aware_utc_now().isoformat()}
            elif operation == 'get_status':
                key_info = self.get_key_info()
                security_status = self.get_security_status()
                result = {'total_keys': len(self.keys), 'current_key_id': self.current_key_id, 'key_details': key_info, 'security_status': security_status, 'config': {'rotation_interval_hours': self.config.rotation_interval_hours, 'key_version_limit': self.config.key_version_limit, 'auto_rotation_enabled': self.config.auto_rotation_enabled, 'security_level': self.config.security_level.value, 'key_derivation_method': self.config.key_derivation_method.value, 'zero_trust_mode': self.config.zero_trust_mode, 'compliance_mode': self.config.compliance_mode}}
            elif operation == 'cleanup':
                removed_count = self.cleanup_expired_keys()
                result = {'cleanup_completed': True, 'keys_removed': removed_count, 'remaining_keys': len(self.keys), 'cleanup_timestamp': aware_utc_now().isoformat()}
            else:
                raise ValueError(f'Unsupported operation: {operation}')
            execution_time = (datetime.now() - start_time).total_seconds()
            return {'orchestrator_compatible': True, 'component_result': result, 'local_metadata': {'output_path': output_path, 'execution_time': execution_time, 'operation': operation, 'component_version': '1.0.0', 'security_status': 'active'}}
        except Exception as e:
            self.logger.error('Orchestrated key management failed: %s', e)
            return {'orchestrator_compatible': True, 'component_result': {'error': str(e), 'operation_failed': True}, 'local_metadata': {'execution_time': (datetime.now() - start_time).total_seconds(), 'error': True, 'component_version': '1.0.0'}}
    ...
    def get_current_key(self) -> tuple[bytes, str]:
        ...
        self.logger.debug('Retrieved current key: %s', self.current_key_id)
    ...
    def get_key_by_id(self, key_id: str) -> bytes | None:
        ...
        if key_id not in self.keys:
            self.logger.warning('Key not found: %s', key_id)
            return None
        key_info = self.keys[key_id]
        if key_info.is_expired(self.config.max_key_age_hours):
            self.logger.warning('Key expired: %s', key_id)
            return None
        key_info.mark_used()
        self.logger.debug('Retrieved key by ID: %s', key_id)
        return key_info.key
    ...
    def get_key_info(self) -> dict[str, dict]:
        ...
        for key_id, key_info in self.keys.items():
            info[key_id] = {'created_at': key_info.created_at.isoformat(), 'last_used': key_info.last_used.isoformat(), 'usage_count': key_info.usage_count, 'version': key_info.version, 'is_current': key_id == self.current_key_id, 'is_expired': key_info.is_expired(self.config.max_key_age_hours), 'should_rotate': key_info.should_rotate(self.config.rotation_interval_hours)}
        return info
    ...
    def cleanup_expired_keys(self) -> int:
        ...
        sorted_keys = sorted(self.keys.items(), key=lambda x: x[1].created_at, reverse=True)
        ...
        for key_id in expired_keys:
            ...
            self.logger.info('Removed expired key: %s', key_id)
    ...
    def _generate_new_key(self) -> str:
        ...
        key_info = KeyInfo(key=key_bytes, key_id=key_id, created_at=aware_utc_now(), version=version, security_level=self.security_level, derivation_method=self.config.key_derivation_method)
        self.keys[key_id] = key_info
        self.current_key_id = key_id
        if self.audit_logger:
            self.audit_logger.log_event(AuditEvent.KEY_GENERATED, {'key_id': key_id, 'version': version, 'derivation_method': self.config.key_derivation_method.value, 'security_level': self.security_level.value, 'entropy_bits': len(key_bytes) * 8}, self.security_level)
        self.logger.info('Generated new encryption key: %s (version %s, %s)', key_id, version, self.config.key_derivation_method.value)
        return key_id
    ...
    def _rotate_key(self) -> str:
        ...
        new_key_id = self._generate_new_key()
        removed_count = self.cleanup_expired_keys()
        self.logger.info('Key rotation completed: %s -> %s, removed %s expired keys', old_key_id, new_key_id, removed_count)
        return new_key_id
    ...
    def _perform_security_audit(self):
        ...
        if self.audit_logger:
            self.audit_logger.log_event(AuditEvent.KEY_ACCESSED, {'audit_type': 'security_audit', 'total_keys': len(self.keys), 'expired_keys': expired_count, 'suspicious_patterns': len(suspicious_patterns), 'security_violations': len(self.security_violations)}, self.security_level)
    ...
    def _analyze_access_patterns(self) -> list[dict[str, Any]]:
        ...
        for key_id, access_times in self.key_access_patterns.items():
            ...
            if time_span > 0:
                ...
                if access_rate > 0.1:
                    suspicious_patterns.append({'key_id': key_id, 'pattern': 'high_frequency_access', 'access_rate': access_rate, 'total_accesses': len(access_times)})
        return suspicious_patterns

    def _log_security_violation(self, message: str):
        """Log security violation for compliance and monitoring."""
        violation = {'timestamp': aware_utc_now().isoformat(), 'message': message, 'security_level': self.security_level.value, 'current_key_id': self.current_key_id}
        self.security_violations.append(violation)
        if len(self.security_violations) > 100:
            self.security_violations = self.security_violations[-100:]
        if self.audit_logger:
            self.audit_logger.log_event(AuditEvent.SECURITY_VIOLATION, violation, self.security_level)
        self.logger.warning('Security violation: %s', message)

    def get_security_status(self) -> dict[str, Any]:
        """Get comprehensive security status report (2025 compliance)."""
        now = aware_utc_now()
        return {'security_level': self.security_level.value, 'zero_trust_mode': self.zero_trust_mode, 'compliance_mode': self.config.compliance_mode, 'total_keys': len(self.keys), 'current_key_age_hours': (now - self.keys[self.current_key_id].created_at).total_seconds() / 3600 if self.current_key_id else 0, 'last_security_audit': self.last_security_audit.isoformat(), 'security_violations_count': len(self.security_violations), 'key_derivation_method': self.config.key_derivation_method.value, 'auto_rotation_enabled': self.config.auto_rotation_enabled, 'audit_logging_enabled': self.config.enable_audit_logging, 'hsm_ready': self.config.hsm_ready, 'suspicious_patterns': len(self._analyze_access_patterns())}
...
class UnifiedKeyManager:
    ...
    def _generate_new_key(self) -> str:
        ...
        key_bytes = Fernet.generate_key()
        key_info = KeyInfo(key=key_bytes, key_id=key_id, created_at=aware_utc_now(), derivation_method=KeyDerivationMethod.direct)
        self.keys[key_id] = key_info
        self.current_key_id = key_id
        self.logger.info('Generated new key: %s', key_id)
        return key_id
    ...
    def encrypt(self, data: bytes, key_id: str | None=None) -> tuple[bytes, str]:
        ...
        self.logger.debug('Encrypted data using key %s', used_key_id)
    ...
    def decrypt(self, encrypted_data: bytes, key_id: str) -> bytes:
        ...
        self.logger.debug('Decrypted data using key %s', key_id)
    ...
    def run_orchestrated_analysis(self, operation: str, parameters: dict=None) -> dict[str, Any]:
        ...
        try:
            ...
            if operation == 'encrypt':
                ...
                key_id = parameters.get('key_id')
                if data is None:
                    return {'orchestrator_compatible': True, 'operation': operation, 'result': {'success': False, 'error': 'No data provided'}, 'timestamp': aware_utc_now().isoformat()}
                if isinstance(data, str):
                    data = data.encode('utf-8')
                encrypted_data, used_key_id = self.encrypt(data, key_id)
                return {'orchestrator_compatible': True, 'operation': operation, 'result': {'encrypted_data': encrypted_data, 'key_id': used_key_id, 'timestamp': aware_utc_now().isoformat()}, 'timestamp': aware_utc_now().isoformat()}
            ...
            if operation == 'decrypt':
                ...
                if encrypted_data is None or key_id is None:
                    return {'orchestrator_compatible': True, 'operation': operation, 'result': {'success': False, 'error': 'Missing encrypted_data or key_id'}, 'timestamp': aware_utc_now().isoformat()}
                try:
                    ...
                    if isinstance(decrypted_data, bytes):
                        ...
                        decrypted_str = decrypted_data
                    return {'orchestrator_compatible': True, 'operation': operation, 'result': {'success': True, 'decrypted_data': decrypted_str, 'key_id': key_id}, 'timestamp': aware_utc_now().isoformat()}
                except Exception as e:
                    return {'orchestrator_compatible': True, 'operation': operation, 'result': {'success': False, 'error': str(e), 'decrypted_data': None}, 'timestamp': aware_utc_now().isoformat()}
                ...
                try:
                    ...
                    return {'orchestrator_compatible': True, 'operation': operation, 'result': {'success': success, 'test_data': test_data, 'encrypted_data': encrypted_data, 'decrypted_data': decrypted_data.decode('utf-8'), 'key_id': key_id, 'performance_ms': (end_time - start_time) * 1000}, 'timestamp': aware_utc_now().isoformat()}
                except Exception as e:
                    return {'orchestrator_compatible': True, 'operation': operation, 'result': {'success': False, 'error': str(e)}, 'timestamp': aware_utc_now().isoformat()}
            elif operation == 'get_status':
                return {'orchestrator_compatible': True, 'operation': operation, 'result': {'key_count': len(self.keys), 'current_key_id': self.current_key_id, 'last_operation': 'get_status', 'unified_manager': True}, 'timestamp': aware_utc_now().isoformat()}
            else:
                return {'orchestrator_compatible': True, 'operation': operation, 'result': None, 'error': f'Unknown operation: {operation}', 'timestamp': aware_utc_now().isoformat()}
        except Exception as e:
            return {'orchestrator_compatible': True, 'operation': operation, 'result': None, 'error': str(e), 'timestamp': aware_utc_now().isoformat()}
    ...
    async def run_orchestrated_analysis_async(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {'orchestrator_compatible': True, 'component_result': result.get('result', {}), 'local_metadata': {'execution_time': 0.0, 'operation': operation, 'component_version': '2.0.0', 'unified_manager': True}}
