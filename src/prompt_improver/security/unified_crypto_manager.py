...
from prompt_improver.security.key_manager import AuditEvent, KeyRotationConfig, SecurityLevel, UnifiedKeyManager
...
class CryptoMetrics:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        stats = {'operation_counts': self.operation_counts.copy(), 'error_counts': self.error_counts.copy(), 'performance_averages': {}, 'last_reset': self.last_reset.isoformat(), 'total_operations': sum(self.operation_counts.values())}
        for operation, times in self.performance_stats.items():
            if times:
                stats['performance_averages'][operation] = {'avg_ms': sum(times) / len(times), 'min_ms': min(times), 'max_ms': max(times), 'count': len(times)}
        return stats
...
class UnifiedCryptoManager:
    ...
    def __init__(self, key_manager: UnifiedKeyManager | None=None, security_level: SecurityLevel=SecurityLevel.enhanced):
    ...
    def hash_data(self, data: str | bytes, algorithm: HashAlgorithm=HashAlgorithm.SHA256, truncate_length: int | None=None) -> str:
        ...
        try:
            ...
            if algorithm == HashAlgorithm.SHA256:
                ...
                self.logger.warning('SHA1 is deprecated and should not be used for security purposes')
                hash_obj = hashlib.sha1(data_bytes)
            elif algorithm == HashAlgorithm.MD5:
                self.logger.warning('MD5 is deprecated and should not be used for security purposes')
            ...
            if self.audit_enabled:
                self._log_crypto_event(CryptoAuditEvent.HASH_GENERATED, {'algorithm': algorithm.value, 'data_length': len(data_bytes), 'truncate_length': truncate_length, 'hash_length': len(hash_result)})
            self.logger.debug('Generated %s hash (length: %s)', algorithm.value, len(hash_result))
            return hash_result
            ...
            self.metrics.record_operation(f'hash_{algorithm.value}', duration_ms, success)
    ...
    def generate_secure_hash(self, data: str | bytes, salt: bytes | None=None) -> tuple[str, bytes]:
    ...
    def generate_random(self, length: int, random_type: RandomType=RandomType.BYTES) -> bytes | str:
        ...
        try:
            ...
            if self.audit_enabled:
                self._log_crypto_event(CryptoAuditEvent.RANDOM_GENERATED, {'type': random_type.value, 'length': length, 'result_length': len(result)})
            self.logger.debug('Generated %s random data (length: %s)', random_type.value, length)
            return result
        except Exception as e:
            success = False
            ...
            duration_ms = (time.time() - start_time) * 1000
            self.metrics.record_operation(f'random_{random_type.value}', duration_ms, success)
    ...
    def encrypt_data(self, data: str | bytes, key_id: str | None=None) -> tuple[bytes, str]:
        ...
        try:
            ...
            if self.audit_enabled:
                self._log_crypto_event(CryptoAuditEvent.ENCRYPTION_PERFORMED, {'data_length': len(data_bytes), 'key_id': used_key_id, 'encrypted_length': len(encrypted_data)})
    ...
    def decrypt_data(self, encrypted_data: bytes, key_id: str) -> bytes:
        ...
        try:
            ...
            if self.audit_enabled:
                self._log_crypto_event(CryptoAuditEvent.DECRYPTION_PERFORMED, {'encrypted_length': len(encrypted_data), 'key_id': key_id, 'decrypted_length': len(decrypted_data)})
    ...
    def derive_key(self, password: str | bytes, salt: bytes | None=None, length: int=32, iterations: int=600000) -> tuple[bytes, bytes]:
        ...
        try:
            ...
            if self.audit_enabled:
                self._log_crypto_event(CryptoAuditEvent.KEY_DERIVED, {'algorithm': 'PBKDF2-HMAC-SHA256', 'iterations': iterations, 'salt_length': len(salt), 'key_length': len(derived_key)})
            self.logger.debug('Derived key using PBKDF2 (%s iterations)', iterations)
    ...
    def derive_key_scrypt(self, password: str | bytes, salt: bytes | None=None, length: int=32, n: int=2 ** 16, r: int=8, p: int=1) -> tuple[bytes, bytes]:
        ...
        try:
            ...
            if isinstance(password, str):
                ...
                password_bytes = password
            kdf = Scrypt(length=length, salt=salt, n=n, r=r, p=p, backend=default_backend())
            derived_key = kdf.derive(password_bytes)
            if self.audit_enabled:
                self._log_crypto_event(CryptoAuditEvent.KEY_DERIVED, {'algorithm': 'Scrypt', 'n': n, 'r': r, 'p': p, 'salt_length': len(salt), 'key_length': len(derived_key)})
            self.logger.debug('Derived key using Scrypt (n={n}, r={r}, p=%s)', p)
            return (derived_key, salt)
        except Exception as e:
            success = False
    ...
    def verify_hash(self, data: str | bytes, hash_value: str, salt: bytes, algorithm: HashAlgorithm=HashAlgorithm.SHA256) -> bool:
    ...
    def get_crypto_status(self) -> dict[str, Any]:
        ...
        return {'crypto_manager': {'security_level': self.security_level.value, 'audit_enabled': self.audit_enabled, 'last_security_check': self.last_security_check.isoformat(), 'failed_operations': len(self.failed_operations)}, 'key_management': key_status, 'performance_metrics': metrics, 'supported_algorithms': {'hash': [algo.value for algo in HashAlgorithm], 'random': [rtype.value for rtype in RandomType], 'encryption': ['Fernet', 'AES-256']}}
    ...
    def _log_crypto_event(self, event: CryptoAuditEvent, details: dict[str, Any]):
        ...
        if not self.audit_enabled:
            return
        audit_entry = {'timestamp': aware_utc_now().isoformat(), 'component': 'UnifiedCryptoManager', 'event': event.value, 'security_level': self.security_level.value, 'details': details}
        if event == CryptoAuditEvent.CRYPTO_ERROR:
            self.logger.error('CRYPTO_AUDIT: %s', audit_entry)
        else:
            self.logger.debug('CRYPTO_AUDIT: %s', audit_entry)

    def _handle_crypto_error(self, operation: str, error: Exception):
        """Handle cryptographic operation errors."""
        error_info = {'operation': operation, 'error_type': type(error).__name__, 'error_message': str(error), 'timestamp': aware_utc_now().isoformat()}
        ...
        self.logger.error('Crypto operation failed: {operation} - %s', error)
_default_crypto_manager: UnifiedCryptoManager | None = None

def get_crypto_manager(security_level: SecurityLevel=SecurityLevel.enhanced) -> UnifiedCryptoManager:
