...
# Keep cryptography imports here for centralization
...
from .key_manager import UnifiedKeyManager, SecurityLevel, AuditEvent, KeyRotationConfig
...
class HashAlgorithm(Enum):
    ...
    SHA256 = "sha256"
    SHA512 = "sha512"
    SHA1 = "sha1"          # Deprecated, for legacy compatibility only
    MD5 = "md5"            # Deprecated, for non-security use only

class RandomType(Enum):
    """Types of random data generation."""
    BYTES = "bytes"         # Raw bytes
    HEX = "hex"            # Hexadecimal string
    URLSAFE = "urlsafe"    # URL-safe base64
    TOKEN = "token"        # Human-readable token
...
class UnifiedCryptoManager:
    ...
    def __init__(self, key_manager: Optional[UnifiedKeyManager] = None, 
    ...
    # ============================================================================
    # Hashing Operations (Standardized)
    # ============================================================================
    ...
    def hash_data(self, data: Union[str, bytes], algorithm: HashAlgorithm = HashAlgorithm.SHA256,
        ...
        try:
            ...
            self.logger.debug(f"Generated {algorithm.value} hash (length: {len(hash_result)})")
            return hash_result
    ...
    def hash_sha256(self, data: Union[str, bytes], truncate_length: Optional[int] = None) -> str:
        """Generate SHA-256 hash (convenience method)."""
        return self.hash_data(data, HashAlgorithm.SHA256, truncate_length)
    
    def hash_sha512(self, data: Union[str, bytes], truncate_length: Optional[int] = None) -> str:
        """Generate SHA-512 hash (convenience method)."""
        return self.hash_data(data, HashAlgorithm.SHA512, truncate_length)
    
    def hash_md5(self, data: Union[str, bytes], truncate_length: Optional[int] = None) -> str:
    ...
    def generate_secure_hash(self, data: Union[str, bytes], salt: Optional[bytes] = None) -> Tuple[str, bytes]:
        ...
        """
        if salt is None:
            salt = self.generate_random_bytes(32)  # 256-bit salt
    ...
    # ============================================================================
    # Random Generation (Standardized)
    # ============================================================================
    
    def generate_random(self, length: int, random_type: RandomType = RandomType.BYTES) -> Union[bytes, str]:
        ...
        try:
            ...
            self.logger.debug(f"Generated {random_type.value} random data (length: {length})")
            return result
    ...
    def encrypt_string(self, text: str, key_id: Optional[str] = None) -> Tuple[bytes, str]:
    ...
    def derive_key_scrypt(self, password: Union[str, bytes], salt: Optional[bytes] = None,
                         length: int = 32, n: int = 2**16, r: int = 8, p: int = 1) -> Tuple[bytes, bytes]:
    ...
    def verify_hash(self, data: Union[str, bytes], hash_value: str, salt: bytes,
        ...
        """
        # Generate hash of data with salt
        if isinstance(data, str):
            data_bytes = data.encode('utf-8')
        else:
            data_bytes = data
...
_default_crypto_manager: Optional[UnifiedCryptoManager] = None
...
def hash_sha256(data: Union[str, bytes], truncate_length: Optional[int] = None) -> str:
    """Generate SHA-256 hash (migration-friendly function)."""
    return get_crypto_manager().hash_sha256(data, truncate_length)

def hash_md5(data: Union[str, bytes], truncate_length: Optional[int] = None) -> str:
...
def encrypt_data(data: Union[str, bytes], key_id: Optional[str] = None) -> Tuple[bytes, str]:
