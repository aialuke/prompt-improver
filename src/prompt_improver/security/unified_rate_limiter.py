...
Integrates with Phase 1 DatabaseServices and new SecureRedisManager.
"""
...
class RateLimitStatus(BaseModel):
    ...
    requests_remaining: int = Field(
        ge=0, description="Number of requests remaining in current window"
    )
    ...
    agent_id: str = Field(
        min_length=1, max_length=100, description="Authenticated agent identifier"
    )
...
class UnifiedRateLimiter:
    ...
    async def initialize(self) -> None:
        """Initialize unified rate limiter with enhanced DatabaseServices."""
        try:
            self._connection_manager = await get_database_services(
                ManagerMode.ASYNC_MODERN
            )
            logger.info(
                "Unified rate limiter initialized with enhanced DatabaseServices"
            )
    ...
    async def check_rate_limit(
        self, agent_id: str, tier: str = "basic", authenticated: bool = True
    ) -> RateLimitStatus:
        ...
        try:
            ...
            if not self._connection_manager.cache.redis_client:
                logger.error("Redis master not available in DatabaseServices")
                return RateLimitStatus(
        ...
        def decorator(func):
            async def wrapper(*args, **kwargs):
                ...
                await self.enforce_rate_limit(
                    agent_id=str(agent_id), tier=tier, authenticated=True
                )
