...
class OWASP2025InputValidator:
    ...
    def __init__(self, max_prompt_length: int=10240):
        ...
        self.html_injection_patterns = ['<script[^>]*>.*?</script>', '<img[^>]+src\\s*=\\s*["\\\']?[^"\\\'>\\s]+', '<iframe[^>]*>.*?</iframe>', 'javascript:', 'data:text/html', '<object[^>]*>.*?</object>', '<embed[^>]*>.*?</embed>']
        self.system_extraction_patterns = ['system\\s*[:]\\s*you\\s+are', 'instructions?\\s*[:]\\s*\\d+\\.', 'api[_\\s]key[:=]\\s*\\w+', 'password[:=]\\s*\\w+', 'secret[:=]\\s*\\w+']
    ...
    def _contains_malicious_patterns(self, text: str) -> bool:
        ...
        malicious_keywords = ['ignore', 'bypass', 'override', 'reveal', 'system', 'instructions', 'administrator', 'delete', 'sudo', 'root']
    ...
    def calculate_threat_score(self, direct_injection: bool, typoglycemia: bool, encoding: bool, html_injection: bool) -> float:
    ...
    def validate_prompt(self, prompt: str) -> ValidationResult:
        ...
        if not prompt or len(prompt.strip()) == 0:
            return ValidationResult(is_valid=False, is_blocked=True, threat_type=None, threat_score=0.0, sanitized_input='', detected_patterns=['Empty prompt'], confidence=1.0)
        if len(prompt) > self.max_prompt_length:
            return ValidationResult(is_valid=False, is_blocked=True, threat_type=None, threat_score=0.3, sanitized_input=prompt[:self.max_prompt_length], detected_patterns=[f'Prompt too long: {len(prompt)} > {self.max_prompt_length}'], confidence=1.0)
        detected_patterns = []
        threat_type = None
        ...
        threat_score = self.calculate_threat_score(direct_injection, typoglycemia_detected, encoding_detected, html_detected)
        is_blocked = threat_score >= 0.5
        sanitized_input = self.sanitize_input(prompt)
        return ValidationResult(is_valid=not is_blocked, is_blocked=is_blocked, threat_type=threat_type, threat_score=threat_score, sanitized_input=sanitized_input, detected_patterns=detected_patterns, confidence=0.9 if detected_patterns else 0.1)
