...
class PreparedStatementCache:
    ...
    async def _analyze_cache_performance(self, **kwargs) -> dict[str, Any]:
        ...
        return {'component': 'PreparedStatementCache', 'analysis_type': 'cache_performance', 'cache_size': len(self._statements), 'max_cache_size': self._max_size, 'cache_utilization': len(self._statements) / self._max_size if self._max_size > 0 else 0, 'total_entries': len(self._statements), 'usage_statistics': dict(self._usage_count), 'most_used_queries': sorted(self._usage_count.items(), key=lambda x: x[1], reverse=True)[:5], 'cache_efficiency': 'high' if len(self._statements) / self._max_size > 0.7 else 'medium' if len(self._statements) / self._max_size > 0.3 else 'low', 'timestamp': datetime.now(UTC).isoformat()}
    ...
    async def _analyze_query_optimization(self, **kwargs) -> dict[str, Any]:
        ...
        return {'component': 'PreparedStatementCache', 'analysis_type': 'query_optimization', 'total_cached_queries': len(self._statements), 'query_complexity_distribution': query_complexity, 'optimization_recommendations': self._generate_optimization_recommendations(), 'cache_hit_potential': sum(self._usage_count.values()) / len(self._usage_count) if self._usage_count else 0, 'timestamp': datetime.now(UTC).isoformat()}
    ...
    async def _analyze_cache_efficiency(self, **kwargs) -> dict[str, Any]:
        ...
        if not self._usage_count:
            return {'component': 'PreparedStatementCache', 'analysis_type': 'cache_efficiency', 'status': 'no_data', 'message': 'No usage data available for analysis'}
        ...
        return {'component': 'PreparedStatementCache', 'analysis_type': 'cache_efficiency', 'total_usage': total_usage, 'average_usage': avg_usage, 'hot_queries_count': len(hot_queries), 'cold_queries_count': len(cold_queries), 'efficiency_score': min(1.0, len(hot_queries) / len(self._usage_count) * 2) if self._usage_count else 0, 'recommendations': self._generate_cache_recommendations(hot_queries, cold_queries), 'timestamp': datetime.now(UTC).isoformat()}
...
class OptimizedQueryExecutor:
    ...
    def __init__(self):
        ...
        self._cache_stats = {'hits': 0, 'misses': 0, 'cache_time_saved_ms': 0.0, 'total_cached_queries': 0}

    @asynccontextmanager
    async def execute_optimized_query(self, session: AsyncSession, query: str, params: dict[str, Any] | None=None, cache_ttl: int=300, enable_cache: bool=True):
        ...
        if cache_key and enable_cache:
            ...
            try:
                ...
                if cached_result:
                    ...
                    query_metrics = QueryPerformanceMetrics(query_hash=self._prepared_cache._hash_query_structure(query), execution_time_ms=cache_time, rows_returned=len(cached_result) if isinstance(cached_result, list) else 1, cache_hit=True, timestamp=datetime.now(UTC))
                    self._performance_metrics.append(query_metrics)
                    logger.debug('Cache hit for query %s... (saved ~%sms)', query[:50], format(50 - cache_time, '.1f'))
                    yield {'result': cached_result, 'cache_hit': True, 'execution_time_ms': cache_time}
                    return
                else:
        ...
        async with measure_database_operation('optimized_query') as perf_metrics:
            ...
            try:
                ...
                if cache_key and enable_cache and (execution_time < 1000):
                    try:
                        import json
                        serialized_rows = json.dumps([{k: str(v) if not isinstance(v, (str, int, float, bool, type(None))) else v for k, v in row.items()} for row in rows], default=str).encode()
                        await self._query_cache.set(cache_key, serialized_rows, expire=cache_ttl)
                        self._cache_stats['total_cached_queries'] += 1
                        logger.debug('Cached query result: %s... (%s rows, TTL: %ss)', query[:50], len(rows), cache_ttl)
                    except Exception as e:
                        logger.warning('Failed to cache query result: %s', e)
                query_metrics = QueryPerformanceMetrics(query_hash=self._prepared_cache._hash_query_structure(query), execution_time_ms=execution_time, rows_returned=len(rows), cache_hit=False, timestamp=datetime.now(UTC))
                self._performance_metrics.append(query_metrics)
                if len(self._performance_metrics) > 1000:
                    self._performance_metrics = self._performance_metrics[-1000:]
                if execution_time > 50:
                    logger.warning('Slow query detected: %sms (target: <50ms) - %s...', format(execution_time, '.2f'), query[:100])
                yield {'result': rows, 'cache_hit': False, 'execution_time_ms': execution_time}
            except Exception as e:
    ...
    def _generate_cache_key(self, query: str, params: dict[str, Any]) -> str:
        ...
        import hashlib
        import json
        cache_data = {'query': query.strip(), 'params': sorted(params.items()) if params else []}
    ...
    async def get_performance_summary(self) -> dict[str, Any]:
        ...
        recent_metrics = [m for m in self._performance_metrics if m.timestamp > datetime.now(UTC) - timedelta(hours=1)]
        ...
        return {'total_queries': len(recent_metrics), 'avg_execution_time_ms': sum(execution_times) / len(execution_times), 'max_execution_time_ms': max(execution_times), 'min_execution_time_ms': min(execution_times), 'queries_meeting_target': sum((1 for t in execution_times if t <= 50)), 'target_compliance_rate': sum((1 for t in execution_times if t <= 50)) / len(execution_times), 'cache_hit_rate': cache_hit_rate, 'cache_hits': self._cache_stats['hits'], 'cache_misses': self._cache_stats['misses'], 'cache_time_saved_ms': self._cache_stats['cache_time_saved_ms'], 'total_cached_queries': self._cache_stats['total_cached_queries'], 'estimated_db_load_reduction_percent': round(estimated_db_load_reduction, 1), 'slow_queries_count': sum((1 for t in execution_times if t > 50))}
...
class DatabaseConnectionOptimizer:
    ...
    def _get_system_resources() -> dict[str, Any]:
        ...
        try:
            ...
            return {'total_memory_gb': memory.total / 1024 ** 3, 'available_memory_gb': memory.available / 1024 ** 3, 'memory_percent': memory.percent, 'cpu_count': cpu_count, 'logical_cpu_count': psutil.cpu_count(logical=True)}
        except Exception as e:
            logger.warning('Failed to get system resources: %s', e)
            return {'total_memory_gb': 4.0, 'available_memory_gb': 2.0, 'memory_percent': 50.0, 'cpu_count': 2, 'logical_cpu_count': 4}
    ...
    def _calculate_optimal_memory_settings(system_resources: dict[str, Any]) -> dict[str, str]:
        ...
        return {'work_mem': f'{work_mem_mb}MB', 'effective_cache_size': f'{effective_cache_size_gb}GB'}
    ...
    async def optimize_connection_settings(self):
        ...
        logger.info('Optimizing database settings for system with %sGB total memory, %s CPU cores', format(system_resources['total_memory_gb'], '.1f'), system_resources['cpu_count'])
        optimization_queries = ["SET plan_cache_mode = 'force_generic_plan'", f"SET work_mem = '{memory_settings['work_mem']}'", f"SET effective_cache_size = '{memory_settings['effective_cache_size']}'", 'SET random_page_cost = 1.1', f"SET max_parallel_workers_per_gather = {min(system_resources['cpu_count'], 4)}", 'SET checkpoint_completion_target = 0.9', 'SET jit = on', 'SET jit_above_cost = 100000']
        async with client.connection() as conn:
            applied_settings = []
            for query in optimization_queries:
                try:
                    await conn.execute(query)
                    logger.debug('Applied optimization: %s', query)
                    applied_settings.append(query)
                except Exception as e:
                    logger.warning('Failed to apply optimization {query}: %s', e)
    ...
    async def _emit_optimization_event(self, applied_settings: list, system_resources: dict, memory_settings: dict):
        ...
        try:
            ...
            await self.event_bus.emit(MLEvent(event_type=EventType.DATABASE_CONNECTION_OPTIMIZED, source='database_connection_optimizer', data={'applied_settings': applied_settings, 'system_resources': system_resources, 'memory_settings': memory_settings, 'optimization_timestamp': datetime.now().isoformat()}))
        except Exception as e:
            logger.debug('Failed to emit optimization event: %s', e)
    ...
    async def _emit_index_creation_event(self, created_indexes: list):
        ...
        try:
            from prompt_improver.ml.orchestration.events.event_types import EventType, MLEvent
            await self.event_bus.emit(MLEvent(event_type=EventType.DATABASE_INDEXES_CREATED, source='database_connection_optimizer', data={'created_indexes': created_indexes, 'index_count': len(created_indexes), 'creation_timestamp': datetime.now().isoformat()}))
        except Exception as e:
            logger.debug('Failed to emit index creation event: %s', e)
    ...
    async def _emit_resource_analysis_event(self, system_resources: dict, memory_settings: dict):
        ...
        try:
            from prompt_improver.ml.orchestration.events.event_types import EventType, MLEvent
            await self.event_bus.emit(MLEvent(event_type=EventType.DATABASE_RESOURCE_ANALYSIS_COMPLETED, source='database_connection_optimizer', data={'system_resources': system_resources, 'recommended_settings': memory_settings, 'analysis_timestamp': datetime.now().isoformat()}))
        except Exception as e:
            logger.debug('Failed to emit resource analysis event: %s', e)
...
async def execute_optimized_query(session: AsyncSession, query: str, params: dict[str, Any] | None=None, cache_ttl: int=300, enable_cache: bool=True):
    """Execute an optimized query with caching and performance monitoring."""
    executor = get_query_executor()
    async with executor.execute_optimized_query(session, query, params, cache_ttl, enable_cache) as result:
        return result
