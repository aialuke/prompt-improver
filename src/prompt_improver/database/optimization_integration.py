...
# Removed non-existent database.cache_layer imports
# Using CacheFacade for all caching needs
...
class DatabaseOptimizationIntegration:
    ...
    def __init__(self) -> None:
        ...
        self._ml_container: MLServiceContainer | None = None

    async def initialize(self, cache_policy: dict[str, Any] | None = None):
        """Initialize all optimization components."""
        if self._initialized:
            return
        logger.info("Initializing database optimization components...")
        # Use CacheFacade for all caching needs
        cache_config = cache_policy or {}
        l1_max_size = cache_config.get('l1_max_size', 500)
        l2_default_ttl = cache_config.get('l2_default_ttl', 600)

        self.pool_optimizer = await get_database_services(ManagerMode.ASYNC_MODERN)
        self.query_cache = CacheFacade(l1_max_size=l1_max_size, l2_default_ttl=l2_default_ttl, enable_l2=True)
    ...
    async def _setup_event_handlers(self) -> None:
        ...
        class DatabaseOptimizationHandler:
            ...
            def __init__(self, integration_instance) -> None:
                self.integration = integration_instance

            async def handle_performance_degradation(self, metrics) -> None:
                ...
                if avg_query_time > 100:
                    logger.warning("Slow queries detected, optimizing cache configuration")
                    # CacheFacade handles optimization internally
                    await self.integration.query_cache.clear()

                if cache_hit_ratio < 50:
                    logger.warning("Low cache hit ratio, clearing cache for fresh start")
                    await self.integration.query_cache.clear()

            async def handle_cache_optimization(self, cache_stats) -> None:
                """Handle cache optimization requests."""
                logger.info("Cache optimization requested")
                # CacheFacade handles optimization internally
                await self.integration.query_cache.clear()

            async def handle_query_optimization(self, query_data) -> None:
    ...
    async def _warm_cache(self) -> None:
        ...
        # Warm cache by executing queries through CacheFacade
        for query, params in warm_queries:
            try:
                cache_key = f"warm_{hash(query)}"
                ...
                logger.warning(f"Failed to warm cache with query {query[:50]}...: {e}")
        ...
        while True:
            try:
                ...
                if snapshot.cache_hit_ratio < 70:
                    # CacheFacade handles cache optimization internally
                    logger.info("Low cache hit ratio detected, cache will self-optimize")
    ...
    async def get_optimization_status(self) -> dict[str, Any]:
        ...
        cache_stats = self.query_cache.get_performance_stats()
        ...
        # Calculate cache reduction from hit rate
        hit_rate = cache_stats.get("hit_rate", 0)
        cache_reduction = hit_rate * 0.8  # Estimate 80% load reduction per cache hit
        ...
        return {
            ...
            "cache_performance": {
                "hit_rate": cache_stats.get("hit_rate", 0),
                "queries_cached": cache_stats.get("total_requests", 0),
                "db_calls_avoided": cache_stats.get("l1_hits", 0) + cache_stats.get("l2_hits", 0),
                "mb_saved": cache_stats.get("memory_usage_mb", 0),
            },
            "pool_performance": {
