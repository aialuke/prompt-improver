...
class DatabaseErrorClassifier:
    ...
    ERROR_MAPPINGS = {asyncpg.ConnectionDoesNotExistError: (ErrorCategory.connection, ErrorSeverity.high), asyncpg.ConnectionFailureError: (ErrorCategory.connection, ErrorSeverity.high), asyncpg.InterfaceError: (ErrorCategory.connection, ErrorSeverity.high), asyncio.CancelledError: (ErrorCategory.timeout, ErrorSeverity.medium), asyncpg.PostgresError: (ErrorCategory.transient, ErrorSeverity.medium), asyncpg.IntegrityConstraintViolationError: (ErrorCategory.constraint, ErrorSeverity.low), asyncpg.UniqueViolationError: (ErrorCategory.constraint, ErrorSeverity.low), asyncpg.ForeignKeyViolationError: (ErrorCategory.constraint, ErrorSeverity.low), asyncpg.SyntaxOrAccessError: (ErrorCategory.syntax, ErrorSeverity.high), asyncpg.UndefinedTableError: (ErrorCategory.syntax, ErrorSeverity.high), asyncpg.UndefinedColumnError: (ErrorCategory.syntax, ErrorSeverity.high), asyncpg.InsufficientPrivilegeError: (ErrorCategory.permission, ErrorSeverity.high), asyncpg.PostgresError: (ErrorCategory.resource, ErrorSeverity.critical)}
    ...
    def is_retryable(cls, error: Exception) -> bool:
        ...
        retryable_categories = {ErrorCategory.connection, ErrorCategory.timeout, ErrorCategory.transient, ErrorCategory.resource}
...
class ErrorMetrics:
    ...
    def __init__(self) -> None:
        ...
        self.error_counter = self.metrics_registry.get_or_create_counter(StandardMetrics.DATABASE_ERRORS_TOTAL, 'Total database errors', ['category', 'severity', 'operation'])
        self.retry_counter = self.metrics_registry.get_or_create_counter('database_retries_total', 'Total database retry attempts', ['operation', 'attempt'])
        self.circuit_breaker_gauge = self.metrics_registry.get_or_create_gauge('database_circuit_breaker_state', 'Database circuit breaker state', ['operation'])

    def record_error(self, category: ErrorCategory, severity: ErrorSeverity, operation: str) -> None:
        """Record database error metrics."""
        self.error_counter.labels(category=category.value, severity=severity.value, operation=operation).inc()
...
def enhance_error_context(error: Exception, operation: str, **kwargs) -> dict[str, Any]:
    ...
    context = {'error_type': type(error).__name__, 'error_message': str(error), 'category': category.value, 'severity': severity.value, 'operation': operation, 'timestamp': aware_utc_now().isoformat(), 'retryable': DatabaseErrorClassifier.is_retryable(error), **kwargs}
...
def create_database_error_classifier():
    ...
    def classify_for_retry(error: Exception):
        ...
        from prompt_improver.core.retry_manager import RetryableErrorType
        category, _ = DatabaseErrorClassifier.classify_error(error)
        category_mapping = {ErrorCategory.connection: RetryableErrorType.NETWORK, ErrorCategory.timeout: RetryableErrorType.TIMEOUT, ErrorCategory.transient: RetryableErrorType.TRANSIENT, ErrorCategory.resource: RetryableErrorType.RESOURCE_EXHAUSTION}
...
def get_default_database_retry_config():
    ...
    from prompt_improver.core.retry_manager import RetryableErrorType, RetryConfig, RetryStrategy
    return RetryConfig(max_attempts=3, strategy=RetryStrategy.EXPONENTIAL_BACKOFF, base_delay=0.1, max_delay=10.0, backoff_multiplier=2.0, jitter=True, enable_circuit_breaker=True, failure_threshold=5, recovery_timeout_seconds=60.0, retryable_errors=[RetryableErrorType.NETWORK, RetryableErrorType.TIMEOUT, RetryableErrorType.TRANSIENT, RetryableErrorType.RESOURCE_EXHAUSTION], error_classifier=create_database_error_classifier(), enable_metrics=True, enable_tracing=True)
