...
from typing import Any, TypeVar, Type, Optional, Dict, List, Callable, Union
...
class DatabaseErrorClassifier:
    ...
    TIMEOUT_ERRORS = {
        "57014", "25P04", "57P05"  # QueryCanceled, TransactionTimeout, IdleSessionTimeout
    }
    ...
    PERMISSION_ERRORS = {
        "28000", "28P01", "42501"  # InvalidAuthorizationSpecification, InvalidPassword, InsufficientPrivilege
    }
    
    RESOURCE_ERRORS = {
        "53000", "53200", "53300"  # InsufficientResources, OutOfMemory, TooManyConnections
    }
    
    SYNTAX_ERRORS = {
        "42000", "42601", "42602", "42611", "42622", "42701", "42702", 
        "42703", "42704", "42710", "42712", "42723", "42725", "42803",
        "42804", "42809", "42830", "42846", "42883", "428C9", "42939",
        "42P01", "42P02", "42P03", "42P04", "42P05", "42P06", "42P07",
        "42P08", "42P09", "42P10", "42P11", "42P12", "42P13", "42P14",
        "42P15", "42P16", "42P17", "42P18", "42P19", "42P20", "42P21", "42P22"
    }
    
    DATA_ERRORS = {
        "22000", "22001", "22002", "22003", "22004", "22005", "22007",
        "22008", "22009", "2200B", "2200C", "2200D", "2200F", "2200G",
        "2200H", "2200L", "2200M", "2200N", "2200S", "2200T", "22010",
        "22011", "22012", "22013", "22014", "22015", "22016", "22018",
        "22019", "2201B", "2201E", "2201F", "2201G", "2201W", "2201X",
        "22021", "22022", "22023", "22024", "22025", "22026", "22027",
        "2202E", "2202G", "2202H", "22030", "22031", "22032", "22033",
        "22034", "22035", "22036", "22037", "22038", "22039", "2203A",
        "2203B", "2203C", "2203D", "2203E", "2203F", "2203G", "22P01",
        "22P02", "22P03", "22P04", "22P05", "22P06"
    }
    ...
    def classify_error(cls, error: Exception) -> tuple[ErrorCategory, ErrorSeverity]:
        ...
        if isinstance(error, psycopg_errors.Error):
            ...
            elif isinstance(error, psycopg_errors.IntegrityError):
                return ErrorCategory.INTEGRITY, ErrorSeverity.MEDIUM
            elif isinstance(error, psycopg_errors.ProgrammingError):
                return ErrorCategory.SYNTAX, ErrorSeverity.LOW
            elif isinstance(error, psycopg_errors.DataError):
                return ErrorCategory.DATA, ErrorSeverity.MEDIUM
            elif isinstance(error, (psycopg_errors.ConnectionTimeout, 
        ...
        if 'connection' in error_type or 'connection failed' in error_message:
            ...
            return ErrorCategory.TIMEOUT, ErrorSeverity.HIGH
        elif 'integrity' in error_type or 'unique violation' in error_message:
            return ErrorCategory.INTEGRITY, ErrorSeverity.MEDIUM
        elif 'syntax' in error_message or 'programming' in error_type:
            return ErrorCategory.SYNTAX, ErrorSeverity.LOW
        
        return ErrorCategory.FATAL, ErrorSeverity.CRITICAL
    
    @classmethod
    def _classify_by_sqlstate(cls, sqlstate: str) -> tuple[ErrorCategory, ErrorSeverity]:
        """Classify error based on SQLSTATE code."""
        if sqlstate in cls.CONNECTION_ERRORS:
            return ErrorCategory.CONNECTION, ErrorSeverity.HIGH
        elif sqlstate in cls.TIMEOUT_ERRORS:
            return ErrorCategory.TIMEOUT, ErrorSeverity.HIGH
        elif sqlstate in cls.RESOURCE_ERRORS:
            return ErrorCategory.RESOURCE, ErrorSeverity.HIGH
        elif sqlstate in cls.TRANSIENT_ERRORS:
            return ErrorCategory.TRANSIENT, ErrorSeverity.MEDIUM
        elif sqlstate in cls.INTEGRITY_ERRORS:
            return ErrorCategory.INTEGRITY, ErrorSeverity.MEDIUM
        elif sqlstate in cls.PERMISSION_ERRORS:
            return ErrorCategory.PERMISSION, ErrorSeverity.HIGH
        elif sqlstate in cls.SYNTAX_ERRORS:
            return ErrorCategory.SYNTAX, ErrorSeverity.CRITICAL
        elif sqlstate in cls.DATA_ERRORS:
            return ErrorCategory.DATA, ErrorSeverity.MEDIUM
        else:
            return ErrorCategory.FATAL, ErrorSeverity.CRITICAL
    ...
    def is_retryable(cls, error: Exception) -> bool:
        ...
        if category in [ErrorCategory.CONNECTION, ErrorCategory.TIMEOUT, ErrorCategory.TRANSIENT]:
            return True
        
        # Check error message for patterns (useful for mock testing)
        error_message = str(error).lower()
        if any(pattern in error_message for pattern in [
            'connection failed', 'connection timeout', 'deadlock detected',
...
class RetryManager:
    ...
    async def retry_async(self, operation: Callable[[], Coroutine[Any, Any, T]], context: ErrorContext) -> T:
        ...
        for attempt in range(self.config.max_attempts):
            try:
                ...
                if attempt == self.config.max_attempts - 1:
                    logger.error(f"Max retry attempts ({self.config.max_attempts}) "
                               f"exceeded for operation {context.operation}")
                    break
                
                logger.warning(f"Retry attempt {attempt + 1} failed: {type(e).__name__}: {e}")
...
class ErrorMetrics:
    ...
    def get_error_rate(self, category: ErrorCategory, window_minutes: int = 60) -> float:
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rates_per_minute": {
                cat.value: self.get_error_rate(cat) 
