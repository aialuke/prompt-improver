...
class DatabaseConfig(BaseSettings):
    ...
    postgres_database: str = Field(default="apes_production", validation_alias="POSTGRES_DATABASE")
    ...
    pool_min_size: int = Field(default=4, validation_alias="DB_POOL_MIN_SIZE")        # Keep connections warm
    pool_max_size: int = Field(default=16, validation_alias="DB_POOL_MAX_SIZE")       # Handle traffic spikes
    pool_timeout: int = Field(default=10, validation_alias="DB_POOL_TIMEOUT")         # Fail fast pattern
    pool_max_lifetime: int = Field(default=1800, validation_alias="DB_POOL_MAX_LIFETIME")  # 30min max age
    pool_max_idle: int = Field(default=300, validation_alias="DB_POOL_MAX_IDLE")      # 5min idle timeout
    ...
    target_query_time_ms: int = Field(default=50, validation_alias="DB_TARGET_QUERY_TIME_MS")
    target_cache_hit_ratio: float = Field(default=0.90, validation_alias="DB_TARGET_CACHE_HIT_RATIO")
