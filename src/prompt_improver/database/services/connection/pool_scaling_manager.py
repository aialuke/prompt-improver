...
class PoolScalingManager:
    ...
    def __init__(self, shared_context: PoolSharedContext):
    ...
    async def perform_background_scaling_evaluation(self) -> None:
        ...
        try:
            ...
            if evaluation["scaling_needed"]:
                ...
                if self.context.pool_state in [PoolState.SCALING_UP, PoolState.SCALING_DOWN]:
                    self.context.pool_state = PoolState.HEALTHY
                    
        except Exception as e:
            logger.error(f"Error during background scaling evaluation: {e}")
    ...
    def _get_cooldown_remaining(self) -> float:
        ...
        remaining = max(0, self.context.scale_cooldown_seconds - elapsed)
        return remaining
