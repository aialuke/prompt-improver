...
class DatabaseTestAdapter:
    ...
    def __init__(self, config: Optional[TestConnectionConfig] = None):
        ...
        try:
            ...
            async with self.get_direct_connection() as conn:
                ...
                return {
                    ...
                    "recommended": False  # Only for testing/benchmarking
                }
        except Exception as e:
    ...
    async def comprehensive_health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": (
                "Use unified_manager for production, direct_connection only for performance testing"
            )
        }
    ...
    async def benchmark_connection_methods(self, iterations: int = 10) -> Dict[str, Any]:
        ...
        return {
            ...
            "unified_manager": {
                ...
                "avg_ms": round(sum(unified_times) / len(unified_times), 2) if unified_times else 0,
            ...
            "direct_connection": {
                ...
                "avg_ms": round(sum(direct_times) / len(direct_times), 2) if direct_times else 0,
                "min_ms": round(min(direct_times), 2) if direct_times else 0,
                "max_ms": round(max(direct_times), 2) if direct_times else 0,
            },
            "performance_ratio": (
                round(
                    (sum(direct_times) / len(direct_times)) / (sum(unified_times) / len(unified_times)), 2
                ) if unified_times and direct_times else 0
            )
        }


# Convenience functions for common use cases
async def get_test_adapter(config: Optional[TestConnectionConfig] = None) -> DatabaseTestAdapter:
