...
class IndexHealthAssessor:
    ...
    async def assess_index_health(self) -> IndexHealthReport:
        ...
        try:
            ...
            report.maintenance_recommendations = self._generate_maintenance_recommendations(report)
            
            assessment_time = (time.perf_counter() - start_time) * 1000
            logger.info(f"Index health assessment completed in {assessment_time:.2f}ms")
    ...
    def _assess_individual_index_health(self, index: IndexStats) -> None:
        ...
        elif index.usage_ratio < self.low_usage_threshold_ratio and not index.is_primary:
        ...
        if index.size_bytes > self.large_index_threshold_mb * 1024 * 1024:
            if index.usage_ratio < 0.5:
                recommendations.append("Large index with low usage - significant space savings possible")
        ...
        if len(index.columns) == 1 and not index.is_unique and not index.is_primary:
            recommendations.append("Single-column non-unique index - check for redundancy")
        
        # Performance recommendations
        if index.selectivity > 0.8:  # Low selectivity
            recommendations.append("Low selectivity - may not be effective")
        
        if index.tuples_read > index.tuples_fetched * 2:  # Reading many more tuples than fetching
            recommendations.append("High tuple read ratio - consider composite index")
        
        # Size-based recommendations
        if index.size_bytes > 500 * 1024 * 1024:  # > 500MB
            recommendations.append("Very large index - monitor growth and consider partitioning")
        ...
        try:
            async with get_session_context() as session:
                ...
                for row in result:
                    ...
                    avg_tuples_per_seq_scan = seq_tup_read / seq_scans if seq_scans > 0 else 0
                    seq_scan_ratio = seq_scans / (seq_scans + idx_scans) if (seq_scans + idx_scans) > 0 else 1
                    ...
                    suggestion = {
                        ...
                        "priority": "high" if seq_scan_ratio > 0.8 and avg_tuples_per_seq_scan > 10000 else "medium",
    ...
    def _generate_maintenance_recommendations(self, report: IndexHealthReport) -> List[str]:
        """
        ...
        if report.unused_indexes:
            ...
            unused_size_mb = sum(idx.size_bytes for idx in report.unused_indexes) / (1024 * 1024)
        ...
        if report.bloated_indexes:
            ...
            recommendations.append(
                f"REINDEX {bloated_count} bloated indexes"
            )
        
        # Missing indexes
        if report.missing_index_suggestions:
            high_priority = len([s for s in report.missing_index_suggestions if s.get("priority") == "high"])
            if high_priority > 0:
                recommendations.append(
        ...
        large_low_usage = [
            idx for idx in report.low_usage_indexes 
    ...
    def _get_redundancy_recommendation(self, row) -> str:
        ...
        elif relationship == "i1_subset_of_i2":
            return f"Consider dropping {row[2]} (subset of {row[3]})"
        elif relationship == "i2_subset_of_i1":
