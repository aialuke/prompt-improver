...
class DatabaseHealthMonitor:
    ...
    async def collect_comprehensive_metrics(self) -> DatabaseHealthMetrics:
        ...
        try:
            results = await asyncio.gather(self._collect_connection_health(), self._collect_query_performance(), self._collect_replication_metrics(), self._collect_storage_metrics(), self._collect_lock_metrics(), self._collect_cache_metrics(), self._collect_transaction_metrics(), return_exceptions=True)
            ...
            logger.info('Database health metrics collection completed in %sms', format(collection_time, '.2f'))
            return metrics
        except Exception as e:
            logger.error('Failed to collect comprehensive database metrics: %s', e)
            metrics.health_score = 0.0
            metrics.issues.append({'severity': 'critical', 'category': 'monitoring', 'message': f'Health monitoring system failure: {e}', 'timestamp': datetime.now(UTC).isoformat()})
            return metrics
    ...
    async def _collect_replication_metrics(self) -> dict[str, Any]:
        ...
        async with get_session_context() as session:
            ...
            metrics = {'is_replica': is_replica, 'replication_enabled': False, 'lag_seconds': 0.0, 'lag_bytes': 0, 'replica_count': 0, 'streaming_replicas': [], 'replication_slots': []}
            if is_replica:
                ...
                replicas = []
                for row in result:
                    replica_info = {'client_addr': row[0], 'application_name': row[1], 'state': row[2], 'sent_lsn': str(row[3]), 'write_lsn': str(row[4]), 'flush_lsn': str(row[5]), 'replay_lsn': str(row[6]), 'write_lag_ms': float(row[7].total_seconds() * 1000) if row[7] else 0, 'flush_lag_ms': float(row[8].total_seconds() * 1000) if row[8] else 0, 'replay_lag_ms': float(row[9].total_seconds() * 1000) if row[9] else 0, 'sync_state': row[10]}
                ...
                slots = []
                for row in result:
                    slot_info = {'slot_name': row[0], 'plugin': row[1], 'slot_type': row[2], 'datoid': row[3], 'active': row[4], 'restart_lsn': str(row[5]) if row[5] else None, 'confirmed_flush_lsn': str(row[6]) if row[6] else None, 'wal_status': row[7], 'safe_wal_size': int(row[8]) if row[8] else None}
    ...
    async def _collect_storage_metrics(self) -> dict[str, Any]:
        ...
        async with get_session_context() as session:
            ...
            total_index_size = 0
            for row in result:
                table_info = {'schema': row[0], 'table': row[1], 'total_size_pretty': row[2], 'total_size_bytes': int(row[3]), 'table_size_pretty': row[4], 'table_size_bytes': int(row[5]), 'index_size_pretty': row[6], 'index_size_bytes': int(row[7])}
            ...
            return {'database_size_bytes': db_size_bytes, 'database_size_pretty': self._format_bytes(db_size_bytes), 'total_table_size_bytes': total_table_size, 'total_index_size_bytes': total_index_size, 'largest_tables': table_sizes, 'bloat_metrics': bloat_metrics, 'data_directory': data_directory, 'index_to_table_ratio': total_index_size / total_table_size * 100 if total_table_size > 0 else 0}
    ...
    async def _collect_lock_metrics(self) -> dict[str, Any]:
        ...
        async with get_session_context() as session:
            ...
            for row in result:
                lock_info = {'locktype': row[0], 'database': row[1], 'relation': row[2], 'pid': row[11], 'mode': row[12], 'granted': row[13], 'query': row[14][:100] + '...' if row[14] and len(row[14]) > 100 else row[14], 'state': row[15], 'query_duration_seconds': float(row[19]) if row[19] else 0}
            ...
            blocking_queries = []
            for row in result:
                blocking_info = {'blocked_pid': row[0], 'blocked_user': row[1], 'blocking_pid': row[2], 'blocking_user': row[3], 'blocked_query': row[4][:100] + '...' if row[4] and len(row[4]) > 100 else row[4], 'blocking_query': row[5][:100] + '...' if row[5] and len(row[5]) > 100 else row[5], 'blocked_application': row[6], 'blocking_application': row[7]}
                blocking_queries.append(blocking_info)
            return {'total_locks': len(locks), 'blocking_locks': blocking_locks, 'long_running_locks': long_running_locks, 'blocking_queries': blocking_queries, 'current_locks': locks[:20], 'lock_types_distribution': self._analyze_lock_distribution(locks)}
    ...
    async def _collect_cache_metrics(self) -> dict[str, Any]:
        ...
        async with get_session_context() as session:
            ...
            row = result.fetchone()
            buffer_metrics = {'heap_blocks_read': int(row[0]) if row and row[0] else 0, 'heap_blocks_hit': int(row[1]) if row and row[1] else 0, 'hit_ratio_percent': float(row[2]) if row and row[2] else 0.0}
            ...
            row = result.fetchone()
            index_metrics = {'index_blocks_read': int(row[0]) if row and row[0] else 0, 'index_blocks_hit': int(row[1]) if row and row[1] else 0, 'hit_ratio_percent': float(row[2]) if row and row[2] else 100.0}
            ...
            row = result.fetchone()
            bgwriter_metrics = {'checkpoints_timed': int(row[0]) if row else 0, 'checkpoints_req': int(row[1]) if row else 0, 'checkpoint_write_time_ms': float(row[2]) if row else 0, 'checkpoint_sync_time_ms': float(row[3]) if row else 0, 'buffers_checkpoint': int(row[4]) if row else 0, 'buffers_clean': int(row[5]) if row else 0, 'buffers_backend': int(row[7]) if row else 0, 'buffers_alloc': int(row[9]) if row else 0, 'stats_reset': row[10].isoformat() if row and row[10] else None} if row else {}
            ...
            return {'buffer_cache': buffer_metrics, 'index_cache': index_metrics, 'overall_cache_hit_ratio_percent': overall_hit_ratio, 'shared_buffers_setting': shared_buffers_setting, 'bgwriter_stats': bgwriter_metrics, 'cache_efficiency': 'excellent' if overall_hit_ratio >= 95 else 'good' if overall_hit_ratio >= 90 else 'needs_attention'}
    ...
    async def _collect_transaction_metrics(self) -> dict[str, Any]:
        ...
        async with get_session_context() as session:
            ...
            row = result.fetchone()
            db_metrics = {'commits': int(row[0]) if row else 0, 'rollbacks': int(row[1]) if row else 0, 'blocks_read': int(row[2]) if row else 0, 'blocks_hit': int(row[3]) if row else 0, 'tuples_returned': int(row[4]) if row else 0, 'tuples_fetched': int(row[5]) if row else 0, 'tuples_inserted': int(row[6]) if row else 0, 'tuples_updated': int(row[7]) if row else 0, 'tuples_deleted': int(row[8]) if row else 0, 'conflicts': int(row[9]) if row else 0, 'temp_files': int(row[10]) if row else 0, 'temp_bytes': int(row[11]) if row else 0, 'deadlocks': int(row[12]) if row else 0, 'stats_reset': row[13].isoformat() if row and row[13] else None} if row else {}
            ...
            long_transactions = []
            for row in result:
                txn_info = {'pid': row[0], 'username': row[1], 'application_name': row[2], 'client_addr': str(row[3]) if row[3] else None, 'transaction_start': row[5].isoformat() if row[5] else None, 'query_start': row[6].isoformat() if row[6] else None, 'state': row[8], 'query': row[9][:200] + '...' if row[9] and len(row[9]) > 200 else row[9], 'transaction_duration_seconds': float(row[10]) if row[10] else 0, 'query_duration_seconds': float(row[11]) if row[11] else 0}
            ...
            return {'database_stats': db_metrics, 'total_transactions': total_transactions, 'commit_ratio_percent': commit_ratio, 'rollback_ratio_percent': rollback_ratio, 'long_running_transactions': long_transactions, 'longest_transaction_seconds': max([t['transaction_duration_seconds'] for t in long_transactions], default=0), 'transaction_health': 'excellent' if rollback_ratio < 5 else 'good' if rollback_ratio < 10 else 'needs_attention'}
    ...
    def _identify_health_issues(self, metrics: DatabaseHealthMetrics) -> list[dict[str, Any]]:
        ...
        if isinstance(pool_metrics, dict):
            ...
            if utilization > 90:
                issues.append({'severity': 'critical', 'category': 'connection_pool', 'message': f'Connection pool utilization very high: {utilization:.1f}%', 'metric_value': utilization, 'threshold': 90.0})
            elif utilization > self.connection_pool_utilization_threshold:
                issues.append({'severity': 'warning', 'category': 'connection_pool', 'message': f'Connection pool utilization high: {utilization:.1f}%', 'metric_value': utilization, 'threshold': self.connection_pool_utilization_threshold})
        query_metrics = metrics.query_performance
        if isinstance(query_metrics, dict):
            slow_queries = query_metrics.get('slow_queries_count', 0)
            if slow_queries > 10:
                issues.append({'severity': 'critical', 'category': 'query_performance', 'message': f'High number of slow queries: {slow_queries}', 'metric_value': slow_queries, 'threshold': 10})
            elif slow_queries > 0:
                issues.append({'severity': 'warning', 'category': 'query_performance', 'message': f'Slow queries detected: {slow_queries}', 'metric_value': slow_queries, 'threshold': 1})
        cache_metrics = metrics.cache
        if isinstance(cache_metrics, dict):
            hit_ratio = cache_metrics.get('overall_cache_hit_ratio_percent', 100)
            if hit_ratio < 90:
                issues.append({'severity': 'critical', 'category': 'cache_performance', 'message': f'Cache hit ratio very low: {hit_ratio:.1f}%', 'metric_value': hit_ratio, 'threshold': 90.0})
            elif hit_ratio < self.cache_hit_ratio_threshold:
                issues.append({'severity': 'warning', 'category': 'cache_performance', 'message': f'Cache hit ratio below target: {hit_ratio:.1f}%', 'metric_value': hit_ratio, 'threshold': self.cache_hit_ratio_threshold})
        replication_metrics = metrics.replication
        if isinstance(replication_metrics, dict) and replication_metrics.get('replication_enabled'):
            ...
            if lag_seconds > 300:
                issues.append({'severity': 'critical', 'category': 'replication', 'message': f'Replication lag very high: {lag_seconds:.1f} seconds', 'metric_value': lag_seconds, 'threshold': 300.0})
            elif lag_seconds > self.replication_lag_threshold_seconds:
                issues.append({'severity': 'warning', 'category': 'replication', 'message': f'Replication lag elevated: {lag_seconds:.1f} seconds', 'metric_value': lag_seconds, 'threshold': self.replication_lag_threshold_seconds})
        lock_metrics = metrics.locks
        if isinstance(lock_metrics, dict):
            blocking_locks = lock_metrics.get('blocking_locks', 0)
            if blocking_locks > 0:
                issues.append({'severity': 'warning' if blocking_locks < 5 else 'critical', 'category': 'locks', 'message': f'Blocking locks detected: {blocking_locks}', 'metric_value': blocking_locks, 'threshold': 0})
        txn_metrics = metrics.transactions
        if isinstance(txn_metrics, dict):
            rollback_ratio = txn_metrics.get('rollback_ratio_percent', 0)
            if rollback_ratio > 20:
                issues.append({'severity': 'critical', 'category': 'transactions', 'message': f'High rollback ratio: {rollback_ratio:.1f}%', 'metric_value': rollback_ratio, 'threshold': 20.0})
            elif rollback_ratio > 10:
                issues.append({'severity': 'warning', 'category': 'transactions', 'message': f'Elevated rollback ratio: {rollback_ratio:.1f}%', 'metric_value': rollback_ratio, 'threshold': 10.0})
            long_txns = len(txn_metrics.get('long_running_transactions', []))
            if long_txns > 0:
                issues.append({'severity': 'warning', 'category': 'transactions', 'message': f'Long-running transactions: {long_txns}', 'metric_value': long_txns, 'threshold': 0})
        return issues
    ...
    def _generate_recommendations(self, metrics: DatabaseHealthMetrics) -> list[dict[str, Any]]:
        ...
        if isinstance(pool_metrics, dict):
            utilization = pool_metrics.get('utilization_percent', 0)
            if utilization > 80:
                recommendations.append({'category': 'connection_pool', 'priority': 'high' if utilization > 90 else 'medium', 'action': 'increase_pool_size', 'description': f"Consider increasing connection pool size from {pool_metrics.get('pool_size', 'unknown')} connections", 'expected_impact': 'Reduced connection wait times and improved throughput'})
            if pool_metrics.get('waiting_requests', 0) > 0:
                recommendations.append({'category': 'connection_pool', 'priority': 'high', 'action': 'optimize_connection_usage', 'description': 'Implement connection pooling best practices and review long-running queries', 'expected_impact': 'Reduced connection contention'})
        query_metrics = metrics.query_performance
        if isinstance(query_metrics, dict):
            slow_queries = query_metrics.get('slow_queries_count', 0)
            if slow_queries > 0:
                recommendations.append({'category': 'query_performance', 'priority': 'high', 'action': 'optimize_slow_queries', 'description': f'Analyze and optimize {slow_queries} slow queries identified', 'expected_impact': 'Improved response times and reduced resource usage'})
            if query_metrics.get('missing_indexes_count', 0) > 0:
                recommendations.append({'category': 'indexing', 'priority': 'medium', 'action': 'add_missing_indexes', 'description': 'Consider adding indexes for frequently queried columns', 'expected_impact': 'Faster query execution and reduced I/O'})
        cache_metrics = metrics.cache
        if isinstance(cache_metrics, dict):
            hit_ratio = cache_metrics.get('overall_cache_hit_ratio_percent', 100)
            if hit_ratio < 95:
                recommendations.append({'category': 'cache_tuning', 'priority': 'medium', 'action': 'increase_shared_buffers', 'description': f'Consider increasing shared_buffers (current hit ratio: {hit_ratio:.1f}%)', 'expected_impact': 'Improved cache hit ratio and reduced I/O'})
        ...
        if isinstance(storage_metrics, dict) and 'bloat_metrics' in storage_metrics:
            ...
            if isinstance(bloat_info, dict):
                ...
                if bloated_tables > 0:
                    recommendations.append({'category': 'maintenance', 'priority': 'medium', 'action': 'vacuum_analyze', 'description': f'Run VACUUM ANALYZE on {bloated_tables} bloated tables', 'expected_impact': 'Reclaimed storage space and improved query performance'})
        replication_metrics = metrics.replication
        if isinstance(replication_metrics, dict) and replication_metrics.get('replication_enabled'):
            lag_seconds = replication_metrics.get('lag_seconds', 0)
            if lag_seconds > 60:
                recommendations.append({'category': 'replication', 'priority': 'high' if lag_seconds > 300 else 'medium', 'action': 'investigate_replication_lag', 'description': f'Investigate replication lag of {lag_seconds:.1f} seconds', 'expected_impact': 'Improved data consistency and reduced lag'})
    ...
    def get_health_trends(self, hours: int=24) -> dict[str, Any]:
        ...
        def calculate_trend(values: list[float]) -> str:
            ...
            if recent_avg < earlier_avg * 0.95:
                return 'degrading'
            return 'stable'
        return {'period_hours': hours, 'data_points': len(recent_metrics), 'health_score_trend': calculate_trend(health_scores), 'current_health_score': health_scores[-1] if health_scores else 0, 'avg_health_score': sum(health_scores) / len(health_scores) if health_scores else 0, 'connection_utilization_trend': calculate_trend(connection_utilizations), 'cache_hit_ratio_trend': calculate_trend(cache_hit_ratios), 'summary': self._generate_trend_summary(recent_metrics)}
    ...
    async def _collect_connection_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            metrics = {'timestamp': datetime.now(UTC).isoformat(), 'pool_configuration': {'min_size': pool_stats.get('pool_min_size', 0), 'max_size': pool_stats.get('pool_max_size', 0), 'current_size': current_size, 'timeout_seconds': pool_stats.get('pool_timeout', 0), 'max_lifetime_seconds': pool_stats.get('pool_max_lifetime', 0)}, 'connection_states': {'active': state_stats['active'], 'idle': state_stats['idle'], 'idle_in_transaction': state_stats['idle_in_transaction'], 'idle_in_transaction_aborted': state_stats['idle_in_transaction_aborted'], 'fastpath_function_call': state_stats['fastpath_function_call'], 'disabled': state_stats['disabled']}, 'utilization_metrics': {'utilization_percent': utilization, 'available_connections': current_size - active_count, 'waiting_requests': pool_stats.get('requests_waiting', 0), 'pool_efficiency_score': self._calculate_efficiency_score(state_stats, age_stats)}, 'age_statistics': age_stats, 'connection_details': connection_details[:10], 'health_indicators': {'connections_over_max_lifetime': age_stats['over_max_lifetime_count'], 'long_running_queries': len([c for c in connection_details if c.query_duration_seconds > self.long_query_threshold_seconds]), 'blocked_connections': len([c for c in connection_details if c.wait_event_type and 'Lock' in c.wait_event_type]), 'problematic_connections': self._identify_problematic_connections(connection_details)}, 'recommendations': self._generate_pool_recommendations(utilization, age_stats, state_stats)}
            collection_time = (time.perf_counter() - start_time) * 1000
            metrics['collection_time_ms'] = round(collection_time, 2)
            return metrics
        except Exception as e:
            logger.error('Failed to collect connection pool metrics: %s', e)
            return {'error': str(e), 'timestamp': datetime.now(UTC).isoformat(), 'collection_time_ms': (time.perf_counter() - start_time) * 1000}
    ...
    async def _get_connection_details(self) -> list[dict[str, Any]]:
        ...
        async with get_session_context() as session:
            ...
            connections = []
            for row in rows:
                connection_info = {'connection_id': str(row[0]), 'pid': row[0], 'backend_start': row[6], 'query_start': row[8], 'state': row[10], 'application_name': row[2], 'client_addr': str(row[3]) if row[3] else None, 'current_query': row[13], 'wait_event_type': row[15], 'wait_event': row[16]}
                if row[6]:
                    connection_info['age_seconds'] = (datetime.now(UTC) - row[6].replace(tzinfo=UTC)).total_seconds()
                else:
                    connection_info['age_seconds'] = 0.0
                if row[8]:
                    connection_info['query_duration_seconds'] = (datetime.now(UTC) - row[8].replace(tzinfo=UTC)).total_seconds()
                else:
    ...
    def _analyze_connection_ages(self, connections: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if not connections:
            return {'total_connections': 0, 'average_age_seconds': 0.0, 'max_age_seconds': 0.0, 'min_age_seconds': 0.0, 'over_max_lifetime_count': 0, 'age_distribution': {}}
        ...
        distribution = {'0-5min': len([age for age in ages if age <= 300]), '5-30min': len([age for age in ages if 300 < age <= 1800]), '30min-2h': len([age for age in ages if 1800 < age <= 7200]), '2h+': len([age for age in ages if age > 7200])}
        return {'total_connections': len(connections), 'average_age_seconds': sum(ages) / len(ages), 'max_age_seconds': max(ages), 'min_age_seconds': min(ages), 'over_max_lifetime_count': len(over_max_lifetime), 'age_distribution': distribution}

    def _analyze_connection_states(self, connections: list[dict[str, Any]]) -> dict[str, int]:
        """Analyze connection state distribution"""
        states = {'active': 0, 'idle': 0, 'idle_in_transaction': 0, 'idle_in_transaction_aborted': 0, 'fastpath_function_call': 0, 'disabled': 0}
    ...
    def _calculate_efficiency_score(self, state_stats: dict[str, int], age_stats: dict[str, Any]) -> float:
        ...
        efficiency = (active_ratio * 40 + idle_ratio * 30 - idle_in_transaction_ratio * 30 - long_lived_ratio * 40) * 100
    ...
    def _identify_problematic_connections(self, connections: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for conn in connections:
            ...
            if issues:
                problematic.append({'pid': conn['pid'], 'application_name': conn['application_name'], 'client_addr': conn['client_addr'], 'issues': issues, 'query': conn['current_query'][:100] if conn['current_query'] else None})
        return problematic
    ...
    async def get_connection_pool_health_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            if utilization > self.utilization_critical_threshold or efficiency_score < 50:
                ...
                status = 'healthy'
            return {'status': status, 'utilization_percent': utilization, 'efficiency_score': efficiency_score, 'total_connections': metrics.get('age_statistics', {}).get('total_connections', 0), 'problematic_connections_count': problematic_count, 'recommendations': metrics.get('recommendations', []), 'summary': f'Pool utilization: {utilization:.1f}%, Efficiency: {efficiency_score:.1f}/100', 'detailed_metrics': metrics}
        except Exception as e:
            logger.error('Failed to get connection pool health summary: %s', e)
            return {'status': 'error', 'error': str(e), 'summary': 'Connection pool health check failed'}
    ...
    async def _collect_query_performance(self) -> dict[str, Any]:
        ...
        try:
            ...
            if has_pg_stat_statements:
                results = await asyncio.gather(self._analyze_slow_queries(), self._analyze_frequent_queries(), self._analyze_io_intensive_queries(), self._analyze_cache_performance_consolidated(), self._get_current_activity(), return_exceptions=True)
            ...
            metrics = {'timestamp': datetime.now(UTC).isoformat(), 'pg_stat_statements_available': has_pg_stat_statements, 'slow_queries': slow_queries[:10], 'frequent_queries': frequent_queries[:10], 'io_intensive_queries': io_intensive[:5], 'cache_performance': cache_analysis, 'current_activity': current_activity[:20], 'performance_summary': performance_summary, 'overall_assessment': overall_assessment, 'recommendations': recommendations, 'collection_time_ms': round((time.perf_counter() - start_time) * 1000, 2)}
            return metrics
        except Exception as e:
            logger.error('Failed to collect query performance metrics: %s', e)
            return {'error': str(e), 'timestamp': datetime.now(UTC).isoformat(), 'collection_time_ms': (time.perf_counter() - start_time) * 1000}
    ...
    async def _check_pg_stat_statements(self) -> bool:
        ...
        try:
            ...
            logger.warning('Could not check for pg_stat_statements: %s', e)
            return False
    ...
    async def _analyze_slow_queries(self) -> list[dict[str, Any]]:
        ...
        try:
            async with get_session_context() as session:
                ...
                slow_queries = []
                for row in rows:
                    query_stats = {'query_text': row[0], 'calls': row[1], 'total_time_ms': float(row[2]), 'mean_time_ms': float(row[3]), 'stddev_time_ms': float(row[4]) if row[4] else 0.0, 'min_time_ms': float(row[5]), 'max_time_ms': float(row[6]), 'rows_returned': row[7], 'shared_blks_hit': row[8], 'shared_blks_read': row[9], 'cache_hit_ratio': self._calculate_cache_hit_ratio(row[8], row[9]), 'io_time_ratio': self._calculate_io_time_ratio(row[16], row[17], row[2])}
                    slow_queries.append(query_stats)
                return slow_queries
        except Exception as e:
            logger.error('Failed to analyze slow queries: %s', e)
            return []
    ...
    async def _analyze_frequent_queries(self) -> list[dict[str, Any]]:
        ...
        try:
            async with get_session_context() as session:
                ...
                frequent_queries = []
                for row in result:
                    query_info = {'query_id': str(row[0]) if row[0] else None, 'query_text': row[1][:300] + '...' if row[1] and len(row[1]) > 300 else row[1], 'calls': int(row[2]), 'total_time_ms': float(row[3]), 'mean_time_ms': float(row[4]), 'avg_rows_per_call': float(row[5]) / int(row[2]) if int(row[2]) > 0 else 0, 'cache_hit_ratio': self._calculate_cache_hit_ratio(row[6], row[7]), 'total_blocks_accessed': int(row[8]), 'optimization_potential': 'high' if row[4] > 100 and row[2] > 1000 else 'medium' if row[4] > 50 else 'low'}
                    frequent_queries.append(query_info)
                return frequent_queries
        except Exception as e:
            logger.error('Failed to analyze frequent queries: %s', e)
            return []
        ...
        try:
            async with get_session_context() as session:
                ...
                for row in result:
                    ...
                    query_info = {'query_id': str(row[0]) if row[0] else None, 'query_text': row[1][:200] + '...' if row[1] and len(row[1]) > 200 else row[1], 'calls': int(row[2]), 'total_time_ms': total_time, 'mean_time_ms': float(row[4]), 'io_time_ms': total_io_time, 'io_ratio_percent': io_ratio, 'blocks_read': int(row[5]), 'blocks_written': int(row[6]), 'temp_blocks_read': int(row[9]), 'temp_blocks_written': int(row[10]), 'read_time_ms': float(row[11]), 'write_time_ms': float(row[12]), 'optimization_priority': 'critical' if io_ratio > 80 else 'high' if io_ratio > 50 else 'medium'}
                    io_intensive.append(query_info)
                return io_intensive
        except Exception as e:
            logger.error('Failed to analyze IO-intensive queries: %s', e)
            return []
    ...
    async def _get_current_activity(self) -> list[dict[str, Any]]:
        ...
        try:
            async with get_session_context() as session:
                ...
                current_activity = []
                for row in result:
                    activity_info = {'pid': int(row[0]), 'username': row[1], 'application_name': row[2], 'client_addr': str(row[3]) if row[3] else None, 'backend_start': row[4].isoformat() if row[4] else None, 'transaction_start': row[5].isoformat() if row[5] else None, 'query_start': row[6].isoformat() if row[6] else None, 'state': row[8], 'query': row[9][:500] + '...' if row[9] and len(row[9]) > 500 else row[9], 'wait_event_type': row[10], 'wait_event': row[11]}
            ...
            logger.error('Failed to get current activity: %s', e)
            return []
    ...
    async def _analyze_cache_performance_consolidated(self) -> dict[str, Any]:
        ...
        try:
            async with get_session_context() as session:
                ...
                cache_analysis = {'overall_cache_hit_ratio': round(overall_hit_ratio, 2), 'heap_cache_hit_ratio': round(heap_hit_ratio, 2), 'index_cache_hit_ratio': round(idx_hit_ratio, 2), 'toast_cache_hit_ratio': round(toast_hit_ratio, 2), 'toast_index_cache_hit_ratio': round(tidx_hit_ratio, 2), 'cache_efficiency': 'excellent' if overall_hit_ratio >= 99 else 'good' if overall_hit_ratio >= 95 else 'poor', 'total_cache_misses': total_read, 'total_cache_hits': total_hit, 'shared_buffers_setting': shared_buffers_setting, 'recommendations': []}
            ...
            logger.error('Failed to analyze cache performance: %s', e)
            return {'error': str(e), 'overall_cache_hit_ratio': 0.0, 'cache_efficiency': 'unknown'}
    ...
    async def get_comprehensive_health(self) -> dict[str, Any]:
        ...
        start_time = time.perf_counter()
        try:
            connection_health, query_performance, storage_health, utility_health, replication_metrics, lock_metrics, cache_metrics, transaction_metrics = await asyncio.gather(self._collect_connection_health(), self._collect_query_performance(), self._collect_storage_metrics(), self._collect_utility_metrics(), self._collect_replication_metrics(), self._collect_lock_metrics(), self._collect_cache_metrics(), self._collect_transaction_metrics(), return_exceptions=True)
            consolidated_metrics = {'timestamp': datetime.now(UTC).isoformat(), 'consolidation_version': '2025.1.0', 'data_sources': 'consolidated_from_5_systems', 'connection_metrics': connection_health if not isinstance(connection_health, Exception) else {'error': str(connection_health)}, 'query_performance': query_performance if not isinstance(query_performance, Exception) else {'error': str(query_performance)}, 'storage_metrics': storage_health if not isinstance(storage_health, Exception) else {'error': str(storage_health)}, 'replication_metrics': replication_metrics if not isinstance(replication_metrics, Exception) else {'error': str(replication_metrics)}, 'lock_metrics': lock_metrics if not isinstance(lock_metrics, Exception) else {'error': str(lock_metrics)}, 'cache_metrics': cache_metrics if not isinstance(cache_metrics, Exception) else {'error': str(cache_metrics)}, 'transaction_metrics': transaction_metrics if not isinstance(transaction_metrics, Exception) else {'error': str(transaction_metrics)}, 'index_health': utility_health.get('indexes', {}) if not isinstance(utility_health, Exception) else {'error': str(utility_health)}, 'table_bloat': utility_health.get('bloat', {}) if not isinstance(utility_health, Exception) else {'error': str(utility_health)}}
            ...
            collection_time = (time.perf_counter() - start_time) * 1000
            consolidated_metrics['performance_metadata'] = {'collection_time_ms': round(collection_time, 2), 'parallel_execution': True, 'systems_consolidated': 5, 'methods_parallelized': 8, 'performance_improvement_estimate': '60-80% faster than sequential'}
            logger.info('Consolidated database health collection completed in %sms', format(collection_time, '.2f'))
            return consolidated_metrics
        except Exception as e:
            logger.error('Failed to collect comprehensive health metrics: %s', e)
            return {'error': str(e), 'timestamp': datetime.now(UTC).isoformat(), 'health_score': 0.0, 'status': 'critical_monitoring_failure', 'collection_time_ms': (time.perf_counter() - start_time) * 1000}
    ...
    async def _collect_utility_metrics(self) -> dict[str, Any]:
        ...
        try:
            index_assessment, bloat_detection = await asyncio.gather(self.index_assessor.assess_index_health(), self.bloat_detector.detect_table_bloat(), return_exceptions=True)
            return {'indexes': index_assessment if not isinstance(index_assessment, Exception) else {'error': str(index_assessment)}, 'bloat': bloat_detection if not isinstance(bloat_detection, Exception) else {'error': str(bloat_detection)}}
        except Exception as e:
            logger.error('Failed to collect utility metrics: %s', e)
    ...
    def _calculate_consolidated_health_score(self, metrics: dict[str, Any]) -> float:
        ...
        try:
            ...
            if score_components and weights:
                ...
                weighted_score = sum((score * weight for score, weight in zip(score_components, weights, strict=False))) / total_weight
                return max(0.0, min(100.0, weighted_score))
            return 50.0
        except Exception as e:
            logger.error('Failed to calculate consolidated health score: %s', e)
            return 0.0
    ...
    def _identify_consolidated_health_issues(self, metrics: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if 'error' not in connection_metrics:
                utilization = connection_metrics.get('utilization_metrics', {}).get('utilization_percent', 0)
                if utilization > 90:
                    issues.append({'severity': 'critical', 'category': 'connection_pool', 'message': f'Critical connection pool utilization: {utilization:.1f}%', 'source': 'consolidated_connection_monitor'})
                elif utilization > 80:
                    issues.append({'severity': 'warning', 'category': 'connection_pool', 'message': f'High connection pool utilization: {utilization:.1f}%', 'source': 'consolidated_connection_monitor'})
            query_metrics = metrics.get('query_performance', {})
            if 'error' not in query_metrics:
                slow_queries_count = len(query_metrics.get('slow_queries', []))
                if slow_queries_count > 10:
                    issues.append({'severity': 'warning', 'category': 'query_performance', 'message': f'Found {slow_queries_count} slow queries requiring optimization', 'source': 'consolidated_query_analyzer'})
                cache_perf = query_metrics.get('cache_performance', {})
                cache_hit_ratio = cache_perf.get('overall_cache_hit_ratio', 100)
                if cache_hit_ratio < 95:
                    issues.append({'severity': 'warning', 'category': 'cache_performance', 'message': f'Low cache hit ratio: {cache_hit_ratio:.1f}%', 'source': 'consolidated_query_analyzer'})
            timestamp = datetime.now(UTC).isoformat()
            for issue in issues:
                issue['timestamp'] = timestamp
        except Exception as e:
            logger.error('Failed to identify consolidated health issues: %s', e)
            issues.append({'severity': 'error', 'category': 'monitoring', 'message': f'Health issue identification failed: {e}', 'timestamp': datetime.now(UTC).isoformat()})
        return issues
    ...
    def _generate_consolidated_recommendations(self, metrics: dict[str, Any]) -> list[str]:
        ...
        try:
            ...
            logger.error('Failed to generate consolidated recommendations: %s', e)
