...
class TableBloatDetector:
    ...
    async def detect_table_bloat(self) -> Dict[str, Any]:
        ...
        try:
            ...
            report.total_bloat_bytes = (
                sum(t.bloat_bytes for t in report.bloated_tables) +
            ...
            logger.info(f"Table bloat detection completed in {detection_time:.2f}ms")
            ...
            return {
                ...
                "summary": {
                    ...
                    "potential_savings_mb": report.potential_space_savings_bytes / (1024 * 1024),
    ...
    async def _analyze_table_bloat(self) -> List[TableBloatInfo]:
        ...
        async with get_session_context() as session:
            ...
            for row in result:
                ...
                table_info.table_size_pretty = self._format_bytes(table_info.table_size_bytes)
                ...
                table_info.bloat_bytes, table_info.bloat_ratio_percent = self._estimate_table_bloat(
                    table_info, relpages, relallvisible
                )
                ...
                table_info.recommendations = self._generate_table_recommendations(table_info)
                
                # Set maintenance priority
                table_info.maintenance_priority = self._determine_maintenance_priority(table_info)
    ...
    async def _analyze_index_bloat(self) -> List[IndexBloatInfo]:
        ...
        async with get_session_context() as session:
            ...
            for row in result:
                ...
                index_info.index_size_pretty = self._format_bytes(index_info.index_size_bytes)
                ...
                index_info.bloat_bytes, index_info.bloat_ratio_percent = self._estimate_index_bloat(
                    index_info, int(row[4] or 0), int(row[5] or 0)
                )
                
                # Generate recommendations
                index_info.recommendations = self._generate_index_recommendations(index_info)
                
                index_bloat_list.append(index_info)
            
            return index_bloat_list
    
    def _estimate_table_bloat(self, table_info: TableBloatInfo, relpages: int, relallvisible: int) -> Tuple[int, float]:
        """
        ...
        try:
            ...
            # Add page overhead
            expected_pages = max(1, int(expected_tuple_space / (self.page_size - self.page_header_size)))
            ...
            dead_tuple_ratio = (table_info.dead_tuples / total_tuples) * 100 if total_tuples > 0 else 0
            if dead_tuple_ratio > self.dead_tuple_threshold_percent:
                # Estimate bloat based on dead tuples
                dead_tuple_bloat_bytes = int(table_info.table_size_bytes * (dead_tuple_ratio / 100))
    ...
    def _estimate_index_bloat(self, index_info: IndexBloatInfo, idx_scan: int, idx_tup_read: int) -> Tuple[int, float]:
        """
        Estimate index bloat (simplified approach)
        ...
        try:
            ...
            if idx_scan > 0 and idx_tup_read > 0:
                ...
                if read_efficiency < 0.1:  # Very low efficiency might indicate bloat
                    ...
                    bloat_bytes = int(index_info.index_size_bytes * (estimated_bloat_ratio / 100))
                    return bloat_bytes, estimated_bloat_ratio
            
            # For large indexes with no usage, assume some bloat
            if idx_scan == 0 and index_info.index_size_bytes > 10 * 1024 * 1024:  # > 10MB
                return int(index_info.index_size_bytes * 0.1), 10.0  # Assume 10% bloat
            
            return 0, 0.0
            
        except Exception as e:
            logger.debug(f"Index bloat estimation failed for {index_info.index_name}: {e}")
            return 0, 0.0
    ...
    def _assess_maintenance_needs(self, table_info: TableBloatInfo) -> None:
        ...
        if table_info.last_autovacuum:
            last_vacuum_time = table_info.last_autovacuum.replace(tzinfo=timezone.utc)
        if table_info.last_vacuum:
            manual_vacuum = table_info.last_vacuum.replace(tzinfo=timezone.utc)
        ...
        vacuum_age_days = (now - last_vacuum_time).days if last_vacuum_time else float('inf')
        dead_tuple_ratio = (table_info.dead_tuples / (table_info.live_tuples + table_info.dead_tuples) * 100) if (table_info.live_tuples + table_info.dead_tuples) > 0 else 0
        total_modifications = table_info.n_tup_ins + table_info.n_tup_upd + table_info.n_tup_del
        
        table_info.needs_vacuum = (
            vacuum_age_days > self.vacuum_threshold_days or
            dead_tuple_ratio > self.dead_tuple_threshold_percent or
            total_modifications > self.high_activity_threshold
        )
        
        # Check ANALYZE needs
        last_analyze_time = None
        if table_info.last_autoanalyze:
            last_analyze_time = table_info.last_autoanalyze.replace(tzinfo=timezone.utc)
        if table_info.last_analyze:
            manual_analyze = table_info.last_analyze.replace(tzinfo=timezone.utc)
            if not last_analyze_time or manual_analyze > last_analyze_time:
                last_analyze_time = manual_analyze
        
        analyze_age_days = (now - last_analyze_time).days if last_analyze_time else float('inf')
        
        table_info.needs_analyze = (
            analyze_age_days > self.analyze_threshold_days or
            total_modifications > self.high_activity_threshold
        )
    ...
    def _generate_table_recommendations(self, table_info: TableBloatInfo) -> List[str]:
        ...
        if table_info.bloat_ratio_percent > self.table_bloat_threshold_percent:
            if table_info.table_size_bytes > self.large_table_threshold_mb * 1024 * 1024:
                recommendations.append("Consider scheduled VACUUM during maintenance window")
            else:
        ...
        if total_modifications > self.high_activity_threshold:
            recommendations.append("High activity table - consider more frequent maintenance")
        ...
        if table_info.n_tup_upd > 0:
            ...
            if hot_update_ratio < 50:
                recommendations.append("Low HOT update ratio - consider increasing fillfactor")
    ...
    def _determine_maintenance_priority(self, table_info: TableBloatInfo) -> str:
        ...
        """
        # Critical priority
        if (table_info.bloat_ratio_percent > 60 or 
            table_info.dead_tuples > 100000 or
            (table_info.table_size_bytes > 1024 * 1024 * 1024 and table_info.bloat_ratio_percent > 30)):  # 1GB+ with 30%+ bloat
            return "critical"
        
        # High priority
        if (table_info.bloat_ratio_percent > 40 or
            table_info.needs_vacuum and table_info.table_size_bytes > 100 * 1024 * 1024):  # 100MB+
            return "high"
        
        # Medium priority
        if (table_info.bloat_ratio_percent > self.table_bloat_threshold_percent or
            table_info.needs_vacuum or table_info.needs_analyze):
            return "medium"
    ...
    def _generate_immediate_actions(self, report: BloatDetectionReport) -> List[str]:
        ...
        critical_tables = [t for t in report.bloated_tables if t.maintenance_priority == "critical"]
        if critical_tables:
            actions.append(f"URGENT: {len(critical_tables)} tables with critical bloat need immediate attention")
        
        # High-impact vacuum operations
        high_priority_vacuum = [t for t in report.tables_needing_vacuum if t.maintenance_priority in ["critical", "high"]]
        if high_priority_vacuum:
            table_names = [f"{t.schema_name}.{t.table_name}" for t in high_priority_vacuum[:3]]
            actions.append(f"Run VACUUM on high-priority tables: {', '.join(table_names)}")
        
        # Immediate reindex needs
        critical_indexes = [i for i in report.bloated_indexes if i.bloat_ratio_percent > 50]
        if critical_indexes:
            actions.append(f"REINDEX {len(critical_indexes)} critically bloated indexes")
        
        return actions
    
    def _generate_scheduled_maintenance(self, report: BloatDetectionReport) -> List[str]:
        """
        ...
        if report.tables_needing_vacuum:
            maintenance.append(f"Schedule regular VACUUM for {len(report.tables_needing_vacuum)} tables")
        
        # Analyze schedule
        if report.tables_needing_analyze:
            maintenance.append(f"Schedule ANALYZE for {len(report.tables_needing_analyze)} tables")
        
        # Bulk reindex operations
        if len(report.bloated_indexes) > 5:
            maintenance.append(f"Plan maintenance window for bulk REINDEX of {len(report.bloated_indexes)} indexes")
        
        # Autovacuum tuning
        high_activity_tables = [t for t in report.bloated_tables if (t.n_tup_ins + t.n_tup_upd + t.n_tup_del) > 50000]
        if high_activity_tables:
            maintenance.append(f"Review autovacuum settings for {len(high_activity_tables)} high-activity tables")
    ...
    def _calculate_bloat_health_score(self, report: BloatDetectionReport) -> float:
        ...
        critical_tables = len([t for t in report.bloated_tables if t.maintenance_priority == "critical"])
