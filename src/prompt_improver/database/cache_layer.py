...
class DatabaseCacheLayer:
    ...
    def _init_metrics(self):
        ...
        self.cache_hit_counter = self.metrics_registry.get_or_create_counter(
            StandardMetrics.CACHE_HITS_TOTAL, "Total database cache hits"
        )
        self.cache_miss_counter = self.metrics_registry.get_or_create_counter(
            StandardMetrics.CACHE_MISSES_TOTAL, "Total database cache misses"
        )
        self.cache_size_gauge = self.metrics_registry.get_or_create_gauge(
            "database_cache_size_bytes", "Current database cache size in bytes"
        )
            ...
            if self._should_cache(query, execution_time_ms):
                ...
                try:
                    ...
                    if len(serialized) <= self.policy.max_size_bytes:
                        with self.cache_operation_histogram.labels(
                            operation="set"
                        ).time():
                            await self.redis_cache.set(
                                cache_key, serialized, expire=ttl
                            )
                        self._stats["queries_cached"] += 1
                        self._stats["bytes_saved"] += len(serialized)
                        logger.debug(
                            f"Cached query result: {query[:50]}... (TTL: {ttl}s, Size: {len(serialized)} bytes)"
                        )
                    else:
                        logger.warning(
                            f"Result too large to cache: {len(serialized)} bytes"
                        )
                except Exception as e:
        ...
        try:
            ...
            while True:
                cursor, keys = await self.redis_cache.redis_client.scan(
                    cursor, match=f"db:query:*{pattern}*", count=100
                )
                if keys:
                    for key in keys:
    ...
    async def warm_cache(
        self, warm_queries: list[tuple[str, dict[str, Any], Callable]]
    ):
    ...
    async def get_cache_stats(self) -> dict[str, Any]:
        ...
        load_reduction = (
            (db_queries_without_cache - db_queries_with_cache)
            / db_queries_without_cache
            * 100
            if db_queries_without_cache > 0
            else 0
        )
        top_queries = sorted(
        ...
        return {
            ...
            "top_cached_queries": [{"key": k, "frequency": v} for k, v in top_queries],
    ...
    async def optimize_cache_policy(self):
        ...
        valuable_queries = [
            k
            for k, v in self._query_costs.items()
            if v > 50 and self._query_frequency.get(k, 0) > 3
        ]
        logger.info(
            f"Identified {len(valuable_queries)} high-value queries for priority caching"
        )
