...
class DomainKeywords:
    ...
    # Technical keywords
    software_development: set[str] = field(
    ...
    # Creative keywords
    creative_writing: set[str] = field(
        default_factory=lambda: {
            "story",
            "narrative",
            "character",
    ...
    content_creation: set[str] = field(
        default_factory=lambda: {
            "blog",
            "article",
            "post",
    ...
    # Business keywords
    business_analysis: set[str] = field(
        default_factory=lambda: {
            "business",
            "analysis",
            "strategy",
...
class DomainDetector:
            ...
            for token in doc:
                if token.text.lower() in self._get_all_technical_terms():
                    domain_entities.append(
                        Span(doc, token.i, token.i + 1, label="TECH_TERM")
                    )
    ...
    def detect_domain(self, text: str) -> DomainClassificationResult:
        ...
        for attr_name in dir(self.domain_keywords):
            ...
            if found_keywords:
                ...
                specificity_boost = sum(len(kw.split()) for kw in found_keywords) / len(
                    found_keywords
                )
        ...
        for domain_name, score in domain_scores.items():
            if domain_name in domain_mapping:
                ...
                if domain_enum in pattern_scores:
                    final_scores[domain_enum] = (
                        score + pattern_scores[domain_enum]
                    ) / 2
        ...
        if not final_scores:
            ...
            secondary_domains = [
                (domain, score) for domain, score in sorted_domains[1:4]
            ]
    ...
    def _calculate_technical_complexity(self, text: str, keywords_found: dict) -> float:
        ...
        total_technical_keywords = sum(
            len(keywords_found.get(domain, [])) for domain in technical_domains
        )
    ...
    def _calculate_domain_specificity(self, domain_scores: dict) -> float:
        ...
        score_variance = sum(
            (score - max_score) ** 2 for score in domain_scores.values()
        )
