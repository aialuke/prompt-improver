...
class TechnicalDomainExtractor(BaseDomainExtractor):
    ...
    def _analyze_technical_vocabulary(
        self, text: str, domain_result: DomainClassificationResult
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_technical_term_length": sum(len(term) for term in technical_terms)
            / max(len(technical_terms), 1),
        }
    ...
    def _analyze_documentation_patterns(self, text: str) -> dict[str, Any]:
        ...
        return {
            ...
            "documentation_completeness": sum([
                has_headings,
                has_lists,
                has_code_blocks,
            ])
...
class CreativeDomainExtractor(BaseDomainExtractor):
    ...
    def _analyze_narrative_structure(self, text: str) -> dict[str, Any]:
        ...
        dialogue_count = len(re.findall(r'"[^"]*"', text)) + len(
            re.findall(r"'[^']*'", text)
        )
...
class DomainFeatureExtractor:
    ...
    def extract_domain_features(self, text: str) -> DomainFeatures:
        ...
        all_domains = [domain_result.primary_domain] + [
            d[0] for d in domain_result.secondary_domains
        ]

        # Check if any domain is technical
        if any(self._is_technical_domain(d) for d in all_domains):
            features.technical_features = self.extractors["technical"].extract_features(
                text, domain_result
            )

        # Check if any domain is creative
        if any(self._is_creative_domain(d) for d in all_domains):
            features.creative_features = self.extractors["creative"].extract_features(
                text, domain_result
            )

        # Check if any domain is academic
        if any(self._is_academic_domain(d) for d in all_domains):
            features.academic_features = self.extractors["academic"].extract_features(
                text, domain_result
            )
        ...
        features.feature_vector, features.feature_names = self._create_feature_vector(
            features
        )
    ...
    def _extract_conversational_features(self, text: str) -> dict[str, Any]:
        ...
        features["sentiment_polarity"] = (positive_count - negative_count) / max(
    ...
    def _create_feature_vector(
        ...
        for key, value in features.technical_features.items():
            feature_vector.append(
                float(value) if isinstance(value, (int, float, bool)) else 0.0
            )
    ...
    def get_all_feature_names(self) -> list[str]:
        ...
        for extractor_name, extractor in self.extractors.items():
            ...
            feature_names.extend([
                f"{extractor_name}_{name}" for name in extractor_features
            ])
    ...
    def analyze_prompt_domain_suitability(
        self, text: str, target_domain: PromptDomain
    ) -> dict[str, Any]:
        ...
        secondary_match = any(
            domain == target_domain for domain, _ in features.secondary_domains
        )

        # Get domain-specific keywords
        target_keywords = self.domain_detector.get_domain_keywords(target_domain)
        found_keywords = sum(
            1 for keyword in target_keywords if keyword.lower() in text.lower()
        )
        ...
        if primary_match:
            ...
            secondary_score = next(
                (
                    score
                    for domain, score in features.secondary_domains
            ...
            )  # Low score for keyword-only match

        # Generate recommendations
        recommendations = []
        if suitability_score < 0.5:
            recommendations.append(
                f"Consider adding more {target_domain.value}-specific terminology"
            )
        ...
        if keyword_coverage < 0.1:
            ...
            recommendations.append(
                f"Consider including keywords like: {', '.join(sample_keywords)}"
            )
