...
class TestResponseTimeRequirements:
    ...
    async def test_session_cleanup_response_time(self, benchmark):
        ...
        store = SessionStore(maxsize=1000, ttl=0.01, cleanup_interval=0.1)  # Very short TTL
...
class TestPerformanceUnderLoad:
    ...
    def test_session_store_under_load(self, benchmark):
            ...
            try:
                async def run_load_test():
                    ...
                    for i in range(1000):
                        if i % 3 == 0:
                            # Set operation
                            operations.append(store.set(f"load_key_{i}", {
                                "user_id": f"user_{i}",
        ...
        assert success_rate > 0.7  # > 70% success rate (some operations may fail naturally)
