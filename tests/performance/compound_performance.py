...
from prompt_improver.database import ManagerMode, get_session_context, get_unified_manager
from prompt_improver.database.cache_layer import CachePolicy, CacheStrategy, DatabaseCacheLayer
...
from prompt_improver.ml.optimization.batch.enhanced_batch_processor import ChunkingStrategy, StreamingBatchConfig, StreamingBatchProcessor
...
class CompoundPerformanceMetrics:
    ...
    def record_system_snapshot(self):
        ...
        snapshot = {'timestamp': time.time(), 'memory_used_gb': memory_info.used / 1024 ** 3, 'memory_percent': memory_info.percent, 'cpu_percent': cpu_info, 'processes': len(psutil.pids()), 'network_connections': len(psutil.net_connections())}
    ...
    def calculate_compound_effects(self) -> dict[str, Any]:
        ...
        for interference_key, impact in self.interference_analysis.items():
            results['interference_summary'][interference_key] = {'performance_degradation': impact.get('degradation_percent', 0), 'resource_conflict': impact.get('resource_conflict', False), 'mitigation_effective': impact.get('mitigation_effective', True)}
        if self.system_snapshots:
            memory_usage = [s['memory_used_gb'] for s in self.system_snapshots]
            cpu_usage = [s['cpu_percent'] for s in self.system_snapshots]
            results['system_efficiency'] = {'peak_memory_gb': max(memory_usage), 'avg_memory_gb': sum(memory_usage) / len(memory_usage), 'peak_cpu_percent': max(cpu_usage), 'avg_cpu_percent': sum(cpu_usage) / len(cpu_usage), 'memory_stability': max(memory_usage) - min(memory_usage), 'resource_efficiency_score': self._calculate_efficiency_score(memory_usage, cpu_usage)}
        return results
    ...
    def generate_compound_performance_report(self) -> str:
        ...
        report = ['# Compound Performance Validation Report', f'Generated: {datetime.now(UTC).isoformat()}', '', '## Executive Summary', f'- Components Tested: {len(self.individual_improvements)}', f'- Compound Tests: {len(self.compound_measurements)}', f'- Interference Analyses: {len(self.interference_analysis)}', f'- Performance Conflicts: {len(self.performance_conflicts)}', f'- System Snapshots: {len(self.system_snapshots)}', '']
        ...
        if 'system_efficiency' in compound_effects:
            ...
            report.extend(['## System Resource Efficiency', f"- Peak Memory Usage: {eff['peak_memory_gb']:.2f} GB", f"- Average Memory Usage: {eff['avg_memory_gb']:.2f} GB", f"- Peak CPU Usage: {eff['peak_cpu_percent']:.1f}%", f"- Average CPU Usage: {eff['avg_cpu_percent']:.1f}%", f"- Memory Stability: {eff['memory_stability']:.2f} GB variation", f"- Resource Efficiency Score: {eff['resource_efficiency_score']:.1f}/100", ''])
...
class TestCompoundPerformance:
    ...
    async def cache_layer(self):
        ...
        policy = CachePolicy(ttl_seconds=300, strategy=CacheStrategy.SMART, warm_on_startup=False)
    ...
    async def ml_orchestrator(self):
        ...
        config = OrchestratorConfig(max_concurrent_workflows=20, component_health_check_interval=2, training_timeout=300, debug_mode=False, enable_performance_profiling=True)
    ...
    @pytest.mark.asyncio
    async def test_database_compound_performance(self, compound_metrics: CompoundPerformanceMetrics, db_client, cache_layer: DatabaseCacheLayer, connection_optimizer):
        """
        ...
        try:
            ...
            test_queries = [('SELECT id, name, description FROM rules WHERE active = true LIMIT 20', {}), ("SELECT COUNT(*) FROM sessions WHERE created_at > NOW() - INTERVAL '2 hours'", {}), ('SELECT r.*, COUNT(pi.id) as improvements FROM rules r LEFT JOIN prompt_improvements pi ON r.id = pi.rule_id GROUP BY r.id LIMIT 10', {}), ('SELECT * FROM prompt_improvements WHERE effectiveness_score > 0.8 ORDER BY created_at DESC LIMIT 15', {})]
            ...
            async with get_session_context() as session:
                for _ in range(3):
                    for query, params in test_queries:
                        query_start = time.perf_counter()
                        async with query_executor.execute_optimized_query(session, query, params, cache_ttl=300, enable_cache=True) as result:
                            query_time = time.perf_counter() - query_start
            ...
            compound_metrics.record_individual_improvement('database_cache', {'response_time_improvement_percent': cache_improvement, 'throughput_improvement_percent': qps_cache_improvement, 'avg_response_time_ms': cache_avg_time * 1000, 'queries_per_second': cache_qps})
            compound_metrics.record_compound_measurement('database_compound', {'baseline_avg_time_ms': baseline_avg_time * 1000, 'cache_avg_time_ms': cache_avg_time * 1000, 'compound_avg_time_ms': compound_avg_time * 1000, 'baseline_qps': baseline_qps, 'cache_qps': cache_qps, 'compound_qps': compound_qps, 'cache_improvement_percent': cache_improvement, 'compound_improvement_percent': compound_improvement, 'qps_compound_improvement_percent': qps_compound_improvement})
            expected_compound_improvement = cache_improvement * 1.2
            actual_additional_improvement = compound_improvement - cache_improvement
            interference_impact = {'expected_additional_improvement': expected_compound_improvement - cache_improvement, 'actual_additional_improvement': actual_additional_improvement, 'synergy_factor': actual_additional_improvement / (expected_compound_improvement - cache_improvement) if expected_compound_improvement - cache_improvement != 0 else 1, 'degradation_percent': max(0, (expected_compound_improvement - compound_improvement) / expected_compound_improvement * 100) if expected_compound_improvement != 0 else 0, 'resource_conflict': False, 'mitigation_effective': True}
            ...
            compound_metrics.record_performance_conflict({'type': 'database_compound_error', 'components': ['database_cache', 'connection_pooling', 'query_optimization'], 'impact': str(e), 'resolution': 'test_failed'})
            raise

    @pytest.mark.asyncio
    async def test_ml_batch_processing_compound_performance(self, compound_metrics: CompoundPerformanceMetrics, ml_orchestrator: MLPipelineOrchestrator):
        """
        ...
        try:
            ...
            generator = ProductionSyntheticDataGenerator(target_samples=test_data_size, generation_method='statistical', use_enhanced_scoring=True)
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.jsonl', delete=False) as f:
                for i, features in enumerate(synthetic_data.get('features', [])):
                    record = {'id': i, 'features': features.tolist() if hasattr(features, 'tolist') else features, 'label': synthetic_data.get('effectiveness_scores', [])[i] if i < len(synthetic_data.get('effectiveness_scores', [])) else 0, 'metadata': {'timestamp': datetime.now(UTC).isoformat()}}
            ...
            try:
                ...
                def simple_ml_processing(batch):
                    ...
                    for item in batch:
                        ...
                        processed_item = {'id': item['id'], 'processed_features': (features * 1.1).tolist(), 'label': item['label']}
                        processed.append(processed_item)
                    return processed
                baseline_config = StreamingBatchConfig(chunk_size=500, worker_processes=1, memory_limit_mb=200, chunking_strategy=ChunkingStrategy.FIXED_SIZE)
                baseline_start = time.perf_counter()
                async with StreamingBatchProcessor(baseline_config, simple_ml_processing) as processor:
                    baseline_metrics = await processor.process_dataset(data_source=temp_file, job_id='baseline_ml_test')
                ...
                def enhanced_ml_processing(batch):
                    ...
                    for item in batch:
                        features = np.array(item['features'])
                        normalized = (features - np.mean(features)) / (np.std(features) + 1e-08)
                        enhanced_features = np.concatenate([normalized, [np.mean(features), np.std(features), np.max(features), np.min(features)]])
                        processed_item = {'id': item['id'], 'original_features': features.tolist(), 'enhanced_features': enhanced_features.tolist(), 'feature_count': len(enhanced_features), 'label': item['label']}
                        processed.append(processed_item)
                    return processed
                enhanced_config = StreamingBatchConfig(chunk_size=1000, worker_processes=4, memory_limit_mb=500, chunking_strategy=ChunkingStrategy.ADAPTIVE)
                enhanced_start = time.perf_counter()
                async with StreamingBatchProcessor(enhanced_config, enhanced_ml_processing) as processor:
                    enhanced_metrics = await processor.process_dataset(data_source=temp_file, job_id='enhanced_ml_test')
                ...
                workflow_params = {'model_type': 'test_model', 'data_source': temp_file, 'processing_config': {'chunk_size': 1000, 'workers': 4, 'enhanced_features': True}, 'test_mode': True}
                workflow_ids = []
                for i in range(3):
                ...
                compound_metrics.record_individual_improvement('enhanced_batch_processing', {'processing_time_improvement_percent': batch_improvement, 'throughput_improvement_percent': throughput_improvement, 'memory_peak_mb': enhanced_metrics.memory_peak_mb, 'items_per_second': enhanced_throughput})
                compound_metrics.record_individual_improvement('ml_orchestrator', {'concurrent_workflows': len(workflow_ids), 'orchestrator_efficiency_items_per_sec': orchestrator_efficiency, 'efficiency_vs_baseline_factor': orchestrator_vs_baseline, 'healthy_components': sum((1 for h in component_health.values() if h)), 'total_components': len(component_health)})
                compound_metrics.record_compound_measurement('ml_batch_compound', {'baseline_time_sec': baseline_time, 'enhanced_time_sec': enhanced_time, 'orchestrator_time_sec': orchestrator_time, 'baseline_throughput': baseline_throughput, 'enhanced_throughput': enhanced_throughput, 'orchestrator_efficiency': orchestrator_efficiency, 'batch_improvement_percent': batch_improvement, 'compound_efficiency_factor': orchestrator_vs_baseline, 'memory_efficiency_mb_per_item': enhanced_metrics.memory_peak_mb / enhanced_metrics.items_processed})
                expected_orchestrator_efficiency = enhanced_throughput * len(workflow_ids) * 0.8
                ...
                interference_impact = {'expected_efficiency': expected_orchestrator_efficiency, 'actual_efficiency': orchestrator_efficiency, 'efficiency_ratio': actual_efficiency_ratio, 'degradation_percent': max(0, (1 - actual_efficiency_ratio) * 100), 'resource_conflict': enhanced_metrics.memory_peak_mb > 600, 'mitigation_effective': actual_efficiency_ratio >= 0.7}
            ...
            compound_metrics.record_performance_conflict({'type': 'ml_batch_compound_error', 'components': ['batch_processing', 'ml_orchestrator'], 'impact': str(e), 'resolution': 'test_failed'})
            raise

    @pytest.mark.asyncio
    async def test_memory_optimization_compound_performance(self, compound_metrics: CompoundPerformanceMetrics):
        """
        ...
        try:
            ...
            for i in range(10000):
                large_object = {'id': i, 'data': [j * 1.5 for j in range(100)], 'metadata': f'object_{i}_metadata_' * 10, 'nested': {'values': list(range(50)), 'computed': [k ** 2 for k in range(50)]}}
            ...
            memory_optimizer.configure_gc_settings({'gc_threshold_0': 1000, 'gc_threshold_1': 100, 'gc_threshold_2': 10})
            ...
            async def create_optimized_data():
                ...
                for i in range(10000):
                    ...
                    efficient_object = {'id': i, 'data': np.array(range(100), dtype=np.float32), 'metadata': f'obj_{i}', 'computed_sum': sum(range(50))}
            ...
            async def memory_intensive_async_operation(data_chunk):
                ...
                for item in data_chunk:
                    result = {'id': item['id'], 'processed_data': np.mean(item['data']), 'timestamp': time.time()}
            ...
            compound_metrics.record_individual_improvement('memory_optimization', {'memory_reduction_percent': memory_improvement, 'baseline_memory_mb': baseline_memory_usage, 'optimized_memory_mb': optimized_memory_usage, 'memory_efficiency_ratio': baseline_memory_usage / optimized_memory_usage if optimized_memory_usage > 0 else 1})
            compound_metrics.record_individual_improvement('async_memory_optimization', {'async_memory_efficiency_percent': async_memory_efficiency, 'processing_throughput_items_per_sec': processing_throughput, 'concurrent_chunks': len(chunks), 'total_processing_time_sec': async_time})
            compound_metrics.record_compound_measurement('memory_compound', {'baseline_memory_mb': baseline_memory_usage, 'optimized_memory_mb': optimized_memory_usage, 'async_memory_mb': async_memory_usage, 'memory_improvement_percent': memory_improvement, 'async_efficiency_percent': async_memory_efficiency, 'compound_improvement_percent': compound_memory_improvement, 'processing_throughput': processing_throughput, 'memory_per_item_kb': async_memory_usage * 1024 / total_items})
            expected_memory_usage = optimized_memory_usage * 1.2
            memory_overhead = max(0, async_memory_usage - expected_memory_usage)
            interference_impact = {'expected_memory_mb': expected_memory_usage, 'actual_memory_mb': async_memory_usage, 'memory_overhead_mb': memory_overhead, 'overhead_percentage': memory_overhead / expected_memory_usage * 100 if expected_memory_usage > 0 else 0, 'degradation_percent': max(0, memory_overhead / expected_memory_usage * 100) if expected_memory_usage > 0 else 0, 'resource_conflict': memory_overhead > 50, 'mitigation_effective': memory_overhead < 100}
            ...
            compound_metrics.record_performance_conflict({'type': 'memory_compound_error', 'components': ['memory_optimization', 'async_processing'], 'impact': str(e), 'resolution': 'test_failed'})
            raise

    @pytest.mark.asyncio
    async def test_overall_system_compound_performance(self, compound_metrics: CompoundPerformanceMetrics, db_client, cache_layer: DatabaseCacheLayer, ml_orchestrator: MLPipelineOrchestrator):
        """
        ...
        try:
            ...
            async def database_workload():
                ...
                async def execute_query(q, p):
                    return await db_client.fetch_raw(q, p)
                queries = [("SELECT COUNT(*) FROM sessions WHERE created_at > NOW() - INTERVAL '1 hour'", {}), ('SELECT id, name FROM rules WHERE active = true ORDER BY created_at DESC LIMIT 10', {}), ('SELECT r.*, COUNT(pi.id) as improvements FROM rules r LEFT JOIN prompt_improvements pi ON r.id = pi.rule_id GROUP BY r.id LIMIT 5', {})]
                for _ in range(50):
                ...
                return {'operations': operations, 'avg_time_ms': sum(db_times) / len(db_times) * 1000, 'total_time_sec': sum(db_times)}

            async def ml_workload():
                """Concurrent ML operations."""
                workflows = []
                for i in range(5):
                    workflow_params = {'model_type': f'system_test_model_{i}', 'quick_training': True, 'test_mode': True}
                ...
                return {'workflows': len(workflows), 'completed': completed, 'success_rate': completed / len(workflows) * 100}
            ...
            async def memory_intensive_workload():
                ...
                for chunk_idx in range(10):
                    chunk_data = []
                    for i in range(1000):
                        item = {'id': chunk_idx * 1000 + i, 'features': np.random.random(50).tolist(), 'metadata': f'chunk_{chunk_idx}_item_{i}'}
                        chunk_data.append(item)
                    processed_chunk = []
                    for item in chunk_data:
                        processed_item = {'id': item['id'], 'feature_sum': sum(item['features']), 'feature_mean': np.mean(item['features'])}
                    ...
                    if chunk_idx % 3 == 0:
                        ...
                        gc.collect()
                    await asyncio.sleep(0.1)
                return {'chunks_processed': len(data_chunks), 'total_items': sum((len(chunk) for chunk in data_chunks))}
            print('  🔄 Running concurrent workloads...')
            workload_tasks = [asyncio.create_task(database_workload()), asyncio.create_task(ml_workload()), asyncio.create_task(memory_intensive_workload())]
            monitoring_task = asyncio.create_task(self._monitor_system_during_compound_test(compound_metrics, workload_tasks))
            ...
            efficiency_factors = {'db_performance': 100 - min(100, db_result['avg_time_ms'] / 5), 'ml_success': ml_result['success_rate'], 'memory_efficiency': max(0, 100 - memory_usage / 10), 'throughput': min(100, operations_per_second * 10), 'resource_stability': 100 - (max((s['memory_percent'] for s in compound_metrics.system_snapshots[-10:])) - min((s['memory_percent'] for s in compound_metrics.system_snapshots[-10:]))) if len(compound_metrics.system_snapshots) >= 10 else 100}
            overall_efficiency = sum(efficiency_factors.values()) / len(efficiency_factors)
            compound_metrics.record_compound_measurement('overall_system_compound', {'total_duration_sec': system_duration, 'total_operations': total_operations, 'operations_per_second': operations_per_second, 'memory_usage_mb': memory_usage, 'db_avg_time_ms': db_result['avg_time_ms'], 'ml_success_rate_percent': ml_result['success_rate'], 'memory_items_processed': memory_result['total_items'], 'overall_efficiency_score': overall_efficiency, 'efficiency_factors': efficiency_factors})
            ...
            compound_metrics.record_performance_conflict({'type': 'overall_system_compound_error', 'components': ['database', 'ml_orchestrator', 'memory_optimization', 'async_processing'], 'impact': str(e), 'resolution': 'test_failed'})
            raise

    @pytest.mark.asyncio
    async def test_generate_compound_performance_report(self, compound_metrics: CompoundPerformanceMetrics):
        """
        ...
        detailed_metrics = {'baseline_metrics': compound_metrics.baseline_metrics, 'individual_improvements': compound_metrics.individual_improvements, 'compound_measurements': compound_metrics.compound_measurements, 'interference_analysis': compound_metrics.interference_analysis, 'performance_conflicts': compound_metrics.performance_conflicts, 'compound_effects': compound_effects, 'system_snapshots': compound_metrics.system_snapshots[-50:]}
    ...
    async def _monitor_system_during_compound_test(self, metrics: CompoundPerformanceMetrics, tasks: list[asyncio.Task]):
