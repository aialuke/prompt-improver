...
class BasicAsyncPGPerformanceTest:
    ...
    def __init__(self):
        self.manager = get_database_services(ManagerMode.ASYNC_MODERN)
    ...
    async def run_basic_performance_test(self) -> dict[str, BasicPerformanceResult]:
        ...
        for op_name, query, iterations in operations:
            ...
            logger.info(
                "%s: avg=%sms, min=%sms, max=%sms, success_rate=%s%%",
                op_name,
                format(result.avg_time_ms, ".2f"),
                format(result.min_time_ms, ".2f"),
                format(result.max_time_ms, ".2f"),
                format(result.success_rate, ".1f"),
            )
        ...
        logger.info(
            "Connection establishment: avg=%sms, success_rate=%s%%",
            format(conn_result.avg_time_ms, ".2f"),
            format(conn_result.success_rate, ".1f"),
        )
