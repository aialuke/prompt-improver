...
class TestCachePerformance:
    ...
    async def test_l1_cache_bulk_performance(self, l1_cache: L1CacheService):
        ...
        operations = [l1_cache.set(f"key_{i}", {"value": i}) for i in range(100)]
        ...
        get_operations = [l1_cache.get(f"key_{i}") for i in range(100)]
    ...
    @pytest.mark.asyncio
    async def test_direct_cache_facade_performance(self):
        """Test direct cache facade performance with cache-aside pattern."""
        cache_facade = CacheFacade(
            l1_max_size=50,
            enable_l2=False,  # Disable L2 for predictable performance
            enable_warming=False,
        )

        test_key = "direct_cache_test"
        test_value = {"direct": "test"}

        # Measure direct facade SET operation (cache-aside pattern)
        start_time = time.perf_counter()
        await cache_facade.set(test_key, test_value)
        set_time = time.perf_counter() - start_time

        assert set_time < 0.001, f"Direct facade SET took {set_time * 1000:.2f}ms, expected <1ms for L1-only"

        # Measure direct facade GET operation (L1 hit)
        start_time = time.perf_counter()
        result = await cache_facade.get(test_key)
        get_time = time.perf_counter() - start_time

        assert result == test_value
        assert get_time < 0.0005, f"Direct facade GET took {get_time * 1000:.3f}ms, expected <0.5ms for L1 hit"
    ...
    async def test_monitoring_service_performance(self):
        ...
        monitoring = CacheMonitoringService(cache_facade)
    ...
    async def test_slo_compliance_tracking(self):
        ...
        cache_facade = CacheFacade(l1_max_size=50, enable_l2=False, enable_warming=False)
        monitoring = CacheMonitoringService(cache_facade)

        # Perform operations to generate response time data
        for i in range(50):
            await cache_facade.set(f"slo_test_{i}", {"value": i})
            await cache_facade.get(f"slo_test_{i}")
