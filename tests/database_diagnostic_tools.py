...
class DatabaseDiagnostics:
    ...
    async def _check_docker_status(self) -> dict[str, any]:
        ...
        try:
            ...
            container_running = (
                "apes_postgres" in container_info and "Up" in container_info
            )
            status = {
                ...
                "status": "✅ HEALTHY"
                if docker_running and container_running
                else "❌ UNHEALTHY",
            }
        ...
        try:
            ...
            if health_info.get("status") == "healthy":
                results["unified_manager"] = {
                    "success": True,
                    "status": "✅ UNIFIED_MANAGER_HEALTHY",
                }
                print("  DatabaseServices: ✅")
            else:
                results["unified_manager"] = {
                    "success": False,
                    "status": "⚠️ DEGRADED",
                ...
                print(
                    f"  DatabaseServices: ⚠️ ({health_info.get('status', 'unknown')})"
                )
        except Exception as e:
            results["unified_manager"] = {
                "success": False,
    ...
    async def _validate_credentials(self) -> dict[str, any]:
        ...
        for key, expected in expected_config.items():
            if current_config[key] != expected:
                mismatches.append(
                    f"{key}: expected '{expected}', got '{current_config[key]}'"
                )
        status = {
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                production_databases = [
                    db for db in databases if not db.startswith("apes_test_")
                ]
                status = {
                ...
                if len(test_databases) > 10:
                    print(
                        f"  ⚠️  Warning: {len(test_databases)} test databases found (cleanup recommended)"
                    )
            else:
    ...
    async def _check_performance(self) -> dict[str, any]:
        ...
        try:
            ...
            async with manager.get_async_session() as session:
                ...
                unified_query_time = (
                    asyncio.get_event_loop().time() - query_start
                ) * 1000
            unified_connection_time = (
                asyncio.get_event_loop().time() - start_time
            ) * 1000
            print(
                f"  DatabaseServices connection: {unified_connection_time:.1f}ms"
            )
            print(f"  DatabaseServices query: {unified_query_time:.1f}ms")
            start_time = asyncio.get_event_loop().time()
            conn = await asyncpg.connect(
                host=self.config.postgres_host,
                port=self.config.postgres_port,
    ...
    def _generate_recommendations(self, results: dict[str, any]) -> list[str]:
        ...
        if not results["connection_tests"]["asyncpg"]["success"]:
            recommendations.append(
                "🔌 Check asyncpg connection - verify credentials and network"
            )
        ...
        if results["credential_validation"]["mismatches"]:
            recommendations.append(
                "🔑 Update database configuration to match Docker setup"
            )
        ...
        if (
            results["performance_check"]["connection_time_ms"]
            and results["performance_check"]["connection_time_ms"] > 1000
        ):
            recommendations.append(
                "⚡ Connection time is slow - check network and container resources"
            )
