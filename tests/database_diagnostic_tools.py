...
class DatabaseDiagnostics:
    ...
    async def run_comprehensive_check(self) -> dict[str, any]:
        ...
        print('=' * 60)
        results = {'timestamp': datetime.now().isoformat(), 'docker_status': await self._check_docker_status(), 'container_status': await self._check_container_status(), 'connection_tests': await self._test_connections(), 'credential_validation': await self._validate_credentials(), 'database_inventory': await self._inventory_databases(), 'cleanup_verification': await self._verify_cleanup_capability(), 'performance_check': await self._check_performance(), 'recommendations': []}
    ...
    async def _check_docker_status(self) -> dict[str, any]:
        ...
        try:
            ...
            result = subprocess.run(['docker', 'ps', '--filter', 'name=apes_postgres', '--format', 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'], check=False, capture_output=True, text=True, timeout=10)
            container_info = result.stdout.strip()
            container_running = 'apes_postgres' in container_info and 'Up' in container_info
            status = {'docker_daemon': docker_running, 'postgres_container': container_running, 'container_details': container_info if container_running else 'Container not running', 'status': '✅ HEALTHY' if docker_running and container_running else '❌ UNHEALTHY'}
            print(f"  Docker daemon: {('✅' if docker_running else '❌')}")
            print(f"  PostgreSQL container: {('✅' if container_running else '❌')}")
            return status
        except Exception as e:
            status = {'docker_daemon': False, 'postgres_container': False, 'error': str(e), 'status': '❌ ERROR'}
    ...
    async def _check_container_status(self) -> dict[str, any]:
        ...
        try:
            result = subprocess.run(['docker', 'exec', 'apes_postgres', 'pg_isready', '-U', 'apes_user'], check=False, capture_output=True, text=True, timeout=10)
            pg_ready = result.returncode == 0
            result = subprocess.run(['docker', 'exec', 'apes_postgres', 'psql', '-U', 'apes_user', '-d', 'apes_production', '-c', 'SELECT version();'], check=False, capture_output=True, text=True, timeout=10)
            db_accessible = result.returncode == 0
            pg_version = result.stdout.strip() if db_accessible else 'Unknown'
            result = subprocess.run(['docker', 'exec', 'apes_postgres', 'psql', '-U', 'apes_user', '-d', 'apes_production', '-c', 'SELECT current_user, session_user;'], check=False, capture_output=True, text=True, timeout=10)
            user_info = result.stdout.strip() if result.returncode == 0 else 'Permission check failed'
            status = {'postgres_ready': pg_ready, 'database_accessible': db_accessible, 'postgres_version': pg_version, 'user_permissions': user_info, 'status': '✅ HEALTHY' if pg_ready and db_accessible else '❌ UNHEALTHY'}
            print(f"  PostgreSQL ready: {('✅' if pg_ready else '❌')}")
            print(f"  Database accessible: {('✅' if db_accessible else '❌')}")
            return status
        except Exception as e:
            status = {'postgres_ready': False, 'database_accessible': False, 'error': str(e), 'status': '❌ ERROR'}
    ...
    async def _test_connections(self) -> dict[str, any]:
        ...
        try:
            conn = await asyncpg.connect(host=self.config.postgres_host, port=self.config.postgres_port, user=self.config.postgres_username, password=self.config.postgres_password, database=self.config.postgres_database, timeout=5.0)
            await conn.close()
        ...
        try:
            engine = create_async_engine(f'postgresql+asyncpg://{self.config.postgres_username}:{self.config.postgres_password}@{self.config.postgres_host}:{self.config.postgres_port}/{self.config.postgres_database}')
        ...
        try:
            result = subprocess.run(['docker', 'exec', 'apes_postgres', 'psql', '-U', self.config.postgres_username, '-d', self.config.postgres_database, '-c', 'SELECT 1;'], check=False, capture_output=True, text=True, timeout=10)
    ...
    async def _validate_credentials(self) -> dict[str, any]:
        ...
        expected_config = {'host': 'localhost', 'port': 5432, 'user': 'apes_user', 'password': 'apes_secure_password_2024', 'database': 'apes_production'}
        current_config = {'host': self.config.postgres_host, 'port': self.config.postgres_port, 'user': self.config.postgres_username, 'password': self.config.postgres_password, 'database': self.config.postgres_database}
        mismatches = []
        for key, expected in expected_config.items():
            if current_config[key] != expected:
                mismatches.append(f"{key}: expected '{expected}', got '{current_config[key]}'")
        status = {'expected_config': expected_config, 'current_config': current_config, 'mismatches': mismatches, 'status': '✅ VALID' if not mismatches else '❌ INVALID'}
        if mismatches:
    ...
    async def _inventory_databases(self) -> dict[str, any]:
        ...
        try:
            result = subprocess.run(['docker', 'exec', 'apes_postgres', 'psql', '-U', 'apes_user', '-d', 'apes_production', '-c', 'SELECT datname FROM pg_database ORDER BY datname;'], check=False, capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                ...
                status = {'total_databases': len(databases), 'production_databases': production_databases, 'test_databases': test_databases, 'test_db_count': len(test_databases), 'status': '✅ HEALTHY'}
                ...
                status = {'error': result.stderr, 'status': '❌ FAILED'}
    ...
    async def _verify_cleanup_capability(self) -> dict[str, any]:
        ...
        try:
            ...
            result = subprocess.run(['docker', 'exec', 'apes_postgres', 'psql', '-U', 'apes_user', '-d', 'apes_production', '-c', f'CREATE DATABASE "{test_db}";'], check=False, capture_output=True, text=True, timeout=10)
            if result.returncode != 0:
                return {'create_capability': False, 'error': result.stderr, 'status': '❌ FAILED'}
            result = subprocess.run(['docker', 'exec', 'apes_postgres', 'psql', '-U', 'apes_user', '-d', 'apes_production', '-c', f'DROP DATABASE "{test_db}";'], check=False, capture_output=True, text=True, timeout=10)
            drop_success = result.returncode == 0
            status = {'create_capability': True, 'drop_capability': drop_success, 'status': '✅ WORKING' if drop_success else '⚠️ PARTIAL'}
            print('  Database creation: ✅')
            print(f"  Database deletion: {('✅' if drop_success else '❌')}")
            return status
        except Exception as e:
            status = {'create_capability': False, 'drop_capability': False, 'error': str(e), 'status': '❌ ERROR'}
    ...
    async def _check_performance(self) -> dict[str, any]:
        ...
        try:
            ...
            await conn.close()
            result = subprocess.run(['docker', 'stats', 'apes_postgres', '--no-stream', '--format', 'table {{.CPUPerc}}\t{{.MemUsage}}'], check=False, capture_output=True, text=True, timeout=10)
            resource_info = result.stdout.strip() if result.returncode == 0 else 'Unable to get stats'
            status = {'connection_time_ms': round(connection_time, 2), 'query_time_ms': round(query_time, 2), 'resource_usage': resource_info, 'status': '✅ GOOD' if connection_time < 1000 else '⚠️ SLOW'}
            print(f'  Connection time: {connection_time:.2f}ms')
            print(f'  Query time: {query_time:.2f}ms')
            return status
        except Exception as e:
            status = {'connection_time_ms': None, 'query_time_ms': None, 'error': str(e), 'status': '❌ ERROR'}
