...
import pytest
from prompt_improver.database.analytics_query_interface import AnalyticsQueryInterface, AnalyticsQueryResult, MetricType, TimeGranularity, TimeSeriesPoint, TrendAnalysisResult
...
class TestAnalyticsQueryInterface:
    ...
    def sample_time_series_data(self):
        ...
        return [MagicMock(time_bucket=base_time + timedelta(days=i), avg_value=0.7 + i * 0.02, session_count=5 + i, std_dev=0.05, min_value=0.6 + i * 0.02, max_value=0.8 + i * 0.02) for i in range(7)]

    @pytest.fixture
    def sample_dashboard_data(self):
        """Sample dashboard metrics data"""
        return {'session_summary': [MagicMock(total_sessions=25, completed_sessions=20, running_sessions=3, failed_sessions=2, avg_duration_hours=2.5)], 'performance': [MagicMock(avg_performance=0.82, max_performance=0.95, min_performance=0.65, avg_improvement=0.15, performance_std=0.08)], 'efficiency': [MagicMock(avg_efficiency=0.35, avg_iterations=12.5, avg_training_hours=2.3)], 'errors': [MagicMock(total_sessions=25, failed_sessions=2), MagicMock(total_iterations=300, failed_iterations=15)]}
    ...
    async def test_get_session_performance_trends(self, analytics_interface, sample_time_series_data):
        ...
        with patch('prompt_improver.database.analytics_query_interface.execute_optimized_query') as mock_execute:
            ...
            result = await analytics_interface.get_session_performance_trends(start_date=datetime.now(UTC) - timedelta(days=7), end_date=datetime.now(UTC), granularity=TimeGranularity.DAY, metric_type=MetricType.PERFORMANCE)
    ...
    async def test_get_dashboard_metrics(self, analytics_interface, sample_dashboard_data):
        ...
        analytics_interface._get_session_summary_metrics = AsyncMock(return_value=sample_dashboard_data['session_summary'][0].__dict__)
        analytics_interface._get_performance_metrics = AsyncMock(return_value=sample_dashboard_data['performance'][0].__dict__)
        analytics_interface._get_efficiency_metrics = AsyncMock(return_value=sample_dashboard_data['efficiency'][0].__dict__)
        analytics_interface._get_error_metrics = AsyncMock(return_value={'session_error_rate': 0.08, 'iteration_error_rate': 0.05, 'total_failed_sessions': 2, 'total_failed_iterations': 15})
        analytics_interface._get_resource_utilization_metrics = AsyncMock(return_value={'avg_memory_usage_mb': 1200.0, 'peak_memory_usage_mb': 1800.0, 'avg_cpu_utilization': 0.65, 'total_compute_hours': 50.0})
        result = await analytics_interface.get_dashboard_metrics(time_range_hours=24, include_comparisons=False)
    ...
    async def test_get_dashboard_metrics_with_comparisons(self, analytics_interface, sample_dashboard_data):
        ...
        analytics_interface._get_session_summary_metrics = AsyncMock(side_effect=[{'total_sessions': 25, 'completed_sessions': 20}, {'total_sessions': 20, 'completed_sessions': 15}])
        analytics_interface._get_performance_metrics = AsyncMock(side_effect=[{'avg_performance': 0.82, 'avg_improvement': 0.15}, {'avg_performance': 0.78, 'avg_improvement': 0.12}])
        ...
        result = await analytics_interface.get_dashboard_metrics(time_range_hours=24, include_comparisons=True)
        assert 'current_period' in result
    ...
    async def test_get_session_comparison_data(self, analytics_interface):
        ...
        mock_result = [MagicMock(session_id='session_1', status='completed', started_at=datetime.now(UTC) - timedelta(hours=3), completed_at=datetime.now(UTC) - timedelta(hours=1), initial_performance=0.7, current_performance=0.85, best_performance=0.87, total_training_time_seconds=7200, current_iteration=15, total_iterations=15, successful_iterations=14, avg_iteration_duration=480.0, avg_improvement_score=0.05, avg_memory_usage=1200.0, peak_memory_usage=1500.0), MagicMock(session_id='session_2', status='completed', started_at=datetime.now(UTC) - timedelta(hours=4), completed_at=datetime.now(UTC) - timedelta(hours=2), initial_performance=0.65, current_performance=0.78, best_performance=0.8, total_training_time_seconds=7200, current_iteration=18, total_iterations=18, successful_iterations=16, avg_iteration_duration=400.0, avg_improvement_score=0.04, avg_memory_usage=1100.0, peak_memory_usage=1400.0)]
        with patch('prompt_improver.database.analytics_query_interface.execute_optimized_query') as mock_execute:
            mock_execute.return_value = mock_result
            result = await analytics_interface.get_session_comparison_data(session_ids=['session_1', 'session_2'], comparison_metrics=['performance', 'efficiency'])
            assert isinstance(result, dict)
    ...
    async def test_get_performance_distribution_analysis(self, analytics_interface):
        ...
        mock_histogram_result = [MagicMock(bucket=1, frequency=2, min_val=0.5, max_val=0.55, avg_val=0.52, probability=0.1), MagicMock(bucket=2, frequency=5, min_val=0.55, max_val=0.65, avg_val=0.6, probability=0.25), MagicMock(bucket=3, frequency=8, min_val=0.65, max_val=0.75, avg_val=0.7, probability=0.4), MagicMock(bucket=4, frequency=4, min_val=0.75, max_val=0.85, avg_val=0.8, probability=0.2), MagicMock(bucket=5, frequency=1, min_val=0.85, max_val=0.95, avg_val=0.9, probability=0.05)]
        mock_stats_result = [MagicMock(total_sessions=20, mean_performance=0.72, std_performance=0.12, q25=0.65, median=0.72, q75=0.8, p90=0.85, p95=0.9)]
        with patch('prompt_improver.database.analytics_query_interface.execute_optimized_query') as mock_execute:
            mock_execute.side_effect = [mock_histogram_result, mock_stats_result]
            result = await analytics_interface.get_performance_distribution_analysis(start_date=datetime.now(UTC) - timedelta(days=30), end_date=datetime.now(UTC), bucket_count=20)
            assert isinstance(result, dict)
    ...
    async def test_get_correlation_analysis(self, analytics_interface):
        ...
        mock_result = [MagicMock(perf_improvement_corr=0.65, perf_duration_corr=-0.25, perf_success_corr=0.45, improvement_duration_corr=-0.15, improvement_success_corr=0.35, duration_success_corr=-0.2, iter_duration_success_corr=-0.3, sample_size=50)]
        with patch('prompt_improver.database.analytics_query_interface.execute_optimized_query') as mock_execute:
            mock_execute.return_value = mock_result
            result = await analytics_interface.get_correlation_analysis(metrics=['performance', 'improvement', 'duration', 'success_rate'], start_date=datetime.now(UTC) - timedelta(days=30), end_date=datetime.now(UTC))
            assert isinstance(result, dict)
    ...
    async def test_period_changes_calculation(self, analytics_interface):
        ...
        current_data = {'session_summary': {'total_sessions': 25}, 'performance': {'avg_performance': 0.82}}
        previous_data = {'session_summary': {'total_sessions': 20}, 'performance': {'avg_performance': 0.78}}
