...
from prompt_improver.cli.core.progress_preservation import ProgressPreservationManager, ProgressSnapshot
...
class TestAsyncSignalHandler:
    ...
    async def test_graceful_shutdown_execution(self, signal_handler):
        ...
        signal_handler.shutdown_context = ShutdownContext(reason=ShutdownReason.USER_INTERRUPT, timeout=30)
...
class TestProgressPreservationManager:
    ...
    def sample_progress_snapshot(self):
        ...
        return ProgressSnapshot(session_id='test_session_123', iteration=5, timestamp=datetime.now(UTC), performance_metrics={'accuracy': 0.85, 'loss': 0.15}, rule_optimizations={'rule_1': {'param': 'value'}}, synthetic_data_generated=100, workflow_state={'workflow_id': 'wf_123', 'status': 'running'}, model_checkpoints=['checkpoint_1.pkl', 'checkpoint_2.pkl'], improvement_score=0.05)
    ...
    async def test_backup_file_rotation(self, progress_manager):
        ...
        for i in range(55):
            snapshot = ProgressSnapshot(session_id=session_id, iteration=i, timestamp=datetime.now(UTC), performance_metrics={'iteration': i}, rule_optimizations={}, synthetic_data_generated=0, workflow_state={}, model_checkpoints=[], improvement_score=0.0)
...
class TestEnhancedStopCommand:
    ...
    def mock_training_manager(self):
        ...
        manager = AsyncMock()
        manager.get_active_sessions.return_value = [MagicMock(session_id='session_1', total_iterations=10, best_performance={'accuracy': 0.8}), MagicMock(session_id='session_2', total_iterations=5, best_performance={'accuracy': 0.7})]
    ...
    def mock_cli_orchestrator(self):
        ...
        orchestrator.stop_training_gracefully.return_value = {'success': True, 'progress_saved': True, 'saved_data': 'session_state'}
    ...
    @pytest.mark.asyncio
    async def test_graceful_stop_single_session(self, mock_training_manager, mock_cli_orchestrator, mock_progress_manager):
        ...
        if graceful:
            result = await mock_cli_orchestrator.stop_training_gracefully(session_id=session.session_id, timeout=30, save_progress=save_progress)
            assert result['success'] is True
            assert result['progress_saved'] is True
            if export_results:
                export_path = await mock_progress_manager.export_session_results(session_id=session.session_id, export_format=export_format, include_iterations=True)
    ...
    @pytest.mark.asyncio
    async def test_force_stop_all_sessions(self, mock_training_manager, mock_cli_orchestrator, mock_progress_manager):
        ...
        for session in active_sessions:
            if save_progress:
                await mock_progress_manager.save_training_progress(session_id=session.session_id, iteration=session.total_iterations, performance_metrics=session.best_performance, rule_optimizations={}, workflow_state={'force_shutdown': True}, improvement_score=0.0)
        ...
        mock_progress_manager.cleanup_old_backups.assert_called_once_with(days_to_keep=30)
        mock_training_manager.stop_training_system.assert_called_once_with(graceful=False)
