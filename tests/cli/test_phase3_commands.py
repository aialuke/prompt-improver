...
class TestCLIFileSystemIntegration:
    ...
    def test_train_command_with_config_file(self, cli_runner, test_data_dir):
        """Test train command with external configuration file."""
        ...
        config_file = test_data_dir / "config" / "train_config.json"
    ...
    def test_train_command_output_to_file(self, cli_runner, test_data_dir):
        """Test train command with output redirection to file."""
        
        output_file = test_data_dir / "logs" / "training_results.json"
        ...
        with patch("prompt_improver.cli.asyncio.run") as mock_run:
            ...
            mock_run.side_effect = lambda coro: mock_results  # Return results and consume coroutine
            
            # Use shell redirection in CLI test (simulated)
            result = cli_runner.invoke(app, [
                "train",
                "--export-results", str(output_file),
                "--verbose"
            ])
            
            assert result.exit_code == 0
            
            # In a real implementation, verify file was created
    ...
    def test_discover_patterns_with_input_file(self, cli_runner, test_data_dir):
        """Test discover patterns command with input data file."""
        
        # Create sample performance data file
        performance_data = [
            {
                "rule_id": "clarity_rule",
                "effectiveness": 0.8,
                "support": 15,
                "parameters": {"weight": 0.9, "threshold": 0.7}
            },
            {
        ...
        input_file = test_data_dir / "data" / "performance_data.json"
    ...
    @given(
        ...
        config_complexity=st.sampled_from(["simple", "complex", "nested"])
    )
    def test_config_file_property_validation(self, cli_runner, test_data_dir, rule_count, config_complexity):
        """Property-based testing of configuration file handling."""
        
        # Generate configuration based on complexity
        if config_complexity == "simple":
            config = {
                "training": {"rules": [f"rule_{i}" for i in range(rule_count)]}
            }
        elif config_complexity == "complex":
            config = {
                "training": {
                    "rules": [f"rule_{i}" for i in range(rule_count)],
                    "ensemble": True,
                    "max_trials": 100,
                    "timeout": 300
                },
                "output": {"verbose": True, "format": "json"},
                "validation": {"cross_folds": 5, "test_split": 0.2}
            }
        else:  # nested
            config = {
                "training": {
                    ...
                    "parameters": {
                        "optimization": {
                            "algorithm": "bayesian",
                            "hyperparams": {"n_trials": 50, "timeout": 300}
                        },
                        "validation": {
        ...
        config_file = test_data_dir / "config" / f"test_config_{config_complexity}.json"
        config_file.parent.mkdir(exist_ok=True)
        ...
        # Test CLI command with generated config
        # Mock asyncio.run to handle nested event loop issue
        def mock_asyncio_run(coro):
            loop = asyncio.get_running_loop()
            task = loop.create_task(coro)
        ...
        with patch("prompt_improver.cli.asyncio.run", side_effect=mock_asyncio_run) as mock_run:
            ...
            # Should handle all valid config complexities
            assert result.exit_code == 0
            mock_run.assert_called_once()
...
class TestCLIPerformanceCharacteristics:
    ...
    def test_large_config_file_processing(self, cli_runner, test_data_dir):
        ...
        assert file_size_mb > 1, "Config file should be substantial for performance testing"
    ...
    def test_concurrent_cli_command_execution(self, cli_runner):
        ...
        async def run_cli_command(command_args):
    ...
    def test_pattern_discovery_to_optimization_workflow(self, cli_runner, test_data_dir):
        ...
        with patch("prompt_improver.cli.asyncio.run") as mock_run:
            ...
            discovery_result = cli_runner.invoke(app, [
                ...
                "--output-file", str(patterns_output_file)
            ])
        ...
        with patch("prompt_improver.cli.asyncio.run") as mock_run:
            ...
            optimization_result = cli_runner.invoke(app, [
                "optimize-rules",
                "--patterns-file", str(patterns_output_file),
                "--apply-optimizations"
            ])
