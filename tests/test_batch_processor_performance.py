...
class DatasetGenerator:
    ...
    def generate_ml_record() -> Dict[str, Any]:
        ...
        return {
            "id": f"record_{random.randint(100000, 999999)}",
            "prompt": " ".join([f"word{i}" for i in range(random.randint(10, 100))]),
            "features": {
                "length": random.randint(10, 1000),
                "complexity": random.random(),
                "embeddings": np.random.randn(768).tolist(),  # BERT-like embeddings
                "metadata": {
...
class PerformanceBenchmark:
    ...
    def measure_performance(self, name: str, metrics: ProcessingMetrics, dataset_info: Dict[str, Any]):
        ...
        self.results[name] = {
            "dataset": {
                ...
                "size_mb": dataset_info.get("file_size_mb", 0)
            },
            "performance": {
                ...
                "memory_efficiency": metrics.memory_peak_mb / dataset_info.get("file_size_mb", 1)
            },
            "reliability": {
                "items_failed": metrics.items_failed,
                "retry_count": metrics.retry_count,
                "checkpoint_count": metrics.checkpoint_count,
                "error_rate": (metrics.items_failed / max(1, metrics.items_processed + metrics.items_failed)) * 100
            }
        }
    ...
    def calculate_improvement(self, baseline_name: str, enhanced_name: str) -> Dict[str, float]:
        ...
        return {
            "throughput_improvement": enhanced["throughput_items_per_sec"] / max(1, baseline["throughput_items_per_sec"]),
            "time_improvement": baseline["processing_time_sec"] / max(0.001, enhanced["processing_time_sec"]),
            "memory_improvement": baseline["memory_peak_mb"] / max(1, enhanced["memory_peak_mb"])
        }
...
# ML Processing function for testing
def process_ml_batch(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        # Simulate embedding generation (lightweight)
        embedding_dim = 128
        ...
        result = {
            "id": item.get("id"),
            "original_prompt": item.get("prompt", "")[:50] + "...",
            "processed": True,
            "ml_score": score,
            "features_extracted": embedding_dim,
...
class TestBatchProcessorPerformance:
    ...
    async def test_small_dataset_baseline(self, tmp_path):
        ...
        # Test with baseline batch processor
        ...
        # Load all data (baseline approach)
        records = []
    ...
    async def test_small_dataset_enhanced(self, tmp_path):
        ...
        assert metrics.memory_peak_mb < 100  # Should be memory efficient
    ...
    async def test_medium_dataset_performance(self, tmp_path):
        ...
        assert metrics.items_processed == num_records
        assert metrics.throughput_items_per_sec > 1000  # Should process >1000 items/sec
    ...
    async def test_large_dataset_performance(self, tmp_path):
        ...
        # Test with enhanced processor optimized for large datasets
        config = StreamingBatchConfig(
            chunk_size=5000,
            worker_processes=8,
        ...
        assert metrics.items_processed == num_records
        assert metrics.throughput_items_per_sec > 5000  # Should process >5000 items/sec
        assert metrics.memory_peak_mb < 2000  # Should stay within memory limit
        assert metrics.checkpoint_count > 0  # Should have created checkpoints
    ...
    async def test_streaming_performance(self):
        ...
        async with StreamingBatchProcessor(config, process_ml_batch) as processor:
            metrics = await processor.process_dataset(
                data_source=DatasetGenerator.async_data_generator(num_records),
    ...
    async def test_error_handling_and_recovery(self, tmp_path):
        ...
        config = StreamingBatchConfig(
            ...
            worker_processes=2,
            max_retries=3,
            error_threshold_percent=15.0  # Allow up to 15% errors
        )
        ...
        assert metrics.retry_count > 0  # Should have retried failed chunks
    ...
    async def test_checkpoint_resume(self, tmp_path):
        ...
        async with StreamingBatchProcessor(config, process_ml_batch) as processor:
            # Simulate interruption by processing only some chunks
            processor.config.error_threshold_percent = 0.1  # Force early stop
            
            try:
                metrics1 = await processor.process_dataset(
                    data_source=str(dataset_file),
                    job_id=job_id
                )
            except:
        ...
        async with StreamingBatchProcessor(config, process_ml_batch) as processor:
            processor.config.error_threshold_percent = 100.0  # Allow completion
            
            metrics2 = await processor.process_dataset(
                data_source=str(dataset_file),
                job_id=job_id,
                resume_from_checkpoint=True
            )
            
        # Should have completed all records
    ...
    async def test_memory_efficiency(self, tmp_path):
        ...
        for strategy in strategies:
            ...
            async with StreamingBatchProcessor(config, process_ml_batch) as processor:
                metrics = await processor.process_dataset(
                    data_source=str(dataset_file),
                    job_id=f"test_memory_{strategy.value}"
                )
        ...
        # Verify memory-based strategies are more efficient
        assert results["memory_based"]["memory_peak_mb"] < results["fixed_size"]["memory_peak_mb"]
        assert results["adaptive"]["gc_collections"] > 0  # Should trigger GC
    ...
    async def test_parallel_processing_scaling(self, tmp_path):
        ...
        for workers in worker_counts:
            ...
            async with StreamingBatchProcessor(config, process_ml_batch) as processor:
                metrics = await processor.process_dataset(
                    data_source=str(dataset_file),
                    job_id=f"test_scaling_{workers}"
                )
                
            results[workers] = metrics.throughput_items_per_sec
            
        # Verify scaling improves performance
        assert results[2] > results[1] * 1.5  # Should see improvement
        assert results[4] > results[2] * 1.5  # Continued improvement
        # Note: 8 workers might not show 2x improvement due to overhead
    ...
    async def test_comprehensive_performance_report(self, tmp_path):
        ...
        # Test configurations
        test_configs = [
            {"records": 1000, "name": "Small (1MB)", "file": "small.jsonl"},
            {"records": 10000, "name": "Medium (10MB)", "file": "medium.jsonl"},
            {"records": 100000, "name": "Large (100MB)", "file": "large.jsonl"},
        ]
        ...
        for test_config in test_configs:
            ...
            if test_config["records"] <= 10000:
                ...
                baseline_metrics = ProcessingMetrics(
                    items_processed=results["processed"],
                    processing_time_ms=baseline_time * 1000,
                    throughput_items_per_sec=results["processed"] / baseline_time,
                    memory_peak_mb=100  # Estimate
                )
                
                benchmark.measure_performance(
                    f"Baseline {test_config['name']}", 
                    baseline_metrics, 
                    dataset_info
                )
            ...
            enhanced_config = StreamingBatchConfig(
                chunk_size=min(5000, test_config["records"] // 10),
                worker_processes=4,
                memory_limit_mb=500,
                chunking_strategy=ChunkingStrategy.ADAPTIVE,
                compression=test_config["records"] > 10000
            )
            ...
            async with StreamingBatchProcessor(enhanced_config, process_ml_batch) as processor:
                enhanced_metrics = await processor.process_dataset(
                    data_source=str(dataset_file),
                    job_id=f"perf_test_{test_config['name']}"
                )
                
            benchmark.measure_performance(
                f"Enhanced {test_config['name']}", 
                enhanced_metrics, 
                dataset_info
            )
        ...
        # Validate 10x improvement for large datasets
