...
class HealthMonitoringTester:
    ...
    async def _test_core_health_system(self):
        ...
        try:
            ...
            is_valid = (
                ...
                and "test_plugin" in health_results
                and (health_results["test_plugin"].status == HealthStatus.HEALTHY)
            )
            self.results.append(
                TestResult(
                    ...
                    details={
                        ...
                        "test_plugin_status": health_results.get(
                            "test_plugin", {}
                        ).status.value
                        if health_results.get("test_plugin")
                        else "missing",
                    },
                )
            )
        ...
        try:
            ...
            is_valid = (
                isinstance(overall_health, HealthCheckResult)
                and hasattr(overall_health, "status")
                and hasattr(overall_health, "message")
                and hasattr(overall_health, "details")
            )
            self.results.append(
                TestResult(
                    ...
                    details={
                        ...
                        "has_details": bool(
                            overall_health.details if overall_health else False
                        ),
                    },
                )
            )
    ...
    async def _test_legacy_compatibility(self):
        ...
        try:
            ...
            is_valid = (
                isinstance(aggregated_result, AggregatedHealthResult)
                and hasattr(aggregated_result, "overall_status")
                and hasattr(aggregated_result, "checks")
                and hasattr(aggregated_result, "timestamp")
                and ("legacy_test_checker" in aggregated_result.checks)
            )
            self.results.append(
                TestResult(
                    ...
                    details={
                        ...
                        "checks_count": len(aggregated_result.checks)
                        if aggregated_result
                        else 0,
                    },
                )
        ...
        try:
            ...
            is_valid = (
                isinstance(specific_result, HealthResult)
                and specific_result.component == "legacy_test_checker"
                and (specific_result.status == BaseHealthStatus.HEALTHY)
            )
            self.results.append(
                TestResult(
                    ...
                    duration_ms=(time.time() - test_start) * 1000,
                    details={
                        "component": specific_result.component
                        if specific_result
                        else "none",
                        "status": specific_result.status.value
                        if specific_result
                        else "none",
                    },
                )
        ...
        try:
            ...
            is_valid = (
                ...
                and "overall_status" in summary
                and ("checks" in summary)
                and ("timestamp" in summary)
                and ("legacy_test_checker" in summary["checks"])
            )
            self.results.append(
                TestResult(
    ...
    async def _test_plugin_system(self):
        ...
        try:
            ...
            self.results.append(
                TestResult(
                    ...
                    duration_ms=(time.time() - test_start) * 1000,
                    details={
                        "plugin_name": simple_plugin.name,
                        "category": simple_plugin.category.value,
                    },
                )
            )
        ...
        try:
            ...
            if success:
                logger.info(
                    "✅ Plugin registration via convenience function successful"
                )
            else:
        ...
        try:
            ...
            is_valid = (
                "simple_test_plugin" in results
                and results["simple_test_plugin"].status == HealthStatus.HEALTHY
            )
            self.results.append(
                TestResult(
                    ...
                    details={
                        ...
                        "status": results.get("simple_test_plugin", {}).status.value
                        if results.get("simple_test_plugin")
                        else "none",
                    },
                )
            )
    ...
    async def _test_advanced_features(self):
        ...
        try:
            ...
            success = service.enable_circuit_breaker(
                component_name="legacy_test_checker", failure_threshold=3, timeout=30
            )
            available_checks = service.get_available_checks()
    ...
    async def _test_error_handling(self):
        ...
        try:
            ...
            timeout_plugin = TimeoutTestPlugin(
                ...
                category=HealthCheckCategory.CUSTOM,
                config=HealthCheckPluginConfig(timeout_seconds=0.05),
            )
            monitor = get_unified_health_monitor()
            ...
            is_valid = (
                plugin_result is not None
                and plugin_result.status == HealthStatus.UNHEALTHY
                and (
                    "timeout" in plugin_result.message.lower()
                    or "failed" in plugin_result.message.lower()
                )
            )
            self.results.append(
                TestResult(
                    ...
                    details={
                        "timed_out": is_valid,
                        "status": results.get("timeout_test_plugin", {}).status.value
                        if results.get("timeout_test_plugin")
                        else "none",
                    },
                )
            )
        ...
        try:
            ...
            is_valid = (
                "exception_test_plugin" in results
                and results["exception_test_plugin"].status == HealthStatus.UNHEALTHY
                and ("failed" in results["exception_test_plugin"].message.lower())
            )
            self.results.append(
                TestResult(
                    ...
                    details={
                        ...
                        "status": results.get("exception_test_plugin", {}).status.value
                        if results.get("exception_test_plugin")
                        else "none",
                    },
                )
            )
    ...
    async def _test_performance(self):
        ...
        try:
            ...
            if is_acceptable:
                logger.info(
                    f"✅ Parallel execution performance successful ({parallel_duration:.2f}ms)",
                    format(parallel_duration, ".2f"),
                )
            else:
                logger.error(
                    f"❌ Parallel execution performance poor ({parallel_duration:.2f}ms)",
                    format(parallel_duration, ".2f"),
                )
        except Exception as e:
            self.results.append(
    ...
    def _generate_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "failures": [
                {"name": r.name, "error": r.error, "duration_ms": r.duration_ms}
