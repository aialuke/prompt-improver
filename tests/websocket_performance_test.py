...
class WebSocketPerformanceTester:
    ...
    async def test_message_throughput(self, num_messages: int = 1000, concurrent_connections: int = 1):
                    ...
                    for i in range(num_messages // concurrent_connections):
                        try:
                            ...
                            message = {
                                ...
                                "sequence": i,
                                "timestamp": send_time,
                                "data": f"Performance test message {i} from connection {connection_id}"
                            }
                            ...
                            errors.append(f"Error on message {i}: {str(e)}")
        ...
        for r in results:
            if isinstance(r, dict):
                ...
                all_errors.append(f"Task exception: {str(r)}")
        
        metrics = PerformanceMetrics(
            test_name=f"throughput_{concurrent_connections}conn_{num_messages}msg",
            duration=duration,
            messages_sent=total_sent,
            messages_received=total_received,
            messages_per_second=total_received / duration if duration > 0 else 0,
            ...
            async with connect(f"{self.base_url}") as websocket:
                ...
                while time.time() < end_time:
                    try:
                        ...
                        errors.append(f"Error at {time.time() - start_time:.1f}s: {str(e)}")
    ...
    async def test_burst_performance(self, burst_size: int = 100, num_bursts: int = 10):
        ...
        try:
                ...
                for burst_num in range(num_bursts):
                    ...
                    for i in range(burst_size):
                        try:
                            ...
                            errors.append(f"Error in burst {burst_num}, message {i}: {str(e)}")
            ...
            errors.append(f"Burst test error: {str(e)}")
        
        duration = time.time() - start_time
        
        metrics = PerformanceMetrics(
            test_name="burst_performance",
            duration=duration,
            messages_sent=messages_sent,
            messages_received=messages_received,
            messages_per_second=messages_received / duration if duration > 0 else 0,
            avg_latency=statistics.mean(burst_times) * 1000 if burst_times else 0,  # Convert to ms
            min_latency=min(burst_times) * 1000 if burst_times else 0,
            max_latency=max(burst_times) * 1000 if burst_times else 0,
            success_rate=(messages_received / messages_sent * 100) if messages_sent > 0 else 0,
            errors=errors
        )
                ...
                async with connect(f"{self.base_url}") as websocket:
                    ...
                    for i in range(10):
                        try:
                            ...
                            errors.append(f"Error on large message {i} (size: {size}): {str(e)}")
                            
            except Exception as e:
                errors.append(f"Large message test error (size: {size}): {str(e)}")
            
            duration = time.time() - start_time
            
            metrics = PerformanceMetrics(
                test_name=f"large_message_{size}_bytes",
                duration=duration,
                messages_sent=messages_sent,
                messages_received=messages_received,
                messages_per_second=messages_received / duration if duration > 0 else 0,
            ...
            "results": [
                {
                    ...
                    "errors": m.errors[:10]  # Save first 10 errors
