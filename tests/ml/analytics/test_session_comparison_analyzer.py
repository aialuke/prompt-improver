...
from prompt_improver.ml.analytics.session_comparison_analyzer import (
...
class TestSessionComparisonAnalyzer:
    ...
    def sample_session_a(self):
        ...
        return MagicMock(
            ...
            total_training_time_seconds=7200,  # 2 hours
    ...
    def sample_session_b(self):
        ...
        return MagicMock(
            ...
            total_training_time_seconds=10800,  # 3 hours
            continuous_mode=True,
            improvement_threshold=0.01,
            max_iterations=50,
            timeout_seconds=86400
    ...
    def sample_iterations_a(self):
        ...
        for i in range(15):
            ...
            iteration.duration_seconds = 400 + (i * 5)  # Consistent duration
            iteration.status = "completed"  # All successful
            iteration.improvement_score = 0.04 + (i * 0.005)  # Steady improvement
            iteration.error_message = None
            iteration.performance_metrics = {
                "model_accuracy": 0.6 + (i * 0.017),  # Steady improvement
                "rule_effectiveness": 0.55 + (i * 0.02),
                "memory_usage_mb": 800 + (i * 20),  # Efficient memory usage
                "quality_score": 0.85 + (i * 0.005)
            }
    ...
    def sample_iterations_b(self):
        ...
        for i in range(20):
            ...
            iteration.duration_seconds = 600 + (i * 15)  # Increasing duration
            iteration.status = "completed" if i < 16 else "failed"  # Some failures
            iteration.improvement_score = 0.02 + (i * 0.003) if i < 16 else 0.0
            iteration.error_message = "Test error" if i >= 16 else None
            iteration.performance_metrics = {
                "model_accuracy": 0.65 + (i * 0.008),  # Slower improvement
                "rule_effectiveness": 0.6 + (i * 0.01),
                "memory_usage_mb": 1200 + (i * 40),  # Higher memory usage
                "quality_score": 0.75 + (i * 0.003)
            } if i < 16 else {}
            iterations.append(iteration)
    ...
    async def test_compare_sessions_performance(self, analyzer, sample_session_a, sample_session_b, sample_iterations_a, sample_iterations_b):
        ...
        analyzer._get_session_with_iterations.side_effect = [
            (sample_session_a, sample_iterations_a),
            (sample_session_b, sample_iterations_b)
        ]
        ...
        assert result.session_a_score > result.session_b_score  # A should perform better
    ...
    async def test_compare_sessions_efficiency(self, analyzer, sample_session_a, sample_session_b, sample_iterations_a, sample_iterations_b):
        ...
        result = await analyzer.compare_sessions(
            "session_a",
            "session_b",
            ComparisonDimension.EFFICIENCY,
            ComparisonMethod.T_TEST
        )

        assert isinstance(result, SessionComparisonResult)
        assert result.comparison_dimension == ComparisonDimension.EFFICIENCY
        # Session A should be more efficient (better performance in less time)
    ...
    async def test_compare_sessions_stability(self, analyzer, sample_session_a, sample_session_b, sample_iterations_a, sample_iterations_b):
        ...
        result = await analyzer.compare_sessions(
            "session_a",
            "session_b",
            ComparisonDimension.STABILITY,
            ComparisonMethod.T_TEST
        )

        assert isinstance(result, SessionComparisonResult)
        assert result.comparison_dimension == ComparisonDimension.STABILITY
        # Both sessions should have stability scores calculated
        assert result.session_a_score >= 0
        assert result.session_b_score >= 0
        # The comparison should complete successfully regardless of which is more stable
    ...
    async def test_statistical_comparison_methods(self, analyzer):
        ...
        # T-test
        result_ttest = await analyzer._perform_statistical_comparison(
        ...
        # Mann-Whitney test
        result_mw = await analyzer._perform_statistical_comparison(
            metrics_a, metrics_b, ComparisonMethod.MANN_WHITNEY
        )
    ...
    async def test_analyze_multiple_sessions(self, analyzer):
        ...
        for i in range(5):
            ...
            session.current_performance = 0.7 + (i * 0.05)  # Varying performance
            ...
            # Create iterations for each session
            iterations = []
            for j in range(10):
                iteration = MagicMock()
                ...
                iteration.performance_metrics = {
                    "model_accuracy": 0.6 + (j * 0.01),
                    "memory_usage_mb": 1000 + (j * 50),
                    "quality_score": 0.8
                }
                iterations.append(iteration)
        ...
        result = await analyzer.analyze_multiple_sessions(
    ...
    async def test_benchmark_session(self, analyzer, sample_session_a, sample_iterations_a):
        ...
        # Create historical sessions for benchmarking
        historical_sessions = []
        for i in range(10):
            session = MagicMock()
            ...
            session.current_performance = 0.6 + (i * 0.02)  # Varying performance
    ...
    async def test_extract_session_features(self, analyzer, sample_session_a, sample_iterations_a):
        ...
        assert features["success_rate"] == 1.0  # All iterations successful
        assert features["total_improvement"] == 0.25  # 0.85 - 0.6
    ...
    async def test_extract_comparison_metrics(self, analyzer, sample_session_a, sample_iterations_a):
        ...
        perf_metrics = await analyzer._extract_comparison_metrics(
            sample_session_a, sample_iterations_a, ComparisonDimension.PERFORMANCE
        )
        assert len(perf_metrics) > 0
        assert all(isinstance(m, (int, float)) for m in perf_metrics)

        # Test efficiency metrics
        eff_metrics = await analyzer._extract_comparison_metrics(
            sample_session_a, sample_iterations_a, ComparisonDimension.EFFICIENCY
        )
        assert len(eff_metrics) > 0

        # Test stability metrics
        stab_metrics = await analyzer._extract_comparison_metrics(
            sample_session_a, sample_iterations_a, ComparisonDimension.STABILITY
        )
        assert len(stab_metrics) >= 0  # May be empty if insufficient data

        # Test speed metrics
        speed_metrics = await analyzer._extract_comparison_metrics(
            sample_session_a, sample_iterations_a, ComparisonDimension.SPEED
        )
    ...
    async def test_pattern_identification(self, analyzer):
        ...
        # Create diverse session features
        session_features = {
            "high_perf_1": {"final_performance": 0.9, "success_rate": 0.95, "avg_efficiency": 0.8},
            "high_perf_2": {"final_performance": 0.88, "success_rate": 0.92, "avg_efficiency": 0.75},
            "low_perf_1": {"final_performance": 0.4, "success_rate": 0.6, "avg_efficiency": 0.3},
            "low_perf_2": {"final_performance": 0.35, "success_rate": 0.55, "avg_efficiency": 0.25},
            "average_1": {"final_performance": 0.7, "success_rate": 0.8, "avg_efficiency": 0.5},
        }
        ...
        # Should identify high and low performers
    ...
    async def test_clustering_analysis(self, analyzer):
        ...
        # Create session features for clustering
        session_features = {}
        for i in range(6):
            session_features[f"session_{i}"] = {
                "final_performance": 0.5 + (i * 0.1),
                "total_improvement": 0.1 + (i * 0.02),
                "improvement_rate": 0.05 + (i * 0.01),
                "success_rate": 0.7 + (i * 0.05),
                "avg_efficiency": 0.3 + (i * 0.1)
            }
        ...
        # All sessions should be assigned to clusters
    ...
    async def test_correlation_matrix_calculation(self, analyzer):
        ...
        for i in range(10):
            session_features[f"session_{i}"] = {
                "final_performance": 0.5 + (i * 0.05),
                "total_improvement": 0.1 + (i * 0.01),
                "improvement_rate": 0.05 + (i * 0.005),
                "success_rate": 0.7 + (i * 0.03),
                "avg_efficiency": 0.3 + (i * 0.07),
                "performance_stability": 0.8 + (i * 0.02)
            }
        ...
        # Should have correlations for all features
    ...
    async def test_performance_tier_classification(self, analyzer):
        ...
        # Create target and historical features
        target_features = {"final_performance": 0.85, "total_improvement": 0.2, "avg_efficiency": 0.7, "success_rate": 0.9}

        historical_features = {}
        for i in range(20):
            historical_features[f"hist_{i}"] = {
                "final_performance": 0.5 + (i * 0.02),  # Range from 0.5 to 0.88
                "total_improvement": 0.1 + (i * 0.01),
                "avg_efficiency": 0.3 + (i * 0.03),
        ...
        # With high performance, should be in upper tiers
    ...
    def test_significance_thresholds(self, analyzer):
        ...
        # Verify threshold values follow Cohen's conventions
