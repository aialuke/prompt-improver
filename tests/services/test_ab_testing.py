...
class TestABTestingFramework:
    ...
    async def test_create_experiment_success(self, ab_testing_service, mock_db_session):
        ...
        with patch.object(ab_testing_service, 'create_experiment', return_value=mock_result) as mock_method:
    ...
    @pytest.mark.asyncio
    async def test_analyze_experiment_statistical_significance(self, ab_testing_service, mock_db_session):
        ...
        with patch.object(ab_testing_service, 'analyze_experiment', return_value=mock_analysis_result):
            result = await ab_testing_service.analyze_experiment("experiment_id", mock_db_session)

        assert result["analysis"]["p_value"] < 0.05  # Check for statistical significance
        assert result["analysis"]["bayesian_probability"] > 0.9  # Check Bayesian probability threshold

    @given(control_mean=st.floats(min_value=0.0, max_value=1.0),
    ...
    def test_statistical_properties(self, control_mean, treatment_mean, sample_size):
        ...
        assume(abs(control_mean - treatment_mean) > 0.01)  # Avoid cases where means are too close
    ...
    async def test_list_active_experiments(self, ab_testing_service, mock_db_session):
        ...
        with patch.object(ab_testing_service, 'list_experiments', return_value=mock_experiments):
            active_experiments = await ab_testing_service.list_experiments(status="running")
...
class TestABEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_create_experiment_conflict(self, ab_testing_service, mock_db_session):
        """Test creation conflict when same experiment already exists."""
        experiment_name = "duplicate_exp"

        with patch.object(ab_testing_service, 'create_experiment', side_effect=ValueError("Experiment already exists")):
    ...
    @pytest.mark.asyncio
    async def test_analyze_experiment_no_results(self, ab_testing_service, mock_db_session):
