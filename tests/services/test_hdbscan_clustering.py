...
class TestHDBSCANClusteringEnhancements:
    ...
    @pytest.mark.asyncio
    async def test_hdbscan_cluster_discovery_performance(self, discovery_service, mock_db_session):
        ...
        with patch.object(discovery_service, '_get_performance_data', return_value=mock_performance_data):
    ...
    async def test_hdbscan_cluster_properties(self, effectiveness_scores):
        ...
        for i, score in enumerate(effectiveness_scores):
            mock_data.append({
                "rule_id": f"rule_{i}",
                "parameters": {"weight": np.random.uniform(0.5, 1.0), "threshold": np.random.uniform(0.5, 0.9)},
                "effectiveness": score,
        ...
        with patch.object(discovery_service, '_get_performance_data', return_value=mock_data):
            result = await discovery_service.discover_advanced_patterns(
                mock_db_session,
    ...
    @pytest.mark.asyncio
    async def test_hdbscan_minimum_cluster_size_parameter(self, discovery_service, mock_db_session):
        ...
        for i in range(10):
            mock_data.append({
                ...
                "parameters": {"weight": 0.9 + np.random.normal(0, 0.05), "threshold": 0.8},
        ...
        for i in range(8):
            mock_data.append({
                ...
                "parameters": {"weight": 0.6 + np.random.normal(0, 0.05), "threshold": 0.6},
    ...
    @pytest.mark.asyncio
    async def test_hdbscan_vs_dbscan_performance(self, discovery_service, mock_db_session):
        ...
        with patch.object(discovery_service, '_get_performance_data', return_value=large_dataset):
        ...
        assert hdbscan_result["processing_time_ms"] < 2000  # Should complete under 2 seconds
    ...
    async def test_hdbscan_outlier_detection(self, discovery_service, mock_db_session):
        ...
        for i in range(15):
            normal_data.append({
                "rule_id": f"normal_{i}",
                "parameters": {"weight": 0.8 + np.random.normal(0, 0.1), "threshold": 0.75},
        ...
        for i in range(3):
            outlier_data.append({
                ...
                "parameters": {"weight": 0.95 + np.random.normal(0, 0.02), "threshold": 0.9},
        ...
        with patch.object(discovery_service, '_get_performance_data', return_value=all_data):
            result = await discovery_service.discover_advanced_patterns(
                mock_db_session,
                pattern_types=["parameter"]
            )
...
class TestHDBSCANEdgeCases:
    ...
    async def test_insufficient_data_handling(self, mock_db_session):
        ...
        with patch.object(discovery_service, '_get_performance_data', return_value=minimal_data):
            result = await discovery_service.discover_advanced_patterns(
                mock_db_session,
    ...
    async def test_hdbscan_unavailable_fallback(self, mock_db_session):
        ...
        mock_data = [{"rule_id": f"rule_{i}", "parameters": {"weight": 0.8}, "effectiveness": 0.8} for i in range(10)]
