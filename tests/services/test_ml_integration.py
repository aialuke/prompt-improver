...
from prompt_improver.database.models import RuleMetadata, RulePerformance, MLModelPerformance
...
class TestDirectPythonIntegration:
    ...
    @pytest.mark.asyncio
    async def test_optimize_rules_success(self, ml_service, sample_training_data, mock_db_session):
...
class TestEnsembleOptimization:
    ...
    async def test_optimize_ensemble_rules_success(self, ml_service, mock_db_session):
        ...
        ensemble_data = {
            "features": [
                [0.8, 150, 1.0, 5, 0.7, 1.0, 0.5, 1.0, 0.2, 0.8]
            ] * 25,
        ...
        with patch('mlflow.start_run'), \
            ...
            result = await ml_service.optimize_ensemble_rules(ensemble_data, mock_db_session)
    ...
    @pytest.mark.asyncio
    async def test_optimize_ensemble_insufficient_data(self, ml_service, mock_db_session):
        ...
        result = await ml_service.optimize_ensemble_rules(insufficient_data, mock_db_session)
...
class TestPatternDiscovery:
    ...
    @pytest.mark.asyncio
    async def test_discover_patterns_insufficient_data(self, ml_service, mock_db_session):
...
class TestPerformanceRequirements:
    ...
    @pytest.mark.asyncio
    async def test_optimization_timeout_handling(self, ml_service, sample_training_data, mock_db_session):
        """Test that optimization respects timeout constraints."""
        with patch('optuna.create_study') as mock_study_create, \
            ...
            result = await ml_service.optimize_rules(sample_training_data, mock_db_session)
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_database_error_handling(self, ml_service, sample_training_data, mock_db_session):
    ...
    @pytest.mark.asyncio 
    async def test_mlflow_error_handling(self, ml_service, sample_training_data, mock_db_session):
