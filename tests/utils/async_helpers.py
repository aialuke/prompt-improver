...
def run_async_test(coro: Awaitable[T]) -> T:
    ...
    try:
        ...
        # Enhanced error reporting for better test debugging
...
# ===============================
# UNIFIED ASYNC CONTEXT MANAGEMENT  
# ===============================

@asynccontextmanager
async def async_test_context(setup_func: Optional[Callable] = None, 
...
async def measure_async_performance(func: Callable[..., Awaitable[T]], 
                                  *args, **kwargs) -> tuple[T, float]:
...
class UnifiedAsyncValidator:
    ...
    async def validate_async(self, test_name: str, 
                           test_func: Callable[[], Awaitable[bool]], 
                           timeout_ms: float = 30000) -> UnifiedValidationResult:
        ...
        try:
            ...
            result = UnifiedValidationResult(
                test_name=test_name,
                passed=passed,
                message="Validation completed successfully" if passed else "Validation failed",
...
async def test_async_database_connection(database_url: str, 
    ...
    """
    try:
        # Try UnifiedConnectionManager first for consistency
        try:
            ...
            if health_info.get('status') == 'healthy':
                # Additional validation with a test query
                async with manager.get_async_session() as session:
                    result = await session.execute(text("SELECT 1"))
                    return result.scalar() == 1
            else:
                # Health check failed, fall back to direct connection test
                pass
        except Exception:
            # UnifiedConnectionManager not available, use direct connection
            pass
        
        # Fallback to direct asyncpg connection for compatibility
        import asyncpg
        
        # Use asyncio.wait_for for consistent timeout handling  
...
async def test_async_redis_connection(redis_url: str,
                                    timeout_ms: float = 5000) -> bool:  
...
# Pytest markers for unified test categorization
pytest_asyncio_auto_mode = True  # Enable automatic async mode