...
container management, and test data generation following 2025 best practices.
"""
...
class RedisContainerManager:
    """
    Advanced Redis container management for testing.
    
    Provides centralized container lifecycle management with health checking,
    connection validation, and resource cleanup for reliable testing.
    """
    
    def __init__(self, image: str = "redis:7-alpine", port: int = 6379):
        self.image = image
        self.port = port
        self.container: Optional[RedisContainer] = None
        self.client: Optional[coredis.Redis] = None
    
    async def start(self) -> Tuple[str, int]:
        """
        Start Redis container and wait for readiness.
        
        Returns:
            Tuple of (host, port) for connection
        """
        self.container = RedisContainer(image=self.image, port=self.port)
        ...
        logger.info(f"Redis container started at {host}:{exposed_port}")
        return host, exposed_port
    ...
    async def _wait_for_readiness(self, max_retries: int = 30, retry_delay: float = 1.0):
        ...
        for i in range(max_retries):
            try:
                client.ping()
                logger.debug(f"Redis container ready after {i + 1} attempts")
                return
            except Exception as e:
                if i == max_retries - 1:
                    raise RuntimeError(f"Redis container failed to start after {max_retries} attempts: {e}")
                await asyncio.sleep(retry_delay)
    
    async def get_client(self, decode_responses: bool = False) -> coredis.Redis:
        """
        Get async Redis client for the container.
        ...
        for operation, req in requirements.items():
            ...
            for metric, threshold in req.items():
                if metric.startswith('max_'):
                    stat_name = metric[4:]  # Remove 'max_' prefix
                    ...
                    stat_name = metric[4:]  # Remove 'min_' prefix
                    if stat_name in stats:
                        actual = stats[stat_name]
...
class CacheTestDataGenerator:
    ...
    def generate_realistic_user_data(user_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'metadata': {
                'created_at': (datetime.now(UTC) - timedelta(days=user_id % 365)).isoformat(),
                'last_login': (datetime.now(UTC) - timedelta(hours=user_id % 24)).isoformat(),
...
class CacheConfigurationManager:
    """
    Configuration manager for cache testing.
    ...
    @staticmethod
    def create_mock_redis_config(host: str, port: int) -> Any:
        """Create mock Redis configuration object."""
        return type('MockRedisConfig', (), {
            'host': host,
            'port': port,
            'database': 0,
            'password': None,
...
class CacheAssertions:
    ...
    def assert_cache_hit_rate(stats: Dict[str, Any], min_hit_rate: float, cache_level: str = 'overall'):
        ...
        assert actual_hit_rate >= min_hit_rate, (
            f"{cache_level} cache hit rate ({actual_hit_rate:.3f}) "
            f"below minimum threshold ({min_hit_rate:.3f})"
        )
    ...
    def assert_response_time_sla(stats: Dict[str, Any], max_p95_ms: float):
        ...
        p95_time = response_times.get('p95', 0) * 1000  # Convert to ms
        
        assert p95_time <= max_p95_ms, (
            f"P95 response time ({p95_time:.2f}ms) "
            f"exceeds SLA threshold ({max_p95_ms}ms)"
        )
    ...
    def assert_warming_effectiveness(stats: Dict[str, Any], min_warming_hit_rate: float = 0.5):
        ...
        assert warming_hit_rate >= min_warming_hit_rate, (
            f"Cache warming hit rate ({warming_hit_rate:.3f}) "
            f"below minimum threshold ({min_warming_hit_rate:.3f})"
        )
    ...
    def assert_error_rate_within_tolerance(stats: Dict[str, Any], max_error_rate: float = 0.01):
        ...
        assert overall_error_rate <= max_error_rate, (
            f"Error rate ({overall_error_rate:.4f}) "
            f"exceeds tolerance ({max_error_rate:.4f})"
        )
    
    @staticmethod
    def assert_cache_size_within_limits(stats: Dict[str, Any], max_l1_size: Optional[int] = None):
        ...
        # L1 should never exceed its max size
    ...
    def assert_data_consistency(original_data: Any, retrieved_data: Any, key: str = "unknown"):
        ...
        assert retrieved_data == original_data, (
            f"Data consistency check failed for key '{key}': "
            f"original={original_data}, retrieved={retrieved_data}"
        )
    ...
    def assert_slo_compliance(stats: Dict[str, Any], min_compliance_rate: float = 0.95):
        ...
        assert compliance_rate >= min_compliance_rate, (
            f"SLO compliance rate ({compliance_rate:.3f}) "
            f"below minimum threshold ({min_compliance_rate:.3f})"
        )
...
class CacheLoadTester:
    ...
    def __init__(self, cache, profiler: Optional[CachePerformanceProfiler] = None):
        self.cache = cache
        ...
        async def user_simulation(user_id: int):
            ...
            for op_id in range(operations_per_user):
                try:
                    ...
                    logger.warning(f"Load test error for user {user_id}: {e}")
    ...
    async def run_sustained_load_test(
        ...
        while time.perf_counter() < end_time:
            try:
                ...
                error_count += 1
                logger.warning(f"Sustained load test error: {e}")
