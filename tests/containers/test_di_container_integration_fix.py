...
async def test_di_container_integration_fix():
    ...
    try:
        ...
        modern_services = [name for name, details in info.items() if 'deprecated' not in str(details).lower()]
        ...
        from prompt_improver.ml.automl.callbacks import RealTimeAnalyticsCallback, create_standard_callbacks
        ...
        info = container.get_registration_info()
        deprecated_count = len([name for name, details in info.items() if 'deprecated' in str(details).lower() or 'legacy' in str(details).lower()])
