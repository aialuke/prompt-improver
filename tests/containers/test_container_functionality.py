...
async def test_ml_service_functionality():
    ...
    ml_service = await container.get(MLServiceInterface)
    try:
        result = await ml_service.process_prompt_improvement_request(prompt='Test prompt', context={'test': True}, improvement_type='test')
...
async def test_container_health_check():
    ...
    print(f"✅ Container health check works! Status: {health_result['container_status']}")
