...
async def test_factory_registration(DIContainer, ServiceLifetime):
    ...
    try:
        ...
        container.register_factory(FactoryService, create_factory_service, ServiceLifetime.SINGLETON)
...
async def main():
    ...
    if not DIContainer:
        ...
        sys.exit(1)
    tests = [test_basic_container_functionality, test_factory_registration, test_container_health_check, test_container_cleanup]
    passed = 0
    total = len(tests)
    for test in tests:
        try:
