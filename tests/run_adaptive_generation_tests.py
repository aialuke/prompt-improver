    ...
    try:
        ...
        async with get_sessionmanager().get_session() as session:
            gap_result = await gap_analyzer.analyze_performance_gaps(
                session=session, rule_ids=None, baseline_window=10
            )
            print(
                f"✓ Standard gap analysis: {gap_result.total_gaps_detected} gaps detected"
            )
            print(f"  - Critical gaps: {len(gap_result.critical_gaps)}")
            print(
                f"  - Improvement opportunities: {len(gap_result.improvement_opportunities)}"
            )
            ...
            enhanced_result = await gap_analyzer.analyze_gaps_for_targeted_generation(
                session=session, rule_ids=None, focus_areas=["clarity", "specificity"]
            )
            print(
                f"✓ Enhanced gap analysis: {len(enhanced_result['enhanced_gaps'])} enhanced gaps"
            )
            print(
                f"  - Generation config ready: {enhanced_result['metadata']['generation_ready']}"
            )
            print(
                f"  - Recommended strategy: {enhanced_result['generation_config']['recommended_strategy']}"
            )
    except Exception as e:
    ...
    try:
        ...
        async with get_sessionmanager().get_session() as session:
            ...
            strategy_recommendation = await strategy_analyzer.analyze_optimal_strategy(
                ...
                hardness_analysis={
                    "distribution": {"hard_examples_ratio": 0.3, "std": 0.2}
                },
                focus_areas=["clarity", "specificity"],
            )
            print("✓ Strategy determination completed")
            print(
                f"  - Primary strategy: {strategy_recommendation.primary_strategy.value}"
            )
            print(
                f"  - Secondary strategy: {strategy_recommendation.secondary_strategy.value}"
            )
...
async def test_difficulty_distribution():
    ...
    try:
        ...
        async with get_sessionmanager().get_session() as session:
            ...
            difficulty_profile = (
                await difficulty_analyzer.analyze_optimal_difficulty_distribution(
                    performance_gaps=all_gaps,
                    hardness_analysis={
                        "distribution": {"hard_examples_ratio": 0.35, "std": 0.22}
                    },
                    focus_areas=["clarity", "specificity"],
                )
            )
            ...
            print(
                f"  - Distribution weights: {difficulty_profile.distribution_weights}"
            )
            print(
                f"  - Hardness threshold: {difficulty_profile.hardness_threshold:.3f}"
            )
            ...
            for target in focus_targets:
                print(
                    f"  - {target.area_name}: priority={target.priority:.3f}, allocation={target.sample_allocation:.3f}"
                )
    except Exception as e:
...
async def test_targeted_data_generation():
    ...
    try:
        ...
        async with get_sessionmanager().get_session() as session:
            ...
            all_gaps = gap_result.critical_gaps + gap_result.improvement_opportunities
            performance_gaps = {
                f"{gap.gap_type}_{gap.rule_id}": gap.gap_magnitude for gap in all_gaps
            }
            ...
            print(
                f"  - Strategy used: {generated_data['metadata']['targeting_info']['strategy_used']}"
            )
            print(
                f"  - Gap targeting enabled: {generated_data['metadata']['targeting_info']['gap_targeting_enabled']}"
            )
...
async def test_adaptive_training_integration():
    ...
    try:
        ...
        data_generator = ProductionSyntheticDataGenerator(
            target_samples=50, enable_gap_targeting=True
        )
        coordinator = AdaptiveTrainingCoordinator(
            orchestrator=orchestrator, data_generator=data_generator
        )
        session_config = {
            "max_iterations": 2,
        ...
        session_id = await coordinator.start_adaptive_training_session(
            session_config=session_config, focus_areas=["clarity", "specificity"]
        )
        ...
        async with get_sessionmanager().get_session() as session:
            ...
            if training_session:
                print(
                    f"✓ Database record created: continuous_mode={training_session.continuous_mode}"
                )
                print(f"  - Status: {training_session.status}")
                print(
                    f"  - Adaptive training: {training_session.checkpoint_data.get('adaptive_training', False)}"
                )
            else:
