...
async def setup_test_database():
    ...
    try:
        async with get_sessionmanager().get_session() as session:
            for i in range(5):
                improvement_session = ImprovementSession(session_id=f'test_setup_session_{i}', original_prompt=f'Test original prompt {i}', final_prompt=f'Test improved prompt {i}', rules_applied=['clarity', 'specificity'], improvement_metrics={'clarity_score': 0.5 + i * 0.1, 'specificity_score': 0.4 + i * 0.12, 'overall_score': 0.45 + i * 0.11})
                session.add(improvement_session)
            rule_ids = ['clarity_rule', 'specificity_rule', 'effectiveness_rule', 'consistency_rule']
            for i, rule_id in enumerate(rule_ids):
                for j in range(3):
                    rule_perf = RulePerformance(rule_id=rule_id, effectiveness_score=0.3 + i * 0.1 + j * 0.05, consistency_score=0.4 + i * 0.08 + j * 0.04, coverage_score=0.5 + i * 0.06 + j * 0.03, confidence_score=0.6 + i * 0.05 + j * 0.02)
...
async def test_performance_gap_analysis():
    ...
    try:
        ...
        async with get_sessionmanager().get_session() as session:
            gap_result = await gap_analyzer.analyze_performance_gaps(session=session, rule_ids=None, baseline_window=10)
            ...
            enhanced_result = await gap_analyzer.analyze_gaps_for_targeted_generation(session=session, rule_ids=None, focus_areas=['clarity', 'specificity'])
...
async def test_strategy_determination():
    ...
    try:
        ...
        async with get_sessionmanager().get_session() as session:
            ...
            print('✓ Strategy determination completed')
...
async def test_difficulty_distribution():
    ...
    try:
        ...
        async with get_sessionmanager().get_session() as session:
            ...
            difficulty_profile = await difficulty_analyzer.analyze_optimal_difficulty_distribution(performance_gaps=all_gaps, hardness_analysis={'distribution': {'hard_examples_ratio': 0.35, 'std': 0.22}}, focus_areas=['clarity', 'specificity'])
            ...
            focus_targets = await difficulty_analyzer.generate_focus_area_targets(performance_gaps=all_gaps, focus_areas=['clarity', 'specificity', 'effectiveness'], current_performance={'clarity_effectiveness': 0.6, 'specificity_effectiveness': 0.5, 'effectiveness_effectiveness': 0.4}, target_samples=500)
            print(f'✓ Focus area targeting: {len(focus_targets)} targets generated')
            for target in focus_targets:
                print(f'  - {target.area_name}: priority={target.priority:.3f}, allocation={target.sample_allocation:.3f}')
    except Exception as e:
...
async def test_targeted_data_generation():
    ...
    try:
        gap_analyzer = PerformanceGapAnalyzer()
        data_generator = ProductionSyntheticDataGenerator(target_samples=100, enable_gap_targeting=True, difficulty_distribution='adaptive')
        async with get_sessionmanager().get_session() as session:
            gap_result = await gap_analyzer.analyze_performance_gaps(session=session)
            all_gaps = gap_result.critical_gaps + gap_result.improvement_opportunities
            performance_gaps = {f'{gap.gap_type}_{gap.rule_id}': gap.gap_magnitude for gap in all_gaps}
            generated_data = await data_generator.generate_targeted_data(performance_gaps=performance_gaps, strategy='gap_based', batch_size=50, focus_areas=['clarity', 'specificity'])
            print('✓ Targeted data generation completed')
...
async def test_adaptive_training_integration():
    ...
    try:
        ...
        data_generator = ProductionSyntheticDataGenerator(target_samples=50, enable_gap_targeting=True)
        coordinator = AdaptiveTrainingCoordinator(orchestrator=orchestrator, data_generator=data_generator)
        session_config = {'max_iterations': 2, 'improvement_threshold': 0.01, 'timeout_hours': 1}
        ...
        async with get_sessionmanager().get_session() as session:
            ...
            result = await session.execute(select(TrainingSession).where(TrainingSession.session_id == session_id))
