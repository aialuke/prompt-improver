...
from prompt_improver.api.analytics_endpoints import SessionComparisonRequest, TimeRangeRequest, TrendAnalysisRequest, analytics_router
from prompt_improver.database.analytics_query_interface import MetricType, TimeGranularity
...
class TestAnalyticsEndpoints:
    ...
    def mock_dashboard_data(self):
        ...
        return {'current_period': {'session_summary': {'total_sessions': 25, 'completed_sessions': 20, 'running_sessions': 3, 'failed_sessions': 2, 'avg_duration_hours': 2.5}, 'performance': {'avg_performance': 0.82, 'max_performance': 0.95, 'min_performance': 0.65, 'avg_improvement': 0.15, 'performance_std': 0.08}, 'efficiency': {'avg_efficiency': 0.35, 'avg_iterations': 12.5, 'avg_training_hours': 2.3}, 'errors': {'session_error_rate': 0.08, 'iteration_error_rate': 0.05, 'total_failed_sessions': 2, 'total_failed_iterations': 15}, 'resources': {'avg_memory_usage_mb': 1200.0, 'peak_memory_usage_mb': 1800.0, 'avg_cpu_utilization': 0.65, 'total_compute_hours': 50.0}}, 'time_range': {'start': (datetime.now(UTC) - timedelta(hours=24)).isoformat(), 'end': datetime.now(UTC).isoformat(), 'hours': 24}, 'last_updated': datetime.now(UTC).isoformat()}

    @pytest.fixture
    def mock_trend_data(self):
        """Mock trend analysis data"""
        base_time = datetime.now(UTC) - timedelta(days=7)
        return {'time_series': [{'timestamp': (base_time + timedelta(days=i)).isoformat(), 'value': 0.7 + i * 0.02, 'metadata': {'session_count': 5 + i}} for i in range(7)], 'trend_direction': 'increasing', 'trend_strength': 0.85, 'correlation_coefficient': 0.92, 'seasonal_patterns': {}, 'metadata': {'granularity': 'day', 'metric_type': 'performance', 'session_count': None}}

    @pytest.fixture
    def mock_session_comparison(self):
        """Mock session comparison data"""
        return {'session_a_id': 'session_a', 'session_b_id': 'session_b', 'comparison_dimension': 'performance', 'statistical_significance': True, 'p_value': 0.023, 'effect_size': 0.65, 'winner': 'session_a', 'insights': ['Session A shows significantly better performance'], 'recommendations': ['Adopt configuration from session_a for better performance']}
    ...
    def test_analyze_performance_trends(self, client, mock_trend_data):
        ...
        with patch('prompt_improver.api.analytics_endpoints.get_analytics_interface') as mock_get_analytics:
            ...
            mock_result.time_series = [MagicMock(timestamp=datetime.fromisoformat(point['timestamp'].replace('Z', '+00:00')), value=point['value'], metadata=point['metadata']) for point in mock_trend_data['time_series']]
            ...
            request_data = {'time_range': {'hours': 24}, 'granularity': 'day', 'metric_type': 'performance'}
    ...
    def test_compare_sessions(self, client, mock_session_comparison):
        ...
        with patch('prompt_improver.api.analytics_endpoints.get_comparison_analyzer') as mock_get_analyzer:
            ...
            request_data = {'session_a_id': 'session_a', 'session_b_id': 'session_b', 'dimension': 'performance', 'method': 't_test'}
    ...
    def test_get_session_summary(self, client):
        ...
        with patch('prompt_improver.api.analytics_endpoints.get_session_reporter') as mock_get_reporter:
            ...
            mock_summary.efficiency_rating = 0.8
            mock_summary.quality_index = 0.9
            ...
            mock_summary.started_at = datetime.now(UTC) - timedelta(hours=2)
            mock_summary.completed_at = datetime.now(UTC)
    ...
    def test_get_performance_distribution(self, client):
        ...
        with patch('prompt_improver.api.analytics_endpoints.get_analytics_interface') as mock_get_analytics:
            mock_analytics = AsyncMock()
            mock_distribution_data = {'histogram': [{'bucket': 1, 'frequency': 2, 'min_value': 0.5, 'max_value': 0.6, 'probability': 0.1}, {'bucket': 2, 'frequency': 5, 'min_value': 0.6, 'max_value': 0.7, 'probability': 0.25}, {'bucket': 3, 'frequency': 8, 'min_value': 0.7, 'max_value': 0.8, 'probability': 0.4}, {'bucket': 4, 'frequency': 4, 'min_value': 0.8, 'max_value': 0.9, 'probability': 0.2}, {'bucket': 5, 'frequency': 1, 'min_value': 0.9, 'max_value': 1.0, 'probability': 0.05}], 'statistics': {'total_sessions': 20, 'mean': 0.72, 'std_dev': 0.12, 'quartiles': {'q25': 0.65, 'median': 0.72, 'q75': 0.8}, 'percentiles': {'p90': 0.85, 'p95': 0.9}}}
    ...
    def test_get_correlation_analysis(self, client):
        ...
        with patch('prompt_improver.api.analytics_endpoints.get_analytics_interface') as mock_get_analytics:
            mock_analytics = AsyncMock()
            mock_correlation_data = {'correlations': {'performance_vs_improvement': 0.65, 'performance_vs_duration': -0.25, 'performance_vs_success_rate': 0.45}, 'interpretations': {'performance_vs_improvement': {'strength': 'moderate', 'direction': 'positive', 'value': 0.65}}, 'sample_size': 50}
    ...
    def test_request_validation(self, client):
        ...
        invalid_request = {'time_range': {'hours': -1}, 'granularity': 'invalid_granularity'}
        response = client.post('/analytics/trends/analysis', json=invalid_request)
        assert response.status_code == 422
        invalid_comparison = {'session_a_id': '', 'session_b_id': 'session_b'}
    ...
    def test_query_parameters(self, client, mock_dashboard_data):
        ...
        with patch('prompt_improver.api.analytics_endpoints.get_analytics_interface') as mock_get_analytics:
            ...
            mock_analytics.get_dashboard_metrics.return_value = mock_dashboard_data
            mock_get_analytics.return_value = mock_analytics
            response = client.get('/analytics/dashboard/metrics?time_range_hours=168')
            assert response.status_code == 200
            response = client.get('/analytics/dashboard/metrics?include_comparisons=false')
            assert response.status_code == 200
            response = client.get('/analytics/dashboard/metrics?time_range_hours=10000')
            assert response.status_code == 422
