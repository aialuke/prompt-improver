...
from src.prompt_improver.analysis.domain_detector import (
    DomainDetector, PromptDomain, DomainClassificationResult
)
from src.prompt_improver.analysis.domain_feature_extractor import (
    DomainFeatureExtractor, DomainFeatures,
    TechnicalDomainExtractor, CreativeDomainExtractor, AcademicDomainExtractor
)
...
class TestDomainDetector:
    ...
    def detector(self):
        ...
        return DomainDetector(use_spacy=False)  # Use False to avoid spaCy dependency in tests
    ...
    def test_academic_writing_detection(self, detector):
        ...
        assert result.primary_domain in [PromptDomain.RESEARCH, PromptDomain.ACADEMIC_WRITING]
        assert result.confidence > 0.15
        assert 'research' in result.domain_keywords_found or 'academic_writing' in result.domain_keywords_found
    ...
    def test_hybrid_domain_detection(self, detector):
        ...
        assert result.hybrid_domain is True or len([s for s in result.secondary_domains if s[1] > 0.2]) > 0
...
class TestTechnicalDomainExtractor:
    ...
    def test_code_detection(self, extractor):
        ...
        domain_result = DomainClassificationResult(
            primary_domain=PromptDomain.SOFTWARE_DEVELOPMENT,
            confidence=0.8
        )
    ...
    def test_api_reference_detection(self, extractor):
        ...
        domain_result = DomainClassificationResult(
            primary_domain=PromptDomain.API_DOCUMENTATION,
            confidence=0.8
        )
...
class TestCreativeDomainExtractor:
    ...
    def test_narrative_elements(self, extractor):
        ...
        domain_result = DomainClassificationResult(
            primary_domain=PromptDomain.CREATIVE_WRITING,
            confidence=0.8
        )
...
class TestAcademicDomainExtractor:
    ...
    def test_research_methodology(self, extractor):
        ...
        domain_result = DomainClassificationResult(
            primary_domain=PromptDomain.RESEARCH,
            confidence=0.8
        )
    ...
    def test_academic_structure(self, extractor):
        ...
        domain_result = DomainClassificationResult(
            primary_domain=PromptDomain.ACADEMIC_WRITING,
            confidence=0.8
        )
...
class TestDomainFeatureExtractor:
    ...
    def test_software_development_features(self, extractor):
        ...
        assert features.domain in [PromptDomain.SOFTWARE_DEVELOPMENT, PromptDomain.WEB_DEVELOPMENT]
...
class TestDomainFeatureIntegration:
    ...
    def test_multi_domain_prompt(self, extractor):
        ...
        has_domain_features = (len(features.technical_features) > 0 or 
                              len(features.creative_features) > 0 or 
                              len(features.secondary_domains) > 0)
    ...
    def test_performance_with_long_text(self, extractor):
        ...
        assert features.domain == PromptDomain.SOFTWARE_DEVELOPMENT or features.domain == PromptDomain.AI_ML
        assert features.confidence > 0.15
        assert len(features.feature_vector) > 0
