...
ROLE_PERMISSIONS = {Role.GUEST: {Permission.READ_MODELS}, Role.USER: {Permission.READ_MODELS, Permission.READ_DATA}, Role.ML_ANALYST: {Permission.READ_MODELS, Permission.WRITE_MODELS, Permission.READ_DATA, Permission.VIEW_PRIVACY_METRICS}, Role.ML_ENGINEER: {Permission.READ_MODELS, Permission.WRITE_MODELS, Permission.DELETE_MODELS, Permission.DEPLOY_MODELS, Permission.READ_DATA, Permission.WRITE_DATA, Permission.RUN_ADVERSARIAL_TESTS}, Role.PRIVACY_OFFICER: {Permission.READ_MODELS, Permission.ACCESS_DIFFERENTIAL_PRIVACY, Permission.CONFIGURE_PRIVACY_BUDGET, Permission.VIEW_PRIVACY_METRICS, Permission.READ_SENSITIVE_DATA}, Role.SECURITY_ANALYST: {Permission.READ_MODELS, Permission.RUN_ADVERSARIAL_TESTS, Permission.VIEW_SECURITY_LOGS, Permission.CONFIGURE_SECURITY}, Role.ADMIN: {Permission.READ_MODELS, Permission.WRITE_MODELS, Permission.DELETE_MODELS, Permission.DEPLOY_MODELS, Permission.READ_DATA, Permission.WRITE_DATA, Permission.MANAGE_USERS, Permission.MANAGE_ROLES, Permission.VIEW_SECURITY_LOGS}, Role.SUPER_ADMIN: set(Permission)}
...
class PostgreSQLAuthorizationDB:
    ...
    def _ensure_tables(self):
        ...
        with psycopg.connect(self.connection_string) as conn:
            with conn.cursor() as cur:
                ...
                for role in Role:
                    cur.execute('INSERT INTO auth_roles (role_name, description) VALUES (%s, %s) ON CONFLICT DO NOTHING', (role.value, f'Role: {role.value}'))
                conn.commit()
    ...
    def assign_role_to_user(self, user_id: str, role: Role):
        ...
        with psycopg.connect(self.connection_string) as conn:
            with conn.cursor() as cur:
                cur.execute('INSERT INTO auth_users (user_id) VALUES (%s) ON CONFLICT DO NOTHING', (user_id,))
                cur.execute('INSERT INTO auth_user_roles (user_id, role_name) VALUES (%s, %s) ON CONFLICT DO NOTHING', (user_id, role.value))
                cur.execute('INSERT INTO auth_audit_log (user_id, action, details) VALUES (%s, %s, %s)', (user_id, 'assign_role', json.dumps({'role': role.value})))
                conn.commit()
    ...
    def remove_role_from_user(self, user_id: str, role: Role):
        ...
        with psycopg.connect(self.connection_string) as conn:
            with conn.cursor() as cur:
                cur.execute('DELETE FROM auth_user_roles WHERE user_id = %s AND role_name = %s', (user_id, role.value))
                cur.execute('INSERT INTO auth_audit_log (user_id, action, details) VALUES (%s, %s, %s)', (user_id, 'remove_role', json.dumps({'role': role.value})))
                conn.commit()
    ...
    def fetch_user_permissions(self, user_id: str) -> set[Permission]:
        ...
        with psycopg.connect(self.connection_string) as conn:
            with conn.cursor() as cur:
                cur.execute('SELECT role_name FROM auth_user_roles WHERE user_id = %s', (user_id,))
                roles = [Role(row[0]) for row in cur.fetchall()]
                permissions = set()
                for role in roles:
                    permissions.update(ROLE_PERMISSIONS.get(role, set()))
                cur.execute('INSERT INTO auth_audit_log (user_id, action, details) VALUES (%s, %s, %s)', (user_id, 'permission_check', json.dumps({'permissions_count': len(permissions)})))
                conn.commit()
                return permissions
    ...
    def check_resource_access(self, user_id: str, resource_id: str) -> bool:
        ...
        with psycopg.connect(self.connection_string) as conn:
            with conn.cursor() as cur:
                cur.execute('SELECT owner_id FROM auth_resources WHERE resource_id = %s', (resource_id,))
                result = cur.fetchone()
                if result:
                    owner_id = result[0]
                    if owner_id == user_id:
    ...
    def set_resource_owner(self, resource_id: str, owner_id: str):
        ...
        with psycopg.connect(self.connection_string) as conn:
            with conn.cursor() as cur:
                cur.execute('INSERT INTO auth_users (user_id) VALUES (%s) ON CONFLICT DO NOTHING', (owner_id,))
                cur.execute('INSERT INTO auth_resources (resource_id, owner_id) VALUES (%s, %s) ON CONFLICT (resource_id) DO UPDATE SET owner_id = %s', (resource_id, owner_id, owner_id))
                conn.commit()
    ...
    def get_audit_log(self) -> list[dict]:
        ...
        with psycopg.connect(self.connection_string) as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute('SELECT * FROM auth_audit_log ORDER BY timestamp DESC LIMIT 100')
                return cur.fetchall()
...
class TestPermissionValidation:
    ...
    def test_basic_permission_checking(self, auth_service):
        ...
        assert auth_service.has_permission('analyst_001', Permission.READ_MODELS) is True
        assert auth_service.has_permission('analyst_001', Permission.WRITE_MODELS) is True
        assert auth_service.has_permission('analyst_001', Permission.DELETE_MODELS) is False
    ...
    def test_privacy_permission_validation(self, auth_service):
        ...
        assert auth_service.has_permission('privacy_001', Permission.ACCESS_DIFFERENTIAL_PRIVACY) is True
        assert auth_service.has_permission('privacy_001', Permission.CONFIGURE_PRIVACY_BUDGET) is True
        assert auth_service.has_permission('privacy_001', Permission.READ_SENSITIVE_DATA) is True
        assert auth_service.has_permission('user_001', Permission.ACCESS_DIFFERENTIAL_PRIVACY) is False
        assert auth_service.has_permission('analyst_001', Permission.CONFIGURE_PRIVACY_BUDGET) is False

    def test_security_permission_validation(self, auth_service):
        """Test security-specific permission validation"""
        assert auth_service.has_permission('security_001', Permission.RUN_ADVERSARIAL_TESTS) is True
        assert auth_service.has_permission('security_001', Permission.VIEW_SECURITY_LOGS) is True
        assert auth_service.has_permission('security_001', Permission.CONFIGURE_SECURITY) is True
        assert auth_service.has_permission('user_001', Permission.VIEW_SECURITY_LOGS) is False
        assert auth_service.has_permission('analyst_001', Permission.CONFIGURE_SECURITY) is False
    ...
    def test_admin_permission_validation(self, auth_service):
        ...
        assert auth_service.has_permission('engineer_001', Permission.SYSTEM_ADMIN) is False
...
class TestResourceLevelAccess:
    ...
    def test_resource_ownership_access(self, auth_service):
        ...
        assert auth_service.can_access_resource('analyst_001', 'model_123', Permission.READ_MODELS) is True
        assert auth_service.can_access_resource('engineer_001', 'dataset_456', Permission.READ_DATA) is True
        assert auth_service.can_access_resource('user_001', 'model_123', Permission.READ_MODELS) is False

    def test_admin_override_resource_access(self, auth_service):
        """Test admin can override resource ownership"""
        assert auth_service.can_access_resource('super_001', 'model_123', Permission.READ_MODELS) is True
        assert auth_service.can_access_resource('super_001', 'dataset_456', Permission.READ_DATA) is True
    ...
    def test_insufficient_permission_for_resource(self, auth_service):
        ...
        assert auth_service.can_access_resource('analyst_001', 'dataset_789', Permission.WRITE_DATA) is False
...
class TestCrossComponentAuthorization:
    ...
    def test_privacy_preserving_ml_authorization(self, auth_service):
        ...
        privacy_operations = [Permission.ACCESS_DIFFERENTIAL_PRIVACY, Permission.CONFIGURE_PRIVACY_BUDGET, Permission.VIEW_PRIVACY_METRICS]
    ...
    def test_adversarial_testing_authorization(self, auth_service):
        ...
        assert auth_service.has_permission('engineer_001', Permission.RUN_ADVERSARIAL_TESTS) is True
        assert auth_service.has_permission('security_001', Permission.RUN_ADVERSARIAL_TESTS) is True
        assert auth_service.has_permission('user_001', Permission.RUN_ADVERSARIAL_TESTS) is False
        assert auth_service.has_permission('analyst_001', Permission.RUN_ADVERSARIAL_TESTS) is False

    def test_model_deployment_authorization(self, auth_service):
        """Test authorization for model deployment"""
        assert auth_service.has_permission('engineer_001', Permission.DEPLOY_MODELS) is True
        assert auth_service.has_permission('admin_001', Permission.DEPLOY_MODELS) is True
        assert auth_service.has_permission('analyst_001', Permission.DEPLOY_MODELS) is False
        assert auth_service.has_permission('user_001', Permission.DEPLOY_MODELS) is False
...
class TestPrivilegeEscalation:
    ...
    def test_prevent_unauthorized_role_assignment(self, auth_service):
        ...
        can_manage_roles = auth_service.has_permission('user_001', Permission.MANAGE_ROLES)
        assert can_manage_roles is False
        can_admin_manage = auth_service.has_permission('admin_001', Permission.MANAGE_ROLES)
...
class TestAuditLogging:
    ...
    def test_resource_access_is_logged(self, auth_service):
        ...
        initial_log_count = len(auth_service.audit_log)
        auth_service.can_access_resource('analyst_001', 'model_123', Permission.READ_MODELS)
        assert len(auth_service.audit_log) > initial_log_count
        resource_logs = [log for log in auth_service.audit_log if log['action'] == 'resource_access']
...
class TestAsyncAuthorization:
    ...
    async def test_async_permission_checking(self, auth_service):
        ...
        tasks = [async_has_permission('user_001', Permission.READ_MODELS), async_has_permission('analyst_001', Permission.WRITE_MODELS), async_has_permission('admin_001', Permission.MANAGE_USERS)]
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_resource_access_with_nonexistent_resource(self, auth_service):
        ...
        result = auth_service.can_access_resource('analyst_001', 'nonexistent_resource', Permission.READ_MODELS)
