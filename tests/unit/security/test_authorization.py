...
class MockAuthorizationService:
    ...
    def assign_role(self, user_id: str, role: Role) -> None:
        ...
        self._log_action("assign_role", user_id, {"role": role.value})
    
    def remove_role(self, user_id: str, role: Role) -> None:
        """Remove role from user"""
        if user_id in self.user_roles:
            self.user_roles[user_id].discard(role)
            self._log_action("remove_role", user_id, {"role": role.value})
    ...
    def has_permission(self, user_id: str, permission: Permission) -> bool:
        ...
        self._log_action("permission_check", user_id, {
    ...
    def can_access_resource(self, user_id: str, resource_id: str, permission: Permission) -> bool:
        ...
        if resource_id in self.resource_owners:
            ...
            if owner != user_id and not self.has_permission(user_id, Permission.SYSTEM_ADMIN):
                return False
        
        self._log_action("resource_access", user_id, {
    ...
    def _log_action(self, action: str, user_id: str, details: Dict[str, Any]) -> None:
        """Log authorization action for audit"""
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_resource_access_with_nonexistent_resource(self, auth_service):
        ...
        assert result is True  # User has permission, resource doesn't exist but no owner restriction
