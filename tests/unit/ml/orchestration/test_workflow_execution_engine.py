...
class TestWorkflowExecutionEngine:
    ...
    async def execution_engine(self):
        ...
        config = OrchestratorConfig(max_concurrent_workflows=2, event_bus_buffer_size=10)
        mock_event_bus = Mock()
    ...
    async def test_start_workflow(self, execution_engine):
        ...
        definition = WorkflowDefinition(workflow_type=workflow_type, name='Test Training Workflow', description='Test workflow for unit testing', steps=[WorkflowStep(step_id='step1', name='Test Step', component_name='test_component', parameters={})])
    ...
    async def test_workflow_definitions(self, execution_engine):
        ...
        from prompt_improver.ml.orchestration.core.workflow_types import WorkflowDefinition, WorkflowStep
        new_definition = WorkflowDefinition(workflow_type='custom_workflow', name='Custom Workflow', description='Custom test workflow', steps=[WorkflowStep(step_id='custom_step', name='Custom Step', component_name='custom_component', parameters={})])
...
class TestWorkflowInstance:
    ...
    def test_workflow_instance_creation(self):
        ...
        instance = WorkflowInstance(workflow_id=workflow_id, workflow_type=workflow_type, state=PipelineState.IDLE, created_at=created_at, metadata=metadata)
    ...
    def test_workflow_instance_state_transitions(self):
        ...
        created_at = datetime.now(timezone.utc)
        instance = WorkflowInstance(workflow_id='test-workflow-456', workflow_type='tier1_training', state=PipelineState.IDLE, created_at=created_at)
        assert instance.state == PipelineState.IDLE
    ...
    def test_workflow_error_handling(self):
        ...
        created_at = datetime.now(timezone.utc)
        instance = WorkflowInstance(workflow_id='test-workflow-error', workflow_type='tier1_training', state=PipelineState.RUNNING, created_at=created_at)
        assert instance.error_message is None
