...
from prompt_improver.ml.orchestration.monitoring.workflow_metrics_collector import MetricAggregation, MetricType, WorkflowMetric, WorkflowMetricsCollector
...
class TestWorkflowMetricsCollector:
    ...
    async def test_record_performance_metric(self, metrics_collector):
        ...
        await metrics_collector.record_metric(workflow_id, MetricType.PERFORMANCE, 'cpu_usage', 75.5, {'unit': 'percentage'})
        await metrics_collector.record_metric(workflow_id, MetricType.PERFORMANCE, 'memory_usage', 2048, {'unit': 'MB'})
        await metrics_collector.record_metric(workflow_id, MetricType.PERFORMANCE, 'gpu_utilization', 90.0, {'unit': 'percentage'})
    ...
    async def test_record_throughput_metric(self, metrics_collector):
        ...
        throughput_values = [100, 120, 150, 180, 200]
        for i, value in enumerate(throughput_values):
            await metrics_collector.record_metric(workflow_id, MetricType.THROUGHPUT, 'records_per_second', value, {'timestamp_offset': i * 10})
    ...
    async def test_record_error_rate_metric(self, metrics_collector):
        ...
        error_rates = [0.01, 0.02, 0.05, 0.03, 0.01]
    ...
    async def test_record_resource_utilization_metric(self, metrics_collector):
        ...
        resource_metrics = [('cpu_cores', 4.0, {'allocated': 8.0, 'utilization_pct': 50.0}), ('memory_gb', 12.5, {'allocated': 16.0, 'utilization_pct': 78.125}), ('gpu_count', 2.0, {'allocated': 2.0, 'utilization_pct': 100.0}), ('storage_gb', 250.0, {'allocated': 500.0, 'utilization_pct': 50.0})]
        for metric_name, value, metadata in resource_metrics:
            await metrics_collector.record_metric(workflow_id, MetricType.RESOURCE_UTILIZATION, metric_name, value, metadata)
        resource_util_metrics = await metrics_collector.get_metrics_by_type(workflow_id, MetricType.RESOURCE_UTILIZATION)
    ...
    async def test_record_workflow_duration_metric(self, metrics_collector):
        ...
        step_durations = [('data_loading', 45.2), ('model_training', 320.8), ('rule_optimization', 180.5), ('validation', 60.3), ('deployment', 25.1)]
    ...
    async def test_record_component_latency_metric(self, metrics_collector):
        ...
        component_latencies = [('training_data_loader', 25.5), ('ml_integration', 150.2), ('rule_optimizer', 200.8), ('production_registry', 45.1)]
    ...
    async def test_metrics_aggregation(self, metrics_collector):
        ...
        for response_time in response_times:
            await metrics_collector.record_metric(workflow_id, MetricType.PERFORMANCE, 'response_time', response_time, {'unit': 'milliseconds'})
        aggregation = await metrics_collector.get_metrics_aggregation(workflow_id, MetricType.PERFORMANCE, 'response_time')
    ...
    async def test_metrics_aggregation_with_time_window(self, metrics_collector):
        ...
        for i, value in enumerate(recent_values):
            ...
            metric = WorkflowMetric(workflow_id=workflow_id, metric_type=MetricType.PERFORMANCE, metric_name=metric_name, value=value, timestamp=timestamp)
        ...
        for i, value in enumerate(old_values):
            ...
            await metrics_collector._store_metric(metric)
        window_minutes = 60
        aggregation = await metrics_collector.get_metrics_aggregation(workflow_id, MetricType.PERFORMANCE, metric_name, window_minutes=window_minutes)
    ...
    async def test_external_system_integration(self, metrics_collector):
        ...
        metrics_data = [(MetricType.PERFORMANCE, 'cpu_usage', 80.0), (MetricType.THROUGHPUT, 'requests_per_second', 500), (MetricType.ERROR_RATE, 'error_percentage', 0.02)]
    ...
    async def test_metrics_retention_and_cleanup(self, metrics_collector):
        ...
        old_metric = WorkflowMetric(workflow_id=workflow_id, metric_type=MetricType.PERFORMANCE, metric_name='old_metric', value=100.0, timestamp=old_time)
        await metrics_collector._store_metric(old_metric)
        recent_metric = WorkflowMetric(workflow_id=workflow_id, metric_type=MetricType.PERFORMANCE, metric_name='recent_metric', value=200.0, timestamp=recent_time)
    ...
    async def test_high_volume_metrics_performance(self, metrics_collector):
        ...
        for i in range(metrics_to_record):
            await metrics_collector.record_metric(workflow_id, MetricType.PERFORMANCE, f'metric_{i % 10}', float(i), {'batch': i // 10})
...
class TestWorkflowMetric:
    ...
    def test_metric_creation(self):
        ...
        metric = WorkflowMetric(workflow_id='test-workflow', metric_type=MetricType.PERFORMANCE, metric_name='cpu_usage', value=75.5, metadata={'unit': 'percentage', 'threshold': 80.0})
    ...
    def test_metric_serialization(self):
        ...
        metric = WorkflowMetric(workflow_id='serialize-test', metric_type=MetricType.THROUGHPUT, metric_name='requests_per_second', value=450.0, metadata={'target': 500.0})
    ...
    def test_metric_age_calculation(self):
        ...
        metric = WorkflowMetric(workflow_id='age-test', metric_type=MetricType.PERFORMANCE, metric_name='test_metric', value=100.0, timestamp=old_time)
        age = metric.age
        assert age.total_seconds() >= 30 * 60
        assert age.total_seconds() < 31 * 60
...
class TestMetricAggregation:
    ...
    def test_aggregation_creation(self):
        ...
        aggregation = MetricAggregation(metric_name='test_metric', metric_type=MetricType.PERFORMANCE, count=len(values), average=sum(values) / len(values), minimum=min(values), maximum=max(values), percentile_50=50.0, percentile_95=95.0, percentile_99=99.0)
...
if __name__ == '__main__':
    ...
    async def smoke_test():
        ...
        try:
            ...
            aggregation = await collector.get_metrics_aggregation(workflow_id, MetricType.PERFORMANCE, 'cpu_usage')
