...
class TestRuleOptimizerIntegrationFix:
    ...
    @pytest.mark.asyncio
    async def test_optimize_rule_with_insufficient_data(
        self, rule_optimizer
    ):
        ...
        insufficient_performance_data = {
            "test_rule": {
                "total_applications": 5,  # Below min_sample_size
                "avg_improvement": 0.75,
                "consistency_score": 0.8
            }
        }
    ...
    async def test_optimize_rule_parameter_validation(
        ...
        # Test with wrong performance_data type (list instead of dict)
        with pytest.raises(AttributeError):
            await rule_optimizer.optimize_rule(
                rule_id=rule_id,
                performance_data=[],  # Wrong type - should be dict
                historical_data=valid_historical_data
            )
    ...
    async def test_optimize_rule_with_empty_performance_data(
        ...
        # Should handle missing rule gracefully
        assert result["status"] == "insufficient_data"
    ...
    async def test_optimize_rule_with_none_historical_data(
        ...
        # Should work with None historical data
        assert "rule_id" in result
        assert result["rule_id"] == rule_id
    ...
    async def test_optimize_rule_integration_with_advanced_features(
        ...
        # Check if advanced optimization features were attempted
        # (They may not run due to library availability, but should not cause errors)
        if result["status"] == "optimized":
            # Multi-objective optimization might be included
            if "multi_objective_optimization" in result:
                assert isinstance(result["multi_objective_optimization"], dict)
            
            # Gaussian process optimization might be included
    ...
    def test_performance_data_structure_validation(self):
        ...
        # Verify the structure matches what optimize_rule expects
