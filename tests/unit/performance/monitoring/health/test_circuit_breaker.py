...
import pytest
from prompt_improver.performance.monitoring.health.circuit_breaker import CircuitBreaker, CircuitBreakerConfig, CircuitBreakerOpen, CircuitBreakerRegistry, CircuitState
...
class TestCircuitBreaker:
    ...
    async def test_circuit_breaker_closes_after_failures(self):
        ...
        config = CircuitBreakerConfig(failure_threshold=3, recovery_timeout=1, response_time_threshold_ms=100)
    ...
    async def test_circuit_breaker_recovery_behavior(self):
        ...
        config = CircuitBreakerConfig(failure_threshold=2, recovery_timeout=0.5, half_open_max_calls=2)
    ...
    async def test_slow_response_counts_as_failure(self):
        ...
        config = CircuitBreakerConfig(failure_threshold=3, response_time_threshold_ms=50)
    ...
    async def test_circuit_breaker_state_transitions(self):
        ...
        config = CircuitBreakerConfig(failure_threshold=2, recovery_timeout=0.3, half_open_max_calls=1)
