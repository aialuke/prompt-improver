...
class RealRetryManager:
    ...
    def _update_circuit_breaker(self, operation_name: str):
        ...
        if cb['failure_count'] >= 3:
            cb['state'] = 'OPEN'
            cb['open_until'] = time.time() + 5.0  # Open for 5 seconds
...
class TestRealRetryBehavior:
    ...
    async def test_retry_with_eventual_success(self, retry_manager):
        ...
        assert call_count == 3
        assert 0.01 < duration < 0.1  # Should include retry delays
    ...
    async def test_retry_exhaustion(self, retry_manager):
        ...
        assert metrics['failed_operations'] == 3  # Each attempt recorded as failure
    ...
    async def test_circuit_breaker_functionality(self, retry_manager):
        ...
        # First operation should fail after retries
        ...
        assert cb_status['state'] == 'CLOSED'  # Not open yet
        
        # Second and third operations to trip circuit breaker
        for i in range(2):
            with pytest.raises(ConnectionError):
                await retry_manager.retry_async(
                    always_failing, 
    ...
    async def test_circuit_breaker_recovery(self, retry_manager):
        ...
        async def initially_failing_then_success():
            nonlocal call_count
            call_count += 1
            if call_count <= 9:  # Fail enough to trip circuit breaker
                raise ConnectionError(f"Failure {call_count}")
            return "recovered"
        ...
        # Wait for circuit breaker to close (5 seconds in our implementation)
    ...
    async def test_different_operations_isolated(self, retry_manager):
        ...
        # Check that metrics are isolated
        ...
        # Check that circuit breakers are isolated
    ...
    def test_metrics_accuracy(self, retry_manager):
        ...
        # This is a synchronous test to verify metrics calculation
        ...
        assert metrics['total_attempts'] == 3  # 2 + 1
        assert metrics['total_duration'] == 0.8  # 0.5 + 0.3
