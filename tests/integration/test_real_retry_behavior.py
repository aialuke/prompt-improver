...
class RealRetryManager:
    ...
    async def retry_async(self, operation, max_attempts=3, initial_delay=0.1, multiplier=2.0, operation_name='test'):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                self._record_success(operation_name, attempt + 1, time.time() - start_time)
    ...
    def _update_circuit_breaker(self, operation_name: str):
        ...
        if operation_name not in self.circuit_breakers:
            self.circuit_breakers[operation_name] = {'failure_count': 0, 'state': 'CLOSED', 'open_until': 0}
    ...
    def _record_success(self, operation_name: str, attempts: int, duration: float):
        ...
        if operation_name not in self.metrics:
            self.metrics[operation_name] = {'total_attempts': 0, 'successful_operations': 0, 'failed_operations': 0, 'total_duration': 0.0, 'errors': []}
    ...
    def get_circuit_breaker_status(self, operation_name: str) -> dict[str, Any]:
        ...
        return self.circuit_breakers.get(operation_name, {'state': 'CLOSED', 'failure_count': 0})
...
class TestRealRetryBehavior:
    ...
    async def test_successful_operation_no_retry(self, retry_manager):
        ...
        async def successful_operation():
            ...
            return 'success'
        start_time = time.time()
        result = await retry_manager.retry_async(successful_operation, operation_name='success_test')
    ...
    async def test_retry_with_eventual_success(self, retry_manager):
        ...
        start_time = time.time()
        result = await retry_manager.retry_async(failing_then_success, max_attempts=3, initial_delay=0.01, operation_name='retry_test')
        duration = time.time() - start_time
    ...
    async def test_retry_exhaustion(self, retry_manager):
        ...
        with pytest.raises(ConnectionError) as exc_info:
            await retry_manager.retry_async(always_failing, max_attempts=3, initial_delay=0.01, operation_name='failure_test')
        duration = time.time() - start_time
    ...
    async def test_exponential_backoff_timing(self, retry_manager):
        ...
        with pytest.raises(TimeoutError):
            await retry_manager.retry_async(failing_operation, max_attempts=3, initial_delay=0.1, multiplier=2.0, operation_name='timing_test')
        assert len(delays) == 2
        if len(delays) >= 2:
    ...
    async def test_circuit_breaker_functionality(self, retry_manager):
        ...
        with pytest.raises(ConnectionError):
            await retry_manager.retry_async(always_failing, max_attempts=3, initial_delay=0.01, operation_name='circuit_test')
    ...
    async def test_circuit_breaker_recovery(self, retry_manager):
        ...
        for i in range(3):
            with pytest.raises(ConnectionError):
                await retry_manager.retry_async(initially_failing_then_success, max_attempts=3, initial_delay=0.01, operation_name='recovery_test')
        cb_status = retry_manager.get_circuit_breaker_status('recovery_test')
        assert cb_status['state'] == 'OPEN'
        await asyncio.sleep(5.1)
        result = await retry_manager.retry_async(initially_failing_then_success, max_attempts=3, initial_delay=0.01, operation_name='recovery_test')
