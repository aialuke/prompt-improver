...
class TestFeatureFlagIntegration(unittest.TestCase):
    ...
    def test_targeting_rules(self):
        ...
        admin_context = EvaluationContext(
            user_id="admin_user", user_type="admin", environment="test"
        )
        ...
        user_context = EvaluationContext(
            user_id="regular_user", user_type="user", environment="test"
        )
    ...
    def test_sticky_bucketing(self):
        ...
        results = []
        for _ in range(10):
            result = manager.evaluate_flag(
                "phase1_config_externalization", user_context
            )
            results.append(result.value)
        self.assertTrue(
            all(r == results[0] for r in results), "User should get consistent results"
        )
        manager.shutdown()
    ...
    def test_hot_reload_functionality(self):
        ...
        self.assertEqual(
            new_result.reason, "DEFAULT", "Should use default variant when enabled"
        )
        manager.shutdown()
    ...
    async def test_concurrent_evaluations(self):
        ...
        async def evaluate_flags(task_id):
            try:
                for i in range(10):
                    ...
                    result = await manager.evaluate_flag(
                        "phase1_config_externalization", context
                    )
        ...
        for task_id in range(5):
            task = asyncio.create_task(evaluate_flags(task_id))
            tasks.append(task)
        ...
        metrics = await manager.get_metrics("phase1_config_externalization")
    ...
    def test_technical_debt_phase_dependencies(self):
        ...
        self.assertIn(
            "phase1_config_externalization", phase2_flag.metadata["dependencies"]
        )
        manager.shutdown()
...
class TestFeatureFlagInitialization(unittest.TestCase):
    ...
    def setUp(self):
        ...
        configs = {
            ...
            "feature_flags_development.yaml": {
                "version": "1.0.0",
                "environment": "development",
                "flags": {},
            },
            "feature_flags_production.yaml": {
                "version": "1.0.0",
...
class TestFeatureFlagPerformance(unittest.TestCase):
    ...
    def test_evaluation_performance(self):
        ...
        self.assertLess(
            duration, 1.0, f"1000 evaluations took {duration:.3f}s, should be under 1s"
        )
    ...
    async def test_concurrent_performance(self):
        ...
        async def concurrent_evaluations(task_id):
            task_results = []
            context = EvaluationContext(
                user_id=f"user_{task_id}", environment="test"
            )
            for i in range(100):
                flag_key = f"perf_flag_{i % 100}"
                result = await manager.evaluate_flag(flag_key, context)
                task_results.append(result)
            results.extend(task_results)

        tasks = []
        for task_id in range(10):
            task = asyncio.create_task(concurrent_evaluations(task_id))
            tasks.append(task)
        await asyncio.gather(*tasks, return_exceptions=True)
        end_time = time.time()
        duration = end_time - start_time
        ...
        self.assertLess(
            duration, 2.0, f"Concurrent test took {duration:.3f}s, should be under 2s"
        )
        manager.shutdown()
