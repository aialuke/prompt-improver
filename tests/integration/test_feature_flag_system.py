...
import pytest
import yaml
from prompt_improver.core.feature_flag_init import get_config_path, initialize_feature_flag_system
from prompt_improver.core.feature_flags import EvaluationContext, EvaluationResult, FeatureFlagDefinition, FeatureFlagManager, FlagState, RolloutConfig, RolloutStrategy
...
class TestFeatureFlagIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_config = {'version': '1.0.0', 'schema_version': '2025.1', 'global': {'default_rollout_percentage': 10.0, 'sticky_bucketing': True, 'evaluation_timeout_ms': 100, 'metrics_enabled': True}, 'flags': {'phase1_config_externalization': {'state': 'rollout', 'default_variant': 'off', 'variants': {'on': True, 'off': False}, 'rollout': {'strategy': 'percentage', 'percentage': 25.0, 'sticky': True}, 'targeting_rules': [{'name': 'admin_users', 'condition': "user_type == 'admin'", 'variant': 'on', 'priority': 100}], 'metadata': {'phase': 1, 'description': 'Configuration externalization'}}, 'phase2_health_checks': {'state': 'rollout', 'default_variant': 'off', 'variants': {'on': True, 'off': False, 'partial': 'basic_only'}, 'rollout': {'strategy': 'percentage', 'percentage': 15.0, 'sticky': True}, 'metadata': {'phase': 2, 'dependencies': ['phase1_config_externalization']}}, 'test_user_list_flag': {'state': 'rollout', 'default_variant': 'off', 'variants': {'on': True, 'off': False}, 'rollout': {'strategy': 'user_list', 'user_list': ['admin001', 'tester001'], 'sticky': True}}}}
    ...
    def test_percentage_based_rollout(self):
        ...
        results = []
        for i in range(100):
            context = EvaluationContext(user_id=f'user_{i:03d}', environment='test')
    ...
    def test_targeting_rules(self):
        ...
        manager = FeatureFlagManager(self.config_path, watch_files=False)
        admin_context = EvaluationContext(user_id='admin_user', user_type='admin', environment='test')
        ...
        user_context = EvaluationContext(user_id='regular_user', user_type='user', environment='test')
    ...
    def test_sticky_bucketing(self):
        ...
        manager = FeatureFlagManager(self.config_path, watch_files=False)
        user_context = EvaluationContext(user_id='consistent_user', environment='test')
        results = []
        for _ in range(10):
            result = manager.evaluate_flag('phase1_config_externalization', user_context)
            results.append(result.value)
        self.assertTrue(all((r == results[0] for r in results)), 'User should get consistent results')
        manager.shutdown()
    ...
    def test_disabled_flag_handling(self):
        ...
        disabled_config['flags']['disabled_flag'] = {'state': 'disabled', 'default_variant': 'off', 'variants': {'on': True, 'off': False}}
    ...
    def test_concurrent_evaluations(self):
        ...
        def evaluate_flags():
            try:
                for i in range(10):
                    context = EvaluationContext(user_id=f'concurrent_user_{threading.current_thread().ident}_{i}', environment='test')
                    result = manager.evaluate_flag('phase1_config_externalization', context)
    ...
    def test_configuration_validation(self):
        ...
        invalid_config = {'flags': {'invalid_flag': {'state': 'invalid_state', 'default_variant': 'missing_variant', 'variants': {'on': True, 'off': False}}}}
...
class TestFeatureFlagInitialization(unittest.TestCase):
    ...
    def setUp(self):
        ...
        configs = {'feature_flags.yaml': {'version': '1.0.0', 'flags': {}}, 'feature_flags_development.yaml': {'version': '1.0.0', 'environment': 'development', 'flags': {}}, 'feature_flags_production.yaml': {'version': '1.0.0', 'environment': 'production', 'flags': {}}}
    ...
    def test_get_config_path_environment_specific(self):
        ...
        staging_path = get_config_path('staging')
    ...
    def test_initialize_feature_flag_system(self):
        ...
        config_info = manager.get_configuration_info()
        self.assertEqual(config_info['flags_count'], 0)
        manager.shutdown()
...
class TestFeatureFlagPerformance(unittest.TestCase):
    ...
    def setUp(self):
        ...
        flags = {}
        for i in range(100):
            flags[f'perf_flag_{i}'] = {'state': 'rollout', 'default_variant': 'off', 'variants': {'on': True, 'off': False}, 'rollout': {'strategy': 'percentage', 'percentage': 10.0, 'sticky': True}}
    ...
    def test_evaluation_performance(self):
        ...
        self.assertGreater(evaluations_per_second, 1000, f'Performance: {evaluations_per_second:.0f} eval/s, should be >1000')
        manager.shutdown()
    ...
    def test_concurrent_performance(self):
        ...
        def concurrent_evaluations():
            thread_results = []
            context = EvaluationContext(user_id=f'user_{threading.current_thread().ident}', environment='test')
            for i in range(100):
                flag_key = f'perf_flag_{i % 100}'
