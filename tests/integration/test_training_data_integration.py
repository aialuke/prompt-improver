...
class TestTrainingDataIntegration:
    ...
    async def sample_training_data(self, test_db_session: AsyncSession):
        ...
        for i in range(20):
            perf = RulePerformance(
                ...
                improvement_score=0.5 + i % 10 * 0.05,
    ...
    async def test_context_learner_integration(
        self, test_db_session: AsyncSession, sample_training_data, training_data_loader
    ):
        """Test Context Learner training data integration."""
        config = ContextConfig(min_sample_size=5, similarity_threshold=0.7)
        learner = ContextSpecificLearner(
            config=config, training_loader=training_data_loader
        )
        result = await learner.train_on_historical_data(test_db_session)
        assert result is True, (
            "Context learner should successfully train on historical data"
        )
        assert hasattr(learner, "context_clusters"), (
            "Context clusters should be created"
        )
        ...
        if hasattr(learner, "context_clusters") and learner.context_clusters:
            for cluster_id, cluster_data in learner.context_clusters.items():
                assert isinstance(cluster_data, dict), (
                    f"Cluster {cluster_id} should be a dictionary"
                )
                assert "size" in cluster_data, (
                    f"Cluster {cluster_id} should have size information"
                )
                assert cluster_data["size"] > 0, (
                    f"Cluster {cluster_id} should have positive size"
                )
    ...
    async def test_clustering_optimizer_integration(
        ...
        config = ClusteringConfig(
            memory_efficient_mode=True, auto_dim_reduction=False, enable_caching=False
        )
        ...
        if result["status"] == "insufficient_data":
            ...
            assert result["status"] == "success", (
                f"Optimization should succeed, got: {result}"
            )
            assert "training_samples" in result, (
                "Result should include training sample count"
            )
        ...
        assert "discovered_patterns" in pattern_result, (
            "Should include discovered patterns"
        )
    ...
    async def test_dimensionality_reducer_integration(
        ...
        config = DimensionalityConfig(
            target_dimensions=5, fast_mode=True, auto_method_selection=True
        )
        ...
        assert result["status"] == "success", (
            f"Feature space optimization should succeed, got: {result}"
        )
        ...
        assert 1 <= optimal_dims <= 20, (
            f"Optimal dimensions should be reasonable, got {optimal_dims}"
        )
        if "variance_preserved" in result:
            variance = result["variance_preserved"]
            assert 0.0 <= variance <= 1.0, (
                f"Variance preserved should be between 0 and 1, got {variance}"
            )
        mock_new_data = np.random.rand(5, 10)
        adaptive_result = await reducer.adaptive_reduction(
            mock_new_data, test_db_session
        )
        assert adaptive_result["status"] == "success", (
            "Adaptive reduction should succeed"
        )
    ...
    async def test_training_data_loader_validation(
        ...
        assert "synthetic_samples" in metadata, (
            "Metadata should include synthetic sample count"
        )
        ...
        assert total_samples == real_samples + synthetic_samples, (
        ...
        assert 0.0 <= synthetic_ratio <= 1.0, (
            f"Synthetic ratio should be between 0 and 1, got {synthetic_ratio}"
        )
    ...
    async def test_false_positive_prevention(
        ...
        context_learner = ContextSpecificLearner(
            config=ContextConfig(min_sample_size=100), training_loader=empty_loader
        )
        ...
        assert context_result is False, (
            "Context learner should not claim success with no data"
        )
        clustering_result = await clustering_optimizer.optimize_with_training_data(
            test_db_session
        )
        assert clustering_result["status"] != "success", (
            "Clustering optimizer should not claim success with no data"
        )
        dimensionality_result = await dimensionality_reducer.optimize_feature_space(
            test_db_session
        )
        assert dimensionality_result["status"] != "success", (
            "Dimensionality reducer should not claim success with no data"
        )
    ...
    async def test_property_based_integration(
        self, sample_size: int, feature_dims: int, improvement_scores: np.ndarray
    ):
        ...
        if feature_dims >= 2 and sample_size >= 5:
            ...
            if result["status"] == "success":
                ...
                assert (
                    1
                    <= result["optimal_dimensions"]
                    <= min(feature_dims, sample_size - 1)
                )
                if "variance_preserved" in result:
    ...
    async def test_integration_performance_bounds(
        ...
        assert elapsed_time < 10.0, (
            f"Context learner training took too long: {elapsed_time:.2f}s"
        )
        if result:
            assert hasattr(learner, "context_patterns"), (
                "Should create context patterns"
            )
