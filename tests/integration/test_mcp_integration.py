...
class TestMCPIntegration:
    ...
    async def test_end_to_end_prompt_improvement(self, test_db_session):
        ...
        result = await service.improve_prompt(prompt='Please help me write code that does stuff', user_context={'project_type': 'python', 'complexity': 'moderate'}, session_id='integration_test', db_session=test_db_session)
    ...
    async def test_performance_requirement_compliance(self, test_db_session):
        ...
        from prompt_improver.services.prompt_improvement import PromptImprovementService
        test_prompts = ['Simple prompt', 'More complex prompt with multiple requirements and context', 'Very detailed prompt with extensive background information and specific technical requirements that need processing']
        response_times = []
        service = PromptImprovementService(enable_bandit_optimization=False, enable_automl=False)
        for i, prompt in enumerate(test_prompts):
            start_time = asyncio.get_event_loop().time()
            result = await service.improve_prompt(prompt=prompt, user_context={'domain': 'performance_test'}, session_id=f'perf_test_{len(prompt)}', db_session=test_db_session)
            end_time = asyncio.get_event_loop().time()
            total_time = (end_time - start_time) * 1000
            processing_time_reported = result['processing_time_ms']
            response_times.append(processing_time_reported)
            assert processing_time_reported < 5000, f'Response time {processing_time_reported}ms exceeds 5000ms limit for real service'
            assert total_time < 10000, f'Total test execution time {total_time}ms too high for real service'
            assert 'improved_prompt' in result
            assert 'applied_rules' in result
            assert isinstance(result['applied_rules'], list)
        avg_response_time = sum(response_times) / len(response_times)
        assert avg_response_time < 3000, f'Average response time {avg_response_time}ms too high for real service'
        max_response_time = max(response_times)
        assert max_response_time < 4000, f'Maximum response time {max_response_time}ms too close to limit for real service'
...
class TestDatabaseIntegration:
    ...
    async def test_database_session_lifecycle(self, test_db_session):
        ...
        test_rule = RuleMetadata(rule_id=unique_id, rule_name='Integration Test Rule', rule_category='test', rule_description='Test rule for integration testing', enabled=True, priority=1, rule_version='1.0', default_parameters={'test': True, 'confidence_threshold': 0.8, 'weight': 1.0}, parameter_constraints={'confidence_threshold': {'min': 0.0, 'max': 1.0}, 'weight': {'min': 0.0, 'max': 2.0}})
        test_db_session.add(test_rule)
        await test_db_session.commit()
        from sqlmodel import select
        result = await test_db_session.execute(select(RuleMetadata).where(RuleMetadata.rule_id == unique_id))
    ...
    async def test_database_transaction_rollback(self, test_db_session):
        ...
        test_rule = RuleMetadata(rule_id=rollback_id, rule_name='Rollback Test Rule', rule_category='test', rule_description='Test rule for rollback testing', enabled=True, priority=1, rule_version='1.0', default_parameters={'test': True})
        test_db_session.add(test_rule)
        await test_db_session.rollback()
        from sqlmodel import select
        result = await test_db_session.execute(select(RuleMetadata).where(RuleMetadata.rule_id == rollback_id))
        stored_rule = result.scalar_one_or_none()
...
class TestServiceIntegration:
    ...
    async def test_prompt_improvement_service_integration(self, test_db_session, sample_rule_metadata):
        ...
        await test_db_session.commit()
        service = PromptImprovementService(enable_bandit_optimization=False, enable_automl=False)
        result = await service.improve_prompt(prompt='Make this better', user_context={'domain': 'integration_test'}, session_id='service_integration_test', db_session=test_db_session)
        assert 'improved_prompt' in result
        assert 'applied_rules' in result
        ...
        rules_metadata = await service.get_rules_metadata(enabled_only=True, db_session=test_db_session)
    ...
    async def test_analytics_service_integration(self, test_db_session, sample_rule_performance):
        ...
        for i, perf in enumerate(sample_rule_performance[:10]):
            if perf.rule_id and perf.rule_id not in created_rules:
                rule_metadata = RuleMetadata(rule_id=perf.rule_id, rule_name=f'Test Rule {i}', category='test', enabled=True, priority=1, rule_version='1.0', default_parameters={'test': True})
            ...
            if perf.session_id and perf.session_id not in created_sessions:
                session = PromptSession(session_id=perf.session_id, original_prompt=f'Test prompt {i}', improved_prompt=f'Improved test prompt {i}', user_context={'test': True}, session_status='completed')
        ...
        performance_summary = await analytics_service.get_performance_summary(days=30, db_session=test_db_session)
        ...
        prompt_analysis = await analytics_service.get_prompt_type_analysis(days=30, db_session=test_db_session)
...
class TestEndToEndWorkflow:
    ...
    async def test_complete_prompt_improvement_workflow(self, test_db_session, sample_rule_metadata):
        ...
        result = await service.improve_prompt(prompt=original_prompt, user_context={'domain': 'software_development', 'complexity': 'moderate'}, session_id='e2e_test_session', db_session=test_db_session)
        ...
        performance_summary = await analytics_service.get_performance_summary(days=1, db_session=test_db_session)
        assert 'total_sessions' in performance_summary
        assert isinstance(performance_summary['total_sessions'], int)
...
class TestDatabaseConstraintValidation:
    ...
    async def test_rule_metadata_constraints(self, test_db_session):
        ...
        valid_rule = RuleMetadata(rule_id=rule_id, rule_name='Test Rule', rule_category='test', enabled=True, priority=5, rule_version='1.0.0')
        test_db_session.add(valid_rule)
        await test_db_session.commit()
        result = await test_db_session.execute(select(RuleMetadata).where(RuleMetadata.rule_id == rule_id))
        stored_rule = result.scalar_one_or_none()
        assert stored_rule is not None
    ...
    async def test_rule_metadata_unique_constraint(self, test_db_session):
        ...
        rule1 = RuleMetadata(rule_id=duplicate_id, rule_name='First Rule', enabled=True, priority=5)
        test_db_session.add(rule1)
        await test_db_session.commit()
        rule2 = RuleMetadata(rule_id=duplicate_id, rule_name='Second Rule', enabled=True, priority=6)
    ...
    async def test_rule_performance_constraints(self, test_db_session):
        ...
        valid_performance = RulePerformance(rule_id=perf_rule_id, rule_name='Test Rule', prompt_id=uuid.uuid4(), improvement_score=0.8, confidence_level=0.9, execution_time_ms=150)
        test_db_session.add(valid_performance)
        await test_db_session.commit()
        result = await test_db_session.execute(select(RulePerformance).where(RulePerformance.rule_id == perf_rule_id))
    ...
    @given(improvement_score=st.floats(min_value=-1.0, max_value=2.0), confidence_level=st.floats(min_value=-1.0, max_value=2.0))
    ...
    async def test_rule_performance_check_constraints(self, test_db_session, improvement_score, confidence_level):
        ...
        performance_record = RulePerformance(rule_id=constraint_rule_id, rule_name='Constraint Test', prompt_id=uuid.uuid4(), improvement_score=improvement_score, confidence_level=confidence_level, execution_time_ms=100)
        test_db_session.add(performance_record)
        if 0.0 <= improvement_score <= 1.0 and 0.0 <= confidence_level <= 1.0:
            try:
                await test_db_session.commit()
                result = await test_db_session.execute(select(RulePerformance).where(RulePerformance.rule_id == constraint_rule_id).order_by(RulePerformance.id.desc()).limit(1))
    ...
    async def test_user_feedback_rating_constraint(self, test_db_session):
        ...
        for rating in valid_ratings:
            ...
            feedback = UserFeedback(original_prompt='Test prompt', improved_prompt='Improved test prompt', user_rating=rating, applied_rules={'rules': ['clarity_rule']}, session_id=session_id, rating=rating, feedback_text='Test feedback text', improvement_areas={'areas': ['clarity']}, is_processed=False, ml_optimized=False)
            test_db_session.add(feedback)
            await test_db_session.commit()
            result = await test_db_session.execute(select(UserFeedback).where(UserFeedback.session_id == session_id))
    ...
    async def test_user_feedback_invalid_rating_constraint(self, test_db_session):
        ...
        for rating in invalid_ratings:
            ...
            feedback = UserFeedback(original_prompt='Test prompt', improved_prompt='Improved test prompt', user_rating=rating, applied_rules={'rules': ['clarity_rule']}, session_id=invalid_session_id)
            test_db_session.add(feedback)
            with pytest.raises(IntegrityError):
                await test_db_session.commit()
            await test_db_session.rollback()
    ...
    async def test_comprehensive_constraint_validation(self, test_db_session):
        ...
        from prompt_improver.database.models import ABExperiment, RulePerformance, UserFeedback
        invalid_scores = [-0.5, 1.5, 2.0, -1.0]
        for score in invalid_scores:
            perf = RulePerformance(rule_id=f'invalid_score_test_{random.randint(10000, 99999)}', rule_name='Invalid Score Test', prompt_id=uuid.uuid4(), improvement_score=score, confidence_level=0.8, execution_time_ms=100)
            test_db_session.add(perf)
            with pytest.raises(IntegrityError):
                await test_db_session.commit()
            await test_db_session.rollback()
        ...
        for rating in valid_boundary_ratings:
            feedback = UserFeedback(original_prompt='Boundary test prompt', improved_prompt='Boundary test improved prompt', user_rating=rating, applied_rules={'rules': ['test_rule']}, session_id=f'boundary_test_{rating}_{random.randint(10000, 99999)}')
            test_db_session.add(feedback)
            await test_db_session.commit()
        valid_statuses = ['planning', 'running', 'completed', 'stopped']
        for status in valid_statuses:
            experiment = ABExperiment(experiment_name=f'Status Test {status}', control_rules={'rules': ['control']}, treatment_rules={'rules': ['treatment']}, status=status)
            test_db_session.add(experiment)
            await test_db_session.commit()
        invalid_experiment = ABExperiment(experiment_name='Invalid Status Test', control_rules={'rules': ['control']}, treatment_rules={'rules': ['treatment']}, status='invalid_status')
    ...
    async def test_constraint_error_messages(self, test_db_session):
        ...
        from prompt_improver.database.models import RulePerformance
        invalid_perf = RulePerformance(rule_id='error_message_test', rule_name='Error Message Test', prompt_id=uuid.uuid4(), improvement_score=2.0, confidence_level=0.8, execution_time_ms=100)
...
class TestDatabaseTransactionIntegrity:
    ...
    async def test_transaction_rollback_on_constraint_violation(self, test_db_session):
        ...
        valid_rule = RuleMetadata(rule_id='transaction_test', rule_name='Transaction Test', enabled=True, priority=5)
        test_db_session.add(valid_rule)
        invalid_performance = RulePerformance(rule_id='transaction_test', rule_name='Transaction Test', prompt_id=uuid.uuid4(), improvement_score=2.0, confidence_level=0.8, execution_time_ms=100)
        test_db_session.add(invalid_performance)
        with pytest.raises(IntegrityError):
            await test_db_session.commit()
        await test_db_session.rollback()
        result = await test_db_session.execute(select(RuleMetadata).where(RuleMetadata.rule_id == 'transaction_test'))
    ...
    async def test_complex_transaction_with_multiple_models(self, test_db_session):
        ...
        from prompt_improver.database.models import ImprovementSession, RuleMetadata, RulePerformance, UserFeedback
        session_id = 'complex_transaction_test'
        rule = RuleMetadata(rule_id='complex_rule', rule_name='Complex Rule', enabled=True, priority=5)
        performance = RulePerformance(rule_id='complex_rule', rule_name='Complex Rule', prompt_id=uuid.uuid4(), improvement_score=0.85, confidence_level=0.9, execution_time_ms=120)
        feedback = UserFeedback(original_prompt='Original complex prompt', improved_prompt='Improved complex prompt', user_rating=4, applied_rules={'rules': ['complex_rule']}, session_id=session_id)
        improvement_session = ImprovementSession(session_id=session_id, original_prompt='Original complex prompt', final_prompt='Improved complex prompt', iteration_count=1, total_improvement_score=0.85, status='completed')
        test_db_session.add(rule)
        test_db_session.add(performance)
        test_db_session.add(feedback)
        test_db_session.add(improvement_session)
        await test_db_session.commit()
        rule_result = await test_db_session.execute(select(RuleMetadata).where(RuleMetadata.rule_id == 'complex_rule'))
        assert rule_result.scalar_one_or_none() is not None
        performance_result = await test_db_session.execute(select(RulePerformance).where(RulePerformance.rule_id == 'complex_rule'))
        assert performance_result.scalar_one_or_none() is not None
        feedback_result = await test_db_session.execute(select(UserFeedback).where(UserFeedback.session_id == session_id))
        assert feedback_result.scalar_one_or_none() is not None
        session_result = await test_db_session.execute(select(ImprovementSession).where(ImprovementSession.session_id == session_id))
    ...
    async def test_concurrent_transaction_handling(self, test_db_session):
        ...
        async def create_rule(rule_id: str, priority: int):
            try:
                rule = RuleMetadata(rule_id=rule_id, rule_name=f'Concurrent Rule {priority}', enabled=True, priority=priority)
                test_db_session.add(rule)
                await test_db_session.commit()
                return True
...
class TestDatabasePerformanceValidation:
    ...
    async def test_bulk_insert_performance(self, test_db_session):
        ...
        for i in range(batch_size):
            record = RulePerformance(rule_id=f'bulk_rule_{i}', rule_name=f'Bulk Rule {i}', prompt_id=uuid.uuid4(), improvement_score=0.7 + i % 3 * 0.1, confidence_level=0.8 + i % 2 * 0.1, execution_time_ms=100 + i % 50)
    ...
    async def test_query_performance_scaling(self, test_db_session, query_count):
        ...
        from prompt_improver.database.models import RuleMetadata
        for i in range(20):
            rule = RuleMetadata(rule_id=f'query_test_rule_{i}', rule_name=f'Query Test Rule {i}', enabled=True, priority=i % 10)
            test_db_session.add(rule)
        await test_db_session.commit()
        query_times = []
        for i in range(query_count):
            start_time = time.time()
            result = await test_db_session.execute(select(RuleMetadata).where(RuleMetadata.priority == i % 10))
...
class TestDatabaseSchemaValidation:
    ...
    async def test_jsonb_field_operations(self, test_db_session):
        ...
        from prompt_improver.database.models import RuleMetadata
        complex_parameters = {'weights': {'clarity': 0.8, 'specificity': 0.7}, 'thresholds': {'min_confidence': 0.6, 'max_iterations': 10}, 'features': ['length', 'complexity', 'domain'], 'nested': {'algorithm': 'gradient_boost', 'hyperparams': {'learning_rate': 0.1, 'n_estimators': 100}}}
        constraints = {'clarity_weight': {'min': 0.0, 'max': 1.0}, 'specificity_weight': {'min': 0.0, 'max': 1.0}, 'confidence_threshold': {'type': 'float', 'range': [0.0, 1.0]}}
        rule = RuleMetadata(rule_id='jsonb_test_rule', rule_name='JSONB Test Rule', default_parameters=complex_parameters, parameter_constraints=constraints, enabled=True, priority=5)
        test_db_session.add(rule)
        await test_db_session.commit()
        result = await test_db_session.execute(select(RuleMetadata).where(RuleMetadata.rule_id == 'jsonb_test_rule'))
        stored_rule = result.scalar_one_or_none()
        assert stored_rule is not None
    ...
    async def test_index_performance_validation(self, test_db_session):
        ...
        for i in range(large_dataset_size):
            performance = RulePerformance(rule_id=f'index_test_rule_{i % 10}', rule_name=f'Index Test Rule {i % 10}', prompt_id=uuid.uuid4(), prompt_type=f'type_{i % 5}', improvement_score=0.5 + i % 5 * 0.1, confidence_level=0.6 + i % 4 * 0.1, execution_time_ms=100 + i % 100)
            test_db_session.add(performance)
        await test_db_session.commit()
        indexed_queries = [select(RulePerformance).where(RulePerformance.rule_id == 'index_test_rule_5'), select(RulePerformance).where(RulePerformance.prompt_type == 'type_2'), select(RulePerformance).where(RulePerformance.improvement_score > 0.7)]
...
class TestRealServiceErrorHandling:
    ...
    async def test_prompt_improvement_service_error_handling(self, test_db_session):
        ...
        from prompt_improver.services.prompt_improvement import PromptImprovementService
        service = PromptImprovementService(enable_bandit_optimization=False, enable_automl=False)
        result = await service.improve_prompt(prompt='', user_context={'domain': 'test'}, session_id='empty_prompt_test', db_session=test_db_session)
        assert 'improved_prompt' in result
        assert 'error' not in result or result.get('error') is None
        assert result['processing_time_ms'] < 5000
        long_prompt = 'A' * 10000
        result = await service.improve_prompt(prompt=long_prompt, user_context={'domain': 'test'}, session_id='long_prompt_test', db_session=test_db_session)
        assert 'improved_prompt' in result
        assert len(result['improved_prompt']) > 0
        assert result['processing_time_ms'] < 10000
        result = await service.improve_prompt(prompt='Test prompt', user_context=None, session_id='invalid_context_test', db_session=test_db_session)
        assert 'improved_prompt' in result
        assert result['processing_time_ms'] < 5000
        special_prompt = 'Test with special chars: @#$%^&*()[]{}|\\:;"\'<>,.?/~`'
        result = await service.improve_prompt(prompt=special_prompt, user_context={'domain': 'test'}, session_id='special_chars_test', db_session=test_db_session)
        assert 'improved_prompt' in result
        assert len(result['improved_prompt']) > 0
        assert result['processing_time_ms'] < 5000
    ...
    async def test_analytics_service_error_handling(self, test_db_session):
        ...
        try:
            ...
            pass
        performance_summary = await analytics_service.get_performance_summary(days=365, db_session=test_db_session)
        assert 'total_sessions' in performance_summary
        assert 'avg_improvement' in performance_summary
        assert isinstance(performance_summary['total_sessions'], int)
    ...
    async def test_database_connection_error_handling(self, test_db_session):
        ...
        service = PromptImprovementService(enable_bandit_optimization=False, enable_automl=False)
        try:
            result = await service.improve_prompt(prompt='Test prompt', user_context={'domain': 'test'}, session_id='no_db_test', db_session=None)
            if result:
                assert 'improved_prompt' in result
        except Exception as e:
    ...
    async def test_concurrent_service_operations(self, test_db_session):
        ...
        async def improve_prompt_task(prompt_id: int):
            try:
                result = await service.improve_prompt(prompt=f'Concurrent test prompt {prompt_id}', user_context={'domain': 'concurrent_test'}, session_id=f'concurrent_test_{prompt_id}', db_session=test_db_session)
                return result
            except Exception as e:
                return {'error': str(e)}
    ...
    async def test_edge_case_prompt_types(self, test_db_session):
        ...
        from prompt_improver.services.prompt_improvement import PromptImprovementService
        service = PromptImprovementService(enable_bandit_optimization=False, enable_automl=False)
        edge_cases = ['测试中文提示符', 'Тест русского текста', '🚀 Emoji test prompt 🎯', "def function(): return 'test'", 'SELECT * FROM users WHERE id = 1;', "Write code that does: print('hello') and explain it", '123456789 + 987654321 = ?', 'Hi', '?', 'test ' * 100]
        for i, prompt in enumerate(edge_cases):
            result = await service.improve_prompt(prompt=prompt, user_context={'domain': 'edge_case_test'}, session_id=f'edge_case_{i}', db_session=test_db_session)
            assert 'improved_prompt' in result
            assert 'processing_time_ms' in result
            assert result['processing_time_ms'] < 5000
            assert len(result['improved_prompt']) > 0
    ...
    async def test_service_performance_under_load(self, test_db_session):
        ...
        for i in range(10):
            start_time = time.time()
            result = await service.improve_prompt(prompt=f'Load test prompt {i}', user_context={'domain': 'load_test'}, session_id=f'load_test_seq_{i}', db_session=test_db_session)
            end_time = time.time()
        ...
        async def concurrent_task(task_id: int):
            start_time = time.time()
            result = await service.improve_prompt(prompt=f'Concurrent load test {task_id}', user_context={'domain': 'concurrent_load_test'}, session_id=f'load_test_conc_{task_id}', db_session=test_db_session)
            end_time = time.time()
        ...
        if concurrent_times:
            ...
            assert avg_concurrent_time < avg_sequential_time * 3, f'Concurrent avg {avg_concurrent_time:.1f}ms vs sequential avg {avg_sequential_time:.1f}ms'
    ...
    async def test_real_service_memory_usage(self, test_db_session):
        ...
        service = PromptImprovementService(enable_bandit_optimization=False, enable_automl=False)
        for i in range(50):
            result = await service.improve_prompt(prompt=f'Memory test prompt {i} with some additional content to test memory usage patterns', user_context={'domain': 'memory_test', 'iteration': i}, session_id=f'memory_test_{i}', db_session=test_db_session)
            assert 'improved_prompt' in result
            if i % 10 == 0:
                gc.collect()
...
class TestRealBehaviorValidation:
    ...
    async def test_real_vs_mock_behavior_comparison(self, test_db_session):
        ...
        results = []
        for i in range(3):
            result = await service.improve_prompt(prompt=test_prompt, user_context={'domain': 'software_development'}, session_id=f'consistency_test_{i}', db_session=test_db_session)
        ...
        analytics_result = await analytics_service.get_performance_trends(db_session=test_db_session)
    ...
    async def test_migration_completeness_validation(self, test_db_session):
        ...
        result = await service.improve_prompt(prompt='Test migration completeness', user_context={'domain': 'validation'}, session_id='migration_validation', db_session=test_db_session)
        required_fields = ['improved_prompt', 'applied_rules', 'processing_time_ms', 'original_prompt', 'session_id', 'improvement_summary', 'confidence_score']
    ...
    async def test_2025_best_practices_compliance(self, test_db_session):
        ...
        from prompt_improver.services.prompt_improvement import PromptImprovementService
        service = PromptImprovementService(enable_bandit_optimization=False, enable_automl=False)
        result = await service.improve_prompt(prompt='Test 2025 best practices', user_context={'domain': 'best_practices'}, session_id='best_practices_test', db_session=test_db_session)
        assert isinstance(result['processing_time_ms'], (int, float))
    ...
    async def test_real_data_persistence_validation(self, test_db_session):
        ...
        from prompt_improver.database.models import RuleMetadata, RulePerformance
        from prompt_improver.services.prompt_improvement import PromptImprovementService
        test_rule = RuleMetadata(rule_id='persistence_test_rule', rule_name='Persistence Test Rule', rule_category='test', enabled=True, priority=1, rule_version='1.0', default_parameters={'test': True})
        test_db_session.add(test_rule)
        await test_db_session.commit()
        service = PromptImprovementService(enable_bandit_optimization=False, enable_automl=False)
        result = await service.improve_prompt(prompt='Test data persistence', user_context={'domain': 'persistence_test'}, session_id='persistence_validation', db_session=test_db_session)
        assert 'improved_prompt' in result
        assert 'applied_rules' in result
        rules_metadata = await service.get_rules_metadata(enabled_only=True, db_session=test_db_session)
        assert isinstance(rules_metadata, list)
        ...
        from sqlmodel import select
        result_query = await test_db_session.execute(select(RuleMetadata).where(RuleMetadata.rule_id == 'persistence_test_rule'))
