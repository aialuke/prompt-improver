...
class APESServiceManagerRealBehaviorTest:
    ...
    async def test_orchestrator_service_lifecycle(self):
        ...
        try:
            ...
            self.log_test('APESServiceManager loaded in orchestrator', 'apes_service_manager' in loaded_components)
            if 'apes_service_manager' in loaded_components:
                ...
                if component.instance:
                    ...
                    self.log_test('Status includes orchestrator integration', status.get('orchestrator_integration', False))
                    self.log_test('Event bus connection detected in status', status.get('event_bus_connected', False))
    ...
    async def test_service_manager_event_integration(self):
        ...
        try:
            ...
            def event_handler(event):
                ...
                logger.info('Received event: %s from %s', event.event_type.value, event.source)
            subscription_id = event_bus.subscribe(EventType.COMPONENT_STARTED, event_handler)
            await asyncio.sleep(0.1)
            await service_manager._emit_service_event('service.started', {'status': 'running', 'test_mode': True, 'timestamp': '2025-01-23T00:00:00Z'})
    ...
    async def test_service_status_real_behavior(self):
        ...
        try:
            ...
            required_fields = ['running', 'pid', 'started_at', 'uptime_seconds', 'memory_usage_mb', 'service_status', 'is_initialized', 'event_bus_connected', 'orchestrator_integration']
            missing_fields = [field for field in required_fields if field not in status]
            self.log_test('Status includes all enhanced fields', len(missing_fields) == 0, f'Missing: {missing_fields}' if missing_fields else '')
            self.log_test('Orchestrator integration flag is True', status.get('orchestrator_integration') is True)
            self.log_test('Service status is tracked', status.get('service_status') in ['unknown', 'stopped', 'starting', 'running', 'failed'])
        except Exception as e:
    ...
    async def test_component_discovery_integration(self):
        ...
        try:
            ...
            self.log_test('APESServiceManager in Tier 4 components', 'apes_service_manager' in tier4_components)
            component = await loader.load_component('apes_service_manager', ComponentTier.TIER_2)
            ...
            if component:
                ...
                self.log_test('Component has correct module path', 'prompt_improver.core.services.manager' in component.module_path)
                ...
                if success:
                    ...
                    self.log_test('Instance has all required methods', len(missing_methods) == 0, f'Missing: {missing_methods}' if missing_methods else '')
        except Exception as e:
