...
from prompt_improver.ml.optimization.algorithms.clustering_optimizer import ClusteringConfig, ClusteringMetrics, ClusteringOptimizer, get_clustering_optimizer

@pytest.fixture
def clustering_config():
    """Create test configuration for clustering optimization."""
    return ClusteringConfig(memory_efficient_mode=True, max_memory_mb=512, auto_dim_reduction=True, target_dimensions=8, hdbscan_min_cluster_size=3, min_silhouette_score=0.2, enable_performance_tracking=True, enable_caching=True)

@pytest.fixture
def high_dimensional_data():
    """Create high-dimensional test data (31 features like linguistic analysis)."""
    X, y = make_classification(n_samples=500, n_features=31, n_informative=20, n_redundant=5, n_clusters_per_class=2, class_sep=1.5, random_state=42)
...
@pytest.mark.asyncio
async def test_high_dimensional_clustering_optimization(clustering_config, high_dimensional_data):
...
@pytest.mark.asyncio
async def test_linguistic_features_clustering(clustering_config, linguistic_like_features):
...
async def test_memory_efficient_processing(clustering_config):
    ...
    X, _ = make_classification(n_samples=2000, n_features=31, n_informative=20, n_redundant=5, n_clusters_per_class=2, class_sep=1.2, random_state=42)
...
async def test_parameter_optimization():
    ...
    from sklearn.datasets import make_classification
    small_X, _ = make_classification(n_samples=50, n_features=31, n_informative=15, n_redundant=5, n_clusters_per_class=1, class_sep=1.5, random_state=42)
    small_result = await optimizer.optimize_clustering(small_X)
    large_X, _ = make_classification(n_samples=1000, n_features=31, n_informative=20, n_redundant=5, n_clusters_per_class=2, class_sep=1.2, random_state=43)
...
async def test_quality_assessment_comprehensive():
    ...
    X, _ = make_blobs(n_samples=400, centers=4, n_features=31, cluster_std=1.5, random_state=42)
    optimizer = ClusteringOptimizer()
    result = await optimizer.optimize_clustering(X)
    assert result['status'] == 'success'
    quality_metrics = result['quality_metrics']
    required_metrics = ['n_clusters', 'n_noise_points', 'noise_ratio', 'silhouette_score', 'calinski_harabasz_score', 'davies_bouldin_score', 'quality_score', 'processing_time_seconds', 'convergence_achieved']
...
async def test_supervised_optimization():
    ...
    X, y = make_classification(n_samples=200, n_features=31, n_classes=3, n_informative=20, random_state=42)
    optimizer = ClusteringOptimizer()
...
async def test_performance_recommendations():
    ...
    from sklearn.datasets import make_classification
    X, _ = make_classification(n_samples=50, n_features=31, n_informative=15, n_redundant=5, n_clusters_per_class=1, class_sep=1.0, random_state=42)
...
if __name__ == '__main__':
    ...
    async def run_debug_test():
        ...
        config = ClusteringConfig(memory_efficient_mode=True, target_dimensions=8, enable_performance_tracking=True)
        np.random.seed(42)
        X = np.random.rand(200, 31)
        optimizer = ClusteringOptimizer(config)
        result = await optimizer.optimize_clustering(X)
        ...
        print(f"Dimensionality: {result['preprocessing_info']['original_shape']} → {result['preprocessing_info']['final_shape']}")
