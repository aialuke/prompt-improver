...
async def test_data_provider_ab_testing_real_behavior():
    ...
    mock_orchestrator.get_active_experiments = AsyncMock(return_value=[{'id': 'exp1', 'name': 'Test Experiment 1', 'status': 'running', 'results': {'statistical_significance': True, 'effect_size': 0.15}}])
    mock_orchestrator.get_all_experiments = AsyncMock(return_value=[{'id': 'exp1'}, {'id': 'exp2'}])
