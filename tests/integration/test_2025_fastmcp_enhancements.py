...
class TestMiddlewareStack:
    ...
    async def test_rate_limiting_middleware_real_behavior(self):
        ...
        middleware = RateLimitingMiddleware(
            max_requests_per_second=2.0, burst_capacity=3
        )
    ...
    async def test_error_handling_middleware_real_behavior(self):
        ...
        middleware = ErrorHandlingMiddleware(
            include_traceback=True, transform_errors=True
        )
...
class TestProgressReporting:
    ...
    async def test_improve_prompt_progress_real_behavior(self):
        ...
        mock_ctx.report_progress = AsyncMock(
            side_effect=lambda **kwargs: progress_reports.append(kwargs)
        )
        mock_ctx.info = AsyncMock(
            side_effect=lambda msg: log_messages.append(("info", msg))
        )
        mock_ctx.debug = AsyncMock(
            side_effect=lambda msg: log_messages.append(("debug", msg))
        )
        mock_ctx.error = AsyncMock(
            side_effect=lambda msg: log_messages.append(("error", msg))
        )
        ...
        assert len(progress_reports) >= 4, (
            f"Expected at least 4 progress reports, got {len(progress_reports)}"
        )
        ...
        assert any(
            (
                "Beginning prompt enhancement" in msg
...
class TestWildcardResources:
    ...
    async def test_rule_category_performance_wildcard_real_behavior(self):
        ...
        with patch("prompt_improver.database.get_unified_manager") as mock_manager:
            ...
            mock_top_rules.fetchall.return_value = [
                ...
                ("sec_002", "SQL Injection Guard", 300, 0.8),
            ]
            mock_session.execute = AsyncMock(side_effect=[mock_result, mock_top_rules])
            result = await server._get_rule_category_performance_impl(
    ...
    async def test_hierarchical_metrics_real_behavior(self):
        ...
        result2 = await server._get_hierarchical_metrics_impl(
            "performance/tools/improve_prompt"
        )
...
class TestPerformanceTargets:
    ...
    async def test_response_time_under_200ms(self):
        ...
        result = await improve_prompt(
            prompt=prompt, session_id=session_id, ctx=mock_ctx, context=None
        )
