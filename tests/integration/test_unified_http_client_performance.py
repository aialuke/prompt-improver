...
import pytest
from prompt_improver.monitoring.unified_http_client import HTTPClientConfig, HTTPClientUsage, UnifiedHTTPClientFactory, get_http_client_factory, make_api_request, make_health_check_request, make_webhook_request
...
class TestUnifiedHTTPClientPerformance:
    ...
    async def http_factory(self):
        ...
        factory.register_client(HTTPClientConfig(name='test_webhook', usage_type=HTTPClientUsage.WEBHOOK_ALERTS, timeout_seconds=5.0, failure_threshold=2, recovery_timeout_seconds=10, circuit_breaker_enabled=True))
        factory.register_client(HTTPClientConfig(name='test_api', usage_type=HTTPClientUsage.API_CALLS, timeout_seconds=10.0, failure_threshold=3, recovery_timeout_seconds=30, circuit_breaker_enabled=True))
        return factory

    @pytest.fixture
    def mock_responses(self):
        """Mock HTTP responses for testing"""
        return {'success': AsyncMock(status=200, headers={}), 'error': AsyncMock(status=500, headers={}), 'timeout': AsyncMock(side_effect=TimeoutError())}
    ...
    async def test_rate_limit_awareness(self, http_factory, mock_responses):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            mock_session = AsyncMock()
            mock_session_class.return_value.__aenter__.return_value = mock_session
            rate_limited_response = AsyncMock(status=200, headers={'x-ratelimit-remaining': '10', 'x-ratelimit-limit': '100', 'x-ratelimit-reset': str(int(time.time()) + 3600)})
    ...
    async def test_webhook_alert_performance(self, mock_responses):
        ...
        test_payload = {'alert_type': 'performance_regression', 'metric_name': 'response_time', 'severity': 'critical', 'message': 'Response time degraded by 150%'}
        with patch('aiohttp.ClientSession') as mock_session_class:
            mock_session = AsyncMock()
            mock_session_class.return_value.__aenter__.return_value = mock_session
    ...
    async def test_concurrent_requests_performance(self, http_factory, mock_responses):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            ...
            async def make_test_request(request_id):
                try:
                    return await http_factory.make_request('test_api', 'GET', f'http://test.com/concurrent/{request_id}')
                except Exception:
                    return None
            start_time = time.time()
...
class TestPhase3IntegrationValidation:
    ...
    async def test_end_to_end_http_client_performance(self):
        ...
        factory = get_http_client_factory()
        client_tests = [('webhook_alerts', 'POST', 'http://webhook.test.com'), ('health_monitoring', 'GET', 'http://health.test.com/status'), ('api_calls', 'GET', 'http://api.test.com/data'), ('downloads', 'GET', 'http://downloads.test.com/file.zip')]
        with patch('aiohttp.ClientSession'):
    ...
    def test_phase3_success_metrics(self):
        ...
        production_clients = [config for config in factory.clients.values() if config.usage_type != HTTPClientUsage.TESTING]
