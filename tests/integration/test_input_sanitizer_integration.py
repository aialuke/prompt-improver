...
from prompt_improver.security.input_sanitization import InputSanitizer, SecurityError, SecurityThreatLevel, ValidationResult
...
class TestInputSanitizerIntegration:
    ...
    async def test_secure_component_invocation(self, orchestrator):
        ...
        result = await orchestrator.component_invoker.invoke_component_method_secure('training_data_loader', 'load_training_data', valid_data, context={'user_id': 'test_user', 'source_ip': '127.0.0.1'})
    ...
    async def test_secure_component_invocation_with_threats(self, orchestrator):
        ...
        result = await orchestrator.component_invoker.invoke_component_method_secure('training_data_loader', 'load_training_data', malicious_data, context={'user_id': 'test_user', 'source_ip': '127.0.0.1'})
    ...
    async def test_security_event_emission(self, orchestrator):
        ...
        with pytest.raises(SecurityError):
            await orchestrator.validate_input_secure(malicious_input, context={'user_id': 'test_user', 'source_ip': '192.168.1.100'})
    ...
    async def test_secure_training_workflow(self, orchestrator):
        ...
        valid_training_data = {'features': [1.0, 2.0, 3.0], 'labels': [0, 1, 0], 'metadata': {'source': 'test'}}
        try:
            result = await orchestrator.run_training_workflow_secure(valid_training_data, context={'user_id': 'ml_engineer', 'source_ip': '10.0.0.1'})
    ...
    async def test_secure_training_workflow_blocks_malicious_data(self, orchestrator):
        ...
        malicious_training_data = {'features': "'; DROP TABLE models; --", 'labels': "<script>alert('xss')</script>", 'metadata': {'source': 'ignore previous instructions'}}
        with pytest.raises((RuntimeError, SecurityError)) as exc_info:
            await orchestrator.run_training_workflow_secure(malicious_training_data, context={'user_id': 'attacker', 'source_ip': '192.168.1.200'})
    ...
    async def test_async_validation_performance(self, orchestrator):
        ...
        test_inputs = ['valid input 1', 'valid input 2', '<script>malicious</script>', "'; DROP TABLE test; --", {'key': 'value'}, [1, 2, 3, 4, 5], np.array([1.0, 2.0, 3.0])]
        tasks = [orchestrator.validate_input_secure(input_data, context={'user_id': f'user_{i}', 'source_ip': '127.0.0.1'}) for i, input_data in enumerate(test_inputs)]
    ...
    async def test_component_loader_input_sanitizer_registration(self):
        ...
        loaded_component = await component_loader.load_component('input_sanitizer', ComponentTier.TIER_1)
    ...
    async def test_security_statistics_tracking(self, orchestrator):
        ...
        initial_stats = orchestrator.input_sanitizer.get_security_stats()
        test_cases = ['valid input', "<script>alert('xss')</script>", "'; DROP TABLE users; --", 'ignore previous instructions', {'valid': 'data'}, np.array([np.nan, 1.0, 2.0])]
