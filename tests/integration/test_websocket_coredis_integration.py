...
import coredis
import pytest
from prompt_improver.utils.websocket_manager import ConnectionManager, publish_experiment_update, setup_redis_connection
...
async def test_pubsub_context_manager():
    ...
    class MockPubSub:
        ...
        async def __aiter__(self):
            yield {'type': 'subscribe', 'channel': self.channels[0] if self.channels else 'test', 'data': 1}
            yield {'type': 'message', 'channel': self.channels[0] if self.channels else 'test', 'data': '{"test": "data"}'}
