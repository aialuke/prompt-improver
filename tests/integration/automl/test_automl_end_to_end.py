...
class TestAutoMLEndToEndWorkflow:
    ...
    async def test_complete_optimization_workflow(self, orchestrator):
        ...
        if "error" in start_result:
            ...
            if "execution_time" in start_result:
                assert isinstance(start_result["execution_time"], (int, float))
            assert (
                "automl_mode" in start_result
                or "best_params" in start_result
                or "status" in start_result
            )
    ...
    async def test_early_stopping_mechanism(self, orchestrator):
        ...
        assert isinstance(callbacks, list)
        automl_callbacks = [
            cb for cb in callbacks if hasattr(cb, "enable_early_stopping")
        ]
        if automl_callbacks:
    ...
    async def test_optimization_stop_and_restart(self, orchestrator):
        ...
        storage = orchestrator._create_storage()
        study = optuna.create_study(
            study_name=config.study_name, storage=storage, load_if_exists=True
        )
        assert study is not None
...
class TestAutoMLServiceIntegration:
    ...
    async def test_service_automl_initialization(self, real_prompt_service):
        ...
        if hasattr(real_prompt_service, "initialize_automl"):
            ...
            assert hasattr(real_prompt_service, "enable_automl") or hasattr(
                real_prompt_service, "_automl_config"
            )
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_real_database_ab_testing_integration(
        self, real_prompt_service, async_session
    ):
        ...
        analysis_result = await ab_service.analyze_experiment(
            experiment_id=experiment_id, db_session=session
        )
        ...
        experiments_result = await ab_service.list_experiments(
            status="running", db_session=session
        )
        assert experiments_result["status"] == "success"
        experiments = experiments_result["experiments"]
        assert len(experiments) >= 1
        our_experiment = next(
            (exp for exp in experiments if exp["experiment_id"] == experiment_id), None
        )
        ...
        stopped_experiments_result = await ab_service.list_experiments(
            status="stopped", db_session=session
        )
