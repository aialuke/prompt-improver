...
class APESIntegrationTester:
    ...
    def test_centralized_utilities_integration(self):
        ...
        try:
            ...
            self.test_collector.logger.info(
                "Test log message from APES integration test"
            )
    ...
    async def test_mcp_server_performance_compliance(self):
        ...
        try:
            ...
            if not sla_violations:
                ...
                return True
            print(
                f"✗ MCP server performance compliance: FAILED - SLA violations: {sla_violations}"
            )
            return False
        except Exception as e:
        ...
        try:
            ...
            for i in range(5):
                api_metric = APIUsageMetric(
                    ...
                    category=EndpointCategory.RULE_APPLICATION,
                    status_code=200,
            ...
            cli_avg_time = sum(
                analytics["endpoint_analytics"][ep]["avg_response_time_ms"]
            ...
            assert cli_avg_time > mcp_avg_time, (
                "CLI and MCP performance profiles not properly separated"
            )
        ...
        try:
            ...
            for i, stage in enumerate(training_stages):
                journey_metric = UserJourneyMetric(
                    ...
                    journey_stage=UserJourneyStage.ML_TRAINING,
                    event_type=f"training_{stage}",
                    endpoint=f"/cli/v1/train/{stage}",
                    success=True,
                    conversion_value=None,
                    time_to_action_seconds=60.0 + i * 30,
                ...
                await self.test_collector.record_api_usage(api_metric)
            journey_analytics = await self.test_collector.get_user_journey_analytics(
                hours=1
            )
            if (
                "status" in journey_analytics
                and journey_analytics["status"] == "no_data"
            ):
                raise AssertionError("ML training journey data not found")
            assert "total_journey_events" in journey_analytics, (
                "Journey analytics missing total_journey_events"
            )
            assert "stage_analytics" in journey_analytics, (
                "Journey analytics missing stage_analytics"
            )
            assert journey_analytics["total_journey_events"] >= len(training_stages), (
                f"Expected at least {len(training_stages)} journey events, got {journey_analytics['total_journey_events']}"
            )
            stats = self.test_collector.get_collection_stats()
            assert stats["journey_events_tracked"] >= len(training_stages), (
                "Not all training stages tracked"
            )
            assert "ml_training" in journey_analytics["stage_analytics"], (
                "ML_TRAINING stage not found in analytics"
            )
            ml_training_stats = journey_analytics["stage_analytics"]["ml_training"]
            assert ml_training_stats["total_events"] >= len(training_stages), (
                "Not all training events recorded in stage analytics"
            )
            print(f"  Training stages tracked: {len(training_stages)}")
            print(f"  Journey events recorded: {stats['journey_events_tracked']}")
            print(
                f"  Total journey events in analytics: {journey_analytics['total_journey_events']}"
            )
    ...
    async def test_global_collector_singleton(self):
        ...
        try:
            ...
            assert stats["api_calls_tracked"] > 0, (
                "Global collector not receiving metrics"
            )
