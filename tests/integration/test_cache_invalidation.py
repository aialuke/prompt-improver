...
async def real_rule_metadata(real_db_session):
    ...
    metadata_records = [RuleMetadata(rule_id=f'clarity_rule_{test_suffix}', rule_name='Clarity Enhancement Rule', category='core', description='Improves prompt clarity', enabled=True, priority=5, default_parameters={'weight': 1.0, 'threshold': 0.7}, parameter_constraints={'weight': {'min': 0.0, 'max': 1.0}}, created_at=aware_utc_now(), updated_at=aware_utc_now()), RuleMetadata(rule_id=f'specificity_rule_{test_suffix}', rule_name='Specificity Enhancement Rule', category='core', description='Improves prompt specificity', enabled=True, priority=4, default_parameters={'weight': 0.8, 'threshold': 0.6}, parameter_constraints={'weight': {'min': 0.0, 'max': 1.0}}, created_at=aware_utc_now(), updated_at=aware_utc_now())]
...
class TestCacheInvalidationFlow:
    ...
    async def test_rule_parameter_update_emits_invalidation_event(self, ml_service, real_db_session, redis_client, real_rule_metadata):
    ...
    async def test_model_training_completion_emits_invalidation_event(self, ml_service, real_db_session, redis_client):
        """Test that model training completion emits pattern.invalidate events using real Redis."""
        training_data = {'features': [[0.5, 100, 0.8, 0.3, 0.7]] * 25, 'effectiveness_scores': [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.3, 0.4, 0.5, 0.6, 0.7]}
        pubsub = redis_client.pubsub()
        await pubsub.subscribe('pattern.invalidate')
        try:
            with patch('prompt_improver.services.ml_integration.redis_client', redis_client), patch('prompt_improver.services.ml_integration.mlflow') as mock_mlflow:
                ...
                for message in messages:
                    if message['type'] == 'message' and message['channel'] == b'pattern.invalidate':
                        try:
                            event_data = json.loads(message['data'])
    ...
    async def test_cache_subscriber_handles_invalidation_events(self, cache_subscriber, redis_client):
        """Test that cache subscriber properly handles pattern.invalidate events using real Redis."""
        test_keys = ['apes:pattern:test_key_1', 'apes:pattern:test_key_2', 'rule:test_key_1', 'rule:test_key_2']
        ...
        try:
            ...
            await asyncio.sleep(0.1)
            test_event = {'type': 'rule_parameters_updated', 'rule_ids': ['test_rule_1', 'test_rule_2'], 'cache_prefixes': ['apes:pattern:', 'rule:']}
    ...
    async def test_cache_invalidation_by_prefix(self, redis_client):
        ...
        with patch('prompt_improver.utils.redis_cache.redis_client', redis_client):
            subscriber = CacheSubscriber()
            test_keys = ['apes:pattern:key1', 'apes:pattern:key2', 'apes:pattern:key3', 'other:key1']
            for key in test_keys:
                await redis_client.set(key, 'test_value')
            for key in test_keys:
                assert await redis_client.exists(key)
    ...
    async def test_ensemble_model_training_emits_invalidation_event(self, ml_service, real_db_session, redis_client):
    ...
    async def test_cache_invalidation_maintains_data_freshness(self, redis_client):
        ...
        with patch('prompt_improver.utils.redis_cache.redis_client', redis_client):
            subscriber = CacheSubscriber()
            cached_keys = ['apes:pattern:rule_123', 'apes:pattern:model_456', 'rule:clarity_rule', 'rule:specificity_rule']
            for key in cached_keys:
                await redis_client.set(key, 'cached_value')
            for key in cached_keys:
                assert await redis_client.exists(key)
            event_data = {'type': 'rule_parameters_updated', 'rule_ids': ['clarity_rule', 'specificity_rule'], 'cache_prefixes': ['apes:pattern:', 'rule:']}
            await subscriber._handle_invalidate_event({'type': 'message', 'data': json.dumps(event_data)})
            for key in cached_keys:
                assert not await redis_client.exists(key)
            pattern_keys = []
