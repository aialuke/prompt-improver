"""
Comprehensive real behavior testing framework for MultiLevelCache using TestContainers.

This test suite provides 100% real behavior testing with actual Redis containers,
...
Features:
- Real Redis container integration via TestContainers
...
def redis_test_container():
    ...
    with RedisContainer(
        ...
        for i in range(max_retries):
            try:
                client.ping()
                logger.info(f"Redis container ready on port {container.get_exposed_port(6379)}")
                break
...
async def test_cache_config(redis_test_container):
    ...
    mock_redis_config = type('MockRedisConfig', (), {
        'host': host,
        'port': port,
        'database': 0,
        'password': None,
...
async def multi_level_cache(test_cache_config):
    ...
    with patch('prompt_improver.core.config.get_config', return_value=test_cache_config['mock_config']):
...
async def mock_fallback_database():
    ...
    database_data = {
        ...
        'session:789': {'id': 789, 'data': {'key': 'value'}, 'expires': time.time() + 3600}
    }
    
    async def mock_db_fetch(key: str) -> Optional[Dict[str, Any]]:
...
class TestBasicCacheOperations:
    ...
    async def test_cache_set_and_get_simple_data(self, multi_level_cache, sample_cache_data):
    ...
    async def test_cache_set_and_get_complex_data(self, multi_level_cache, sample_cache_data):
...
class TestMultiLevelBehavior:
    ...
    async def test_l2_cache_hit_with_l1_population(self, multi_level_cache, redis_client):
    ...
    async def test_l3_fallback_behavior(self, multi_level_cache, mock_fallback_database):
        """Test L3 fallback to database with cache population."""
        cache = multi_level_cache
        
        # Test L3 fallback
        result = await cache.get('user:123', fallback_func=lambda: mock_fallback_database('user:123'))
...
class TestEnhancedStatistics:
    ...
    async def test_cache_efficiency_metrics(self, multi_level_cache, mock_fallback_database):
        ...
        await cache.get('l3_test', fallback_func=lambda: mock_fallback_database('user:123'))
...
class TestPerformanceCharacteristics:
    ...
    async def test_throughput_characteristics(self, multi_level_cache):
        ...
        logger.info(f"Cache throughput: {throughput:.2f} ops/sec")
    ...
    async def test_cache_warming_performance(self, multi_level_cache, redis_client):
        ...
        if successful_warms > 0:
            ...
            logger.info(f"Cache warming throughput: {warming_throughput:.2f} keys/sec")
...
class TestSystemIntegration:
    ...
    async def test_full_system_workflow(self, multi_level_cache, mock_fallback_database):
        ...
        start_time = time.perf_counter()
        result1 = await cache.get(user_id, fallback_func=lambda: mock_fallback_database(user_id))
    ...
    async def test_high_concurrency_scenario(self, multi_level_cache):
        ...
        stats = cache.get_performance_stats()
        assert stats['total_requests'] >= concurrent_requests * 6  # Set + 5 gets per user
        assert stats['performance_metrics']['error_rate']['overall_error_rate'] == 0
        ...
        logger.info(f"Concurrency test: {operations_per_second:.2f} ops/sec with {concurrent_requests} concurrent users")
...
class TestPerformanceBenchmarking:
    ...
    async def test_baseline_performance_benchmark(self, multi_level_cache):
        ...
        benchmark_results['full_cache_avg_time'] = sum(full_cache_times) / len(full_cache_times)
        benchmark_results['full_cache_p95_time'] = sorted(full_cache_times)[int(0.95 * len(full_cache_times))]
        ...
        assert benchmark_results['full_cache_avg_time'] < 0.01  # 10ms avg for full cache
        assert benchmark_results['full_cache_p95_time'] < 0.05  # 50ms p95 for full cache
    ...
    async def test_throughput_under_load(self, multi_level_cache):
        ...
        stats = cache.get_performance_stats()
        assert stats['performance_metrics']['error_rate']['overall_error_rate'] < 0.01  # Less than 1% errors
        
        logger.info(f"Sustained load throughput: {throughput:.2f} ops/sec over {actual_duration:.2f}s")
    ...
    async def test_memory_efficiency_benchmark(self, multi_level_cache):
        ...
        memory_overhead_ratio = memory_increase / expected_data_size if expected_data_size > 0 else 0
        ...
        logger.info(f"Memory efficiency: {memory_overhead_ratio:.2f}x overhead, {memory_per_item:.0f} bytes/item")
...
async def cleanup_after_test():
    ...
    try:
        ...
        if _global_caches:
            for cache_name in ['rule_cache', 'session_cache', 'analytics_cache', 'prompt_cache']:
