...
async def test_unified_manager(redis_container):
    ...
    # Mock config with test Redis settings
    config = AppConfig()
...
class TestUnifiedCacheMonitoringIntegration:
    ...
    async def test_cache_operation_monitoring(self, monitoring_system, security_context):
        ...
        # Get operation (miss)
        miss_result = await unified_manager.get_cached("nonexistent_key", security_context)
        ...
        # Should have enhanced monitoring data
        ...
        # Should have operation summaries
    ...
    async def test_dependency_tracking_and_invalidation(self, monitoring_system, security_context):
        ...
        root_key = "user:123"
        dependent_keys = ["user:123:profile", "user:123:settings", "user:123:preferences"]
        
        # Create cache entries
        await unified_manager.set_cached(root_key, {"id": 123, "name": "test"}, 300, security_context)
        for dep_key in dependent_keys:
            await unified_manager.set_cached(dep_key, {"user_id": 123, "data": "test"}, 300, security_context)
        ...
        async with cache_monitor.trace_invalidation(InvalidationType.DEPENDENCY, [root_key]):
            invalidated_count = await cache_monitor.invalidate_by_dependency(root_key)
            assert invalidated_count > 0
        
        # Verify invalidation history
    ...
    async def test_cross_level_coordination(self, monitoring_system, security_context):
        ...
        for _ in range(15):  # High frequency access
            await unified_manager.get_cached(hot_key, security_context)
            coordinator.track_cache_access(hot_key, CacheLevel.L1, True, operation="get")
        ...
        if hot_entry and cold_entry:
            # Hot entry should have higher promotion score
    ...
    async def test_slo_integration_and_compliance(self, monitoring_system):
        ...
        # Should calculate multiple SLI types
        expected_sli_types = [CacheSLIType.HIT_RATE]  # Others may not have data yet
        
        for sli_type in expected_sli_types:
            if sli_type in cache_slis:
                sli = cache_slis[sli_type]
                
                # Verify SLI structure
                assert sli.sli_type == sli_type
                assert 0.0 <= sli.current_value <= 1.0 or sli.current_value >= 0  # Hit rate 0-1, latency >0
        ...
        # Should analyze trends (may be empty with limited data)
        assert isinstance(trends, dict)
        
        # Test comprehensive SLO report
    ...
    async def test_predictive_alerting(self, monitoring_system, security_context):
        ...
        # Generate performance data with degrading trend
        test_key = "degrading_performance_key"
        
        # Simulate degrading hit rate over time
        for i in range(25):  # Need minimum data points
            if i < 10:
                # Good hit rate initially
                await unified_manager.set_cached(f"{test_key}_{i}", f"data_{i}", 300, security_context)
                result = await unified_manager.get_cached(f"{test_key}_{i}", security_context)
                assert result is not None
            else:
                # Simulate increasing cache misses
                result = await unified_manager.get_cached(f"nonexistent_{i}", security_context)
                assert result is None
            
            # Record in SLI history manually for testing
            hit = i < 10
            hit_rate = max(0.3, 1.0 - (i * 0.03))  # Decreasing hit rate
        ...
        # May or may not generate alerts depending on trend analysis
        for alert in alerts:
    ...
    async def test_comprehensive_monitoring_report(self, monitoring_system, security_context):
    ...
    async def test_cache_warming_intelligence(self, monitoring_system, security_context):
        """Test intelligent cache warming based on access patterns."""
        components = monitoring_system
        unified_manager = components["unified_manager"]
        cache_monitor = components["cache_monitor"]
        
        # Create access patterns for warming analysis
        warming_candidates = ["user:456", "product:789", "config:settings"]
        
        for key in warming_candidates:
            # Set initial data
            await unified_manager.set_cached(key, f"data_for_{key}", 300, security_context)
            
            # Simulate regular access pattern
            for _ in range(8):  # High enough frequency for warming consideration
                await unified_manager.get_cached(key, security_context)
        ...
        # Should identify warming candidates
    ...
    async def test_coherence_monitoring_and_repair(self, monitoring_system, security_context):
        ...
        # Track entry in coordinator
        coordinator.track_cache_access(test_key, CacheLevel.L1, True, len(test_value), "set")
        coordinator.track_cache_access(test_key, CacheLevel.L2, True, len(test_value), "set")
        
        # Check coherence
        violations = await coordinator.check_cache_coherence([test_key])
        
        # Should not have violations for properly set key
        assert test_key not in violations or len(violations[test_key]) == 0
        
        # Test coherence repair (even if no violations)
    ...
    async def test_alert_callback_integration(self, monitoring_system):
        ...
        # Generate test alert manually
        from prompt_improver.monitoring.cache.unified_cache_monitoring import CachePerformanceAlert
    ...
    async def test_opentelemetry_metrics_integration(self, monitoring_system, security_context):
        ...
        with patch('prompt_improver.monitoring.cache.unified_cache_monitoring.OPENTELEMETRY_AVAILABLE', True), \
             patch('prompt_improver.monitoring.cache.unified_cache_monitoring.cache_operations_counter') as mock_counter, \
             patch('prompt_improver.monitoring.cache.unified_cache_monitoring.cache_hit_ratio') as mock_gauge:
            
            # Perform cache operations
            test_key = "otel_test_key"
            await unified_manager.set_cached(test_key, "test_data", 300, security_context)
            
            # Record operation manually to test metrics
            cache_monitor.record_cache_operation("set", CacheLevel.L1, True, 50.0, test_key)
            
            # Verify OpenTelemetry metrics were called
            # Note: In real test environment, these would be actual metrics
            # Here we verify the code path is exercised
            
            # Get hit ratio should be called
            await unified_manager.get_cached(test_key, security_context)
            cache_monitor.record_cache_operation("get", CacheLevel.L1, True, 25.0, test_key)
...
class TestMonitoringSystemResilience:
    ...
    async def test_monitoring_with_redis_unavailable(self, monitoring_system):
        ...
        original_redis = unified_manager._redis_master
        unified_manager._redis_master = None
        
        try:
            # Should still function with L1 cache only
            test_key = "redis_unavailable_test"
            
            # This should work with L1 cache
            cache_monitor.record_cache_operation("get", CacheLevel.L1, True, 30.0, test_key)
            ...
            unified_manager._redis_master = original_redis
    ...
    async def test_monitoring_with_invalid_data(self, monitoring_system):
        ...
        # Test with invalid cache operation data
        try:
            cache_monitor.record_cache_operation("invalid_op", CacheLevel.L1, True, -100.0, None)
    ...
    async def test_concurrent_monitoring_operations(self, monitoring_system, security_context):
        ...
        async def cache_operation_batch(batch_id):
            for i in range(10):
                ...
                await unified_manager.set_cached(key, f"data_{i}", 300, security_context)
                await unified_manager.get_cached(key, security_context)
                cache_monitor.record_cache_operation("get", CacheLevel.L1, True, 40.0, key)
