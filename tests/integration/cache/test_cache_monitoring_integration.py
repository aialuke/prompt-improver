...
from prompt_improver.database.unified_connection_manager import ManagerMode, SecurityContext, UnifiedConnectionManager
from prompt_improver.monitoring.cache import AlertSeverity, CacheLevel, CacheSLIType, InvalidationType, get_cache_monitoring_report, get_cache_slo_integration, get_cross_level_coordinator, get_unified_cache_monitor, initialize_comprehensive_cache_monitoring
...
async def test_unified_manager(redis_container):
    ...
    manager = UnifiedConnectionManager(mode=ManagerMode.ASYNC_MODERN, config=config)
...
async def security_context():
    ...
    return SecurityContext(agent_id='test_agent', tier='basic', authenticated=True, created_at=time.time())
...
async def monitoring_system(test_unified_manager):
    ...
    return {'unified_manager': test_unified_manager, 'cache_monitor': cache_monitor, 'slo_integration': slo_integration, 'coordinator': coordinator}
...
class TestUnifiedCacheMonitoringIntegration:
    ...
    async def test_predictive_alerting(self, monitoring_system, security_context):
        ...
        for i in range(25):
            ...
            slo_integration._sli_history['hit_rate'].append({'timestamp': time.time() + i, 'value': hit_rate, 'compliance': 1.0 if hit_rate >= 0.85 else hit_rate / 0.85})
        trends = await slo_integration.analyze_performance_trends()
    ...
    async def test_alert_callback_integration(self, monitoring_system):
        ...
        test_alert = CachePerformanceAlert(alert_id='test_alert_123', severity=AlertSeverity.WARNING, metric_name='test_metric', current_value=0.5, threshold_value=0.7, message='Test alert message', cache_level=CacheLevel.L1, recommendations=['Test recommendation'])
    ...
    async def test_opentelemetry_metrics_integration(self, monitoring_system, security_context):
        ...
        unified_manager = components['unified_manager']
        cache_monitor = components['cache_monitor']
        with patch('prompt_improver.monitoring.cache.unified_cache_monitoring.OPENTELEMETRY_AVAILABLE', True), patch('prompt_improver.monitoring.cache.unified_cache_monitoring.cache_operations_counter') as mock_counter, patch('prompt_improver.monitoring.cache.unified_cache_monitoring.cache_hit_ratio') as mock_gauge:
