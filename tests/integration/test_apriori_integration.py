...
from prompt_improver.database.models import AprioriAssociationRule, AprioriPatternDiscovery, PromptSession, RulePerformance, UserFeedback
...
async def sample_data(db_session: AsyncSession):
    ...
    sessions = [PromptSession(session_id=f'session_{i}', original_prompt=f'Test prompt {i}', improved_prompt=f'Improved prompt {i}', quality_score=0.8 + i % 3 * 0.1, improvement_score=0.7 + i % 4 * 0.1, confidence_level=0.8, created_at=datetime.now(UTC) - timedelta(days=i % 30)) for i in range(20)]
    ...
    for i, session in enumerate(sessions):
        ...
        for rule_name in rules_to_apply:
            performance = RulePerformance(rule_id=rule_name, rule_name=rule_name, improvement_score=0.6 + hash(f'{session.session_id}_{rule_name}') % 100 / 250, execution_time_ms=int(50.0 + i % 20 * 5), confidence_level=0.7 + i % 3 * 0.1, rule_parameters={'threshold': 0.5 + i % 3 * 0.1})
            performances.append(performance)
            db_session.add(performance)
    feedbacks = [UserFeedback(session_id=f'session_{i}', rating=3 + i % 3, feedback_text=f'Feedback for session {i}', improvement_areas=['clarity', 'structure'] if i % 2 else ['specificity']) for i in range(0, 20, 2)]
    for feedback in feedbacks:
        db_session.add(feedback)
    await db_session.commit()
    return {'sessions': len(sessions), 'performances': len(performances), 'feedbacks': len(feedbacks)}
...
class TestAprioriAnalyzer:
    ...
    async def test_apriori_analyzer_initialization(self):
        ...
        config = AprioriConfig(min_support=0.1, min_confidence=0.6, min_lift=1.0, max_itemset_length=5)
    ...
    async def test_prompt_characteristics_extraction(self):
        ...
        db_manager = get_unified_manager(ManagerMode.ML_TRAINING)
        analyzer = AprioriAnalyzer(db_manager=db_manager)
        test_cases = [('Short prompt', ['length_short', 'domain_general']), ('Write a detailed function that processes data step by step', ['length_medium', 'complexity_sequential', 'domain_technical']), ('Can you provide an example of a creative story?', ['length_medium', 'pattern_examples', 'domain_creative']), ('Analyze the research data and provide insights', ['length_medium', 'domain_analytical'])]
    ...
    async def test_frequent_itemset_mining(self):
        ...
        db_manager = get_unified_manager(ManagerMode.ML_TRAINING)
        analyzer = AprioriAnalyzer(db_manager=db_manager)
        transactions = [['rule_ClarityRule', 'quality_high', 'domain_technical'], ['rule_ClarityRule', 'quality_high', 'domain_technical'], ['rule_ClarityRule', 'quality_high', 'domain_technical'], ['rule_SpecificityRule', 'quality_medium', 'domain_creative'], ['rule_SpecificityRule', 'quality_medium', 'domain_creative'], ['rule_ClarityRule', 'rule_SpecificityRule', 'quality_high'], ['rule_ClarityRule', 'rule_SpecificityRule', 'quality_high'], ['rule_ClarityRule', 'rule_SpecificityRule', 'quality_high']]
        frequent_itemsets = await analyzer.mine_frequent_itemsets(transactions, min_support=0.3)
    ...
    async def test_association_rule_generation(self):
        ...
        db_manager = get_unified_manager(ManagerMode.ML_TRAINING)
        analyzer = AprioriAnalyzer(db_manager=db_manager)
        mock_itemsets = pd.DataFrame({'support': [0.8, 0.6, 0.5, 0.4], 'itemsets': [frozenset(['rule_ClarityRule']), frozenset(['quality_high']), frozenset(['rule_ClarityRule', 'quality_high']), frozenset(['rule_ClarityRule', 'quality_high', 'domain_technical'])]})
        rules = analyzer.generate_association_rules(mock_itemsets, min_confidence=0.5, min_lift=1.1)
...
class TestMLIntegration:
    ...
    async def test_ml_service_with_apriori(self):
        ...
        async with db_manager.get_async_session() as db_session:
            results = await ml_service.discover_patterns(db_session=db_session, min_effectiveness=0.5, min_support=3, use_advanced_discovery=True, include_apriori=True)
    ...
    async def test_contextualized_patterns(self):
        ...
        async with db_manager.get_async_session() as db_session:
            results = await ml_service.get_contextualized_patterns(context_items=context_items, db_session=db_session, min_confidence=0.5)
...
class TestDatabaseIntegration:
    ...
    async def test_apriori_association_rule_storage(self, db_session: AsyncSession):
        ...
        rule = AprioriAssociationRule(antecedents='["rule_ClarityRule", "domain_technical"]', consequents='["quality_high"]', support=0.75, confidence=0.85, lift=2.1, conviction=3.2, rule_strength=0.88, business_insight='Applying clarity rule to technical prompts leads to high quality', pattern_category='rule_performance', discovery_run_id='test_run_123')
        db_session.add(rule)
        await db_session.commit()
        stmt = select(AprioriAssociationRule).where(AprioriAssociationRule.discovery_run_id == 'test_run_123')
    ...
    async def test_pattern_discovery_metadata_storage(self, db_session: AsyncSession):
        ...
        discovery_run = AprioriPatternDiscovery(discovery_run_id='test_discovery_456', min_support=0.1, min_confidence=0.6, min_lift=1.2, max_itemset_length=4, data_window_days=30, transaction_count=100, frequent_itemsets_count=25, association_rules_count=15, execution_time_seconds=12.5, top_patterns_summary=[{'itemset': ['rule_ClarityRule'], 'support': 0.8}, {'itemset': ['quality_high'], 'support': 0.6}], pattern_insights={'rule_performance_patterns': ['ClarityRule leads to high quality'], 'quality_improvement_patterns': ['Technical domain + clarity → high quality']}, status='completed')
        db_session.add(discovery_run)
        await db_session.commit()
        stmt = select(AprioriPatternDiscovery).where(AprioriPatternDiscovery.discovery_run_id == 'test_discovery_456')
        result = await db_session.execute(stmt)
...
class TestAdvancedPatternDiscovery:
    ...
    async def test_comprehensive_pattern_discovery(self, db_session: AsyncSession):
        ...
        db_manager = get_unified_manager(ManagerMode.ML_TRAINING)
        discovery_service = AdvancedPatternDiscovery(db_manager=db_manager)
        results = await discovery_service.discover_advanced_patterns(db_session=db_session, min_effectiveness=0.5, min_support=3, pattern_types=['parameter', 'apriori'], use_ensemble=True, include_apriori=True)
        assert 'discovery_metadata' in results
...
class TestEndToEndWorkflow:
    ...
    async def test_complete_apriori_workflow(self, db_session: AsyncSession):
        ...
        db_manager = get_unified_manager(ManagerMode.ML_TRAINING)
        ml_service = MLModelService(db_manager=db_manager)
        discovery_results = await ml_service.discover_patterns(db_session=db_session, min_effectiveness=0.5, min_support=3, use_advanced_discovery=True, include_apriori=True)
        ...
        contextual_results = await ml_service.get_contextualized_patterns(context_items=context_items, db_session=db_session, min_confidence=0.5)
    ...
    async def test_performance_benchmarks(self):
        ...
        results = await analyzer.analyze_patterns(window_days=30, save_to_database=False)
