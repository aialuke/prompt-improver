...
class TestBackgroundTaskManagerEnhanced:
    ...
    def test_concurrent_task_execution_performance(self, benchmark, event_loop):
        ...
        async def benchmark_concurrent_execution():
            ...
            for i in range(100):
                task_id = await manager.submit_task(
                    f"concurrent_{i}", cpu_bound_task, multiplier=i
                )
                task_ids.append(task_id)
            completed_tasks = 0
            while completed_tasks < 100:
                ...
                completed_tasks = sum(
                    ...
                    if manager.get_task_status(task_id)
                    and manager.get_task_status(task_id).status == TaskStatus.COMPLETED
                )
        ...
        assert execution_time < 2000, (
            f"Concurrent execution too slow: {execution_time}ms"
        )
    ...
    def test_manager_lifecycle_performance(self, benchmark, event_loop):
        ...
        assert lifecycle_time < 1000, (
            f"Lifecycle performance too slow: {lifecycle_time}ms"
        )

    @pytest.mark.asyncio
    async def test_concurrent_task_access_thread_safety(
        self, clean_manager, task_factory
    ):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_task_cancellation_thread_safety(
        self, clean_manager, task_factory
    ):
        ...
        for i in range(10):
            task_id = await clean_manager.submit_task(
                f"long_task_{i}", task_factory, duration=2.0, task_name=f"long_task_{i}"
            )
            task_ids.append(task_id)
        await asyncio.sleep(0.1)
        ...
        cancel_results = await asyncio.gather(
            cancel_tasks(group1), cancel_tasks(group2), return_exceptions=True
        )
    ...
    async def test_shared_state_thread_safety(self, clean_manager):
        ...
        for i in range(50):
            task_id = await clean_manager.submit_task(
                f"shared_access_{i}", increment_counter, index=str(i)
            )
            task_ids.append(task_id)
    ...
    @pytest.mark.asyncio
    async def test_complete_task_lifecycle_validation(
        self, clean_manager, task_factory
    ):
        """Test complete task lifecycle with state transitions and timing validation."""
        task_id = await clean_manager.submit_task(
            "lifecycle_test", task_factory, duration=0.2
        )
        task = clean_manager.get_task_status(task_id)
    ...
    async def test_failed_task_lifecycle_validation(self, clean_manager, task_factory):
        ...
        task_id = await clean_manager.submit_task(
            "failed_task", task_factory, should_fail=True
        )
        await asyncio.sleep(0.2)
        task = clean_manager.get_task_status(task_id)
    ...
    @pytest.mark.asyncio
    async def test_cancelled_task_lifecycle_validation(
        self, clean_manager, task_factory
    ):
        """Test cancelled task lifecycle with proper cleanup."""
        task_id = await clean_manager.submit_task(
            "cancelled_task", task_factory, duration=2.0
        )
        await asyncio.sleep(0.1)
    ...
    async def test_manager_lifecycle_with_active_tasks(self, task_factory):
        ...
        for i in range(10):
            task_id = await manager.submit_task(
                f"lifecycle_{i}", task_factory, duration=0.5
            )
            task_ids.append(task_id)
        await asyncio.sleep(0.1)
        running_count = sum(
    ...
    async def test_resource_cleanup_during_lifecycle(self, clean_manager):
        ...
        for i in range(5):
            task_id = await clean_manager.submit_task(
                f"resource_{i}", resource_task, resource_id=str(i)
            )
            task_ids.append(task_id)
        await asyncio.sleep(0.5)
    ...
    async def test_network_failure_scenario(self, clean_manager):
        ...
        for i in range(10):
            ...
            if should_fail:
                expected_failures += 1
            task_id = await clean_manager.submit_task(
                f"network_{i}", network_task, should_fail=should_fail
            )
            task_ids.append(task_id)
        await asyncio.sleep(0.5)
        successful_tasks = 0
    ...
    async def test_resource_exhaustion_scenario(self, clean_manager):
        ...
        for i in range(3):
            task_id = await limited_manager.submit_task(
                f"resource_{i}", resource_heavy_task, index=str(i)
            )
            task_ids.append(task_id)
        await asyncio.sleep(0.1)
        rejected_count = 0
    ...
    async def test_memory_pressure_scenario(self, clean_manager):
        ...
        for i in range(5):
            data_size = 10000 * (i + 1)
            task_id = await clean_manager.submit_task(
                f"memory_{i}", memory_intensive_task, data_size=data_size
            )
            task_ids.append(task_id)
        await asyncio.sleep(0.5)
        for task_id in task_ids:
    ...
    async def test_cascading_failure_scenario(self, clean_manager):
        ...
        async def cascading_task(index: str, should_trigger_cascade: bool = False):
            ...
            if should_trigger_cascade:
                ...
                if failure_count["count"] <= 3:
                    raise RuntimeError(
                        f"Cascading failure #{failure_count['count']} in task {index}"
                    )
            await asyncio.sleep(0.1)
        ...
        for i in range(8):
            ...
            task_id = await clean_manager.submit_task(
                ...
                cascading_task,
                index=str(i),
                should_trigger_cascade=should_trigger,
            )
            task_ids.append(task_id)
        await asyncio.sleep(0.5)
        failed_tasks = 0
