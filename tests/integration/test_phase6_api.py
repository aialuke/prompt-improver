...
async def test_api_endpoints():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(f"{base_url}/orchestrator/status") as response:
                print(f"\n✅ GET /orchestrator/status - Status: {response.status}")
        ...
        try:
            async with session.get(f"{base_url}/orchestrator/components") as response:
                print(f"\n✅ GET /orchestrator/components - Status: {response.status}")
                if response.status == 200:
                    data = await response.json()
        ...
        try:
            async with session.get(f"{base_url}/orchestrator/history?limit=10") as response:
                print(f"\n✅ GET /orchestrator/history - Status: {response.status}")
                if response.status == 200:
                    data = await response.json()
