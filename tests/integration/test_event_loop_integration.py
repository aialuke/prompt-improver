...
from prompt_improver.utils.session_event_loop import SessionEventLoopWrapper, get_session_manager
...
async def test_session_concurrent_tasks(session_manager):
    ...
    tasks = [
        wrapper.create_task(asyncio.sleep(0.1)) for _ in range(5)
    ]
...
async def test_benchmark_utilities():
    ...
    assert "comprehensive_benchmark" in results, "Missing comprehensive benchmark results"
    ...
    assert results["comprehensive_benchmark"]["meets_target"], "Comprehensive benchmark should meet target latency"
    assert results["prompt_simulation"]["meets_target"], "Prompt simulation should meet target latency"
    assert results["session_benchmark"]["meets_target"], "Session benchmark should meet target latency"
...
def test_session_wrapper_benchmark(benchmark, session_manager):
    ...
    def create_and_use_session():
        ...
        wrapper = session_manager.get_session_wrapper(f"benchmark_session_{id(wrapper)}")
