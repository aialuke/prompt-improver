...
class TestKeyDerivation:
    ...
    def test_pbkdf2_with_custom_parameters(self):
        ...
        key, salt = crypto_manager.derive_key(
            password, salt=custom_salt, length=16, iterations=100000
        )
        assert len(key) == 16
    ...
    def test_scrypt_with_custom_parameters(self):
        ...
        key, salt = crypto_manager.derive_key_scrypt(
            password, length=16, n=2**14, r=8, p=1
        )
        assert len(key) == 16
        assert len(salt) == 32
...
class TestPerformanceAndMetrics:
    ...
    async def test_concurrent_operations(self):
        """Test crypto manager under concurrent async load."""
        import asyncio
        ...
        tasks = [asyncio.create_task(worker(worker_id)) for worker_id in range(3)]
