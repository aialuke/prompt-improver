...
import psutil
import pytest
from src.prompt_improver.database.unified_connection_manager import ManagerMode, SecurityContext, create_security_context, get_unified_manager
from src.prompt_improver.security.authorization import AuthorizationService, Permission, Role
from src.prompt_improver.security.key_manager import AuditEvent, SecurityLevel, UnifiedKeyManager, get_unified_key_manager
from src.prompt_improver.security.unified_rate_limiter import RateLimitExceeded, RateLimitResult, RateLimitStatus, RateLimitTier, UnifiedRateLimiter, get_unified_rate_limiter
from src.prompt_improver.security.unified_security_manager import SecurityConfiguration, SecurityMode, SecurityOperationType, SecurityThreatLevel, UnifiedSecurityManager, create_security_test_adapter, get_unified_security_manager
...
class SecurityConsolidationValidator:
    ...
    async def validate_security_manager_modes(self, unified_connection_manager) -> dict[str, Any]:
        ...
        for mode in SecurityMode:
            try:
                ...
                results[mode.value] = {'initialized': True, 'initialization_time': initialization_time, 'status': status, 'security_level': status['security_level']}
        ...
        results['summary'] = {'modes_tested': modes_tested, 'successful_initializations': successful_initializations, 'success_rate': success_rate}
        assert success_rate >= 1.0, f'Security manager mode initialization success rate {success_rate:.1%} below target (100%)'
        return results

    async def validate_authentication_flows(self, unified_security_manager, test_count: int=10000) -> dict[str, Any]:
        ...
        for i in range(test_count // 2):
            ...
            try:
                success, security_context = await unified_security_manager.authenticate_agent(agent_id=agent_id, credentials=credentials, additional_context={'test_scenario': 'valid_auth'})
        ...
        for i in range(test_count // 2):
            ...
            auth_start = time.perf_counter()
            try:
                success, security_context = await unified_security_manager.authenticate_agent(agent_id=agent_id, credentials=credentials, additional_context={'test_scenario': 'invalid_auth'})
                auth_time = time.perf_counter() - auth_start
                timing_measurements.append(auth_time)
                if not success:
        ...
        results = {'total_operations': total_operations, 'successful_authentications': successful_auths, 'failed_authentications': failed_auths, 'blocked_agents': blocked_agents, 'average_auth_time_ms': avg_auth_time * 1000, 'auth_ops_per_second': auth_ops_per_second, 'total_time_seconds': total_time, 'security_status': security_status}
        ...
        assert avg_auth_time < 0.01, f'Authentication time {avg_auth_time * 1000:.2f}ms exceeds target (10ms)'
        assert auth_ops_per_second >= 100, f'Authentication throughput {auth_ops_per_second:.1f} below target (100 ops/s)'
        return results

    async def validate_rate_limiting_performance(self, unified_rate_limiter, test_count: int=5000) -> dict[str, Any]:
        ...
        for tier in tiers:
            ...
            for i in range(test_count // len(tiers) // 2):
                ...
                try:
                    status = await unified_rate_limiter.check_rate_limit(agent_id=agent_id, tier=tier, authenticated=True)
            ...
            for i in range(burst_requests):
                check_start = time.perf_counter()
                try:
                    status = await unified_rate_limiter.check_rate_limit(agent_id=burst_agent, tier=tier, authenticated=True)
                    check_time = time.perf_counter() - check_start
                    timing_measurements.append(check_time)
                    if status.result == RateLimitResult.ALLOWED:
                        allowed_requests += 1
        ...
        results = {'total_operations': total_operations, 'allowed_requests': allowed_requests, 'rate_limited_requests': rate_limited_requests, 'authentication_required': authentication_required, 'errors': errors, 'average_check_time_ms': avg_check_time * 1000, 'rate_limit_ops_per_second': rate_limit_ops_per_second, 'total_time_seconds': total_time}
    ...
    async def validate_key_manager_operations(self, unified_key_manager, test_count: int=1000) -> dict[str, Any]:
        ...
        test_data_samples = [b'small data', b'medium sized data that is longer than the small sample' * 10, b'large data sample' * 100, json.dumps({'complex': 'data', 'with': {'nested': 'structures'}}).encode(), secrets.token_bytes(1024)]
        ...
        results = {'total_operations': total_operations, 'successful_operations': successful_operations, 'failed_operations': failed_operations, 'keys_generated': len(generated_keys), 'avg_key_generation_time_ms': avg_key_gen_time * 1000, 'avg_encryption_time_ms': avg_encryption_time * 1000, 'avg_decryption_time_ms': avg_decryption_time * 1000, 'crypto_ops_per_second': crypto_ops_per_second, 'total_time_seconds': total_time, 'key_manager_status': key_status}
    ...
    async def validate_security_audit_logging(self, unified_security_manager) -> dict[str, Any]:
        ...
        for threat_level in threat_levels:
            for operation_type in operation_types:
                ...
                incident_id = await test_adapter.simulate_security_incident(threat_level=threat_level, agent_id=agent_id)
                if incident_id != 'no_incident_created':
                    incidents_created += 1
        all_incidents = await unified_security_manager.get_security_incidents(limit=100)
        recent_incidents = await unified_security_manager.get_security_incidents(limit=50, threat_level=SecurityThreatLevel.HIGH)
        security_status = await unified_security_manager.get_security_status()
        results = {'incidents_created': incidents_created, 'total_incidents_recorded': len(all_incidents), 'high_threat_incidents': len(recent_incidents), 'active_incidents': security_status['metrics'].get('active_incidents', 0), 'resolved_incidents': security_status['metrics'].get('resolved_incidents', 0), 'security_violations': security_status['metrics'].get('security_violations', 0), 'audit_status': security_status}
    ...
    async def validate_fail_secure_policies(self, unified_security_manager, unified_rate_limiter) -> dict[str, Any]:
        """Validate fail-secure policies under error conditions."""
        print('\n🛡️  Testing Fail-Secure Policy Enforcement')
        fail_secure_tests = 0
        fail_secure_successes = 0
        try:
            success, context = await unified_security_manager.authenticate_agent(agent_id=None, credentials={'corrupted': 'data'}, additional_context={'test': 'fail_secure_auth'})
            fail_secure_tests += 1
        ...
        try:
            status = await unified_rate_limiter.check_rate_limit(agent_id='test_agent', tier='basic', authenticated=False)
            fail_secure_tests += 1
        ...
        try:
            test_context = await create_security_context(agent_id='test_agent', tier='basic', authenticated=True)
            test_context.created_at = time.time() - 7200
            is_authorized = await unified_security_manager.authorize_operation(security_context=test_context, operation='test_operation', resource='test_resource')
            fail_secure_tests += 1
        ...
        results = {'fail_secure_tests': fail_secure_tests, 'fail_secure_successes': fail_secure_successes, 'fail_secure_rate': fail_secure_rate, 'security_vulnerabilities': fail_secure_tests - fail_secure_successes}
    ...
    async def validate_timing_attack_resistance(self, unified_security_manager, sample_size: int=1000) -> dict[str, Any]:
        ...
        for i in range(sample_size // 2):
            ...
            start_time = time.perf_counter()
            try:
                success, context = await unified_security_manager.authenticate_agent(agent_id=agent_id, credentials=credentials)
        ...
        results = {'valid_auth_samples': len(valid_auth_times), 'invalid_auth_samples': len(invalid_auth_times), 'valid_avg_time_ms': valid_avg * 1000, 'invalid_avg_time_ms': invalid_avg * 1000, 'valid_std_dev_ms': valid_std * 1000, 'invalid_std_dev_ms': invalid_std * 1000, 'timing_difference_ms': timing_difference * 1000, 'timing_difference_ratio': timing_difference_ratio, 'timing_attack_resistant': timing_difference_ratio < 0.1}
...
class TestSecurityConsolidation:
    ...
    async def test_authentication_performance(self, unified_security_manager):
        ...
        validator = SecurityConsolidationValidator()
        results = await validator.validate_authentication_flows(unified_security_manager, test_count=10000)
    ...
    async def test_rate_limiting_performance(self, unified_rate_limiter):
        ...
        validator = SecurityConsolidationValidator()
        results = await validator.validate_rate_limiting_performance(unified_rate_limiter, test_count=5000)
    ...
    async def test_key_manager_operations(self, unified_key_manager):
        ...
        validator = SecurityConsolidationValidator()
        results = await validator.validate_key_manager_operations(unified_key_manager, test_count=1000)
    ...
    async def test_fail_secure_policies(self, unified_security_manager, unified_rate_limiter):
        ...
        validator = SecurityConsolidationValidator()
        results = await validator.validate_fail_secure_policies(unified_security_manager, unified_rate_limiter)
    ...
    async def test_timing_attack_resistance(self, unified_security_manager):
        ...
        validator = SecurityConsolidationValidator()
        results = await validator.validate_timing_attack_resistance(unified_security_manager, sample_size=1000)
    ...
    @pytest.mark.asyncio
    async def test_overall_security_consolidation_improvement(self, unified_connection_manager, unified_security_manager, unified_rate_limiter, unified_key_manager):
        ...
        start_time = time.perf_counter()
        total_operations = 0
        auth_results = await validator.validate_authentication_flows(unified_security_manager, test_count=1000)
        total_operations += auth_results['total_operations']
        rate_results = await validator.validate_rate_limiting_performance(unified_rate_limiter, test_count=2000)
        total_operations += rate_results['total_operations']
        crypto_results = await validator.validate_key_manager_operations(unified_key_manager, test_count=500)
        ...
        crypto_weight = 0.3
        weighted_improvement = auth_results['auth_ops_per_second'] / 100 * auth_weight + rate_results['rate_limit_ops_per_second'] / 500 * rate_limit_weight + crypto_results['crypto_ops_per_second'] / 200 * crypto_weight
        results = {'total_operations': total_operations, 'overall_ops_per_second': overall_ops_per_second, 'total_time_seconds': total_time, 'memory_delta_mb': memory_delta, 'improvement_factor': improvement_factor, 'weighted_improvement': weighted_improvement, 'performance_targets_met': {'authentication': auth_results['average_auth_time_ms'] < 10.0, 'rate_limiting': rate_results['average_check_time_ms'] < 2.0, 'cryptography': crypto_results['avg_encryption_time_ms'] < 1.0}}
