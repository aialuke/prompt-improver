...
from prompt_improver.database import create_security_context
...
class TestAuthenticationService:
    ...
    async def test_session_management(self, authentication_service):
        ...
        success, _security_context = await authentication_service.authenticate_agent(
            agent_id, credentials
        )
        assert success is True
        ...
        _session_valid, _ = await authentication_service.validate_session("mock_session_token")
...
class TestSecurityServiceFacade:
    ...
    async def test_end_to_end_security_workflow(self, security_facade):
        ...
        if auth_success:
            ...
            is_valid, _validation_results = await security_facade.validate_input(
    ...
    async def test_facade_error_handling_and_fail_secure(self, security_facade):
        ...
        _is_valid, _results = await security_facade.validate_input(
