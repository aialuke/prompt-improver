...
class TestDatabaseOptimizationPhase2:
    ...
    async def cache_layer(self):
        ...
        policy = CachePolicy(ttl_seconds=60, strategy=CacheStrategy.AGGRESSIVE, warm_on_startup=False)
    ...
    async def query_executor(self):
        ...
        executor._cache_stats = {'hits': 0, 'misses': 0, 'cache_time_saved_ms': 0.0, 'total_cached_queries': 0}
    ...
    async def test_cache_with_different_parameters(self, cache_layer):
        ...
        async def execute_query(q, p):
            return await client.fetch_raw(q, p)
        result1, cached1 = await cache_layer.get_or_execute(query, {'user_id': 'user1'}, execute_query)
        result2, cached2 = await cache_layer.get_or_execute(query, {'user_id': 'user2'}, execute_query)
        result3, cached3 = await cache_layer.get_or_execute(query, {'user_id': 'user1'}, execute_query)
        assert not cached1
        assert not cached2
        assert cached3
        assert cache_layer._stats['hits'] == 1
    ...
    async def test_optimized_query_executor_with_caching(self, query_executor):
        ...
        async with get_session_context() as session:
            ...
            start1 = time.perf_counter()
            async with query_executor.execute_optimized_query(session, query, {}, cache_ttl=60, enable_cache=True) as result1:
                time1 = (time.perf_counter() - start1) * 1000
                assert not result1['cache_hit']
                rows1 = result1['result']
            start2 = time.perf_counter()
            async with query_executor.execute_optimized_query(session, query, {}, cache_ttl=60, enable_cache=True) as result2:
                time2 = (time.perf_counter() - start2) * 1000
    ...
    async def test_combined_optimization_effect(self, cache_layer, pool_optimizer, query_executor):
        ...
        cache_layer._stats = {'hits': 0, 'misses': 0, 'invalidations': 0, 'bytes_saved': 0, 'queries_cached': 0, 'db_calls_avoided': 0}
        queries = [('SELECT * FROM rules WHERE active = true', {}), ("SELECT * FROM sessions WHERE created_at > NOW() - INTERVAL '1 hour'", {}), ('SELECT COUNT(*) FROM prompt_improvements', {}), ('SELECT id, name FROM rules ORDER BY created_at DESC LIMIT 10', {})]
        total_executions = 0
        cache_hits = 0
        async with get_session_context() as session:
            for _ in range(3):
                for query, params in queries:
                    async with query_executor.execute_optimized_query(session, query, params, cache_ttl=120, enable_cache=True) as result:
                        total_executions += 1
                        if result['cache_hit']:
                            cache_hits += 1
    ...
    async def test_cache_performance_under_load(self, cache_layer):
        ...
        async def access_cache(worker_id: int):
            ...
            for i in range(10):
                ...
                results.append({'worker': worker_id, 'iteration': i, 'cached': was_cached, 'duration_ms': duration})
            return results
        workers = 5
    ...
    async def test_smart_cache_strategy(self, cache_layer):
        ...
        async def execute_query(q, p):
            ...
            return await client.fetch_raw(q, p)
        test_queries = [('SELECT id FROM rules LIMIT 1', {}, 'simple'), ('SELECT COUNT(*) FROM sessions', {}, 'aggregate'), ('SELECT r.*, s.* FROM rules r JOIN sessions s ON r.id = s.rule_id LIMIT 10', {}, 'complex'), ('SELECT id FROM rules LIMIT 1', {}, 'simple'), ('SELECT COUNT(*) FROM sessions', {}, 'aggregate')]
        results = []
        for query, params, query_type in test_queries:
            start = time.perf_counter()
            result, was_cached = await cache_layer.get_or_execute(query, params, execute_query)
            duration = (time.perf_counter() - start) * 1000
            results.append({'type': query_type, 'cached': was_cached, 'duration_ms': duration})
        stats = await cache_layer.get_cache_stats()
