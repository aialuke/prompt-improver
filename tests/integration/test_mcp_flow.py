...
async def mcp_server_process() -> AsyncGenerator[subprocess.Popen, None]:
    ...
    process = subprocess.Popen(
        ...
        universal_newlines=False,  # Use bytes mode for better control
...
class TestMCPFlow:
    ...
    @patch("prompt_improver.mcp_server.mcp_server.batch_processor")
    async def test_training_data_persistence(self, mock_batch_processor):
        """Test training data is persisted to storage stub."""
        # Mock the batch processor to capture enqueued data
        mock_batch_processor.enqueue = AsyncMock()
        mock_batch_processor.get_queue_size.return_value = 0
        ...
        async with mcp_server_process() as process:
            ...
            # Verify data was enqueued for persistence
            assert mock_batch_processor.enqueue.called
            call_args = mock_batch_processor.enqueue.call_args
            
            # Check the enqueued record structure
            record = call_args[0][0]  # First positional argument
            priority = call_args[0][1]  # Second positional argument
            
            assert record["original"] == "Test prompt for persistence"
            assert record["data_source"] == "real"
            assert record["session_id"] == "persistence_test"
            assert priority == 100  # Real data priority
