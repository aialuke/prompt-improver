...
class MCPClient:
    ...
    async def send_request(self, method: str, params: dict[str, Any] | None=None) -> dict[str, Any]:
        ...
        while time.time() - start_time < timeout:
            ...
            try:
                line = await asyncio.wait_for(asyncio.get_event_loop().run_in_executor(None, self.process.stdout.readline), timeout=0.1)
    ...
    async def initialize(self) -> dict[str, Any]:
        ...
        return await self.send_request('initialize', {'protocolVersion': '2024-11-05', 'capabilities': {'resources': {}, 'tools': {}, 'prompts': {}, 'experimental': {}}, 'clientInfo': {'name': 'test-client', 'version': '1.0.0'}})
...
async def mcp_server_process() -> AsyncGenerator[subprocess.Popen, None]:
    ...
    process = subprocess.Popen([python_path, server_script], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1, universal_newlines=True, cwd='.')
    try:
...
class TestMCPFlow:
    ...
    async def test_improve_prompt_response_time(self):
        ...
        async with mcp_server_process() as process:
            ...
            start_time = time.time()
            response = await client.send_request('tools/call', {'name': 'improve_prompt', 'arguments': {'prompt': 'Help me write better code', 'context': {'domain': 'software_engineering'}, 'session_id': 'integration_test'}})
            response_time = (time.time() - start_time) * 1000
            assert response_time < 200, f'Response time {response_time:.1f}ms exceeds 200ms limit'
            assert 'result' in response
            result = response['result']
            assert 'content' in result
            assert len(result['content']) > 0
            content_text = str(result['content'])
            assert 'processing_time_ms' in content_text or 'improved_prompt' in content_text
    ...
    async def test_training_data_persistence(self):
        ...
        async with mcp_server_process() as process:
            client = MCPClient(process)
            await client.initialize()
            response = await client.call_tool('improve_prompt', {'prompt': 'Test prompt for persistence', 'context': {'domain': 'testing'}, 'session_id': 'persistence_test'})
            assert 'result' in response
            result = response['result']
            assert 'content' in result
            assert len(result['content']) > 0
            ...
            async with get_session() as db_session:
                ...
                trends = await analytics_service.get_performance_trends(db_session=db_session, days=1)
                ...
                performance_summary = await analytics_service.get_performance_summary(days=1, db_session=db_session)
    ...
    async def test_health_endpoints_operational(self):
        ...
        async with mcp_server_process() as process:
            ...
            assert live_data['status'] == 'live', f"Expected 'live', got {live_data['status']}"
    ...
    async def test_store_prompt_tool_functionality(self):
        ...
        async with mcp_server_process() as process:
            ...
            await client.initialize()
            start_time = time.time()
            response = await client.call_tool('store_prompt', {'original': 'Original test prompt', 'enhanced': 'Enhanced test prompt with improvements', 'metrics': {'improvement_score': 0.85, 'clarity_gain': 0.3}, 'session_id': 'store_test'})
            response_time = (time.time() - start_time) * 1000
            assert response_time < 200, f'Store response time {response_time:.1f}ms exceeds 200ms limit'
            assert 'result' in response
            result = response['result']
            assert 'content' in result
            content_text = str(result['content'])
    ...
    async def test_concurrent_requests_performance(self):
        ...
        async with mcp_server_process() as process:
            ...
            async def make_request(request_id: int) -> float:
                start_time = time.time()
                response = await client.call_tool('improve_prompt', {'prompt': f'Concurrent test prompt {request_id}', 'context': {'domain': 'performance_testing'}, 'session_id': f'concurrent_test_{request_id}'})
                response_time = (time.time() - start_time) * 1000
                assert 'result' in response
            ...
            for i, response_time in enumerate(response_times):
                assert response_time < 200, f'Request {i} took {response_time:.1f}ms, exceeds 200ms limit'
            avg_response_time = sum(response_times) / len(response_times)
            assert avg_response_time < 150, f'Average response time {avg_response_time:.1f}ms too high'
