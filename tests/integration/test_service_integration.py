...
class TestPromptServiceIntegration:
    ...
    async def test_service_database_integration(self, test_db_session):
        ...
        result = await service.improve_prompt(prompt='Test prompt for integration testing', user_context={'domain': 'testing', 'test_run': True}, session_id='integration_test_session', db_session=test_db_session)
    ...
    async def test_rule_effectiveness_calculation(self, test_db_session, sample_prompt_sessions, sample_rule_metadata, sample_rule_performance):
        ...
        effectiveness = await analytics.get_rule_effectiveness(db_session=test_db_session)
    ...
    async def test_performance_monitoring_integration(self, test_db_session, sample_prompt_sessions, sample_rule_metadata):
        ...
        perf_record = RulePerformance(rule_id=sample_rule_metadata[0].rule_id, rule_name=sample_rule_metadata[0].rule_name, improvement_score=0.85, confidence_level=0.9, execution_time_ms=10, prompt_characteristics={'length': 25, 'complexity': 0.7}, before_metrics={'clarity': 0.6, 'specificity': 0.5}, after_metrics={'clarity': 0.8, 'specificity': 0.7}, user_satisfaction_score=0.9, session_id=sample_prompt_sessions[0].session_id, created_at=start_time)
...
class TestMLServiceIntegration:
    ...
    async def test_pattern_discovery_integration(self, test_db_session, sample_prompt_sessions, sample_rule_metadata, sample_rule_performance):
        ...
        result = await ml_service.discover_patterns(test_db_session, min_effectiveness=0.1, min_support=1, use_advanced_discovery=False)
...
class TestPerformanceIntegration:
    ...
    async def test_database_query_performance(self, test_db_session, sample_prompt_sessions, sample_rule_metadata, sample_rule_performance):
        ...
        assert query_time < 1000, f'Database queries took {query_time}ms, should be <1000ms for real behavior'
        assert isinstance(summary, dict)
        assert isinstance(effectiveness, list)
    ...
    async def test_concurrent_operations_performance(self, test_db_session):
        ...
        start_time = asyncio.get_event_loop().time()
        for i in range(3):
            result = await service.improve_prompt(prompt=f'Test prompt {i} for performance testing', user_context={'domain': 'performance_test', 'test_run': True}, session_id=f'perf_test_{i}', db_session=None)
            results.append(result)
        end_time = asyncio.get_event_loop().time()
        total_time = (end_time - start_time) * 1000
        avg_time_per_operation = total_time / len(results)
        assert len(results) == 3
        assert total_time < 5000, f'Sequential operations took {total_time}ms, should be <5000ms for real behavior'
        assert avg_time_per_operation < 2000, f'Average operation time {avg_time_per_operation}ms too high for real behavior'
...
class TestErrorHandlingIntegration:
    ...
    async def test_service_error_propagation(self, test_db_session):
        ...
        result = await service.improve_prompt(prompt='', user_context={}, session_id='error_test', db_session=test_db_session)
        if 'error' in result:
    ...
    async def test_timeout_handling(self, test_db_session):
        ...
        import asyncio
        try:
            result = await asyncio.wait_for(ml_service.discover_patterns(test_db_session, min_effectiveness=0.9, min_support=100, use_advanced_discovery=False), timeout=5.0)
            assert 'status' in result
