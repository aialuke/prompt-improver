...
class TestPromptServiceIntegration:
    ...
    async def test_service_database_integration(self, test_db_session, sample_rule_metadata, sample_rule_performance):
    ...
    async def test_rule_effectiveness_calculation(self, test_db_session, sample_rule_performance):
...
class TestMLServiceIntegration:
    ...
    async def test_pattern_discovery_integration(self, test_db_session, sample_rule_performance):
...
class TestPerformanceIntegration:
    ...
    async def test_database_query_performance(self, test_db_session, sample_rule_performance):
        ...
        assert query_time < 100, f"Database queries took {query_time}ms, should be <100ms"
    ...
    async def test_concurrent_operations_performance(self, test_db_session):
        ...
        with patch.object(service, '_apply_rule_improvements') as mock_apply:
            ...
            assert total_time < 500, f"Concurrent operations took {total_time}ms, should be <500ms"
            assert avg_time_per_operation < 100, f"Average operation time {avg_time_per_operation}ms too high"
...
class TestErrorHandlingIntegration:
    ...
    async def test_service_error_propagation(self, test_db_session):
        ...
        with patch.object(service, '_apply_rule_improvements', side_effect=Exception("Test error")):
