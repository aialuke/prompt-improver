...
class TestCLICommandPaths:
    ...
    @pytest.mark.parametrize(
        "command,args,expected_patterns",
        [
            ...
            (
                "data-stats",
                ["--real-vs-synthetic"],
                ["DATA STATISTICS", "real", "synthetic"],
            ),
            ...
            ("canary-adjust", [], ["{", "canary"]),
            ("ml-status", ["--detailed"], ["Phase 3 ML System Status", "detailed"]),
            ("ml-status", ["--no-models"], ["Phase 3 ML System Status", "models"]),
            (
                "ml-status",
                ["--no-experiments"],
                ["Phase 3 ML System Status", "experiments"],
            ),
        ],
    )
    def test_cli_command_paths_real_behavior(
        self, command, args, expected_patterns, monkeypatch
    ):
        ...
        if result.exit_code == 0:
            ...
            assert any(
                pattern.lower() in output_lower for pattern in expected_patterns
            ), f"None of {expected_patterns} found in output: {result.stdout}"

    @pytest.mark.parametrize(
        "command,args,error_type",
        [
            ("start", ["--mcp-port", "invalid"], "invalid port"),
            ("start", ["--mcp-port", "70000"], "port out of range"),
    ...
    def test_cli_command_error_paths_real_behavior(
        self, command, args, error_type, monkeypatch
    ):
        ...
        if result.exit_code == 0:
            ...
            assert len(total_output) > 0, (
                f"Successful command should produce output for {error_type}"
            )
        else:
    ...
    def test_cli_background_modes_real_behavior(
        self, command, background_args, monkeypatch
    ):
        ...
        if result.exit_code == 0:
            ...
            assert any(
                indicator in output_lower for indicator in background_indicators
    ...
    @pytest.mark.parametrize(
        "command,timeout_args",
        [("stop", ["--timeout", "10"]), ("service-stop", ["--timeout", "10"])],
    )
    def test_cli_timeout_handling_real_behavior(
        self, command, timeout_args, monkeypatch
    ):
    ...
    def test_cli_database_connection_errors_real_behavior(self, monkeypatch):
        ...
        monkeypatch.setenv("HOME", str(self.home_dir))
        # Test database connection error handling by using invalid database config
    ...
    @pytest.mark.parametrize(
        "command,json_args",
        [("status", ["--json"]), ("canary-status", []), ("canary-adjust", [])],
    )
    ...
    async def test_cli_concurrent_execution_real_behavior(self):
        """Test CLI behavior under real concurrent async execution scenarios."""
        import asyncio
        ...
        tasks = []
        for _ in range(3):
            task = asyncio.create_task(run_safe_command())
            tasks.append(task)
        ...
        successful_results = [
            r for r in results if r[0] == "success" and r[1].exit_code == 0
        ]
    ...
    def test_cli_performance_monitoring_real_behavior(self, monkeypatch):
        ...
        assert execution_time < 10.0, (
            f"Help command should complete in <10s, took {execution_time:.2f}s"
        )
        start_time = time.time()
        # Test actual CLI performance with real database
        result = self.runner.invoke(app, ["status"])
        end_time = time.time()
        execution_time = end_time - start_time
