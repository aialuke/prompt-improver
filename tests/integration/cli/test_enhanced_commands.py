...
class TestEnhancedTrainCommand:
    ...
    async def test_train_command_with_real_data_priority(
        ...
        with (
            ...
            priority_mentioned = any(
                "priority" in call.lower() for call in console_calls
            )
    ...
    async def test_train_command_verbose_mode(
        ...
        with (
            patch("prompt_improver.services.ml_integration.mlflow") as mock_mlflow,
            patch(
                "prompt_improver.cli.console.print", side_effect=capture_console_print
            ),
        ):
            mock_mlflow.active_run.return_value = None
            mock_mlflow.start_run.return_value.__enter__ = MagicMock()
            ...
            assert any(
                ...
                for keyword in ["training", "processing", "ml"]
            )
...
class TestMLStatusCommand:
    ...
    @pytest.mark.asyncio
    async def test_ml_status_default(
        self, cli_runner, test_db_session, sample_rule_metadata
    ):
...
class TestOptimizeRulesCommand:
    ...
    @pytest.mark.asyncio
    async def test_optimize_rules_default(
        self, cli_runner, test_db_session, sample_rule_metadata, sample_rule_performance
    ):
    ...
    @pytest.mark.asyncio
    async def test_optimize_rules_specific_rules(
        self, cli_runner, test_db_session, sample_rule_metadata, sample_rule_performance
    ):
    ...
    def test_phase3_commands_database_error_handling(self, cli_runner):
        ...
        with (
            patch(
                ...
                side_effect=mock_asyncio_run_with_db_error,
            ),
    ...
    def test_async_commands_use_asyncio_run(self, cli_runner):
        ...
        def track_asyncio_run(coro):
            asyncio_calls.append(
                coro.__name__ if hasattr(coro, "__name__") else str(type(coro))
            )
        ...
        with (
            patch(
                "prompt_improver.cli.asyncio.run", side_effect=track_asyncio_run
            ) as mock_run,
            patch("prompt_improver.services.ml_integration.mlflow") as mock_mlflow,
            patch("prompt_improver.cli.console") as mock_console,
        ):
...
class TestCLIErrorHandlingAndLogging:
    ...
    def test_train_command_ml_service_error(self, cli_runner):
        ...
        with (
            ...
            patch(
                "prompt_improver.cli.asyncio.run",
                side_effect=mock_asyncio_run_with_ml_error,
            ),
            patch("prompt_improver.cli.console.print") as mock_console_print,
        ):
            mock_mlflow.active_run.return_value = None
            ...
            assert any(
                "error" in call.lower() or "failed" in call.lower()
...
class TestCLIFileSystemIntegration:
    ...
    @given(
        rule_count=st.integers(min_value=1, max_value=5), ensemble_mode=st.booleans()
    )
        ...
        large_data = [
            {
                ...
                "support": 5 + i % 10,
                "parameters": {"weight": 0.5 + i % 3 * 0.2, "data": "x" * 1000},
            }
            for i in range(1000)
        ]
...
class TestCLIEndToEndWorkflows:
    ...
    def test_complete_training_workflow_with_files(self, cli_runner, test_data_dir):
        ...
        with (
            patch(
                "prompt_improver.cli.asyncio.run", side_effect=capture_and_process
            ) as mock_run,
            patch("prompt_improver.services.ml_integration.mlflow") as mock_mlflow,
        ):
            mock_mlflow.active_run.return_value = None
