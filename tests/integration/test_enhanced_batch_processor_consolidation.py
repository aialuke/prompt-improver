...
async def test_enhanced_batch_processor_features():
    ...
    try:
        from prompt_improver.ml.optimization.batch.batch_processor import (
            BatchProcessor, BatchProcessorConfig, ProcessingMode, 
            PartitionStrategy, WorkerScalingMode
        )
        ...
        local_result = await processor._process_partitions_ray([test_items], "test_batch")
        assert isinstance(local_result, list)
        
        processor.config.processing_mode = ProcessingMode.DISTRIBUTED_DASK
        dask_result = await processor._process_partitions_dask([test_items], "test_batch")
        ...
        logger.error(f"❌ Enhanced BatchProcessor test failed: {e}")
        return False
...
async def test_new_config_system():
    ...
    try:
        ...
        logger.error(f"❌ New config system test failed: {e}")
        return False
...
async def test_performance_improvements():
    ...
    try:
        from prompt_improver.ml.optimization.batch.batch_processor import (
            BatchProcessor, BatchProcessorConfig, ProcessingMode, PartitionStrategy
        )
        ...
        enhanced_result = await enhanced_processor.process_with_circuit_breaker(test_items)
        ...
        logger.info(f"Enhanced processor time: {enhanced_time:.3f}s")
        logger.info(f"Basic processor time: {basic_time:.3f}s")
        logger.info(f"Enhanced processor partitions: {enhanced_result.get('partitions', 1)}")
...
async def test_old_batch_processor_removed():
    ...
    try:
        ...
        old_file_path = Path(__file__).parent / "src" / "prompt_improver" / "performance" / "optimization" / "batch_processor.py"
        ...
        logger.error(f"❌ Old batch processor removal test failed: {e}")
        return False
