...
class TestNetworkErrorService:
    ...
    async def test_network_error_retry_recommendations(
        ...
        retry_scenarios = [
            (TimeoutError("Connection timeout"), True, "exponential_backoff"),
...
class TestValidationErrorService:
    ...
    async def test_security_validation_error_detection(
        ...
        for error, suspicious_input in zip(security_errors, suspicious_inputs, strict=False):
            start_time = time.perf_counter()
    ...
    async def test_business_rule_validation_errors(
        ...
        for error, business_context in zip(business_rule_errors, business_contexts, strict=False):
            start_time = time.perf_counter()
...
class TestErrorHandlingFacade:
    ...
    async def test_unified_error_handling_with_classification(
        ...
        test_errors = [
            ...
            (TimeoutError("HTTP request timeout"), "api_call", ErrorServiceType.NETWORK),
