...
class MLTestContainer:
    ...
    async def start(self) -> "MLTestContainer":
        ...
        try:
            ...
            logger.error(f"Failed to start ML test container: {e}")
            await self.stop()
            raise
    ...
    async def _initialize_test_models(self):
        ...
        try:
            ...
            logger.error(f"Failed to initialize test models: {e}")
            raise
    ...
    async def _create_default_datasets(self):
        ...
        try:
            ...
            logger.error(f"Failed to create default datasets: {e}")
            raise
        ...
        try:
            if feature_type == "tfidf":
                ...
                return MLModelResult(
                    ...
                    feature_importance=dict(zip(feature_names[:10], [1.0] * 10)),  # Top 10 features
                )
    ...
    def get_test_dataset(self, size: str = "small") -> Optional[MLTestDataset]:
