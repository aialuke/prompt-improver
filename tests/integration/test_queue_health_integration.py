...
class TestQueueHealthIntegration:
        ...
        if processor.priority_queue:
            for i in range(10):
                processor.priority_queue.enqueue(
                    {"id": f"test_{i}", "data": f"test_data_{i}"}, priority=i % 3
                )
        yield processor
    ...
    async def real_task_manager(self):
        ...
        for i in range(3):
            await manager.submit_task(
                task_id=f"pending_task_{i}", coroutine=test_task, duration=0.5
            )
        for i in range(2):
            await manager.submit_task(
                task_id=f"running_task_{i}", coroutine=test_task, duration=0.05
            )
    ...
    @pytest.mark.asyncio
    async def test_queue_health_checker_basic_functionality(
        self, real_batch_processor, real_task_manager, monkeypatch
    ):
    ...
    @pytest.mark.asyncio
    async def test_queue_health_checker_in_health_service(
        self, real_batch_processor, real_task_manager, monkeypatch
    ):
    ...
    @pytest.mark.asyncio
    async def test_queue_health_mcp_endpoint_integration(
        self, real_batch_processor, real_task_manager, monkeypatch
    ):
    ...
    @pytest.mark.asyncio
    async def test_queue_health_comprehensive_integration(
        self, real_batch_processor, real_task_manager, monkeypatch
    ):
        ...
        if real_batch_processor.priority_queue:
            for i in range(5):
                real_batch_processor.priority_queue.enqueue(
                    {"id": f"new_test_{i}", "data": f"new_data_{i}"}, priority=1
                )
    ...
    async def test_background_task_lifecycle(self, real_task_manager):
        ...
        task_id = await real_task_manager.submit_task(
            task_id="lifecycle_test", coroutine=lifecycle_test_task
        )
        ...
        assert task.status.value == "completed", (
            f"Task status is {task.status.value} after {waited}s"
        )
