...
from prompt_improver.performance.monitoring.health.background_manager import BackgroundTaskManager, get_background_task_manager
...
class TestQueueHealthIntegration:
    ...
    async def real_batch_processor(self):
        ...
        config = BatchProcessorConfig(batch_size=10, concurrency=3, max_queue_size=1000, enable_priority_queue=True, max_attempts=3, batch_timeout=1, timeout=5000)
        processor = BatchProcessor(config=config)
        processor.metrics = {'processed': 100, 'failed': 5, 'retries': 2, 'start_time': time.time()}
        if processor.priority_queue:
            for i in range(10):
                processor.priority_queue.enqueue({'id': f'test_{i}', 'data': f'test_data_{i}'}, priority=i % 3)
        yield processor
    ...
    async def real_task_manager(self):
        ...
        async def test_task(duration: float=0.1):
            ...
            return 'completed'
        for i in range(3):
            await manager.submit_task(task_id=f'pending_task_{i}', coroutine=test_task, duration=0.5)
        for i in range(2):
            await manager.submit_task(task_id=f'running_task_{i}', coroutine=test_task, duration=0.05)
    ...
    async def test_queue_health_checker_basic_functionality(self, real_batch_processor, real_task_manager, monkeypatch):
        ...
        monkeypatch.setattr('prompt_improver.performance.monitoring.health.checkers.get_background_task_manager', lambda: real_task_manager)
        ...
        assert result.details['training_queue_size'] >= 0
    ...
    async def test_queue_health_status_evaluation(self, real_batch_processor):
        ...
        checker = QueueHealthChecker(batch_processor=real_batch_processor)
        healthy_metrics = {'queue_capacity_utilization': 0.5, 'retry_backlog_ratio': 0.1, 'success_rate': 0.98, 'task_utilization': 0.6}
        status = checker._evaluate_queue_health(healthy_metrics)
        assert status == HealthStatus.HEALTHY
        warning_metrics = {'queue_capacity_utilization': 0.85, 'retry_backlog_ratio': 0.1, 'success_rate': 0.98, 'task_utilization': 0.6}
        status = checker._evaluate_queue_health(warning_metrics)
        assert status == HealthStatus.WARNING
        failed_metrics = {'queue_capacity_utilization': 0.5, 'retry_backlog_ratio': 0.6, 'success_rate': 0.98, 'task_utilization': 0.6}
    ...
    async def test_queue_health_error_handling(self, monkeypatch):
        ...
        monkeypatch.setattr('prompt_improver.performance.monitoring.health.checkers.get_background_task_manager', mock_no_background_manager)
    ...
    async def test_queue_health_status_messages(self, real_batch_processor):
        ...
        checker = QueueHealthChecker(batch_processor=real_batch_processor)
        healthy_metrics = {'total_queue_backlog': 10, 'queue_capacity_utilization': 0.3, 'success_rate': 0.98}
        ...
        warning_metrics = {'total_queue_backlog': 800, 'queue_capacity_utilization': 0.85, 'success_rate': 0.92}
    ...
    async def test_real_queue_processing_behavior(self, real_batch_processor):
        ...
        if real_batch_processor.priority_queue:
            for i in range(5):
                real_batch_processor.priority_queue.enqueue({'id': f'new_test_{i}', 'data': f'new_data_{i}'}, priority=1)
    ...
    async def test_background_task_lifecycle(self, real_task_manager):
        ...
        task_id = await real_task_manager.submit_task(task_id='lifecycle_test', coroutine=lifecycle_test_task)
