...
class TestUnifiedSessionConsolidation:
    ...
    async def test_session_store_cli_integration(self, unified_session_manager):
        ...
        success = await unified_session_manager.create_training_session(session_id=session_id, training_config=training_config)
        assert success, 'Training session creation should succeed'
        progress_updated = await unified_session_manager.update_training_progress(session_id=session_id, iteration=5, performance_metrics={'accuracy': 0.85, 'loss': 0.15}, improvement_score=0.12)
    ...
    async def test_analytics_session_management(self, unified_session_manager):
        ...
        analytics_session_id = await unified_session_manager.create_analytics_session(analysis_type='performance_comparison', target_session_ids=target_sessions)
        assert analytics_session_id.startswith('analytics_'), 'Analytics session should have proper prefix'
        updated = await unified_session_manager.update_analytics_progress(session_id=analytics_session_id, progress_percentage=75.0, results={'comparison_metrics': {'improvement': 0.15}})
    ...
    async def test_session_recovery_consolidation(self, unified_session_manager):
        ...
        training_config = {'max_iterations': 20, 'improvement_threshold': 0.1, 'continuous_mode': True}
        await unified_session_manager.create_training_session(session_id=session_id, training_config=training_config)
    ...
    async def test_progress_preservation_integration(self, unified_session_manager):
        ...
        success = await preservation_manager.save_training_progress(session_id=session_id, iteration=3, performance_metrics={'accuracy': 0.75}, rule_optimizations={'rule1': {'improvement': 0.1}}, workflow_state={'step': 'optimization'}, synthetic_data_generated=100, improvement_score=0.08)
    ...
    async def test_training_system_manager_integration(self, unified_session_manager):
        ...
        training_config = {'max_iterations': 15, 'improvement_threshold': 0.12, 'continuous_mode': False}
        ...
        updated = await training_manager.update_training_progress(iteration=2, performance_metrics={'loss': 0.25, 'accuracy': 0.78}, improvement_score=0.15)
    ...
    async def test_session_type_isolation(self, unified_session_manager):
        ...
        await unified_session_manager.create_training_session(session_id=training_session_id, training_config={'max_iterations': 5})
        analytics_session = await unified_session_manager.create_analytics_session(analysis_type='isolation_test', target_session_ids=[training_session_id])
    ...
    async def test_mcp_server_session_integration(self, mcp_server):
        ...
        test_data = {'test_key': 'test_value', 'timestamp': datetime.now(UTC).isoformat()}
