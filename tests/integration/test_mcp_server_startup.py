...
async def test_mcp_server_startup() -> dict[str, Any]:
    ...
    results = {'server_instantiation': False, 'server_initialization': False, 'basic_tools': {}, 'health_checks': {}, 'database_operations': {}, 'issues_found': []}
    try:
        os.environ['DATABASE_URL'] = 'postgresql://apes_user:apes_secure_password_2024@localhost:5432/apes_production'
        ...
        try:
            set_result = await server._set_session_impl('test_session', {'test': 'data'})
            ...
            session_working = set_result.get('success', False) and get_result.get('exists', False) and touch_result.get('success', False) and delete_result.get('success', False)
            results['basic_tools']['session_management'] = session_working
            print(f"✅ Session management: {('✅ Working' if session_working else '❌ Failed')}")
        ...
        try:
            ...
            print(f"✅ Performance monitoring: {('✅ Working' if perf_working else '❌ Failed')}")
        except Exception as e:
            results['basic_tools']['performance_status'] = False
            results['issues_found'].append(f'Performance monitoring error: {str(e)[:100]}')
        ...
        try:
            ...
            print(f"✅ Event loop benchmark: {('✅ Working' if bench_working else '❌ Failed')}")
        except Exception as e:
            results['basic_tools']['event_loop_benchmark'] = False
            results['issues_found'].append(f'Event loop benchmark error: {str(e)[:100]}')
        ...
        try:
            ...
            print(f"✅ Health ready: {('✅ Working' if ready_working else '❌ Failed')}")
        except TimeoutError:
            results['health_checks']['ready'] = False
            results['issues_found'].append('Health ready check timed out (database connection issue)')
        ...
        try:
            ...
            print(f"✅ List tables: {('✅ Working' if tables_working else '❌ Failed')}")
        except TimeoutError:
            results['database_operations']['list_tables'] = False
            results['issues_found'].append('List tables timed out (database connection issue)')
        ...
        try:
            prompt_task = asyncio.create_task(server._improve_prompt_impl('Please help me write a professional email', {'purpose': 'business'}, 'test_session', 100))
            prompt_result = await asyncio.wait_for(prompt_task, timeout=10.0)
            if 'error' in prompt_result:
                if 'Input validation failed' in prompt_result['error']:
                    print('⚠️ Prompt improvement blocked by security validation (expected behavior)')
                    ...
                    results['issues_found'].append(f"Prompt improvement error: {prompt_result['error'][:100]}")
...
def main():
    ...
    print('=' * 60)
    print(f"Server Instantiation: {('✅ PASS' if results['server_instantiation'] else '❌ FAIL')}")
    print(f"Server Initialization: {('✅ PASS' if results['server_initialization'] else '❌ FAIL')}")
    print('\nBasic Tools:')
    ...
    if results['issues_found']:
        ...
        for i, issue in enumerate(results['issues_found'], 1):
            print(f'  {i}. {issue}')
    total_tests = 1 + 1 + len(results['basic_tools']) + len(results['health_checks']) + len(results['database_operations'])
    passed_tests = (1 if results['server_instantiation'] else 0) + (1 if results['server_initialization'] else 0) + sum((1 for status in results['basic_tools'].values() if status in [True, 'security_blocked'])) + sum((1 for status in results['health_checks'].values() if status)) + sum((1 for status in results['database_operations'].values() if status))
