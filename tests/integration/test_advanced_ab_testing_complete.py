...
class TestAdvancedABTestingComplete:
    ...
    def create_statistically_significant_data(
        self, test_db_session, experiment_id_suffix=""
    ):
        ...
        for i in range(120):
            record = RulePerformance(
                ...
                confidence_level=np.random.normal(0.9, 0.05),
                created_at=datetime.utcnow() - timedelta(hours=i // 10),
            )
    ...
    def create_time_expired_data(self, test_db_session, max_duration_days=30):
        ...
        for i in range(100):
            record = RulePerformance(
                ...
                - timedelta(days=max_duration_days + 5 - i // 10),
            )
    ...
    async def orchestrator(self, test_db_session):
        ...
        statistical_validator = AdvancedStatisticalValidator(
            alpha=0.05, bootstrap_samples=100
        )
    ...
    async def sample_experiment_data(self, test_db_session):
        ...
        for i in range(115):
            record = RulePerformance(
                ...
                rule_id="chain_of_thought_rule",
                improvement_score=np.random.normal(0.75, 0.1),
                execution_time_ms=np.random.normal(110, 20),
                confidence_level=np.random.normal(0.88, 0.05),
                created_at=datetime.utcnow() - timedelta(hours=i // 4),
            )
            records.append(record)
        for i in range(3):
        ...
        for i in range(50):
            record = RulePerformance(
                ...
                rule_id="specificity_rule",
                improvement_score=np.random.normal(0.65, 0.1),
                execution_time_ms=np.random.normal(120, 25),
                confidence_level=np.random.normal(0.8, 0.05),
        ...
        for i in range(200):
            record = RulePerformance(
                ...
                rule_id="clarity_rule",
                improvement_score=np.random.normal(0.7, 0.05),
                execution_time_ms=np.random.normal(100, 10),
                confidence_level=np.random.normal(0.9, 0.02),
        ...
        for i in range(200):
            record = RulePerformance(
                ...
                rule_id="chain_of_thought_rule",
                improvement_score=np.random.normal(0.8, 0.05),
                execution_time_ms=np.random.normal(105, 10),
    ...
    async def test_experiment_setup_validation_errors(self, orchestrator):
        ...
        invalid_config = ExperimentConfiguration(
            ...
            arms=[ExperimentArm("single", "Single", "Only arm", {})],
    ...
    async def test_error_handling_and_recovery(self, orchestrator, simple_ab_config):
        ...
        assert (
            ...
            or "session" in result["error"].lower()
        )
        from prompt_improver.evaluation.advanced_statistical_validator import (
            AdvancedStatisticalValidator,
        )
        ...
        assert setup_result["success"]
        analysis_result = await fresh_orchestrator.analyze_experiment(
            simple_ab_config.experiment_id
        )
        assert analysis_result.experiment_id == simple_ab_config.experiment_id
        assert "insufficient" in analysis_result.stopping_recommendation.lower()
    ...
    async def test_integration_with_real_time_analytics(
        ...
        assert (
            simple_ab_config.experiment_id
            in orchestrator.real_time_service.monitoring_tasks
        )
        status = await orchestrator.get_experiment_status(
            simple_ab_config.experiment_id
        )
