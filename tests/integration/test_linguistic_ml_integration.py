...
class TestLinguisticMLIntegration:
    ...
    def sample_results(self) -> list[dict[str, Any]]:
        ...
        return [{'originalPrompt': 'Write a Python function to calculate factorial.', 'overallScore': 0.7, 'clarity': 0.6, 'completeness': 0.5, 'actionability': 0.8, 'effectiveness': 0.7, 'context': {'projectType': 'api', 'domain': 'education', 'complexity': 'medium', 'teamSize': 5}}, {'originalPrompt': 'Please write a comprehensive Python function that calculates the factorial of a number. The function should handle edge cases like negative numbers and zero. Include proper error handling, documentation, and examples of usage.', 'overallScore': 0.9, 'clarity': 0.9, 'completeness': 0.9, 'actionability': 0.9, 'effectiveness': 0.9, 'context': {'projectType': 'api', 'domain': 'education', 'complexity': 'high', 'teamSize': 8}}, {'originalPrompt': 'Create ML model.', 'overallScore': 0.3, 'clarity': 0.2, 'completeness': 0.2, 'actionability': 0.4, 'effectiveness': 0.3, 'context': {'projectType': 'ml', 'domain': 'other', 'complexity': 'low', 'teamSize': 3}}, {'originalPrompt': 'Develop a machine learning model using Python and scikit-learn for predicting customer churn. The model should include data preprocessing, feature engineering, model training with cross-validation, and performance evaluation using appropriate metrics.', 'overallScore': 0.95, 'clarity': 0.9, 'completeness': 0.95, 'actionability': 0.9, 'effectiveness': 0.95, 'context': {'projectType': 'ml', 'domain': 'finance', 'complexity': 'very_high', 'teamSize': 12}}]

    @pytest.fixture
    def context_config_with_linguistics(self) -> ContextConfig:
        """Context config with linguistic features enabled."""
        return ContextConfig(enable_linguistic_features=True, linguistic_feature_weight=0.3, cache_linguistic_analysis=True, use_advanced_clustering=False, min_sample_size=2)

    @pytest.fixture
    def context_config_without_linguistics(self) -> ContextConfig:
        """Context config with linguistic features disabled."""
        return ContextConfig(enable_linguistic_features=False, use_advanced_clustering=False, min_sample_size=2)
    ...
    def test_linguistic_integration_improves_clustering(self, sample_results):
        ...
        config_with = ContextConfig(enable_linguistic_features=True, use_advanced_clustering=False, min_sample_size=2)
        learner_with = ContextSpecificLearner(config_with)
        config_without = ContextConfig(enable_linguistic_features=False, use_advanced_clustering=False, min_sample_size=2)
    ...
    async def test_end_to_end_context_analysis_with_linguistics(self, context_config_with_linguistics, sample_results):
        ...
        learner = ContextSpecificLearner(context_config_with_linguistics)
        context_grouped_data = {'api_education_medium': {'historical_data': sample_results[:2], 'sample_size': 2}, 'ml_mixed_complexity': {'historical_data': sample_results[2:], 'sample_size': 2}}
    ...
    def test_linguistic_features_improve_context_discrimination(self, sample_results):
        ...
        config = ContextConfig(enable_linguistic_features=True, use_advanced_clustering=False, min_sample_size=2)
        ...
        features_diff = sum((abs(h - l) for h, l in zip(features_high, features_low, strict=False)))
