...
async def test_context_feature_extractor():
    ...
    try:
        ...
        print(
            f"✅ Initialized with config: weight={config.weight}, timeout={config.timeout_seconds}s"
        )
        ...
        print(
            f"✅ Async extracted {len(features_async)} features in {async_extraction_time:.3f}s"
        )
        ...
        orchestrator_result = await extractor.run_orchestrated_analysis(
            orchestrator_config
        )
...
async def test_domain_feature_extractor():
    ...
    try:
        ...
        print(f"   Feature count: {orchestrator_result['result']['feature_count']}")
        print(
            f"   Analyzer usage rate: {orchestrator_result['result']['extraction_metadata']['analyzer_usage_rate']:.1f}%"
        )
        health_config = {
            "operation": "health_check",
...
async def test_circuit_breaker_behavior():
    ...
    try:
        ...
        for i in range(5):
            try:
                ...
                if health["circuit_breaker_state"] == "open":
                    print(
                        f"   ✅ Circuit breaker opened after {failed_attempts} failures"
                    )
                    break
        ...
        print(
            f"✅ Circuit breaker recovered, state: {final_health['circuit_breaker_state']}"
        )
        return True
    except Exception as e:
...
async def test_performance_comparison():
    ...
    try:
        ...
        print(
            f"✅ ContextFeatureExtractor: {context_avg_time:.4f}s avg, {context_min_time:.4f}s min, {context_max_time:.4f}s max"
        )
        ...
        print(
            f"✅ DomainFeatureExtractor: {domain_avg_time:.4f}s avg, {domain_min_time:.4f}s min, {domain_max_time:.4f}s max"
        )
