...
class TestRealBehaviorMLComponents:
    ...
    async def test_failure_analyzer_real_metrics_collection(self, real_behavior_environment):
        ...
        # Verify real metrics were emitted
    ...
    async def test_failure_classifier_real_data_processing(self, real_behavior_environment):
        ...
        # Create FailureClassifier instance
        classifier = FailureClassifier()
        
        # Prepare real training data
        training_features = [
            [0.8, 150, 1.0, 5, 0.7],  # High severity timeout
            [0.3, 50, 0.2, 2, 0.1],   # Low severity connection issue
            [0.9, 300, 1.0, 8, 0.9],  # Critical system failure
            [0.5, 100, 0.6, 3, 0.4],  # Medium severity error
            [0.2, 25, 0.1, 1, 0.05],  # Minor warning
        ] * 10  # 50 samples for reliable training
        
        training_labels = [
            "timeout", "connection", "system", "error", "warning"
        ] * 10
        ...
        assert training_result["accuracy"] > 0.5  # Reasonable accuracy threshold
        
        # Test real classification
        test_features = [0.7, 200, 0.8, 6, 0.6]  # Should classify as timeout
    ...
    async def test_end_to_end_ml_pipeline_real_behavior(self, real_behavior_environment):
        ...
        # Create ML component instances
        analyzer = FailureAnalyzer()
        classifier = FailureClassifier()
        
        # Prepare real failure scenarios
        failure_scenarios = [
            {
                "id": str(uuid.uuid4()),
                "error_type": "timeout",
        ...
        with tracer.start_as_current_span("ml_pipeline_test") as pipeline_span:
            for scenario in failure_scenarios:
                ...
                # Step 4: Store results in Redis cache
        ...
        # Verify database contains all ML metrics
        result = await database.execute(
        ...
        assert metric_count["count"] >= len(failure_scenarios) * 2  # At least 2 metrics per scenario
        
        # Verify failure analysis records
    ...
    async def test_ml_component_performance_real_metrics(self, real_behavior_environment):
        ...
        total_duration = (end_time - start_time).total_seconds() * 1000  # Convert to ms
        
        # Validate performance metrics in database
        result = await database.execute(
            "SELECT * FROM ml_metrics WHERE metric_name LIKE %s",
            ("%duration%",)
        )
        ...
        # Validate metric values are realistic
        for metric in duration_metrics:
            assert metric["metric_value"] > 0
            assert metric["metric_value"] < 1000  # Should be under 1 second per operation
