...
from prompt_improver.database.models import MLModelPerformance, RuleMetadata, RulePerformance
...
class TestDirectPythonIntegration:
    ...
    @pytest.mark.asyncio
    async def test_optimize_rules_success(self, ml_service, sample_training_data, real_db_session, real_rule_metadata):
        """Test successful rule optimization with real ML models and database."""
        result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=[rule.rule_id for rule in real_rule_metadata])
        ...
        stmt = select(MLModelPerformance).where(MLModelPerformance.model_id == result['model_id'])
    ...
    async def test_optimize_rules_insufficient_data(self, ml_service, real_db_session):
        ...
        degenerate_data = {'features': [[0.5, 100, 1.0, 5, 0.7, 1.0]] * 5, 'effectiveness_scores': [0.5] * 5}
        ...
        insufficient_samples = {'features': [[0.5, 100, 1.0, 5, 0.7, 1.0]] * 5, 'effectiveness_scores': [0.1, 0.3, 0.5, 0.7, 0.9]}
    ...
    async def test_predict_rule_effectiveness(self, ml_service, sample_training_data, real_db_session):
        ...
        train_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['test_rule'])
    ...
    async def test_predict_model_not_found(self, ml_service):
        ...
        result = await ml_service.predict_rule_effectiveness('nonexistent_model', [0.5, 100, 1.0])
        assert 'error' in result
...
class TestEnsembleOptimization:
    ...
    async def test_optimize_ensemble_rules_success(self, ml_service, real_db_session):
        ...
        for i in range(25):
            ...
            varied_feature = [base_feature[j] + random.uniform(-0.1, 0.1) for j in range(len(base_feature))]
        ...
        ensemble_data = {'features': features, 'effectiveness_scores': effectiveness_scores}
        result = await ml_service.optimize_ensemble_rules(ensemble_data, real_db_session)
        assert result['status'] == 'success'
        assert 'model_id' in result
    ...
    @pytest.mark.asyncio
    async def test_optimize_ensemble_insufficient_data(self, ml_service, real_db_session):
        """Test ensemble optimization with insufficient data using real validation."""
        insufficient_data = {'features': [[0.5, 100]] * 15, 'effectiveness_scores': [0.5] * 15}
        result = await ml_service.optimize_ensemble_rules(insufficient_data, real_db_session)
        assert 'error' in result
...
class TestPatternDiscovery:
    ...
    @pytest.mark.asyncio
    async def test_discover_patterns_success(self, ml_service, real_db_session, real_rule_metadata, sample_rule_performance, real_prompt_sessions):
        ...
        result = await ml_service.discover_patterns(real_db_session, min_effectiveness=0.7, min_support=3)
        assert result['status'] == 'success'
    ...
    @pytest.mark.asyncio
    async def test_discover_patterns_insufficient_data(self, ml_service, real_db_session):
...
class TestDatabaseIntegration:
    ...
    async def test_update_rule_parameters(self, ml_service, real_db_session, real_rule_metadata):
        ...
        await ml_service._update_rule_parameters(real_db_session, [rule_id], optimized_params, 0.85, 'test_model_123')
    ...
    async def test_store_model_performance(self, ml_service, real_db_session):
        ...
        await ml_service._store_model_performance(real_db_session, 'test_model_123', 0.85, 0.9, 0.88, 0.92)
        stmt = select(MLModelPerformance).where(MLModelPerformance.model_id == 'test_model_123')
        result = await real_db_session.execute(stmt)
    ...
    async def test_cache_invalidation_on_rule_update(self, ml_service, real_db_session, real_rule_metadata):
        ...
        with patch('prompt_improver.services.ml_integration.redis_client') as mock_redis:
            ...
            optimized_params = {'n_estimators': 100, 'max_depth': 10}
            await ml_service._update_rule_parameters(real_db_session, rule_ids, optimized_params, 0.85, 'test_model_123')
...
class TestPerformanceRequirements:
    ...
    async def test_prediction_latency_requirement(self, ml_service, sample_training_data, real_db_session):
        ...
        small_data = {'features': sample_training_data['features'][:15], 'effectiveness_scores': sample_training_data['effectiveness_scores'][:15]}
        train_result = await ml_service.optimize_rules(small_data, real_db_session, rule_ids=['perf_test_rule'])
        assert train_result['status'] == 'success'
        model_id = train_result['model_id']
        rule_features = [0.7, 150, 1.0, 5, 0.8, 1.0]
        result = await ml_service.predict_rule_effectiveness(model_id, rule_features)
    ...
    @pytest.mark.asyncio
    async def test_optimization_timeout_handling(self, ml_service, sample_training_data, real_db_session):
        ...
        result = await ml_service_with_timeout.optimize_rules(sample_training_data, real_db_session)
        end_time = time.time()
...
class TestErrorHandling:
    ...
    async def test_optimization_with_invalid_data(self, ml_service, real_db_session):
        ...
        invalid_data = {'features': 'invalid_format', 'effectiveness_scores': [0.5, 0.6]}
    ...
    @pytest.mark.asyncio
    async def test_mlflow_error_handling(self, ml_service, sample_training_data, real_db_session):
        """Test handling of MLflow errors."""
        with patch('mlflow.start_run', side_effect=Exception('MLflow unavailable')):
            result = await ml_service.optimize_rules(sample_training_data, real_db_session)
            assert 'error' in result
            assert result['status'] == 'error'
...
class TestMLModelContracts:
    ...
    @given(features=arrays(dtype=np.float64, shape=st.tuples(st.integers(min_value=1, max_value=100), st.integers(min_value=5, max_value=20)), elements=st.floats(min_value=0.0, max_value=1.0, allow_nan=False, allow_infinity=False)))
    @pytest.mark.asyncio
    ...
    async def test_prediction_contract_feature_validation(self, ml_service, features, sample_training_data, real_db_session):
        ...
        train_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['contract_test_rule'])
        assert train_result['status'] == 'success'
        model_id = train_result['model_id']
    ...
    @given(n_samples=st.integers(min_value=10, max_value=100), n_features=st.integers(min_value=5, max_value=15), effectiveness_scores=st.lists(st.floats(min_value=0.0, max_value=1.0), min_size=10, max_size=100))
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture, HealthCheck.filter_too_much], max_examples=3, deadline=60000)
    async def test_training_data_contract_validation(self, ml_service, real_db_session, n_samples, n_features, effectiveness_scores):
        ...
        training_data = {'features': features, 'effectiveness_scores': effectiveness_scores}
    ...
    @given(model_params=st.dictionaries(keys=st.sampled_from(['n_estimators', 'max_depth', 'min_samples_split', 'learning_rate']), values=st.one_of(st.integers(min_value=1, max_value=1000), st.floats(min_value=0.001, max_value=1.0)), min_size=1, max_size=4))
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    async def test_model_parameter_contract_validation(self, ml_service, mock_db_session, model_params):
        ...
        with patch.object(ml_service, '_update_rule_parameters') as mock_update:
            ...
            await ml_service._update_rule_parameters(mock_db_session, ['test_rule'], model_params, 0.8, 'test_model')
...
class TestMLPropertyBasedValidation:
    ...
    @given(rule_count=st.integers(min_value=1, max_value=10), feature_dimension=st.integers(min_value=5, max_value=20))
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    async def test_prediction_consistency_property(self, ml_service, rule_count, feature_dimension, sample_training_data, real_db_session):
        """Property: identical inputs should produce identical predictions."""
        train_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['consistency_test_rule'])
        assert train_result['status'] == 'success'
        model_id = train_result['model_id']
        test_features = [0.5] * 6
        results = []
    ...
    @given(data_size=st.integers(min_value=20, max_value=100), noise_level=st.floats(min_value=0.0, max_value=0.1))
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture, HealthCheck.filter_too_much], max_examples=2, deadline=120000)
    async def test_optimization_convergence_property(self, ml_service, real_db_session, data_size, noise_level):
        ...
        training_data = {'features': features.tolist(), 'effectiveness_scores': effectiveness_scores}
        result = await ml_service.optimize_rules(training_data, real_db_session)
        if result['status'] == 'success':
            assert result['best_score'] >= 0.4, f"Optimization failed to find meaningful patterns: {result['best_score']}"
            assert result['best_score'] <= 1.0, 'Invalid optimization score'
            expected_timeout = 30000 if data_size < 50 else 60000
            assert result['processing_time_ms'] < expected_timeout, f"Optimization took too long: {result['processing_time_ms']}ms > {expected_timeout}ms"
    ...
    @given(effectiveness_threshold=st.floats(min_value=0.5, max_value=0.9), support_threshold=st.integers(min_value=2, max_value=10))
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    async def test_pattern_discovery_threshold_property(self, ml_service, mock_db_session, effectiveness_threshold, support_threshold):
        ...
        for i in range(15):
            ...
            support = 2 + i % 6
            mock_patterns.append({'parameters': {'param': i}, 'avg_effectiveness': effectiveness, 'support_count': support, 'rule_ids': [f'rule_{i}']})
        mock_result = MagicMock()
        mock_result.fetchall.return_value = mock_patterns
        mock_db_session.execute.return_value = mock_result
        result = await ml_service.discover_patterns(mock_db_session, min_effectiveness=effectiveness_threshold, min_support=support_threshold)
        if result['status'] == 'success' and result['patterns']:
            for pattern in result['patterns']:
                assert pattern['avg_effectiveness'] >= effectiveness_threshold, f"Pattern effectiveness {pattern['avg_effectiveness']} below threshold {effectiveness_threshold}"
                assert pattern['support_count'] >= support_threshold, f"Pattern support {pattern['support_count']} below threshold {support_threshold}"
...
class TestMLPerformanceCharacterization:
    ...
    async def test_prediction_latency_characterization(self, ml_service, sample_training_data, real_db_session):
        ...
        for model_name, data_size in test_scenarios:
            train_data = {'features': sample_training_data['features'][:data_size], 'effectiveness_scores': sample_training_data['effectiveness_scores'][:data_size]}
            train_result = await ml_service.optimize_rules(train_data, real_db_session, rule_ids=[f'{model_name}_rule'])
            assert train_result['status'] == 'success'
            model_id = train_result['model_id']
            test_features = [0.5] * 6
            latencies = []
        ...
        latency_growth_factor = medium_latency / simple_latency if simple_latency > 0 else 1
        assert latency_growth_factor < 2.0, f'Latency grows too much with complexity: {latency_growth_factor:.2f}x'

    @pytest.mark.asyncio
    async def test_optimization_scalability_characterization(self, ml_service, real_db_session):
        ...
        for data_size in data_sizes:
            ...
            for i in range(data_size):
                ...
                varied_feature = [base_feature[0] + random.uniform(-0.2, 0.2), base_feature[1] + random.uniform(-50, 50), base_feature[2] + random.uniform(-0.3, 0.3), base_feature[3] + random.randint(-2, 2), base_feature[4] + random.uniform(-0.2, 0.2), base_feature[5]]
        ...
        if len(optimization_times) >= 2:
            ...
            assert scaling_factor < 5.0, f'Optimization scaling factor {scaling_factor:.2f} indicates poor scalability'
...
class TestMLDataQualityValidation:
    ...
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture, HealthCheck.filter_too_much], max_examples=2, deadline=120000)
    async def test_corrupted_data_handling(self, ml_service, real_db_session, corrupted_ratio):
        ...
        for i in range(clean_size):
            clean_features.append([np.random.uniform(0.3, 0.9), np.random.uniform(100, 200), np.random.uniform(0.5, 1.0), np.random.randint(3, 8), np.random.uniform(0.4, 0.8), np.random.uniform(0.6, 1.0)])
            if i < clean_size // 2:
        ...
        training_data = {'features': clean_features + corrupted_features, 'effectiveness_scores': clean_scores + corrupted_scores}
        result = await ml_service.optimize_rules(training_data, real_db_session)
        assert 'status' in result
        if corrupted_ratio == 0.0:
            ...
            if result['status'] == 'error':
                assert 'data' in result['error'].lower() or 'invalid' in result['error'].lower() or 'nan' in result['error'].lower() or ('inf' in result['error'].lower()), f"Error message should indicate data issues: {result['error']}"
            else:
...
class TestMLMetamorphicProperties:
    ...
    async def test_prediction_determinism(self, ml_service, sample_training_data, real_db_session):
        ...
        train_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['determinism_test_rule'])
        assert train_result['status'] == 'success'
        model_id = train_result['model_id']
    ...
    @given(perturbation_factor=st.floats(min_value=0.001, max_value=0.1), feature_index=st.integers(min_value=0, max_value=5))
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=3)
    async def test_small_perturbation_stability(self, ml_service, sample_training_data, real_db_session, perturbation_factor, feature_index):
        """Property: Small input perturbations should produce stable output changes."""
        train_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['perturbation_test_rule'])
        assert train_result['status'] == 'success'
        model_id = train_result['model_id']
    ...
    @given(effectiveness_threshold_1=st.floats(min_value=0.5, max_value=0.7), effectiveness_threshold_2=st.floats(min_value=0.7, max_value=0.9))
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=2)
    async def test_pattern_discovery_monotonicity(self, ml_service, real_db_session, real_rule_metadata, sample_rule_performance, real_prompt_sessions, effectiveness_threshold_1, effectiveness_threshold_2):
        ...
        for i, performance in enumerate(sample_rule_performance[:10]):
            ...
            real_db_session.add(performance)
        await real_db_session.commit()
        patterns_low = await ml_service.discover_patterns(real_db_session, min_effectiveness=effectiveness_threshold_1, min_support=2)
        patterns_high = await ml_service.discover_patterns(real_db_session, min_effectiveness=effectiveness_threshold_2, min_support=2)
        if patterns_low['status'] == 'success' and patterns_high['status'] == 'success':
            assert patterns_high['patterns_discovered'] <= patterns_low['patterns_discovered'], f"Higher threshold found more patterns: {patterns_high['patterns_discovered']} > {patterns_low['patterns_discovered']}"

    @given(feature_multiplier=st.floats(min_value=1.1, max_value=2.0))
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=3)
    async def test_feature_enhancement_monotonicity(self, ml_service, sample_training_data, real_db_session, feature_multiplier):
        """Property: Enhanced features should generally improve or maintain effectiveness predictions."""
        train_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['monotonicity_test_rule'])
        assert train_result['status'] == 'success'
        model_id = train_result['model_id']
        baseline_features = [0.6, 100, 0.8, 4, 0.7, 0.9]
        enhanced_features = [baseline_features[0] * feature_multiplier, baseline_features[1] * feature_multiplier, baseline_features[2] * feature_multiplier, baseline_features[3], baseline_features[4] * feature_multiplier, baseline_features[5]]
        ...
        if baseline_pred['prediction'] >= 0.8:
            assert prediction_improvement >= -0.5, f'Enhanced features decreased high-effectiveness prediction too much: {prediction_improvement}'
        else:
            assert prediction_improvement >= -0.8, f'Enhanced features decreased effectiveness too much: {prediction_improvement}'

    @given(noise_level=st.floats(min_value=0.0, max_value=0.1))
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=3)
    async def test_input_noise_robustness(self, ml_service, sample_training_data, real_db_session, noise_level):
        """Property: Model should be robust to small amounts of input noise."""
        train_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['robustness_test_rule'])
        assert train_result['status'] == 'success'
        model_id = train_result['model_id']
        clean_features = [0.7, 150, 1.0, 5, 0.8, 1.0]
        np.random.seed(42)
        noise = np.random.normal(0, noise_level, len(clean_features))
        noisy_features = [max(0.0, min(1.0, f + n)) for f, n in zip(clean_features, noise, strict=False)]
        ...
        assert prediction_diff <= max_prediction_change, f'Model not robust to noise: prediction changed by {prediction_diff}'
        assert confidence_diff <= max_confidence_change, f'Model not robust to noise: confidence changed by {confidence_diff}'
    ...
    async def test_ensemble_consistency(self, ml_service, sample_training_data, real_db_session):
        ...
        model1_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['ensemble_test_1'])
        model2_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['ensemble_test_2'])
    ...
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=2)
    async def test_batch_consistency(self, ml_service, sample_training_data, real_db_session, batch_size):
        """Property: Batch predictions should be consistent with individual predictions."""
        train_result = await ml_service.optimize_rules(sample_training_data, real_db_session, rule_ids=['batch_test_rule'])
        assert train_result['status'] == 'success'
        model_id = train_result['model_id']
        feature_sets = [[0.7, 150, 1.0, 5, 0.8, 1.0], [0.6, 100, 0.9, 4, 0.7, 0.9], [0.8, 200, 0.8, 6, 0.9, 0.8], [0.5, 80, 0.7, 3, 0.6, 0.7], [0.9, 180, 0.95, 7, 0.85, 0.95]][:batch_size]
    ...
    @pytest.mark.asyncio
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=2)
    async def test_optimization_score_boundary_consistency(self, ml_service, sample_training_data, real_db_session, threshold_offset):
        ...
        result = await ml_service.optimize_rules(boundary_data, real_db_session, rule_ids=['boundary_test_rule'])
        if result['status'] == 'success':
