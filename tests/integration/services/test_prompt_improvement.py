...
class TestTriggerOptimization:
    ...
    async def test_trigger_optimization_feedback_not_found(
        self, prompt_service, mock_db_session
    ):
    ...
    async def test_trigger_optimization_insufficient_data(
        self, prompt_service, mock_db_session, sample_user_feedback
    ):
...
class TestRunMLOptimization:
    ...
    async def test_run_ml_optimization_success(
        self, prompt_service, mock_db_session, sample_rule_performance
    ):
...
class TestDiscoverPatterns:
    ...
    async def test_discover_patterns_success(self, prompt_service, mock_db_session):
        ...
        mock_ml_service.discover_patterns.assert_called_once_with(
            db_session=mock_db_session, min_effectiveness=0.7, min_support=5
        )
...
class TestPhase3HelperMethods:
    ...
    async def test_store_ml_optimization_results(self, prompt_service, mock_db_session):
        ...
        with (
            ...
            patch(
                "prompt_improver.services.prompt_improvement.MLModelPerformance"
            ) as mock_perf_class,
        ):
            mock_datetime.utcnow.return_value = datetime(2025, 1, 1)
    ...
    async def test_create_ab_experiments_from_patterns(
        ...
        experiment_result.scalar_one_or_none.return_value = (
            None  # No existing experiments
        )
        mock_db_session.execute.return_value = experiment_result

        with (
            patch(
                "prompt_improver.services.prompt_improvement.ABExperiment"
            ) as mock_ab_class,
            patch(
                "prompt_improver.services.prompt_improvement.datetime"
            ) as mock_datetime,
    ...
    async def test_store_pattern_discovery_results(
        ...
        with (
            patch(
                "prompt_improver.services.prompt_improvement.DiscoveredPattern"
            ...
            await prompt_service._store_pattern_discovery_results(
                mock_db_session, discovery_result
            )
