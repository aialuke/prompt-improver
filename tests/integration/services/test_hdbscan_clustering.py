...
from prompt_improver.services.advanced_pattern_discovery import AdvancedPatternDiscovery, FrequentPattern, PatternCluster
...
class TestHDBSCANClusteringEnhancements:
    ...
    async def real_performance_data(self, real_db_session):
        ...
        rule_metadata = [RuleMetadata(rule_id='clarity_rule_1', rule_name='Clarity Enhancement Rule 1', description='Improves clarity through weight adjustment', category='clarity', default_parameters={'weight': 0.8, 'threshold': 0.7}, created_at=aware_utc_now(), updated_at=aware_utc_now()), RuleMetadata(rule_id='clarity_rule_2', rule_name='Clarity Enhancement Rule 2', description='Improves clarity through threshold adjustment', category='clarity', default_parameters={'weight': 0.9, 'threshold': 0.6}, created_at=aware_utc_now(), updated_at=aware_utc_now()), RuleMetadata(rule_id='specificity_rule_1', rule_name='Specificity Enhancement Rule 1', description='Improves specificity through parameter tuning', category='specificity', default_parameters={'weight': 0.7, 'threshold': 0.8}, created_at=aware_utc_now(), updated_at=aware_utc_now())]
        prompt_sessions = []
        for i in range(60):
            prompt_sessions.append(PromptSession(session_id=f'session_{i}', original_prompt=f'Original prompt {i}', improved_prompt=f'Improved prompt {i}', user_context={'test': True}, quality_score=0.8, improvement_score=0.75, confidence_level=0.9, created_at=aware_utc_now() - timedelta(days=30, hours=i), updated_at=aware_utc_now() - timedelta(days=30, hours=i)))
        performance_data = []
        ...
        for i in range(60):
            if i < 20:
                ...
                rule_id = f'clarity_rule_{i % 2 + 1}'
            performance_data.append(RulePerformance(rule_id=rule_id, session_id=f'session_{i}', improvement_score=max(0.0, min(1.0, effectiveness)), execution_time_ms=max(50, execution_time), confidence_level=np.random.uniform(0.6, 0.9), parameters_used={'weight': np.random.uniform(0.5, 1.0), 'threshold': np.random.uniform(0.5, 0.9)}, created_at=base_time + timedelta(hours=i)))
    ...
    @pytest.mark.asyncio
    async def test_hdbscan_cluster_discovery_performance(self, discovery_service, real_db_session, real_performance_data):
        """Test HDBSCAN cluster discovery performance with real data."""
        result = await discovery_service.discover_advanced_patterns(real_db_session, min_effectiveness=0.7, pattern_types=['parameter'])
    ...
    @pytest.mark.asyncio
    @given(effectiveness_scores=st.lists(st.floats(min_value=0.3, max_value=1.0), min_size=15, max_size=50))
    ...
    async def test_hdbscan_cluster_properties(self, effectiveness_scores, discovery_service, real_db_session):
        ...
        for i, score in enumerate(effectiveness_scores):
            property_test_data.append(RulePerformance(rule_id=f'prop_rule_{i}', session_id=f'prop_session_{i}', effectiveness_score=max(0.0, min(1.0, score)), execution_time_ms=np.random.randint(50, 300), memory_usage_mb=np.random.uniform(10, 50), context_relevance=np.random.uniform(0.5, 1.0), created_at=base_time + timedelta(minutes=i), updated_at=base_time + timedelta(minutes=i)))
        for performance in property_test_data:
            real_db_session.add(performance)
        await real_db_session.commit()
        result = await discovery_service.discover_advanced_patterns(real_db_session, min_effectiveness=0.5, pattern_types=['parameter'])
        if result['status'] == 'success' and 'parameter_patterns' in result['pattern_discovery']:
            param_patterns = result['pattern_discovery']['parameter_patterns']
            if 'clusters' in param_patterns:
                for cluster in param_patterns['clusters']:
                    assert cluster.effectiveness_range[0] >= 0.0
                    assert cluster.effectiveness_range[1] <= 1.0
                    assert cluster.effectiveness_range[0] <= cluster.effectiveness_range[1]
            if 'clusters' in param_patterns:
                for cluster in param_patterns['clusters']:
    ...
    @pytest.mark.asyncio
    async def test_hdbscan_minimum_cluster_size_parameter(self, discovery_service, real_db_session, real_performance_data):
        """Test HDBSCAN min_cluster_size parameter affects clustering results with real data."""
        discovery_service.min_cluster_size = 5
        result_small = await discovery_service.discover_advanced_patterns(real_db_session, pattern_types=['parameter'])
        discovery_service.min_cluster_size = 15
        result_large = await discovery_service.discover_advanced_patterns(real_db_session, pattern_types=['parameter'])
        if result_small['status'] == 'success' and result_large['status'] == 'success':
            small_clusters = result_small['pattern_discovery'].get('parameter_patterns', {}).get('clusters', [])
            large_clusters = result_large['pattern_discovery'].get('parameter_patterns', {}).get('clusters', [])
            assert len(small_clusters) >= len(large_clusters)
        assert result_small['total_samples'] == 60
        assert result_large['total_samples'] == 60

    @pytest.mark.asyncio
    async def test_hdbscan_vs_dbscan_performance(self, discovery_service, real_db_session):
        ...
        for i in range(140):
            ...
            additional_data.append(RulePerformance(rule_id=rule_id, session_id=f'perf_session_{i}', effectiveness_score=effectiveness, execution_time_ms=execution_time, memory_usage_mb=np.random.uniform(10, 100), context_relevance=np.random.uniform(0.3, 1.0), created_at=base_time + timedelta(hours=i), updated_at=base_time + timedelta(hours=i)))
        for performance in additional_data:
            real_db_session.add(performance)
        await real_db_session.commit()
        hdbscan_result = await discovery_service.discover_advanced_patterns(real_db_session, pattern_types=['parameter'])
    ...
    async def test_hdbscan_outlier_detection(self, discovery_service, real_db_session, real_performance_data):
        ...
        for i in range(5):
            outlier_data.append(RulePerformance(rule_id=f'outlier_rule_{i}', session_id=f'outlier_session_{i}', effectiveness_score=0.98 + np.random.normal(0, 0.01), execution_time_ms=25 + np.random.normal(0, 5), memory_usage_mb=5 + np.random.uniform(0, 2), context_relevance=0.99 + np.random.normal(0, 0.005), created_at=base_time + timedelta(minutes=i), updated_at=base_time + timedelta(minutes=i)))
        for performance in outlier_data:
            real_db_session.add(performance)
        await real_db_session.commit()
        result = await discovery_service.discover_advanced_patterns(real_db_session, pattern_types=['parameter'])
        if result['status'] == 'success' and 'parameter_patterns' in result['pattern_discovery']:
            param_patterns = result['pattern_discovery']['parameter_patterns']
...
class HDBSCANStateMachine(RuleBasedStateMachine):
    ...
    @rule(target=patterns, effectiveness=st.floats(min_value=0.3, max_value=1.0), weight=st.floats(min_value=0.5, max_value=1.0))
    def add_pattern(self, effectiveness, weight):
        """Add a pattern to the current dataset."""
        pattern = {'rule_id': f'rule_{len(self.current_patterns)}', 'parameters': {'weight': weight, 'threshold': 0.7}, 'effectiveness': effectiveness}
...
class TestHDBSCANEdgeCases:
    ...
    async def test_insufficient_data_handling(self, minimal_discovery_service, real_db_session):
        ...
        minimal_data = [RulePerformance(rule_id='rule_1', session_id='session_1', effectiveness_score=0.85, execution_time_ms=150, memory_usage_mb=25, context_relevance=0.8, created_at=aware_utc_now(), updated_at=aware_utc_now()), RulePerformance(rule_id='rule_2', session_id='session_2', effectiveness_score=0.78, execution_time_ms=200, memory_usage_mb=30, context_relevance=0.75, created_at=aware_utc_now(), updated_at=aware_utc_now())]
        for performance in minimal_data:
            real_db_session.add(performance)
        await real_db_session.commit()
        result = await minimal_discovery_service.discover_advanced_patterns(real_db_session, min_support=5, pattern_types=['parameter'])
    ...
    async def test_hdbscan_unavailable_fallback(self, minimal_discovery_service, real_db_session):
        ...
        fallback_data = []
        for i in range(15):
            fallback_data.append(RulePerformance(rule_id=f'fallback_rule_{i}', session_id=f'fallback_session_{i}', effectiveness_score=0.8 + np.random.normal(0, 0.1), execution_time_ms=150 + np.random.normal(0, 30), memory_usage_mb=25 + np.random.uniform(0, 10), context_relevance=0.8 + np.random.uniform(-0.1, 0.1), created_at=aware_utc_now() - timedelta(hours=i), updated_at=aware_utc_now() - timedelta(hours=i)))
        for performance in fallback_data:
            real_db_session.add(performance)
        await real_db_session.commit()
        with patch('prompt_improver.services.advanced_pattern_discovery.HDBSCAN_AVAILABLE', False):
            result = await minimal_discovery_service.discover_advanced_patterns(real_db_session, pattern_types=['parameter'])
        assert result['status'] == 'success'
        assert 'parameter_patterns' in result['pattern_discovery']
