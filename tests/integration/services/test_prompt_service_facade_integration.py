...
class MockPromptRule(BasePromptRule):
    ...
    async def check(self, prompt: str, context: dict[str, Any]) -> bool:
        """Check if rule should be applied."""
        return True

    def to_llm_instruction(self) -> str:
        """Convert rule to LLM instruction."""
        return f"Apply {self.name} improvement rule"
