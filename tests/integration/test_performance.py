...
class PerformanceTester:
    ...
    async def test_improve_prompt_performance(
        ...
        print(
            f"🧪 Testing real prompt improvement performance ({iterations} iterations)"
        )
        test_prompts = [
    ...
    async def run_comprehensive_test(self) -> dict[str, Any]:
        ...
        for tool_name, stats in results.items():
            ...
            print(
                f"   P95 Response Time:     {stats.get('p95_response_time', 'N/A'):.2f}ms"
            )
            print(
                f"   P99 Response Time:     {stats.get('p99_response_time', 'N/A'):.2f}ms"
            )
            print(f"   Standard Deviation:    {stats['std_dev']:.2f}ms")
            print(f"   Success Rate:          {stats['success_rate']:.1f}%")
            print(
                f"   Under 500ms Rate:      {stats.get('under_500ms_rate', 'N/A'):.1f}%"
            )
            print(
                f"   Under 200ms Rate:      {stats.get('under_200ms_rate', 'N/A'):.1f}%"
            )
            if (
                stats["avg_response_time"] < 1000
                and stats.get("under_500ms_rate", 0) >= 70
            ):
        ...
        overall_under_500 = [
            stats.get("under_500ms_rate", 0) for stats in results.values()
        ]
        print("\n🎯 OVERALL REAL BEHAVIOR ASSESSMENT:")
        if avg_response_times:
            overall_avg = statistics.mean(avg_response_times)
            overall_under_500_avg = statistics.mean([
                stats.get("under_500ms_rate", 0) for stats in results.values()
            ])
            ...
            if overall_avg < 1000 and overall_under_500_avg >= 70:
                ...
                print(
                    "   🎉 Real system response times validated under acceptable thresholds"
                )
            elif overall_avg < 1500:
                print(
                    "   ⚠️  REAL BEHAVIOR PERFORMANCE ACCEPTABLE BUT NEEDS OPTIMIZATION"
                )
            else:
                print(
                    "   ❌ REAL BEHAVIOR PERFORMANCE TARGET NEEDS SIGNIFICANT OPTIMIZATION"
                )
        else:
...
class TestBenchmarkPerformance:
    ...
    def test_benchmark_improve_prompt_sync_wrapper(self, benchmark):
        ...
        result = benchmark.pedantic(
            sync_improve_prompt, iterations=3, rounds=2, warmup_rounds=1
        )
    ...
    def test_benchmark_store_prompt_performance(self, benchmark):
        ...
        result = benchmark.pedantic(
            sync_store_prompt, iterations=3, rounds=2, warmup_rounds=1
        )
...
class TestStatisticalPerformanceValidation:
    ...
    async def test_response_time_distribution_analysis(self, stats_services):
        ...
        assert mean_time < 2000, (
            f"Mean response time {mean_time}ms exceeds realistic target"
        )
        assert median_time < 1500, (
            f"Median response time {median_time}ms exceeds realistic target"
        )
        ...
        assert outlier_rate <= 0.2, (
            f"Too many outliers: {outlier_rate:.2%} of responses - real behavior should be more consistent"
        )
        sorted_times = sorted(response_times)
        p95 = sorted_times[int(0.95 * len(sorted_times))]
        p99 = sorted_times[int(0.99 * len(sorted_times))]
        assert p95 < mean_time * 2, (
            f"P95 {p95}ms is too high compared to mean {mean_time}ms"
        )
        assert p99 < mean_time * 3, (
            f"P99 {p99}ms is too high compared to mean {mean_time}ms"
        )
    ...
    async def test_performance_scaling_properties(
        ...
        if concurrent_operations > 1:
            ...
            assert efficiency_factor < 2.0, (
                f"Poor scaling: {avg_time_per_operation:.1f}ms per operation with {concurrent_operations} concurrent operations"
            )
...
class TestPerformanceRegression:
    ...
    async def test_performance_baseline_validation(
        self, test_config, regression_services
    ):
        """Validate real performance against established baselines."""
        target_response_time = test_config.get("performance", {}).get(
    ...
    async def test_memory_usage_stability(self, regression_services):
        ...
        assert total_growth < 150, (
            f"Total memory growth {total_growth:.1f}MB exceeds threshold for real behavior"
        )
