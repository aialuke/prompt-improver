...
from prompt_improver.ml.automl.orchestrator import AutoMLConfig, AutoMLOrchestrator, create_automl_orchestrator
from prompt_improver.ml.evaluation.experiment_orchestrator import ExperimentArm, ExperimentConfiguration, ExperimentOrchestrator, ExperimentType
from prompt_improver.ml.lifecycle.ml_platform_integration import MLPlatformIntegration, create_ml_platform
...
class MLBackgroundTaskIntegrationValidator:
    ...
    async def test_automl_orchestrator_integration(self) -> dict[str, Any]:
        ...
        try:
            config = AutoMLConfig(study_name='test_automl_integration', n_trials=5, timeout=30)
            orchestrator = await create_automl_orchestrator(config=config, task_manager=self.task_manager)
            initial_status = await orchestrator.get_optimization_status()
            task_stats = self.task_manager.get_statistics()
            result = {'status': 'success', 'initial_status': initial_status, 'task_manager_integration': {'task_manager_type': type(orchestrator.task_manager).__name__, 'total_tasks_managed': task_stats.get('total_submitted', 0), 'background_task_integration': True}, 'test_time': time.time()}
    ...
    async def test_experiment_orchestrator_integration(self) -> dict[str, Any]:
        ...
        try:
            ...
            orchestrator = ExperimentOrchestrator(db_session=mock_db_session, task_manager=self.task_manager)
            orchestrator_status = await orchestrator.get_orchestrator_status()
            result = {'status': 'success', 'orchestrator_status': orchestrator_status, 'task_manager_integration': orchestrator_status['task_management'], 'test_time': time.time()}
    ...
    async def test_ml_platform_integration(self) -> dict[str, Any]:
        ...
        try:
            platform = MLPlatformIntegration(storage_path=Path('./test_ml_platform'), enable_distributed=False, enable_monitoring=True, task_manager=self.task_manager)
            success = await platform.initialize_platform()
            task_stats = self.task_manager.get_statistics()
            result = {'status': 'success' if success else 'failed', 'platform_initialization': success, 'platform_status': platform.platform_status.value, 'task_manager_integration': {'task_manager_type': type(platform.task_manager).__name__, 'background_tasks_started': len(platform._background_task_ids), 'total_tasks_managed': task_stats.get('total_submitted', 0)}, 'test_time': time.time()}
    ...
    async def test_task_lifecycle_management(self) -> dict[str, Any]:
        ...
        try:
            ...
            task_id = await self.task_manager.submit_enhanced_task(task_id='lifecycle_test_task', coroutine=test_task(), priority=TaskPriority.NORMAL, timeout=10, tags={'type': 'lifecycle_test'})
            ...
            test_result = {'status': 'success', 'task_lifecycle': {'task_id': task_id, 'initial_status': initial_status['status'] if initial_status else 'unknown', 'final_status': final_status['status'] if final_status else 'unknown', 'task_result': result, 'execution_time': final_status.get('metrics', {}).get('total_duration', 0) if final_status else 0}, 'task_manager_stats': {'total_submitted': final_stats.get('total_submitted', 0), 'total_completed': final_stats.get('total_completed', 0), 'success_rate': final_stats.get('total_completed', 0) / max(1, final_stats.get('total_submitted', 0))}, 'test_time': time.time()}
    ...
    async def test_task_cancellation_and_retry(self) -> dict[str, Any]:
        ...
        try:
            ...
            task_id = await self.task_manager.submit_enhanced_task(task_id='cancellation_test_task', coroutine=long_running_task(), priority=TaskPriority.NORMAL, timeout=60, tags={'type': 'cancellation_test'})
            ...
            result = {'status': 'success', 'cancellation_test': {'task_id': task_id, 'cancellation_success': cancellation_success, 'final_status': cancelled_status['status'] if cancelled_status else 'unknown'}, 'test_time': time.time()}
    ...
    async def run_comprehensive_validation(self) -> dict[str, Any]:
        ...
        self.test_results = {'automl_orchestrator': await self.test_automl_orchestrator_integration(), 'experiment_orchestrator': await self.test_experiment_orchestrator_integration(), 'ml_platform': await self.test_ml_platform_integration(), 'task_lifecycle': await self.test_task_lifecycle_management(), 'task_cancellation': await self.test_task_cancellation_and_retry()}
        total_tests = len(self.test_results)
        passed_tests = sum((1 for result in self.test_results.values() if result['status'] == 'success'))
        failed_tests = total_tests - passed_tests
        final_stats = self.task_manager.get_statistics()
        comprehensive_result = {'validation_summary': {'total_tests': total_tests, 'passed_tests': passed_tests, 'failed_tests': failed_tests, 'success_rate': passed_tests / total_tests, 'validation_time': time.time() - start_time}, 'task_manager_performance': {'total_tasks_submitted': final_stats.get('total_submitted', 0), 'total_tasks_completed': final_stats.get('total_completed', 0), 'total_tasks_failed': final_stats.get('total_failed', 0), 'overall_success_rate': final_stats.get('total_completed', 0) / max(1, final_stats.get('total_submitted', 0)), 'task_manager_type': type(self.task_manager).__name__}, 'individual_test_results': self.test_results, 'integration_status': 'SUCCESS' if failed_tests == 0 else 'PARTIAL' if passed_tests > 0 else 'FAILED'}
        if failed_tests == 0:
            logger.info('🎉 ALL TESTS PASSED! %s/%s ML background task integrations validated successfully', passed_tests, total_tests)
        else:
            logger.warning('⚠️  SOME TESTS FAILED: %s/%s tests passed, %s failed', passed_tests, total_tests, failed_tests)
