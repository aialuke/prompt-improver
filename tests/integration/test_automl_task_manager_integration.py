...
async def test_automl_background_integration():
    ...
    try:
        ...
        from src.prompt_improver.core.events.ml_event_bus import (
            MLEventBus,
            MLEventType
        )
        ...
        task_manager = EnhancedBackgroundTaskManager(max_concurrent_tasks=5, enable_metrics=True)
        ...
        assert orchestrator.task_manager is task_manager, "Task manager not properly integrated"
        ...
        tasks_submitted = final_stats.get("total_submitted", 0) - initial_stats.get("total_submitted", 0)
        assert tasks_submitted > 0, f"No tasks were submitted to task manager (submitted: {tasks_submitted})"
        logger.info(f"✅ {tasks_submitted} tasks submitted to task manager")
        
        # Validate task completion
        tasks_completed = final_stats.get("total_completed", 0) - initial_stats.get("total_completed", 0)
        ...
        training_started_events = [e for e in captured_events if e['type'] == 'ml.training.started']
        ...
        completion_events = [e for e in captured_events if e['type'] in ['ml.training.completed', 'ml.training.failed']]
        ...
        assert "background_task_integration" in status, "Background task integration info not in status"
        assert status["background_task_integration"]["task_manager_type"] == "EnhancedBackgroundTaskManager"
        ...
        assert task_manager_tasks > 0, "No tasks processed through task manager - possible direct task creation"
        
        logger.info(f"✅ All background tasks processed through task manager: {task_manager_tasks} tasks")
        ...
        assert "execution_time" in optimization_result, "Optimization result missing execution time"
        
        # Validate reasonable execution time (should complete within timeout)
        assert execution_time < config.timeout, f"Execution time {execution_time}s exceeded timeout {config.timeout}s"
        
        logger.info(f"✅ AutoML functionality preserved, execution time: {execution_time:.2f}s")
        ...
        summary = {
            ...
            "metrics": {
                ...
                "task_manager_integration": status["background_task_integration"]["task_manager_type"]
            }
        }
        ...
        assert all_criteria_met, f"Not all success criteria met: {summary['success_criteria']}"
