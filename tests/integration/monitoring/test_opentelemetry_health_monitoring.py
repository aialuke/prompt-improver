...
class TestOpenTelemetryHealthMonitoring:
    ...
    def test_health_check_metrics_collection(self):
        ...
        self.http_metrics.record_request(
            method="GET", endpoint="/health", status_code=200, duration_ms=5.2
        )
        self.http_metrics.record_request(
            method="GET", endpoint="/health/ready", status_code=200, duration_ms=3.8
        )
        self.http_metrics.record_request(
            method="GET", endpoint="/health/live", status_code=200, duration_ms=2.1
        )
    ...
    def test_database_health_metrics(self):
        ...
        self.database_metrics.record_query(
            operation="SELECT", table="pg_stat_activity", duration_ms=2.5, success=True
        )
        self.database_metrics.record_query(
            operation="SELECT", table="health_status", duration_ms=1.8, success=True
        )
    ...
    def test_service_availability_metrics(self):
        ...
        for service in services:
            self.business_metrics.record_feature_usage(
                feature=f"{service}_health_check", user_tier="system"
            )
    ...
    async def test_health_monitoring_workflow(self):
        ...
        self.database_metrics.record_query(
            operation="SELECT", table="health_check", duration_ms=3.2, success=True
        )
        dependencies = ["postgresql", "redis", "ml_models"]
        for dep in dependencies:
            self.business_metrics.record_feature_usage(
                feature=f"{dep}_dependency_check", user_tier="system"
            )
        self.business_metrics.record_session(
            user_id="health_monitor_system", session_duration_s=time.time() - start_time
        )
    ...
    def test_health_check_failure_scenarios(self):
        ...
        self.http_metrics.record_request(
            method="GET", endpoint="/health", status_code=503, duration_ms=1000.0
        )
        self.database_metrics.record_query(
            operation="SELECT", table="health_check", duration_ms=5000.0, success=False
        )
        self.business_metrics.record_feature_usage(
            feature="database_health_check_failure", user_tier="system"
        )
        assert isinstance(self.http_metrics, HttpMetrics)
        assert isinstance(self.database_metrics, DatabaseMetrics)
        assert isinstance(self.business_metrics, BusinessMetrics)
    ...
    def test_health_metrics_performance_monitoring(self):
        ...
        assert recording_duration < 100, (
            f"Health metrics recording too slow: {recording_duration}ms"
        )
        print(
            f"✅ Health metrics performance test completed in {recording_duration:.1f}ms"
        )

    def test_health_alerting_integration(self):
        """Test health alerting integration with OpenTelemetry."""
        self.http_metrics.record_request(
            method="GET", endpoint="/health", status_code=200, duration_ms=500.0
        )
        self.database_metrics.record_query(
            operation="SELECT", table="health_check", duration_ms=100.0, success=True
        )
        self.business_metrics.record_feature_usage(
            feature="performance_degraded_alert", user_tier="system"
        )
        assert isinstance(self.http_metrics, HttpMetrics)
        assert isinstance(self.database_metrics, DatabaseMetrics)
        assert isinstance(self.business_metrics, BusinessMetrics)
    ...
    def test_health_monitoring_sla_compliance(self):
        ...
        sla_compliant_requests = sum(
            1 for i in range(100) if 50.0 + i * 1.0 < sla_threshold_ms
        )
        sla_compliance_rate = sla_compliant_requests / 100.0
        self.business_metrics.record_feature_usage(
            feature="sla_compliance_check", user_tier="sla_monitor"
        )
        assert isinstance(self.http_metrics, HttpMetrics)
        assert isinstance(self.business_metrics, BusinessMetrics)
        print(
            f"✅ SLA compliance metrics recorded: {sla_compliance_rate:.1%} compliance rate"
        )
        ...
        for minute in range(5):
            for _check in range(2):
                self.http_metrics.record_request(
                    ...
                    endpoint="/health",
                    status_code=200,
                    duration_ms=3.0 + minute * 0.5,
                )
        self.http_metrics.record_request(
            method="GET", endpoint="/health/deep", status_code=200, duration_ms=25.0
        )
        self.database_metrics.record_query(
            operation="SELECT", table="pg_stat_database", duration_ms=5.0, success=True
        )
        assert isinstance(self.http_metrics, HttpMetrics)
        assert isinstance(self.database_metrics, DatabaseMetrics)
