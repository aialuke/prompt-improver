...
class TestDatabaseAccessMigrationValidation:
    ...
    async def test_health_repository_comprehensive_operations(self, health_repository):
        ...
        print("✅ Health repository comprehensive operations validated")
    ...
    def test_no_direct_database_imports(self):
        ...
        violations = [f"DatabaseHealthChecker contains: {pattern}" for pattern in violation_patterns if pattern in checker_source]

        # Check OrchestrationService
        violations.extend(f"OrchestrationService contains: {pattern}" for pattern in violation_patterns if pattern in manager_source)

        # Check BaselineCollector
        violations.extend(f"BaselineCollector contains: {pattern}" for pattern in violation_patterns if pattern in collector_source)
