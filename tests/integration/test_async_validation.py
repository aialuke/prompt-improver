...
class TestAsyncExecution:
    ...
    async def test_mcp_improve_prompt_async(self, test_db_session):
        ...
        with patch('prompt_improver.database.get_session') as mock_get_session, patch('prompt_improver.core.services.prompt_improvement.get_session') as mock_get_session_wrapper:
            ...
            result = await server_instance._improve_prompt_impl(prompt=test_prompt, context={'domain': 'testing'}, session_id='async_test', rate_limit_remaining=None)
    ...
    async def test_real_prompt_improvement_behavior(self, prompt, test_db_session):
        ...
        with patch('prompt_improver.database.get_session') as mock_get_session, patch('prompt_improver.core.services.prompt_improvement.get_session') as mock_get_session_wrapper:
            ...
            from prompt_improver.mcp_server import APESMCPServer
            server_instance = APESMCPServer()
            result = await server_instance._improve_prompt_impl(prompt=prompt, context={'domain': 'testing'}, session_id='property_test', rate_limit_remaining=None)
            assert 'improved_prompt' in result
            assert 'original_prompt' in result
            assert 'session_id' in result
            assert 'applied_rules' in result
    ...
    async def test_concurrent_async_operations(self):
        ...
        async def dummy_operation(delay):
            ...
            return delay
        start_time = asyncio.get_event_loop().time()
        results = await asyncio.gather(dummy_operation(0.001), dummy_operation(0.002), dummy_operation(0.003))
        end_time = asyncio.get_event_loop().time()
        execution_time = (end_time - start_time) * 1000
        assert results == [0.001, 0.002, 0.003]
        sequential_time = sum(results) * 1000
...
class TestAsyncPropertyBasedTesting:
    ...
    @given(operation_count=st.integers(min_value=1, max_value=10), delay_ms=st.floats(min_value=0.001, max_value=0.01))
    async def test_concurrent_operations_scaling(self, operation_count, delay_ms):
        ...
        if operation_count > 1:
            ...
            assert concurrent_time < expected_sequential_time * 0.8, f'Concurrent time {concurrent_time}ms not efficient vs sequential {expected_sequential_time}ms'
    ...
    async def test_async_sleep_accuracy(self, sleep_duration):
        ...
        assert actual_duration <= sleep_duration * 1.5, f'Sleep took {actual_duration:.4f}s, expected ~{sleep_duration:.4f}s'

    @given(tasks=st.lists(st.floats(min_value=0.001, max_value=0.01), min_size=2, max_size=8))
    async def test_asyncio_gather_order_preservation(self, tasks):
        ...
        async def timed_operation(delay, task_id):
            await asyncio.sleep(delay)
            return task_id
        task_coroutines = [timed_operation(delay, idx) for idx, delay in enumerate(tasks)]
        results = await asyncio.gather(*task_coroutines)
        expected_order = list(range(len(tasks)))
        assert results == expected_order, f'Order not preserved: got {results}, expected {expected_order}'
    ...
    async def test_event_loop_context_preservation(self, concurrent_count):
        ...
        results = await asyncio.gather(*[context_checker(i) for i in range(concurrent_count)])
