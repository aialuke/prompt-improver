...
class ContextCacheManagerIntegrationTest:
    ...
    async def test_orchestrator_component_loading(self):
        ...
        try:
            ...
            loaded_component = await loader.load_component(
                "context_cache_manager", ComponentTier.TIER_2
            )
            self.log_test(
                "Component loads through orchestrator", loaded_component is not None
            )
            if loaded_component:
                from prompt_improver.ml.learning.algorithms.context_cache_manager import (
                    ContextCacheManager,
                )

                self.log_test(
                    "Correct class loaded",
                    loaded_component.component_class == ContextCacheManager,
                )
    ...
    async def test_cache_functionality(self):
        ...
        try:
            ...
            cache_manager = ContextCacheManager(
                linguistic_cache_size=10, domain_cache_size=5, ttl_seconds=2
            )
            ...
            self.log_test(
                "Linguistic cache returns correct data",
                result1 == {"features": [1, 2, 3]},
            )
            cache_manager.update_domain_cache(
                "domain_key1", {"domain": "technical", "confidence": 0.9}
            )
            domain_result = cache_manager.get_domain_cache("domain_key1")
            self.log_test(
                "Domain cache stores and retrieves", domain_result is not None
            )
            stats = cache_manager.get_cache_stats()
            self.log_test(
                "Cache statistics available", "linguistic_cache_size" in stats
            )
            ...
            self.log_test(
                "Manual expiration works", "linguistic_expired" in expire_stats
            )
            return True
        except Exception as e:
    ...
    async def test_cache_performance_and_metrics(self):
        ...
        try:
            ...
            cache_manager = ContextCacheManager(
                linguistic_cache_size=100, domain_cache_size=50, ttl_seconds=300
            )
            start_time = time.time()
            for i in range(50):
                cache_manager.update_linguistic_cache(
                    f"perf_key_{i}", {"data": f"value_{i}"}
                )
                cache_manager.update_domain_cache(
                    f"domain_{i}", {"domain": f"type_{i}"}
                )
            for i in range(50):
            ...
            stats = cache_manager.get_cache_stats()
            self.log_test(
                "Cache utilization calculated",
                stats["linguistic_cache_utilization"] > 0,
            )
            ...
            self.log_test(
                "Caches empty after clearing",
                post_clear_stats["linguistic_cache_size"] == 0,
            )
            return True
        except Exception as e:
    ...
    async def test_integration_with_context_learner(self):
        ...
        try:
            ...
            cache_manager.update_linguistic_cache(
                "sample_text_hash", linguistic_features
            )
            cached_features = cache_manager.get_linguistic_cache("sample_text_hash")
            self.log_test(
                "Linguistic features cached and retrieved",
                cached_features == linguistic_features,
            )
            domain_info = {
                "domain": "technical",
                "confidence": 0.95,
            ...
            self.log_test(
                "Domain info cached and retrieved", cached_domain == domain_info
            )
            return True
        except Exception as e:
