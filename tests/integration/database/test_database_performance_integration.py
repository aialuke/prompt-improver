...
import asyncpg
import pytest
from prompt_improver.database.performance_monitor import DatabasePerformanceMonitor, get_performance_monitor
...
class TestDatabasePerformanceIntegration:
    ...
    async def db_connection(self):
        """Create database connection using UnifiedConnectionManager for real behavior testing."""
        try:
    ...
    async def test_performance_degradation_detection(self, performance_monitor, event_bus):
        ...
        performance_events = [e for e in emitted_events if e.event_type in [EventType.DATABASE_PERFORMANCE_DEGRADED, EventType.DATABASE_CACHE_HIT_RATIO_LOW, EventType.DATABASE_SLOW_QUERY_DETECTED]]
    ...
    async def test_continuous_monitoring_with_real_intervals(self, performance_monitor):
        ...
        monitoring_task = asyncio.create_task(performance_monitor.start_monitoring(interval_seconds=1))
