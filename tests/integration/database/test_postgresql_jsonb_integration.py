...
from src.prompt_improver.metrics.api_metrics import APIUsageMetric, AuthenticationMethod, AuthenticationMetric, EndpointCategory, HTTPMethod, RateLimitMetric, UserJourneyMetric, UserJourneyStage
try:
    import asyncpg
...
class PostgreSQLJSONBTester:
    ...
    def serialize_metric_to_jsonb(self, metric) -> dict[str, Any]:
        ...
        if isinstance(metric, APIUsageMetric):
            return {'endpoint': metric.endpoint, 'method': metric.method.value, 'category': metric.category.value, 'status_code': metric.status_code, 'response_time_ms': metric.response_time_ms, 'request_size_bytes': metric.request_size_bytes, 'response_size_bytes': metric.response_size_bytes, 'user_id': metric.user_id, 'session_id': metric.session_id, 'ip_address': metric.ip_address, 'user_agent': metric.user_agent, 'timestamp': metric.timestamp.isoformat(), 'query_parameters_count': metric.query_parameters_count, 'payload_type': metric.payload_type, 'rate_limited': metric.rate_limited, 'cache_hit': metric.cache_hit, 'authentication_method': metric.authentication_method.value, 'api_version': metric.api_version}
        if isinstance(metric, UserJourneyMetric):
            return {'user_id': metric.user_id, 'session_id': metric.session_id, 'journey_stage': metric.journey_stage.value, 'event_type': metric.event_type, 'endpoint': metric.endpoint, 'success': metric.success, 'conversion_value': metric.conversion_value, 'time_to_action_seconds': metric.time_to_action_seconds, 'previous_stage': metric.previous_stage.value if metric.previous_stage else None, 'feature_flags_active': metric.feature_flags_active, 'cohort_id': metric.cohort_id, 'timestamp': metric.timestamp.isoformat(), 'metadata': metric.metadata}
    ...
    async def test_jsonb_serialization(self):
        ...
        try:
            api_metric = APIUsageMetric(endpoint='/api/v1/test', method=HTTPMethod.POST, category=EndpointCategory.PROMPT_IMPROVEMENT, status_code=200, response_time_ms=150.5, request_size_bytes=1024, response_size_bytes=2048, user_id='test_user', session_id='test_session', ip_address='192.168.1.1', user_agent='TestAgent/1.0', timestamp=datetime.now(UTC), query_parameters_count=3, payload_type='application/json', rate_limited=False, cache_hit=True, authentication_method=AuthenticationMethod.JWT_TOKEN, api_version='v1')
            journey_metric = UserJourneyMetric(user_id='test_user', session_id='test_session', journey_stage=UserJourneyStage.FIRST_USE, event_type='test_event', endpoint='/api/v1/test', success=True, conversion_value=10.0, time_to_action_seconds=30.0, previous_stage=UserJourneyStage.ONBOARDING, feature_flags_active=['test_flag'], cohort_id='test_cohort', timestamp=datetime.now(UTC), metadata={'test': 'data', 'nested': {'key': 'value'}})
    ...
    async def test_database_storage_retrieval(self):
        ...
        try:
            metrics = []
            for i in range(5):
                api_metric = APIUsageMetric(endpoint=f'/api/v1/test_{i}', method=HTTPMethod.GET if i % 2 == 0 else HTTPMethod.POST, category=EndpointCategory.PROMPT_IMPROVEMENT, status_code=200, response_time_ms=100.0 + i * 10, request_size_bytes=500 + i * 100, response_size_bytes=1000 + i * 200, user_id=f'user_{i}', session_id=f'session_{i}', ip_address='192.168.1.1', user_agent='TestAgent/1.0', timestamp=datetime.now(UTC), query_parameters_count=i, payload_type='application/json', rate_limited=False, cache_hit=i % 2 == 0, authentication_method=AuthenticationMethod.JWT_TOKEN, api_version='v1')
                metrics.append(('api_usage', api_metric))
            async with self.connection_manager.get_connection() as conn:
                for metric_type, metric in metrics:
                    metric_data = self.serialize_metric_to_jsonb(metric)
                    await conn.execute('\n                        INSERT INTO test_api_metrics \n                        (metric_type, metric_data, endpoint, user_id, session_id)\n                        VALUES ($1, $2, $3, $4, $5)\n                    ', metric_type, json.dumps(metric_data), metric.endpoint, metric.user_id, metric.session_id)
                await conn.commit()
            async with self.connection_manager.get_connection() as conn:
    ...
    async def test_jsonb_indexing_performance(self):
        ...
        try:
            async with self.connection_manager.get_connection() as conn:
                for i in range(100):
                    api_metric = APIUsageMetric(endpoint=f'/api/v1/perf_test_{i % 10}', method=HTTPMethod.GET if i % 2 == 0 else HTTPMethod.POST, category=EndpointCategory.PROMPT_IMPROVEMENT, status_code=200 if i % 10 != 9 else 500, response_time_ms=100.0 + i % 50 * 5, request_size_bytes=500 + i * 10, response_size_bytes=1000 + i * 20, user_id=f'perf_user_{i % 20}', session_id=f'perf_session_{i}', ip_address='192.168.1.1', user_agent='PerfTestAgent/1.0', timestamp=datetime.now(UTC) - timedelta(minutes=i), query_parameters_count=i % 5, payload_type='application/json', rate_limited=i % 20 == 0, cache_hit=i % 3 == 0, authentication_method=AuthenticationMethod.JWT_TOKEN, api_version='v1')
                    metric_data = self.serialize_metric_to_jsonb(api_metric)
                    await conn.execute('\n                        INSERT INTO test_api_metrics \n                        (metric_type, metric_data, endpoint, user_id, session_id)\n                        VALUES ($1, $2, $3, $4, $5)\n                    ', 'api_usage', json.dumps(metric_data), api_metric.endpoint, api_metric.user_id, api_metric.session_id)
                await conn.commit()
            query_tests = [('Simple JSONB field access', "SELECT COUNT(*) FROM test_api_metrics WHERE metric_data->>'method' = 'GET'"), ('JSONB numeric comparison', "SELECT COUNT(*) FROM test_api_metrics WHERE (metric_data->>'status_code')::int = 200"), ('JSONB boolean query', "SELECT COUNT(*) FROM test_api_metrics WHERE (metric_data->>'cache_hit')::boolean = true"), ('Complex JSONB query', "SELECT COUNT(*) FROM test_api_metrics WHERE metric_data->>'method' = 'POST' AND (metric_data->>'status_code')::int >= 400"), ('JSONB aggregation', "SELECT AVG((metric_data->>'response_time_ms')::float) FROM test_api_metrics")]
            performance_results = {}
            async with self.connection_manager.get_connection() as conn:
...
async def main():
    ...
    try:
        ...
        if not setup_success:
            ...
            return False
        tests = [tester.test_jsonb_serialization, tester.test_database_storage_retrieval, tester.test_jsonb_indexing_performance]
        results = []
        for test in tests:
            result = await test()
