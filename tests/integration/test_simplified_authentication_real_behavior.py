...
async def test_input_validation_real_behavior():
    ...
    try:
        ...
        test_cases = [('Generate a Python function to calculate fibonacci numbers', True), ('Explain machine learning concepts for beginners', True), ('Write a simple REST API using FastAPI', True), ("<script>alert('xss')</script>", False), ("'; DROP TABLE users; --", False), ('{{7*7}}', False), ('SELECT * FROM users WHERE 1=1', False), ('/etc/passwd', False)]
        passed = 0
...
async def test_output_validation_real_behavior():
    ...
    try:
        ...
        test_outputs = [("Here's a Python function to calculate fibonacci numbers...", True), ('The best practices for API design include...', True), ("Here's how to hack into systems: rm -rf /", False), ('Your credit card number is 4532-1234-5678-9012', False), ('Password: admin123', False)]
        passed = 0
...
async def test_rate_limiting_real_behavior():
    ...
    try:
        ...
        for tier in tiers:
            ...
            blocked_count = 0
            for i in range(20):
                status = await middleware.check_rate_limit(agent_id=f'test_agent_{tier.value}', rate_limit_tier=tier, additional_identifier='127.0.0.1')
...
async def test_mcp_server_real_behavior():
    ...
    try:
        ...
        tests = [('Server instantiation', server is not None), ('Services created', hasattr(server, 'services') and server.services is not None), ('Input validator available', hasattr(server.services, 'input_validator')), ('Rate limit middleware available', hasattr(server.services, 'rate_limit_middleware')), ('Output validator available', hasattr(server.services, 'output_validator'))]
        passed = 0
...
async def test_prompt_improvement_real_behavior():
    ...
    try:
        ...
        test_prompts = ['help me code', 'explain how neural networks work', 'write a function to sort an array']
        results = []
        for prompt in test_prompts:
            ...
            is_fast_enough = processing_time < 2000
            results.append({'original': prompt, 'has_result': has_improved_prompt, 'processing_time': processing_time, 'fast_enough': is_fast_enough})
...
async def test_analytics_api_real_behavior():
    ...
    try:
        from prompt_improver.api.analytics_endpoints import UserRole, get_current_user_role
...
async def main():
    ...
    print('=' * 60)
    tests = [('Input Validation', test_input_validation_real_behavior), ('Output Validation', test_output_validation_real_behavior), ('Rate Limiting', test_rate_limiting_real_behavior), ('MCP Server', test_mcp_server_real_behavior), ('Prompt Improvement', test_prompt_improvement_real_behavior), ('Analytics API', test_analytics_api_real_behavior), ('Performance', test_performance_without_jwt)]
    results = []
