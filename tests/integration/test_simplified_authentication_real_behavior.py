    ...
    try:
        ...
        for prompt, should_pass in test_cases:
            ...
            if is_safe == should_pass:
                ...
                status = "✗"
            print(
                f"{status} Input: '{prompt[:50]}...' Expected: {should_pass}, Got: {is_safe}"
            )
    ...
    try:
        ...
        for output, should_be_safe in test_outputs:
            ...
            if result.is_safe == should_be_safe:
                ...
                status = "✗"
            print(
                f"{status} Output safety check: Expected {should_be_safe}, Got {result.is_safe}"
            )
        success_rate = passed / total
...
async def test_mcp_server_real_behavior():
    ...
    try:
        ...
        tests = [
            ...
            (
                "Services created",
                hasattr(server, "services") and server.services is not None,
            ),
            ("Input validator available", hasattr(server.services, "input_validator")),
            (
                "Rate limit middleware available",
                hasattr(server.services, "rate_limit_middleware"),
            ),
            (
                "Output validator available",
                hasattr(server.services, "output_validator"),
            ),
        ]
        passed = 0
...
async def test_prompt_improvement_real_behavior():
    ...
    try:
        ...
        for prompt in test_prompts:
            ...
            has_improved_prompt = (
                "improved_prompt" in result and len(result["improved_prompt"]) > 0
            )
            ...
            print(
                f"Prompt: '{prompt[:30]}...' - Result: {has_improved_prompt}, Time: {processing_time:.1f}ms"
            )
...
async def test_performance_without_jwt():
    ...
    try:
        ...
        for _ in range(100):
            ...
            output_result = output_validator.validate_output(
