...
from prompt_improver.ml.optimization.batch import ChunkingStrategy, StreamingBatchConfig, StreamingBatchProcessor
...
def generate_sample_data(num_records: int, file_path: str):
    ...
    with open(file_path, 'w') as f:
        for i in range(num_records):
            record = {'id': f'sample_{i}', 'text': f'This is sample text {i} with some content that needs processing.', 'features': {'length': random.randint(10, 100), 'complexity': random.random(), 'domain': random.choice(['tech', 'medical', 'legal', 'general'])}, 'metadata': {'timestamp': '2025-01-20T10:00:00Z', 'source': 'demo'}}
...
def process_ml_items(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        result = {'id': item['id'], 'original_text': item['text'][:50] + '...', 'processed': True, 'ml_score': random.random() * text_length / 100, 'enhanced_features': {**item.get('features', {}), 'processed_length': text_length, 'quality_score': random.random()}}
...
async def demo_basic_usage():
    ...
    config = StreamingBatchConfig(chunk_size=1000, worker_processes=4, memory_limit_mb=500, chunking_strategy=ChunkingStrategy.FIXED_SIZE)
    print('\nProcessing data with streaming batch processor...')
    async with StreamingBatchProcessor(config, process_ml_items) as processor:
        metrics = await processor.process_dataset(data_source=data_file, job_id='demo_basic')
    print(f'\n✅ Processing Complete!')
...
async def demo_memory_efficient_processing():
    ...
    config = StreamingBatchConfig(chunk_size=500, max_chunk_memory_mb=20, worker_processes=2, memory_limit_mb=200, chunking_strategy=ChunkingStrategy.MEMORY_BASED, compression=True, gc_threshold_mb=100)
    print('\nProcessing large dataset with memory constraints...')
    print(f'Memory limit: {config.memory_limit_mb} MB')
    async with StreamingBatchProcessor(config, process_ml_items) as processor:
        metrics = await processor.process_dataset(data_source=data_file, job_id='demo_memory_efficient')
...
async def demo_checkpoint_resume():
    ...
    config = StreamingBatchConfig(chunk_size=1000, worker_processes=2, enable_checkpointing=True, checkpoint_interval=5000, checkpoint_dir='./demo_checkpoints')
    job_id = 'demo_checkpoint_job'
    print('\nStarting processing (will interrupt after 10 seconds)...')
    try:
        async with StreamingBatchProcessor(config, process_ml_items) as processor:
            await asyncio.wait_for(processor.process_dataset(data_source=data_file, job_id=job_id), timeout=3.0)
    ...
    async with StreamingBatchProcessor(config, process_ml_items) as processor:
        metrics = await processor.process_dataset(data_source=data_file, job_id=job_id, resume_from_checkpoint=True)
...
async def demo_streaming_data_source():
    ...
    async def stream_ml_data(num_records: int):
        ...
        for i in range(num_records):
            yield {'id': f'stream_{i}', 'text': f'Streaming record {i} with real-time data', 'features': {'timestamp': i, 'value': random.random()}}
            if i % 100 == 0:
                await asyncio.sleep(0.01)
    config = StreamingBatchConfig(chunk_size=500, worker_processes=4, chunking_strategy=ChunkingStrategy.ADAPTIVE, prefetch_chunks=2)
    print('\nProcessing streaming data source...')
    async with StreamingBatchProcessor(config, process_ml_items) as processor:
        metrics = await processor.process_dataset(data_source=stream_ml_data(10000), job_id='demo_streaming')
    ...
    with open(data_file, 'w') as f:
        for i in range(10000):
            if i % 100 == 0:
                ...
                size_type = 'small'
            record = {'id': f'mixed_{i}', 'text': text, 'size_type': size_type, 'features': {'size': len(text)}}
            f.write(json.dumps(record) + '\n')
    config = StreamingBatchConfig(chunk_size=1000, max_chunk_memory_mb=50, worker_processes=4, chunking_strategy=ChunkingStrategy.ADAPTIVE, memory_limit_mb=500)
    print('\nProcessing mixed dataset with adaptive strategy...')
    async with StreamingBatchProcessor(config, process_ml_items) as processor:
        metrics = await processor.process_dataset(data_source=data_file, job_id='demo_adaptive')
