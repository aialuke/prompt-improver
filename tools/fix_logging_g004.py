                    ...
                    if isinstance(value, ast.Str):
                        ...
                        if value.format_spec is not None:
                            ...
                            if spec_str is not None:
                                expr = ast.Call(func=ast.Name(id='format', ctx=ast.Load()), args=[expr, ast.Constant(value=spec_str)], keywords=[])
                            else:
                                expr = ast.Call(func=ast.Name(id='str', ctx=ast.Load()), args=[expr], keywords=[])
                        fmt_args.append(expr)
                    else:
                        fmt_parts.append('%s')
                        fmt_args.append(ast.Call(func=ast.Name(id='str', ctx=ast.Load()), args=[value], keywords=[]))
...
def main() -> int:
    ...
    for dirpath, _, filenames in os.walk(root):
        ...
        for name in filenames:
            ...
            if '.venv' in path.parts or '.ruff_cache' in path.parts or '.mypy_cache' in path.parts:
                continue
