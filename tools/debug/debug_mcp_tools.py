...
async def debug_mcp_server():
    ...
    process = subprocess.Popen([sys.executable, 'src/prompt_improver/mcp_server/mcp_server.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1)
    try:
        ...
        if process.poll() is not None:
            ...
            return False
        init_request = {'jsonrpc': '2.0', 'id': 1, 'method': 'initialize', 'params': {'protocolVersion': '2024-11-05', 'capabilities': {'tools': {}, 'resources': {}}, 'clientInfo': {'name': 'debug-client', 'version': '1.0.0'}}}
        ...
        test_formats = [{'jsonrpc': '2.0', 'id': 3, 'method': 'tools/call', 'params': {'name': 'improve_prompt', 'arguments': {'prompt': 'Help me debug this', 'context': {'domain': 'debugging'}, 'session_id': 'debug_test'}}}, {'jsonrpc': '2.0', 'id': 4, 'method': 'tools/call', 'params': {'name': 'improve_prompt', 'prompt': 'Help me debug this', 'context': {'domain': 'debugging'}, 'session_id': 'debug_test'}}]
        ...
        process.terminate()
        try:
            await asyncio.wait_for(asyncio.get_event_loop().run_in_executor(None, process.wait), timeout=3)
