...
class SimpleExtractor:
    ...
    async def simple_extract(self, training_data: dict) -> np.ndarray:
        ...
        # Convert features to numpy array (CPU-bound operation, but fast)
        feature_matrix = np.array(features)
        
        self.logger.info(f"Extracted {feature_matrix.shape[0]} context feature vectors of dimension {feature_matrix.shape[1]}")
