...
def debug_component(component_name: str, module_path: str):
    ...
    try:
        ...
        import inspect
        classes = [obj for name, obj in inspect.getmembers(module) if inspect.isclass(obj) and obj.__module__ == module.__name__]
        if not classes:
            print('❌ No classes found in module')
            return
        print(f'📋 Found {len(classes)} classes: {[cls.__name__ for cls in classes]}')
        main_class = None
        possible_names = [component_name.title().replace('_', ''), f"{component_name.title().replace('_', '')}Service", f"{component_name.title().replace('_', '')}Manager", f"{component_name.title().replace('_', '')}Analyzer"]
