...
def simulate_pytest_test_execution():
    ...
    from prompt_improver.database.registry import clear_registry, get_registry_manager
    clear_registry()
    ...
    try:
        ...
        async def test_ab_experiment():
            async with get_session_context() as session:
                ...
                result = await ab_service.create_experiment(experiment_name='debug_test', control_rules=control_rules, treatment_rules=treatment_rules, db_session=session, target_metric='improvement_score', sample_size_per_group=50)
                return result
