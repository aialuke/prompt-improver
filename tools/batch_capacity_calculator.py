...
class BatchCapacityCalculator:
    ...
    def __init__(self, batch_config: BatchConfig | None=None, stats_config: StatisticalConfig | None=None):
    ...
    def calculate_throughput_capacity(self) -> CapacityEstimate:
        ...
        batch_processing_time_ms = processing_time_ms * prompts_per_batch / concurrent_tests
        overhead_factor = 1.3
        ...
        bottleneck_factors = {'ML_INFERENCE': actual_batch_time_ms / (processing_time_ms * prompts_per_batch), 'CONCURRENCY': concurrent_tests / 10, 'BATCH_SIZE': prompts_per_batch / 20}
        primary_bottleneck = min(bottleneck_factors.keys(), key=lambda k: bottleneck_factors[k])
        efficiency = min(1.0, hour_ms / (actual_batch_time_ms * batches_per_hour))
        recommendations = self._generate_capacity_recommendations(hourly_prompts, bottleneck_factors, efficiency)
        return CapacityEstimate(hourly_throughput=int(hourly_prompts), daily_throughput=int(daily_prompts), weekly_throughput=int(weekly_prompts), bottleneck_factor=primary_bottleneck, efficiency_score=efficiency, recommendations=recommendations)

    def calculate_sample_size_requirements(self, effect_size: float | None=None) -> SampleSizeEstimate:
        ...
        final_sample_size = max(sequential_sample_size, self.stats_config.minimum_sample_size * 2)
        ...
        power_analysis = {'achieved_power': power, 'effect_size': effect_size, 'alpha': alpha, 'beta': 1 - power, 'critical_value': z_alpha_2, 'sample_size_per_group': final_sample_size // 2}
        group_sequential_params = {'inflation_factor': sequential_inflation, 'groups_planned': 5, 'interim_alpha': alpha / 5, 'monitoring_frequency': 'daily'}
        return SampleSizeEstimate(required_sample_size=final_sample_size, test_duration_days=0, confidence_interval=confidence_interval, power_analysis=power_analysis, group_sequential_params=group_sequential_params)

    def estimate_test_duration(self, capacity: CapacityEstimate, sample_size: SampleSizeEstimate) -> float:
    ...
    def _generate_capacity_recommendations(self, hourly_throughput: float, bottlenecks: dict[str, float], efficiency: float) -> list[str]:
        """Generate optimization recommendations."""
        recommendations = []
        if efficiency < 0.6:
            recommendations.append('Low efficiency detected. Consider optimizing ML inference pipeline.')
        if bottlenecks['CONCURRENCY'] < 0.5:
            recommendations.append('Increase max_concurrent_tests from 5 to 8-10 for better parallelization.')
        if bottlenecks['BATCH_SIZE'] < 0.5:
            recommendations.append('Consider increasing batch_size from 10 to 15-20 for better throughput.')
        if hourly_throughput < 100:
            recommendations.append('Low throughput. Consider ML model optimization or hardware scaling.')
        if bottlenecks['ML_INFERENCE'] > 1.5:
            recommendations.append('ML inference is bottleneck. Cache models or use faster inference.')
    ...
    def generate_capacity_report(self) -> dict:
        ...
        report = {'timestamp': datetime.now().isoformat(), 'capacity_analysis': {'hourly_throughput': capacity.hourly_throughput, 'daily_throughput': capacity.daily_throughput, 'weekly_throughput': capacity.weekly_throughput, 'bottleneck': capacity.bottleneck_factor, 'efficiency': f'{capacity.efficiency_score:.1%}', 'recommendations': capacity.recommendations}, 'sample_size_analysis': {}, 'configuration': {'batch_size': self.batch_config.batch_size, 'max_concurrent_tests': self.batch_config.max_concurrent_tests, 'timeout_hours': self.batch_config.timeout_seconds / 3600, 'significance_level': self.stats_config.significance_level, 'power_threshold': self.stats_config.power_threshold}}
        for effect_size, estimate in sample_estimates.items():
            report['sample_size_analysis'][f'effect_size_{effect_size}'] = {'required_sample_size': estimate.required_sample_size, 'test_duration_days': round(estimate.test_duration_days, 1), 'achieved_power': estimate.power_analysis['achieved_power'], 'confidence_interval': estimate.confidence_interval, 'group_sequential_inflation': estimate.group_sequential_params['inflation_factor']}
        return report
...
def main():
    """Main CLI interface."""
    parser = argparse.ArgumentParser(description='Batch Capacity Calculator for A/B Testing')
    parser.add_argument('--batch-size', type=int, default=10, help='Batch size for processing (default: 10)')
    parser.add_argument('--concurrent-tests', type=int, default=5, help='Max concurrent tests (default: 5)')
    parser.add_argument('--processing-time', type=float, default=500, help='Processing time per prompt in ms (default: 500)')
    parser.add_argument('--effect-size', type=float, default=0.1, help='Minimum effect size to detect (default: 0.1)')
    parser.add_argument('--power', type=float, default=0.8, help='Statistical power threshold (default: 0.8)')
    parser.add_argument('--export', type=str, help='Export report to JSON file')
    args = parser.parse_args()
    logging.basicConfig(level=logging.INFO)
    batch_config = BatchConfig(batch_size=args.batch_size, max_concurrent_tests=args.concurrent_tests, processing_time_per_prompt_ms=args.processing_time)
    stats_config = StatisticalConfig(power_threshold=args.power, min_effect_size=args.effect_size)
