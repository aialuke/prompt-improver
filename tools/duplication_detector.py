...
class DuplicationDetector:
    ...
    def analyze_project(self) -> DuplicationReport:
        ...
        logger_pattern = re.compile('logger\\s*=\\s*logging\\.getLogger\\(__name__\\)')
        config_pattern = re.compile('try:\\s*.*config.*=.*get_config|config.*=.*get_config.*except')
        metrics_pattern = re.compile('self\\.metrics_registry\\s*=\\s*get_metrics_registry\\(\\)')
        ...
        recommendations = self._generate_recommendations(duplicate_imports, duplicate_logger_inits, duplicate_config_patterns, duplicate_metrics_patterns, duplicate_error_patterns, similar_classes, similar_functions)
        return DuplicationReport(duplicate_imports=dict(duplicate_imports), duplicate_logger_inits=duplicate_logger_inits, duplicate_config_patterns=duplicate_config_patterns, duplicate_metrics_patterns=duplicate_metrics_patterns, duplicate_error_patterns=duplicate_error_patterns, similar_classes=dict(similar_classes), similar_functions=dict(similar_functions), total_files_analyzed=len(python_files), recommendations=recommendations)
    ...
    def _normalize_import(self, import_line: str) -> Optional[str]:
        ...
        patterns = ['import logging', 'from.*logging.*import.*getLogger', 'from.*metrics_registry.*import.*get_metrics_registry', 'from.*config.*import.*get_config', 'from pydantic import BaseModel', 'from typing import.*Optional.*Dict.*Any', 'import asyncio', 'from datetime import datetime', 'from dataclasses import dataclass']
    ...
    def _has_duplicate_error_pattern(self, content: str) -> bool:
        ...
        error_patterns = ['try:\\s*.*except.*Exception.*:.*logger\\.warning', 'try:\\s*.*except.*:.*fallback', 'except.*Exception.*as.*e:.*logger\\.error']
        for pattern in error_patterns:
            if re.search(pattern, content, re.MULTILINE | re.DOTALL):
                return True
        return False

    def _analyze_ast_similarities(self, tree: ast.AST, file_path: str, similar_classes: Dict[str, List[str]], similar_functions: Dict[str, List[str]]):
        ...
        class SimilarityAnalyzer(ast.NodeVisitor):
            ...
            def visit_ClassDef(self, node: ast.ClassDef) -> None:
                ...
                attrs = [n.targets[0].id for n in node.body if isinstance(n, ast.Assign) and len(n.targets) == 1 and isinstance(n.targets[0], ast.Name)]
            ...
            def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
                ...
                body_types = [type(n).__name__ for n in node.body[:3]]
    ...
    def _generate_recommendations(self, duplicate_imports: Dict[str, List[str]], duplicate_logger_inits: List[str], duplicate_config_patterns: List[str], duplicate_metrics_patterns: List[str], duplicate_error_patterns: List[str], similar_classes: Dict[str, List[str]], similar_functions: Dict[str, List[str]]) -> List[str]:
        """Generate refactoring recommendations."""
        _ = similar_functions
        recommendations: List[str] = []
        if duplicate_logger_inits:
            recommendations.append(f"🔄 CRITICAL: Replace {len(duplicate_logger_inits)} duplicate logger initializations with 'from prompt_improver.core.common import get_logger'")
        if duplicate_config_patterns:
            recommendations.append(f'🔄 HIGH: Consolidate {len(duplicate_config_patterns)} duplicate config loading patterns using ConfigMixin or get_config_safely()')
        if duplicate_metrics_patterns:
            recommendations.append(f'🔄 HIGH: Consolidate {len(duplicate_metrics_patterns)} duplicate metrics patterns using MetricsMixin')
        if duplicate_error_patterns:
            recommendations.append(f'🔄 MEDIUM: Standardize {len(duplicate_error_patterns)} duplicate error handling patterns using error_handling utilities')
        large_similar_groups = {k: v for k, v in similar_classes.items() if len(v) > 3}
        if large_similar_groups:
            recommendations.append(f'🔄 MEDIUM: Consider creating base classes for {len(large_similar_groups)} groups of similar classes')
        high_usage_imports = {k: v for k, v in duplicate_imports.items() if len(v) > 10}
        if high_usage_imports:
            recommendations.append(f'🔄 LOW: Consider creating import shortcuts for {len(high_usage_imports)} frequently used import patterns')
        return recommendations

    def generate_report(self, report: DuplicationReport) -> str:
        """Generate a formatted report."""
        lines = ['# Code Duplication Analysis Report', f"**Generated:** {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", f'**Files Analyzed:** {report.total_files_analyzed}', '', '## 📊 Summary', '', f'- **Duplicate Logger Initializations:** {len(report.duplicate_logger_inits)}', f'- **Duplicate Config Patterns:** {len(report.duplicate_config_patterns)}', f'- **Duplicate Metrics Patterns:** {len(report.duplicate_metrics_patterns)}', f'- **Duplicate Error Patterns:** {len(report.duplicate_error_patterns)}', f'- **Similar Class Groups:** {len([g for g in report.similar_classes.values() if len(g) > 2])}', f'- **Similar Function Groups:** {len([g for g in report.similar_functions.values() if len(g) > 2])}', '', '## 🎯 Recommendations', '']
        for rec in report.recommendations:
            lines.append(f'- {rec}')
        lines.extend(['', '## 📋 Detailed Findings', '', '### Logger Initialization Duplication', f'Found {len(report.duplicate_logger_inits)} files with duplicate `logger = logging.getLogger(__name__)` patterns:', ''])
        for file_path in report.duplicate_logger_inits[:10]:
            lines.append(f'- `{file_path}`')
        if len(report.duplicate_logger_inits) > 10:
            lines.append(f'- ... and {len(report.duplicate_logger_inits) - 10} more files')
        lines.extend(['', '### Configuration Loading Duplication', f'Found {len(report.duplicate_config_patterns)} files with duplicate config loading patterns:', ''])
        ...
        lines.extend(['', '### Metrics Registry Duplication', f'Found {len(report.duplicate_metrics_patterns)} files with duplicate metrics registry patterns:', ''])
        ...
        if similar_groups:
            lines.extend(['', '### Similar Class Structures', f'Found {len(similar_groups)} groups of classes with similar structures:', ''])
            for i, group in enumerate(similar_groups[:5]):
                lines.append(f'**Group {i + 1}:** {len(group)} similar classes')
                for class_info in group[:3]:
    ...
    def save_report(self, report: DuplicationReport, output_path: str):
        ...
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump({'duplicate_imports': report.duplicate_imports, 'duplicate_logger_inits': report.duplicate_logger_inits, 'duplicate_config_patterns': report.duplicate_config_patterns, 'duplicate_metrics_patterns': report.duplicate_metrics_patterns, 'duplicate_error_patterns': report.duplicate_error_patterns, 'similar_classes': report.similar_classes, 'similar_functions': report.similar_functions, 'total_files_analyzed': report.total_files_analyzed, 'recommendations': report.recommendations}, f, indent=2)
