...
class DeepComponentInspector:
    ...
    def __init__(self):
        # These are the exact paths from our orchestrator
    ...
    def _find_main_class(self, module: Any, component_name: str) -> Optional[type]:
        ...
        # Return the most likely main class
        for cls in module_classes:
            if (not cls.__name__.startswith('_') and 
                len(cls.__name__) > 3 and
                cls.__name__ not in ['ABC', 'BaseModel', 'Enum']):
                return cls
    ...
    async def deep_inspect_component(self, component_name: str, module_path: str) -> Dict[str, Any]:
        ...
        try:
            # Phase 1: Module Import (like orchestrator)
            print(f"📦 Phase 1: Importing {module_path}")
            module = importlib.import_module(module_path)
            print("✅ Module import successful")
            
            # Phase 2: Class Discovery (like orchestrator)
            ...
            # Phase 4: Mock Generation (like orchestrator)
            ...
            # Phase 5: Instantiation (like orchestrator)
            ...
            try:
                ...
                methods = [method for method in dir(component_instance) 
                          if not method.startswith('_') and callable(getattr(component_instance, method))]
                print(f"Found {len(methods)} public methods: {methods[:5]}{'...' if len(methods) > 5 else ''}")
                
                # Phase 7: Basic Method Testing
                ...
                # Check for import issues within the class
                try:
