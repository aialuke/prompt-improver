...
class CircularImportAnalyzer:
        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, (ast.Import, ast.ImportFrom)):
                    self._process_import_node(node, module_name, in_function, in_type_checking)
    ...
    def _is_type_checking_block(self, node: ast.If) -> bool:
        ...
        if isinstance(node.test, ast.Attribute):
            if isinstance(node.test.value, ast.Name) and node.test.value.id == 'typing' and (node.test.attr == 'TYPE_CHECKING'):
                return True
        return False
    ...
    def _process_import_node(self, node, module_name: str, in_function: bool, in_type_checking: bool):
        ...
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                self._add_import(module_name, target_module, ImportType.MODULE, node.lineno, f'import {alias.name}', in_function, in_type_checking)
            ...
            if node.module:
                ...
                self._add_import(module_name, target_module, import_type, node.lineno, stmt, in_function, in_type_checking)
    ...
    def _add_import(self, source: str, target: str, import_type: ImportType, line_number: int, statement: str, in_function: bool, in_type_checking: bool):
        """Add an import to the graph and details."""
        if target and target in self.module_files:
            self.import_graph[source].add(target)
            import_info = ImportInfo(source_module=source, target_module=target, import_type=ImportType.TYPE_ONLY if in_type_checking else import_type, line_number=line_number, import_statement=statement, is_conditional=in_type_checking, is_in_function=in_function)
    ...
    def _create_circular_chain(self, cycle: List[str]):
        ...
        circular_chain = CircularChain(modules=cycle[:-1], severity=severity, import_details=import_details, root_cause=root_cause, remediation_strategy=remediation)
    ...
    def _determine_severity(self, import_details: List[ImportInfo]) -> CircularImportSeverity:
        ...
        if any((not detail.is_in_function and (not detail.is_conditional) and (detail.import_type in [ImportType.DIRECT, ImportType.MODULE]) for detail in import_details)):
    ...
    def _generate_analysis_report(self) -> Dict[str, any]:
        ...
        return {'summary': {'total_modules': len(self.module_files), 'total_imports': sum((len(imports) for imports in self.import_graph.values())), 'circular_chains': len(self.circular_chains), 'severity_breakdown': {severity.value: sum((1 for chain in self.circular_chains if chain.severity == severity)) for severity in CircularImportSeverity}}, 'circular_chains': [{'modules': chain.modules, 'severity': chain.severity.value, 'root_cause': chain.root_cause, 'remediation_strategy': chain.remediation_strategy, 'import_details': [{'source': detail.source_module, 'target': detail.target_module, 'type': detail.import_type.value, 'line': detail.line_number, 'statement': detail.import_statement, 'conditional': detail.is_conditional, 'in_function': detail.is_in_function} for detail in chain.import_details]} for chain in self.circular_chains], 'import_graph': {module: list(imports) for module, imports in self.import_graph.items()}}
if __name__ == '__main__':
    ...
    try:
        ...
        critical_chains = [c for c in analyzer.circular_chains if c.severity == CircularImportSeverity.CRITICAL]
        if critical_chains:
            print(f'\n🚨 CRITICAL ISSUES ({len(critical_chains)}):')
            for chain in critical_chains[:5]:
