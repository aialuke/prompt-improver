...
class MLOrchestratorIntegrationAuditor:
    ...
    async def _scan_existing_components(self):
        ...
        component_dirs = ['src/prompt_improver/ml/learning/algorithms', 'src/prompt_improver/ml/learning/quality', 'src/prompt_improver/ml/optimization/algorithms', 'src/prompt_improver/ml/optimization/validation', 'src/prompt_improver/ml/evaluation', 'src/prompt_improver/ml/preprocessing', 'src/prompt_improver/performance/monitoring', 'src/prompt_improver/performance/testing', 'src/prompt_improver/performance/analytics', 'src/prompt_improver/performance/optimization', 'src/prompt_improver/services', 'src/prompt_improver/ml/automl']
    ...
    async def _analyze_python_file(self, file_path: str):
        ...
        try:
            ...
            ml_component_patterns = ['.*Analyzer.*', '.*Optimizer.*', '.*Engine.*', '.*Scorer.*', '.*Validator.*', '.*Monitor.*', '.*Orchestrator.*', '.*Manager.*', '.*Processor.*', '.*Extractor.*', '.*Transformer.*', '.*Framework.*', '.*Learner.*', '.*Service.*', '.*Handler.*', '.*Controller.*', '.*Detector.*', '.*Classifier.*', '.*Predictor.*', '.*Evaluator.*', '.*Generator.*', '.*Builder.*', '.*Factory.*', '.*Provider.*', '.*Reducer.*', '.*Clusterer.*', '.*Sampler.*', '.*Selector.*']
            for class_name in classes:
                if any((re.match(pattern, class_name) for pattern in ml_component_patterns)):
                    ...
                    component_key = re.sub('[^a-z_]', '', component_key)
                    self.existing_components[component_key] = {'class_name': class_name, 'file_path': file_path, 'module_path': module_path}
        except Exception as e:
            pass
    ...
    async def _analyze_integration_gaps(self):
        ...
        for comp_key, comp_info in self.existing_components.items():
            ...
            audit_result = ComponentAuditResult(name=comp_key, file_path=comp_info['file_path'], class_name=comp_info['class_name'], integration_status=integration_status, priority=priority, tier_assignment=tier, issues=issues, required_actions=required_actions, dependencies=[], estimated_effort='2-4 hours' if integration_status == IntegrationStatus.NOT_INTEGRATED else '0 hours')
    ...
    def _determine_component_priority(self, comp_key: str, comp_info: Dict[str, Any]) -> Priority:
        ...
        high_priority_patterns = ['scorer', 'analyzer', 'validator', 'engine', 'learner', 'optimizer', 'orchestrator', 'monitor', 'processor', 'extractor']
        medium_priority_patterns = ['transformer', 'manager', 'framework', 'service', 'handler']
    ...
    def _determine_component_tier(self, comp_key: str, comp_info: Dict[str, Any]) -> str:
        ...
        if 'ml.core' in module_path or 'ml.integration' in module_path:
            return 'tier1_core'
        elif 'ml.optimization' in module_path or 'ml.learning' in module_path or 'optimization' in comp_key or ('learner' in comp_key) or ('optimizer' in comp_key):
            return 'tier2_optimization'
        elif 'ml.evaluation' in module_path or 'evaluation' in module_path or 'analyzer' in comp_key or ('validator' in comp_key) or ('statistical' in comp_key):
            return 'tier3_evaluation'
        elif 'performance' in module_path or 'monitoring' in module_path or 'monitor' in comp_key or ('analytics' in comp_key):
    ...
    def _generate_integration_roadmap(self) -> Dict[str, Any]:
        ...
        roadmap = {'critical': [], 'high': [], 'medium': [], 'low': [], 'summary': {'total_components': len(self.audit_results), 'not_integrated': len([r for r in self.audit_results if r.integration_status == IntegrationStatus.NOT_INTEGRATED]), 'partially_integrated': len([r for r in self.audit_results if r.integration_status == IntegrationStatus.PARTIALLY_INTEGRATED]), 'fully_integrated': len([r for r in self.audit_results if r.integration_status == IntegrationStatus.FULLY_INTEGRATED])}}
        for result in self.audit_results:
            if result.integration_status != IntegrationStatus.FULLY_INTEGRATED:
                roadmap[result.priority.value].append({'name': result.name, 'class_name': result.class_name, 'file_path': result.file_path, 'tier': result.tier_assignment, 'status': result.integration_status.value, 'issues': result.issues, 'actions': result.required_actions, 'effort': result.estimated_effort})
        return roadmap
    ...
    def _generate_audit_report(self, roadmap: Dict[str, Any]):
        ...
        for priority in ['critical', 'high', 'medium', 'low']:
            ...
            if count > 0:
                ...
                for comp in roadmap[priority][:3]:
