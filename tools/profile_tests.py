...
def run_pytest_with_profiling():
    ...
    cmd = [sys.executable, '-m', 'pytest', '-v', '--tb=short', '--durations=20', '--collect-only', '-q']
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print(result.stderr)
            return False
        cmd = [sys.executable, '-m', 'pytest', '-v', '--tb=short', '--durations=20', '--maxfail=10', '-x', '--timeout=10']
...
def generate_performance_report():
    ...
    print('=' * 50)
    report = {'recommendations': ['🔧 Use deterministic fixtures with reduced sample sizes', '⚡ Implement parallel test execution for independent tests', '📦 Cache expensive fixtures at session scope when possible', '🗄️ Use PostgreSQL containers for unit tests instead of mocks', '🎯 Mock external dependencies to reduce I/O overhead', '📊 Profile database operations and optimize queries', '🔄 Implement test markers to categorize fast vs slow tests', '🚀 Use pytest-xdist for parallel execution'], 'configuration_improvements': ['Enable pytest-benchmark for performance regression testing', 'Set up CI/CD pipeline with performance monitoring', 'Configure test timeouts to prevent hanging tests', 'Use pytest-cov for coverage-guided optimization', 'Implement test result caching for unchanged code']}
