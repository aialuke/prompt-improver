...
try:
    ...
    # Note: Tree-sitter successfully imported (no print during import)
except ImportError:
    # Note: tree-sitter not available, falling back to AST parsing (no print during import)
    pass
...
# Voyage AI client - initialized lazily when needed
vo: Optional[Any] = None

def get_voyage_client() -> Any:
    """Get Voyage AI client with hardcoded API key for reliability."""
    global vo, voyageai
    if vo is None:
        try:
            ...
            # CLEAN: Hardcode API key to avoid configuration issues
            vo = voyageai.Client(api_key='pa-fhFUHl_VQ2SjvA7TNQ0gF8v021efH3R82qbigC3pnnB')
            print("🚀 Voyage AI client initialized successfully")
            print("   Using voyage-context-3 model with contextualized embeddings")
...
# Enhanced Configuration for voyage-code-3 (2025 best practices)
MAX_TOKENS_PER_BATCH = 120000  # voyage-code-3 token limit per batch
MAX_ITEMS_PER_BATCH = 1000     # Maximum items per batch
RATE_LIMIT_TPM = 3000000       # voyage-code-3 tokens per minute (3M TPM)
RATE_LIMIT_RPM = 2000          # voyage-code-3 requests per minute (2K RPM)

# Matryoshka Embedding Configurations with Binary Rescoring Support
EMBEDDING_CONFIGS = {
    # 2025 Best Practice Configurations (Contextualized Embeddings)
    "contextualized_ultra_high": {
        "name": "contextualized_ultra_high",
        "contextualized_model": "voyage-context-3",  # CLEAN: Use voyage-context-3 for contextualized embeddings
        "output_dimension": 2048,
        "output_dtype": "float",
        "generate_binary": True,
        "binary_dimension": 256,
        "use_contextualized": True,
        "max_batch_size": 1000,
        "enable_token_counting": True,
        "description": "Ultra-high quality with contextualized embeddings (2025 best practice)"
    },
    "contextualized_high": {
        "name": "contextualized_high",
        "contextualized_model": "voyage-context-3",  # CLEAN: Use voyage-context-3 for contextualized embeddings
        "output_dimension": 256,  # FIXED: Match binary rescoring dimensions (256-dim → 32 bytes)
        "output_dtype": "float",
        "generate_binary": True,
        "binary_dimension": 256,
        "use_contextualized": True,
        "max_batch_size": 1000,
        "enable_token_counting": True,
        "description": "High quality with contextualized embeddings (recommended)"
    },
    "contextualized_balanced": {
        "name": "contextualized_balanced",
        "contextualized_model": "voyage-context-3",  # CLEAN: Use voyage-context-3 for contextualized embeddings
        ...
        "max_batch_size": 1000,
        "enable_token_counting": True,
        "description": "Balanced performance with contextualized embeddings"
    },
    "contextualized_fast": {
        "name": "contextualized_fast",
        "contextualized_model": "voyage-context-3",  # CLEAN: Use voyage-context-3 for contextualized embeddings
        ...
        "max_batch_size": 1000,
        "enable_token_counting": True,
        "description": "Fast performance with contextualized embeddings"
    },

    # Standard Configurations (non-contextualized) - CORRECTED: Use voyage-code-3 for code embeddings
    "standard_high": {
        "name": "standard_high",
        "model": "voyage-code-3",     # CORRECTED: Use voyage-code-3 for code embeddings
        "output_dimension": 256,  # FIXED: Match binary rescoring dimensions (256-dim → 32 bytes)
        "output_dtype": "float",
        "generate_binary": True,
        "binary_dimension": 256,
        "use_contextualized": False,
        "description": "High quality standard embeddings"
    },
    "standard_balanced": {
        ...
        "binary_dimension": 256,
        "use_contextualized": False,
        "description": "Balanced standard embeddings"
    }
}
...
def create_contextualized_batches(chunks_by_file: Dict[str, List[CodeChunk]],
                                 max_tokens: int = 120000) -> List[Dict[str, List[CodeChunk]]]:
    """
    Create batches of files for contextualized embeddings that respect token limits.

    Args:
        chunks_by_file: Dictionary mapping file paths to their chunks
        max_tokens: Maximum tokens per batch (API limit)

    Returns:
        List of batches, each containing a subset of files
    """
    vo = get_voyage_client()

    batches = []
    current_batch: Dict[str, List[CodeChunk]] = {}
    current_tokens = 0

    for file_path, file_chunks in chunks_by_file.items():
        # Count tokens for this file
        file_content = [chunk.content for chunk in file_chunks]
        file_tokens = vo.count_tokens(file_content, model="voyage-code-3")  # CORRECTED: Use voyage-code-3

        # If this file alone exceeds the limit, split it
        if file_tokens > max_tokens:
            # If we have a current batch, save it
            if current_batch:
            ...
            chunks_per_batch = max(1, len(file_chunks) // ((file_tokens // max_tokens) + 1))
            for i in range(0, len(file_chunks), chunks_per_batch):
                chunk_subset = file_chunks[i:i + chunks_per_batch]
                subset_content = [chunk.content for chunk in chunk_subset]
                subset_tokens = vo.count_tokens(subset_content, model="voyage-code-3")  # CORRECTED: Use voyage-code-3

                if subset_tokens <= max_tokens:
                    batches.append({f"{file_path}_part_{i//chunks_per_batch}": chunk_subset})
                else:
                    # Further split if still too large
                    for chunk in chunk_subset:
                        chunk_tokens = vo.count_tokens([chunk.content], model="voyage-context-3")
                        if chunk_tokens <= max_tokens:
                            batches.append({f"{file_path}_chunk_{chunk.start_line}": [chunk]})
            continue
        ...
        if current_tokens + file_tokens > max_tokens:
            ...
            if current_batch:
                batches.append(current_batch)
            current_batch = {file_path: file_chunks}
...
def generate_contextualized_embeddings_with_config(chunks_by_file: Dict[str, List[CodeChunk]],
                                                  config: Dict[str, Any],
                                                  batch_num: int = 1,
                                                  total_batches: int = 1) -> Tuple[List[List[float]], Optional[List[List[int]]]]:
    """
    Generate contextualized embeddings using Voyage AI's contextualized embedding API (voyage-context-3).

    CLEAN: Uses contextualized_embed() with voyage-context-3 for proper document-level context.
    ...
    Returns:
        Tuple of (embeddings, binary_embeddings)
    """
    print(f"🧠 Generating contextualized embeddings (batch {batch_num}/{total_batches})")

    try:
        vo = get_voyage_client()

        # Create batches that respect API token limits
        max_tokens = config.get("max_tokens_per_request", 120000)
        batches = create_contextualized_batches(chunks_by_file, max_tokens)

        print(f"   📦 Created {len(batches)} contextualized batches (max {max_tokens:,} tokens each)")
        ...
        # Process each batch
        for batch_idx, batch_chunks_by_file in enumerate(batches):
            print(f"   ⚡ Processing contextualized batch {batch_idx + 1}/{len(batches)}")

            # Prepare inputs for contextualized embeddings
            # Format: [["file1_chunk1", "file1_chunk2"], ["file2_chunk1", "file2_chunk2"]]
            contextualized_inputs = []
            batch_chunk_order = []

            for _, file_chunks in batch_chunks_by_file.items():
                if not file_chunks:
                    continue

                # Group chunks from the same file
                file_chunk_contents = [chunk.content for chunk in file_chunks]
                contextualized_inputs.append(file_chunk_contents)

                # Track chunk order for result mapping
                for chunk in file_chunks:
                    batch_chunk_order.append(chunk)
                    chunk_order.append(chunk)

            # Count tokens for this batch
            batch_tokens = vo.count_tokens([chunk.content for chunk in batch_chunk_order], model=config.get("contextualized_model", "voyage-context-3"))
            print(f"      📊 Batch tokens: {batch_tokens:,}")

            # Generate contextualized embeddings for this batch
            start_time = time.time()

            contextualized_result = vo.contextualized_embed(
                inputs=contextualized_inputs,
                model=config.get("contextualized_model", "voyage-context-3"),  # CLEAN: Use voyage-context-3
                input_type="document",
                output_dimension=config.get("output_dimension", 256),  # FIXED: Default to 256 dimensions
                output_dtype=config.get("output_dtype", "float")
            )

            # Flatten results for this batch
            batch_embeddings = []
            for result in contextualized_result.results:
                batch_embeddings.extend(result.embeddings)

            all_embeddings.extend(batch_embeddings)

            generation_time = time.time() - start_time
            print(f"      ✅ Generated {len(batch_embeddings)} embeddings in {generation_time:.1f}s")

            # Generate binary embeddings for this batch if requested
            if config.get("generate_binary", False):
                binary_inputs = [[chunk.content for chunk in file_chunks] for file_chunks in batch_chunks_by_file.values() if file_chunks]

                binary_result = vo.contextualized_embed(
                    inputs=binary_inputs,
                    model=config.get("contextualized_model", "voyage-context-3"),  # CLEAN: Use voyage-context-3
                    input_type="document",
                    output_dimension=config.get("binary_dimension", 256),
                    output_dtype="binary"
                )

                # Flatten binary results for this batch
                batch_binary_embeddings = []
                for result in binary_result.results:
                    batch_binary_embeddings.extend(result.embeddings)

                all_binary_embeddings.extend(batch_binary_embeddings)

        print(f"   ✅ Generated {len(all_embeddings)} total contextualized embeddings")
        print(f"   📈 Total chunks processed: {len(chunk_order)}")

        binary_embeddings = all_binary_embeddings if all_binary_embeddings else None
        if binary_embeddings:
            print(f"   🔧 Generated {len(binary_embeddings)} binary embeddings")

        return all_embeddings, binary_embeddings

    except Exception as e:
        print(f"   ❌ Contextualized embedding generation failed: {e}")
        raise
...
def generate_embeddings_with_config(content_list: List[str],
                                   ...
                                   batch_num: int = 1,
                                   total_batches: int = 1) -> Tuple[List[List[float]], Optional[List[List[int]]]]:
    """Generate embeddings using specified Matryoshka configuration with optional binary embeddings.

    Returns:
        Tuple of (main_embeddings, binary_embeddings) where binary_embeddings is None if not requested
    """
    ...
    try:
        ...
        # Prepare embedding parameters (2025 best practices)
        embed_params = {
            "model": config.get("model", "voyage-context-3"),  # FIXED: Use model from config, default to voyage-context-3
            "input_type": "document",
            "output_dtype": config["output_dtype"],
            "truncation": True  # Enable truncation for over-length inputs (2025 best practice)
        }
        ...
        # Generate main embeddings
        client = get_voyage_client()
        response = client.embed(content_list, **embed_params)
        embeddings = response.embeddings

        # Generate binary embeddings if requested (for binary rescoring)
        binary_embeddings = None
        if config.get("generate_binary", False):
            print(f"      🔧 Generating binary embeddings for rescoring...")
            binary_response = client.embed(content_list,
                                     model=config.get("model", "voyage-code-3"),  # CORRECTED: Use model from config
                                     input_type="document",
                                     output_dimension=256,  # Returns 32 bit-packed elements (256/8=32)
                                     output_dtype="binary",  # Voyage AI's offset binary format (int8)
                                     truncation=True)  # Enable truncation (2025 best practice)

            # Store raw binary embeddings - best practices applied during search
            binary_embeddings = binary_response.embeddings

        batch_time = time.time() - batch_start_time
        avg_time_per_chunk = batch_time / len(content_list) if content_list else 0

        binary_info = f" + binary" if binary_embeddings else ""
        print(f"      ✅ Batch completed in {batch_time:.2f}s ({avg_time_per_chunk:.3f}s per chunk){binary_info}")

        # Handle different output types for main embeddings
        ...
        return cast(List[List[float]], embeddings), cast(Optional[List[List[int]]], binary_embeddings)
...
def handle_embedding_error(error: Exception,
    ...
    if "rate limit" in error_str:
        print("   🕐 Rate limit detected. Implementing enhanced exponential backoff...")

        # Enhanced exponential backoff with jitter (2025 best practices)
        import random
        base_delay = 1  # Start with 1 second
        max_delay = 60  # Cap at 60 seconds

        for attempt in range(6):  # Increased attempts for better resilience
            # Exponential backoff with jitter to avoid thundering herd
            delay = min(base_delay * (2 ** attempt), max_delay)
            jitter = random.uniform(0.1, 0.3) * delay  # 10-30% jitter
            wait_time = delay + jitter

            print(f"   ⏳ Waiting {wait_time:.1f}s before retry attempt {attempt + 1}/6...")
            time.sleep(wait_time)

            try:
                # Test with a small sample first
                test_content = batch_content[:1]
                get_voyage_client().embed(test_content,
                        model="voyage-code-3",
                        input_type="document",
                        output_dtype=config["output_dtype"],
                        truncation=True)
                print(f"   ✅ Rate limit cleared after {wait_time:.1f}s wait")
                return True
            except Exception as retry_error:
                if attempt == 5:  # Last attempt (0-indexed)
                    print(f"   ❌ Rate limit retry failed after 6 attempts: {retry_error}")
                    return False
                continue
        ...
        try:
            get_voyage_client().embed(batch_content[:1],
                    model="voyage-code-3",
                    input_type="document",
                    output_dtype="float",
                    truncation=True)
...
# Global variables moved to main() function to avoid module-level execution

def main() -> None:
    """Main function to generate embeddings when run as script."""
    # Load configuration to get project root and exclusions
    import json
    config_path = os.path.join(os.path.dirname(__file__), "../config.json")
    with open(config_path, "r") as f:
        config = json.load(f)

    # Get project root from configuration (relative to voyage-search directory)
    voyage_search_dir = os.path.dirname(os.path.dirname(__file__))
    project_root = os.path.abspath(os.path.join(voyage_search_dir, config["project_root"]))

    # Get exclusions from configuration
    exclude_dirs = set(config["exclude_dirs"])
    exclude_files = set(config["exclude_files"])

    print("🔍 Scanning Python files across entire project and extracting code chunks...")
    ...
    # Read all .py files in the entire project and parse them into chunks
    for root, dirs, files in os.walk(project_root):
        # Filter out excluded directories
        dirs[:] = [d for d in dirs if d not in exclude_dirs and not d.startswith('.')]
        for file in files:
            # Skip excluded file patterns
            if any(pattern in file for pattern in exclude_files):
                continue

            if file.endswith(".py"):
                file_path = os.path.join(root, file)

                # Skip test files and temporary files for cleaner embeddings
                if any(skip_pattern in file.lower() for skip_pattern in ['test_', '_test.py', '.tmp', '.temp']):
                    print(f"⚠️  Skipping test/temp file: {file_path}")
    ...
    # Generate embeddings with configurable parameters (Phase 3.0: Contextualized Embeddings)
    embedding_config = get_embedding_config("contextualized_high")  # Use contextualized embeddings (2025 best practice)
    print(f"📊 Using embedding configuration: {embedding_config.get('description', embedding_config.get('name', 'unknown'))}")
    print(f"   Model: {embedding_config.get('model', 'voyage-code-3')}")
    print(f"   Contextualized model: {embedding_config.get('contextualized_model', 'N/A')}")
    print(f"   Dimensions: {embedding_config['output_dimension']}")
    print(f"   Data type: {embedding_config['output_dtype']}")
    print(f"   Binary rescoring: {'✅ Enabled' if embedding_config.get('generate_binary', False) else '❌ Disabled'}")
    print(f"   Contextualized embeddings: {'✅ Enabled' if embedding_config.get('use_contextualized', False) else '❌ Disabled'}")
    print(f"   Token counting: {'✅ Enabled' if embedding_config.get('enable_token_counting', False) else '❌ Disabled'}")

    all_embeddings: List[List[float]] = []
    all_chunks_processed: List[CodeChunk] = []
    binary_embeddings: List[List[int]] = []  # For binary rescoring

    # Check if we should use contextualized embeddings
    if embedding_config.get('use_contextualized', False):
        print(f"🧠 Using contextualized embeddings for better code understanding...")

        # Group chunks by file for contextualized embeddings
        ...
        print(f"📁 Grouped {len(processed_chunks)} chunks from {len(chunks_by_file)} files")

        # Generate contextualized embeddings
        all_embeddings, binary_embeddings_result = generate_contextualized_embeddings_with_config(
            chunks_by_file,
            embedding_config,
            1,
            1
        )
        if binary_embeddings_result is not None:
            binary_embeddings = binary_embeddings_result
        all_chunks_processed = processed_chunks

    else:
        # Use regular embeddings (legacy approach)
        print(f"📦 Using regular embeddings...")
        ...
        for batch_idx, (batch_content, batch_chunks) in enumerate(batches):
            ...
            try:
                ...
                batch_embeddings, batch_binary_embeddings = generate_embeddings_with_config(
                    batch_content,
                    embedding_config,
                    batch_idx + 1,
                    len(batches)
                ...
                # Store binary embeddings if generated
                if batch_binary_embeddings is not None:
                    if 'binary_embeddings' not in locals():
                        binary_embeddings = []
                    binary_embeddings.extend(batch_binary_embeddings)
    ...
    # Save embeddings to configured location within voyage-search directory
    embeddings_path = os.path.join(voyage_search_dir, config["embeddings_file"])
    ...
    try:
        ...
        save_data = {
            ...
            "generation_timestamp": time.time(),
            "version": "2.2",  # Version for binary rescoring features

            # Binary rescoring support (Phase 2.2)
            "binary_embeddings": binary_embeddings if binary_embeddings else None,
            "binary_rescoring_enabled": embedding_config.get("generate_binary", False)
        }

        # Ensure module references are correct for pickle compatibility
        # Force the current module to be 'generate_embeddings' instead of '__main__'
        current_module = sys.modules.get(__name__)
        if current_module and __name__ == '__main__':
            # Temporarily add this module as 'generate_embeddings' for pickle
            sys.modules['generate_embeddings'] = current_module
