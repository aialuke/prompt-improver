...
class BusinessMetricsValidator:
    ...
    async def validate_dashboard_export(self) -> bool:
        ...
        try:
            ...
            # Test ML performance export
            ml_performance = await dashboard_exporter.export_ml_performance(
                time_range=TimeRange.LAST_HOUR,
                export_format=ExportFormat.JSON
            )
            ...
            # Test real-time monitoring export
            real_time_data = await dashboard_exporter.export_real_time_monitoring(
                export_format=ExportFormat.JSON
            )
            ...
            self.validation_results["test_results"][test_name] = {
                ...
                "executive_summary_keys": list(executive_summary.keys()) if isinstance(executive_summary, dict) else [],
                "ml_performance_keys": list(ml_performance.keys()) if isinstance(ml_performance, dict) else [],
                "real_time_keys": list(real_time_data.keys()) if isinstance(real_time_data, dict) else []
            }
            self.validation_results["tests_passed"] += 1
            logger.info(f"✅ {test_name}: PASSED")
            return True
    ...
    async def validate_instrumentation_decorators(self) -> bool:
        ...
        try:
            ...
            async def test_ml_function(prompt: str, user_id: str = None):
            ...
            async def test_feature_function(data: dict, user_id: str = None):
            ...
            async def test_cost_function(user_id: str = None):
                await asyncio.sleep(0.02)  # Simulate processing
                return {"status": "completed"}
            
            # Test the decorated functions
            ml_result = await test_ml_function(
                prompt="Test prompt for decorator validation",
                user_id="test_user_decorator"
            )
            
            feature_result = await test_feature_function(
                data={"test": "data"},
                user_id="test_user_decorator"
            )
            
            cost_result = await test_cost_function(user_id="test_user_decorator")
