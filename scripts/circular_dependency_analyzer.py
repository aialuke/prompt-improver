...
class CircularDependencyAnalyzer:
    ...
    def analyze_import_patterns(self) -> Dict[str, Any]:
        ...
        analysis = {'total_modules': len(self.modules), 'total_dependencies': sum((len(deps) for deps in self.dependencies.values())), 'modules_with_most_imports': [], 'most_imported_modules': [], 'potential_circular_areas': []}
    ...
    def generate_resolution_strategy(self, cycles: List[List[str]]) -> Dict[str, any]:
        ...
        strategies = {'cycles_found': len(cycles), 'affected_modules': set(), 'resolution_steps': [], 'common_utilities_to_extract': [], 'interface_candidates': []}
        for cycle in cycles:
        ...
        if cycles:
            strategies['resolution_steps'] = ['1. Create prompt_improver.common package for shared utilities', '2. Move cross-cutting concerns to common package', '3. Extract interfaces for frequently imported modules', '4. Use dependency injection for complex dependencies', '5. Implement lazy imports where appropriate', '6. Update import statements to use absolute imports']
            for module in frequent_modules:
                if 'manager' in module.lower():
                    strategies['interface_candidates'].append(f'{module} -> Extract manager protocol')
                elif 'service' in module.lower():
                    strategies['interface_candidates'].append(f'{module} -> Extract service interface')
                elif 'config' in module.lower():
                    strategies['interface_candidates'].append(f'{module} -> Move to common.config')
        return strategies

    def save_analysis_report(self, cycles: List[List[str]], analysis: Dict, strategies: Dict) -> str:
        """Save detailed analysis report."""
        report = {'timestamp': str(Path(__file__).stat().st_mtime), 'summary': {'total_modules_scanned': len(self.modules), 'circular_dependencies_found': len(cycles), 'affected_modules': len(strategies['affected_modules'])}, 'circular_dependencies': cycles, 'import_analysis': analysis, 'resolution_strategies': strategies, 'detailed_cycles': []}
        for i, cycle in enumerate(cycles):
            cycle_detail = {'cycle_id': i + 1, 'modules': cycle, 'cycle_length': len(cycle) - 1, 'severity': 'HIGH' if len(cycle) > 4 else 'MEDIUM' if len(cycle) > 2 else 'LOW'}
    ...
    def print_analysis_summary(self, cycles: List[List[str]], analysis: Dict, strategies: Dict) -> None:
        ...
        if cycles:
            ...
            for i, cycle in enumerate(cycles, 1):
                ...
                severity = 'HIGH' if len(cycle) > 4 else 'MEDIUM' if len(cycle) > 2 else 'LOW'
                print(f'   Severity: {severity} (length: {len(cycle) - 1})')
                print()
        else:
            print(f'\n✅ NO CIRCULAR DEPENDENCIES FOUND!')
        print(f'\n📈 IMPORT PATTERNS:')
        print(f'   Modules with most imports:')
        ...
        if strategies['common_utilities_to_extract']:
            ...
            print(f'   Modules to refactor (appear in multiple cycles):')
