...
class ProductionToolsInstaller:
    ...
    def __init__(self):
        ...
        self.tools_status = {}
        self.versions = {'k6': '0.48.0', 'prometheus': '2.48.1', 'grafana': '10.2.3', 'safety': 'latest', 'bandit': 'latest'}
    ...
    async def _check_existing_tools(self) -> None:
        ...
        tools_to_check = {'k6': ['k6', 'version'], 'prometheus': ['prometheus', '--version'], 'grafana-server': ['grafana-server', '--version'], 'safety': ['safety', '--version'], 'bandit': ['bandit', '--version']}
    ...
    async def _install_python_tools(self) -> None:
        ...
        for tool in python_tools:
            try:
                ...
                result = subprocess.run([sys.executable, '-m', 'pip', 'install', '--upgrade', tool], capture_output=True, text=True, timeout=300)
                if result.returncode == 0:
    ...
    async def _install_k6_macos(self) -> None:
        """Install k6 on macOS"""
        try:
            result = subprocess.run(['brew', 'install', 'k6'], capture_output=True, text=True, timeout=300)
            if result.returncode == 0:
    ...
    async def _install_k6_linux(self) -> None:
        """Install k6 on Linux"""
        package_managers = [(['apt', 'update'], ['apt', 'install', '-y', 'k6']), (['yum', 'update'], ['yum', 'install', '-y', 'k6']), (['dnf', 'update'], ['dnf', 'install', '-y', 'k6'])]
    ...
    async def _create_prometheus_docker_config(self) -> None:
        ...
        prometheus_config = {'global': {'scrape_interval': '15s', 'evaluation_interval': '15s'}, 'scrape_configs': [{'job_name': 'apes-application', 'static_configs': [{'targets': ['localhost:8080']}], 'metrics_path': '/metrics', 'scrape_interval': '5s'}]}
    ...
    async def _create_grafana_docker_config(self) -> None:
        ...
        grafana_config = {'server': {'http_port': 3000, 'domain': 'localhost'}, 'security': {'admin_user': 'admin', 'admin_password': 'admin'}, 'datasources': [{'name': 'Prometheus', 'type': 'prometheus', 'url': 'http://localhost:9090', 'access': 'proxy', 'isDefault': True}]}
    ...
    async def _create_tool_configs(self) -> None:
        ...
        k6_config = {'options': {'stages': [{'duration': '2m', 'target': 20}, {'duration': '5m', 'target': 50}, {'duration': '2m', 'target': 100}, {'duration': '5m', 'target': 100}, {'duration': '2m', 'target': 0}], 'thresholds': {'http_req_duration': ['p(95)<200'], 'http_req_failed': ['rate<0.01']}}}
    ...
    async def _verify_installations(self) -> None:
        ...
        verification_commands = {'k6': ['k6', 'version'], 'safety': ['safety', '--version'], 'bandit': ['bandit', '--version'], 'pip-audit': ['pip-audit', '--version']}
