...
def get_status_emoji(status: str) -> str:
    ...
    emojis = {'IMPROVED': '🚀', 'BETTER': '✅', 'STABLE': '➡️', 'WORSE': '⚠️', 'DEGRADED': '❌', 'NEW': '🆕'}
...
def compare_system_metrics(baseline: Dict, current: Dict) -> None:
    ...
    metrics = [('cpu_percent', 'CPU Usage', True), ('memory_percent', 'Memory Usage', True), ('memory_used_gb', 'Memory Used (GB)', True), ('disk_percent', 'Disk Usage', True), ('process_count', 'Process Count', True)]
    for metric_key, metric_name, lower_is_better in metrics:
        if metric_key in b_sys and metric_key in c_sys:
            status, change, interpretation = compare_metric(b_sys[metric_key], c_sys[metric_key], metric_name, lower_is_better)
            emoji = get_status_emoji(status)
            print(f'{emoji} {metric_name:15} {b_sys[metric_key]:8.1f} → {c_sys[metric_key]:8.1f} ({change:+.1f}%)')
...
def compare_ml_metrics(baseline: Dict, current: Dict) -> None:
    ...
    for model_name in sorted(all_models):
        ...
        if model_name in b_models and model_name in c_models:
            ...
            metrics = [('initialization_time_ms', 'Init Time', True), ('inference_time_ms', 'Inference Time', True), ('memory_usage_mb', 'Memory Usage', True), ('features_extracted', 'Features', False)]
            for metric_key, metric_name, lower_is_better in metrics:
                if metric_key in b_model and metric_key in c_model:
                    status, change, _ = compare_metric(b_model[metric_key], c_model[metric_key], metric_name, lower_is_better)
                    emoji = get_status_emoji(status)
                    print(f'     {emoji} {metric_name:12} {b_model[metric_key]:8.2f} → {c_model[metric_key]:8.2f} ({change:+.1f}%)')
...
def compare_cpu_benchmarks(baseline: Dict, current: Dict) -> None:
    ...
    benchmarks = [('prime_calculation', 'execution_time_ms', 'Prime Calculation'), ('string_processing', 'execution_time_ms', 'String Processing'), ('json_processing', 'execution_time_ms', 'JSON Processing')]
    for bench_key, metric_key, bench_name in benchmarks:
        if bench_key in b_cpu and bench_key in c_cpu:
            ...
            if b_val > 0 and c_val > 0:
                ...
                emoji = get_status_emoji(status)
                print(f'{emoji} {bench_name:18} {b_val:8.2f}ms → {c_val:8.2f}ms ({change:+.1f}%)')
...
def compare_io_benchmarks(baseline: Dict, current: Dict) -> None:
    ...
    io_metrics = [('file_write', 'write_speed_mb_per_sec', 'Write Speed (MB/s)', False), ('file_read', 'read_speed_mb_per_sec', 'Read Speed (MB/s)', False), ('directory_operations', 'execution_time_ms', 'Directory Ops (ms)', True)]
    for io_key, metric_key, metric_name, lower_is_better in io_metrics:
        if io_key in b_io and io_key in c_io:
            ...
            if b_val > 0 and c_val > 0:
                ...
                print(f'{emoji} {metric_name:18} {b_val:8.2f}{unit} → {c_val:8.2f}{unit} ({change:+.1f}%)')
...
def generate_summary(baseline: Dict, current: Dict) -> None:
    ...
    if 'cpu_utilization_percent' in b_summary and 'cpu_utilization_percent' in c_summary:
        cpu_status, cpu_change, _ = compare_metric(b_summary['cpu_utilization_percent'], c_summary['cpu_utilization_percent'], 'CPU', True)
    ...
    if 'total_memory_used_mb' in b_summary and 'total_memory_used_mb' in c_summary:
        mem_status, mem_change, _ = compare_metric(b_summary['total_memory_used_mb'], c_summary['total_memory_used_mb'], 'Memory', True)
    ...
    if 'benchmark_duration_seconds' in b_summary and 'benchmark_duration_seconds' in c_summary:
        duration_status, duration_change, _ = compare_metric(b_summary['benchmark_duration_seconds'], c_summary['benchmark_duration_seconds'], 'Benchmark Duration', True)
