...
class DatabaseMetrics:
    ...
    insert_time_ms: float | None = None
...
class BaselineReport:
    ...
    redis_metrics: RedisMetrics | None
...
class PerformanceBenchmark:
    ...
    async def benchmark_database(self) -> DatabaseMetrics | None:
    ...
    async def benchmark_redis(self) -> RedisMetrics | None:
    ...
    async def benchmark_api_endpoints(
        ...
        print(f"📊 Benchmarking API endpoints at {base_url}...")
    ...
    async def benchmark_cpu_intensive_tasks(self) -> dict[str, Any]:
        ...
        try:
            ...
            for i in range(1000):
                text = (
                    f"This is sample text number {i} for benchmarking string processing performance. "
                    * 10
                )
    ...
    async def benchmark_io_operations(self) -> dict[str, Any]:
        ...
        try:
            ...
            results["file_write"] = {
                ...
                "write_speed_mb_per_sec": round(
                    file_size / (1024 * 1024) / (write_time / 1000), 2
                ),
            }
            start_time = time.time()
            ...
            results["file_read"] = {
                ...
                "read_speed_mb_per_sec": round(
                    len(content) / (1024 * 1024) / (read_time / 1000), 2
                ),
            }
            start_time = time.time()
    ...
    def calculate_summary_statistics(
        self, metrics_list: list[Any], field: str
    ) -> dict[str, float]:
    ...
    async def run_comprehensive_benchmark(self) -> BaselineReport:
        ...
        if api_metrics:
            summary["api_response_times"] = self.calculate_summary_statistics(
                api_metrics, "response_time_ms"
            )
        if ml_metrics:
            summary["ml_inference_times"] = self.calculate_summary_statistics(
                ml_metrics, "inference_time_ms"
            )
            summary["ml_initialization_times"] = self.calculate_summary_statistics(
                ml_metrics, "initialization_time_ms"
            )
        if database_metrics:
        ...
        if redis_metrics:
            summary["redis_performance"] = {
                ...
                "avg_operation_time_ms": round(
                    (
                        redis_metrics.set_time_ms
                        + redis_metrics.get_time_ms
                        + redis_metrics.delete_time_ms
                    )
                    / 3,
                    2,
                ),
        ...
        if cpu_benchmarks:
            summary["cpu_performance"] = {
                "prime_calculation_ms": cpu_benchmarks.get("prime_calculation", {}).get(
                    "execution_time_ms", 0
                ),
                "string_processing_ms": cpu_benchmarks.get("string_processing", {}).get(
                    "execution_time_ms", 0
                ),
                "json_processing_ms": cpu_benchmarks.get("json_processing", {}).get(
                    "execution_time_ms", 0
                ),
            }
        if io_benchmarks:
            summary["io_performance"] = {
                "file_write_speed_mb_per_sec": io_benchmarks.get("file_write", {}).get(
                    "write_speed_mb_per_sec", 0
                ),
                "file_read_speed_mb_per_sec": io_benchmarks.get("file_read", {}).get(
                    "read_speed_mb_per_sec", 0
                ),
                "directory_ops_ms": io_benchmarks.get("directory_operations", {}).get(
                    "execution_time_ms", 0
                ),
            }
...
def save_baseline_report(report: BaselineReport, output_path: str = None):
    ...
    if output_path is None:
        ...
        output_path = (
            f"/Users/lukemckenzie/prompt-improver/performance_baseline_{timestamp}.json"
        )
...
def print_baseline_summary(report: BaselineReport):
    ...
    print(
        f"   Memory Usage:     {report.system_metrics.memory_percent}% ({report.system_metrics.memory_used_gb}GB)"
    )
    print(
        f"   Disk Usage:       {report.system_metrics.disk_percent}% ({report.system_metrics.disk_free_gb}GB free)"
    )
    print(f"   Process Count:    {report.system_metrics.process_count}")
    if report.database_metrics:
        print("\n🗄️  DATABASE PERFORMANCE")
        print(
            f"   Connection Time:  {report.database_metrics.connection_time_ms:.2f}ms"
        )
        print(f"   Query Time:       {report.database_metrics.query_time_ms:.2f}ms")
        print(
            f"   Transaction Time: {report.database_metrics.transaction_time_ms:.2f}ms"
        )
        if report.database_metrics.select_time_ms:
            print(
                f"   Complex Query:    {report.database_metrics.select_time_ms:.2f}ms"
            )
    ...
    if report.api_metrics:
        ...
        if successful_apis:
            avg_response_time = statistics.mean([
                m.response_time_ms for m in successful_apis
            ])
            print(f"   Average Response: {avg_response_time:.2f}ms")
            print(
                f"   Successful Calls: {len(successful_apis)}/{len(report.api_metrics)}"
            )
            for api in successful_apis:
                print(
                    f"   {api.endpoint:15} {api.response_time_ms:6.2f}ms ({api.status_code})"
                )
    ...
    if report.cpu_benchmarks:
        ...
        if "prime_calculation" in report.cpu_benchmarks:
            ...
            print(
                f"   Prime Calculation: {pc['execution_time_ms']:.2f}ms ({pc['primes_found']} primes)"
            )
        if "string_processing" in report.cpu_benchmarks:
            sp = report.cpu_benchmarks["string_processing"]
            print(
                f"   String Processing: {sp['execution_time_ms']:.2f}ms ({sp['strings_processed']} strings)"
            )
        if "json_processing" in report.cpu_benchmarks:
            jp = report.cpu_benchmarks["json_processing"]
            print(
                f"   JSON Processing:   {jp['execution_time_ms']:.2f}ms ({jp['objects_processed']} objects)"
            )
    if report.io_benchmarks:
        ...
        if "file_write" in report.io_benchmarks:
            ...
            print(
                f"   File Write:       {fw['execution_time_ms']:.2f}ms ({fw['write_speed_mb_per_sec']:.2f} MB/s)"
            )
        if "file_read" in report.io_benchmarks:
            fr = report.io_benchmarks["file_read"]
            print(
                f"   File Read:        {fr['execution_time_ms']:.2f}ms ({fr['read_speed_mb_per_sec']:.2f} MB/s)"
            )
        if "directory_operations" in report.io_benchmarks:
            do = report.io_benchmarks["directory_operations"]
            print(
                f"   Directory Ops:    {do['execution_time_ms']:.2f}ms ({do['files_created']} files)"
            )
    print("\n📈 PERFORMANCE TARGETS")
    print("   Target Response Time:    <200ms")
    print("   Target Memory Usage:     <1000MB")
    print("   Target CPU Usage:        <80%")
    print("   Target DB Query Time:    <50ms")
    print("\n✅ BASELINE ASSESSMENT")
...
async def main():
    ...
    try:
        ...
        print(
            f"⏱️  Total benchmark time: {report.summary['benchmark_duration_seconds']}s"
        )
        return output_path
    except Exception as e:
