...
def main():
    ...
    if pyproject_file.exists():
        ...
        for group_deps in optional_deps.values():
            for dep in group_deps:
                pkg_name = dep.split('>=')[0].split('==')[0].split('<=')[0].split('~=')[0].split('[')[0]
                all_packages.add(pkg_name.strip('"').strip("'"))
    known_unused = {'adal', 'azure-common', 'azure-core', 'azure-graphrbac', 'azure-mgmt-authorization', 'azure-mgmt-containerregistry', 'azure-mgmt-core', 'azure-mgmt-keyvault', 'azure-mgmt-network', 'azure-mgmt-resource', 'azure-mgmt-storage', 'azureml-core', 'boto3', 'botocore', 'google-api-core', 'google-auth', 'google-cloud-core', 'google-cloud-storage', 'google-crc32c', 'google-resumable-media', 'googleapis-common-protos', 'pip-audit', 'pipdeptree', 'pipreqs', 'radon', 'knack', 'mando', 'docopt', 'yarg'}
    unused_packages = []
    potentially_unused = []
    for pkg in all_packages:
        ...
        if pkg in known_unused:
            ...
            if pkg_normalized in {'setuptools', 'wheel', 'pip', 'packaging', 'certifi', 'urllib3', 'charset_normalizer', 'idna', 'requests', 'typing_extensions', 'zipp', 'importlib_metadata'}:
                continue
