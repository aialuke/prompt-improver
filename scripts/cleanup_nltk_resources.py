...
def analyze_nltk_resources() -> Dict[str, any]:
    ...
    # Essential English resources to keep
    essential_english = {
        'tokenizers/punkt',
...
def _analyze_tokenizers(tokenizers_path: Path, essential: set) -> Dict[str, any]:
    ...
    # Check punkt_tab for non-English languages
...
def _analyze_corpora(corpora_path: Path, essential: set) -> Dict[str, any]:
    ...
    # Check stopwords for non-English languages
    ...
    # Check other large corpora that might not be needed
    ...
    for corpus_name in large_corpora:
        ...
        if corpus_path.exists():
            ...
            if corpus_size_mb > 1:  # Only suggest removal if > 1MB
...
def cleanup_non_english_resources(dry_run: bool = True) -> Dict[str, any]:
    ...
    # Remove non-English punkt tokenizers
...
def verify_english_resources() -> Dict[str, bool]:
    ...
    try:
        ...
        verification['all_essential_available'] = all([
            verification['punkt_tokenizer'],
            verification['english_stopwords'],
            verification['wordnet']
        ])
...
def main():
    ...
    if analysis['non_english_languages']:
        ...
        for lang in analysis['non_english_languages'][:10]:  # Show first 10
            print(f"    • {lang}")
    ...
    # Verify English resources still work
