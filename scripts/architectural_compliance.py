...
from analyze_dependencies import DependencyAnalyzer, ArchitecturalLayers
...
class ComplianceRule:
    ...
    check_function: str  # Name of the function to call
...
class ArchitecturalComplianceChecker:
    ...
    def check_circular_dependencies(self, parameters: Dict[str, Any]) -> List[ComplianceViolation]:
        ...
        for cycle in cycles:
            violations.append(ComplianceViolation(
                ...
                file_path="",  # Multiple files involved
                line_number=None,
                details={"cycle": cycle}
            ))
    ...
    def check_interface_protocols(self, parameters: Dict[str, Any]) -> List[ComplianceViolation]:
        ...
        # Find modules that should have interfaces
            ...
            if len(dependents) >= min_dependents:
                ...
                if not protocol_path:
                    violations.append(ComplianceViolation(
                        ...
                        file_path=module,
                        line_number=None,
                        details={"dependent_count": len(dependents), "dependents": list(dependents)}
                    ))
        ...
        for module, deps in self.dependency_analyzer.dependencies.items():
            if '.security.' in module:
                ...
                for dep in deps:
                    ...
                    # Security modules should not depend on application or interface layers
    ...
    def check_test_isolation(self, parameters: Dict[str, Any]) -> List[ComplianceViolation]:
        ...
        for module, dependents in self.dependency_analyzer.reverse_dependencies.items():
            if '.test_' in module or 'tests.' in module:
                # Test modules should not be imported by non-test code
                non_test_dependents = [dep for dep in dependents 
                ...
                if non_test_dependents:
                    violations.append(ComplianceViolation(
                        ...
                        severity="error",
                        message=f"Test module imported by non-test code",
                        file_path=module,
                        line_number=None,
    ...
    def check_utility_stability(self, parameters: Dict[str, Any]) -> List[ComplianceViolation]:
        ...
        for module, deps in self.dependency_analyzer.dependencies.items():
            if '.utils.' in module:
                # Count external dependencies (non-utils, non-core)
                external_deps = [dep for dep in deps 
                               if not (dep.startswith('prompt_improver.utils') or 
                                     dep.startswith('prompt_improver.core'))]
                ...
                if len(external_deps) > max_external_deps:
                    violations.append(ComplianceViolation(
                        ...
                        file_path=module,
                        line_number=None,
                        details={"external_dependencies": external_deps, "limit": max_external_deps}
                    ))
    ...
    def run_all_checks(self) -> ComplianceReport:
        ...
        # Score calculation: errors are more heavily weighted
...
def main():
    ...
    parser.add_argument('--min-score', type=float, default=80.0,
                        help='Minimum compliance score (0-100)')
    
    args = parser.parse_args()
    
    # Generate rules config if requested
