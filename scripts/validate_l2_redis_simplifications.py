...
class L2RedisValidationRunner:
    ...
    def check_docker_availability(self) -> bool:
        ...
        try:
            ...
            logger.exception("❌ Docker command not found. Please install Docker.")
            return False
        except subprocess.TimeoutExpired:
            logger.exception("❌ Docker check timed out. Docker may be unresponsive.")
            return False
        except Exception as e:
            logger.exception(f"❌ Unexpected error checking Docker: {e}")
            return False
    ...
    def check_dependencies(self) -> bool:
        ...
        for package in required_packages:
            try:
                ...
                logger.exception(f"❌ {package} is not available")
    ...
    async def run_basic_functionality_validation(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"❌ Basic functionality validation failed: {e}")
            return {"error": str(e)}
    ...
    async def run_testcontainer_validation(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"❌ Testcontainer validation failed: {e}")
            return {"error": str(e)}

    async def _test_basic_operations(self, service: L2RedisService, results: dict[str, Any]) -> None:
    ...
    async def _test_close_method(self, service: L2RedisService, results: dict[str, Any]) -> None:
    ...
    async def _test_performance_tracking(self, service: L2RedisService, results: dict[str, Any]) -> None:
    ...
    async def _test_connection_management(self, service: L2RedisService, fixture: Any, results: dict[str, Any]) -> None:
    ...
    async def _test_performance_targets(self, service: L2RedisService, fixture: Any, results: dict[str, Any]) -> None:
    ...
    async def _test_error_handling(self, service: L2RedisService, fixture: Any, results: dict[str, Any]) -> None:
    ...
    def generate_validation_report(self) -> str:
        ...
        # Summary
        total_tests = len([k for k in self.validation_results if not k.startswith("_")])
        passed_tests = len([
