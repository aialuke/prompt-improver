...
class MCPHealthMonitor:
    ...
    def __init__(self, config_path: Optional[str]=None):
        ...
        self.metrics_file = Path(self.config.get('metrics_file', PROJECT_ROOT / 'logs/mcp-health-metrics.json'))
        self.log_file = Path(self.config.get('log_file', PROJECT_ROOT / 'logs/mcp-health-monitor.log'))
    ...
    def _load_config(self, config_path: Optional[str]) -> Dict[str, Any]:
        ...
        default_config = {'monitor_interval': 30, 'metrics_history_size': 100, 'alert_cooldown_seconds': 300, 'auto_restart': True, 'max_restart_attempts': 3, 'restart_delay_seconds': 10, 'health_check_endpoint': None, 'notification_webhooks': [], 'thresholds': {}}
    ...
    async def _collect_metrics(self) -> HealthMetrics:
        ...
        return HealthMetrics(timestamp=time.time(), pid=self.process.pid, status=self.process.status(), memory_mb=memory_info.rss / 1024 / 1024, cpu_percent=cpu_percent, num_threads=num_threads, open_files=open_files, connections=connections, uptime_seconds=uptime_seconds, response_time_ms=response_time_ms, error_count=error_count, last_error=last_error)

    async def _test_response_time(self) -> Optional[float]:
    ...
    def _analyze_health(self, metrics: HealthMetrics) -> str:
        ...
        if metrics.memory_mb > self.thresholds.memory_mb_critical:
            issues.append(f'CRITICAL: Memory usage {metrics.memory_mb:.1f}MB exceeds critical threshold {self.thresholds.memory_mb_critical}MB')
        elif metrics.memory_mb > self.thresholds.memory_mb_warning:
            issues.append(f'WARNING: Memory usage {metrics.memory_mb:.1f}MB exceeds warning threshold {self.thresholds.memory_mb_warning}MB')
        if metrics.cpu_percent > self.thresholds.cpu_percent_critical:
            issues.append(f'CRITICAL: CPU usage {metrics.cpu_percent:.1f}% exceeds critical threshold {self.thresholds.cpu_percent_critical}%')
        elif metrics.cpu_percent > self.thresholds.cpu_percent_warning:
            issues.append(f'WARNING: CPU usage {metrics.cpu_percent:.1f}% exceeds warning threshold {self.thresholds.cpu_percent_warning}%')
        if metrics.response_time_ms:
            if metrics.response_time_ms > self.thresholds.response_time_ms_critical:
                issues.append(f'CRITICAL: Response time {metrics.response_time_ms:.1f}ms exceeds critical threshold {self.thresholds.response_time_ms_critical}ms')
            elif metrics.response_time_ms > self.thresholds.response_time_ms_warning:
                issues.append(f'WARNING: Response time {metrics.response_time_ms:.1f}ms exceeds warning threshold {self.thresholds.response_time_ms_warning}ms')
        ...
        if metrics.uptime_seconds < self.thresholds.uptime_minimum_seconds:
            issues.append(f'WARNING: Process uptime {metrics.uptime_seconds:.1f}s is less than minimum {self.thresholds.uptime_minimum_seconds}s')
    ...
    async def _handle_health_status(self, status: str, metrics: HealthMetrics):
        ...
        if status == 'healthy':
            self._clear_alerts()
            logger.debug('Health check passed - Memory: %sMB, CPU: %s%%', format(metrics.memory_mb, '.1f'), format(metrics.cpu_percent, '.1f'))
            return
        if status == 'warning':
            logger.warning('Health warning detected - Memory: %sMB, CPU: %s%%', format(metrics.memory_mb, '.1f'), format(metrics.cpu_percent, '.1f'))
        elif status == 'critical':
            logger.error('Critical health issue detected - Memory: %sMB, CPU: %s%%', format(metrics.memory_mb, '.1f'), format(metrics.cpu_percent, '.1f'))
    ...
    async def _attempt_restart(self):
        ...
        logger.info('Attempting to restart MCP server (attempt %s/%s)', self._restart_attempts, max_attempts)
        try:
            if self._is_systemd_service():
                result = subprocess.run(['sudo', 'systemctl', 'restart', 'mcp-server'], capture_output=True, text=True, timeout=30)
    ...
    def _is_systemd_service(self) -> bool:
        ...
        try:
            result = subprocess.run(['systemctl', 'is-active', 'mcp-server'], capture_output=True, text=True, timeout=5)
            return result.returncode == 0
        except:
            return False

    async def _send_alert(self, status: str, metrics: Optional[HealthMetrics], message: str=''):
        ...
        alert_data = {'timestamp': datetime.now().isoformat(), 'status': status, 'message': message, 'server': 'mcp-server', 'metrics': asdict(metrics) if metrics else None}
    ...
    def _store_metrics(self, metrics: HealthMetrics):
        ...
        try:
            metrics_data = {'latest': asdict(metrics), 'history_count': len(self.health_history), 'monitor_uptime': time.time() - self.start_time}
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {'status': 'healthy', 'latest_metrics': asdict(latest), 'uptime_seconds': latest.uptime_seconds, 'monitor_uptime': time.time() - self.start_time, 'history_points': len(self.health_history)}
...
async def main():
    ...
    parser.add_argument('--pid-file', help='PID file path', default='/var/run/mcp-server.pid')
    parser.add_argument('--metrics-file', help='Metrics output file', default='logs/mcp-health-metrics.json')
    parser.add_argument('--interval', type=int, help='Monitor interval in seconds', default=30)
    parser.add_argument('--status', action='store_true', help='Show current health status and exit')
    args = parser.parse_args()
    config = {'pid_file': args.pid_file, 'metrics_file': args.metrics_file, 'monitor_interval': args.interval}
