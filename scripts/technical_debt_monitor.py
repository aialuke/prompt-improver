...
class TechnicalDebtMonitor:
    ...
    async def run_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        results = {'timestamp': datetime.now().isoformat(), 'project_root': str(self.project_root), 'metrics': {}}
        analysis_tasks = [('code_quality', self.analyze_code_quality()), ('security', self.analyze_security()), ('dependencies', self.analyze_dependencies()), ('complexity', self.analyze_complexity()), ('test_coverage', self.analyze_test_coverage()), ('performance', self.analyze_performance_patterns())]
    ...
    async def analyze_code_quality(self) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(['ruff', 'check', '.', '--output-format=json', '--statistics'], cwd=self.project_root, capture_output=True, text=True)
            if result.stdout:
                violations = json.loads(result.stdout)
                critical_count = sum((1 for v in violations if v.get('code') in ['F401', 'F841', 'E722', 'S105', 'S106', 'S608']))
                return {'total_violations': len(violations), 'critical_violations': critical_count, 'fixable_violations': sum((1 for v in violations if v.get('fixable', False))), 'top_violations': self.get_top_violations(violations), 'status': 'success'}
            else:
    ...
    async def analyze_security(self) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(['pip-audit', '--format=json'], cwd=self.project_root, capture_output=True, text=True)
            if result.returncode == 0:
                ...
                return {'total_vulnerabilities': len(vulnerabilities), 'critical_vulnerabilities': sum((1 for v in vulnerabilities if 'CRITICAL' in v.get('description', ''))), 'vulnerabilities': vulnerabilities[:10], 'status': 'success'}
            else:
    ...
    async def analyze_dependencies(self) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(['pip', 'list', '--outdated', '--format=json'], cwd=self.project_root, capture_output=True, text=True)
            if result.returncode == 0:
                outdated = json.loads(result.stdout)
                return {'total_outdated': len(outdated), 'major_updates': sum((1 for pkg in outdated if self.is_major_update(pkg))), 'outdated_packages': outdated[:10], 'status': 'success'}
            else:
                return {'error': result.stderr, 'status': 'failed'}
        except Exception as e:
            return {'error': str(e), 'status': 'failed'}
            ...
            for file_path in src_files:
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        total_lines += lines
                        if lines > 1000:
                            large_files.append({'file': str(file_path.relative_to(self.project_root)), 'lines': lines})
                except Exception:
                    continue
            function_count = 0
            for file_path in src_files:
                try:
                    ...
                    continue
            return {'total_source_lines': total_lines, 'total_functions': function_count, 'large_files_count': len(large_files), 'large_files': large_files[:10], 'avg_lines_per_file': total_lines // len(src_files) if src_files else 0, 'status': 'success'}
        except Exception as e:
            return {'error': str(e), 'status': 'failed'}
    ...
    async def analyze_test_coverage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'test_files_count': len(test_files), 'source_files_count': len(src_files), 'test_lines': test_lines, 'source_lines': src_lines, 'coverage_ratio': round(coverage_ratio, 2), 'status': 'success'}
        except Exception as e:
            return {'error': str(e), 'status': 'failed'}
    ...
    async def analyze_performance_patterns(self) -> Dict[str, Any]:
        ...
        try:
            src_files = list(self.project_root.glob('src/**/*.py'))
            anti_patterns = {'blocking_io_in_async': 0, 'subprocess_without_check': 0, 'bare_except': 0, 'string_concatenation_in_loop': 0}
            for file_path in src_files:
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
                        content = f.read()
                    ...
                    continue
            return {'anti_patterns': anti_patterns, 'total_anti_patterns': sum(anti_patterns.values()), 'status': 'success'}
        except Exception as e:
            return {'error': str(e), 'status': 'failed'}
    ...
    def calculate_debt_score(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            weights = {'code_quality': 0.25, 'security': 0.2, 'dependencies': 0.15, 'complexity': 0.15, 'test_coverage': 0.15, 'performance': 0.1}
            scores = {}
            total_score = 0
            ...
            return {'overall_score': round(debt_score, 2), 'category_scores': scores, 'interpretation': self.interpret_debt_score(debt_score)}
        except Exception as e:
            return {'error': str(e)}
    ...
    def get_top_violations(self, violations: List[Dict]) -> List[Dict]:
        ...
        return sorted([{'code': code, 'count': count} for code, count in violation_counts.items()], key=lambda x: x['count'], reverse=True)[:10]
