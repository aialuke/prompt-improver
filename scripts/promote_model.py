...
def promote_model(model_name, model_version, to_stage):
    ...
    try:
        ...
        if current_model.version != model_version:
            print(f"Archiving version {current_model.version} currently in '{to_stage}'.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Promote an MLflow model to a new stage.")
    parser.add_argument("--model-name", required=True, help="The name of the registered model.")
    parser.add_argument("--model-version", required=True, type=int, help="The version of the model to promote.")
    parser.add_argument("--to-stage", required=True, choices=['Staging', 'Production'], help="The target stage.")
