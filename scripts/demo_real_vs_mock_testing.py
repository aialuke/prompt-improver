...
class RealDatabaseClient:
    ...
    async def execute_query(self, query: str, params: dict[str, Any] | None = None):
        ...
        if "INSERT" in query.upper() and params:
            ...
            return [
                {"id": len(self.real_data), "email": email, "name": params.get("name")}
            ]
...
async def performance_comparison():
    ...
    print(
        f"🎭 Mock Testing: 100 operations in {mock_duration:.3f}s (~{mock_duration * 10:.1f}ms per op)"
    )
    ...
    print(
        f"🔧 Real Testing: 100 operations in {real_duration:.3f}s (~{real_duration * 10:.1f}ms per op)"
    )

    # Analysis
    print("\n📊 Performance Analysis:")
    print(
        f"   - Real testing is ~{real_duration / mock_duration:.1f}x slower than mocks"
    )
    print(f"   - But real testing provides {100}x more confidence")
    print("   - Container startup: ~2-3s one-time cost for entire test suite")
    print("   - Real testing catches issues mocks miss: PRICELESS")
...
def print_recommendations():
    ...
    print("\n❌ DON'T: Use Mock Database as PostgreSQL Substitute")
