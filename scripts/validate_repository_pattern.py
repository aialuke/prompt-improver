...
class RepositoryPatternValidator:
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                sql_found.extend({
                        ...
                        } for pattern in self.compiled_patterns if pattern.search(line))
    ...
    def generate_report(self) -> str:
        ...
        # Show first 10
        report.extend(f"✓ {item['file']}:{item['line']} - {item['pattern']}" for item in repo_results['repository_files'][:10])
        ...
        if repo_results['violations']:
            # Show first 15
            report.extend(f"✗ {item['file']}:{item['line']} - {item['content'][:80]}..." for item in repo_results['violations'][:15])
        ...
        report.extend(f"✗ {problem_file}" for problem_file in service_problems)
        report.append("")
