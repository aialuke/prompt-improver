...
class ArchitecturalLayers:
    ...
    LAYERS = {'infrastructure': ['database', 'cache', 'security', 'utils'], 'core': ['core', 'shared'], 'domain': ['rule_engine', 'ml'], 'application': ['performance', 'monitoring', 'metrics'], 'interface': ['api', 'cli', 'tui', 'mcp_server'], 'external': ['feedback', 'dashboard']}
    ALLOWED_DEPENDENCIES = {'external': ['interface', 'application', 'domain', 'core', 'infrastructure'], 'interface': ['application', 'domain', 'core', 'infrastructure'], 'application': ['domain', 'core', 'infrastructure'], 'domain': ['core', 'infrastructure'], 'core': ['infrastructure'], 'infrastructure': []}
...
class DependencyAnalyzer:
        ...
        for module, deps in self.dependencies.items():
            ...
            for dep in deps:
                ...
                if dep_layer not in allowed_layers and dep_layer != module_layer:
                    violations.append({'from_module': module, 'from_layer': module_layer, 'to_module': dep, 'to_layer': dep_layer, 'violation_type': 'layer_breach', 'severity': self._calculate_violation_severity(module_layer, dep_layer)})
        return violations
    ...
    def identify_interface_opportunities(self) -> List[Dict[str, Any]]:
        ...
        for module, dependents in self.reverse_dependencies.items():
            if len(dependents) >= 3:
                opportunities.append({'module': module, 'dependent_count': len(dependents), 'dependents': list(dependents), 'recommendation': 'Consider creating interface/protocol for this module', 'potential_interface_name': f"I{module.split('.')[-1].title()}"})
    ...
    def analyze(self) -> DependencyMetrics:
        ...
        return DependencyMetrics(total_modules=len(self.dependencies), total_dependencies=sum((len(deps) for deps in self.dependencies.values())), circular_dependencies=circular_deps, high_coupling_modules=high_coupling, architectural_violations=architectural_violations, interface_opportunities=interface_opportunities)
    ...
    def generate_dependency_graph(self, output_path: Path, max_nodes: int=50):
        ...
        if not HAS_VISUALIZATION:
            ...
            return
        G = nx.DiGraph()
        top_modules = sorted(self.dependencies.items(), key=lambda x: len(x[1]) + len(self.reverse_dependencies[x[0]]), reverse=True)[:max_nodes]
        ...
        colors = {'infrastructure': 'lightblue', 'core': 'lightgreen', 'domain': 'yellow', 'application': 'orange', 'interface': 'red', 'external': 'purple', 'unknown': 'gray'}
        node_colors = [colors.get(self.module_to_layer.get(f'prompt_improver.{node}', 'unknown'), 'gray') for node in G.nodes()]
        nx.draw(G, pos, with_labels=True, node_color=node_colors, node_size=1000, font_size=8, arrows=True)
...
class RefactoringRecommendations:
    ...
    def generate_recommendations(metrics: DependencyMetrics) -> Dict[str, List[str]]:
        ...
        if metrics.circular_dependencies:
            recommendations['immediate'].extend([f"CRITICAL: Break circular dependency: {' -> '.join(cycle)}" for cycle in metrics.circular_dependencies])
        for module, count in metrics.high_coupling_modules[:5]:
            recommendations['short_term'].append(f'Reduce dependencies in {module} (currently {count}): Extract service interfaces and use dependency injection')
        critical_violations = [v for v in metrics.architectural_violations if v['severity'] == 'critical']
        if critical_violations:
            recommendations['long_term'].extend([f"Architectural violation: {v['from_module']} ({v['from_layer']}) should not depend on {v['to_module']} ({v['to_layer']})" for v in critical_violations[:3]])
        for opportunity in metrics.interface_opportunities[:3]:
            recommendations['long_term'].append(f"Create interface {opportunity['potential_interface_name']} for {opportunity['module']} (used by {opportunity['dependent_count']} modules)")
...
def main():
    ...
    parser.add_argument('--generate-graph', action='store_true', help='Generate dependency visualization')
    ...
    if args.format == 'json':
        report = {'timestamp': timestamp, 'metrics': asdict(metrics), 'recommendations': recommendations}
        ...
        with open(output_file, 'w') as f:
            ...
            if metrics.architectural_violations:
                ...
                for violation in metrics.architectural_violations[:10]:
                    f.write(f"  {violation['severity'].upper()}: {violation['from_module']} -> {violation['to_module']}\n")
                f.write('\n')
